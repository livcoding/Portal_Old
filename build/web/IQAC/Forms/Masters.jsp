<%-- 
    Document   : Masters
    Created on : Nov 4, 2014, 2:56:33 PM
    Author     : Mohit.sharma
--%>
...
<html>
    <head>
        ...
        <script src="../js/IQTest/MastersJs.js"></script>
        <script>
    ...
    <body >
      ...
      //  out.println("nipun"+mMemberID+">>>>>"+mMemberCode+">>>>>"+mMemberName);
        ...
        <form name="masterform" >
            <input type="hidden" id="menuid" name="menuid" value="<%=request.getParameter("menuid") != null ? request.getParameter("menuid").toString() : ""%>">
            <input type="hidden" id="compsession"  value="UNIV"/>
            <input type="hidden" id="instsession"  value="JIIT"/>
            <input type="hidden" id="companyid"  value="<%=mLoginComp%>"/>
            <input type="hidden" id="entryby"  value="<%=mChkMemID%>"/>
            <input type="hidden" id="entrydate"  value="to_date(sysdate,'dd-MM-RRRR HH:SS PM')"/>
            
            <!-- Above Is  to handle  the session values   -->
            <div id="windowheader" class="ui-widget-header ui-corner-all div" style="height:28px " ></div> 
            <div style="width: 99%; padding: 10px ;border: .3em solid;border-radius: 25px;" > 
                <table  id="commonmasterid" style="text-align: center;font-size: 18px; border:1px"></table>
            </div>
