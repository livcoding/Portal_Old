<%-- 
    Document   : ITServices
    Created on : 14 Sep, 2017, 11:08:19 AM
    Author     : VIVEK.SONI
--%>
...
<html>
    <head>
        ...
        <script src="../js/jquery/jquery.multiselect.js"></script>
        ...
        <script src="../js/IQTest/ITServices.js"></script>
        <script>
...
//                });
            ...
            document.getElementById("finalSaveForm").disabled = true;
        </script>
    </head><%
...
else
{
    ...
    <body >
      ...
      //  out.println("nipun"+mMemberID+">>>>>"+mMemberCode+">>>>>"+mMemberName);
         ...
         String qry = "Select WEBKIOSK.ShowLink('413','" + mChkMemID + "','" + mChkMType + "','" + mRole + "','" + mIPAddress + "') SL from dual";
         //out.print(qry);
        ...
        <form name="masterform" >
            ...
            <input type="hidden" id="dNameValue"  value="<%=departmentCode%>"/>
            <input type="hidden" id="entryBy"  value="<%=mChkMemID%>"/>
              <input type="hidden" id="subjective"  value=" "/>
            <!-- Above Is  to handle  the session values   -->
             <div id="windowheader" class="ui-widget-header ui-corner-all div" style="width: 100%;height:28px"><div style="width: 60%;float: left;text-align: right;font-size: large">IT Services <%=mInstitute%></div><div style="width: 35%;float: left;text-align: right;font-size: large">[Form:QA-SR-IT-1]</div></div>
            <div style="width: 99%; padding: 10px ;border: .3em solid;border-radius: 25px;" >
                <table  id="commonmasterid" style="text-align: center;font-size: 18px; border:1px">
                    ...
                    <tr>
                        <td style="text-align: right">Event Code<span class="req"> *</span> :</td><td style="text-align: left"><select  name='EventCode' id='EventCode'  class='combo' style=''  title='Event Code ' onchange='getQuestions()'><%=ccd.commonJspCombo("{\"comboId\":\"EventCombo\"}")%></select></td>
                       <td style="text-align: right">Department Name :</td><td style='text-align: left'><input type='text' name='departmentName' id='departmentName' value="<%=departmentNameDB%>" maxlength='160' class='textbox'  title='Department Name' readonly/></td>
                    </tr>
                    ...
                    <tr>
                     ...
                     <td style="text-align: right">Remarks :</td><td style='text-align: left'><input type='text' name='remarks' id='headerremarks' value='' maxlength='99' class='textbox'  title='Remarks'/></td>
                    </tr>
                ...
                <input type="button" title='Save Record' id="DraftSaveForm" style="cursor:pointer;margin-left: 70%" onclick="formsubmit()" value="Draft" disabled>
                <input type="button" title='Save Record' id="finalSaveForm" style="cursor:pointer;margin-left: 0%" onclick="finalsubmit()" value="Final Save" disabled>
                <input type="button"  title='Reset Fields'  id="Reset" style="cursor:pointer;" onclick=" resetValues()" value="Reset">
                <input type="button" title='Exit Form'   id="exit" style="cursor:pointer;" onClick="history.go(-1);" value="Exit"></div>
            ...
            <div id="programNameGrid" title="Program Name">
