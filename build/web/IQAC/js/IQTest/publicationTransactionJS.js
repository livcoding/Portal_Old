...
$(document).ajaxComplete(function(event, xhr, settings) {
    switch (settings.handller) {
        ...
        case 'selectAuthorInfo':
            ...
            break;
       case 'selectOtherInfo':
            selectOtherInfo = jQuery.parseJSON(xhr.responseText);
            getSelectOtherInfoInPopUp(selectOtherInfo);
            $('#TOTAL').html("Total No.of Record(s): " + selectOtherInfo[1].totalrecords);
            break;
           ...
           // getPublicationCombo(dataList[0]["publicationID"]);
            ...
            for (var key in childData) {
                ...
                for (var key1 in subrow) {
                    ...
                    if((subrow[key1]=='O' || subrow[key1]=='I') && key1!="staffType"){
...
function getOtherInfo(pno)
{
   // alert("OTHERS");
     var totIDS = 0;
    jData = {};
    if ($("#paggingPopUp").val() == "ALL") {
        jData.epg = selectOtherInfo[1].totalrecords;
    } else {
        jData.epg = (eval(pno) + eval($("#paggingPopUp").val()));
    }
    jData.spg = pno;
    ...
    jData.searchNames = $("#searchNames").val();
    jData.service = "PublicationTransactionServlet";
    jData.handller = "selectOtherInfo";
    $(document).getServace(jData);
}
...
function getSelectOtherInfoInPopUp(){
    var table = "";
    $("#popupHeader").html("");
    $("#popupHeader").html("<th style='width: 5%'>S No.</th><th style='width: 23%'>Author Code</th><th style='width: 23%'>Author Name</th><th style='width: 26%'>Department Name</th><th style='width: 23%'>Author Type</th>");

    for (var key in selectOtherInfo) {
        table = table + "<tr ondblclick='setNames("+key+")'  style='cursor:pointer;'><td  style='width: 5%;text-align: left'>" + selectOtherInfo[key]["sno"] + "</td><td  style='width: 23%;text-align: left'>" + selectOtherInfo[key]["employeeCode"] + "</td><td  style='width: 23%;text-align: left'>" + selectOtherInfo[key]["employeename"] + "</td><td  style='width: 26%;text-align: left'>" + selectOtherInfo[key]["departmentName"] + "</td><td  style='width: 23%;text-align: left'>" + getEmployeeType(selectOtherInfo[key]["employeeType"]) + "</td></tr>";
     //  table = table + "<tr ondblclick='setNames(" + key + ")'  style='cursor:pointer;'><td  style='width: 5%;text-align: left'>" + selectOtherInfo[key]["sno"] + "</td><td  style='width: 47%;text-align: left'>" + selectOtherInfo[key]["employeeCode"] + "</td><td  style='width: 48%';text-align: left>" + selectOtherInfo[key]["employeename"] + "</td><td  style='width: 26%;text-align: left'>" + selectAuthorInfo[key]["departmentName"] + "</td><td  style='width: 23%;text-align: left'>" + getEmployeeType(selectAuthorInfo[key]["employeeType"]) + "</td></tr>";
    }
...
function setNames(key)
{
    ...
    $("#authorNames").dialog("close");
        ...
        {
  //  alert("qqqqqqqqqq");
    $("#staffID"+currRowNo).val(selectOtherInfo[key]["employeeID"]);
    $("#authorName"+currRowNo).val(selectOtherInfo[key]["employeename"]);
    $("#departmentCode"+currRowNo).val(selectOtherInfo[key]["departmentCode"]);
    $("#departmentName"+currRowNo).val(selectOtherInfo[key]["departmentName"]);
    $("#staffType"+currRowNo).val(selectOtherInfo[key]["employeeType"]);
    $("#authorType"+currRowNo).val(getEmployeeType(selectOtherInfo[key]["employeeType"]));
    $("#authorNames").dialog("close");
   // alert("qqqqqqqqqq");
    // alert  ($("#authorType").val(getEmployeeType(selectOtherInfo[key]["employeeType"])));
