...
<form name="frm" method=post>
		...
		qry="Select EnrollmentNo enrollmentno,Studentname Name, StudentID , nvl(Semester,0)Semester, nvl(ProgramCODE,'UNK') ProgramCode, nvl(BranchCode,'UNK') BranchCode,quota,AcademicYear From StudentMaster where studentid='"+mChkMemID+"' and InstiTuteCode='"+mInst+"'";
		//out.print(qry);
...
int mSupFees=0;
String mSupRegcode="";
qry="SELECT NVL(PREVEXAMCODE,' ')PREVEXAMCODE,nvl(SUPFEES,0)SUPFEES,NVL(SUPREGCODE,' ')SUPREGCODE  FROM EXAMMASTER where InstiTuteCode='"+mInst+"' and   examcode='"+SupExamCode+"' and NVL(LOCKEXAM,'N')='N'";
//out.println(qry);
rs=db.getRowset(qry);
if(rs.next())
{
	...
	if(!msubject.equals("N"))
	   {
		...
		/*qry="SELECT DISTINCT v.fstid fstid , a.enrollmentno en2, v.subjectid subjectid, v.subject, v.subjectcode,   v.semester semester,v.semestertype semestertype, b.grade, B.coursecreditpoint          FROM studentmaster a,                studentresultdetail b,                exammaster d,                v#studentltpdetail v                       WHERE a.institutecode ='"+ mInst +"'            AND a.studentid = b.studentid            AND a.studentid = v.studentid            AND b.studentid = v.studentid   and v.subjectid='"+msubject+"'         AND d.examcode = b.examcode            AND d.institutecode = a.institutecode    AND b.fstid = v.fstid            AND b.examcode = v.examcode            AND  B.examcode NOT in( '"+mPrevExmCode+"','"+SummerExamCode+"')   AND NVL(b.fail,'N')='Y' AND a.studentid =  '"+mChkMemID+"'";*/

qry="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,                v.subjectcode, nvl(v.semester,0)semester, b.grade, nvl(b.coursecreditpoint,0)coursecreditpoint,                v.ltp,V.FSTID,V.SEMESTERTYPE           FROM studentmaster a,                studentresult b,                exammaster d,                v#studentltpdetail v          WHERE  v.subjectid='"+msubject+"'   and   a.semester > 6            AND v.ltp IN ('L', 'P')            AND a.institutecode = '"+ mInst +"'           and a.STUDENTID IN ( SELECT c.STUDENTID FROM STUDENTSGPACGPA c  WHERE c.semester="+mMaxsem+" AND   c.institutecode = '"+ mInst +"'                  and a.INSTITUTECODE=c.institutecode and A.STUDENTID=c.STUDENTID AND c.CGPA >= "+mMinsem+"  AND c.studentid = '"+mChkMemID+"'        )      AND a.studentid = b.studentid            AND a.studentid = v.studentid            AND b.subjectid = v.subjectid            AND b.studentid = v.studentid            AND d.examcode = v.examcode            AND d.institutecode = v.institutecode            AND NVL (b.fail, 'N') = 'Y'            AND a.studentid = '"+mChkMemID+"'            AND NVL (a.deactive, 'N') = 'N'            AND (   v.subject NOT LIKE '%PROJECT%'                 OR v.subject LIKE '%PROJECT MANAGEMENT%'                )            AND v.subjectid NOT IN (                   SELECT subjectid                     FROM v#studentltpdetail                    WHERE institutecode = '"+ mInst +"'                        AND studentid = '"+mChkMemID+"'                      AND examcode IN ('"+mPrevExmCode+"'))  ";

	
	/*	
qry="SELECT  DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject, v.subjectcode,         v.semester, b.grade, B.coursecreditpoint ,v.LTP FROM    studentmaster a, studentresultDetail b, exammaster d, v#studentltpdetail v WHERE   a.SEMESTER      >6  and v.subjectid='"+msubject+"' and     v.LTP in ('L','P') and     a.institutecode ='"+ mInst +"'  AND     a.studentid     = b.studentid AND     a.studentid     = v.studentid AND     b.studentid     = v.studentid AND     b.fstid         = v.fstid AND     b.examcode      = v.examcode AND     b.examcode not in( '"+mPrevExmCode+"') And     d.ExamCode      = v.ExamCode And     d.InstituteCode = v.InstituteCode AND     NVL(b.fail,'N')='Y'  AND     a.studentid = '"+mChkMemID+"'  And     nvl(a.deactive,'N') = 'N'AND     (v.subject NOT LIKE '%PROJECT%' OR v.subject LIKE '%PROJECT MANAGEMENT%') AND     v.SUBJECTID NOT IN (  SELECT  SUBJECTID                             FROM    debarstudentdetail                             WHERE   NVL (debar, 'N')    = 'Y'                             AND     institutecode       ='"+ mInst +"'                            AND     examcode IN ( '"+mPrevExmCode+"')                             ) AND v.subjectid NOT IN ( SELECT   x.subjectid                              FROM    NRSTUDENTFAILSUBJECTS x                             WHERE   x.institutecode   = '"+ mInst +"'                            And     not exists(select 'y'                                                from  v#studentltpdetail y                                                where y.institutecode = a.institutecode                                                and   y.subjectid = v.subjectid                                                and   y.ltp =  v.LTP                                                and   y.studentid = a.studentid)                            AND     studentid       = a.StudentID) AND v.subjectid NOT IN ( SELECT    t.subjectid                                  FROM    studentresult t                                 WHERE   t.institutecode   ='"+ mInst +"'                                AND     t.SubjectID       = subjectid                                    AND     t.studentid       = a.StudentID                                and     Nvl(t.fail,'N')   <> 'Y')                             ";
//out.print(qry);
		*/
...
//		out.print(qry);
	...
	<hr>
	<p align=left>I Mr./Ms. <b><%=mSname%></b> hereby apply for the Special Supplementary Examination <B><%=SupExamCode%></B>. <br>I am enclosing a receipt of payment/demand draft of Rs. ................................... for .......................................... number of subjects @ Rs. <%=mSupFees%>/- per subject.
	<br><br>
...
<tr><td colspan=2 align=Right>Relationship with Student: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp; &nbsp;</td></tr>
	...
	<p Align=Left>Received Registration Fee for Special Supplementary Examination (<B><%=SupExamCode%></B>) and receipt of Payment/DD form.
	<br>
...
<br>
<br>
	<p align=left>Please retain this receipt as authority for the hostel students to avail of hostel facilities for the duration of special supplementary exam.
