...
<HTML>
<head>
<TITLE> <%=mHead%> [Second Supplimentary Registration Form]</TITLE>
...
<form name="frm" method=post>
			...
			<tr>
			<TD align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: arial"><u><b>SECOND SUPPLEMENTARY REGISTRATION FORM
			...
			<%
			qry="select distinct nvl(examcode,' ')examcode, EXAMPERIODFROM from exammaster where institutecode='"+mInst+"' AND" +
                    " (eXAMCODE LIKE '2015%' ) and nvl(deactive,'N')='N' and nvl(LOCKEXAM,'N')='N' and  " +
                    "trunc(sysdate) >= trunc(examperiodfrom) and trunc(sysdate) <=trunc(examperiodto)   order by examcode Desc";
			//out.println(qry);
			rs=db.getRowset(qry);
            
			%>
			<select name="exam" tabindex="0" id="exam" style="WIDTH: 200px" multiple readonly>
			<!--option selected value='N' > << Select Exam Code  >>  -->
			<%
			...
			else
			{
				...
				{
			   		mexamcode=rs.getString("examcode");
                    if(mexamcode.equals(request.getParameterValues("exam").toString().trim()))
					{
						QryExam=mexamcode;
						%>
					    	<!--option selected value=<%=mexamcode%>><%=mexamcode%></option-->
		...
		//	}
			...
			if(request.getParameter("x")!=null)
			{
                ...
                if(request.getParameterValues("exam")==null)
				{
					QryExam="";
				}
				else
				{
                    String QryExam1[]=request.getParameterValues("exam");
				for(int j=0;j<QryExam1.length;j++)
                {
              //  out.print(QryExam1[j]);
                QryExam2=QryExam2+"'"+QryExam1[j]+"',";
                }
                }
               QryExam2=QryExam2.substring(0,QryExam2.length()-1);
...
if(!QryExam.equals("N"))
		...
		qry="Select EnrollmentNo enrollmentno,Studentname Name, StudentID , Semester, nvl(ProgramCODE,'UNK') ProgramCode, " +
                "nvl(BranchCode,'UNK') BranchCode,quota From StudentMaster where studentid='"+mChkMemID+"' and InstiTuteCode='"+mInst+"'";
		//out.print(qry);
...
qry="SELECT nvl(SUMMEREXAMCODE,' ')SUMMEREXAMCODE, nvl(PREVEXAMCODE,' ')PREVEXAMCODE, nvl(SUPFEES,'')SUPFEES  " +
        "FROM EXAMMASTER where InstiTuteCode='"+mInst+"' and examcode in("+QryExam2+") and NVL(LOCKEXAM,'N')='N'";
//out.println(qry);
rs=db.getRowset(qry);
while(rs.next())
{
	mPrevExmCode=mPrevExmCode+"'"+rs.getString("PREVEXAMCODE")+"',";
	mSupFees=rs.getString("SUPFEES");
	mSumExam=mSumExam+"'"+rs.getString("SUMMEREXAMCODE")+"',";

	//out.print(mSumExam+" -- mSumExam");

}
 mPrevExmCode=mPrevExmCode.substring(0,mPrevExmCode.length()-1);
 mSumExam=mSumExam.substring(0,mSumExam.length()-1);
...
<form name="frm1" method=post action="RegistrationSaveJIIT2.jsp">
...
/*	qry="SELECT distinct a.enrollmentno en2,v.subjectid, v.subject, v.subjectcode, v.semester, b.grade,       e.coursecreditpoint, TO_CHAR (examperiodto, 'Month YYYY') exammonth  FROM studentmaster a,       studentresultdetail b,       exammaster d,       v#studentltpdetail v,       programsubjecttagging e WHERE a.institutecode =  '"+ mInst +"' and a.INSTITUTECODE=e.INSTITUTECODE and b.EXAMCODE=e.EXAMCODE   AND a.studentid = b.studentid   AND a.studentid = v.studentid   AND b.studentid = v.studentid   AND d.examcode = b.examcode   AND d.institutecode = a.institutecode   AND b.fstid = v.fstid   AND b.examcode = v.examcode   AND b.examcode = '"+mPrevExmCode+"'    AND b.grade LIKE '%F%'   AND a.studentid =  '"+mChkMemID+"'";*/
	...
	qry="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject, v.subjectcode,            " +
            "    v.semester, b.grade, B.coursecreditpoint,                TO_CHAR (examperiodto, 'Month YYYY') exammonth    " +
            "       FROM studentmaster a,                studentresultdetail b,                exammaster d,          " +
            "      v#studentltpdetail v                       WHERE a.institutecode ='"+ mInst +"'         " +
            "   AND a.studentid = b.studentid            AND a.studentid = v.studentid            AND b.studentid = v.studentid    " +
            "        AND d.examcode = b.examcode            AND d.institutecode = a.institutecode    AND b.fstid = v.fstid      " +
            "      AND b.examcode = v.examcode            AND  b.examcode in( "+mPrevExmCode+","+mSumExam+") " +
            "  AND NVL(b.fail,'N')='Y'  AND a.studentid =  '"+mChkMemID+"'  ";

if(	mInst.equals("JUIT"))
qry=qry+ "	AND  (v.subject NOT LIKE '%PROJECT%'            OR v.subject LIKE '%PROJECT MANAGEMENT%') " ;

      qry=qry+"  and  (v.SUBJECTID,v.STUDENTID ) not IN(SELECT SUBJECTID,STUDENTID FROM STUDENTRESULT WHERE NVL(FAIL,'N')='N' " +
              " and institutecode ='"+ mInst +"'  AND a.studentid =  '"+mChkMemID+"'   ) ";
	qry=qry+ " AND    SUBJECTID NOT IN (             SELECT SUBJECTID FROM debarstudentdetail WHERE (NVL (debar, 'N') = 'Y'" +
            " or NVL (UFM, 'N') = 'Y' or NVL(AbsentCase, 'N') = 'Y')  AND institutecode ='"+ mInst +"'   " +
            "          AND examcode IN ("+mPrevExmCode+") AND studentid ='"+mChkMemID+"'              )";
...
qry=qry+ "	AND  (v.subject NOT LIKE '%PROJECT%'            OR v.subject LIKE '%PROJECT MANAGEMENT%') " ;
	...
	qry=qry+ " AND    SUBJECTID NOT IN (             SELECT SUBJECTID FROM debarstudentdetail WHERE (NVL (debar, 'N') = 'Y' or NVL (UFM, 'N') = 'Y')  AND institutecode ='"+ mInst +"'             AND examcode IN ('"+mPrevExmCode+"') AND studentid ='"+mChkMemID+"'              ) AND SUBJECTCODE='BAE5C016' ";
*/
...
int SrNo=0;
	...
	//out.print(StudentRecordSet.getString("subjectid"));

qrydebar="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,    " +
        "            v.subjectcode, v.semester, b.grade, b.coursecreditpoint    FROM studentmaster a,   studentresultdetail b," +
        "                exammaster d,                v#studentltpdetail v     WHERE a.institutecode = '"+ mInst +"'" +
        "            AND a.studentid = b.studentid     AND a.studentid = v.studentid    AND b.studentid = v.studentid   " +
        "         AND d.examcode = b.examcode  and   v.SUBJECTID='"+StudentRecordSet.getString("subjectid")+"'  " +
        "      AND d.institutecode = a.institutecode    AND b.fstid = v.fstid            AND b.examcode = v.examcode   " +
        "         AND B.examcode in( "+mPrevExmCode+","+mSumExam+")                    AND a.studentid = '"+mChkMemID+"' and" +
        " (a.studentid,v.subjectid)  in             (select studentid,subjectid from  DEBARSTUDENTDETAIL where        " +
        "     EXAMCODE IN ( "+mPrevExmCode+")       and studentid='"+mChkMemID+"' AND INSTITUTECODE='"+ mInst +"' " +
        " AND (NVL (debar, 'N') = 'Y' or NVL (UFM, 'N') = 'Y'  or NVL (AbsentCase, 'N') = 'Y')  AND  (a.studentid, v.subjectid)" +
        "  IN (                   SELECT studentid, subjectid                     FROM debarstudentdetail        " +
        "            WHERE examcode IN ( "+mSumExam+")                            AND studentid = '"+mChkMemID+"'       " +
        "               AND institutecode = '"+ mInst +"'                       AND (NVL (debar, 'N') = 'Y' or NVL (UFM, 'N') = 'Y') )  " +
        "    and  (v.subject NOT LIKE '%PROJECT%'            OR v.subject LIKE '%PROJECT MANAGEMENT%') )";
qrydebar=qrydebar+" union SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,          " +
        "      v.subjectcode, v.semester, b.grade, b.coursecreditpoint    FROM studentmaster a,   studentresultdetail b,  " +
        "              exammaster d,                v#studentltpdetail v     WHERE a.institutecode = '"+ mInst +"'        " +
        "    AND a.studentid = b.studentid     AND a.studentid = v.studentid    AND b.studentid = v.studentid           " +
        " AND d.examcode = b.examcode  and   v.SUBJECTID='"+StudentRecordSet.getString("subjectid")+"'       " +
        " AND d.institutecode = a.institutecode    AND b.fstid = v.fstid            AND b.examcode = v.examcode       " +
        "     AND B.examcode in( "+mPrevExmCode+","+mSumExam+")                    AND a.studentid = '"+mChkMemID+"' " +
        "and (a.studentid,v.subjectid)  in             (select studentid,subjectid from  DEBARSTUDENTDETAIL where        " +
        "     EXAMCODE IN ( "+mPrevExmCode+")       and studentid='"+mChkMemID+"' AND INSTITUTECODE='"+ mInst +"'  " +
        "AND (NVL (debar, 'N') = 'Y' or NVL (UFM, 'N') = 'Y' or NVL (AbsentCase, 'N') = 'Y')  AND  (a.studentid, v.subjectid)  IN ( " +
        "                  SELECT studentid, subjectid                     FROM debarstudentdetail              " +
        "      WHERE examcode IN ( "+mSumExam+")                            AND studentid = '"+mChkMemID+"'           " +
        "           AND institutecode = '"+ mInst +"'                       AND (NVL (debar, 'N') = 'Y' or NVL (UFM, 'N') = 'Y') )  " +
        "    and SUBJECTCODE='BAE5C016' )";


		rsdebar=db.getRowset(qrydebar);
	 	// out.println(""+qrydebar+"<br><br>");
		if(!rsdebar.next())
		{
	...
	ctot+=StudentRecordSet.getDouble("CourseCreditPoint");
		...
		qry1="SELECT 'Y' FROM SUPPLEMENTARYSUBJECTTAGGING WHERE INSTITUTECODE='"+ mInst +"' AND examcode in( "+QryExam2+") AND " +
                " STUDENTID='"+mChkMemID+"' AND  SUBJECTID='"+StudentRecordSet.getString("subjectid")+"' ";
		//out.print(qry1);
...
<% int ss=0,ssA=0;
	qry="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,           " +
            "     v.subjectcode, v.semester, b.grade, b.coursecreditpoint    FROM studentmaster a,   studentresultdetail b,  " +
            "              exammaster d,                v#studentltpdetail v     WHERE a.institutecode = '"+ mInst +"'       " +
            "     AND a.studentid = b.studentid     AND a.studentid = v.studentid    AND b.studentid = v.studentid         " +
            "   AND d.examcode = b.examcode            AND d.institutecode = a.institutecode    AND b.fstid = v.fstid     " +
            "       AND b.examcode = v.examcode            AND B.examcode in( "+mPrevExmCode+","+mSumExam+")           " +
            "         AND a.studentid = '"+mChkMemID+"' and (a.studentid,v.subjectid)  in     " +
            "        (select studentid,subjectid from  DEBARSTUDENTDETAIL where         " +
            "    EXAMCODE IN ( "+mPrevExmCode+")       and studentid='"+mChkMemID+"' AND INSTITUTECODE='"+ mInst +"' " +
            " AND (NVL (debar, 'N') = 'Y' or NVL (UFM, 'N') = 'Y')) and  (v.subject NOT LIKE '%PROJECT%'  " +
            "          OR v.subject LIKE '%PROJECT MANAGEMENT%') ";
...
qry="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,           " +
            ...
            " (select studentid,subjectid from  DEBARSTUDENTDETAIL where         " +
            " EXAMCODE IN ( "+mPrevExmCode+")       and studentid='"+mChkMemID+"' AND INSTITUTECODE='"+ mInst +"' " +
            " AND NVL (AbsentCase, 'N') = 'Y' ) and  (v.subject NOT LIKE '%PROJECT%'  " +
            " OR v.subject LIKE '%PROJECT MANAGEMENT%') ";

	/*qry=qry+" UNION ALL SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,           
 *      v.subjectcode, v.semester, b.grade, b.coursecreditpoint,                TO_CHAR (examperiodto, 'Month YYYY') exammonth
 *           FROM studentmaster a,                studentresultdetail b,                exammaster d,                v#studentltpdetail v
 *        WHERE a.institutecode = '"+ mInst +"'            AND a.studentid = b.studentid            AND a.studentid = v.studentid
 *       AND b.studentid = v.studentid            AND d.examcode = b.examcode            AND d.institutecode = a.institutecode
 *     AND b.fstid = v.fstid         AND b.examcode = v.examcode            AND  B.examcode in( '"+mPrevExmCode+"','"+mSumExam+"')
 *                     AND a.studentid = '"+mChkMemID+"' and (a.studentid,v.subjectid)  in             (select studentid,subjectid from
 *  DEBARSTUDENTDETAIL where            examcode in( '"+mPrevExmCode+"','"+mSumExam+"')  and studentid='"+mChkMemID+"' AND
INSTITUTECODE='"+ mInst +"'  AND (NVL(DEBAR,'N')='Y'  or NVL (MEDICALCASE, 'N') = 'Y' )   )  AND v.subject LIKE '%PROJECT MANAGEMENT%' ";*/
//	out.print(qry);
	rs=db.getRowset(qry);
	rs1=db.getRowset(qry);
	if(rs1.next())
	...
	<center>
	<font face=verdana color=red size=2 ><b>
	Due to you being Absent in a following subject(s) you are not allowed<br> to register for the same !
	</b></font>
	</center>
...
<%
    ...
    /********************For Absent Case Updated by Gyan on 06-Jul-2015 (Start)************************/

    /********************For Absent Case Updated by Gyan on 06-Jul-2015 (End)************************/
 ...
 // }
			...
			/*}
				...
				{
				out.print("<center><font size=2 face=verdana color=red><b>Please Select Exam Code !</b></font></center>");
				}
