...
qry=qry+"SELECT DISTINCT c.enrollmentno en2, b.subjectid subjectid, b.subject,                b.subjectcode, c.semester, ' ' grade, a.coursecreditpoint,                a.supfees  ,decode(a.BASKET,'A','CORE','B','ELECTIVE','D','ELECTIVE','',a.BASKET)BASKET         FROM offersubjecttagging a, studentmaster  c, subjectmaster b          WHERE a.institutecode = '"+ mInst +"'               AND b.subjectid = a.subjectid            and a.INSTITUTECODE=b.INSTITUTECODE            AND a.examcode ='"+SumExamCode+"'                AND NVL (c.deactive, 'N') = 'N'            AND a.institutecode = c.institutecode            AND NVL (c.deactive, 'N') = 'N'            AND c.studentid ='"+mChkMemID+"'              and a.BASKET in ('D','B')     and a.subjectid='"+msubject+"'     union   ";

qry=qry+"SELECT DISTINCT c.enrollmentno en2, c.subjectid subjectid, c.subject,                c.subjectcode, c.semester, ' ' grade, a.coursecreditpoint,                a.supfees     ,decode(a.BASKET,'A','CORE','B','ELECTIVE','D','ELECTIVE','',a.BASKET)BASKET        FROM offersubjecttagging a,                 v#studentltpdetail c          WHERE a.institutecode =  '"+ mInst +"' and a.subjectid='"+msubject+"'             AND c.subjectid = a.subjectid            AND a.examcode ='"+SumExamCode+"'             AND NVL (c.deactive, 'N') = 'N'            AND a.institutecode = c.institutecode                     AND NVL (c.deactive, 'N') = 'N'            AND c.studentid = '"+mChkMemID+"'             AND (c.studentid, c.subjectid) IN (                   SELECT studentid, subjectid                     FROM debarstudentdetail                    WHERE examcode = '"+PrevExmCode+"'        and subjectid='"+msubject+"'                     AND studentid = '"+mChkMemID+"'                       AND institutecode = '"+ mInst +"' AND NVL (GRADE, 'N') = 'F' )                      and                          (c.studentid, c.subjectid) not IN (                SELECT studentid, subjectid                     FROM studentresult                    WHERE studentid = '"+mChkMemID+"'                       AND institutecode =  '"+ mInst +"'   and subjectid='"+msubject+"'    ) union ";
qry=qry+"SELECT DISTINCT v.enrollmentno en2, c.subjectid subjectid, c.subject,                c.subjectcode, B.semester, b.grade, a.coursecreditpoint,                a.supfees  ,decode(a.BASKET,'A','CORE','B','ELECTIVE','D','ELECTIVE','',a.BASKET)BASKET  FROM offersubjecttagging a, studentresult b, studentmaster v,subjectmaster c WHERE a.subjectid='"+msubject+"' and a.institutecode = '"+ mInst +"' AND v.studentid = b.studentid AND v.studentid = v.studentid AND C.subjectid = a.subjectid AND B.subjectid = a.subjectid AND C.subjectid = B.subjectid AND NVL (b.fail, 'N') = 'Y'  and a.examcode = '"+SumExamCode+"' and nvl(v.DEACTIVE,'N')='N'  AND A.INSTITUTECODE=V.INSTITUTECODE AND A.INSTITUTECODE=B.INSTITUTECODE AND  A.INSTITUTECODE=C.INSTITUTECODE AND C.INSTITUTECODE=B.INSTITUTECODE  AND NVL(V.DEACTIVE,'N')='N'  AND V.studentid = '"+mChkMemID+"'    ";
/*
...
*/	
qry=qry+"  union SELECT DISTINCT v.enrollmentno en2, c.subjectid subjectid, c.subject,                 c.subjectcode, B.semester, 'Not Registered' grade, a.coursecreditpoint,                a.supfees ,decode(a.BASKET,'A','CORE','B','ELECTIVE','D','ELECTIVE','',a.BASKET)BASKET  FROM offersubjecttagging a, nrstudentfailsubjects b,studentmaster v,subjectmaster c WHERE a.subjectid='"+msubject+"' and a.institutecode = '"+ mInst +"' AND v.studentid = b.studentid AND v.studentid = v.studentid and nvl(v.DEACTIVE,'N')='N' AND NVL(V.DEACTIVE,'N')='N' AND a.subjectid = b.subjectid AND a.subjectid = b.subjectid  and a.examcode ='"+SumExamCode+"' and a.subjectid=c.subjectid and a.subjectid=b.subjectid and a.institutecode=c.institutecode and a.institutecode=v.institutecode AND NVL(REGISTERED,'N')='N' AND V.studentid = '"+mChkMemID+"'  ) ORDER BY basket,coursecreditpoint DESC";
...
//out.print(ctot+"sdfsdfsfsf");		
	...
	%>
	<tr><TD colspan=7 align=right><b>Total Credit: <%=ctot%></b></td>
	<TD>&nbsp; &nbsp;</TD>
	</tr>
	
	<tr><TD colspan=8 align=right><B>Payment Total Rs. &nbsp;&nbsp; <%=mTot%>/-</B></td>
...
<tr><td colspan=2 align=Right><b>Signature of Student (<%=mSname%>)</b></td></tr>
	...
	<TD><FONT COLOR=WHITE><b>Subject Type</FONT></TD>
