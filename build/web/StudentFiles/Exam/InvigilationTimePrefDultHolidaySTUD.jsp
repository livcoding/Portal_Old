...
<table align=center width="100%" bottommargin=0 topmargin=0><tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><b><u><FONT SIZE=4>Invigilation Duty Time Prefernce (unavailability)/Duty Holiday </FONT></u></b></font></td></tr></table>
<table cellpadding=2 cellspacing=0 align=center rules=groups border=2>
<!*********--Institute--************>
<INPUT Type="Hidden" Name=Inst id=Inst Value=<%=mInst%>>
<!--*********ExamCode**********-->
<tr><td><FONT color=black face=Arial size=2><b>Exam Event Code </b></FONT>
<%
qry="Select nvl(EXAMCODE,' ') Exam , EXAMPERIODFROM ,EXCLUDEINATTENDANCE ";
qry=qry+" from EXAMMASTER Where INSTITUTECODE='"+mInst+"' AND nvl(Deactive,'N')='N' ";
qry=qry+" and Nvl(LOCKEXAM,'N')='N' and nvl(EXCLUDEININVIGDUTYSEATPLAN,'N')='N' ";
qry=qry+" GROUP BY EXAMCODE, EXAMPERIODFROM, EXCLUDEINATTENDANCE order by EXAMPERIODFROM Desc";
//out.print(qry);
	rs=db.getRowset(qry);
...
<tr><td><input id="DutyHoliday" name="DutyHoliday" Type=Radio value="Y"><STRONG><FONT color=black face=Arial size=2>Duty Holiday (Full day duty dennied)</strong></font>
</td></tr></table>
</td>
</tr>
<%
...
<%	
		  ...
		  if (mDate1.length()==10 && mDate2.length()==10  &&  GlobalFunctions.iSValidDate(mDate1)==true && GlobalFunctions.iSValidDate(mDate2))
		  {
			...
			{						  
				...
				if(!mtext.equals(""))
				{
					...
					//qry=qry+" and EXAMCODE='"+qryexam+"'";   					qry=qry+" And INVIGILATORID='"+mDMemberID+"' and ( trunc(to_date('"+mDate1+"','dd-mm-yyyy')) between trunc(PREFROMDATE) and trunc(PRETODATE) or trunc(to_date('"+mDate2+"','dd-mm-yyyy')) between trunc(PREFROMDATE) and trunc(PRETODATE) )";
					...
					if (!rs.next())
					{
						if (mDutyHoliDay.equals("Y"))
						{
				 			...
				 			qry=qry+" VALUES ('"+mInst+"','"+mComp+"','"+mExam+"','"+mStaffType+"','"+mDMemberID+"',to_date('"+mDate1+"','dd-mm-yyyy'),to_date('"+mDate2+"','dd-mm-yyyy'),'Y','"+mtext+"','"+mDMemberID+"',SYSDATE)";
						}
						else
						{
					 		...
					 		qry=qry+" VALUES ('"+mInst+"','"+mComp+"','"+mExam+"','"+mStaffType+"','"+mDMemberID+"',to_date('"+mDate1+"','dd-mm-yyyy'),to_date('"+mDate2+"','dd-mm-yyyy'),'N',To_date('"+mFromTime+"','dd-mm-yyyy hh:mi PM'), To_date('"+mUptoTime+"','dd-mm-yyyy hh:mi PM'),'"+mtext+"','"+mDMemberID+"',SYSDATE)";
						}
				 		try
					 	{
	...
	qry=" SELECT to_Char(EntryDate,'dd-mm-yyyy') RFDT, to_char(PREFROMDATE,'dd-mm-yyyy') ||' to '||to_char(PRETODATE,'dd-mm-yyyy') || '( '||decode(DUTYHOLIDAY,'Y','Full Day Leave/Holiday',to_char(PREFFROMTIME,'hh:mi PM')||' to '|| to_char(PRETOTIME,'hh:mi PM'))||' )' Tim , nvl(REMARKS,' ') REMARKS, nvl(APPROVED,'N')APPROVED FROM INVIGILATIONTIMEPREF"; 
