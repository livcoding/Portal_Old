...
ResultSet rs=null;
ResultSet rs1=null,rsi=null,rs2=null,rs3=null,rs4=null;
GlobalFunctions gb =new GlobalFunctions();
String  qry1="",qry2="",qry3="",fail="";
String mMemberID="";
String mMemberType="";
String mMemberName="",mPrevExmCode1="",mPrevExmCode2="";
...
<form name="frm" method=post>
			...
			qry="select distinct nvl(examcode,' ')examcode, EXAMPERIODFROM from exammaster where institutecode='"+mInst+"' AND" +
                    " (eXAMCODE LIKE '2015SUP%' ) and nvl(deactive,'N')='N' and nvl(LOCKEXAM,'N')='N' and  " +
			...
			<option selected value='N' > << Select Exam Code  >>  </option>
			<%
...
qry="SELECT nvl(SUMMEREXAMCODE,' ')SUMMEREXAMCODE, nvl(PREVEXAMCODE,' ')PREVEXAMCODE, nvl(PREVEXAMCODE1,' ')PREVEXAMCODE1, nvl(PREVEXAMCODE2,' ')PREVEXAMCODE2, nvl(SUPFEES,'')SUPFEES  FROM EXAMMASTER where InstiTuteCode='"+mInst+"' and examcode='"+QryExam+"' and NVL(LOCKEXAM,'N')='N'";
//out.println(qry);
rs=db.getRowset(qry);
if(rs.next())
{
	mPrevExmCode=rs.getString("PREVEXAMCODE");
    mPrevExmCode1=rs.getString("PREVEXAMCODE1");
    mPrevExmCode2=rs.getString("PREVEXAMCODE2");
...
/*	qry="SELECT distinct a.enrollmentno en2,v.subjectid, v.subject, v.subjectcode, v.semester, b.grade,       e.coursecreditpoint, TO_CHAR (examperiodto, 'Month YYYY') exammonth  FROM studentmaster a,       studentresultdetail b,       exammaster d,       v#studentltpdetail v,       programsubjecttagging e WHERE a.institutecode =  '"+ mInst +"' and a.INSTITUTECODE=e.INSTITUTECODE and b.EXAMCODE=e.EXAMCODE   AND a.studentid = b.studentid   AND a.studentid = v.studentid   AND b.studentid = v.studentid   AND d.examcode = b.examcode   AND d.institutecode = a.institutecode   AND b.fstid = v.fstid   AND b.examcode = v.examcode   AND b.examcode = '"+mPrevExmCode+"'    AND b.grade LIKE '%F%'   AND a.studentid =  '"+mChkMemID+"'";*/
	...
	qry="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject, v.subjectcode,            " +
            ...
            "      AND b.examcode = v.examcode            AND  b.examcode in( '"+mPrevExmCode+"','"+mPrevExmCode1+"','"+mPrevExmCode2+"','"+mSumExam+"') " +
...
qry=qry+ "	AND  (v.subject NOT LIKE '%PROJECT%'            OR v.subject LIKE '%PROJECT MANAGEMENT%') " ;
	...
	qry=qry+ " AND    SUBJECTID NOT IN (             SELECT SUBJECTID FROM debarstudentdetail WHERE (NVL (debar, 'N') = 'Y'" +
            ...
            "          AND examcode IN ('"+mPrevExmCode+"','"+mPrevExmCode1+"','"+mPrevExmCode2+"') AND studentid ='"+mChkMemID+"'              )";
...
qrydebar="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,                v.subjectcode, v.semester, b.grade, b.coursecreditpoint    FROM studentmaster a,   studentresultdetail b,                exammaster d,                v#studentltpdetail v     WHERE a.institutecode = '"+ mInst +"'            AND a.studentid = b.studentid     AND a.studentid = v.studentid    AND b.studentid = v.studentid            AND d.examcode = b.examcode  and   v.SUBJECTID='"+StudentRecordSet.getString("subjectid")+"'        AND d.institutecode = a.institutecode    AND b.fstid = v.fstid            AND b.examcode = v.examcode            AND B.examcode in( '"+mPrevExmCode+"','"+mSumExam+"')                    AND a.studentid = '"+mChkMemID+"' and (a.studentid,v.subjectid)  in             (select studentid,subjectid from  DEBARSTUDENTDETAIL where             EXAMCODE IN ( '"+mPrevExmCode+"')       and studentid='"+mChkMemID+"' AND INSTITUTECODE='"+ mInst +"'  AND (NVL (debar, 'N') = 'Y' or NVL (UFM, 'N') = 'Y'  or NVL (AbsentCase, 'N') = 'Y')  AND  (a.studentid, v.subjectid)  IN (                   SELECT studentid, subjectid                     FROM debarstudentdetail                    WHERE examcode IN ( '"+mSumExam+"')                            AND studentid = '"+mChkMemID+"'                      AND institutecode = '"+ mInst +"'                       AND (NVL (debar, 'N') = 'Y' or NVL (UFM, 'N') = 'Y') )      and  (v.subject NOT LIKE '%PROJECT%'            OR v.subject LIKE '%PROJECT MANAGEMENT%') )";
qrydebar=qrydebar+" union SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,                v.subjectcode, v.semester, b.grade, b.coursecreditpoint    FROM studentmaster a,   studentresultdetail b,                exammaster d,                v#studentltpdetail v     WHERE a.institutecode = '"+ mInst +"'            AND a.studentid = b.studentid     AND a.studentid = v.studentid    AND b.studentid = v.studentid            AND d.examcode = b.examcode  and   v.SUBJECTID='"+StudentRecordSet.getString("subjectid")+"'        AND d.institutecode = a.institutecode    AND b.fstid = v.fstid            AND b.examcode = v.examcode            AND B.examcode in( '"+mPrevExmCode+"','"+mSumExam+"')                    AND a.studentid = '"+mChkMemID+"' and (a.studentid,v.subjectid)  in             (select studentid,subjectid from  DEBARSTUDENTDETAIL where             EXAMCODE IN ( '"+mPrevExmCode+"')       and studentid='"+mChkMemID+"' AND INSTITUTECODE='"+ mInst +"'  AND (NVL (debar, 'N') = 'Y' or NVL (UFM, 'N') = 'Y' or NVL (AbsentCase, 'N') = 'Y')  AND  (a.studentid, v.subjectid)  IN (                   SELECT studentid, subjectid                     FROM debarstudentdetail                    WHERE examcode IN ( '"+mSumExam+"')                            AND studentid = '"+mChkMemID+"'                      AND institutecode = '"+ mInst +"'                       AND (NVL (debar, 'N') = 'Y' or NVL (UFM, 'N') = 'Y') )      and SUBJECTCODE='BAE5C016' )";
...
<% int ss=0,ssA=0;
	...
	qry1="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,           " +
            "     v.subjectcode, v.semester, b.grade, b.coursecreditpoint    FROM studentmaster a,   studentresultdetail b,  " +
            "              exammaster d,                v#studentltpdetail v     WHERE a.institutecode = '"+ mInst +"'       " +
            "     AND a.studentid = b.studentid     AND a.studentid = v.studentid    AND b.studentid = v.studentid         " +
            "   AND d.examcode = b.examcode            AND d.institutecode = a.institutecode    AND b.fstid = v.fstid     " +
            "       AND b.examcode = v.examcode            AND B.examcode in('"+mPrevExmCode+"', '"+mPrevExmCode1+"','"+mSumExam+"')           " +
            "         AND a.studentid = '"+mChkMemID+"' and (a.studentid,v.subjectid)  in     " +
            "        (select studentid,subjectid from  DEBARSTUDENTDETAIL where         " +
            "    EXAMCODE IN ( '"+mPrevExmCode+"','"+mPrevExmCode1+"')       and studentid='"+mChkMemID+"' AND INSTITUTECODE='"+ mInst +"' " +
            " AND (NVL (debar, 'N') = 'Y' or NVL (UFM, 'N') = 'Y')) and  (v.subject NOT LIKE '%PROJECT%'  " +
            "          OR v.subject LIKE '%PROJECT MANAGEMENT%') ";
out.print(qry1+"<br>");
...
qry2="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,           " +
            "     v.subjectcode, v.semester, b.grade, b.coursecreditpoint    FROM studentmaster a,   studentresultdetail b,  " +
            "              exammaster d,                v#studentltpdetail v     WHERE a.institutecode = '"+ mInst +"'       " +
            "     AND a.studentid = b.studentid     AND a.studentid = v.studentid    AND b.studentid = v.studentid         " +
            "   AND d.examcode = b.examcode            AND d.institutecode = a.institutecode    AND b.fstid = v.fstid     " +
            ...
            "         AND a.studentid = '"+mChkMemID+"' and (a.studentid,v.subjectid)  in     " +
            "        (select studentid,subjectid from  DEBARSTUDENTDETAIL where         " +
            "    EXAMCODE IN ( '"+mPrevExmCode+"','"+mPrevExmCode1+"','"+mPrevExmCode2+"')       and studentid='"+mChkMemID+"' AND INSTITUTECODE='"+ mInst +"' " +
            " AND (NVL (debar, 'N') = 'Y' or NVL (UFM, 'N') = 'Y')) and  (v.subject NOT LIKE '%PROJECT%'  " +
            "          OR v.subject LIKE '%PROJECT MANAGEMENT%') ";
...
out.print(qry2);
if(rs3.next()){
    if(rs3.getString("grade").equals("F"))
    {
		%>
	<div id="debarcase">
    <center>
	...
	<TD><FONT COLOR=WHITE><b>Credits</FONT></TD>
		  ...
		  <TR>
	<TD><b><%=ss%></b></TD>
	<TD><b><%=rs4.getString("SubjectCode")%></b></TD>
	<TD><b><%=rs4.getString("Subject")%></b></TD>
	<TD><b><%=rs4.getInt("Semester")%></b></TD>
	<TD><b><%=rs4.getString("Grade")%></b></TD>
	<TD align=right><b><%=rs4.getDouble("CourseCreditPoint")%></b></TD>
	</tr>
		  <%
		}}}}}}
    }else
        {
        qry2="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject, b.break#slno  break#slno  ,      " +
            "     v.subjectcode, v.semester, b.grade, b.coursecreditpoint    FROM studentmaster a,   studentresultdetail b,  " +
            "              exammaster d,                v#studentltpdetail v     WHERE a.institutecode = '"+ mInst +"'       " +
            "     AND a.studentid = b.studentid     AND a.studentid = v.studentid    AND b.studentid = v.studentid         " +
            "   AND d.examcode = b.examcode            AND d.institutecode = a.institutecode    AND b.fstid = v.fstid     " +
            "       AND b.examcode = v.examcode            AND B.examcode in('"+mPrevExmCode2+"', '"+mPrevExmCode1+"','"+mPrevExmCode+"','"+mSumExam+"')           " +
            "         AND a.studentid = '"+mChkMemID+"' and (a.studentid,v.subjectid)  in     " +
            "        (select studentid,subjectid from  DEBARSTUDENTDETAIL where         " +
            "    EXAMCODE IN ( '"+mPrevExmCode2+"','"+mPrevExmCode1+"','"+mPrevExmCode+"')       and studentid='"+mChkMemID+"' AND INSTITUTECODE='"+ mInst +"' " +
            " AND (NVL (debar, 'N') = 'Y' or NVL (UFM, 'N') = 'Y')) and  (v.subject NOT LIKE '%PROJECT%'  " +
            "          OR v.subject LIKE '%PROJECT MANAGEMENT%') ";
rs3=db.getRowset(qry2);
rs4=db.getRowset(qry2);
...
if(rs3.next()){
	...
	<TD><FONT COLOR=WHITE><b>Credits</FONT></TD>
		  ...
		  while(rs4.next())
		  {//out.print("GYan");
              qry="select nvl(fail,'') fail from studentresultdetail where BREAK#SLNO='"+rs4.getString("break#slno")+"'" +
                      " and studentid='"+mChkMemID+"' and grade<>'F'";
             // out.print(qry);
              rs=db.getRowset(qry);
              if(rs.next())
              {
              fail=rs.getString("fail");
              }else
              {
                  fail="Y";
              }
...
qry="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,           " +
            ...
            " AND a.studentid = b.studentid     AND a.studentid = v.studentid    AND b.studentid = v.studentid         " +
            " AND d.examcode = b.examcode            AND d.institutecode = a.institutecode    AND b.fstid = v.fstid     " +
            " AND b.examcode = v.examcode            AND B.examcode in( '"+mPrevExmCode1+"','"+mSumExam+"')           " +
            " AND a.studentid = '"+mChkMemID+"' and (a.studentid,v.subjectid)  in     " +
            " (select studentid,subjectid from  DEBARSTUDENTDETAIL where         " +
            " EXAMCODE IN ( '"+mPrevExmCode+"')       and studentid='"+mChkMemID+"' AND INSTITUTECODE='"+ mInst +"' " +
...
//out.print(qry);
	...
	qry1="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,           " +
            ...
            "        (select studentid,subjectid from  DEBARSTUDENTDETAIL where         " +
            "    EXAMCODE IN ( '"+mPrevExmCode+"','"+mPrevExmCode1+"')       and studentid='"+mChkMemID+"' AND INSTITUTECODE='"+ mInst +"' " +
            " NVL (AbsentCase, 'N') = 'Y' ) and  (v.subject NOT LIKE '%PROJECT%'  " +
            "          OR v.subject LIKE '%PROJECT MANAGEMENT%') ";
rs2=db.getRowset(qry1);
if(rs2.next())
{if(rs2.getString("grade").equals("F")){
