...
String mDateFrom="",mDateTo="",mStudName="",mCompCode="",mInst="", mRightsID="", mSRCType="";
...
///
String LoginComp="";
//LoginComp=session.getAttribute("LoginComp").toString().trim();
//out.print(mCompCode+"dd"+LoginComp);
///


//out.println(mMemberID);
...
//out.println(LoginIDTime);
	...
	if (RsChk.next() && RsChk.getString("SL").equals("Y"))
	{
		...
		<tr><TD align=middle class=pageheading><font color="#a52a2a" FACE="VERDANA" SIZE=4> Self Attendance Detail</TD>
		</font></td></tr>
		</TABLE>
		<%
		...
		<tr><td nowrap>
		<FONT color=black face=Arial size=2><b>Student &nbsp;</b></FONT>
		<%	
	
	qry=" Select Distinct a.membercode, A.memberid FacultyID, nvl(A.EMPLOYEENAME,' ') EmployeeName ";
	qry=qry+" from V#ALLMEMBERS A where A.INSTITUTECODE='"+mInst+"' and A.memberid ='"+mChkMemID+"' and A.MEMBERTYPE='"+mChkMType+"' order by EmployeeName ";
			...
			while(rs.next())
			{
				if(mFaculty.equals(""))
				{
					mFaculty=rs.getString("FacultyID");
				  	%>
					<OPTION Value ='<%=mFaculty%>'><%=rs.getString("EmployeeName")%> -  <%=rs.getString("membercode")%></option>
				  	<%
				}
				else
				{
					%>
					<OPTION Value ='<%=rs.getString("FacultyID")%>'><%=rs.getString("EmployeeName")%> -  <%=rs.getString("membercode")%></option>
		 			<%
				}
			}
		}
		else
		{
			while(rs.next())
			{
				...
				{
					%>
					<OPTION selected Value ='<%=mFaculty%>'><%=rs.getString("EmployeeName")%>-  <%=rs.getString("membercode")%></option>
					<%
				}
				else
				{
	...
	if (request.getParameter("x")!=null)
	{
		...
		<tr><TD colspan=0 align=middle ><font color="#a52a2a" FACE="VERDANA" SIZE=4>Available List</TD>
		</font></td>	
		</table>
		...
		<tbody>
		<%
		qry="SELECT distinct MEMBERID, to_char(WORKINGDATE,'dd-mm-yyyy')WORKINGDATE,TO_CHAR (workingdate, 'yyyymmdd') workingdate1, nvl(LEAVESTATUS,' ')LEAVESTATUS,COMPANYCODE,MEMBERTYPE,LOGINCARDIDTIME FROM ATT_TEMPWEEKREC where  MEMBERID='"+mFaculty+"' and COMPANYCODE='"+mCompCode+"' ";
		qry=qry+ " and LOGINCARDIDTIME='"+LoginIDTime+"' ";
		qry=qry+ "  and trunc(WORKINGDATE) between trunc(decode(to_date('"+mDateFrom+"','dd-mm-yyyy'),'',WORKINGDATE,to_date('"+mDateFrom+"','dd-mm-yyyy'))) and trunc(decode(to_date('"+mDateTo+"','dd-mm-yyyy'),'',WORKINGDATE,to_date('"+mDateTo+"','dd-mm-yyyy'))) order by workingdate1";
	//	out.println(qry);
		rs=db.getRowset(qry);
		while(rs.next())
		{
			...
			{
			%>
			<td><a title="Click to View Detail" target=New_ href="StudDailyAttendance.jsp?WorkDt=<%=rs.getString("WORKINGDATE")%>"><%=rs.getString("WORKINGDATE")%></a></td>	
			...
			{
			%>
			<td><a title="Click to View Detail" target=New_ href="StudDailyAttendance.jsp?WorkDt=<%=rs.getString("WORKINGDATE")%>">
