...
if (session.getAttribute("MemberID")!=null && session.getAttribute("MemberCode")!=null)
{
	...
	if (1==1)
	{
		...
		qry=" Select SEMESTER from  STUDENTMASTER where StudentID='" +mDID+ "' and InstituteCode='" + mInst + "'";
		//out.print(qry);
	      rs=db.getRowset(qry);
		if (rs.next())
		...
		qry="select EXAMCODEFORATTENDNACEENTRY ed from CompanyInstituteTagging where InstituteCode='" + mInst + "'  ";
		//out.print(qry);
		rs=db.getRowset(qry);
		if (rs.next())
		{
		...
		qry=qry+" UNION ";
		qry=qry+" Select distinct A.Semester Semester, A.SubjectID SubjectID, C.Subject||' ('||C.SubjectCode||')' Subj, 'Not Registered' Status From NRSTUDENTFAILSUBJECTS A, SubjectMaster C ";
		qry=qry+" where A.institutecode='"+mInst +"' And A.studentid= '"+mChkMemID+"' ";
		qry=qry+" And A.semester< "+(mSem)+" AND A.institutecode=C.institutecode And A.subjectID=C.subjectID";
		qry=qry+" order by status ,semester , Subj";
		//out.print(qry);
		rs=db.getRowset(qry);
		if (rs.next())
		{
		...
		<tr bgcolor=darkblue>
		<td><font color=white><b>Semester</b></font></td><td><font color=white><b>Subject</b></font></td><td><font color=white><b>Subject Type</b></font></td><td><font color=white><b>Credit</b></font></td></td><td><font color=white><b>Remarks</b></font></td>
		...
		while(rs.next())
			...
			qry="Select B.COURSECREDITPOINT COURSECREDITPOINT, decode(nvl(COURSETYPE,'C'),'C','A','B') BASKET from ProgramSubjectTagging B where ";
			qry=qry+" B.iNSTITUTEcODE='"+ mInst +"' AND b.PROGRAMCODE='"+mProg +"' And b.academicyear='"+mAcad+"' aND B.subjectID='"+ mSubjID +"'";
			...
			qry="Select  SubjectID from FacultySubjectTagging B where B.institutecode='"+mInst +"' And B.ExamCode='"+mExam+"' ";
	...
	<br>	<br>	<br>
	<A href="../StudentPageFinal.jsp"><fotn color=red size=2 face='verdana'><b>Click to View Webkiosk</b></font></A>
	...
	else
	{
		...
		String URL="../StudentPageFinal.jsp?SID="+mChkMemID;
