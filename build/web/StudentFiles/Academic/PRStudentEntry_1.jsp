...
function Clickedsel11(k,j,c,b,t){
	...
	//alert(ttt+"cvxc");
	for(j=1;j<=ttt;j++){
		//alert(document.getElementById("Select"+j));
		
		if(document.getElementById("Select"+j)!=null)
		{
		if(document.getElementById("Select"+j).value=="" || document.getElementById("Select"+j).value==null){
			tt=0;			
		}
		}
	...
	//alert(tt+"sdff");
	if(tt==0){
		alert("First choose one compulsory PDE Elective. Then balance Elective if you are within Course Credit Limit");
		//alert(document.getElementById("Select").focus()+"sdff");
		document.getElementById(c).value="";
...
//	alert(t+"ssd");
	...
	if(t==0 && ec==0){
		...
		if(parseInt(cr)+3<=total){		
			for(i=1;i<=t;i++){
				//alert(document.getElementById("Selectf"+i).value+"sss");
				if(document.getElementById("Selectf"+i).value==""){
					tt=0;			
				}
			}
...
//	alert(t+"tt");
	...
	if(document.frm1.mSem.value=='8'){
		...
		if((corecr+3)>=total && points=='0'){
			...
			//alert(u+"uu"+c3+"total"+total+"corecr"+corecr);
...
//		alert(ec+"sdfsdf"+document.frm1.studelecchoise.value);
		document.frm1.studelecchoise.disabled=true;
		for(i=1;i<=ec;i++){
			document.getElementById("Select"+i).disabled=true;
		}		
...
int mSem=0, mSno=0, mChoice=1, mTot=0,mTotB=0;
String mySect="";
...
// basket   E to B
	qry="SELECT MINSUBJECT MINSUBJECT, MAXSUBJECT MAXSUBJECT FROM basketmaster WHERE INSTITUTECODE='"+mInst+"'  ";
	qry=qry+" AND ACADEMICYEAR='"+mAcad+"' AND PROGRAMCODE='"+mProg+"' AND TAGGINGFOR='"+mTag+"' AND SECTIONBRANCH='"+mySect+"' AND SEMESTER="+mSem+" and basket='B' ";
	//out.println(qry);
	rs=db.getRowset(qry);
	if(rs.next())
	{
				 ...
				 if(!mOldSubj.equals(rs.getString("SUBJECTID")))
				   {	
					...
					if(mSubjType.equals("C"))
					{
						...
						{
						%>
							<td align=center>ddd<input Type=checkbox id='<%=mName1%>' name='<%=mName1%>' value='Y' onclick="FunRefresh('<%=mCourseCrPt%>', '<%=mSno%>','<%=mBasket%>','<%=mName1%>')"></td>
							<%Count++;
...
i=0;
			...
			//	out.print(qry);
				//out.print(mAcad+"mAcad");
				while(rsc.next())
				{
					mTot=rsc.getInt("cnt");
					mTotB=rsc.getInt("cnt");
					...
					%><INPUT TYPE="hidden" NAME="pd" value=<%=mTotB%>><%
				...
				//out.print(qry);
					...
					while(rs1.next())
					{
						...
						//   {
							...
							<select name='<%=mName10%>' id='Selectf<%=i%>' onchange="Clickedsel11('<%=mSno%>','<%=mCourseCrPt%>','Selectf<%=i%>','<%=mBasket%>','<%=mTotB%>')"	style="Width:40">								
							<%
		...
		<font color=red>
		<h3>	<br><img src='../../Images/Error1.jpg'>		
		Pre- Registration Event has not been declared or Registration completed  ff</FONT></P>
		 <%
		}
	   }
	   else
	...
	qry="Select to_char(Sysdate,'dd-mm-yyyy hh:mi PM')date1 from Dual";	
		...
		<br>Please press Click to Print button to complete your Pre-Registration process
i.e freeze <br> the choices even though you do not want to print the registration form.
</b>
		</FONT><Br>
		...
		if(rs.next())
		{
			mSubSect=rs.getString(1);
				...
				double sumcrt=0;
				qry="Select distinct  B.Subject||' ('||B.SubjectCode||')' SUBJECT, A.COURSECREDITPOINT COURSECREDITPOINT, a.subjectid subjectid,nvl(subjecttype,'')subjecttype,semester,nvl(A.BASKET,'A')BASKET from PROGRAMSCHEME A,SUBJECTMASTER B";
				qry=qry+" where A.institutecode='"+mInst +"' and A.Academicyear='"+mAcad+"' and A.programcode='"+mProg+"' and A.TAGGINGFOR='"+mTag+"' AND A.SECTIONBRANCH='"+mySect+"' ";
