	...
	else
	{
		...
		if(document.getElementById(temp).value=='Y'){	
			...
			if(document.frm1.TotalCCP.value>=z){
				...
				//alert(document.getElementById(tem+i).value);
...
function finalsave(){
	var kkt=document.frm1.kk.value;
	var val,j,flag=0;
	var z=parseInt(document.frm1.TotalCCPAld.value);
	if(document.frm1.TotalCCP.value>=z){
		if(kkt!=null)
			for(i=1;i<=kkt;i++){
				val=document.getElementById("pd"+i).value;
				for(j=1;j<=val;j++){
					if(document.getElementById("SelectfEL-"+i+j).value==""){
						alert("Please select all elective(S)");						
						return false;
					}
				}
			}
...
i=0;
				...
				while(rsc.next())
				{
					mTot=rsc.getInt("cnt");
					kk++;
					%><INPUT TYPE="hidden" id="pd<%=kk%>" NAME="pd<%=kk%>" value="<%=mTot%>"><%
...
//out.println(mElecCode);
	      			      if(!mElective.equals(mElecCode))
						   {
							...
							-->
							<input type=hidden name='temp1<%=nn%>' value="<%=value%>" size=1 id="temp1<%=i%>">
