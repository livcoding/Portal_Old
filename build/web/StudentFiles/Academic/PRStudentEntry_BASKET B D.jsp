...
function test(){
	...
	if(document.frm1.studelecchoise.value=='0'){
		//alert(document.frm1.mSem.value+"sdfsdf"+ec);
		document.frm1.studelecchoise.disabled=true;
		for(i=1;i<=ec;i++){
			document.getElementById("Select"+i).disabled=true;
		}		
	...
	if(test<parseInt(document.frm1.minlimt.value)){
		alert('ssAs per Curriculum Structure you Minimum Corurse Credit Point must be '+document.frm1.minlimt.value+' whereas your Total calculated Course Credit Point is '+test+' only.');			
	}
}
 	...
 	if(!rss1.next())
		      ...
		      else if(rss1.getString("SENDTOHOD").equals("Y"))
			{
			...
			else
	      	{
				...
				<td><FONT face=Arial size=2 color=black><STRONG>Pre Registration for Semester </STRONG></FONT><select name=sem id=sem><option value=<%=String.valueOf(mSem)%>><%=mSem%></option></select></td>
...
/*getting draft values*/
					...
					else
					{
						...
						//Current Elective    
//						out.print(electivecount+"electivecount");
					//	electivecount=9;
						%>
