...
function finalsave(){
	...
	var cgpa=document.frm1.mCGPA.value;
//alert(cgpa+"cgpa");
var cggg=1;
	...
	if(t==0 && ec==0){
		...
		if(parseInt(cr)+3<=total){
			...
			for(k=1;k<=ec;k++){
			//alert(k+"KKK"+document.getElementById("BASKET"+k).value);

			if(	document.getElementById("BASKET"+k).value=="E" || document.getElementById("BASKET"+k).value=="D")
			{
			//	for(k=1;k<=ec;k++){
	...
	/*
					if(cgpa >= 6 )
					{
						if((document.getElementById("SUBJID"+k).value=="090310") && (document.getElementById("Select"+i).value=="") )
						{
						//  SUBJECTID IN(   '090310','090311','120007')
					//	alert("HII" );
						cggg=0;
						}
						if((document.getElementById("SUBJID"+k).value=="120007"  ) && (document.getElementById("Select"+i).value==""))
						{
								cggg=0;
						}

						if((document.getElementById("SUBJID"+k).value=="090311"  ) && (document.getElementById("Select"+i).value==""))
						{
								cggg=0;
						}
					}*/
	...
	if(cggg==0)
			{
				alert("Please Select these Subject Compulsoury Listed Elective(s) 1. ADVANCED SATELLITE AND FIBRE OPTIC COMMUNCIATIOS (10M11EC112)  2. MULTIMEDIA COMMUNICATION THEORY AND STANDARDS (12M1WEC131) 3.				ADVANCED COMMUNICATION SYSTEMS (10M11EC111) ");
				return false;
			}
...
function test(){
	...
	//if(document.frm1.mSem.value=='7'){ // change by Deepak & satendra on 20may2016
		...
		if((corecr+3)>=total && points=='0'){
			...
			//alert(u+"UU"+c3+"sada"+corecr+"corecr"+total+"total");
...
if(document.frm1.mProg.value=='M.T.')
	...
	if(document.frm1.studelecchoise.value=='0'){
		...
		document.frm1.studelecchoise.disabled=true;
		for(i=1;i<=ec;i++){
			document.getElementById("Select"+i).disabled=true; //Satendra
		}
	}
...
double mCGPA=0;
	...
	qry2="SELECT semester,  cgpa cgpa  FROM studentsgpacgpa#pub WHERE institutecode = '"+ mInst +"' AND studentid = '" +mDID+ "'  AND SEMESTER = 5 ";
//out.print(qry2);
	rs=db.getRowset(qry2);
	if(rs.next())
		mCGPA=rs.getDouble("cgpa");


//mCGPA=7;
	//out.print(mCGPA+"mCGPA");



	qry=" Select distinct nvl(STUDENTID, ' ') STUDENTID, nvl(PROGRAMCODE,' ') PROGRAMCODE,nvl(BRANCHCODE,' ') BRANCHCODE, ";
	qry=qry+" SEMESTER SEMESTER1, TaggingFor, ACADEMICYEAR, SECTIONBRANCH from ";
...
//out.print("sdfsfsafasdf");
	if (rs.next())
	{
	//	out.print(rs.getString("SEMESTER1")+"GGGGGGGGGGGGGGGGGG");
		mSEMESTER=rs.getString("SEMESTER1");
		//--------------------Changed by Vijay---------------
		mSem=rs.getInt("SEMESTER1");
...
//out.print(qry);
 	if(!rss1.next())
			...
			<input Type=hidden id="mCGPA" name="mCGPA" value=<%=mCGPA%>>
				...
				<font color=white face=arial>STUDENT PRE REGISTRATION/SUBJECT-CHOICE ENTRY SCREEN</font></B></td>
				</tr>
				<tr>
				...
				<input type="text" name="studelecchoise" maxlength='2' size='2' value="<%=rss1.getString("NOOFELESUBJECTSCHOICES")%>"   readonly>
				...
				<td colspan=2 align=center nowrap><font color=white face=arial size=2><b>Subject Type<b></font></td>
...
qry1="SELECT sTUDENTID,ENROLLMENTNO FROM STUDENTMASTER A WHERE A.INSTITUTECODE='"+mInst+"'  and  nvl(A.DEACTIVE,'N')='N'  AND  '"+mSCode+"' IN ('091714','091026','091014','091118','091601','091016','091204','091253','091619','091030','091637','091081','091567','091294','091286','091228') ";
//out.print(qry1);
...
mStudSelect="Y";
				...
				while(rsc.next())
				{
					...
					int j=0;
					qry="Select A.SUBJECTID SUBJECTID, B.Subject||'('||B.SubjectCode||')' Subj, A.COURSECREDITPOINT COURSECREDITPOINT,A.ELECTIVECODE From PR#ELECTIVESUBJECTS A,  SubjectMaster B ";
					qry=qry+" where A.INSTITUTECODE='"+mInst+"' and A.EXAMCODE='"+mExam+"' and A.ACADEMICYEAR='"+mAcad+"'";
					qry=qry+" and A.ELECTIVECODE='"+mElecCode+"' and A.PROGRAMCODE='"+mProg+"' and A.TAGGINGFOR='"+mTag+"' and A.SECTIONBRANCH='"+mySect+"'";

					qry=qry+" and A.SEMESTER="+mSem+" and nvl(A.DEACTIVE,'N')='N'  and nvl(b.deactive,'N')='N' and B.subjectID=A.subjectID  and a.INSTITUTECODE=b.INSTITUTECODE and nvl(A.BASKET,'B')='B' ";
					qry=qry+" Group By A.SUBJECTID,a.ELECTIVECODE , B.Subject||'('||B.SubjectCode||')', A.COURSECREDITPOINT order by A.ELECTIVECODE , B.Subject||'('||B.SubjectCode||')' ";
				//out.print(qry);
...
//out.print(mStudSelect+" : : "+mSubjId);
							...
							<td align=center><Font face=arial size=2><%=mSno%>.</font></td>
							<td align=center><Font face=arial size=2>CURRENT</font></td>
							<td align=center nowrap><Font face=arial size=2>ELECTIVE (<%=mElecCode%>)</font></td>
							...
							System.out.print(mStudSelect+" kkkk "+mSubjId);
									if(mStudSelect.equals("Y") && mSubjId.equals("120084"))
										{
											st1="Selected";
											//m=1;
											%>

									<option <%=st1%>  Value="1">1</option>
									<%
										}
									else
							   {
						...
						else
						{
							...
							<select name='<%=mName10%>' id='Selectf<%=i%>' onchange="Clickedsel11('<%=mSno%>','<%=mCourseCrPt%>','Selectf<%=i%>','<%=mBasket%>','<%=mTot%>')"	style="Width:40">
								...
								// Compulsory Subject  for some students   System.out.print(mStudSelect+" pppp "+mSubjId);
									if(mStudSelect.equals("Y") && mSubjId.equals("120084"))
										{
											st1="Selected";
											m=1;
