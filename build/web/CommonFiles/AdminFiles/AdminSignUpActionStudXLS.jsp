...
<HTML>
<head>
<TITLE>#### <%=mHead%> [ Student Signup XLS ] </TITLE>
...
<hr>
<h1><font size=3>Student Password Generated.</font></h1>
<!--Institute-->
...
//	int mFlag=0;
//out.print(mAcademicyear+"mAcademicyear"+mStr);
	%>
	...
	<%
		if (mStr.equals(""))
		{
		 qry="select A.STUDENTID, A.ENROLLMENTNO, A.STUDENTNAME, A.PROGRAMCODE, A.academicyear,B.PWD from STUDENTMASTER A, MEMBERMASTER b Where A.academicyear='"+mAcademicyear+"' AND B.ORATYP='"+pMemberType+"' AND B.ORAADM='"+pMemberRole+"' and a.QUOTA=decode('"+mQUOTA+"','ALL',quota,'"+mQUOTA+"') ";
		 qry=qry+"and A.INSTITUTECODE='"+mInst+"' AND a.ENROLLMENTNO=b.MEMCODE and A.ENROLLMENTNO is not null and nvl(A.deactive,'N')='N' order by ENROLLMENTNO";
		}
		else
		{
		qry="select A.STUDENTID, A.ENROLLMENTNO, A.STUDENTNAME, A.PROGRAMCODE, A.academicyear,B.PWD from STUDENTMASTER A, MEMBERMASTER b Where A.ProgramCode IN ("+mStr+") and  a.ENROLLMENTNO=b.MEMCODE AND B.ORATYP='"+pMemberType+"' AND B.ORAADM='"+pMemberRole+"' AND  ";
		qry=qry+" A.academicyear='"+mAcademicyear+"' and a.QUOTA=decode('"+mQUOTA+"','ALL',quota,'"+mQUOTA+"')  and A.ENROLLMENTNO is not null and A.INSTITUTECODE='"+mInst+"' and nvl(A.deactive,'N')='N' order by ENROLLMENTNO";
		}
		rs=db.getRowset(qry);
//out.print(qry);
		while (rs.next())
