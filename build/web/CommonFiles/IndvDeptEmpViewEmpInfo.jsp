...
try
{
    ...
    //sout.print(request.getParameter("DNAME")+"dddd");
	...
	{
		mDNAME=request.getParameter("DNAME").toString().trim();
	}

  //  out.print(mDNAME+"zxzxzxzx");
	OLTEncryption enc=new OLTEncryption();
	if(!mMemberID.equals("") && !mMemberCode.equals("") && !mMemberName.equals("") && ! mDNO.equals("")) 
	{
		...
		{
			qry="Select nvl(DEPARTMENT,' ') DEPARTMENT , nvl(dEPARTMENTcODE,' ')dEPARTMENTcODE from DepartmentMaster where" +
                    " dEPARTMENTcODE like ('"+mDNO+"%') or dEPARTMENTcODE='"+mDNO+"' ";
            //out.print(qry);
			rs=db.getRowset(qry);
			if(rs.next())
                {
		...
		<!-- 	<td><font color="white"><b>View SRS</b></font></td> -->
			...
			<%
/*			qry="Select Distinct EMPLOYEEID, nvl(EmployeeCode,' ') EmployeeCode,nvl( EMPLOYEENAME,' ')  EMPLOYEENAME," +
                    " nvl(DESIGNATIONCODE,' ') DESIGNATIONCODE from EmployeeMaster  where CompanyCode='"+ mCompCode +"' " +
                    "and DepartmentCode='"+mDNO+"' and nvl(Deactive,'N')='N' order by EmployeeName,EmployeeCode";
*/
            qry="SELECT DISTINCT a.employeeid, NVL (a.employeecode, ' ') employeecode," +
                    "                NVL (a.employeename, ' ') employeename," +
                    "                NVL (a.designationcode,' ') designationcode," +
                    " nvl(c.DESIGNATION,' ')DESIGNATION" +
                    "                  FROM employeemaster a,DESIGNATIONMASTER c" +
                    "          WHERE a.companycode   in ( 'UNIV','JPBS') " +
                    "            AND a.departmentcode = '"+mDNO+"' " +
                    "                        AND NVL (a.deactive, 'N') = 'N'  and a.DESIGNATIONCODE=c.DESIGNATIONCODE" +
                    "                    ORDER BY employeename, employeecode,DESIGNATION";
            rs=db.getRowset(qry);
			//out.print(qry);
			while(rs.next())
			{
				...
				<td><font size=2x><%=rs.getString("DESIGNATION")%></font></td>
