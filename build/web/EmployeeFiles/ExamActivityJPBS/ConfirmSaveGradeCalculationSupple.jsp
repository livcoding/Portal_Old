   	...
   	while(itun.hasNext())
   {
	...
	//qry="select enrollmentno,studentname from studentmaster where studentid='"+mStudidInit+"' and nvl(DEACTIVE,'N')='N' ";FINAL EXAM
	 ...
	 qry2=" select  c.studentname studentname ,a.subsectioncode subsectioncode ,c.enrollmentno enrollmentno , c.studentid studentid , a.fstid fstid ,";
					qry2+=" nvl(sum(e.weightage),0)weightage, nvl(sum( round((((d.marksawarded2/e.maxmarks)*e.weightage)),2)),-1) ab0, ";
                                        qry2+=" (select NVL (CEIL (SUM ((y.marksawarded2 / z.maxmarks) * z.weightage)),0) ";
...
if(rsa.next())
{
    ...
    "   from facultysubjecttagging x ,studenteventsubjectmarks y,  exameventsubjecttagging z  where  x.fstid = y.fstid    and x.fstid = z.fstid" +
           ...
           // "   y.eventsubevent not like '%TA%' AND ";
            "   x.subjectid = '"+mSubjectCode+"' And         y.studentid = '"+rs.getString("studentid")+"'";
...
<%}else{
   ...
   TA=Double.parseDouble(rs.getString("T1T2T3Total"))-Double.parseDouble(mMassCut);
...
//else if(Math.ceil(TA+Mrk)<0){
else if(Math.ceil(Mrk)<0){
%>

<td align=center ><%= tempDisplay%></td>      <!--weightageround>-->

		<%
	}else{
                %>
<!----<td align=center ><%= Math.ceil(TA+Mrk)%></td> --->
<td align=center ><%= Math.ceil(Mrk)%></td> 
