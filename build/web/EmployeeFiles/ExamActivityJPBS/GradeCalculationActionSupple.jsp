...
//System.out.print("*****3***********");
String qrtx="" ,xBRK="";
 String del1="",del2="",del3="",del4="",mDEL="",del5="";
 String Qryim="",mFSTIDD="",mSpecial="",qryspe="",qrys="",mREGCONFIRMATIONDATE="",mAcademicYeay="",prevLFSTID="",mLFSTID="",mTypeofaction="",mLTP="";
ResultSet rssx=null, rsim=null,rsBatchDate=null,rse=null,rss=null; int  ctim=0 ;
String qryin="",qryt=""; ResultSet rsst=null;
double mLP=0,mL=0,mPercL=0;
double mMassCutMarks=0,mNetMarks=0,mTATotalMarks=0,mMarksAdd=0;
double xATTENDANCEPERCENTAGE=0,xMASSCUTS=0,xMARKSAWARDED=0,xTATOTAL=0;
String mHead="",Rrevied1="",mprog="" ,StudidOL="''",StudidOLAP="''" ,StudidOLFP="''" ;
String mSubJ="",mEmplo="";
String  Qryapim=""; ResultSet rsiim=null; int APIMC=0,ktm=0;
...
function Copy()
        {
            ...
            if(document.frm.Copythis.checked==true)
            {
alert(" Initial boundaries will be copied as Revised  boundary ");
             
                document.frm.LOWERLIMIT1.value=document.frm.chk1.value;
                document.frm.LOWERLIMIT2.value=document.frm.chk2.value;
                document.frm.LOWERLIMIT3.value=document.frm.chk3.value;

                document.frm.LOWERLIMIT4.value=document.frm.chk4.value;
                document.frm.LOWERLIMIT5.value=document.frm.chk5.value;
                document.frm.LOWERLIMIT6.value=document.frm.chk6.value;
				 document.frm.LOWERLIMIT7.value=document.frm.chk7.value;
				  document.frm.LOWERLIMIT8.value=document.frm.chk8.value;
				   document.frm.LOWERLIMIT9.value=document.frm.chk9.value;
			...
			document.frm.LOWERLIMIT1.value='';
                document.frm.LOWERLIMIT2.value='';
                document.frm.LOWERLIMIT3.value='';

                document.frm.LOWERLIMIT4.value='';
                document.frm.LOWERLIMIT5.value='';
                document.frm.LOWERLIMIT6.value='';
				 document.frm.LOWERLIMIT7.value='';
				  document.frm.LOWERLIMIT8.value='';
				   document.frm.LOWERLIMIT9.value='';
...
String mExamCode="",mPREVEXAMCODE="";
int mSno=0;
String mName="";
String mSubjectCode="";
...
if (session.getAttribute("pxFSTID")==null)
{
	xpxFSTID="";
}
else
{
	xpxFSTID=session.getAttribute("pxFSTID").toString().trim();
}


session.setAttribute("pxFSTID",xpxFSTID);
...
{
	mDEL=request.getParameter("DELETE").toString().trim();
}
...
%>
 <form name="frm"  method="post" action="GradeCalculationActionSupple.jsp" >
...
<%
	...
	if(request.getParameter("ExamCode")==null)
        ...
        // if(request.getParameter("PREVEXAMCODE")==null)
		//mPREVEXAMCODE="";
	//else
		//mPREVEXAMCODE=request.getParameter("PREVEXAMCODE").toString().trim();

   if (session.getAttribute("PREVEXAMCODE")==null)
           {
	mPREVEXAMCODE="";
           }
       else
          {
	mPREVEXAMCODE=session.getAttribute("PREVEXAMCODE").toString().trim();
          }
...
if(request.getParameter("BRK")==null)
		xBRK="";
	else
		xBRK=request.getParameter("BRK").toString().trim();
...
del2="delete from gradecalculationgrades where BREAK#SLNO='"+xBRK+"' and examcode='"+mExamCode+"'  ";
 int a2=db.update(del2);

 del1="delete from gradecalculation where BREAK#SLNO='"+xBRK+"' and examcode='"+mExamCode+"'   ";
 int a1=db.update(del1);

del3="delete from EX#GRADESUBJECTBREAKUP where BREAK#SLNO='"+xBRK+"'  and FSTID IN ("+lFSTDL+")";
 int a3=db.update(del3);
del4="delete from studentwisegrade where  FSTID IN ("+lFSTDL+")  and BREAK#SLNO='"+xBRK+"' and examcode='"+mExamCode+"' ";
 int a4=db.update(del4);

	del5="UPDATE EXAMEVENTSUBJECTTAGGING	 set LOCKED='N' WHERE FSTID IN ("+lFSTDL+") ";
	del5=del5+" AND nvl(PUBLISHED,'N')='N' and nvl(PROCEEDSECOND,'N')='N' AND ";
	del5=del5+" nvl(DEACTIVE,'N')='N' ";
 int a5=db.update(del5);
 // out.print(del4);
	...
	qryCAl="select  distinct NVL((select  Distinct NVL(CALCULATIONVALUE,0)CALCULATIONVALUE from grademaster where examcode='"+mExamCode+"' and  grade ='A+' and institutecode='"+mInst+"' and programcode='"+mprog+"' and academicyear='"+mACAD+"'),0) CALAP,NVL( (select  Distinct NVL(CALCULATIONVALUE,0)CALCULATIONVALUE from grademaster where examcode='"+mExamCode+"' and  grade ='F' and institutecode='"+mInst+"' and programcode='"+mprog+"' and academicyear='"+mACAD+"' ),0)  CALF from grademaster where examcode='"+mExamCode+"' and institutecode='"+mInst+"'  and programcode='"+mprog+"' and academicyear='"+mACAD+"'" ;
...
%>
	<input type=hidden name="ExamCode" id="ExamCode" value="<%=mExamCode%>">
        <input type=hidden name="PREVEXAMCODE" id="PREVEXAMCODE" value="<%=mPREVEXAMCODE%>">
...
qry2=qry2+" and a.subjectID='"+mSubjectCode+"' And c.studentid = a.studentid            and c.fstID = a.FstID  and nvl(a.DEACTIVE,'N')='N' and ";
...
qry2=qry2+"  having (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END) <='"+CALVALUEF+"'   ";
...
/*qry2="select a.fstid,b.ltp, a.institutecode,a.ExamCode,NVL( A.SEMESTER,0)SEMESTER,a.semestertype,a.subjectID,a.studentid studentid,";
qry2=qry2+" a.enrollmentno,NVL (c.Masscuts, 0) Masscuts,  NVL ( ( (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE                      WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL   ELSE c.Masscuts     END)), 0   )              marksawarded2 , ";
qry2=qry2+" a.studentname,NVL(sum(b.weightage),0)weightage from V#STUDENTEVENTSUBJECTMARKS a, ";
qry2=qry2+" V#EXAMEVENTSUBJECTTAGGING b , STUDENTGRADEMASSCUTS c where  ";
qry2=qry2+" a.fstid in("+mCheckFstid+") ";
...
qry2=qry2+" nvl(a.LOCKED,'N')='Y' and a.subjectID=b.subjectID and nvl(a.DEACTIVE,'N')='N' ";
qry2=qry2+" and a.fstid=b.fstid AND c.studentid = a.studentid            AND c.fstID = a.FstID   AND A.STUDENTID NOT IN (SELECT STUDENTID FROM DEBARSTUDENTDETAIL WHERE EXAMCODE='"+mExamCode+"' AND NVL(MEDICALWITHDRAWAL,'N')='Y')";
qry2=qry2+" group by a.fstid ,b.ltp, a.institutecode,a.ExamCode,a.Semester,a.semestertype,a.subjectID,a.studentid, ";
qry2=qry2+" a.enrollmentno,a.studentname ,           c.Masscuts,           c.TATOTAL";*/

//    ----------------------------------------------------------------------------------------------------------------

qry2="select a.fstid,b.ltp, a.institutecode,a.ExamCode,a.Semester,a.semestertype,a.subjectID,a.studentid studentid,";
...
while(rs2.next())
{
	...
	mLTP=rs2.getString("ltp");
	mFSTIDD=rs2.getString("fstid"); 
 
qrtx=" select nvl(ATTENDANCEPERCENTAGE,0) ATTENDANCEPERCENTAGE ,NVL ( ( (CASE WHEN (TATOTAL - Masscuts) < 0 THEN TATOTAL  ELSE  Masscuts END)),0  )MASSCUTS ,nvl(MARKSAWARDED,0)MARKSAWARDED,nvl(TATOTAL,0)TATOTAL from STUDENTGRADEMASSCUTS where fstid='"+mFSTIDD+"' and studentid='"+Studid+"' and examcode='"+mExamCode+"' and institutecode='"+mInst+"' ";
//out.print("********111111111111**********");
rssx=db.getRowset(qrtx);
//out.print("**********33333333********");
//out.print(qrtx);
if(rssx.next())
{
...
mPercL=rssx.getDouble("ATTENDANCEPERCENTAGE");
//out.print("*********444444*********");
	mMassCutMarks=rssx.getDouble("MASSCUTS");
	//out.print("*******55555***********");
	xMARKSAWARDED=rssx.getDouble("MARKSAWARDED");
	//out.print("********66666**********");
	xTATOTAL=rssx.getDouble("TATOTAL");
	//out.print("*********777777*********");
...
//Start masscur  insert in masscuts
...
qryin="INSERT INTO STUDENTGRADEMASSCUTS (  INSTITUTECODE, FSTID, STUDENTID, MASSCUTS,    ENTRYDATE, ENTRYBY, DEACTIVE,    MARKSAWARDED,   ATTENDANCEPERCENTAGE) VALUES ('"+mInst+"','"+mFSTIDD +"' ,'"+Studid+"' ,'"+mMassCutMarks+"' ,sysdate    ,'"+mChkMemID+"' , 'N', '"+mMarksawarded2+"'   ,'"+mPercL+"' ) ";
//out.print(qryin);
 int Ins=db.insertRow(qryin);
}else{

qryin="UPDATE STUDENTGRADEMASSCUTS SET    MASSCUTS  = '"+mMassCutMarks+"' , MARKSAWARDED='"+mMarksawarded2+"' , ATTENDANCEPERCENTAGE='"+mPercL+"',entrydate=sysdate  WHERE  FSTID = '"+mFSTIDD+"'  AND    STUDENTID  = '"+Studid+"' ";
...
%>  
	...
	<TR>
		...
		<TD ALIGN=CENTER><b>Revised Range (%) of marks<FONT SIZE="4" COLOR="black">&nbsp;<sup>+</sup></FONT>(<input type=checkbox name="Copythis" id="Copythis"  onclick="Copy();" ><sup>Copy Initial Range</sup>) </TD>
...
qry2=qry2+" group by a.institutecode,a.ExamCode,a.Semester,a.semestertype,a.subjectID,a.studentid, ";
qry2=qry2+" a.enrollmentno,a.studentname,c.Masscuts,c.TATOTAL ";
qry2=qry2+"  having  (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  >="+mCalValue+"  ";
rs2=db.getRowset(qry2);
...
qry21=qry21+" a.enrollmentno,NVL(c.Masscuts,0)Masscuts, NVL ( ( (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage))) - (CASE                     WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL  ELSE c.Masscuts  END)),  0  )  marksawarded2 , ";
qry21=qry21+" a.studentname,NVL(sum(b.weightage),0)weightage from V#STUDENTEVENTSUBJECTMARKS a, ";
qry21=qry21+" V#EXAMEVENTSUBJECTTAGGING b , STUDENTGRADEMASSCUTS c where  ";
...
qry21=qry21+" and a.subjectID='"+mSubjectCode+"'  And c.studentid = a.studentid            and c.fstID = a.FstID  and nvl(a.DEACTIVE,'N')='N' and ";
...
qry21=qry21+" a.enrollmentno,a.studentname,c.Masscuts,c.TATOTAL";
qry21=qry21+"  having  (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  >="+mCalValue+"   ";
...
%>
<TD ALIGN=CENTER><%=Marks80%><INPUT TYPE="hidden"  id="chk1" NAME="chk1" value="<%=Marks80%>"></td>
...
qry2="SELECT    institutecode, examcode, NVL(semester,0)semester, semestertype,subjectid,         studentid , enrollmentno,        NVL(marksawarded2,0)marksawarded2 ,         studentname, NVL(weightage,0)weightage  FROM (         SELECT    a.institutecode, a.examcode, NVL(A.semester,0)semester, a.semestertype, a.subjectid,         a.studentid studentid, a.enrollmentno,      NVL ( ( (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage))) - (CASE                     WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL  ELSE c.Masscuts  END)),  0  )  marksawarded2  ,         a.studentname, NVL(SUM (b.weightage),0) weightage    FROM v#studenteventsubjectmarks a, v#exameventsubjecttagging b ,STUDENTGRADEMASSCUTS c   WHERE  a.fstid in("+mCheckFstid+")        AND a.examcode ='"+mExamCode+"'  and A.studentid not in (select studentid from debarstudentdetail where examcode='"+mExamCode+"' and  nvl(MEDICALWITHDRAWAL,'N')='Y' AND fstid IN ( "+mCheckFstid+" ))   AND a.examcode = b.examcode     AND a.eventsubevent = b.eventsubevent     AND a.studentid = b.studentid     AND a.subjectid = '"+mSubjectCode+"'   And c.studentid = a.studentid            and c.fstID = a.FstID   AND NVL (a.deactive, 'N') = 'N'     AND NVL (a.LOCKED, 'N') = 'Y'     AND a.subjectid = b.subjectid     AND NVL (a.deactive, 'N') = 'N'    AND a.fstid = b.fstid   ";
...
qry2=qry2+" GROUP BY a.institutecode,  a.examcode,   a.semester,  a.semestertype,  a.subjectid,   a.studentid,         a.enrollmentno,         a.studentname,c.Masscuts,c.TATOTAL  HAVING (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  < '"+CALVALUEAP+"'     AND (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  > '"+CALVALUEF+"'     ORDER BY marksawarded2 DESC) WHERE ROWNUM<= "+Count10+" ORDER BY marksawarded2";
rs2=db.getRowset(qry2);
 //out.print(qry2);
...
//out.print(Count10+"lkhgdfhgjkdfgyhdfiogy"); i m here *

//INTIAL COUNT 
qry="SELECT    institutecode, examcode, NVL(semester,0)semester, semestertype,subjectid,         studentid , enrollmentno,        NVL(marksawarded2,0)marksawarded2 ,         studentname, NVL(weightage,0)weightage  FROM (   SELECT    a.institutecode, a.examcode, NVL(A.semester,0)semester, a.semestertype, a.subjectid,         a.studentid studentid, a.enrollmentno,        NVL(c.Masscuts,0)Masscuts ,NVL ( ( (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage))) - (CASE                     WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL  ELSE c.Masscuts  END)),  0  )  marksawarded2 ,         a.studentname, NVL(SUM (b.weightage),0) weightage    FROM v#studenteventsubjectmarks a, v#exameventsubjecttagging b ,STUDENTGRADEMASSCUTS c   WHERE  a.fstid in("+mCheckFstid+")    and A.studentid not in (select studentid from debarstudentdetail where examcode='"+mExamCode+"' and  nvl(MEDICALWITHDRAWAL,'N')='Y' AND fstid IN ( "+mCheckFstid+" ))   AND a.examcode ='"+mExamCode+"'     AND a.examcode = b.examcode     AND a.eventsubevent = b.eventsubevent     AND a.studentid = b.studentid     AND a.subjectid = '"+mSubjectCode+"'   And c.studentid = a.studentid           and c.fstID = a.FstID    AND NVL (a.deactive, 'N') = 'N'     AND NVL (a.LOCKED, 'N') = 'Y'     AND a.subjectid = b.subjectid     AND NVL (a.deactive, 'N') = 'N'    AND a.fstid = b.fstid  ";
...
qry=qry+" GROUP BY a.institutecode,         a.examcode,         a.semester,         a.semestertype,         a.subjectid,         a.studentid,         a.enrollmentno,         a.studentname,c.Masscuts,c.TATOTAL  HAVING (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  <  '"+CALVALUEAP+"'     AND (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  > '"+CALVALUEF+"'     AND (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  >= "+Marks10+" AND (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  < "+Marks80+"   ORDER BY marksawarded2 DESC) ORDER BY marksawarded2   ";
// out.print(qry); i m here now
rs=db.getRowset(qry);
while(rs.next())
			{
CountA10++;
...
<TD ALIGN=CENTER> <%=Marks10%><INPUT TYPE="hidden"  id="chk2" NAME="chk2" value="<%=Marks10%>"></td>
...
qry2=qry2+" GROUP BY a.institutecode,         a.examcode,         a.semester,         a.semestertype,         a.subjectid,         a.studentid,         a.enrollmentno,         a.studentname,c.Masscuts,c.TATOTAL  HAVING (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  < '"+CALVALUEAP+"'     AND (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  > '"+CALVALUEF+"'   and   (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  < "+Marks10+" ORDER BY marksawarded2 DESC) WHERE ROWNUM<= "+CountB20+" ORDER BY marksawarded2";
rs2=db.getRowset(qry2);
 //out.print(qry2); here now
while(rs2.next())
{
	PerB20++;
...
qry=qry+" GROUP BY a.institutecode,         a.examcode,         a.semester,         a.semestertype,         a.subjectid,         a.studentid,         a.enrollmentno,         a.studentname ,c.Masscuts,c.TATOTAL  HAVING  (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)   < '"+CALVALUEAP+"'     AND  (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)   > '"+CALVALUEF+"'     AND  (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  >= "+MarksB20+" AND  (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)   < "+Marks10+" ORDER BY marksawarded2 DESC) ORDER BY marksawarded2";
//out.print(qry);
rs=db.getRowset(qry);
while(rs.next())
			{
...
<TD ALIGN=CENTER>  <%=MarksB20%><INPUT TYPE="hidden"  id="chk3" NAME="chk3" value="<%=MarksB20%>"></td>
...
qry2="SELECT    institutecode, examcode, NVL(semester,0)semester, semestertype,subjectid,         studentid , enrollmentno,        ceil(NVL(marksawarded2,0))marksawarded2,         studentname, NVL(weightage,0)weightage  FROM (         SELECT    a.institutecode, a.examcode, NVL(a.semester,0)semester , a.semestertype, a.subjectid,         a.studentid studentid, a.enrollmentno,       NVL( c.Masscuts,0)Masscuts, NVL ( ( (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage))) - (CASE                     WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL  ELSE c.Masscuts  END)),  0  )  marksawarded2 ,         a.studentname, NVL(SUM (b.weightage),0) weightage    FROM v#studenteventsubjectmarks a, v#exameventsubjecttagging b  , STUDENTGRADEMASSCUTS c WHERE  a.fstid in("+mCheckFstid+")      and A.studentid not in (select studentid from debarstudentdetail where examcode='"+mExamCode+"' and  nvl(MEDICALWITHDRAWAL,'N')='Y' AND fstid IN ( "+mCheckFstid+" ))   AND a.examcode ='"+mExamCode+"'     AND a.examcode = b.examcode     AND a.eventsubevent = b.eventsubevent     AND a.studentid = b.studentid     AND a.subjectid = '"+mSubjectCode+"'  And c.studentid = a.studentid            and c.fstID = a.FstID     AND NVL (a.deactive, 'N') = 'N'     AND NVL (a.LOCKED, 'N') = 'Y'     AND a.subjectid = b.subjectid     AND NVL (a.deactive, 'N') = 'N'    AND a.fstid = b.fstid    ";


 qry2=qry2+"AND NOT EXISTS ( SELECT 'S' FROM debarstudentdetail t                       WHERE t.examcode = a.examcode                         AND t.institutecode = a.institutecode                         AND t.subjectid = a.subjectid                         AND a.studentid = t.studentid                         and t.GRADE IN ('F','I') )";

qry2=qry2+"GROUP BY a.institutecode,         a.examcode,         a.semester,         a.semestertype,         a.subjectid,         a.studentid,         a.enrollmentno,         a.studentname , c.Masscuts,c.TATOTAL  HAVING (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  < '"+CALVALUEAP+"'     AND (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  > '"+CALVALUEF+"'   and   (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  < "+MarksB20+" ORDER BY marksawarded2 DESC) WHERE ROWNUM<= "+CountB25+" ORDER BY marksawarded2";

rs2=db.getRowset(qry2);
//out.print(qry2); here to here
while(rs2.next())
{
	PerB25++;
...
qry="SELECT    institutecode, examcode, NVL(semester,0)semester, semestertype,subjectid,         studentid , enrollmentno,        ceil(NVL(marksawarded2,0))marksawarded2,         studentname,  NVL(weightage,0)weightage FROM (         SELECT    a.institutecode, a.examcode, NVL(a.semester,0)semester , a.semestertype, a.subjectid,         a.studentid studentid, a.enrollmentno,NVL( c.Masscuts,0)Masscuts,NVL ( ( (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage))) - (CASE                     WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL  ELSE c.Masscuts  END)),  0  )  marksawarded2 ,         a.studentname, nvl(SUM (b.weightage),0) weightage    FROM v#studenteventsubjectmarks a, v#exameventsubjecttagging b  , STUDENTGRADEMASSCUTS c  WHERE  a.fstid in("+mCheckFstid+")    and A.studentid not in (select studentid from debarstudentdetail where examcode='"+mExamCode+"' and  nvl(MEDICALWITHDRAWAL,'N')='Y' AND fstid IN ( "+mCheckFstid+" ))   AND a.examcode ='"+mExamCode+"'     AND a.examcode = b.examcode     AND a.eventsubevent = b.eventsubevent     AND a.studentid = b.studentid     AND a.subjectid = '"+mSubjectCode+"'  And c.studentid = a.studentid            and c.fstID = a.FstID    AND NVL (a.deactive, 'N') = 'N'     AND NVL (a.LOCKED, 'N') = 'Y'     AND a.subjectid = b.subjectid     AND NVL (a.deactive, 'N') = 'N'    AND a.fstid = b.fstid  ";
...
qry=qry+" GROUP BY a.institutecode,         a.examcode,         a.semester,         a.semestertype,         a.subjectid,         a.studentid,         a.enrollmentno,         a.studentname,c.Masscuts,c.TATOTAL  HAVING  (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  < '"+CALVALUEAP+"'     AND  (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  > '"+CALVALUEF+"'     AND  (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)   >= "+MarksB25+" AND  (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  < "+MarksB20+" ORDER BY marksawarded2 DESC) ORDER BY marksawarded2";
//out.print(qry); i m here *****5
rs=db.getRowset(qry);
while(rs.next())
			{
CountB225++;
...
%>
 ...
 <TD ALIGN=CENTER> <%=MarksB25%><INPUT TYPE="hidden"  id="chk4" NAME="chk4" value="<%=MarksB25%>"></td>
...
qry=qry+"	GROUP BY a.institutecode,         a.examcode,         a.semester,         a.semestertype,         a.subjectid,         a.studentid,         a.enrollmentno,         a.studentname,c.Masscuts ,c.TATOTAL HAVING  (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END) < '"+CALVALUEAP+"'     AND  (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END) > '"+CALVALUEF+"'     AND  (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  >= "+MarksC25+" AND  (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END) < "+MarksB25+" ORDER BY marksawarded2 DESC) ORDER BY marksawarded2";
//out.print(qry); here i am
rs=db.getRowset(qry);
while(rs.next())
			{
CountC225++;
...
%>
 ...
 <TD ALIGN=CENTER>   <%=MarksC25%><INPUT TYPE="hidden"  id="chk5" NAME="chk5" value="<%=MarksC25%>"></td>
...
qry2=qry2+" GROUP BY a.institutecode,         a.examcode,         a.semester,         a.semestertype,         a.subjectid,         a.studentid,         a.enrollmentno,         a.studentname, c.Masscuts,c.TATOTAL  HAVING (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  < '"+CALVALUEAP+"'     AND (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  > '"+CALVALUEF+"'   and   (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)  < "+MarksC25+" ORDER BY marksawarded2 DESC) WHERE ROWNUM<= "+CountC15+" ORDER BY marksawarded2";
...
qry=qry+" GROUP BY a.institutecode,         a.examcode,         a.semester,         a.semestertype,         a.subjectid,         a.studentid,         a.enrollmentno,         a.studentname,c.Masscuts,c.TATOTAL  HAVING (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)   < '"+CALVALUEAP+"'     AND (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)   >  '"+CALVALUEF+"'      AND (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)   >= "+MarksC15+" AND (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)   < "+MarksC25+" ORDER BY marksawarded2 DESC) ORDER BY marksawarded2";
 /// out.print("$$$$$$$$$$$$$$AAAAAAAAAA"+qry); i m here ******6
rs=db.getRowset(qry);
while(rs.next())
			{
CountC115++;
...
%>
 ...
 <TD ALIGN=CENTER>   <%=MarksC15%><INPUT TYPE="hidden"  id="chk6" NAME="chk6" value="<%=MarksC15%>"></td>
...
qry2=qry2+" GROUP BY a.institutecode,         a.examcode,         a.semester,         a.semestertype,         a.subjectid,         a.studentid,         a.enrollmentno,         a.studentname, c.Masscuts ,c.TATOTAL  HAVING (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)   < '"+CALVALUEAP+"'     AND (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)   >= '"+CALVALUEF+"'   and   (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)   < "+MarksC15+" ORDER BY marksawarded2 DESC) /* WHERE ROWNUM<= "+CountD5+" */ ORDER BY marksawarded2";

rs2=db.getRowset(qry2);
 //out.print("DFSFS"+qry2); here
while(rs2.next())
{
	PerD5++;
...
qry=qry+" GROUP BY a.institutecode,         a.examcode,         a.semester,         a.semestertype,         a.subjectid,         a.studentid,         a.enrollmentno,         a.studentname,c.Masscuts ,c.TATOTAL  HAVING (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)   < '"+CALVALUEAP+"'     AND (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)   >= '"+CALVALUEF+"'      AND (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage)))  - (CASE  WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL                 ELSE c.Masscuts  END)   < "+MarksC15+" ORDER BY marksawarded2 DESC) ORDER BY marksawarded2";
 //out.print("D"+qry); hererererere i m here ********7
rs=db.getRowset(qry);
while(rs.next())
			{
CountD15++;
...
qryIm="SELECT    institutecode, examcode, nvl(semester,0)semester, semestertype,subjectid,         studentid , enrollmentno,        ceil(nvl(marksawarded2,0))marksawarded2 ,         studentname,  nvl(weightage,0)weightage FROM (         SELECT    a.institutecode, a.examcode,nvl(a.semester,0)semester , a.semestertype, a.subjectid,         a.studentid studentid, a.enrollmentno,nvl(c.Masscuts,0)Masscuts,NVL ( ( (CEIL (SUM ( (a.marksawarded2 / a.maxmarks) * b.weightage))) - (CASE                     WHEN (c.TATOTAL - c.Masscuts) < 0 THEN c.TATOTAL  ELSE c.Masscuts  END)),  0  )  marksawarded2 ,         a.studentname, NVL(SUM (b.weightage),0) weightage     FROM v#studenteventsubjectmarks a, v#exameventsubjecttagging b, STUDENTGRADEMASSCUTS c   WHERE  a.fstid in("+mCheckFstid+")  and A.studentid not in (select studentid from debarstudentdetail where examcode='"+mExamCode+"' and  nvl(MEDICALWITHDRAWAL,'N')='Y' AND fstid IN ( "+mCheckFstid+" ))    AND a.examcode ='"+mExamCode+"'     AND a.examcode = b.examcode     AND a.eventsubevent = b.eventsubevent     AND a.studentid = b.studentid     AND a.subjectid = '"+mSubjectCode+"' And c.studentid = a.studentid            and c.fstID = a.FstID      AND NVL (a.deactive, 'N') = 'N'     AND NVL (a.LOCKED, 'N') = 'Y'     AND a.subjectid = b.subjectid     AND NVL (a.deactive, 'N') = 'N'    AND a.fstid = b.fstid    ";
...
%>
 ...
 <TD ALIGN=CENTER>  <%=CALVALUEF%><INPUT TYPE="hidden"  id="chk7" NAME="chk7" value="<%=CALVALUEF%>"></td>
...
<%
//	if(MarksD5!=0.0 || MarksD5!=0) herererererererr i m 
...
<TD ALIGN=CENTER><%=Marks30%><INPUT TYPE="hidden"  id="chk8" NAME="chk8" value="<%=Marks30%>"></td>
...
<%@ page import="java.lang.*" %>
	...
	<%@ include file="StudentGradeCalculationSupple.jsp"%>
...
out.print("<font color=red size=4>Please Insert Lower Limit in Descending Order</font>");
	...
	%>
 <form name="frm11"  method="post" action="ConfirmSaveGradeCalculationSupple.jsp" >
...
%><TD align=center><IMG SRC="images/arrow_cool.gif" WIDTH="30" HEIGHT="20" BORDER="0" ALT="">  <input type=submit value="Continue"  > 
</TD>
<%
}
%>
...
<form name="frm111"  method="post" action="StudentGrades1Supple.jsp" >
<%
...
<table width=10% align=center border=1 bordercolor=magroon><tr><td align=center nowrap><font color=blue>
