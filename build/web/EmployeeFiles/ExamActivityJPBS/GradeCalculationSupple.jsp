...
int ctr=0;
String mHead="", mExamid="",mPREVEXAMCODE="";
...
//session.setAttribute("PREVEXAMCODE",mPREVEXAMCODE);
...
if(!mMemberID.equals("") && !mMemberCode.equals("") && !mMemberName.equals("")) 
{
   ...
   if (RsChk.next() && RsChk.getString("SL").equals("Y"))
   {
	...
	//**************CHANGE THE QUERY 27/01/2010**************************
		...
		qry+=" nvl(Deactive,'N')='N' AND NVL(LOCKEXAM,'N')='N' and  EXAMPERIODTO >=trunc(sysdate)  and   nvl(SUPFEES,0)>10 ";
		...
		{ 
			...
			while(rs.next())
			{
				mExamid=rs.getString("Exam");
                                  //mPREVEXAMCODE=rs.getString("PREVEXAMCODE");
                                 //  session.setAttribute("PREVEXAMCODE",mPREVEXAMCODE);
				%>
				<OPTION Value =<%=mExamid%>><%=rs.getString("Exam")%></option>
                                <!--- <input type=hidden value=<%=mPREVEXAMCODE%> id="PREVEXAMCODE" name="PREVEXAMCODE">--->
				<%
                                //mExamid=request.getParameter("Exam")==null?"":request.getParameter("Exam");
			}
	...
	if(request.getParameter("xx")!=null)
	{
	   if(request.getParameter("ExamCode")!=null && !request.getParameter("ExamCode").equals("NONE"))
	   {
		...
		mExamid=request.getParameter("ExamCode").toString().trim();
                qry=" Select nvl(PREVEXAMCODE,' ') PREVEXAMCODE from EXAMMASTER Where INSTITUTECODE='"+mInst+"' AND EXAMCODE='"+mExamid+"'";
                rs=db.getRowset(qry);
                if(rs.next())
		{
                mPREVEXAMCODE=rs.getString("PREVEXAMCODE");
...
// to remove TA exam from previous exam 22-10-2019
                                //qry="Select nvl(A.EVENTSUBEVENT,' ') EVENTSUBEVENT,nvl(A.WEIGHTAGE,0) WEIGHTAGE  from V#EXAMEVENTSUBJECTTAGGING A ";
				//qry=qry+" WHERE A.INSTITUTECODE='"+mInst+"' AND A.EXAMCODE='"+mPREVEXAMCODE+"' AND A.SUBJECTID='"+mSC+"' AND  NVL(A.PUBLISHED,'N')='Y'";
				//qry=qry+" and  eventsubevent like '%TA%' Group By A.EVENTSUBEVENT, A.WEIGHTAGE ORDER BY EVENTSUBEVENT,nvl(A.WEIGHTAGE,0)";

                             // ResultSet rsa=db.getRowset(qry);

                               // while(rsa.next()){
                              //  mAllowedWeightage=mAllowedWeightage+rsa.getDouble("WEIGHTAGE");
  ...
  // to remove TA exam from previous exam 22-10-2019
				...
				{
					%>
					<form name="frm1"  method="post" action="GradeCalculationActionSupple.jsp" >
...
%>
<form name="frm11"  method="post" action="FreezeGradeSupple.jsp" >
					<input id="xx" name="xx" type=hidden>
...
<TABLE width="75%" class="sort-table" id="table-1"   ALIGN=center rules=COLUMNS CELLSPACING=0   BORDER=1> 
<thead>
<tr bgcolor="#ff8c00">
...
qryi=" SELECT distinct a.fstid FROM STUDENTWISEGRADE A,V#STUDENTEVENTSUBJECTMARKS  B where A.BREAK#SLNO IN ( select BREAK#SLNO from  EX#GRADESUBJECTBREAKUP  where fstid IN ( select distinct fstid from ( select distinct fstid from ";
...
%><input type=hidden value="<%=mfst%>" id="mFSTD" name="mFSTD">
		...
		<tr><td COLSPAN=6 ALIGN=CENTER><input type=submit value="FREEZE GRADE" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="GradeCalculationActionSupple.jsp?ACAD=<%=xAcad%>&amp;prog=<%=xProg%>&amp;BRK=<%=xBrkSlno%>&amp;EXAM=<%=mEC%>&amp;DELETE=<%=mDEL%>&amp;SUB=<%=mSC%>&amp;FSTIDL=<%=mfst%>&amp;ETOD=<%=mETOD%>"><font color=blue>Re-Evaluate the students Grade</font></a></td></tr></tbody></table>
		<script type="text/javascript">
	var st1 = new SortableTable(document.getElementById("table-1"),[ "Number","Number","CaseInsensitiveString","Number","Number","CaseInsensitiveString"]);
	</script><BR><BR>
</form>
<%
					...
					<form name="frm11"  method="post" action="DeleteGradeCalculationSupple.jsp" >
					<input id="xx" name="xx" type=hidden>
					<input type=hidden value=<%=mInst%>id="InstCode" name="InstCode">
					<input type=hidden value=<%=mExamid%> id="ExamCode" name="ExamCode">
					<input id="z" name="z"  type=hidden>
