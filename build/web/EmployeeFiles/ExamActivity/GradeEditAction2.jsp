...
String bno="";
int total=0;
String studentid="",fstid="";
String total1="";
String studentid1="",fstid1="";
String finalgrade="";
String finalmarks="";
...
{
    total1=request.getParameter("Count");
}
  ...
  //-----------------------------
	...
	if (RsChk.next() && RsChk.getString("SL").equals("Y"))
	{
        ...
        <table width="100%" ALIGN=CENTER bottommargin=0  topmargin=0>
            <tr>
                <TD colspan=0 align=middle>
                    <font color="#a52a2a" style="FONT-SIZE: large; FONT-FAMILY: fantasy"><b>Grade Sheet</b></font>
                </TD>
            </tr>
        </table>
        ...
        <TR>
            ...
            <TD nowrap ><b>&nbsp; Subject Code :</b><%=mSC%></TD>
        </TR>
        </TABLE>
        <table align=center>
            <!-- <tr>
        ...
        for(int i=1;i<=total;i++)
            {
              if (request.getParameter("studentid" + i) == null) {
                        studentid1 = "";
                    } else {
                        studentid1 = request.getParameter("studentid" + i);
                    }

                    if (request.getParameter("fstid" + i) == null) {
                        fstid1 = "";
                    } else {
                        fstid1 = request.getParameter("fstid" + i);
                    }

                    if (request.getParameter("update" + i) == null) {
                        finalmarks = "";
                    } else {
                        finalmarks = request.getParameter("update" + i);
                    }

                    if (request.getParameter("gradeupdate" + i) == null) {
                        finalgrade = "";
                    } else {
                        finalgrade = request.getParameter("gradeupdate" + i);
                    }
                    //out.print(studentid1+"**"+fstid1+"**"+finalmarks+"**"+finalgrade);

              qry="update STUDENTWISEGRADE set INITIALMARKS='"+finalmarks+"'," +
                      "INITIALGRADE='"+finalgrade+"', FINALMARKS='"+finalmarks+"'," +
                      " FINALGRADE='"+finalgrade+"' where FSTID='"+fstid1+"'AND " +
                      "BREAK#SLNO='"+bno+"'AND STUDENTID='"+studentid1+"'";
              int n=db.update(qry);
              //out.print(qry);
            }
        %>
