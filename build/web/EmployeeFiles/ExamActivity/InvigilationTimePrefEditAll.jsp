...
String mDate1="",mDate2="",mDate3="",mDate4="";
String mCurDate1="";
String mTmpTime1="";
String mTmpTime2="";
String AprStatus="",EmployeeId="";
String mPREFTO="",mPREFROM="";
...
function fn_DateCompare()
{
    ...
    var mDate1=document.getElementById("TXT1").value;
     var mDate2=document.getElementById("TXT2").value;
	 var mDate3=document.getElementById("TXT3").value;
     var mDate4=document.getElementById("TXT4").value;
	// alert("111111111111111111111111"+mDate3);
     
     if ((new Date(mDate2).getTime()) >= (new Date(mDate1).getTime()))
     {
     //alert('correct');
     }
     else
     {
     alert('Please Enter From Date is Less than or equal to To Date');
     return false;
     }
}
...
String mPREFROMT="",mPREFTOT="";
	

String  mPrefFromDate1="",mPrefToDate2="",mchecked="",mchecked1="",mRemarks="",EmpName="";
...
if(request.getParameter("EmployeeId")==null)
	...
	EmployeeId=request.getParameter("EmployeeId").toString().trim();
...
if(request.getParameter("EmpName")==null)
	...
	EmpName=request.getParameter("EmpName").toString().trim();
...
if(request.getParameter("PREFROM")==null)
			{
	mPREFROM="";
	
			}
else{
	mPREFROM=request.getParameter("PREFROM").toString().trim();
}


if(request.getParameter("PREFTO")==null)
	mPREFTO="";
else
			{
	mPREFTO=request.getParameter("PREFTO").toString().trim();
			}



if(request.getParameter("PREFROMT")==null)
	mPREFROMT="";
else
	mPREFROMT=request.getParameter("PREFROMT").toString().trim();


if(request.getParameter("PREFTOT")==null)
	mPREFTOT="";
else
	mPREFTOT=request.getParameter("PREFTOT").toString().trim();

//out.print("---0--"+mTempPREFROM);
qry="SELECT INSTITUTECODE, COMPANYCODE, EXAMCODE, INVIGILATORTYPE, INVIGILATORID,to_char(PREFROMDATE,'dd-mm-yyyy')PREFROMDATE,to_char(PRETODATE,'dd-mm-yyyy')PRETODATE, nvl(DUTYHOLIDAY,'N')DUTYHOLIDAY,  to_char(PREFFROMTIME,'hh:mi PM')PREFFROMTIME, to_char(PRETOTIME,'hh:mi PM')PRETOTIME, to_char(PREFFROMTIME,'DD-MM-YYYY')PREFFROMTIMEDATE, to_char(PRETOTIME,'DD-MM-YYYY')PRETOTIMEDATE, NVL(REMARKS,'N')REMARKS, APPROVED,  ENTRYBY, ENTRYDATE, APPROVEDBY,  APPROVEDDATE, DEACTIVE FROM INVIGILATIONTIMEPREF WHERE INSTITUTECODE='"+mInst+"' and COMPANYCODE='"+mComp+"' AND INVIGILATORID='"+EmployeeId+"' and EXAMCODE='"+mExamCode+"' AND TO_CHAR(entrydate,'DD-MM-YYYY')='"+EntryDate+"' and to_char(PREFROMDATE,'dd-mm-yyyy')='"+mPREFROM+"' and  to_char(PRETODATE,'dd-mm-yyyy')='"+mPREFTO+"' ";
//out.print(qry);
rs=db.getRowset(qry);
if(rs.next())
{
	...
	mPrefFromTime=mPREFROMT;
	mPrefToTime=mPREFTOT;
	mDutyHoliday=rs.getString("DUTYHOLIDAY").toString().trim();
	mRemarks=rs.getString("REMARKS").toString().trim();
}
//out.print(mPrefToDate2+"::"+mPrefFromDate1);
//out.print("---1--"+mTempPREFROM);
%>
<form name="frm"  method="get" >
<input id="ExamCode" name="ExamCode" value="<%=mExamCode%>" type=hidden>
<input id="EmpName" name="EmpName" value="<%=EmpName%>" type=hidden>
...
<table align=center width="100%" bottommargin=0 topmargin=0>
 <tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: verdana"><b><u><FONT SIZE=4>Edit Leave/Time Prefernce on Invigilation Duty [ALL]</FONT></u></b></font></td></tr>
</table>
...
<tr><td>
<FONT color=black face=Arial size=2><b>Employee Name : <INPUT TYPE="text" NAME="Emp" value="<%=EmpName%>"></b></FONT>
...
if(!mDutyHoliday.equals("Y"))
	...
	if (request.getParameter("x")!=null)
	{
		...
		mDate4=request.getParameter("TXT4").toString().trim();
...
<input Name=TXT1 Id=TXT1 Type=text maxlength=10 size=10 value='<%=mPREFROM%>'> to 
<input Name=TXT2 Id=TXT2 Type=text Value='<%=mPREFTO%>' maxlength=10 size=10></b>
<input Name=TXT3 Id=TXT3 Type=hidden maxlength=10 size=10 value='<%=mPREFROM%>'> to 
<input Name=TXT4 Id=TXT4 Type=hidden Value='<%=mPREFTO%>' maxlength=10 size=10></b>
...
<%=mRemarks%>	</TextArea>
<FONT color=red>*</FONT>
<INPUT id=submit1 style="FONT-WEIGHT: bold; FONT-SIZE: smaller; FLOAT: none; WIDTH: 50px; HEIGHT: 23px; FONT-VARIANT: normal" type=submit size=5 value="Update" onclick="return fn_DateCompare();" name=submit1></td></tr>
</table>
</form>
<%
...
if(request.getParameter("x")!=null)
	{
		...
		if (request.getParameter("TXT2")!=null)
			...
			mDate2="";
		if (request.getParameter("TXT3")!=null)
			mDate3=request.getParameter("TXT3").toString().trim();
		else
			mDate3="";
		if (request.getParameter("TXT4")!=null)
			mDate4=request.getParameter("TXT4").toString().trim();
		else
			mDate4="";
	//}
				...
				if(!mtext.equals(""))
				{
					mtext=GlobalFunctions.replaceSignleQuot(mtext);
						...
						if (mDutyHoliDay.equals("Y"))
						{
							...
							qry="UPDATE INVIGILATIONTIMEPREF SET          PREFROMDATE     = to_date('"+mDate1+"','dd-mm-yyyy'),       PRETODATE       = to_date('"+mDate2+"','dd-mm-yyyy'),       DUTYHOLIDAY     = 'Y',           REMARKS         = '"+mtext+"' WHERE  INSTITUTECODE   = '"+mInst+"' AND    EXAMCODE        ='"+mExam+"' AND    INVIGILATORID   = '"+EmployeeId+"' and  PREFROMDATE     = to_date('"+mDate3+"','dd-mm-yyyy') and  PRETODATE =to_date('"+mDate4+"','dd-mm-yyyy')";
						...
						else
						{
				 		...
				 		try
					 	{
								...
								//out.print(qry);
								int n=db.update(qry);
							//out.print(qry+"-----1"+n+"---2---"+mDate3+"---3--"+mDate4);

							//response.sendRedirect("InvigilationStatus.jsp");
							//out.print(qry);
		   // Log Entry
  		   //-----------------
	...
	qry=qry+" Where INSTITUTECODE='"+mInst+"'  and EXAMCODE='"+mExam+"'"; 
	qry=qry+" And INVIGILATORID='"+EmployeeId+"' AND  nvl(DEACTIVE,'N')='N'";
    	rs= db.getRowset(qry);
		//out.print(qry);
	%>
