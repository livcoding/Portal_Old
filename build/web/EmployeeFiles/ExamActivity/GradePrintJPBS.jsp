...
<%
// Weitage 100 remove Date 26.102018
...
if(!mMemberID.equals("") && !mMemberCode.equals("") && !mMemberName.equals("")) 
   ...
   qry="Select WEBKIOSK.ShowLink('146','"+ mChkMemID+"','"+mChkMType+"','"+mRole+"','"+ mIPAddress +"') SL from dual";
	...
	<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><b>View saved Grades</b></TD>
	</font></td></tr>
	</TABLE>
...
qry="SELECT NVL(EXAMCODE,' ')EXAMCODE FROM EXAMMASTER Where InstituteCode='"+mInst+"' AND NVL(LOCKEXAM,'N')='N' ORDER BY 1 DESC";
		//out.println(qry);
		rs=db.getRowset(qry);
...
<%
int count=0;
qry="select subjectid,BREAK#SLNO,TOTALSTUDENT,decode(NVL(FINALIZED,'N'),'Y','<font color=green>Freezed</font>','N','<font color=red>Draft Saved</font>',NVL(FINALIZED,'N')) status1,decode(NVL(STATUS,'D'),'A','<font color=green>Approved</font>','<font color=red>Pending</font>') status2 from GRADECALCULATION where examcode='"+mEC+"' and institutecode='"+mInst+"' and GRADEFLAG='N' and subjectid='"+mSC+"'";
//out.println(qry);
...
%><TR bgcolor=white>
<TD><B><%=++count%>.</B></TD>
<TD></b><A target=_child HREF="GradePrintActionJPBS.jsp?ExamCode=<%=mEC%>&ETOD=<%=mETOD%>&Subject=<%=mSC%>&bno=<%=rssss.getString("BREAK#SLNO")%>">Batch - <%=count%> </A></b></TD>
