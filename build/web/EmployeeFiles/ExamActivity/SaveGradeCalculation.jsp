<!--Date Modified 16-07-2020 By Anoop---->
<%@ page buffer="1kb" autoFlush="true" language="java" import="java.sql.*,tietwebkiosk.*,java.util.*" %>
<%@ page errorPage="../../CommonFiles/ExceptionHandler.jsp" %> 
<%
// Date 07.01.19 Rollback and Auto commit change and
// delete from STUDENTWISEGRADE qry added  Vivek

String mHead="";
String qrymm=""; ResultSet rsmm=null,rsi=null;
String qryttt=""; ResultSet rsttt=null;
...
String  mMemberID="",mMemberType="",mMemberName="",mMemberCode="",qryll= "";
String mDMemberCode="",mDMemberType="";
String mInst="",mExamCode="",mSubjectCode="",del1="";
...
String mGrade="",mRecommendedFrom="",mRecommendedTo="",GradeMasterLowerLimit="",mFst="",xpxFSTID="";
...
String mSemType="",mSem="",mETOD="N",mSem1="",del2="",del3="";
...
qryss="select subjectid from  subjectmaster where subjectid='"+mSubjectCode+"' and institutecode='"+mInst+"' ";
...
if(rssl.next())
{
 ...
 //out.print(mBreakSlno+"mCheck");
 //System.out.println(mBreakSlno+"mCheck");
for(int i=1;i<=mCheck;i++)
{ 

	mName1="FSTID"+i;
	abc=request.getParameter(mName1);
// out.print(abc+"LLL"+mName1);
...
/// LINK BREAK SERIAL NUMBER



	if(abc.equals("null") || abc.equals("") || abc==null  && !xpxFSTID.equals("") )
{

qryll= " select distinct fstid from facultysubjecttagging where fstid in ("+xpxFSTID+") ";
rsi=db.getRowset(qryll);
  //out.print(qryi);
while(rsi.next())
{
...
// out.print(abc+"LLL"+mName1);
qry6="select 'y' from EX#GRADESUBJECTBREAKUP where FSTID='"+rsi.getString("fstid")+"' and EMPLOYEEID='"+mChkMemID+"'  ";
 //out.print(qry6);
rs6=db.getRowset(qry6);
...
else
{
	...
	if(rsi.getString("fstid").equals("null") || rsi.getString("fstid").equals("") || rsi.getString("fstid")==null)
	{
	...
	else
	{
		...
		qry1="INSERT INTO EX#GRADESUBJECTBREAKUP(FSTID, BREAK#SLNO, ETOD,FACULTYTYPE, EMPLOYEEID) ";
		qry1=qry1+" VALUES ('"+rsi.getString("fstid")+"' ,'"+mBreakSlno+"' ,'"+mETOD+"','"+mFType+"','"+mChkMemID+"' ) ";
		st.addBatch(qry1);
...
del2="delete from gradecalculationgrades where BREAK#SLNO='"+mBreakSlno+"' and examcode='"+mExamCode+"' and INSTITUTECODE='"+mInst+"'  ";
 int a2=db.update(del2);


del1="delete from gradecalculation where BREAK#SLNO='"+mBreakSlno+"' and examcode='"+mExamCode+"' and INSTITUTECODE='"+mInst+"'   ";
 int a1=db.update(del1);

 del3="delete from STUDENTWISEGRADE where BREAK#SLNO='"+mBreakSlno+"' and examcode='"+mExamCode+"' AND DOCMODE='D' and INSTITUTECODE='"+mInst+"'   ";
 int a3=db.update(del3);


 //String tempqry="Insert into STUDENTWISEGRADE (FSTID, BREAK#SLNO, STUDENTID, INSTITUTECODE, EXAMCODE, INITIALMARKS, INITIALGRADE, MASSCUTS, FINALMARKS, FINALGRADE, GRADEFLAG, ENTRYDATE, ENTRYBY, DOCMODE) Values   ('JIIT1806437', '170030001', 'JIIT1501122', 'JIIT', '2018ODDSEM', 51, 'C+', 0, 51, 'B', 'N', TO_DATE('12/17/2018 10:22:51', 'MM/DD/YYYY HH24:MI:SS'), 'UNIV-V00026', 'D')";

//int n=db.insertRow(tempqry);
...
qry2=qry2+" DEVIATION,GRADEFLAG,ENTRYBY,ENTRYDATE,STATUS,FINALIZED , FINALIZEDBY	, FINALIZEDDATE ) ";
qry2=qry2+" VALUES ('"+mInst+"' ,'"+mExamCode+"' ,'"+mSubjectCode+"' ,'"+mBreakSlno+"' , ";
qry2=qry2+" '"+mTotalStudents+"' ,'"+mStudentsRejected+"' ,'"+mStudentsConsidered+"' ,";
qry2=qry2+" '"+mMean+"' ,'"+mInitialAVGP+"' ,'"+mFinalAVGP+"', '"+mDeviation+"','"+mETOD+"','"+mChkMemID+"',SYSDATE,'D','Y','"+mChkMemID+"',sysdate ) ";
st.addBatch(qry2);
}catch(Exception e){
rollbackflag=true;
}
 // out.print(qry2);
   ...
   while(it.hasNext())	
   {
	   //out.print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
		...
		mRecommendedFrom= element.substring(pos111+3,pos211);
		mRecommendedTo=element.substring(pos311+3,pos411);
		GradeMasterLowerLimit=  element.substring(pos11+3,pos111);
		InitialCount=element.substring(pos211+3,pos311);
...
Set mGradeUnChecked=new HashSet();
	...
	///out.print(mGradeUnChecked+"RRRRRRRRRR");
   	...
   	while(itun.hasNext())	
   {
		...
		//System.out.println(mFST+""+mStudidInit+""+mMarksInit+""+mGradInit+""+mWeightageInit+""+mMassCut+"*******" );
 	    while(iti.hasNext())	
		 {

			//System.out.println(iti+"UUUUUUUUU");
...
//System.out.println("RRRRRRRRRRRRRRRRRRRRRR");
                               ...
                               // System.out.println("mStudidInit  is --"+mStudidInit+"mStudidis -->"+mStudid);
...
qryttt=" Select 'Y' from  debarstudentdetail where STUDENTID='"+mStudid+"' and (UFM='Y' OR ABSENTCASE='Y' OR DEBAR='Y') and examcode='"+mExamCode+"' and institutecode='"+mInst+"' and fstid IN  ('"+mfst+"' )";
rsttt=db.getRowset(qryttt);
 //System.out.println(qryttt);
	if(rsttt.next())
	{
mGrad="F";
	}else{
...
qrymm="select nvl(MASSCUTS,'0')masscut from  STUDENTGRADEMASSCUTS where  fstid IN  ('"+mfst+"' ) and  STUDENTID='"+mStudid+"' and examcode='"+mExamCode+"' ";
rsmm=db.getRowset(qrymm);
 //System.out.println(qryttt);
	if(rsmm.next())
	{
mMassCut=rsmm.getString("masscut");
	}else{
	mMassCut="0";
	}
...
rollbackflag=true;
 ...
 //System.out.println(qry4);	//System.out.println(addBatch);	
		break;
	   } // closing of if
  } // closing of while
...
//out.print("****"+mStatus);
if(mStatus!=1 && rollbackflag!=true)

 {
   ...
   db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"Grade Entry ", "ExamCode: "+mExamCode+ " subjectid :  "+mSubjectCode+" MemberID :"+mChkMemID,"NO MAC Address",mIPAddress);
	...
	<br>
                            ...
                            <TR>
                            <td align=center>
                            <!-- <a href="GradeReport.jsp?ExamCode=<%=mExamCode%>&amp;SEMESTER=<%=mSem1%>&amp;INSTITUTECODE=<%=mInst%>&amp;Subject=<%=mSubjectCode%>&amp;TOTALSTUDENTS=<%=mTotalStudents%>&amp;STUDENTREJECTED=<%=mStudentsRejected%>&amp;STUDENTCONSIDERED=<%=mStudentsConsidered%>&amp;MEAN=<%=mMean%>&amp;DEVIATION=<%=mDeviation%>&amp;INITIALAVGP=<%=mInitialAVGP%>&amp;FINALAVGP=<%=mFinalAVGP%>&amp;CHECKRADIO=Y" target=_new > -->
                            <!--
...
<a href="StudentGrades1.jsp?ExamCode=<%=mExamCode%>&amp;Subject=<%=mSubjectCode%>&amp;CHECKRADIO=Y " > 
-->
<font color=green><b>Record Not Saved Partial Entry Data Found  ...<!-- <br>
Click here to view and take print out of result..</b></font></a> -->
</td>
</tr>
</table>
