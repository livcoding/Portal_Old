...
<html>
<head>
<title>#### <%=mHead%> [ Examination Event/Sub Event Tagging by Admin	 ] </title>
			...
			if (GlobalFunctions.iSValidDate(mDate1)==true && GlobalFunctions.iSValidDate(mDate2))
			{
				...
				if(EXPWEIGHTAGE>0 && EXPWEIGHTAGE>=WEIGHTAGE && MAXMARKS>0) 
				{
					...
					if(rs.next())
					{
						...
						qry=qry+" and SUBJECTID NOT IN (SELECT SUBJECTID FROM GRADECALCULATION WHERE examcode='"+mExamID+"' AND  INSTITUTECODE='"+mInst+"' ";
...
*/
						...
						qry=qry+" And BB.ExamCode='"+mExamID+"'  and AA.examcode='"+mExamID+"' ";
						...
						while(rs.next())
						{
							mFstid=rs.getString("FSTID");
                                                        qry="select nvl(max(sequenceno),0)MaxSeqNo from exameventsubjecttagging where fstid='"+mFstid+"'";
                                                        ResultSet rset=null;int mSequenceNo=0;
                                                        rset=db.getRowset(qry);
                                                        if(rset.next())
                                                            mSequenceNo=rset.getInt(1);
                                                        else
                                                            mSequenceNo=0;
                                                        mSequenceNo+=1;
                                                        //out.print(qry+" "+mSequenceNo);
								
							qry1="select A.COORDINATORID COORDINATORID, A.COORDINATORTYPE COORDINATORTYPE ,a.FSTID FSTID from V#EX#SUBJECTGRADECOORDINATOR A, ";
						qry1=qry1+" SUBJECTMASTER B where A.LTP IN ('L','P','E') and  A.EXAMCODE='"+mExamID+"'";
						qry1=qry1+" AND A.INSTITUTECODE='"+mInst+"' and a.fstid='"+mFstid+"' and a.Institutecode=b.Institutecode and a.subjectid=b.Subjectid and A.SUBJECTID='"+mSubj+"' ";
							rs1=db.getRowset(qry1);
						if(rs1.next())
							{

							qry="INSERT INTO EXAMEVENTSUBJECTTAGGING (FSTID, EVENTSUBEVENT, DOUBLEENTRY, FROMDATE, TODATE, WEIGHTAGE, MARKSORPERCENTAGE,MAXMARKS, ENTRYDATE, ENTRYBY, CORDFACULTYID, CORDFACULTYTYPE, SEQUENCENO)";
							qry=qry+" VALUES ('"+rs1.getString("FSTID")+"','"+ mEventCode +"','"+mDualMarks+"',to_date('"+mDate1+"','dd-mm-yyyy'),to_date('"+mDate2+"','dd-mm-yyyy'),"+WEIGHTAGE+",'"+MOM+"',"+ MAXMARKS+",Sysdate,'"+mDMemberID+"','"+rs1.getString("COORDINATORID")+"','"+rs1.getString("COORDINATORTYPE")+"',"+mSequenceNo+")";
						...
						//-----------------
							...
							{
								out.print("<br><img src='../../Images/Error1.jpg'>");
								out.print(" &nbsp;&nbsp;&nbsp <b><font size=3 face='Arial' color='Red'>Subject Coordinator is not Defined</font> <br>");
							}
					}
				}
				else
