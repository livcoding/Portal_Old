...
qry=qry+" AND ( TO_DATE ('" +mFromTime+ "', 'dd-MM-yyyy HH:MI PM') between bookingfromtime     and bookinguptotime )   and  (TO_DATE ('" +mUptoTime+ "', 'dd-MM-yyyy HH:MI PM')between bookingfromtime     and bookinguptotime ) ";
		//	out.print(qry);	
	      RsChk= db.getRowset(qry);
		if (!RsChk.next())
		{
...
qry="SELECT distinct ROOMCODE, BOOKINGPURPOSE, to_char(BOOKINGFROMTIME,'dd-mm-yyyy hh:mi am')BOOKINGFROMTIME,to_char( BOOKINGUPTOTIME,'dd-mm-yyyy hh:mi am') BOOKINGUPTOTIME, BOOKEDBY, BOOKINGDATETIME, b.EMPLOYEENAME,b.EMPLOYEECODE   FROM roombookinginfo a, employeemaster b   WHERE institutecode ='"+mInst+"' and a.BOOKEDBY=b.EMPLOYEEID    AND roomcode = '"+mRoomCode+"'   AND TO_CHAR (bookingfromdate, 'dd-MM-yyyy') = '" +mDate1+ "'   AND TO_CHAR (bookinguptodate, 'dd-mm-yyyy') = '" +mDate2+ "' ";
//AND bookingfromtime = to_date ('"+mFromTime+"','dd-MM-yyyy HH:MI PM')        AND bookinguptotime = to_date ('"+mUptoTime+"','dd-MM-yyyy HH:MI PM')   ";
//out.print(qry);
rs=db.getRowset(qry);
while(rs.next())
					{
