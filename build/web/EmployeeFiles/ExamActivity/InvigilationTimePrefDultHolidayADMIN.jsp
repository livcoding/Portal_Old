...
if(mLoginComp.equalsIgnoreCase("JPBS")){
mNcomp="JPBS";
}else{
mNcomp=mComp;
}
...
function ChangeOptions(exam,DataComboFac,Faculty)
{
  ...
  //alert(DataComboFac.options(1).value);
...
qry=qry+" order by EXAMPERIODFROM DESC ";
	...
	if (request.getParameter("x")==null)
	{
		...
		onChange= "return ChangeOptions(Exam.value,DataComboFac,Faculty)" >
...
%>
<!--********DataComboFac***********-->
<tr><td><FONT color=black face=Arial size=2><b>FAculty Name </b></FONT>
...
qry="select distinct nvl(EMPLOYEEID,' ')Faculty,NVL (employeename || '(' || employeecode || ')',' ') facultyname, ( SELECT  GRADEENTRYEXAMID FROM COMPANYINSTITUTETAGGING                WHERE   institutecode = '"+mInst+"' " +
        "AND  companycode = '"+mLoginComp+"'   )Exam from EMPLOYEEMASTER where " +
        "EMPLOYEETYPE in ('"+mEmpType+"','NTEC') and  nvl(deactive,'N')='N' and CompanyCode='"+mLoginComp+"' ";
	qry=qry+" union  select distinct nvl(A.STAFFID,' ')Faculty, NVL (b.studentname||'('||enrollmentno||')', ' ') facultyname ,a.examcode Exam";
	...
	qry=qry+" union select distinct nvl(b.employeeid,' ')Faculty, NVL (b.employeename||'('||employeecode||')', ' ') facultyname ,a.examcode  Exam";
	...
	{
		%>
		<select name='DataComboFac' tabindex="0" id="DataComboFac" style="WIDTH: 230px">
		<%
		while(rs.next())
		{
...
<%
	qry="select distinct nvl(EMPLOYEEID,' ')Faculty,NVL (employeename || '(' || employeecode || ')',' ') facultyname, ( SELECT  GRADEENTRYEXAMID FROM COMPANYINSTITUTETAGGING       " +
            "         WHERE  GRADEENTRYEXAMID = '"+qryexam+"'     AND " + // AND institutecode = '"+mInst+"'
	...
	qry=qry+" union select distinct nvl(A.STAFFID,' ')Faculty, NVL (employeename || '(' || employeecode || ')',' ') facultyname,a.examcode Exam";
	qry=qry+" from OTHERSTAFFLOADTAGGING A, EMPLOYEEMASTER B where A.STAFFID=B.EMPLOYEEID and NVL (b.deactive, 'N') = 'N' and a.examcode='"+qryexam+"' order by FacultyName";
//out.print(qry);
	rs=db.getRowset(qry);
	%>
					...
					if (!rs.next())
					{
				 		try
					 	{
    							if (mDutyHoliDay.equals("Y"))
							{
				 				...
				 				qry=qry+" VALUES ('"+mInst+"','"+mNcomp+"','"+mExam+"','"+mStaffType+"','"+QryFaculty+"',to_date('"+mDate1+"','dd-mm-yyyy'),to_date('"+mDate2+"','dd-mm-yyyy'),'Y','"+mtext+"','"+mDMemberID+"',SYSDATE)";
							}
							else
							{
						 		...
						 		qry=qry+" VALUES ('"+mInst+"','"+mNcomp+"','"+mExam+"','"+mStaffType+"','"+QryFaculty+"',to_date('"+mDate1+"','dd-mm-yyyy'),to_date('"+mDate2+"','dd-mm-yyyy'),'N',To_date('"+mFromTime+"','dd-mm-yyyy hh:mi PM'), To_date('"+mUptoTime+"','dd-mm-yyyy hh:mi PM'),'"+mtext+"','"+mDMemberID+"',SYSDATE)";
							}
