...
try
{
    ...
    qry=qry+" SP.EXAMCODE||'('||SP.EXAMEVENTCODE||')-'||SP.SPCODE in ("+DSC+") and nvl(SP.Status,'N')='F')";
	 qry=qry+" and EXAMCODE IN (SELECT EXAMCODE FROM  INVIGILATIONDUTY  where  nvl(Status,'N')='F' ) ";
    rs=db.getRowset(qry);
//out.print(qry);
	%>
...
//out.print(mStaffID+"<br>");
	while(rs.next())
	{
		...
		qry="select A.Invigilatorid Invigilatorid, B.DESIGNATIONCODE, C.GRADE,b.DATEOFJOINING from INVIGILATIONDUTYALLOCATION A, EMPLOYEEMASTER B, DESIGNATIONMASTER C WHERE A.institutecode='"+mInst+"' AND ";
		...
		qry=qry+" and to_char(A.DATETIME,'dd-mm-yyyy hh:mi PM')='"+QDateTime+"' and ROOMCODE='"+QRmCode+"' ORDER BY Grade,b.DATEOFJOINING";
		//out.print(qry);
		...
		while(rsDuty.next())
		{
			...
			ctr++;
			    ...
			    mInvgRole2="To Collect  Green Bag and Return  it to Exam Cell";
			    mInvgRole3=" Assign for pre examination announcement";
				
			//out.print(ctr+"::"+mInvgID);
				if(ctr!=3 && (mInvgID.equals(mStaffID)))
				{
					mInvgRole2=" To Collect  Green Bag and Return  it to Exam Cell ,Assign for pre examination announcement ";
				}
