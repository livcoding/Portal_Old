...
ResultSet rsqrtt=null, rs=null,rss=null,rs1=null,rs2=null,rs3=null,rse=null,rsm=null,rsmm=null,rschk1=null ,rsee=null;
	...
	if(!mMemberID.equals("") && !mMemberCode.equals("") && !mMemberName.equals(""))
	{
	  ...
	  //----------------------
        	...
        	<form name="frm" method="get" >
                ...
                <tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: arial"><B>Event/Sub Event wise Students Marks Entry(Re-Exam)</B></TD>
                </font></td></tr>
                </TABLE>
				...
				qry+=" Select nvl(EXAMCODE,' ') GRADEENTRYEXAMID, EXAMPERIODFROM from EXAMMASTER Where INSTITUTECODE='"+mInst+"' AND";
           			qry+=" nvl(Deactive,'N')='N'  and examcode in (Select examcode from facultysubjecttagging Where INSTITUTECODE='"+mInst+"')";
...
/* qry="select distinct EVENTSUBEVENT,ExamCode from V#EXAMEVENTSUBJECTTAGGING  Where INSTITUTECODE='"+mInst+"' AND ";
     ...
     qry="select distinct a.EVENTSUBEVENT,a.ExamCode from V#EXAMEVENTSUBJECTTAGGING a, ExamEventMaster b  Where  a.INSTITUTECODE='"+mInst+"'  And   a.InstituteCode  = b.InstituteCode    And   a.ExamCode       = b.ExamCode     And   a.EventSubEvent  = b.ExamEventCode AND ";
     qry=qry+"  trunc(sysdate) >= trunc(b.ReExamfromdate) and trunc(sysdate) <= trunc(b.ReExamtodate) ";
     //and a.facultytype=DECODE ('E', 'E', 'I', 'E')";
     qry=qry+"  and  nvl(a.STUDENTLTPDEACTIVE,'N')='N'  AND (a.fstid) in ((select fstid from facultysubjecttagging where ";
     //facultytype = DECODE ('E', 'E', 'I', 'E') and
     qry=qry+" employeeid='"+mDMemberID+"' and nvl(deactive,'N')='N' AND (LTP='L' OR LTP='P') AND INSTITUTECODE='"+mInst+"' AND  nvl(PROJECTSUBJECT,'N')='N' ";
     qry=qry+")";
     qry=qry+" UNION select fstid from EX#SUBJECTGRADECOORDINATOR where companycode='"+mComp+"' and institutecode='"+mInst+"' ";
     //and facultytype= DECODE ('E', 'E', 'I', 'E')"
    qry=qry+"  and FACULTYID='"+mDMemberID+"' AND FSTID NOT IN (SELECT FSTID FROM FACULTYSUBJECTTAGGING WHERE PROJECTSUBJECT='Y'))";
     qry=qry+"GROUP BY a.EVENTSUBEVENT,a.ExamCode ORDER BY a.EVENTSUBEVENT";
	...
	%>
                    ...
                    try
                    {
                     qry="select a.EVENTSUBEVENT from V#EXAMEVENTSUBJECTTAGGING a,ExamEventMaster b  Where a.INSTITUTECODE='"+mInst+"'  And   a.InstituteCode  = b.InstituteCode  And   a.ExamCode       = b.ExamCode And   a.EventSubEvent  = b.ExamEventCode  AND TRUNC (SYSDATE) >= TRUNC (b.ReExamfromdate)  AND TRUNC (SYSDATE) <= TRUNC (b.ReExamtodate) ";
...
{/*
	...
	qry="SELECT   subject || ' ( ' || subjectcode || ' )' subject, subjectid, a.examcode, a.eventsubevent  FROM v#exameventsubjecttagging a, ExamEventMaster b   WHERE a.institutecode ='"+mInst+"' AND  ";
                        qry=qry+"    a.InstituteCode  = b.InstituteCode    And   a.ExamCode       = b.ExamCode    And   a.EventSubEvent  = b.ExamEventCode    AND TRUNC (SYSDATE) >= TRUNC (b.ReExamfromdate)     AND TRUNC (SYSDATE) <= TRUNC (b.ReExamtodate)  ";//  AND facultytype = DECODE ('E', 'E', 'I', 'E') ";
                        qry=qry+"  and  nvl(STUDENTLTPDEACTIVE,'N')='N'   AND (fstid) IN ((SELECT fstid   FROM facultysubjecttagging   WHERE ";//facultytype = DECODE ('E', 'E', 'I', 'E')    AND " +
                           qry=qry+" employeeid='"+mDMemberID+"'   AND NVL (deactive, 'N') = 'N'   AND (ltp = 'L' OR ltp = 'P')    AND institutecode ='"+mInst+"' AND  PROJECTSUBJECT='N' ";
                       /* qry=qry+" UNION select fstid from MULTIFACULTYSUBJECTTAGGING where companycode='"+mComp+"' and institutecode='"+mInst+"' and facultytype=decode('"+mDMemberType+"','E','I','E') and employeeid='"+mDMemberID+"' AND FSTID NOT IN (SELECT FSTID FROM FACULTYSUBJECTTAGGING WHERE PROJECTSUBJECT='Y')*/
						qry=qry+" )";
                        qry=qry+" UNION select fstid from EX#SUBJECTGRADECOORDINATOR where companycode='"+mComp+"' and institutecode='"+mInst+"'  AND ";
                        //facultytype = DECODE ('E', 'E', 'I', 'E') and
                       qry=qry+" FACULTYID='"+mDMemberID+"' AND FSTID NOT IN (SELECT FSTID FROM FACULTYSUBJECTTAGGING WHERE PROJECTSUBJECT='Y'))";
                        qry=qry+"GROUP BY subject || ' ( ' || subjectcode || ' )',   subjectid,       a.examcode,    a.eventsubevent";
	//out.print(qry);
...
qryee="select 'Y' from exameventmaster where EXAMEVENTCODE LIKE '"+mEvent+"'  and REEXAMFROMDATE <= TRUNC (SYSDATE) AND REEXAMTODATE >= TRUNC (SYSDATE) and examcode='"+mEC+"'  AND INSTITUTECODE='"+mIC+"'  " ;
    rsee=db.getRowset(qryee);
			 // out.print(qryee);
			if(rsee.next())   {
                        ...
                        {
                            qry="select 'Y' from V#EXAMEVENTSUBJECTTAGGING";
                            qry=qry+" where institutecode='"+mIC+"' and nvl(DEACTIVE,'N')='N'  ";
			...
			if(rsmm.next())
				...
				if(mSelf.equals("Self"))
                        {
                            qrtt="select 'Y' from  STUDENTEVENTSUBJECTMARKS where EVENTSUBEVENT='"+mEvent+"'";
			qrtt=qrtt+"  and nvl(REEXAMLOCKED,'N')='Y' and FSTID in (select FSTID from FacultySubjectTagging where INSTITUTECODE='"+mIC+"' And ExamCode='"+mEC+"' and SubjectID='"+mSC+"' and (LTP='L' OR PROJECTSUBJECT='Y' OR LTP='P') and EMPLOYEEID='"+mDMemberID+"'";
			qrtt=qrtt+" UNION (Select FSTID from FACULTYSUBJECTTAGGING where INSTITUTECODE='"+mIC+"' and COMPANYCODE='"+mComp+"' AND EMPLOYEEID='"+mDMemberID+"' and FSTID IN (Select FSTID from FacultySubjectTagging Where INSTITUTECODE='"+mIC+"' And ExamCode='"+mEC+"' and SubjectID='"+mSC+"' and (LTP='L' OR PROJECTSUBJECT='Y' OR LTP='P')) and facultytype=decode('"+mDMemberType+"','E','I','E')))";
                		}
                        else
                        {
                            qrtt="select 'Y' from STUDENTEVENTSUBJECTMARKS where EVENTSUBEVENT='"+mEvent+"'";
			qrtt=qrtt+" and nvl(REEXAMLOCKED,'N')='Y' AND NVL (REEXAM, 'N') = 'Y' and FSTID in (select FSTID from FacultySubjectTagging where INSTITUTECODE='"+mIC+"' And ExamCode='"+mEC+"' and SubjectID='"+mSC+"' and (LTP='L' OR PROJECTSUBJECT='Y' OR LTP='P') and EMPLOYEEID='"+mDMemberID+"'";
			qrtt=qrtt+" UNION (Select FSTID from EX#SUBJECTGRADECOORDINATOR where INSTITUTECODE='"+mIC+"' and COMPANYCODE='"+mComp+"' AND FACULTYID='"+mDMemberID+"' and FSTID IN (Select FSTID from FacultySubjectTagging Where INSTITUTECODE='"+mIC+"' And ExamCode='"+mEC+"' and SubjectID='"+mSC+"' and (LTP='L' OR PROJECTSUBJECT='Y' OR LTP='P')) and facultytype=decode('"+mDMemberType+"','E','I','E')))";
                        }

  rsqrtt=db.getRowset(qrtt);
			 // out.print(qrtt);
			if(!rsqrtt.next())
...
myMasterGrid.getCellObject(myRow,3).readOnly(true);
	...
	if(value!='' &&  value!='A'  && value!='D' && value!='M' && value!='U' && value!='P' ){
        ...
        if(value.length>5){
            ...
            if(value>maxmarks*60/100){
                alert('Marks Must be <= 60% of Maximum Marks Assigned for Evaluation In This case Max Marks is '+maxmarks);
                myMasterGrid.getCellObject(myRow,3).setValue("");
...
<font  face="Arial" size="2"><STRONG>No of records changed since last saved:</STRONG></font> <input type="text" readonly="true" size="3" value="0" style="border:none;background-color:#fce9c5;font-weight:bold;font-size:11px;"  id="updated"/>
                      ...
                      onbeforepasteevent="beforepaste(eventArgs)"
                        toolbarenabled="true"
						 ...
						 onbeforecopyevent=""
						  ...
						  gethandler="MarksEntryGetHandlernew.jsp"
						  savehandler="MarksEntrySaveHandlernew.jsp"
						  entertab="down"
						  >
 ...
 <ntb:textcolumn sortenabled="false" width="100" align="center" label="TotalSum" xdatafld="Sum1" editable="false"></ntb:textcolumn>
                            ...
                            <tr>
                            <TD><form name="frm2" id="frm2" method="post" action="MarksEntryTempPagenew.jsp"></TD>
                            </tr>
                            <tr>
                            <TD>&nbsp;</TD>
                            </tr>
				    ...
				    <form name="frm2" id="frm2" method="post" action="MarksEntryTempPagenew.jsp">
...
out.print(" &nbsp;&nbsp;&nbsp <img src='../../Images/Error1.jpg'><b><font size=3 face='Arial' color='Red'> Re-Exam Marks Entry has been locked. </font> <br>");
					...
					out.print(" &nbsp;&nbsp;&nbsp <img src='../../Images/Error1.jpg'><b><font size=3 face='Arial' color='Red'> Re-Exam Event Code has been expired ! </font> <br>");
					}
                    }
                }
            }
