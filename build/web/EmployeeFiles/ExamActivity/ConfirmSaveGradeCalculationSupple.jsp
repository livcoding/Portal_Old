...
<%@ page buffer="1kb" autoFlush="true" language="java" import="java.sql.*,tietwebkiosk.*,java.util.* ,java.math.*,java.text.DecimalFormat,java.math.RoundingMode" %>
...
int mCheck=0;
String mName1="",abc="",qry1="",qry2="",qry3="",qry4="",qry5="",qry6="",qry7="",qry8="";
ResultSet rs5=null,rs6=null,rs1=null,rsttt=null,rsdd=null,rscase=null,rsta=null;
...
%>
<form name="frm"  method="post" action="SaveGradeCalculationsupple.jsp" >
...
qry="SELECT DISTINCT b.eventsubevent,nvl( b.weightage,0)weightage, ";
qry=qry+"decode(b.eventsubevent,'TEST-1','TEST -1',1,'TEST-2', 2, 'TEST-3', 3,'TA#TA1',4,'TA#TA2',5,'TA#TA3',6,'TA#TA4',7,'TA#TA5',8,'TA',9) ordno";
qry=qry+" FROM facultysubjecttagging a, exameventsubjecttagging b ";
qry=qry+" WHERE  a.fstid = b.fstid AND a.institutecode ='"+mInst+"' AND a.examcode    = '"+mExamCode+"' ";
...
<td  align=center><b><font color=white >TA</font></b></td>
...
Map uniqueStudent = new HashMap();
   	while(itun.hasNext())
   {
		...
		if(!uniqueStudent.containsKey(mStudidInit))
			{
				uniqueStudent.put(mStudidInit,mStudidInit);
...
curtaweightage=rsta.getDouble("WEIGHTAGE");
 ...
 qry2=" select  c.studentname studentname ,a.subsectioncode subsectioncode ,c.enrollmentno enrollmentno , c.studentid studentid , a.fstid fstid ,";
					qry2+=" nvl(sum(e.weightage),0)weightage, nvl(sum( round((((d.marksawarded2/e.maxmarks)*e.weightage)),2)),-1) ab0,  sum( round((((nvl(d.marksawarded2,0)/e.maxmarks)*e.weightage)),2)) detain, ";
                                        qry2+=" (select  NVL (   ((SUM ((y.marksawarded2 / z.maxmarks) * z.weightage)*"+curtaweightage+")/sum(z.weightage)),0) ";
                                        qry2+=" from facultysubjecttagging x ,studenteventsubjectmarks y, ";
                                        qry2+=" exameventsubjecttagging z  where  x.fstid = y.fstid ";
                                        qry2+=" and x.fstid = z.fstid AND y.eventsubevent = z.eventsubevent ";
                                        qry2+=" and x.INSTITUTECODE =  '"+mInst+"' And x.examcode = '"+mPrevExamCode+"'  and ";
                                        qry2+=" y.eventsubevent like '%TA%' AND x.subjectid = a.subjectid And ";
                                        qry2+=" y.studentid=c.studentid  ) t1t2t3total " ;
					//qry2+=" ";
					qry2+=" from facultysubjecttagging a , studentmaster c ,studenteventsubjectmarks d, exameventsubjecttagging e  where  a.fstid = d.fstid and a.fstid = e.fstid AND d.eventsubevent = e.eventsubevent ";
					qry2+=" and c.studentid='"+mStudidInit+"' and a.fstid in("+mCheckFstid+") and a.examcode='"+mExamCode+"' and a.INSTITUTECODE='"+mInst+"'   ";
					qry2+="  and c.studentid=d.studentid and a.subjectID='"+mSubjectCode+"'  ";
					qry2+=" and nvl(d.DEACTIVE,'N')='N' and nvl(d.LOCKED,'N')='Y'  ";
					qry2+=" and nvl(c.DEACTIVE,'N')='N'  ";
					qry2+=" group by c.enrollmentno,c.studentname,c.studentid,a.fstid,a.subsectioncode ,subjectid order by enrollmentno,studentname    ";
				 //out.print(qry2);
				...
				for(int j=0;j<i;j++)
							{
                                                         ...
                                                         Mrk=Double.parseDouble(rs.getString("AB"+String.valueOf(j)));  //rs.getString("GRADE")==null?"":rs.getString("GRADE").toString().trim()
						      // String s=rs.getString("AB"+String.valueOf(j))==""
                //--------------------------------- Medical Case Date 24.03.2018 10:45:56 -----------------------------------------------------------------

String firqry="select *from DEBARSTUDENTDETAIL   where INSTITUTECODE = '"+mInst+"' and MEDICALCASE='Y' And examcode = '"+mPrevExamCode+"' and studentid = '"+rs.getString("studentid")+"' AND    subjectid = '"+mSubjectCode+"' ";
...
if(rsa.next())
{
    ...
    "(select NVL(a.EXAMEVENTTYPE,'O') from exameventmaster a where  a.INSTITUTECODE = x.INSTITUTECODE and a.examcode =x.examcode and a.EXAMEVENTCODE =y.EVENTSUBEVENT and  rownum=1) EXAMEVENTTYPE "+
            ...
            "   y.eventsubevent not like '%TA%' AND    x.subjectid = '"+mSubjectCode+"' And         y.studentid = '"+rs.getString("studentid")+"'";
    //out.print("secqry"+secqry);
 ...
 Mrk =(((Mrk/rs.getDouble("weightage"))*mEndTermWeightage))+mprvMaks;
 //double input=12.454656;
 DecimalFormat df2 = new DecimalFormat(".##");
 //Mrk=Double.
 Mrk=Double.parseDouble(df2.format(Mrk));
 //Mrk=(double)df2.format(Mrk);
// input=df2.format(input);
//System.out.println("confirmgradecalculation--"+Mrk+"rs2.getString(studentid)"+rs.getString("studentid"));
...
<%}else{
   ...
   TA=Double.parseDouble(rs.getString("T1T2T3Total"));
...
%>
	...
	<%
        ...
        %>
<%
// condition for checking if marks is -1 than it will not add Marks in TA for  total marks  query
//also change on line no 806 (sum( round((((nvl(d.marksawarded2,0)/e.maxmarks)*e.weightage)),2)) detain,)  on date 01.08.2018
if(Mrk==-1){%>
<td align=center ><%= Math.ceil(TA+Double.parseDouble(rs.getString("detain")))%></td>
<%}else{
%>
<td align=center ><%= Math.ceil(TA+Mrk)%></td>      <!--weightageround>
<%}%>
