...
String QryFaculty="",mfaculty="",mfacultyid="", mfacultycode="";
...
<!--*********Exam**********-->
<tr><td valign=top nowrap><font color=black face=arial size=2>&nbsp;<STRONG>Exam Code : </STRONG></font>
<%

//**************CHANGE THE QUERY 27/01/2010**************************
//**************REMOVING THE query 03/02/2010 nvl(EXCLUDEINPREREG,'N')='N' FROM THE QUERY********

qry="select nvl(examcode,' ')examcode from exammaster where institutecode='"+mInstitute+"'";
qry=qry+" AND NVL(LOCKEXAM,'N')='N' and nvl(deactive,'N')='N' order by 1 desc";
//qry="select nvl(GRADEENTRYEXAMID,' ')examcode from companyinstitutetagging where companycode='"+mLoginComp+"' and institutecode='"+mInstitute+"'";
rs=db.getRowset(qry);
//out.print(qry);
%>
...
<td valign=top nowrap><font color=black face=arial size=2>&nbsp;<STRONG>Co Ordinator : </STRONG></font>
<%
...
qry="select distinct nvl(A.COORDINATORID,' ')EntryBy, NVL(B.EMPLOYEENAME,' ')EName, NVL(B.EMPLOYEECODE,' ')ECode from V#EX#SUBJECTGRADECOORDINATOR A, EMPLOYEEMASTER B where A.COORDINATORID='"+mChkMemID+"' and A.COORDINATORID=B.EmployeeID";
qry=qry+" and A.EXAMCODE IN (select nvl(GRADEENTRYEXAMID,' ') EXAMCODE from CompanyInstituteTagging Where CompanyCode='"+mLoginComp+"' and InstituteCode='"+mInstitute+"')";
*/

if(mInstitute.equals("JIIT") || mInstitute.equals("J128"))
{
qry="select distinct nvl(A.COORDINATORID,' ')EntryBy, NVL(B.EMPLOYEENAME,' ')EName, NVL(B.EMPLOYEECODE,' ')ECode from V#EX#SUBJECTGRADECOORDINATOR A, EMPLOYEEMASTER B where A.COORDINATORID='"+mChkMemID+"' and A.COORDINATORID=B.EmployeeID";
qry=qry+" and A.EXAMCODE IN (SELECT  NVL (Examcode, ' ') examcode FROM ExamMaster WHERE institutecode = '"+mInstitute+"' and nvl(lockexam,'N')='N')";
}
else
{
qry="SELECT DISTINCT NVL (a.EMPLOYEEID, ' ') entryby,                NVL (b.employeename, ' ') ename,                NVL (b.employeecode, ' ') ecode           FROM V#EXAMEVENTSUBJECTTAGGING a, employeemaster b          WHERE a.EMPLOYEEID = '"+mChkMemID+"'             AND a.EMPLOYEEID = b.employeeid            AND a.examcode IN (                     SELECT NVL (examcode, ' ') examcode                       FROM exammaster                      WHERE institutecode = '"+mInstitute+"'                            AND NVL (lockexam, 'N') = 'N')  ";
}
...
if(request.getParameter("x")==null)
{
	while(rs.next())
	{
		...
		mfacultycode=rs.getString("ECode");
	 	mfaculty=rs.getString("EName");
		if(QryFaculty.equals(""))
		{			
			QryFaculty=mfaculty;
			%>
			<OPTION Selected Value =<%=mfacultyid%>><%=mfaculty%> (<%=mfacultycode%>)</option>
			<%
		}
		else
		{
			%>
			<option value=<%=mfacultyid%>><%=mfaculty%> (<%=mfacultycode%>)</option>
			<%
		}
	}
}
...
<%	
	...
	<td><font color="White"><b>Subject's Faculty</b></font></td>
	<td nowrap><font color="White"><b>Event -<BR>Sub Event</b></font></td>	
	<td nowrap align=left><font color="White"><b>Weightage<BR>&nbsp; &nbsp;  (in %)</b></font></td>
	<td nowrap align=left><font color="White"><b>&nbsp; Full<BR>Marks</b></font></td>
	...
	<%
		...
		qry=qry+" nvl(MAXMARKS,0) MAXMARKS, decode(nvl(LOCKED,' '),'Y','YES','N','NO') LOCKED from V#EXAMEVENTSUBJECTTAGGING WHERE INSTITUTECODE='"+mInstitute+"' AND CORDFACULTYID='"+mChkMemID+"'";
		qry=qry+" AND EXAMCODE IN (SELECT  NVL (Examcode, ' ') examcode FROM ExamMaster WHERE institutecode = '"+mInstitute+"' and nvl(lockexam,'N')='N' AND EXAMCODE='"+QryExam+"')";
...
//*********************************************************************
			if(!QryFSTID.equals(""))
			{
				...
				<td nowrap>
				<%
				qry="Select DISTINCT nvl(EMPLOYEENAME,' ')EMPNAME, nvl(EMPLOYEECODE,' ')EMPCODE FROM V#EXAMEVENTSUBJECTTAGGING WHERE INSTITUTECODE='"+mInstitute+"' And EXAMCODE='"+mExamCode+"' AND FSTID IN ("+QryFSTID+") ORDER BY EMPNAME";
				rs=db.getRowset(qry);
				//out.print(qry);
				while(rs.next())
				{
					mcolor="blue";
					%>
					<font face=verdana size=1 color=<%=mcolor%>><%=rs.getString("EMPNAME")%> (<%=rs.getString("EMPCODE")%>)<br>
					<%
				}
				mcolor="Black";
				%>
				...
				<td align=center nowrap><font color=<%=mcolor%>><%=rs1.getString("WEIGHTAGE")%></font></td>
				<td align=center nowrap><font color=<%=mcolor%>><%=rs1.getString("MAXMARKS")%></font></td>
				<%
