...
<%
  ...
  //--------------------------------------
   ...
   //--------------------------------------
			...
			<tr><td bgcolor=red>&nbsp;&nbsp;&nbsp</td><td>Error/Not Changed</td>
			...
			if(request.getParameter("TotalCount")!=null && Integer.parseInt(request.getParameter("TotalCount").toString().trim())>0)
			{
				   ...
				   // mMarks=Double.parseDouble(request.getParameter(mName3));
					...
					try
					{
						if(mMax<mMarks || mMarks<0)
						{
							...
							<td>&nbsp;<%=mShowMarks%></td>
							<td><font color=red>(Not Changed)</font></td>
							</tr>
							<%
						}
						else
				...
				// Log Entry
				//-----------------
				    db.saveTransLog(mINSTITUTECODE,mLogEntryMemberID,mLogEntryMemberType ,"PROJ-MARKS-ENTRY FORCIBLY BY HOD", "ExamCode: "+mExam +"EventSubevent "+ mEventsubevent+ "SubjectID"+mSubject+"Fstid:"+mFstid +"MaxMarks: "+mMax, "NO MAC Address" , mIPAddress);
				//-----------------
			}    //----------closing of for loop
