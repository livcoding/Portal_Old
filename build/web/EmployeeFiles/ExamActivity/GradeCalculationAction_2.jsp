...
try
{
	...
	String mNext="",mPrev="",mColor="",qryz="",		mDebar="",mMedical="",mAbsent="";		
	ArrayList subevents=new ArrayList();
	double mSumMark=0.0;		
			int start=0,last=0,count=0,totalcount=0;
			int NoStud=0;
			int mYesNo=0,fs=0;
	...
	if(!mMemberID.equals("") && !mMemberCode.equals("") && !mMemberName.equals("")) 
	{
		...
		if (RsChk.next() && RsChk.getString("SL").equals("Y"))
		{
			...
			<table  width="100%" ALIGN=CENTER bottommargin=0  topmargin=0 cellspacing=0 cellpadding=0>
				<tr>
					<TD colspan=0 align=middle><font color="#a52a2a" FACE="VERDANA" SIZE=4><U>Grade Calculation</U></font>
					</td>
				</tr>
			</TABLE> 
...
//out.print(abc);  AND nvl(STATUS,'N')='D'
				...
				//out.print(qry3+"coasdsadasdasdaunt");
						...
						if(rs3.next())
						{
							...
							totalcount=Integer.parseInt(c1);
	...
	String mPage="";
				...
				if(count < 1)
				{
					out.print(" <center><b><font size=4 face='Arial' color='Red'>Marks are not locked !</font>");
				}
				else
				{
					int sno=0;
					...
					%>
				<form name="FrmGrade" method=post action="SaveGradeCalculation.jsp">
					...
					<tr bgcolor="#ff8c00">							
						...
						for( i=0; i<subevents.size();i++)
						{
							...
							%><td ><Font color=white><b><%=cccc%> <BR>  (<%=dddd%>)</b></font></td>
							<%
						...
						<td align="center" ><Font color=white><b>Total Weightage<br>Out of 100 </b></font></td>
						<td align='center' ><Font color=white><b>Grades</b></font></td>							
					</tr>
					</thead>
					<tbody>
					<%
...
out.print(qryrow);
					...
					{	sno=rs2.getInt("rn");
						...
						<tr bgcolor=<%=mColor%>>
							<%
							String fstidstudid=rs2.getString("fstid")+"@@@"+rs2.getString("studentid")+"***"+rs2.getString("Weightage");	
							String str="";	
...
qry=" select NVL(A.DEBAR,'N')DEBAR,NVL(A.MEDICALCASE,'N')MEDICALCASE,NVL(A.ABSENTCASE,'N')ABSENTCASE from DEBARSTUDENTDETAIL a  WHERE a.EXAMCODE='"+mExamCode1+"' and a.fstid = '"+rs2.getString("fstid")+"'  and a.INSTITUTECODE='"+mInst+"' and a.STUDENTID='"+rs2.getString("studentid")+"' AND A.SUBJECTID='"+mSubjectID+"' and nvl(a.DEACTIVE,'N')='N' ";
							//out.print(qry);
							rs=db.getRowset(qry);
							if(rs.next())
							{
...
//---------------------------------------------------------
							...
							%>		
							<td align='center'><%=rs2.getString("Weightage")%></td>				
...
//out.print(mCheckFstid+"mCheckFstid");
					...
					<%
						...
						{
							%>
							<form method="Post" Action="GradeCalculationAction.jsp">
							<input id="x" name="x" type=hidden>
