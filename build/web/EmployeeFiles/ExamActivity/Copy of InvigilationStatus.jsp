...
<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy">Invigilation Duty Time Preference Status (<%=Heading%>)</TD>
</font></td></tr>
</TABLE>
...
{ 
   if(mSrcType.equals("A"))
	...
	{
	qry="Select Distinct nvl(EXAMCODE,' ') Exam from V#INVIGILATIONTIMEPREF where  InstituteCode='"+mInstitute+"' And INVIGILATORID='"+mDMemberID+"' ";
	qry=qry+" order by Exam Desc";
	}
//	out.print(qry);
	rs=db.getRowset(qry);
...
//	qry=qry+" from V#INVIGILATIONTIMEPREF A Where A.InstituteCode='"+mInstitute+"' order by examcode,facultyid ";
	...
	{
	qry=" Select Distinct A.INVIGILATORID FacultyID, nvl(A.EMPLOYEENAME,' ') EmployeeName,A.examcode exam ";
	qry=qry+" from V#INVIGILATIONTIMEPREF A where  A.InstituteCode='"+mInstitute+"' And A.INVIGILATORID='"+mDMemberID+"' ";
...
<%
	...
	qry="Select Distinct A.ENTRYBY EntryBy,A.INVIGILATORID FacultyID,to_char(A.PREFROMDATE,'dd-mm-yyyy') ||' to '||to_char(A.PRETODATE,'dd-mm-yyyy') || '( '||decode(A.DUTYHOLIDAY,'Y','Full Day Leave/Holiday',to_char(A.PREFFROMTIME,'hh:mi PM')||' to '|| to_char(A.PRETOTIME,'hh:mi PM'))||' )' Tim , ";
	qry=qry+" nvl(A.REMARKS,' ') REMARKS, nvl(A.APPROVED,'N')APPROVED,nvl(A.EMPLOYEENAME,' ') EmployeeName"; 
	qry=qry+" from V#INVIGILATIONTIMEPREF A, EMPLOYEEMASTER B where A.INSTITUTECODE='"+mInstitute+"'";
      ...
      qry=qry+" and trunc(to_date('"+mDateTo+"','dd-mm-yyyy')) ";
		...
		while(rs1.next())
		{
			...
			else
			{
				...
				{
				qry="select studentname from studentmaster where studentid='"+mEntryby+"' And InstituteCode='"+mInstitute+"'";
				rs=db.getRowset(qry);	
				if(rs.next())
				{
				%>
