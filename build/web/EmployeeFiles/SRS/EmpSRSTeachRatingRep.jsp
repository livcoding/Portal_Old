...
<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><b>SRS Teacher Rating Report [Overall Average]</b></TD>
</font></td></tr>
</TABLE>
...
try
{
	...
	{
		%>
		<select name=Program tabindex="0" id="Program" style="WIDTH: 340px" onclick="ChangeOptions(Program.value,DataCombo,DataComboFac,Subject,Faculty);" onChange="ChangeOptions(Program.value,DataCombo,DataComboFac,Subject,Faculty);">	
...
{ 
	...
	qry=qry+" Select Distinct nvl(SRSEVENTCODE,' ')Exam  from v#SRSEVENTS WHERE  NVL(FINALIZED,'N')='Y' and nvl(deactive,'N')='N' ) order by entrydate desc ";
	rs=db.getRowset(qry);
	if (request.getParameter("x")==null)
	{
		%>
...
<select name="Subject" ID="Subject" tabindex="0" style="WIDTH: 340px" onclick="ChangeCombo(Subject.value,DataComboFac,Faculty);" onChange="ChangeCombo(Subject.value,DataComboFac,Faculty);">

<%
if(request.getParameter("x")==null)
{
	qry1="Select Distinct B.SubjectID, B.SubjectCode, B.Subject, B.Subject||' ('||B.SubjectCode||') ' Subj from v#SRSEVENTS B  where nvl(B.FINALIZED,'N')='Y' and nvl(B.deactive,'N')='N' ORDER BY B.SUBJECTCODE";
...
<%
	while(rs1.next())
	{
		...
		if(mSubjID.equals(""))
 		%>
		<OPTION Value ="<%=mSubjID%>"><%=rs1.getString("Subj")%></option>
		<%			
	}
...
{		lpc1=mSubjID;
 		...
 		%>
		<OPTION selected Value ="<%=mSubjID%>"><%=rs1.getString("Subj")%></option>
		<%
		}	
		else
		{
...
try
{ 
	qry1="Select Distinct B.SubjectID, B.SubjectCode,B.Programcode, B.Subject, B.Subject||' ('||B.SubjectCode||') ' Subj from v#SRSEVENTS B  where nvl(B.FINALIZED,'N')='Y' and nvl(B.deactive,'N')='N' ORDER BY B.SUBJECTCODE";
	rs1=db.getRowset(qry1);
	//out.print(qry1);	
	if (request.getParameter("x")==null) 
	{
...
try
{ 
	qry="select Distinct nvl(A.EMPLOYEEID,' ') EMPLOYEEID,nvl(A.subjectid,' ') subjectid,nvl(A.subjectcode,' ') subjectcode,nvl(A.EMPLOYEENAME,' ') EMPLOYEENAME from V#SRSEVENTS A Where nvl(A.APPROVED,'N')='Y' and nvl(A.FINALIZED,'N')='Y' and nvl(Deactive,'N')='N' ORDER BY  EMPLOYEENAME,subjectcode";
	rs=db.getRowset(qry);
...
<select name="Faculty" ID="Faculty" tabindex="0" style="WIDTH: 340px">
<%
...
<table align=middle class="sort-table" id="table-2"  border=0 cellpadding=0 cellspacing=0 width="70%"><thead>
<tr bgcolor="#ff8c00">
<td colspan=3 align=middle><b><font color="white" >Resp./Regd.</font></b></td></tr>
<tr bgcolor="#ff8c00">
...
<tr bgcolor="#ff8c00">
<td colspan=3><b><font color="white" >Responded</font></b></td></tr><tr bgcolor="#ff8c00"><td><b><font color="white" >L</font></b></td>
...
//-----------------------------
	...
	%>
			 ...
			 <td><font size=2 face=Tahoma><%=mLtot%>/<%=Ltot%></font></td>
			 <td><font size=2 face=Tahoma><%=mTtot%>/<%=Ttot%></font></td>
			 <td><font size=2 face=Tahoma><%=mPtot%>/<%=Ptot%></font></td>
			</tr></table>
...
if (!z.equals(""))
	...
	%>
		...
		<td><font size=2 face=Tahoma><%=mLtot%>/<%=Ltot%></font></td>
		<td><font size=2 face=Tahoma><%=mTtot%>/<%=Ttot%></font></td>
		<td><font size=2 face=Tahoma><%=mPtot%>/<%=Ptot%></font></td></tr></table>
		</td>
