<%@ page buffer="500kb" language="java" import="java.sql.*,tietwebkiosk.*,java.util.*" %>
...
try
{
	...
	String mName11="",abc1="";
	String Stdidclub="",mBreakSlno="",mFstid="''", qry12="";

	String qrysl="";
				ResultSet rssl=null,rsk=null;
		...
		String Fstid="";
		String mGrade="";
		String marks="",PREVIOUSEXAMCODE="",PREVIOUSSUBJECTID="";
   String mPrevGrade1="";
			String prevgrade="",prevFstid="",  prevStudid="" , prevStdidclub="",  mPrevGrade="";
	...
	for( yy =1; yy<=sno;yy++)
	{
		...
		else
		{
			...
			else
			{
				...
				/*if(request.getParameter("gradex"+yy)==null)
			{
				mPrevGrade1="";
			}
			else
			{
				mPrevGrade1=request.getParameter("gradex"+yy);			
			}*/
//System.out.println("mPrevGrade 1"+mPrevGrade1);
	//	if(!mPrevGrade1.equals("X"))
	//		mGrade=mPrevGrade1;
	//	else
	//		mGrade=mGrade;
			...
			if( !mGrade.equals(" "))
			{
			 ...
			 qry12="select 'Y' FROM STUDENTWISEGRADE WHERE INSTITUTECODE='"+mInst+"' AND EXAMCODE='"+mExamCode+"' AND FSTID='"+Fstid+"' AND BREAK#SLNO='"+mBreakSlno+"' AND STUDENTID='"+Studid+"' ";
			//	out.print(qry12);
			 rsk=db.getRowset(qry12);
			if(rsk.next() )
			{
				...
				//out.println(count+"::::xxxxxxxxxxx=1"+qry4);
				st.addBatch(qry4);
	...
	for(yy =1; yy<=sno;yy++) 
	{
		...
		else
		{
			...
			count++;
			//out.println(count);
			if(request.getParameter("prevfstidstudid"+yy)==null)
			{
				prevgrade="";
			}
			else
			{
				prevgrade=request.getParameter("prevfstidstudid"+yy);
				prevFstid=prevgrade.substring(0,prevgrade.indexOf("@@@"));
				prevStudid=prevgrade.substring(prevgrade.indexOf("@@@")+3,prevgrade.indexOf("***"));
				if(prevStdidclub.equals(""))
					prevStdidclub="'"+prevStudid+"'";
				else
					prevStdidclub=prevStdidclub+",'"+prevStudid+"'";
			...
			{
				mPrevGrade=request.getParameter("grade"+yy);			
			}
			
			 qry12="select nvl(FINALGRADE,' ')FINALGRADE FROM STUDENTWISEGRADE WHERE INSTITUTECODE='"+mInst+"' AND EXAMCODE='"+PREVIOUSEXAMCODE+"' AND FSTID='"+prevFstid+"' AND STUDENTID='"+prevStudid+"' ";
		
			 rsk=db.getRowset(qry12);
			if(rsk.next() && !mPrevGrade.equals(" ")  )
			{

				if(rsk.getString("FINALGRADE").equals("X"))
				{
							qry="UPDATE STUDENTWISEGRADE SET FINALGRADE='"+mPrevGrade+"', ENTRYBY ='"+mChkMemID+"',ENTRYDATE=SYSDATE WHERE INSTITUTECODE='"+mInst+"' AND EXAMCODE='"+PREVIOUSEXAMCODE+"' AND FSTID='"+prevFstid+"' AND  STUDENTID='"+prevStudid+"'  ";
							//out.println(count+"::::updateSTAUS=1"+qry);
							st.addBatch(qry);
				}
...
if(mStatus==0)
{
//out.print("mStatus==00000000000000");
for( yy =1; yy<=sno;yy++)
{
...
//System.out.println("mPrevGrade 0"+mPrevGrade1);
	//	if(!mPrevGrade1.equals("X"))
//			mGrade=mPrevGrade1;
//		else
//			mGrade=mGrade;
		...
		String qryck="select 'Y' FROM STUDENTWISEGRADE WHERE INSTITUTECODE='"+mInst+"' AND EXAMCODE='"+mExamCode+"' AND FSTID='"+Fstid+"' AND BREAK#SLNO='"+mBreakSlno+"' AND STUDENTID='"+Studid+"' ";
			...
			if(rsk.next() )
			{
				...
				//out.println(count+"xxxxxxxxxxx"+qry4);
				st.addBatch(qry4);
			}
			else
			{
...
for(yy =1; yy<=sno;yy++) 
	{
		...
		else
		{
			...
			if(rsk.next() && !mPrevGrade.equals(" ") )
			{
				if(rsk.getString("FINALGRADE").equals("X"))
				{
				qry4="UPDATE STUDENTWISEGRADE SET FINALGRADE='"+mPrevGrade+"',ENTRYBY ='"+mChkMemID+"',ENTRYDATE=SYSDATE WHERE INSTITUTECODE='"+mInst+"' AND EXAMCODE='"+PREVIOUSEXAMCODE+"' AND FSTID='"+prevFstid+"' AND  STUDENTID='"+prevStudid+"'  ";
...
try
{
	...
	qryss="select distinct count(studentid)ss from  studentwisegrade where  InstituteCode='"+mInst+"' and  examcode='"+mExamCode+"' and BREAK#SLNO='"+mBreakSlno+"' ";
	//out.print("FDFGDFGDGDGDFFFFFFFFFFFFFFF"+qryss);
	rs2=db.getRowset(qryss);
	if(rs2.next())
	{
		mStudentCount=rs2.getInt("ss");			
	}
	//out.println(sno+"sd**********fsf"+mStudentCount);
	
			
String url="ProjectGradeCalculationAction.jsp?Subject="+mSubjectCode+"&ExamCode="+mExamCode+"&checkctr="+mCheck+"&count="+mCount+"&FS="+fs+"&NoStudent="+sno+"&PREVIOUSEXAMCODE="+PREVIOUSEXAMCODE+"&PREVIOUSSUBJECTID="+PREVIOUSSUBJECTID+"&x=";

session.setAttribute("mCheckFstid",mCheckFstid);

	if(mCount > mStudentCount)
	{
		//out.print(url);
		response.sendRedirect(url);
		//out.print("<center> <a href="+url+" ><b>Click Back to Enter more Grade</b></a> </center>");		
 ...
 if(mCount==mStudentCount)
{
	...
	<form name="frm" method="post" action="ProjectTempPagesecond.jsp">
...
session.setAttribute("mCheckFstid",null);


	String mNam="";
	String mSc="";
	String time="",mSubjectID="";
	ResultSet rslist=null , rssub=null;
