...
<%
        ...
        academicyear=request.getParameter("academic")==null?"":request.getParameter("academic");
        programcode=request.getParameter("program")==null?"":request.getParameter("program");
       // System.out.print("AAAAAAAAAAA"+academicyear);
...
try{
qry= "select distinct NVL(BRANCHCODE,' ') b FROM v#studentmaster where nvl(Deactive,'N')='N' and PROGRAMCODE='"+programcode+"' and ACADEMICYEAR='"+academicyear+"'";

	System.out.println("#####################################33"+qry);
	rs2=db.getRowset(qry);
	while(rs2.next()) {
		result=result+rs2.getString("b")+"$";
	}
