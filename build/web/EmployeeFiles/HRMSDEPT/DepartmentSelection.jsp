...
int mTSelected=0 ;
		qry1="select distinct count(c.applicantid)selected,A.APPLICANTID from   hr#applicantshortlistmaster c,  hr#applicationmaster a,hr#applicantaddress D where a.vacancycode in (select vacancycode from HR#VACANCYMASTER WHERE a.vacancycode='"+mVacCode+"' and institutecode='"+mInst+"' and NVL(BROADCAST,'N')='Y' AND SYSDATE BETWEEN FROMPERIOD AND TOPERIOD ) and a.departmentcode ='"+mDeptCode+"'          and     (c.shortlistseqno =2 and c.status = 'S')    and a.vacancycode ='"+mVacCode+"'    AND a.applicantid = c.applicantid			and a.institutecode='"+mInst+"' AND a.vacancycode = c.vacancycode AND a.institutecode = c.institutecode  AND a.applicantid = D.applicantid AND C.applicantid = D.applicantid GROUP BY  A.APPLICANTID ";
		//out.print(qry1);
		...
		int mTReject= 0;
			qry2="select distinct count(c.applicantid)reject,A.APPLICANTID from   hr#applicantshortlistmaster c,  hr#applicationmaster a ,hr#applicantaddress D where a.vacancycode in (select vacancycode from HR#VACANCYMASTER WHERE a.vacancycode='"+mVacCode+"' and institutecode='"+mInst+"' and NVL(BROADCAST,'N')='Y' AND SYSDATE BETWEEN FROMPERIOD AND TOPERIOD ) and a.departmentcode ='"+mDeptCode+"'            AND (c.shortlistseqno =2 and c.status = 'R')    and a.vacancycode ='"+mVacCode+"'              AND a.applicantid = c.applicantid			and a.institutecode='"+mInst+"' AND a.vacancycode = c.vacancycode AND a.institutecode = c.institutecode  AND a.applicantid = D.applicantid AND C.applicantid = D.applicantid GROUP BY  A.APPLICANTID";
		...
		int mTNotProcess=0;
			qry2="select distinct count(c.applicantid)mTNotProcess, A.APPLICANTID from   hr#applicantshortlistmaster c,  hr#applicationmaster a,hr#applicantaddress D where a.vacancycode in (select vacancycode from HR#VACANCYMASTER WHERE a.vacancycode='"+mVacCode+"' and institutecode='"+mInst+"' and NVL(BROADCAST,'N')='Y' AND SYSDATE BETWEEN FROMPERIOD AND TOPERIOD ) and a.departmentcode ='"+mDeptCode+"'         and  ((c.shortlistseqno =1 and c.status = 'S') or (c.shortlistseqno =2 and c.status = 'N') )   and a.vacancycode ='"+mVacCode+"'              AND a.applicantid = c.applicantid			and a.institutecode='"+mInst+"' AND a.vacancycode = c.vacancycode AND a.institutecode = c.institutecode  AND a.applicantid = D.applicantid AND C.applicantid = D.applicantid GROUP BY  A.APPLICANTID ";
			out.print(qry2);
		rs1=db.getRowset(qry2);
		while(rs1.next())
		{	
		...
		int mALLSum=0;
		qry2="select distinct count(c.applicantid)AllResume  ,A.APPLICANTID from   hr#applicantshortlistmaster c,  hr#applicationmaster a ,hr#applicantaddress D where a.vacancycode in (select vacancycode from HR#VACANCYMASTER WHERE institutecode='"+mInst+"' and NVL(BROADCAST,'N')='Y' AND SYSDATE BETWEEN FROMPERIOD AND TOPERIOD ) and a.departmentcode ='"+mDeptCode+"'         and  c.SHORTLISTSEQNO in (1,2)  and a.vacancycode ='"+mVacCode+"'   AND a.applicantid = D.applicantid AND C.applicantid = D.applicantid AND a.applicantid = c.applicantid			and a.institutecode='"+mInst+"' AND a.vacancycode = c.vacancycode AND a.institutecode = c.institutecode  GROUP BY  A.APPLICANTID";
		//out.print(qry2);
		rs1=db.getRowset(qry2);
		while(rs1.next())
			{
