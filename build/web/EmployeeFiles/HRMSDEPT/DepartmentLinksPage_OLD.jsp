...
{
	mComp=request.getParameter("COMPANYCODE");
}
 ...
 <tr bgcolor="#FFCF83">
   ...
   <table  align=center >
            <tr><td align=center ><h2>&nbsp;&nbsp; Initial ShortListing by Nominated Members &nbsp;&nbsp;</h2></td></tr>
			   ...
			   <tr><td align=center ><h2>Institute - <%=mInst%>  &nbsp;&nbsp; Vacancy-<%=mVacDesc%> &nbsp;&nbsp; Department-<%=mDeptDesc%> </h2></td></tr>
...
String memailid="",memailmsg="";
      ...
      qry = " SELECT nvl(B.EMAILID,' ')EMAILID , nvl(APPLICANTREFNO,' ')APPLICANTREFNO,  Decode(c.status, 'S','1','R','2','3') forsorting, to_char(c.SHORTLISTDATE,'yyymmddhhmiss')datee,nvl(round(months_between(sysdate,dateofbirth)/12),'')AGE , nvl(a.SHORTLISTED,' ')SHORTLISTED, decode(a.SHORTLISTED,'S','Selected','R','Rejected','',' ',a.SHORTLISTED)FINALSHORTLISTED,nvl(c.SHORTLISTSEQNO,0) SHORTLISTSEQNO,a.APPLICANTID,nvl(a.firstname,' ') firstname,nvl(a.middlename,' ') middlename,nvl(a.lastname,' ') lastname,to_char(a.dateofbirth,'DD-MM-YYYY')dateofbirth, decode(a.GENDER,'M','Male','F','Female')GENDER " +
                ",nvl(b.CADDRESS1,' ')CADDRESS1, nvl(b.CADDRESS2,' ')CADDRESS2, nvl(b.CADDRESS3,' ')CADDRESS3, " +
                "nvl(b.CCITY,' ')ccity, nvl(b.CDISTRICT,' ')CDISTRICT, nvl(b.CSTATE,' ')CSTATE, nvl(b.CPIN,0)CPIN,nvl(c.STATUS ,'N')STATUS ,nvl(c.REMARKS,' ')REMARKS ,nvl(C.final,'N') final   " +
"  FROM hr#applicationmaster a, hr#applicantaddress b,HR#APPLICANTSHORTLISTMASTER c" +
" WHERE a.applicantid = b.applicantid  " +                "   AND a.vacancycode = '" + mVacCode + "'" +
                "   AND a.departmentcode = '" + mDeptCode + "' and a.institutecode =  '"+mInst+"'    and a.APPLICANTID=c.APPLICANTID   and b.APPLICANTID=c.APPLICANTID   and a.VACANCYCODE=c.VACANCYCODE   and a.INSTITUTECODE=c.INSTITUTECODE AND A.COMPANYCODE=C.COMPANYCODE     AND A.INSTITUTECODE=C.INSTITUTECODE     AND a.institutecode = c.institutecode   AND  ";
		if(mShow.equals("ALL"))
		...
		//out.print(qry);
//qqry="select * from (select a.* , nvl(ROWNUM,'')rn    from("+qry+") a where rownum<='"+last+"') where rn>='"+start+"'  order by firstname,shortlistseqno desc ,forsorting,rn ";
...
qry2="select distinct nvl(A.NOTESOFINTERVIEWER,' ')NOTESOFINTERVIEWER  ,X.EMAILID      ,NVL( c.vacancycode,' ')vacancycode1            from hr#applicantaddress x ,HR#INTERVIEWRESULT A ,hr#applicationmaster c                  where  a.APPLICANTID=c.APPLICANTID and x.APPLICANTID=c.APPLICANTID and A.APPLICANTID=X.APPLICANTID                    AND X.EMAILID=LTRIM('"+memailid+"')                     AND                     x.applicantid in(select y.applicantid                                          from hr#applicationmaster y                                          where x.applicantid = y.applicantid                                             and y.applicantid in(select i.applicantid                                                                 from hr#interviewResult i                                                                 where i.applicantid = y.applicantid                                                                and  i.RESULT<>'S'                                                                )                                         ) ";
