...
<HTML>
<head>
<TITLE>#### <%=mHead%> [ Employee Salary History ] </TITLE>
...
try
{
	...
	{	
		...
		//-----------------------------
		qry="Select WEBKIOSK.ShowLink('4','"+ mChkMemID+"','"+mChkMType+"','"+mRole+"','"+ mIPAddress +"') SL from dual";
		...
		if (RsChk.next() && RsChk.getString("SL").equals("Y"))
	   	{
			...
			<tr>
			<TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><b><u><FONT SIZE=4>Employee Salary History</FONT></u></b></td>
			</tr>
			</table>
			</center>
			<%
			...
			<tr>
				<td colspan=8 align=center><font color="#00008b" face=times new roman size=2><b>&nbsp;<%=mEmployeeName%> </b></font>
			...
			try
			{
				qry="select nvl(PFLOANRECOVERY,0)PFLOANRECOVERY,nvl(ADVANCERECOVERY,0)ADVANCERECOVERY,nvl(BASICACTRECOVERY,0)BASICACTRECOVERY,nvl(PRESENTDAYS,0)PRESENTDAYS,nvl(WEEKLYOFFDAYS,0)WEEKLYOFFDAYS,nvl(HOLIDAYDAYS,0)HOLIDAYDAYS,nvl(PAIDLEAVE,0)PAIDLEAVE,nvl(LWPDAYS,0)LWPDAYS,nvl(ABSENTDAYS,0)ABSENTDAYS from SALARY where  COMPANYCODE='"+mComp+"' and EMPLOYEEID='"+mChkMemID+"' and YEARMONTH=(select to_char(to_date('"+mYearMonth+"','YYYYMM'),'yyyymm') from dual)";
				//out.print(qry);
				rs=db.getRowset(qry);
				if(rs.next())
				{
			...
			<%
			qry="SELECT NVL(BASICPBL,0)CURRBASIC, NVL(BASICPBLARREAR,0)ARRBASIC FROM SALARY WHERE COMPANYCODE='"+mComp+"' AND EMPLOYEEID='"+mChkMemID+"' AND YEARMONTH=(select to_char(to_date('"+mYearMonth+"','YYYYMM'),'yyyymm') from dual)";
			//out.print(qry);
			rs=db.getRowset(qry);
			if(rs.next())
			{
			...
			qry=qry+" where S.COMPANYCODE=E.COMPANYCODE AND S.EDID=E.EDID AND S.COMPANYCODE='"+mComp+"' and S.EMPLOYEEID='"+mChkMemID+"'";
			...
			try
			{
				...
				qry="Select Payroll.NetPay('"+mComp+"','"+mChkMemID+"','"+mYMDate+"') Pay from dual";
				rs=db.getRowset(qry);
				//out.print(qry);
				while(rs.next())
				{
