...
String mEmpName="",mWorkDt="",mCompCode="",mInst="";
...
%>
	...
	<tr><TD align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy">Attendance Detail of <%=mEmpName%> for <br>the Working Date <%=mWorkDt%></font></TD>
	</font></td></tr>
	</TABLE>
	<br>
	<br>
	...
	<% int Time=0,TotalWork,TotalHrs;
		qry="select to_char(INTIME,'HH:MI PM')INTIME, nvl(to_char(nvl(OUTTIME,''),'HH:MI PM'),'-')OUTTIME,  decode (workhrs_minute,0,'0',workhrs_minute)  WORKHRS_MINUTE,decode(ISMANUAL,'Y','Manual','N','Biometric','Biometric',ISMANUAL)ISMANUAL,decode(trim(INTERMINAL),'032','VC Office','034','Faculty-2','033','LRC','029','JPBS2','030','JPBS1',INTERMINAL)INTERMINAL,nvl(decode(trim(OUTTERMINAL),'032','VC Office','034','Faculty-2','033','LRC','029','JPBS2','030','JPBS1',OUTTERMINAL),'-')OUTTERMINAL from Att_AttendLog where INSTITUTECODE='"+mInst+"' and USERID='"+mChkMemID+"' and USERTYPE='"+mChkMType+"' and to_char(WORKINGDATE,'dd-mm-yyyy')='"+mWorkDt+"' and COMPANYCODE='"+mCompCode+"' order by WorkingDate,InTime";
		//out.println(qry);
		rs=db.getRowset(qry);
		while(rs.next() )
		   {
				//if ( !rs.getString("WORKHRS_MINUTE").equals("0"))
			   //{
			%>
			<tr>
