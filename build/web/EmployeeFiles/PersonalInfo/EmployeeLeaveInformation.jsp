...
try
{
	...
	{	
		...
		//-----------------------------
		qry="Select WEBKIOSK.ShowLink('3','"+ mChkMemID+"','"+mChkMType+"','"+mRole+"','"+ mIPAddress +"') SL from dual";
		...
		%>
			...
			<tr>
			<TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><b><u><FONT SIZE=4>Employee Leave Information</FONT></u></b></td>
			</tr>
			</table>
			</center>
			<%
			try
			{
				qry="select distinct nvl(A.EMPLOYEENAME,' ')EMPLOYEENAME,B.DEPARTMENT DEPARTMENT,C.DESIGNATION DESIGNATION from EMPLOYEEMASTER A,DEPARTMENTMASTER B, DESIGNATIONMASTER C where A.employeeid='"+mChkMemID+"' and A.DEPARTMENTCODE=B.DEPARTMENTCODE and A.DESIGNATIONCODE=C.DESIGNATIONCODE and nvl(A.DEACTIVE,'N')='N' and nvl(B.DEACTIVE,'N')='N' and nvl(C.DEACTIVE,'N')='N' ";
				//out.println(qry);
				rs=db.getRowset(qry);
				if(rs.next())
				{
			...
			<tr>
				...
				&nbsp;&nbsp;
				<%
				qry="Select distinct LEAVEYEARCODE from LEAVEYEAR where COMPANYCODE='"+mComp+"' and rownum<6 order by LEAVEYEARCODE desc ";
				//out.println(qry);
				rs=db.getRowset(qry);
				%>
			...
			//out.println(qry);
				...
				qry="Select nvl(EMPCATEGORYCODE,'REG')CATEGORYCODE, nvl(SEX,'M') sex from EmployeeMaster where CompanyCode='"+mComp+"' and EmployeeId='"+mChkMemID+"'";
				//out.println(qry);
				rs=db.getRowset(qry);
				while(rs.next())  
				{
				...
				qry="Select nvl(NATUREOFJOB,' ') NATUREOFJOB from EMPLOYEEDETAIL where EmployeeId ='"+mChkMemID+"'";
				//out.println(qry);
				rs=db.getRowset(qry);
				while(rs.next())  
				{
				...
				qry="Select Leave.Present('"+mComp+"','"+mCategoryCode+"','"+mChkMemID+"',To_date('"+mFromDate+"','dd-mm-yyyy'),To_date('"+mToDate+"','dd-mm-yyyy')) Present from dual";
				//out.println(qry);
				rs=db.getRowset(qry);
				while(rs.next())  
				{
			...
			<tr>
				<td>
				<%--<font face="Arial" size=2><b>Total Present</b></font> <font face="Arial" size=2 color="navy"><b><%=mDay%></b></font>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp;<!--  <font face="Arial" size=2><b>Total Days</b></font> <font face="Arial" size=2 color="navy"><b><%=mPresentDays%></font> -->--%>
				</td>
			</tr>
			</table>
			</center>
...
//------To find out the leave code-------
				qry="select leavecode, leavedescription , nvl(MAINTAINBALANCE,'Y') MAINTAINBALANCE  from  LEAVEMASTER a ";
				qry=qry+" where  nvl(deactive,'N') = 'N' and companycode = '"+mComp+"' and Sexspecific in ('B','"+mSex+"') ";
				qry=qry+" and EMPCATEGORYCODE in ((Select nvl(EMPCATEGORYCODE,'REG') from EMPLOYEEMASTER where COMPANYCODE='"+mComp+"'" +
                        " and EMPLOYEEID='"+mChkMemID+"' AND EMPCATEGORYCODE NOT IN ('TRN') ) Union Select 'ALL' from Dual)";
				qry=qry+" and LEAVEAPPLICABLETO in ((Select nvl(EMPLOYEETYPE,'TEC') from EMPLOYEEMASTER where COMPANYCODE='"+mComp+"' " +
                        "and EMPLOYEEID='"+mChkMemID+"'  AND EMPCATEGORYCODE NOT IN ('TRN'))  Union Select 'ALL' from Dual)";
				qry=qry+" Union Select 'A', 'Absent','N' from Dual Union Select 'LWP', 'LWP','N' from Dual";
//------To find out the leave code-------
				...
				while(rs.next())
				{
					...
					qry1="Select Leave.TotalLeaveCodeWise('"+mComp+"','"+mChkMemID+"',To_date('"+mFromDate+"','DD-MM-YYYY'),To_date('"+mToDate+"','DD-MM-YYYY'),'"+mLeaveCode+"')LEAVETAKEN from dual";
					//out.println(qry1);
					...
					qry2="Select Leave.LeaveBalance('"+mComp+"','"+mCategoryCode+"','"+mChkMemID+"' ,'"+mLeaveCode+"', To_date('"+mCurrDate+"','dd-mm-yyyy')) Bal from dual";
					//out.print(qry2);
					...
					if(Double.parseDouble(mLeaveTaken)>=0)
					{
						...
						<TR bgcolor="white">
							...
							<TD align="center"><font face="Arial" size=2>&nbsp;<A href="EmployeeLeaveHistory.jsp?LeaveCode=<%=mLeaveCode%>&amp;FromDate=<%=mFromDate%>&amp;ToDate=<%=mToDate%>" target=_New title="Click to show details of leave taken <%=mLeaveCode%> as <%=mLeaveTaken%> day(s)"><%=mLeaveTaken%></a></TD>
...
QryL=" SELECT   remark, employeeid, todate,fromdate ,to_char(( (todate-fromdate)+1)/2 ) noofdays FROM vacationduty a WHERE todate = (    SELECT MAX(todate)    FROM vacationduty   b    WHERE a.employeeid = b.employeeid      AND a.institutecode = b.institutecode      and b.employeeid = '"+mChkMemID+"'    and b.institutecode = '"+mInst+"'  ) and a.employeeid = '"+mChkMemID+"' AND vacationcode IN (SELECT vacationcode    FROM vacationmaster  WHERE showonweb = 'Y' AND institutecode = '"+mInst+"') ";
rs=db.getRowset(QryL);

//	out.print("###"+QryL);
if (rs.next())         
{
    Lremarks=rs.getString("remark");
	LCount=rs.getString("noofdays");
}
%>

 <CENTER><FONT SIZE="3" COLOR="red"><B>##&nbsp;Last EL converted in lieu of <%=Lremarks%>  (<%=LCount%>)</B></FONT></CENTER> 
