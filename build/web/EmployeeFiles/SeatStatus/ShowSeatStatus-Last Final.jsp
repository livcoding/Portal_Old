...
// For JIIT Noida
//------------------------
ResultSet rsPrgPrint=null, rsProg=null, rs=null, StudentRecordSet =null,BranchRecordSet=null;
	...
	for(int jk=0;jk<yourCat.length;jk++)
	 {
		...
		<%
			...
			%> 
		<br><table><tr><td><br><font  FACE=VERDANA size=4 color=white><b>Seats ...</b></font><br></td></tr>
		<tr><td height='50px'><font  FACE=VERDANA size=4 color=white><b>Total</b></font></td></tr>
		<tr><td><font  FACE=VERDANA size=4 color=white><b>Alloted</b></font></td></tr>
		<tr><td><font  FACE=VERDANA size=4 color=white><b>Available</b></td></tr></table>
		<%
		...
		for(int opl=0;opl<mInst1.length;opl++)
		  {
			...
			qry="select count(distinct BRANCHCODE), PROGRAMCODE   from (";
			qry+="select distinct BranchCode,INSTITUTECODE,PROGRAMCODE, Alpha from C#SEATDISPLAY where INSTITUTECODE ='"+mInst1[opl]+"' And CATEGORYCODE='"+yourCat[jk]+"' and nvl(NOOFSEATS,0)>=0 order by ALPHA) Group by ProgramCode order by ProgramCode";
			rsPrgPrint=db.getRowset(qry);	
			while(rsPrgPrint.next())
			   {		
					 
				%>
					<td BGCOLOR=RED align=center colspan=<%=rsPrgPrint.getInt(1)%>><FONT COLOR=WHITE FACE=VERDANA SIZE=5><b><%=rsPrgPrint.getString(2)%></b></FONT></td>
					<%
				...
				while (rs.next())
				  { 
				   bArray[k++]=rs.getString(1);
					//bgcolor='#ffdead'
				  %>
				  <td bgcolor="#ffdead" valign=middle align=center ><font color=blue  FACE=VERDANA color='#8b0000' size="4"><b><%=rs.getString("PROGRAMCODE")%> <%=rs.getString(2)%></b>&nbsp;</font></td>
				<%
		  	  	}
			     }
			   k--;
