...
else
{
	...
	qry+=" (A.fstid) in ("; 
	qry+=" select AA.fstid from facultysubjecttagging AA where facultytype=decode('"+mDMemberType+"','E','I','E') and nvl(deactive,'N')='N' AND employeeid='"+mDMemberID+"'  AND ( LTP='P' and  NVL(PROJECTSUBJECT,'N')='Y')";
	...
	qry+=" UNION select AA.fstid from StudentEventSubjectMarks aa where aa.ENTRYBY='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
      qry=qry+" ) and A.examcode='"+QryExam+"'  AND (a.ltp = 'P' and NVL (a.projectsubject, 'N') = 'Y') order by eventsubevent";
	//  out.print(qry);
                    ...
                    try
                    {
					  ...
					  qry="select A.subject ||' ( '||A.subjectcode||' )' subject, A.subjectID from v#eXAMEVENTSUBJECTTAGGING  A where ";
	qry+=" (A.fstid) in ("; 
	qry+=" select AA.fstid from facultysubjecttagging AA where facultytype=decode('"+mDMemberType+"','E','I','E') and nvl(deactive,'N')='N' AND employeeid='"+mDMemberID+"'  AND ( LTP='P' and  NVL(PROJECTSUBJECT,'N')='Y')";
	qry+=" UNION select AA.fstid from MultiFacultySubjectTagging aa where aa.INSTITUTECODE='"+mInst+"' and aa.COMPANYCODE='"+mComp+"' and aa.EMPLOYEEID='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
	qry+=" UNION select AA.fstid from Ex#SubjectGradeCoordinator aa where aa.INSTITUTECODE='"+mInst+"' and aa.COMPANYCODE='"+mComp+"' and aa.FACULTYID='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
	...
	qry+=" UNION select AA.fstid from StudentEventProjectMarks2 aa where aa.MARKS2ENTRYBY='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
	qry+=" UNION select AA.fstid from StudentEventSubjectMarks aa where aa.ENTRYBY='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
	qry=qry+" ) and eventsubevent='"+QryEventSubevent+"' and examcode='"+QryExam+"' and (a.ltp = 'P' and NVL (a.projectsubject, 'N') = 'Y') ";
...
if(request.getParameter("y")!=null || mFlag==1)
{
	...
	qry=qry+"and A.facultytype=decode('"+mDMemberType+"','E','I','E') and ( A.PROJECTSUBJECT='Y' and A.LTP='P') and A.subjectID='"+mSC+"'  and a.fstid in (  SELECT aa.fstid FROM studenteventprojectmarks1 aa   WHERE aa.marks1entryby = '"+mDMemberID+"' and aa.STUDENTID=a.STUDENTID and aa.EVENTSUBEVENT='"+mEvent+"'   AND aa.fstid = a.fstid  AND NVL (aa.deactive, 'N') = 'N'   UNION     SELECT aa.fstid    FROM studenteventprojectmarks2 aa   WHERE aa.marks2entryby = '"+mDMemberID+"'  and aa.STUDENTID=a.STUDENTID and aa.EVENTSUBEVENT='"+mEvent+"'   AND aa.fstid = a.fstid    AND NVL (aa.deactive, 'N') = 'N'  UNION    SELECT aa.fstid   FROM studenteventsubjectmarks aa   WHERE aa.entryby ='"+mDMemberID+"'  and aa.STUDENTID=a.STUDENTID and aa.EVENTSUBEVENT='"+mEvent+"'    AND aa.fstid = a.fstid    AND NVL (aa.deactive, 'N') = 'N') ";
...
*/
	...
	qry=qry+" and A.examcode='"+mEC+"'  and A.subjectID='"+mSC+"' and ( A.PROJECTSUBJECT='Y' and A.LTP='P')  and a.fstid in (  SELECT aa.fstid              FROM studenteventprojectmarks1 aa             WHERE  aa.fstid = a.fstid     and aa.EVENTSUBEVENT='"+mEvent+"' and aa.STUDENTID=a.STUDENTID         AND NVL (aa.deactive, 'N') = 'N'            UNION            SELECT aa.fstid              FROM studenteventprojectmarks2 aa             WHERE  aa.fstid = a.fstid     and aa.EVENTSUBEVENT='"+mEvent+"'   and aa.STUDENTID=a.STUDENTID           AND NVL (aa.deactive, 'N') = 'N'            UNION            SELECT aa.fstid              FROM studenteventsubjectmarks aa             WHERE  aa.fstid = a.fstid  and aa.EVENTSUBEVENT='"+mEvent+"'   and aa.STUDENTID=a.STUDENTID   AND NVL (aa.deactive, 'N') = 'N') ";
	qry=qry+" GROUP BY A.fstid,nvl(A.studentid,' '),nvl(A.studentname,' '), ";
	qry=qry+" nvl(A.enrollmentno,' '),nvl(A.semester,0),A.subsectioncode, ";
		...
		while(rs1.next())
		{
			...
			if(ctr==1)
			{
				...
				%>
					...
					<center><font color=blue><a style="cursor:hand" onClick="window.print();"><img src="../../Images/printer.gif"><b>Click to Print</b></a></font></center>
