...
ResultSet rs5=null,rs6=null,rs1=null;
...
<thead>
	<tr bgcolor="#ff8c00">
		...
		<TD ALIGN=CENTER><font color=white><b>Suggested Range<B></TD>
			...
			//	out.print(qry);
					...
					if(subevents.size()>0)
					{
						...
						for( i=0; i<subevents.size();i++)
						{
							...
							%><td align=right>><Font color=white><b><%=cccc%> <BR>  (<%=dddd%>)</b></font></td>
							<%
...
<%
   	...
   	while(itun.hasNext())	
   {
	...
	//qry="select enrollmentno,studentname from studentmaster where studentid='"+mStudidInit+"' and nvl(DEACTIVE,'N')='N' ";
	 ...
	 qry2="Select distinct subsectioncode, enrollmentno, studentname, studentid,fstid,"+qryadd+","+sumadd+"  Weightage from ( select ";
...
<%
	...
	<%
		...
		%>										
		<td align=right><%=weightageround%></td>		
		<%
...
var st1 = new SortableTable(document.getElementById("table-1"),["Number","Number","CaseInsensitiveString","Number","Number","Number","Number","Number","CaseInsensitiveString"]);
</script>
