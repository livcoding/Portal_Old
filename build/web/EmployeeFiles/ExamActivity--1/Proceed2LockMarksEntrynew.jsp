...
<%	try{
  ...
  qry=qry+" where A.institutecode='"+mIC+"' and A.EVENTSUBEVENT='"+mEventsubevent+"'   and nvl(a.DEACTIVE,'N')='N' and ";
  ...
  qry=qry+" UNION select A.fstid from FACULTYSUBJECTTAGGING where companycode='"+mComp+"' and institutecode='"+mInst+"' and facultytype=decode('"+mDMemberType+"','E','I','E') and employeeid='"+mDMemberID+"' AND studentid IN (   SELECT DISTINCT studentid     FROM debarstudentdetail                      WHERE NVL (reexamination, 'N') = 'Y' and examcode='"+mEC+"'  AND subjectID='"+mSC+"' ) and FSTID in (select fstid from facultysubjecttagging where examcode='"+mEC+"' AND subjectID='"+mSC+"'))";
  ...
  qry=qry+" ) and   exists (select 'y' from V#STUDENTEVENTSUBJECTMARKS b where a.fstid=b.fstid and a.studentid=b.studentid and a.EVENTSUBEVENT=b.EVENTSUBEVENT and (nvl(detained,'N')='D' or nvl(detained,'N')='A' or nvl(marksawarded1,-1)<0 ) and nvl(reexam,'N')='Y' ) order by enrollmentno ";
	...
	qry1=qry1+" where a.institutecode='"+mIC+"' and nvl(a.DEACTIVE,'N')='N'  and ";
	...
	qry1=qry1+" and   exists (select 'y' from V#STUDENTEVENTSUBJECTMARKS b where a.fstid=b.fstid and a.studentid=b.studentid and a.EVENTSUBEVENT=b.EVENTSUBEVENT and (nvl(detained,'N')='D'       OR  NVL (detained, 'N') = 'U'                            OR NVL (detained, 'N') = 'M'              or nvl(detained,'N')='A' or nvl(marksawarded1,-1)<0 ) and nvl(reexam,'N')='Y' )";
	qry1=qry1+" AND a.studentid IN ( SELECT DISTINCT studentid   FROM debarstudentdetail                             WHERE NVL (reexamination, 'N') = 'Y' and examcode='"+mEC+"' and subjectID='"+mSC+"' and REEXAMEVENTSUBEVENT='"+mEventsubevent+"'  ) GROUP BY a.fstid, a.studentid, a.StudentName, a.enrollmentno, a.Semester, a.programcode, a.SECTIONBRANCH, a.subsectioncode";
	...
	qry1=qry1+" and   exists (select 'y' from V#STUDENTEVENTSUBJECTMARKS b where a.fstid=b.fstid and a.studentid=b.studentid and a.EVENTSUBEVENT=b.EVENTSUBEVENT and (nvl(detained,'N')='D'                             OR  NVL (detained, 'N') = 'U'                            OR NVL (detained, 'N') = 'M'                       or nvl(detained,'N')='A' or nvl(marksawarded1,-1)<0 )and nvl(reexam,'N')='Y' )";
	qry1=qry1+" AND a.studentid IN (  SELECT DISTINCT studentid  FROM debarstudentdetail  WHERE NVL (reexamination, 'N') = 'Y' AND FSTID IN (SELECT FSTID FROM EX#SUBJECTGRADECOORDINATOR WHERE COMPANYCODE='"+mComp+"' and INSTITUTECODE='"+mIC+"' and FACULTYTYPE=decode('"+mDMemberType+"','E','I','E') and FACULTYID='"+mDMemberID+"') )GROUP BY a.fstid, a.studentid, a.StudentName, a.enrollmentno, a.Semester, a.programcode, a.SECTIONBRANCH, a.subsectioncode";
	qry1=qry1+" order by enrollmentno";
    }
	   ...
	   if(1==2)
  {
	...
	if(mflag.equals("Y"))
	{
	    ...
	    <%	
		 ...
		 {
		%>
		<tr><td colspan=3 align=center><a href='MarksEntryGridVersionnew.jsp'><font color=blue><b>Continue with Marks Entry</b></font></a></td></tr>
		</table>
		<%
		}
		else
		...
		if(mself.equals("Self"))
		{
			...
			qry="Update STUDENTEVENTSUBJECTMARKS set REEXAMLOCKED='Y' where EVENTSUBEVENT='"+mEventsubevent+"'";
			qry=qry+" and FSTID in (select FSTID from FacultySubjectTagging where INSTITUTECODE='"+mIC+"' And ExamCode='"+mEC+"' and SubjectID='"+mSC+"' and (LTP='L' OR PROJECTSUBJECT='Y' OR LTP='P') and EMPLOYEEID='"+mDMemberID+"'";
			qry=qry+" UNION (Select FSTID from FACULTYSUBJECTTAGGING where INSTITUTECODE='"+mInst+"' and COMPANYCODE='"+mComp+"' AND EMPLOYEEID='"+mDMemberID+"' and FSTID IN (Select FSTID from FacultySubjectTagging Where INSTITUTECODE='"+mIC+"' And ExamCode='"+mEC+"' and SubjectID='"+mSC+"' and (LTP='L' OR PROJECTSUBJECT='Y' OR LTP='P')) and facultytype=decode('"+mDMemberType+"','E','I','E')))";
			//out.println(qry);
      		a=db.update(qry);
...
<%
 ...
 qry="Select 'Y' From StudentEventSubjectMarks Where FSTID='"+rs7.getString("fstid")+"' and EVENTSUBEVENT='"+rs7.getString("PEVENTSUBEVENT")+"' and STUDENTID='"+rs7.getString("studentid")+"'";
                ...
                if(rs.next())
					...
					qry="Update StudentEventSubjectMarks set MARKSAWARDED1=0, MARKSAWARDED2=0, DETAINED='N' ,REEXAMLOCKED='Y' Where FSTID='"+rs7.getString("fstid")+"' and EVENTSUBEVENT='"+rs7.getString("PEVENTSUBEVENT")+"' and STUDENTID='"+rs7.getString("studentid")+"'";
				//	out.println(qry);
					db.update(qry);
...
qry1="SELECT distinct b.studentname,b.ENROLLMENTNO, nvl(a.EVENTSUBEVENT,'N') PEVENTSUBEVENT,  B.WEIGHTAGE WEIGHTAGE,A.PRORATA,b.EVENTSUBEVENT, b.STUDENTID,b.FSTID FROM DEBARSTUDENTDETAIL A,V#STUDENTEVENTSUBJECTMARKS b WHERE   a.STUDENTID=b.STUDENTID and a.FSTID=b.FSTID and b.EXAMCODE='"+mEC+"' AND NVL(A.PRORATA,'N')='Y'  and b.INSTITUTECODE='"+mIC+"'   and b.subjectid='"+mSC+"'  and a.INSTITUTECODE=b.INSTITUTECODE and a.EXAMCODE=b.EXAMCODE and a.REEXAMEVENTSUBEVENT=b.EVENTSUBEVENT  AND B.EVENTSUBEVENT IN (SELECT  EXAMEVENTCODE FROM EXAMEVENTMASTER C  WHERE      C.EXAMCODE='"+mEC+"' AND C.INSTITUTECODE='"+mIC+"' AND NVL(C.PRORATAMARKS,'N')='Y'  ) ";
...
//mSMarks=mProrataTotalMarks;
 ...
 qry="Select 'Y' From StudentEventSubjectMarks Where FSTID='"+rs6.getString("fstid")+"' and EVENTSUBEVENT='"+mProrataEvent+"' and STUDENTID='"+rs6.getString("studentid")+"'";
                ...
                if(!rs.next())
				{
					qry = "Insert Into StudentEventSubjectMarks (FSTID, EVENTSUBEVENT, STUDENTID, MARKSAWARDED1, MARKSAWARDED2, DETAINED, LOCKED, ENTRYDATE, ENTRYBY, DEACTIVE, DETAINED2, REEXAM) VALUES (";
				...
				else
				{
					...
					qry="Update StudentEventSubjectMarks set MARKSAWARDED1='"+mSMarks+"', MARKSAWARDED2='"+mSMarks+"', DETAINED='N', DETAINED2='N' ,REEXAM='Y' Where FSTID='"+rs6.getString("fstid")+"' and EVENTSUBEVENT='"+mProrataEvent+"' and STUDENTID='"+rs6.getString("studentid")+"'";
				//out.println(qry);
					db.update(qry);
