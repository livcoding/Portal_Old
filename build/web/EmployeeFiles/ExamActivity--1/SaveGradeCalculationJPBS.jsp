...
try
{
	...
	String qrysl="";
		...
		String Studid="";
		String Fstid="";
		String mGrade="",MassCutCheck="";
		String marks="",MassCuts="",TotalMarks="",MarksAdd="";
	...
	if(!mMemberID.equals("") && !mMemberCode.equals("") && !mMemberName.equals("")) 
	{
		...
		//out.print(mCheckFstid+"   99999999999999 "+mExamCode);
...
if(mStatus==1)
{
	...
	for(int yy =1; yy<=sno;yy++)
	{
		...
		else
		{
			...
			if(request.getParameter("MassCuts"+yy)==null)
				{
					MassCuts="0";
				}
				else
				{
					MassCuts=request.getParameter("MassCuts"+yy);
				}
					if(request.getParameter("MarksAdd"+yy)==null)
				{
					MarksAdd="0";
				}
				else
				{
					MarksAdd=request.getParameter("MarksAdd"+yy);
				}
				
				if(request.getParameter("FinalMarks"+yy)==null)
				{
					marks="";
				}
				else
				{
					marks=request.getParameter("FinalMarks"+yy);
				}
				
				if(request.getParameter("TotalMarks"+yy)==null)
				{
					TotalMarks=marks;
				}
				else
				{
					TotalMarks=request.getParameter("TotalMarks"+yy);
				}
			...
			if(rsk.next())
			{
				qry4="UPDATE STUDENTWISEGRADE SET MassCuts='"+MassCuts+"',MarksAdd='"+MarksAdd+"', INITIALMARKS='"+marks+"' ,FINALMARKS='"+TotalMarks+"', INITIALGRADE='"+mGrade+"', FINALGRADE='"+mGrade+"',ENTRYBY ='"+mChkMemID+"',ENTRYDATE=SYSDATE WHERE INSTITUTECODE='"+mInst+"' AND EXAMCODE='"+mExamCode+"' AND FSTID='"+Fstid+"' AND BREAK#SLNO='"+mBreakSlno+"' AND STUDENTID='"+Studid+"'  ";
			...
			else
			{
				...
				qry4=qry4+" INITIALMARKS,INITIALGRADE,MassCuts, MarksAdd , ";
				qry4=qry4+" FINALMARKS,FINALGRADE,ENTRYBY,ENTRYDATE,DOCMODE,ETOD) ";
				qry4=qry4+" VALUES ('"+mInst+"','"+mExamCode+"','"+Fstid+"' ,'"+mBreakSlno+"', ";
				qry4=qry4+" '"+Studid+"' ,'"+marks+"' ,'"+mGrade+"','"+MassCuts+"' ,'"+MarksAdd+"', ";
				qry4=qry4+" '"+TotalMarks+"','"+mGrade+"','"+mChkMemID+"',SYSDATE,'D','"+mETOD+"') ";
...
qryss="select nvl(count(studentid),0)aaa from  studentwisegrade where  InstituteCode='"+mInst+"' and  examcode='"+mExamCode+"' and BREAK#SLNO='"+mBreakSlno+"' ";
//out.print(qryss);
...
%>
<center>
<a href="GradeCalculationActionJPBS.jsp?Subject=<%=mSubjectCode%>&amp;ExamCode=<%=mExamCode%>&amp;mCheckFstid=<%=mCheckFstid%>&amp;checkctr=<%=mCheck%>&amp;count=<%=mCount%>&amp;FS=<%=fs%>&amp;NoStudent=<%=mStudentCount%>&amp;x=<%=x%>" ><font color=""><b>Click to Enter more Grades</b></font></a>
</center>
<%	//out.print(mExamCode+"mCount:::"+mCheckFstid+"mStudentCount"+mSubjectCode+"vxx"+mCheck+"sd"+mCount+"xcv"+fs+"mStudentCount"+mStudentCount+"xx"+x);
//response.sendRedirect("GradeCalculationActionJPBS.jsp?Subject="+mSubjectCode+"&ExamCode="+mExamCode+"&mCheckFstid="+mCheckFstid+"&checkctr="+mCheck+"&count="+mCount+"&FS="+fs+"&NoStudent="+mStudentCount+"&x="+x);
...
%>
	...
	<form name="frm" method="post" action="TempPagesecondJPBS.jsp">
	<%
...
<tr bgcolor="#ff8c00">
	...
	<td><b><font color=white >Student Name</font></b></td>
		...
		<td><b><font color=white >Marks Cuts</font></b></td>
			<td><b><font color=white >Marks Add</font></b></td>
			<%
			}
	%>
	<!-- 			<td><b><font color=white >Initial Marks</font></b></td> -->
	...
	<%
		if(MassCutCheck.equals("Y"))
			...
			%>
				...
				<td>&nbsp;<%=rslist.getString("MarksAdd")%></b></td>
			<%
			}
			%>

<!-- <td>&nbsp;<%=rslist.getString("INITIALMARKS")%></b></td> -->
