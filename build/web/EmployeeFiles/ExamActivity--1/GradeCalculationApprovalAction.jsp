		...
		if (request.getParameter("TotalCount")!=null && Integer.parseInt(request.getParameter("TotalCount").toString().trim())>0)
		{
			...
			//-------------
				if(!mChk.equals(""))
				{
					...
					{
						qry="UPDATE GRADECALCULATION SET STATUS='A', APPROVEDBY='"+mChkMemID+"', ";
...
//-----------------------------
				...
				qry="select Distinct A.EXAMCODE EXAMCODE, A.SUBJECTID SUBJECTID,B.SUBJECTCODE SUBJCODE, B.SUBJECT SUBJNAME, decode(A.Status,'A','Approved','C','Cancelled','D','Pending','F','Finalized','Not Approved')Status, nvl(A.APPROVEDBY,' ') AprBy";
				qry=qry+" from GRADECALCULATION A, SUBJECTMASTER B where A.INSTITUTECODE=B.INSTITUTECODE and A.SUBJECTID=B.SUBJECTID And A.INSTITUTECODE='"+mInst+"'";
				qry=qry+" And A.EXAMCODE='"+mECode+"' Order by SUBJNAME";
