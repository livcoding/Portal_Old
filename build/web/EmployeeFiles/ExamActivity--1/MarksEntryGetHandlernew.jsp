...
//System.out.print("JILIT - "+qry2);
		...
		//qry=qry+" AND employeeid='"+mDMemberID+"' and facultytype=decode('"+mDMemberType+"','E','I','E')";
                qry=qry+" AND (EMPLOYEEID=(Select '"+mDMemberID+"' EmployeeID from dual)) ";
		//qry=qry+" AND employeeid in (Select '"+mDMemberID+"' EmployeeID from dual UNION Select EmployeeID from FacultySubjectTagging where FSTID in (SELECT FSTID FROM MULTIFACULTYSUBJECTTAGGING WHERE COMPANYCODE='"+mComp+"' and INSTITUTECODE='"+mIC+"' and FACULTYTYPE=decode('"+mDMemberType+"','E','I','E') and EMPLOYEEID='"+mDMemberID+"')) and facultytype=decode('"+mDMemberType+"','E','I','E')";
                qry=qry+" AND EVENTSUBEVENT='"+mEvent+"'   and STUDENTID   IN (Select distinct studentid from debarstudentdetail where nvl(REEXAMINATION,'N' )='Y' and subjectID='"+mSC+"' and REEXAMEVENTSUBEVENT='"+mEvent+"' and examcode='"+mEC+"')  " ;
		qry=qry+" GROUP BY fstid,studentid,StudentName,enrollmentno,Semester, programcode,SECTIONBRANCH, subsectioncode";
		qry=qry+" order by "+mList+ " "+mOrder+ " ";
...
//System.out.print("JILIT - "+qry2);
		...
		qry=qry+"  and STUDENTID   IN (Select distinct studentid from debarstudentdetail where nvl(REEXAMINATION,'N' )='Y' AND FSTID IN (SELECT FSTID FROM EX#SUBJECTGRADECOORDINATOR WHERE COMPANYCODE='"+mComp+"' and INSTITUTECODE='"+mIC+"' and FACULTYTYPE=decode('"+mDMemberType+"','E','I','E') and FACULTYID='"+mDMemberID+"') and REEXAMEVENTSUBEVENT='"+mEvent+"' and examcode='"+mEC+"') GROUP BY fstid,studentid,StudentName,enrollmentno,Semester, programcode,SECTIONBRANCH, subsectioncode";
		qry=qry+" order by "+mList+ " "+mOrder+ " ";
		...
		qry=qry+" AND FSTID IN (SELECT FSTID FROM EX#SUBJECTGRADECOORDINATOR WHERE COMPANYCODE='"+mComp+"' and INSTITUTECODE='"+mIC+"' and FACULTYTYPE=decode('"+mDMemberType+"','E','I','E') and FACULTYID='"+mDMemberID+"')";
		qry=qry+" and STUDENTID   IN (Select distinct studentid from debarstudentdetail where nvl(REEXAMINATION,'N' )='Y' and  examcode='"+mEC+"' and REEXAMEVENTSUBEVENT='"+mEvent+"' and subjectID='"+mSC+"' )  GROUP BY fstid,studentid,StudentName,enrollmentno,Semester, programcode,SECTIONBRANCH, subsectioncode";
		qry=qry+" order by "+mList+ " "+mOrder+ " ";
