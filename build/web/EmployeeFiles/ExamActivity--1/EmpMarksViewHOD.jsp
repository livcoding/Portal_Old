...
<HTML>
<head>
<TITLE>#### <%=mHead%> [ View Students Marks By HOD] </TITLE>
...
else
{
  ...
  //-----------------------------
	...
	qry="Select WEBKIOSK.ShowLink('265','"+ mChkMemID+"','"+mChkMType+"','"+mRole+"','"+ mIPAddress +"') SL from dual";
...
%>
	...
	<tr><TD align=middle><font color="#a52a2a" size=4 face=verdana ><b>View Student Marks By HOD</b><font size=3></font</font></td></tr>
	</table>
...
/*	qry="select distinct A.EVENTSUBEVENT EVENTSUBEVENT, A.ExamCode ExamCode from V#STUDENTEVENTSUBJECTMARKS A where ";
	...
	qry="SELECT DISTINCT a.eventsubevent eventsubevent, a.examcode examcode           FROM v#studenteventsubjectmarks a          WHERE a.institutecode = '"+mInst+"'                      AND NVL (a.published, 'N') = 'Y' and  nvl(a.LOCKED,'N')='Y' and                       ( a.SUBJECTID,a.EXAMCODE) in (select subjectid,examcode  from PR#DEPARTMENTSUBJECTTAGGING where                      institutecode ='"+mInst+"' and DEPARTMENTCODE='"+mDept+"'  ) group by a.eventsubevent , a.examcode        ORDER BY examcode ";
...
try
{
	...
	qry="SELECT   a.subject || ' ( ' || a.subjectcode || ' )' subject, a.subjectid,         a.examcode, a.eventsubevent    FROM v#studenteventsubjectmarks a   WHERE a.institutecode = '"+mInst+"'      AND   ( a.SUBJECTID,a.EXAMCODE) in (select subjectid,examcode      from PR#DEPARTMENTSUBJECTTAGGING where                      institutecode = '"+mInst+"'  and DEPARTMENTCODE='"+mDept+"' ) and  nvl(a.LOCKED,'N')='Y'      AND NVL (a.published, 'N') = 'Y' GROUP BY a.subject || ' ( ' || a.subjectcode || ' )',         a.subjectid,         a.examcode,         a.eventsubevent ORDER BY examcode, eventsubevent ";
	//out.print(qry);
...
<%
	...
	qry="SELECT   a.subject || ' ( ' || a.subjectcode || ' )' subject, a.subjectid,         a.examcode, a.eventsubevent    FROM v#studenteventsubjectmarks a   WHERE a.institutecode = '"+mInst+"'      AND   ( a.SUBJECTID,a.EXAMCODE) in (select subjectid,examcode      from PR#DEPARTMENTSUBJECTTAGGING where                      institutecode = '"+mInst+"'  and DEPARTMENTCODE='"+mDept+"' ) and  nvl(a.LOCKED,'N')='Y'      AND NVL (a.published, 'N') = 'Y' GROUP BY a.subject || ' ( ' || a.subjectcode || ' )',         a.subjectid,         a.examcode,         a.eventsubevent ORDER BY subject ";
...
try{
	...
	qry=qry+" A.examcode='"+mEC+"'  and A.facultytype=decode('"+mDMemberType+"','E','I','E') and (A.ltp='L' OR A.PROJECTSUBJECT='Y' OR A.LTP='P') and A.subjectID='"+mSC+"' ";
