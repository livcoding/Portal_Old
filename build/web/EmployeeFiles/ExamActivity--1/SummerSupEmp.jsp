...
int mSems=0;
int mFlag=0;	
String mAcad="",mDesg="",mDept="",Studentid="",QryStudentid="",Enrollment="";
...
<form name="frm" method=post>
			...
			<tr><td><font color=#00008b face=arial size=2><STRONG>&nbsp; Name:&nbsp;</STRONG></font><font face="Vardana"><%=GlobalFunctions.toTtitleCase(mMemberName)%> [<%=mDMemberCode%>]</font>
			&nbsp;&nbsp;<font color=#00008b face=arial size=2><STRONG>Department/Designation:&nbsp;</STRONG></font><font face="Vardana"><%=mDept%>(<%=mDesg%>)</font>
			
</td></tr>
			<tr><td ALIGN=CENTER><HR><font color=#00008b face=arial size=2><STRONG>&nbsp; Exam Code</STRONG></font>
			<%
			qry="select distinct nvl(examcode,' ')examcode, EXAMPERIODFROM from exammaster where institutecode='"+mInst+"' AND eXAMCODE LIKE '%SUMM%' and nvl(deactive,'N')='N' and nvl(LOCKEXAM,'N')='N'    order by examcode Desc";
			//out.println(qry);
			rs=db.getRowset(qry);
			%>
			...
			<font color=#00008b face=arial size=2><STRONG>&nbsp; Student </STRONG></font>
			
		

			<input type=text name="Enrollment" tabindex="0" id="Enrollment" size=10 >	
			...
			if(request.getParameter("x")!=null)
			{
				...
				{
					 Enrollment=request.getParameter("Enrollment").toString().trim();
				}
			
		qry="Select EnrollmentNo enrollmentno,Studentname Name, StudentID , Semester, nvl(ProgramCODE,'UNK') ProgramCode, nvl(BranchCode,'UNK') BranchCode,quota From StudentMaster where enrollmentno='"+Enrollment+"' and InstiTuteCode='"+mInst+"' and nvl(deactive,'N')='N' and nvl(PROGRAMCOMPLETED ,'N')='N'  ";
		//out.print(qry);
  ...
  { 
	...
	QryStudentid=StudentRecordSet.getString("StudentID");
...
qry1=" SELECT max(semester)sem FROM STUDENTSGPACGPA  WHERE studentid = '"+QryStudentid+"' and institutecode = '"+mInst+"' ";  
...
<form name="frm1" method=post action="SummerSupActionEmp.jsp">
...
<INPUT TYPE="hidden" NAME="mMaxsem" ID="mMaxsem" value="<%=mMaxsem%>">
	...
	String Checked="";
/*
qry="SELECT DISTINCT C.enrollmentno en2, c.subjectid subjectid, c.subject,                c.subjectcode, c.semester, ' ' grade, a.coursecreditpoint,                a.supfees           FROM offersubjecttagging a,                DEBARSTUDENTDETAIL b,                 v#studentltpdetail c          WHERE a.institutecode = '"+ mInst +"'         AND  C.STUDENTID=B.STUDENTID             AND c.subjectid = a.subjectid            AND b.subjectid = a.subjectid            AND c.subjectid = b.subjectid          AND NVL(b.REGISTEREDSTATUS,'N')='N'            AND a.examcode = '"+QryExam+"'                       AND NVL (C.deactive, 'N') = 'N'                        AND a.institutecode = C.institutecode                        AND a.institutecode = b.institutecode                        AND a.institutecode = c.institutecode                        AND c.institutecode = b.institutecode                        AND NVL (C.deactive, 'N') = 'N'            AND C.studentid = '"+QryStudentid+"' AND (C.studentid, C.subjectid) IN (                  SELECT studentid, subjectid                     FROM debarstudentdetail                    WHERE examcode = '"+mPrevExmCode+"'                        AND studentid ='"+QryStudentid+"'                      AND institutecode = '"+ mInst +"'                      ) union ";*/



qry="SELECT DISTINCT c.enrollmentno en2, c.subjectid subjectid, c.subject,                c.subjectcode, c.semester, ' ' grade, a.coursecreditpoint,                a.supfees           FROM offersubjecttagging a,                 v#studentltpdetail c          WHERE a.institutecode =  '"+ mInst +"'            AND c.subjectid = a.subjectid            AND a.examcode ='"+QryExam+"'             AND NVL (c.deactive, 'N') = 'N'            AND a.institutecode = c.institutecode                     AND NVL (c.deactive, 'N') = 'N'            AND c.studentid = '"+QryStudentid+"'             AND (c.studentid, c.subjectid) IN (                   SELECT studentid, subjectid                     FROM debarstudentdetail                    WHERE examcode = '"+mPrevExmCode+"'                          AND studentid = '"+QryStudentid+"'                       AND institutecode = '"+ mInst +"' AND NVL (registeredstatus, 'N') = 'N')                      and                          (c.studentid, c.subjectid) not IN (                SELECT studentid, subjectid                     FROM studentresult                    WHERE studentid = '"+QryStudentid+"'                       AND institutecode =  '"+ mInst +"'  ) union ";
qry=qry+"SELECT DISTINCT v.enrollmentno en2, c.subjectid subjectid, c.subject,                c.subjectcode, B.semester, b.grade, a.coursecreditpoint,                a.supfees FROM offersubjecttagging a, studentresult b, studentmaster v,subjectmaster c WHERE a.institutecode = '"+ mInst +"' AND v.studentid = b.studentid AND v.studentid = v.studentid AND C.subjectid = a.subjectid AND B.subjectid = a.subjectid AND C.subjectid = B.subjectid AND NVL (b.fail, 'N') = 'Y'  and a.examcode = '"+QryExam+"' and nvl(v.DEACTIVE,'N')='N'  AND A.INSTITUTECODE=V.INSTITUTECODE AND A.INSTITUTECODE=B.INSTITUTECODE AND  A.INSTITUTECODE=C.INSTITUTECODE AND C.INSTITUTECODE=B.INSTITUTECODE  AND NVL(V.DEACTIVE,'N')='N'  AND V.studentid = '"+QryStudentid+"'   union SELECT DISTINCT v.enrollmentno en2, c.subjectid subjectid, c.subject,                c.subjectcode, B.semester, b.grade, a.coursecreditpoint,          a.supfees  FROM offersubjecttagging a, studentresult b, studentmaster v,subjectmaster c WHERE a.institutecode = '"+ mInst +"' AND v.studentid = b.studentid AND v.studentid = v.studentid and nvl(v.DEACTIVE,'N')='N' AND NVL(V.DEACTIVE,'N')='N' AND C.subjectid = a.subjectid AND C.subjectid = B.subjectid AND a.examcode = '"+QryExam+"' AND A.INSTITUTECODE=V.INSTITUTECODE AND A.INSTITUTECODE=B.INSTITUTECODE AND A.INSTITUTECODE=V.INSTITUTECODE AND A.INSTITUTECODE=C.INSTITUTECODE AND A.INSTITUTECODE=V.INSTITUTECODE AND C.INSTITUTECODE=B.INSTITUTECODE AND  b.GRADE not in ('A','A+','B+')  AND V.studentid = '"+QryStudentid+"'   and V.STUDENTID IN ( SELECT A.STUDENTID FROM STUDENTSGPACGPA A,STUDENTMASTER B  WHERE A.semester="+mMaxsem+" AND A.STUDENTID=B.STUDENTID AND CGPA< "+mMinsem+" ) union SELECT DISTINCT v.enrollmentno en2, c.subjectid subjectid, c.subject,                 c.subjectcode, B.semester, 'Not Registered' grade, a.coursecreditpoint,                a.supfees FROM offersubjecttagging a, nrstudentfailsubjects b,studentmaster v,subjectmaster c WHERE a.institutecode = '"+ mInst +"' AND v.studentid = b.studentid AND v.studentid = v.studentid and nvl(v.DEACTIVE,'N')='N' AND NVL(V.DEACTIVE,'N')='N' AND a.subjectid = b.subjectid AND a.subjectid = b.subjectid  and a.examcode ='"+QryExam+"' and a.subjectid=c.subjectid and a.subjectid=b.subjectid and a.institutecode=c.institutecode and a.institutecode=v.institutecode AND NVL(REGISTERED,'N')='N' AND V.studentid = '"+QryStudentid+"'   ORDER BY coursecreditpoint DESC";
//	out.print(qry);
...
int SrNo=0;
	while (StudentRecordSet.next())
	{
		...
		SrNo++;
		/*qrydebar="SELECT 'Y' FROM DEBARSTUDENTDETAIL WHERE INSTITUTECODE ='"+ mInst +"'   and STUDENTID='"+QryStudentid+"'  AND EXAMCODE='"+mPrevExmCode+"'   AND  SUBJECTID='"+StudentRecordSet.getString("subjectid")+"'  AND NVL(DEBAR,'N')='Y'   AND NVL(REGISTEREDSTATUS,'N')='N'";
	//	out.print(qrydebar+"LL");
	...
	ctot+=StudentRecordSet.getDouble("CourseCreditPoint");
		...
		qry1="SELECT 'Y' FROM SUPPLEMENTARYSUBJECTTAGGING WHERE INSTITUTECODE='"+ mInst +"' AND EXAMCODE='"+QryExam+"' AND  STUDENTID='"+QryStudentid+"' AND  SUBJECTID='"+StudentRecordSet.getString("subjectid")+"' ";	
		// out.print(qry1);
		rs1=db.getRowset(qry1);
		if(rs1.next())
		{
...
<input type="hidden" name="SubjectID<%=sno%>" id="SubjectID<%=sno%>" value="<%=StudentRecordSet.getString("subjectid")%>">
		<%
	qry2="select 'Y' from subjectmaster where subjectid='"+StudentRecordSet.getString("subjectid")+"'  and institutecode='"+ mInst +"' AND SUBJECT LIKE '%PROJECT%' ";
