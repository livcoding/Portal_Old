	...
	if (request.getParameter("CHOICE")!=null)
		...
		/*	qry=" SELECT distinct a.EXAMCODE from PREVENTMASTER a WHERE a.INSTITUTECODE='"+ mInst+"'  and nvl(a.PRCOMPLETED,'N')='N' and nvl(a.PRBROADCAST,'N')='Y'";
			...
			{	*/	
				...
				<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><b>Pre Registration Report</B></TD>
				</font></td></tr>
				</TABLE>
				<table cellpadding=1 cellspacing=0  align=center  border=2>
				<!--Institute****-->
					<tr><td><FONT color=black face=Arial size=2><STRONG>Institute</STRONG>
			&nbsp; &nbsp;<input type=text name=Inst tabindex="0" id="Inst" value="<%=mInst%>" readonly style="WIDTH: 40px">		</FONT>
			
			&nbsp;&nbsp;&nbsp;
				<!--*********Exam**********-->
			<FONT color=black><FONT face=Arial size=2><STRONG>Pre Reg Exam Code</STRONG></FONT></FONT>
				...
				try
				{
					...
					{
					%>
						<input type=text name=Exam tabindex="1" id="Exam" value="<%=rs.getString("EXAM")%>" readonly style="WIDTH: 120px">	
					<%
					}
				...
				<!--SubJect Type**********-->
					...
					<TR><TD>
				<INPUT TYPE="radio" NAME="CHOICE" value='Y' <%=mchk%> > 
				<FONT face=Arial size=2><b>Student have Given Preferences</b></FONT>	
				</TD></TR>
					
				<TR><TD>
				<INPUT TYPE="radio" NAME="CHOICE" value='N' <%=mchk1%>> 
				...
				<tr><td>
					<INPUT TYPE="checkbox" NAME="Elective" id="Elective" <%=mCheck%> value="E"><b><FONT face=Arial size=2>Students Having Elective Subjects.</font></b>
				</td></tr>
				
				<tr><td>
					<INPUT TYPE="checkbox" NAME="Back" id="Back"   <%=mCheck1%> value="B"><b><FONT face=Arial size=2>Students BackLog in Subjects (Offered Now)</font></b>
				</td></tr>

				<tr><td>
					<INPUT TYPE="checkbox" NAME="Core" id="Core" <%=mCheck2%> value="C"><b><FONT face=Arial size=2>Students Having Core Subjects or Unoffered Backlog Subjects.</font></b>
				</td></tr>
              ...
              // out.print("mCHOICE"+mCHOICE);

			%>	<form name=frm1 method=post action="PreRegistrationReportXLS.jsp"	>
				...
				if(mElective.equals("E"))
				{
					...
					qry="SELECT DISTINCT A.STUDENTID,nvl(C.ENROLLMENTNO,' ')ENROLLMENTNO,nvl(C.STUDENTNAME,' ')STUDENTNAME,nvl(A.PROGRAMCODE,' ')PROGRAMCODE,nvl(A.BRANCHCODE,' ')BRANCHCODE,A.SEMESTER SEMESTER FROM  STUDENTREGISTRATION A,PR#ELECTIVESUBJECTS B,STUDENTMASTER C WHERE NVL(A.REGALLOW,'N')='Y' and nvl(c.PROGRAMCOMPLETED,'N')='N' and C.ENROLLMENTNO is not null AND  A.EXAMCODE='"+mE+"' AND A.INSTITUTECODE='"+mInstitute+"'  and c.ENROLLMENTNO is not null   AND A.INSTITUTECODE=B.INSTITUTECODE AND A.STUDENTID=C.STUDENTID AND A.EXAMCODE=B.EXAMCODE AND A.ACADEMICYEAR=B.ACADEMICYEAR AND A.PROGRAMCODE=B.PROGRAMCODE AND A.TAGGINGFOR=B.TAGGINGFOR AND A.SECTIONBRANCH=B.SECTIONBRANCH AND A.SEMESTER=B.SEMESTER and  nvl(b.deactive,'N')='N' and  nvl(c.deactive,'N')='N'  and "+mExists+" (select studentid from pr#studentsubjectchoice ee where   ee.examcode = '"+mE+"' AND ee.institutecode = '"+mInstitute+"'  and  NVL (ee.deactive, 'N') = 'N'     AND a.institutecode = ee.institutecode  AND a.studentid =ee.studentid      AND a.examcode = ee.examcode  ) ORDER BY enrollmentno    ";
