...
String msubj="";
String mSubjType="";
String msubjType="",mST="",mCustomRun="",mCustom="";
int mFlag=0;
...
<HTML>
<head>
<TITLE>#### <%=mHead%> [Pre-Elective Subject(s) to be Run by HOD ] </TITLE>
		...
		if (RsChk.next() && RsChk.getString("SL").equals("Y"))
		  ...
		  //----------------------
			...
			if(rs.next())
			{
				...
				<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><B>Pre-Elective Subject(s) Running Finalization by HOD [Pre registration]</b></TD>
				</font></td></tr>
				</TABLE>
				...
				qry=qry+" INSTITUTECODE='"+mInstitute+"' ";
				//and ( nvl(ELRNNINGFINALIZEDBYHOD,'N')<>'Y' or nvl(LOADDISTRIBUTIONSTATUS,'N') in //('F','A'))";
				...
				<form name="frm1" ID="frm1" Action="RePRRegElectiveSubjToBeRunAction.jsp" method=post>
				<tr bgcolor='#ff8c00'>
				...
				qry=qry+" from PREVENTS E Where E.MEMBERTYPE<>'S' and E.MEMBERID='"+mDMemberID+"'  ";
				// And nvl(E.ELRNNINGFINALIZEDBYHOD,'N')='N' 
				qry=qry+" and nvl(E.LOADDISTRIBUTIONSTATUS,'N')='N' and nvl(E.DEACTIVE,'N')='N') )";
				//out.print(qry);
				rs=db.getRowset(qry);
					...
					if (mST.equals("E"))
						{
						qry="select BDT.PROGRAMCODE PROGRAMCODE ,BDT.ACADEMICYEAR ACADEMICYEAR,BDT.TAGGINGFOR TAGGINGFOR, BDT.SECTIONBRANCH SECTIONBRANCH, nvl(A.Subject,A.SubjectCode ) SUBJECT , A.SubjectCode SC, B.SubjectID SID, nvl(B.ElectiveCode, ' ') ElectiveCode, max(B.CHOICE) MaxChoice, nvl(C.SubjectRunning,'N')LastStatus,NVL(C.CUSTOMFINALIZED,'N')CUSTOMFINALIZED,B.SUBJECTTYPE SUBJECTTYPE,B.SEMESTER SEMESTER,B.SEMESTERTYPE SEMESTERTYPE from SUBJECTMASTER A, ";
						...
						qry=qry+" from PREVENTS D Where D.MEMBERTYPE<>'S' and MEMBERID='"+mDMemberID+"'  ";//And nvl(D.ELRNNINGFINALIZEDBYHOD,'N')='N' 
						qry=qry+" and nvl(D.LOADDISTRIBUTIONSTATUS,'N')='N' and nvl(D.DEACTIVE,'N')='N') )";
						...
						qry=qry+" Group By BDT.PROGRAMCODE ,BDT.ACADEMICYEAR,BDT.TAGGINGFOR, BDT.SECTIONBRANCH,B.ElectiveCode,A.SubjectCode,B.SubjectID,nvl(A.Subject,A.SubjectCode), nvl(C.SubjectRunning,'N'),C.CUSTOMFINALIZED,B.SUBJECTTYPE,B.SEMESTER,B.SEMESTERTYPE";
						qry=qry+" Order by ElectiveCode,count(*) Desc,Subject";


						/*qry="select distinct  BDT.PROGRAMCODE PROGRAMCODE ,BDT.ACADEMICYEAR ACADEMICYEAR,BDT.TAGGINGFOR TAGGINGFOR, BDT.SECTIONBRANCH SECTIONBRANCH, nvl(A.Subject,A.SubjectCode ) SUBJECT , A.SubjectCode SC, B.SubjectID SID, nvl(B.ElectiveCode, ' ') ElectiveCode, max(B.CHOICE) MaxChoice, nvl(C.SubjectRunning,'N')LastStatus,B.SUBJECTTYPE SUBJECTTYPE,B.SEMESTER SEMESTER,B.SEMESTERTYPE SEMESTERTYPE,B.EXAMCODE EXAMCODE from SUBJECTMASTER A, ";
						qry=qry+" PR#STUDENTSUBJECTCHOICE B, PR#ELECTIVESUBJECTS C, BRANCHDEPTTAGGING BDT  where A.INSTITUTECODE=B.INSTITUTECODE And A.INSTITUTECODE=C.INSTITUTECODE And B.SUBJECTTYPE='"+mST+"'";
						qry=qry+" And B.INSTITUTECODE='"+mInstitute+"' AND A.SUBJECTID=B.SUBJECTID and A.SUBJECTID=C.SUBJECTID and b.ExamCode=C.ExamCode and B.EXAMCODE='"+mE+"'  And  B.SEMESTERTYPE=decode('"+ mSemType +"','ALL',B.SEMESTERTYPE,'"+ mSemType +"') "; 
						qry=qry+" And B.SUBJECTTYPE='"+mST+"' And (b.INSTITUTECODE,b.EXAMCODE) in (select PE.INSTITUTECODE,PE.ExamCode from PREVENTMASTER PE Where PE.INSTITUTECODE='"+mInstitute+"' and PE.EXAMCODE='"+mE+"' and nvl(PE.PRCOMPLETED,'N')='N' and nvl(PE.PRBROADCAST,'N')='Y' and nvl(PE.PRREQUIREDFOR,'N')<>'S' and nvl(PE.DEACTIVE,'N')='N'";
						qry=qry+" And (PE.INSTITUTECODE, PE.PREVENTCODE) in (Select D.INSTITUTECODE, D.PREVENTCODE ";
						...
						qry=qry+" And C.INSTITUTECODE=BDT.INSTITUTECODE and C.ACADEMICYEAR=BDT.ACADEMICYEAR and C.PROGRAMCODE=BDT.PROGRAMCODE and c.TAGGINGFOR=BDT.TAGGINGFOR and c.SECTIONBRANCH=BDT.SECTIONBRANCH";
						qry=qry+" And BDT.DEPARTMENTCODE in (Select Departmentcode from HODList where EmployeeID='"+mDMemberID+"')";
						qry=qry+" Group By BDT.PROGRAMCODE ,BDT.ACADEMICYEAR,BDT.TAGGINGFOR, BDT.SECTIONBRANCH,B.ElectiveCode,A.SubjectCode,B.SubjectID,nvl(A.Subject,A.SubjectCode), nvl(C.SubjectRunning,'N'),B.SUBJECTTYPE,B.SEMESTER,B.SEMESTERTYPE,B.EXAMCODE ";
						qry=qry+" Order by ElectiveCode,count(*) Desc,Subject";*/
						}
					//out.print(qry);
					rs=db.getRowset(qry);
					...
					//	mData=1;
						...
						//out.print(mLastStatus);
						//mSubRun=rs.getString("SUBJECTRUNNING");
						%>
						<tr bgcolor="<%=mColor%>">
						...
						<td>
						<%
						String linkstr="RePRRegElectiveSubjCriteria.jsp?PCode="+rs.getString("PROGRAMCODE")+"&amp;SecBranch="+rs.getString("SECTIONBRANCH")+"&amp;SubjectID="+rs.getString("SID")+"&amp;SubjectType="+rs.getString("SUBJECTTYPE")+"&amp;Semester="+rs.getString("SEMESTER")+"&amp;SemType="+rs.getString("SEMESTERTYPE")+"&amp;AYear="+rs.getString("ACADEMICYEAR") ;
						%>
						...
						{
							%>
							  <a href ="RePRRegElectiveSubjCriteria.jsp?PCode=<%=rs.getString("PROGRAMCODE")%>&amp;SecBranch=<%=rs.getString("SECTIONBRANCH")%>&amp;SubjectID=<%=rs.getString("SID")%>&amp;SubjectType=<%=rs.getString("SUBJECTTYPE")%>&amp;Semester=<%=rs.getString("SEMESTER")%>&amp;SemType=<%=rs.getString("SEMESTERTYPE")%>&amp;AYear=<%=rs.getString("ACADEMICYEAR")%>">Click</a>
						  <%
						}
						else 
						{
						...
						//}
							 ...
							 for (int mCh=1;mCh<=maxCol;mCh++)
							{
								...
								qry=qry+" And (PE.INSTITUTECODE, PE.PREVENTCODE) in (Select D.INSTITUTECODE, D.PREVENTCODE ";
								qry=qry+" from PREVENTS D Where D.MEMBERTYPE<>'S' and MEMBERID='"+mDMemberID+"' ";
								//And nvl(D.ELRNNINGFINALIZEDBYHOD,'N')='N'
								qry=qry+" and nvl(D.LOADDISTRIBUTIONSTATUS,'N')='N' and nvl(D.DEACTIVE,'N')='N') )";
			...
			if (mData==0)
	     	{
				%>				 
				<tr><td colspan=<%=maxCol+5%> ALIGN=CENTER><font color=Blue size=3><b>Pre-Elective Subject(s) to be Run is <sup>#</sup>Finalized</b>?&nbsp; <input name="finalized" id="finalized" type="Radio" Value="YES"><b>Yes</b>&nbsp; &nbsp; &nbsp;<input Type="Radio" value="NO" checked name="finalized" id="finalized"><b>No</b>  </font></td></tr>
