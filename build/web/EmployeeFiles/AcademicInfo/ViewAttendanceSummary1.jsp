...
//------------------Start of 'L' Percentage--------------------
                               ...
                               qry1 = "SELECT distinct count(tcount )tcount FROM (select distinct CLASSTIMEFROM Tcount from V#STUDENTATTENDANCE a where  SubjectID='" + mSubject + "' and LTP='L' ";
			   		 ...
			   		 if(rs1.getLong("Semester")==1 &&   !mREGCONFIRMATIONDATE.equals(" "))
						{
						   qry1 = qry1 +" and trunc(ATTENDANCEDATE)>=TO_Date('"+mREGCONFIRMATIONDATE+"','dd-mm-yyyy')";
                               ...
                               qry1 = qry1 + "  UNION ALL select  distinct CLASSTIMEFROM  Tcount from STUDENTPREVATTENDENCE where  subjectid = '"+mSubject+"'     AND ltp ='L'     AND examcode = '"+mExam+"'  AND studentid = '"+rs1.getString("studentid")+"'  ";
							   qry1=qry1+"  AND  semestertype='"+QrySemType+"'	";  
						//	qry1=qry1+"  and employeeid = '"+QryEmpID+"'     		";  
					 if(rs1.getLong("Semester")==1 && !mREGCONFIRMATIONDATE.equals(" "))
					      {
					...
					//out.print(rs1.getLong("Semester")+" ==="+qry1);
					 ...
					 else
						{
                               ...
                               qry1 = "SELECT distinct count(pcount )pcount FROM (select distinct CLASSTIMEFROM Pcount from V#STUDENTATTENDANCE a where SubjectID='" + mSubject + "' and LTP='L' and EXAMCODE='" + mExam + "'";
									qry1=qry1+"  AND  semestertype='"+QrySemType+"'	";  
							//qry1=qry1+"  and employeeid = '"+QryEmpID+"'     		";  
						//	qry1=qry1+" and (   a.employeeid = '"+mChkMemID+"'          OR (a.fstid IN (                      SELECT b.fstid                        FROM multifacultysubjecttagging b                       WHERE a.fstid = b.fstid                             AND b.employeeid = '"+mChkMemID+"'  UNION (SELECT C.FSTID  FROM    EX#SUBJECTGRADECOORDINATOR c WHERE   c.INSTITUTECODE = '"+mInst+"' AND C.FACULTYID = '"+mChkMemID+"' AND C.FSTID=A.FSTID  )   )             )) ";
							qry1=qry1+" and SECTIONBRANCH=decode('"+QrySecBr+"','ALL',SECTIONBRANCH,'"+QrySecBr+"') and SUBSECTIONCODE=decode('"+QrySubSec+"','ALL',SUBSECTIONCODE,'"+QrySubSec+"')";
                              ...
                              qry1=qry1+"  UNION ALL  select  distinct CLASSTIMEFROM  Pcount from STUDENTPREVATTENDENCE where  subjectid = '"+mSubject+"'     AND ltp ='L'  AND NVL (present, 'N') = 'Y'   AND examcode = '"+mExam+"'  AND studentid = '"+ rs1.getString("studentid") +"'  ";
							  qry1=qry1+"  AND  semestertype='"+QrySemType+"'	";  
							//qry1=qry1+"  and employeeid = '"+QryEmpID+"'     		";  
							  if(rs1.getLong("Semester")==1 && !mREGCONFIRMATIONDATE.equals(" "))
								{
...
//------------------End of 'L' Percentage--------------------
%>
                        <td ALIGN=left>&nbsp;<a Title="View Date wise Student 'L' Attendance" target=_New href="ViewAttendanceSummaryReportDet11.jsp?RightsID=<%=mRightsID%>&amp;EXAM=<%=mExam%>&amp;SID=<%=rs1.getString("studentid")%>&amp;SC=<%=mSubject%>&amp;LTP=L&amp;SEC=<%=QrySecBr%>&amp;SUBSEC=<%=QrySubSec%>&amp;SEMESTERTYPE=<%=QrySemType%>&amp;EMPID=<%=QryEmpID%>"><font color="<%=mColor%>"><b><%=QryPercDecimal%></b> %</font></a></td>
                        <%
                           }
                           //else
                           //{
...
//------------------Start of 'T' Percentage--------------------
                               ...
                               mTotTCls = 0;
                               mTotTPrs = 0;
                               qry1 = "SELECT distinct count(tcount )tcount FROM (select distinct CLASSTIMEFROM  Tcount from V#STUDENTATTENDANCE a where SubjectID='" + mSubject + "' ";
                               //qry1=qry1+" and FSTID='"+rs1.getString("Fstid")+"'";
                               qry1 = qry1 + " and EXAMCODE='" + mExam + "' and INSTITUTECODE='" + mInst + "' and nvl(DEACTIVE,'N')='N'  ";
							   	qry1=qry1+"  AND  semestertype='"+QrySemType+"' and LTP='T'	";  
						//	qry1=qry1+" and (   a.employeeid = '"+QryEmpID+"'             OR (a.fstid IN (                      SELECT b.fstid                        FROM multifacultysubjecttagging b                       WHERE a.fstid = b.fstid                             AND b.employeeid = '"+QryEmpID+"'  UNION (SELECT C.FSTID  FROM    EX#SUBJECTGRADECOORDINATOR c WHERE   c.INSTITUTECODE = '"+mInst+"' AND C.FACULTYID = '"+QryEmpID+"' AND C.FSTID=A.FSTID  )   )             )) "; 
							qry1=qry1+" and SECTIONBRANCH=decode('"+QrySecBr+"','ALL',SECTIONBRANCH,'"+QrySecBr+"') and SUBSECTIONCODE=decode('"+QrySubSec+"','ALL',SUBSECTIONCODE,'"+QrySubSec+"')";
			   					if(rs1.getLong("Semester")==1 && !mREGCONFIRMATIONDATE.equals(" "))
								{
								qry1=qry1+" and trunc(ATTENDANCEDATE)>=TO_Date('"+mREGCONFIRMATIONDATE+"','dd-mm-yyyy')";
			                    ...
			                    qry1=qry1+"  AND  semestertype='"+QrySemType+"'	";  
							//	qry1=qry1+"  and employeeid = '"+QryEmpID+"'     		";  
                  			            qry1=qry1+"  UNION ALL select  distinct CLASSTIMEFROM  Tcount from STUDENTPREVATTENDENCE where  subjectid = '"+mSubject+"'   AND ltp ='T'     AND examcode = '"+mExam+"'  AND studentid = '"+rs1.getString("studentid")+"'  ";
								if(rs1.getLong("Semester")==1 && !mREGCONFIRMATIONDATE.equals(" "))
									{
								qry1=qry1+" and trunc(ATTENDANCEDATE)>=TO_Date('"+mREGCONFIRMATIONDATE+"','dd-mm-yyyy')";
									}
                               ...
                               qry1 = "SELECT distinct count(pcount )pcount FROM (select distinct CLASSTIMEFROM Pcount from V#STUDENTATTENDANCE a where SubjectID='" + mSubject + "' and LTP='T' and EXAMCODE='" + mExam + "'";
                               	qry1=qry1+"  AND  semestertype='"+QrySemType+"'	";  
						//	qry1=qry1+" and (   a.employeeid = '"+QryEmpID+"'             OR (a.fstid IN (                      SELECT b.fstid                        FROM multifacultysubjecttagging b                       WHERE a.fstid = b.fstid                             AND b.employeeid = '"+QryEmpID+"'  UNION (SELECT C.FSTID  FROM    EX#SUBJECTGRADECOORDINATOR c WHERE   c.INSTITUTECODE = '"+mInst+"' AND C.FACULTYID = '"+QryEmpID+"' AND C.FSTID=A.FSTID  )   )             )) "; 
							...
							qry1=qry1+" and SECTIONBRANCH=decode('"+QrySecBr+"','ALL',SECTIONBRANCH,'"+QrySecBr+"') and SUBSECTIONCODE=decode('"+QrySubSec+"','ALL',SUBSECTIONCODE,'"+QrySubSec+"')";
                              ...
                              qry1=qry1+"   UNION ALL select  distinct CLASSTIMEFROM  Pcount from STUDENTPREVATTENDENCE where  subjectid = '"+mSubject+"'      AND ltp ='T'  AND NVL (present, 'N') = 'Y'   AND examcode = '"+mExam+"'  AND studentid = '"+ rs1.getString("studentid") +"'   ";
							  if(rs1.getLong("Semester")==1 && !mREGCONFIRMATIONDATE.equals(" "))
								{
								qry1=qry1+" and trunc(ATTENDANCEDATE)>=TO_Date('"+mREGCONFIRMATIONDATE+"','dd-mm-yyyy')";
								}
...
//------------------End of 'T' Percentage--------------------
%>
                        <td ALIGN=left>&nbsp;<a Title="View Date wise Student 'T' Attendance" target=_New href="ViewAttendanceSummaryReportDet11.jsp?RightsID=<%=mRightsID%>&amp;EXAM=<%=mExam%>&amp;SID=<%=rs1.getString("studentid")%>&amp;SC=<%=mSubject%>&amp;LTP=T&amp;SEC=<%=QrySecBr%>&amp;SUBSEC=<%=QrySubSec%>&amp;SEMESTERTYPE=<%=QrySemType%>&amp;EMPID=<%=QryEmpID%>"><font color="<%=mColor%>"><b><%=QryPercDecimal%></b> %</font></a></td>
                        <%
                           }
                           //else
                           //{
                        ...
                        <%
                           ...
                           if (TFST > 0 && LFST > 0) {
                               try {
								  ...
								  // out.print(mTotLCls+"mTotLCls_____mTotTCls"+mTotTCls+"mTLTCls_"+mTLTCls);
								  // out.print(mTotLPrs+"mTotLPrs_____mTotTPrs"+mTotTPrs+"mTLTPrs"+mTLTPrs);
                                   if (mTLTCls == 0) {
                                       mPercAtt = 0;
                                   } else {
								   ...
								   QryPracPercDecimal=new BigDecimal(mPercAtt);
                                   // out.println ( bd.setScale ( 2,BigDecimal.ROUND_HALF_UP )  ) ;
                                    QryPracPercDecimal= QryPracPercDecimal.setScale ( 2,BigDecimal.ROUND_HALF_UP ) ;

									//out.print(QryPracPercDecimal+"QryPracPercDecimal") ;
                               } catch (ArithmeticException e) {
                                   //out.print(e);
                               }
                        %>
                        <td ALIGN=CENTER>&nbsp;<a Title="View Date wise Student 'T' Attendance" target=_New href="ViewAttendanceSummaryReportDet11.jsp?RightsID=<%=mRightsID%>&amp;EXAM=<%=mExam%>&amp;SID=<%=rs1.getString("studentid")%>&amp;SC=<%=mSubject%>&amp;LTP=LT&amp;SEC=<%=QrySecBr%>&amp;SUBSEC=<%=QrySubSec%>&amp;SEMESTERTYPE=<%=QrySemType%>&amp;EMPID=<%=QryEmpID%>"><font color=""><b><%=QryPracPercDecimal%></b> %</font></a></td>
                        <%	
                           }
                           //else
                           //{
...
//------------------Start of 'P' Percentage--------------------
                               QryTotCls = 0;
                               QryTotPrs = 0;
                               qry1 = "SELECT distinct count(tcount )tcount FROM (select distinct CLASSTIMEFROM Tcount from V#STUDENTATTENDANCE a where SubjectID='" + mSubject + "' and LTP='P' ";
	                         qry1 = qry1 + "  and EXAMCODE='" + mExam + "' and INSTITUTECODE='" + mInst + "' and nvl(DEACTIVE,'N')='N'   ";
							 	qry1=qry1+"  AND  semestertype='"+QrySemType+"'	";  
						//	qry1=qry1+" and (   a.employeeid = '"+QryEmpID+"'             OR (a.fstid IN (                      SELECT b.fstid                        FROM multifacultysubjecttagging b                       WHERE a.fstid = b.fstid                             AND b.employeeid = '"+QryEmpID+"'  UNION (SELECT C.FSTID  FROM    EX#SUBJECTGRADECOORDINATOR c WHERE   c.INSTITUTECODE = '"+mInst+"' AND C.FACULTYID = '"+QryEmpID+"' AND C.FSTID=A.FSTID  )   )             )) "; 
		 			 ...
		 			 if(rs1.getLong("Semester")==1 && !mREGCONFIRMATIONDATE.equals(" "))
                   	 ...
                   	 qry1 = qry1 + "  UNION ALL select distinct CLASSTIMEFROM Tcount from STUDENTPREVATTENDENCE where  subjectid = '"+mSubject+"'      AND ltp ='P'     AND examcode = '"+mExam+"'  AND studentid = '"+rs1.getString("studentid")+"'  ";
					 if(rs1.getLong("Semester")==1 && !mREGCONFIRMATIONDATE.equals(" "))
						{
							qry1=qry1+" and trunc(ATTENDANCEDATE)>=TO_Date('"+mREGCONFIRMATIONDATE+"','dd-mm-yyyy')";
						}
					 qry1=qry1+" and   NVL (deactive, 'N') = 'N')";
                            //   out.print(qry1);
                               ...
                               qry1 = "SELECT distinct count(pcount )pcount FROM (select distinct CLASSTIMEFROM pcount from V#STUDENTATTENDANCE a where SubjectID='" + mSubject + "' and LTP='P' and EXAMCODE='" + mExam + "'";
                               	qry1=qry1+"  AND  semestertype='"+QrySemType+"'	";  
						//	qry1=qry1+" and (   a.employeeid = '"+QryEmpID+"'             OR (a.fstid IN (                      SELECT b.fstid                        FROM multifacultysubjecttagging b                       WHERE a.fstid = b.fstid                             AND b.employeeid = '"+QryEmpID+"'  UNION (SELECT C.FSTID  FROM    EX#SUBJECTGRADECOORDINATOR c WHERE   c.INSTITUTECODE = '"+mInst+"' AND C.FACULTYID = '"+QryEmpID+"' AND C.FSTID=A.FSTID  )   )             )) "; 
							...
							qry1=qry1+" and SECTIONBRANCH=decode('"+QrySecBr+"','ALL',SECTIONBRANCH,'"+QrySecBr+"') and SUBSECTIONCODE=decode('"+QrySubSec+"','ALL',SUBSECTIONCODE,'"+QrySubSec+"')";
							   ...
							   if(rs1.getLong("Semester")==1 && !mREGCONFIRMATIONDATE.equals(" "))
								...
								qry1=qry1+" and trunc(ATTENDANCEDATE)>=TO_Date('"+mREGCONFIRMATIONDATE+"','dd-mm-yyyy')";
								 ...
								 qry1=qry1+"   UNION ALL select  distinct CLASSTIMEFROM Pcount from STUDENTPREVATTENDENCE where  subjectid = '"+mSubject+"'    AND ltp ='P'  AND NVL (present, 'N') = 'Y'   AND examcode = '"+mExam+"'  AND studentid = '"+ rs1.getString("studentid") +"'  ";
								 if(rs1.getLong("Semester")==1 && !mREGCONFIRMATIONDATE.equals(" "))
								{
								qry1=qry1+" and trunc(ATTENDANCEDATE)>=TO_Date('"+mREGCONFIRMATIONDATE+"','dd-mm-yyyy')";
								}
...
//------------------End of 'P' Percentage--------------------
%>
                        <td ALIGN=left>&nbsp;<a Title="View Date wise Student 'P' Attendance" target=_New href="ViewAttendanceSummaryReportDet11.jsp?RightsID=<%=mRightsID%>&amp;EXAM=<%=mExam%>&amp;SID=<%=rs1.getString("studentid")%>&amp;SC=<%=mSubject%>&amp;LTP=P&amp;SEC=<%=QrySecBr%>&amp;SUBSEC=<%=QrySubSec%>&amp;SEMESTERTYPE=<%=QrySemType%>&amp;EMPID=<%=QryEmpID%>"><font color="<%=mColor%>"><b><%=QryPercDecimal%></b> %</font></a></td>
                        <%
                           }
                       //else
                       //{
