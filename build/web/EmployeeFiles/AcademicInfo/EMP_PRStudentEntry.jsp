<%@ page language="java"  buffer="1000kb" import="java.sql.*,tietwebkiosk.*" %>
...
function Clickedsel(k,j,c,b,t){	
	...
	if(tt==0){
		...
		if(document.frm1.studelecchoise.value!='' && document.frm1.studelecchoise.value!=0){		
...
function limit(e){	
	...
	var newval=parseInt(c4)*3;
	
	if(document.frm1.ggg.value==""){
		document.frm1.ggg.value=ald-newval;
	}
	var newald= parseInt(document.frm1.ggg.value)+choisept;
	document.frm1.TotalCCPAld.value=parseInt(document.frm1.ggg.value);
...
//document.frm1.TotalCCPAld.value=parseInt(document.frm1.ggg.value)+(parseInt(c4)*3);
			}else{
				for(i=1;i<=ttt;i++){
					if(document.getElementById("Selectf"+i).value==""){
						tt=0;			
				...
				if(tt==0){
					...
					if(!(parseInt(document.frm1.TotalCCP.value)>=(corecr+3))){
						...
						if(parseInt(c1)>=parseInt(c2)){	// && parseInt(c1)<=parseInt(c3)
							...
							alert("No. Of Subject should be greater then "+document.frm1.maxValueLmitE.value);
							document.frm1.studelecchoise.value="";
							frm1.studelecchoise.focus();
							for(i=1;i<=ec;i++){
								document.getElementById("Select"+i).value="";
								document.getElementById("temp"+i).value="Y";			
...
function test(){
	...
	if(document.frm1.mSem.value!='7'){
		...
		if(document.frm1.studelecchoise.value=='0' || document.frm1.studelecchoise.value=='' ){
			var aaa=(parseInt(document.frm1.minlimt.value)-(parseInt(document.frm1.TotalCCPAld.value)+3))/3;
			document.frm1.studelecchoise.value=aaa;
		}
	}
	if(document.frm1.studelecchoise.value=='0' && document.frm1.mSem.value!='7'){
		document.frm1.studelecchoise.disabled=true;
		for(i=1;i<=ec;i++){
			document.getElementById("Select"+i).disabled=true;
		}		
...
else
{
	mLoginComp=session.getAttribute("LoginComp").toString().trim();
		...
		qry="select Studentid,studentname from studentmaster where ENROLLMENTNO='"+studRoll+"'";
		//out.println(qry);
		 RsChk= db.getRowset(qry);
	...
	qry="select nvl(PREREGEXAMID,' ') EXAMID from COMPANYINSTITUTETAGGING Where COMPANYCODE='"+mLoginComp+"' And INSTITUTECODE='"+mInst+"'";
	rs=db.getRowset(qry);
	//out.print(qry);
	if (rs.next())
...
//--------------------Changed by Vijay---------------
		...
		String qryu="SELECT approved FROM prevents WHERE preventcode =(SELECT preventcode FROM preventmaster WHERE institutecode = '"+ mInst +"' AND NVL (prcompleted, 'N') = 'N' AND NVL (prbroadcast, 'N') = 'Y' AND prrequiredfor = 'E' AND NVL (deactive, 'N') = 'N' AND examcode = '"+mExam+"') AND memberid IN ( SELECT DISTINCT b.employeeid FROM pr#departmentsubjecttagging a, hodlist b WHERE examcode = '"+mExam+"' AND a.departmentcode = b.departmentcode AND NVL (a.deactive, 'N') = 'N' AND NVL (b.deactive, 'N') = 'N' AND subjectid IN ((SELECT   a.subjectid FROM pr#electivesubjects a WHERE a.institutecode = '"+ mInst +"' AND a.examcode = '"+mExam+"' AND a.academicyear = '"+mAcad+"' AND a.programcode = '"+mProg+"' AND a.taggingfor = '"+mTag+"' AND a.sectionbranch = '"+mySect+"' AND a.semester = '"+mSem+"' AND NVL (a.basket, 'B') = 'B' AND NVL (a.deactive, 'N') = 'N' GROUP BY a.subjectid) UNION  (SELECT DISTINCT a.subjectid subjectid   FROM pr#electivesubjects a,subjectmaster b WHERE a.institutecode = '"+ mInst +"'   AND a.academicyear = '"+mAcad+"'   AND a.programcode = '"+mProg+"' AND a.taggingfor = '"+mTag+"' AND a.sectionbranch = '"+mySect+"' AND a.semester = '"+mSem+"'  AND a.basket IN ('D', 'E') AND a.institutecode =b.institutecode AND a.subjectid = b.subjectid)))  ";
		ResultSet rsu=db.getRowset(qryu);
//		out.println(qryu);
		if(rsu.next())
		{			
			response.sendRedirect("EMP_PRStudentReEntry.jsp?studentEnRoll="+studRoll);
		}
		else
		{
 	...
 	if(!rss1.next())
		      ...
		      else if(rss1.getString("SENDTOHOD").equals("Y"))
			{
			...
			else
	      	{
				...
				<form name="frm1" method=post action="EMP_PRStudentAction.jsp">
...
<INPUT TYPE="hidden" NAME="ggg" value="">
				...
				<tr>
			     	<td colspan=2><FONT face=Arial size=2 color=black><STRONG>Student Name - </STRONG></FONT><%=GlobalFunctions.toTtitleCase(mSsname)%> (<%=studRoll%>)</td>
				...
				<input type="test" name="studelecchoise" value="<%=rss1.getString("NOOFELESUBJECTSCHOICES")%>" maxlength='1' size='2'   onkeydown="if(event.keyCode==13){event.keyCode=9;return event.keyCode}" <%=readonly%> onKeyup="return limit(event);">
				<%
				}else{%>
