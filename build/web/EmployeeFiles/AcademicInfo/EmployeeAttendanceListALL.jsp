...
<tr><TD align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: arial"><b>Faculty wise All Class Attendance</b><font size=3></font</font></td></tr>
</table>
...
<%
	...
	qry=qry +" ExamCode='"+QryExam+"' and institutecode='"+mINSTITUTECODE+"' and nvl(deactive,'N')='N'  order by FacultyName ";
	//out.print(qry);
	rs=db.getRowset(qry);
	%>
...
<%
	...
	%>
	<td align=center><a Title="View Date wise Attendance History" target=_New href='ViewDatewiseStudAttendanceALL.jsp?EXAM=<%=QryExam%>&amp;FAC=<%=mEmpID%>&amp;TYPE=<%=mClasstype%>&amp;SID=<%=mSUBJECTID%>&amp;SC=<%=mSUBJECTCODE%>&amp;LTP=<%=mLTP%>&amp;PROG=<%=mProg%>&amp;SEC=<%=mSec%>&amp;SUB=<%=mSub%>&amp;AttDate=<%=mAttDate%>&amp;Date1=<%=mDate1%>&amp;Date2=<%=mDate2%>&amp;TotClass=<%=mTotClass%>'><%=mTotClass%></a></td>
	<%
