...
try
{
  ...
  //----------------------
		...
		<!--*********Exam**********-->
			...
			try
			{
			 qry="Select Distinct NVL(EXAMCODE,' ')EXAMCODE from EXAMEVENTMASTER WHERE INSTITUTECODE='"+mInst+"' and nvl(Deactive,'N')='N' AND  EXAMCODE IN (SELECT EXAMCODE FROM EXAMMASTER WHERE INSTITUTECODE='"+mInst+"' AND NVL(LOCKEXAM,'N')='N' )  ";
			 rs=db.getRowset(qry);
		...
		<!--********Subject*****-->
			...
			qry=qry+" subjectmaster B where (A.LTP='L' OR A.PROJECTSUBJECT='Y') and A.employeeid='"+mDMemberID+"' and A.examcode='"+mExamID+"'  and a.INSTITUTECODE='"+mInst+"' and A.facultytype=decode('"+mDMemberType+"','E','I','E') ";	
			...
			if(request.getParameter("y")!=null)
			{
				...
				//*******************************************************************
					...
					qry="select nvl(B.EMPLOYEENAME,' ')||' ('||B.EMPLOYEECODE||')' EMPNM,B.EMPLOYEEID EMPLOYEEID FROM FACULTYSUBJECTTAGGING A, V#STUDENTLTPDETAIL B WHERE A.EMPLOYEEID=B.EMPLOYEEID AND A.FSTID=B.FSTID  AND  a.INSTITUTECODE='"+mInst+"'  and A.INSTITUTECODE=B.INSTITUTECODE and   nvl(B.deactive,'N')='N' AND NVL(B.STUDENTDEACTIVE,'N')='N' and A.SUBJECTID=B.SUBJECTID AND	A.FSTID in ("+QryFSTID+") order by EMPNM,B.EMPLOYEEID ";
					//out.println(qry);
					...
					<%
					qry="select  nvl(B.EMPLOYEENAME,' ')||' ('||B.EMPLOYEECODE||')' EMPNM,nvl(B.PROGRAMCODE,'')PROGRAMCODE, nvl(A.SECTIONBRANCH,' ')SECBR, nvl(A.SUBSECTIONCODE,'')SUBSEC, nvl(B.FSTID,'')FSTID,NVL(B.STUDENTNAME,'')STUDENTNAME,NVL(B.ENROLLMENTNO,' ')ENROLLMENTNO FROM FACULTYSUBJECTTAGGING A, V#STUDENTLTPDETAIL B WHERE A.EMPLOYEEID=B.EMPLOYEEID  AND		A.SECTIONBRANCH=B.SECTIONBRANCH  AND A.PROGRAMCODE=B.PROGRAMCODE AND  nvl(B.deactive,'N')='N' AND NVL(B.STUDENTDEACTIVE,'N')='N' AND	A.SUBSECTIONCODE=B.SUBSECTIONCODE AND A.SEMESTER=B.SEMESTER  and a.INSTITUTECODE='"+mInst+"'  and A.INSTITUTECODE=B.INSTITUTECODE AND A.FSTID=B.FSTID AND A.FSTID in ("+QryFSTID+") group by B.EMPLOYEENAME,B.EMPLOYEECODE,B.PROGRAMCODE,A.SECTIONBRANCH,A.SUBSECTIONCODE,B.FSTID,B.STUDENTNAME,B.ENROLLMENTNO ORDER BY EMPNM, SECBR, SUBSEC,PROGRAMCODE,STUDENTNAME";
