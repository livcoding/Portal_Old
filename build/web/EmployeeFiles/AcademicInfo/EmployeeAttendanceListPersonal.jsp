...
<tr><TD align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: arial"><b>Self Class Attendance</b><font size=3></font</font></td></tr>
</table>
...
<%
  qry="(select nvl(A.employeeid,' ')Faculty, nvl(A.employeename,' ')FacultyName from Employeemaster A where nvl(A.deactive,'N')='N' and A.EMPLOYEEID='"+mChkMemID+"' and A.EMPLOYEEID IN (SELECT EMPLOYEEID from v#studentattendance))";
  qry=qry+" union (select nvl(A.employeeid,' ')Faculty, nvl(A.employeename,' ')FacultyName from Employeemaster A where nvl(A.deactive,'N')='N' and A.EMPLOYEEID='"+mChkMemID+"' and A.EMPLOYEEID IN (SELECT EMPLOYEEID from studentattendanceexcused)) order by Faculty asc";
  rs=db.getRowset(qry);
  //out.print(qry);
	%>
...
<input style="width:200px;font-size:20px; 
	color:red;font-weight:bold;BORDER-LEFT: c000000px solid;BORDER-TOP: c00000 0px solid;
	BORDER-RIGHT: c000000px solid;BORDER-BOTTOM: c000000px solid;background-color:transparent;border-bottom-style:hidden; border-left-style:hidden; border-right-style:hidden;border-top-style:hidden;" name="twait" readonly id="twait" type="text" value="Please Wait.......">
</center>
</form>
<%
if(request.getParameter("x")!=null)
