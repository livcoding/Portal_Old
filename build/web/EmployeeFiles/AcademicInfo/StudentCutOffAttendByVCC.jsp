...
<HTML>
<head>
<TITLE>#### <%=mHead%> [ Student Cut-Off Attendance By Administrator ] </TITLE>
...
{  //1
  ...
  //-----------------------------
	qry="Select WEBKIOSK.ShowLink('267','"+ mChkMemID+"','"+mChkMType+"','"+mRole+"','"+ mIPAddress +"') SL from dual";
  ...
  //----------------------
	...
	<tr><TD align=middle><font color="#a52a2a" face="Arial" size="4">Complete Cut-off Attendance : Administrator</b><font size=3></font</font></td></tr>
	</table>
	<table rules=groups cellspacing=1 cellpadding=4 align=center border=1>
	<tr><td nowrap>
	<font color=black face=arial size=2>Exam Code</font>
	<%
	...
	//10102196  2011ODDSEM   1011   B.T
	%>
	</select>
	<font color=black face=arial size=2>&nbsp; &nbsp; Admission Year</font>
	<%
	...
	<font color=black face=arial size=2>&nbsp; &nbsp; Program Code</font>
	<%
...
String mSubjID="",QrySubCode="";
	...
	qry="select distinct  A.SUBJECTID SUBJECTID, A.SUBJECTCODE SUBJECTCODE, A.SUBJECT SUBJECT " +
			...
			qry=qry+" where A.ExamCode='"+QryExam+"' and nvl(A.STUDENTDEACTIVE,'N')='N' " +
                    ...
                    "Where A.FSTID=C.FSTID )  union   SELECT DISTINCT a.subjectid subjectid, a.subjectcode subjectcode,                a.subject subject           FROM v#studentltpdetail a          WHERE a.examcode = '"+QryExam+"'  and SEMESTERTYPE IN ('RWJ','SAP')            AND NVL (a.studentdeactive, 'N') = 'N'            AND a.academicyear = '"+QryAcadYr+"'             AND programcode = '"+QryProgCode+"'            AND a.institutecode = '"+mInst+"'            AND (a.fstid) IN (SELECT c.fstid                                FROM studentattendance c                               WHERE a.fstid = c.fstid)   order By SUBJECT";
			//out.print(qry);
			rs=db.getRowset(qry);
...
<form name="frm1" method=get action="StudentCutOffAttendVCCExl.jsp">
...
*/
		...
		ResultSet rsq=null,rsb=null;
				...
				String mSUP="",mRWJ="",QrySemType="",QryEmpID="";
			ResultSet rs1=null;
			double mCGPA=0.0;
			...
			qry="select distinct A.Studentid Studentid, A.StudentName StudentName, A.EnrollmentNo EnrollmentNo, " +
                    ...
                    " A.LTP LTP, A.SEMESTER SEMESTER, A.SECTIONBRANCH SECBR, A.SUBSECTIONCODE SUBSEC,a.ACADEMICYEAR ACADEMICYEAR,NVL(a.SEMESTERTYPE,' ')SEMESTERTYPE,a.employeeid  From" +
                    " v#studentltpdetail A";
			qry=qry+" where A.ExamCode='"+QryExam+"' and nvl(A.STUDENTDEACTIVE,'N')='N' and nvl(A.DEACTIVE,'N')='N' and " +
...
qry1=" SELECT   distinct count(tcount )tcount FROM (select distinct CLASSTIMEFROM Tcount from V#STUDENTATTENDANCE where SubjectID='"+mSubject+"' and LTP IN ("+QryLTPVal+")  and ACADEMICYEAR='"+QryAcadYr+"' and PROGRAMCODE='"+QryProgCode+"' ";
//qry1=qry1+" and FSTID='"+rs.getString("Fstid")+"'";
	qry1=qry1+"  and employeeid = '"+QryEmpID+"'     		";  
	qry1=qry1+"  AND  semestertype='"+QrySemType+"'	";  
qry1=qry1+" and INSTITUTECODE='"+mInst+"' and EXAMCODE='"+QryExam+"' and  nvl(DEACTIVE,'N')='N'";
if(rs.getInt("SEMESTER")==1 && !mREGCONFIRMATIONDATE.equals(""))
{
...
qry1=qry1+" union all select distinct CLASSTIMEFROM Tcount from STUDENTPREVATTENDENCE where SubjectID='"+mSubject+"' and EXAMCODE='"+QryExam+"' and LTP IN ("+QryLTPVal+") and SECTIONBRANCH='"+rs.getString("SECBR")+"' and SUBSECTIONCODE='"+rs.getString("SUBSEC")+"' and STUDENTID='"+rs.getString("studentid")+"'  and ACADEMICYEAR='"+QryAcadYr+"' and PROGRAMCODE='"+QryProgCode+"' ";
	qry1=qry1+"  AND  semestertype='"+QrySemType+"'	";  
		qry1=qry1+"  and employeeid = '"+QryEmpID+"'     		";  
if(rs.getInt("Semester")==1 && !mREGCONFIRMATIONDATE.equals(""))
{
...
qry1=qry1+" and  INSTITUTECODE='"+mInst+"' and nvl(DEACTIVE,'N')='N') ";
...
qry1="SELECT   distinct count(pcount )pcount FROM (select distinct CLASSTIMEFROM pcount from V#STUDENTATTENDANCE where SubjectID='"+mSubject+"' and LTP IN ("+QryLTPVal+") and EXAMCODE='"+QryExam+"'  and ACADEMICYEAR='"+QryAcadYr+"' and PROGRAMCODE='"+QryProgCode+"' ";
				//qry1=qry1+" and FSTID='"+rs.getString("Fstid")+"'";
					qry1=qry1+"  AND  semestertype='"+QrySemType+"'	"; 
						qry1=qry1+"  and employeeid = '"+QryEmpID+"'     		";  
					if(rs.getInt("SEMESTER")==1 && !mREGCONFIRMATIONDATE.equals(""))
		...
		qry1=qry1+" and trunc(ATTENDANCEDATE)>=TO_Date('"+mREGCONFIRMATIONDATE+"','dd-mm-yyyy')";
				...
				qry1=qry1+" and SECTIONBRANCH=decode('"+rs.getString("SECBR")+"','ALL',SECTIONBRANCH,'"+rs.getString("SECBR")+"') and SUBSECTIONCODE='"+rs.getString("SUBSEC")+"'";
				//qry1=qry1+" GROUP BY EXAMCODE, SubjectID";
qry1=qry1+" union all select  distinct CLASSTIMEFROM pcount from STUDENTPREVATTENDENCE where SubjectID='"+mSubject+"' and EXAMCODE='"+QryExam+"' and LTP IN ("+QryLTPVal+")  and ACADEMICYEAR='"+QryAcadYr+"' and PROGRAMCODE='"+QryProgCode+"' and SECTIONBRANCH='"+rs.getString("SECBR")+"' and SUBSECTIONCODE='"+rs.getString("SUBSEC")+"'  ";
	qry1=qry1+"  AND  semestertype='"+QrySemType+"'	";  
		qry1=qry1+"  and employeeid = '"+QryEmpID+"'     		";  
		if(rs.getInt("Semester")==1 && !mREGCONFIRMATIONDATE.equals(""))
		{
		...
		qry1=qry1+"  and STUDENTID='"+rs.getString("studentid")+"'  and INSTITUTECODE='"+mInst+"' and nvl(PRESENT,'N')='Y' and nvl(DEACTIVE,'N')='N'   )";
				rs2=db.getRowset(qry1);
				//out.print(qry1);
				if(rs2.next())
