...
ResultSet rsds=null, rsds1=null, rs=null;
...
try
{
	...
	if(!mMemberID.equals("") && !mMemberCode.equals("") && !mMemberName.equals(""))
	{
		...
		qry="UPDATE PR#HODLOADDISTRIBUTION SET MERGEWITHFSTID =NULL WHERE MERGEWITHFSTID=fstid";
		db.update(qry);
     ...
     //out.print(mInstitute+"mInstitute"+"mCompany"+mCompany);
		if(mType.equals("D"))
		{
		   mRights=129;
		   Heading="DOAA";	
		...
		if (RsChk.next() && RsChk.getString("SL").equals("Y"))
		  ...
		  //----------------------
				...
				<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: verdana"><b>Assigned Load List</B></TD>
				</font></td></tr>
				</TABLE>
				...
				try
				{
					qry="Select distinct ExamCode Exam from PR#HODLOADDISTRIBUTION where nvl(DEACTIVE,'N')='N' and  INSTITUTECODE='"+mInstitute+"' ";
					rs=db.getRowset(qry);
...
//--------------------------------------------SELF-------------------------------------
				 ...
				 {
				    qry="select distinct A.DEPARTMENTCODE DeptCode, nvl(A.DEPARTMENT,' ')DeptName from DEPARTMENTMASTER A ";
				    qry=qry+" where DEPARTMENTCODE IN (Select DEPARTMENTCODE from USERWISELOADPERMITION ";
				    qry=qry+" where COMPANYCODE='"+mCompany+"' And EMPLOYEEID='"+mChkMemID+"') order by DeptName";
/*
				    qry="select distinct B.DEPARTMENTCODE DeptCode, nvl(A.DEPARTMENT,' ')DeptName from DEPARTMENTMASTER A, V#STAFF B where A.DEPARTMENTCODE = B.DEPARTMENTCODE and nvl(A.deactive,'N')='N'";
...
*/
				...
				<select ID=ShowLoadOn Name=ShowLoadOn>
				<%
				if(request.getParameter("ShowLoadOn")==null)
				{
					mShowLoadOn="A";
					%>
			     		<option Selected value="D">Draft Load Distribution</option>
				...
				{
				%>
					<tr><td align=center><b>Department of <%=GlobalFunctions.toTtitleCase(rs.getString("Dept"))%><b></td></tr>
				<%
				}
				%>
				</table>
				...
				<tr bgcolor="#ff8c00">
				 ...
				 <td nowrap align=center title="Course Title"><font color=white><b>Course Title</b></font></td>
				  <td nowrap align=center title="Academic Year"><font color=white><b>Academic Year</b></font></td>
				 ...
				 <td nowrap title="Total Assigned Load" align=center><font color=white><b>  Multiple Faculty </b></font></td>
				</tr>
				</thead>
				<tbody>
				<%
...
//--------------------------------------------HOD-------------------------------------
					if(mType.equals("H"))
					{
						qry=" select Distinct A.SEMESTERTYPE, A.FACULTYID, A.FACULTYTYPE,a.ACADEMICYEAR, B.EMPLOYEENAME||' ['||B.EMPLOYEECODE||']' EMPLOYEENAME,";
						...
						qry=qry+" and A.INSTITUTECODE='"+mInstitute+"' and a.COMPANYCODE='"+mCompany+"' And EXAMCODE='"+mE+"' and nvl(STATUS,'D')='"+mShowLoadOn+"' and B.DEPARTMENTCODE='"+QryDept+"'";
...
//--------------------------------------------DOAA-------------------------------------
					else if(mType.equals("D"))
					{
						...
						qry=qry+" and A.COMPANYCODE=b.COMPANYCODE and A.facultyTYPE=b.EMPLOYEETYPE and ";
						qry=qry+" B.DEPARTMENTCODE=C.DEPARTMENTCODE AND B.DESIGNATIONCODE=D.DESIGNATIONCODE AND A.SUBJECTID=E.SUBJECTID and a.institutecode=e.institutecode";
						qry=qry+" and A.INSTITUTECODE='"+mInstitute+"' and a.COMPANYCODE='"+mCompany+"' And EXAMCODE='"+mE+"' and nvl(STATUS,'D')='"+mShowLoadOn+"' and B.DEPARTMENTCODE='"+QryDept+"' and nvl(b.deactive,'N')='N' Order By EMPLOYEENAME";
					}
//--------------------------------------------SELF-------------------------------------
					if(mType.equals("I"))
					{
						...
						qry=qry+" And A.FACULTYID IN (Select EMPLOYEEID from V#STAFF  where DEPARTMENTCODE IN (Select DEPARTMENTCODE from USERWISELOADPERMITION ";
						qry=qry+" where COMPANYCODE='"+mCompany+"' And EMPLOYEEID='"+mChkMemID+"')) and nvl(b.deactive,'N')='N' Order By EMPLOYEENAME";
/*
...
*/
				...
				//	out.println(qry);
					...
					{ 
						...
						%>
						<td nowrap><font color=<%=mcolor%>><%=rs.getString("SUBJECT")%> <B>(<%=rs.getString("SEMESTERTYPE")%>)</B></font></td>
						...
						{
						%>
							<td nowrap align=center><font size=2 color=<%=mcolor%>>Total <%=mLLoad%></font>						
							<Table align=left>
							<%
							qry="select  SUBSECTIONCODE , FSTID,  NOOFCLASSINAWEEK||'@'||DURATIONOFCLASS From PR#HODLOADDISTRIBUTION A where InstituteCode='"+mInstitute+"' And EXAMCODE='"+mE+"' and LTP='L' and  FACULTYTYPE='"+rs.getString("FACULTYTYPE")+"' and FACULTYID='"+rs.getString("FACULTYID")+"' And SUBJECTID='"+rs.getString("SUBJECTID")+"' and SEMESTERTYPE='"+rs.getString("SEMESTERTYPE")+"' AND  nvl(a.deactive,'N')='N' ";
						if(rs.getString("SEMESTERTYPE").equals("REG"))
								qry=qry+" and MERGEWITHFSTID is null "; 
							qry=qry+" Order By 1"; 
						//and MERGEWITHFSTID is null Order By 1";
							//out.println(qry);
							...
							while(rsds.next())
							{
								...
								qry1="select  SUBSECTIONCODE From PR#HODLOADDISTRIBUTION A where MERGEWITHFSTID='"+ rsds.getString("FSTID") +"' AND InstituteCode='"+mInstitute+"' And EXAMCODE='"+mE+"' and LTP='L' and  FACULTYTYPE='"+rs.getString("FACULTYTYPE")+"' and FACULTYID='"+rs.getString("FACULTYID")+"' And SUBJECTID='"+rs.getString("SUBJECTID")+"' AND SEMESTERTYPE='"+rs.getString("SEMESTERTYPE")+"' and nvl(a.deactive,'N')='N' Order By 1";
								rsds1=db.getRowset(qry1);
								//out.println(qry);
								mstr=rsds.getString("SUBSECTIONCODE");
								while(rsds1.next())
								{
								  mstr=mstr+",  "+rsds1.getString("SUBSECTIONCODE");
								//	mstr=rsds1.getString("SUBSECTIONCODE")+",";
								
								}	
								%>
								<tr>
								<td align=left><font size=2><font size=2 color=<%=mcolor%>>
								<%=mstr%></font>
								</td>
								</tr>
								<% 
						...
						{
						%>
							<td nowrap align=center><font size=2 color=<%=mcolor%>><%=mTLoad%></font>
							<Table align=left>
							<%
							qry="select  SUBSECTIONCODE , FSTID,  NOOFCLASSINAWEEK||'@'||DURATIONOFCLASS From PR#HODLOADDISTRIBUTION A where InstituteCode='"+mInstitute+"' And EXAMCODE='"+mE+"' and LTP='T' and  FACULTYTYPE='"+rs.getString("FACULTYTYPE")+"' and FACULTYID='"+rs.getString("FACULTYID")+"' And SUBJECTID='"+rs.getString("SUBJECTID")+"' and SEMESTERTYPE='"+rs.getString("SEMESTERTYPE")+"' AND nvl(a.deactive,'N')='N' ";
							if(rs.getString("SEMESTERTYPE").equals("REG"))
								qry=qry+" and MERGEWITHFSTID is null "; 
						qry=qry+" Order By 1"; 
						//and MERGEWITHFSTID is null Order By 1";
						//	out.println(qry);
							...
							while(rsds.next())
							{
								qry="select  SUBSECTIONCODE From PR#HODLOADDISTRIBUTION A where MERGEWITHFSTID='"+ rsds.getString("FSTID") +"' AND InstituteCode='"+mInstitute+"' And EXAMCODE='"+mE+"' and LTP='T' and  FACULTYTYPE='"+rs.getString("FACULTYTYPE")+"' and FACULTYID='"+rs.getString("FACULTYID")+"' And SUBJECTID='"+rs.getString("SUBJECTID")+"' AND SEMESTERTYPE='"+rs.getString("SEMESTERTYPE")+"' and nvl(a.deactive,'N')='N' Order By 1";
								rsds1=db.getRowset(qry);
								mstr=rsds.getString(1);
								while(rsds1.next())
								{
								    mstr=mstr+", "+rsds1.getString(1);
								}
								%>
								<tr>
								<td align=left><font size=2><font size=2 color=<%=mcolor%>>
						...
						{
						%>
							<td nowrap align=center><font size=2 color=<%=mcolor%>>-</font></td>
						<%
						}
						else
						{
						%>
							<td nowrap align=center><font color=<%=mcolor%>><%=mPLoad%>		</font>					
							<Table align=left>
							<%
							qry="select  SUBSECTIONCODE , FSTID,  NOOFCLASSINAWEEK||'@'||DURATIONOFCLASS From PR#HODLOADDISTRIBUTION A where InstituteCode='"+mInstitute+"' And EXAMCODE='"+mE+"' and LTP='P' and  FACULTYTYPE='"+rs.getString("FACULTYTYPE")+"' and FACULTYID='"+rs.getString("FACULTYID")+"' And SUBJECTID='"+rs.getString("SUBJECTID")+"' and nvl(a.deactive,'N')='N' and SEMESTERTYPE='"+rs.getString("SEMESTERTYPE")+"' ";
						if(rs.getString("SEMESTERTYPE").equals("REG"))
								qry=qry+" and MERGEWITHFSTID is null "; 
							qry=qry+" Order By 1"; 
						//MERGEWITHFSTID is null Order By 1";
						//	out.println(qry);
							rsds=db.getRowset(qry);
							while(rsds.next())
							{
								qry="select  SUBSECTIONCODE From PR#HODLOADDISTRIBUTION A where MERGEWITHFSTID='"+ rsds.getString("FSTID") +"' AND InstituteCode='"+mInstitute+"' And EXAMCODE='"+mE+"' and LTP='P' and  FACULTYTYPE='"+rs.getString("FACULTYTYPE")+"' and FACULTYID='"+rs.getString("FACULTYID")+"' And SUBJECTID='"+rs.getString("SUBJECTID")+"' and nvl(a.deactive,'N')='N' Order By 1";
								rsds1=db.getRowset(qry);
								mstr=rsds.getString(1);
								while(rsds1.next())
								{
