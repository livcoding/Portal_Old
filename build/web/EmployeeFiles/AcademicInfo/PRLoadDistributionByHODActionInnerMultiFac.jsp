...
//ssion.setAttribute("InterDept",null);
...
//out.println(session.getAttribute("subsecsess")+"ses");
    ...
    <%
                String ChkE = "", ChkI = "";
                if (request.getParameter("InterDept") == null) {
                    mInterDept = "E";
                } else {
                    mInterDept = request.getParameter("InterDept").toString().trim();
                }
                if (mInterDept.equals("I")) {
                    ChkI = "checked";
                    ChkE = "";
                } else {
                    ChkE = "checked";
    ...
    %>
    <table cellspacing=0 cellpadding=5 align=center border=5 rules=groups>
        <tr>
        <td align=left><input type=radio name="InterDept" value="I" <%=ChkI%>><b><font color=navy face=arial size=2>Include Inter Department Faculty</font><b>
            &nbsp; &nbsp;<input type=radio name="InterDept" value="E" <%=ChkE%>><b><font color=navy face=arial size=2>Exclude Inter Department Faculty</font><b>
            &nbsp; &nbsp;<input type=submit name="InterDeptSubmit" value="OK">
        </td>
...
<form name="frmfirst"  method="post" action="PRLoadDistributionByHODActionInnerMultiFac.jsp" ><%
...
//out.print("Inter Dept - "+mInterDept);

%>
<input type=hidden name="InterDept"		value=<%=mInterDept%>>
<input type=hidden name="mProg"		value=<%=mProg%>>
<input type=hidden name="Subjid"		value=<%=mSubjid%>>
<input type=hidden name="mAcad"		value=<%=mAcad%>>
<input type=hidden name="Sub"			value=<%=mSubj%>>
...
<tr bgcolor="#ff8c00">
    ...
    <tr>
    <tr>
    <td align=left><a href="#" onClick="window.location.reload( true );">Reload</a></td>
    <tr>
    ...
    %><input Type="hidden" value="<%=mImMergeSec%>" name="fstid"><%
                ...
                int i = 0;
                qry = "select Distinct a.EMPLOYEEID,a.FACULTYTYPE,a.NOFHRS from Temp#pr#loaddistribution a where  SUBJECTID='" + mSubjid + "' and LTP='" + mLTP + "' and SECTIONBRANCH='" + session.getAttribute("secbranch") + "' and SUBSECTIONCODE='" + session.getAttribute("subsecsess") + "' and academicyear='" + mAcad + "' and programcode='" + mProg + "'";
              //  out.print(qry);
                rs = db.getRowset(qry);
                if (rs.next()) {
                    qry = "select Distinct EMPLOYEEID,nvl(set1,'0')set1,nvl(set2,'0')set2,nvl(set3,'0')set3  from Temp#pr#loaddistribution where SUBJECTID='" + mSubjid + "' and LTP='" + mLTP + "' and SECTIONBRANCH='" + session.getAttribute("secbranch") + "' and SUBSECTIONCODE='" + session.getAttribute("subsecsess") + "' and academicyear='" + mAcad + "' and programcode='" + mProg + "'";
                    //out.print(qry);
                    rs = db.getRowset(qry);
                    while (rs.next()) {
                    ...
                    qry = "select employeeid,nvl(set1,'')set1,nvl(set2,'')set2,nvl(set3,'')set3 from MULTIFACULTYSUBJECTTAGGING  where fstid='" + mImMergeSec + "'";
                    //out.println(qry);
                    rs = db.getRowset(qry);
                    while (rs.next()) {
        ...
        <td>
            <select tabindex="0" style="WIDTH: 400px; HEIGHT: 200px" multiple name="MultiFaculti" >
                ...
                while (rs.next()) {
                    ...
                    //if(!mEmpIdTemp.equals(mFac)) // Main Faculty in MultiFacultySubjectTagging
                    //{
                    ...
                    if (request.getParameter("x") == null) {
                        for (int j = 0; j < list.size(); j++) {
                            //System.out.println(list.get(j)+" "+mFac);
...
%>
<form name="frmsecond"  method="post" action="PRLoadDistributionByHODActionInnerMultiFac.jsp" >
<input type=hidden name="InterDept"		value=<%=mInterDept%>>
...
<tr>				
    ...
    <tr>
        ...
        try {
            ...
            String qry12345 = "select EMPLOYEEID, nvl(NOFHRS,'')NOFHRS from MULTIFACULTYSUBJECTTAGGING  where fstid='" + mImMergeSec + "' and EMPLOYEEID='" + cc + "'";
...
//out.println(list.size());
                if (list.size() > 0) {
%>
<form name="frmthird"  method="post" action="PRLoadDistributionByHODActionInnerMultiFac.jsp" >
<input type=hidden name="InterDept"		value=<%=mInterDept%>>
<input type=hidden name="mProg"		value=<%=request.getParameter("mProg")%>>
<input type=hidden name="mAcad"		value=<%=request.getParameter("mAcad")%>>
<input type=hidden name="SUBJID"		value=<%=request.getParameter("Subjid")%>>
    ...
    while (rs.next()) {
        ...
        if (request.getParameter("x") == null) {
            for (int j = 0; j < list.size(); j++) {
                if (list.get(j).equals(mFac)) {
                    //viewList.add(mFacv);
                    String qry123 = "select nvl(NOFHRS,'')NOFHRS,nvl(set1,'')set1,nvl(set2,'')set2,nvl(set3,'')set3 from MULTIFACULTYSUBJECTTAGGING  where fstid='" + mImMergeSec + "' and FACULTYTYPE='" + mTyp + "' and EMPLOYEEID='" + mFac + "'";// and COMPANYCODE='"+mcmp+"' ";
                    //out.println(qry123);
                    ...
                    if (rssession.next()) {
                        ...
                        String qry1234 = "";
                        qry1234 = "select nvl(NOFHRS,'')NOFHRS,nvl(set1,'')set1,nvl(set2,'')set2,nvl(set3,'')set3 from Temp#pr#loaddistribution  where  FACULTYTYPE='" + mTyp + "' and EMPLOYEEID='" + mFac + "' and COMPANYCODE='" + mcmp + "' and LTP ='" + mLTP + "' and SECTIONBRANCH='" + session.getAttribute("secbranch") + "' and SUBSECTIONCODE='" + session.getAttribute("subsecsess") + "'";
                        //out.println(qry1234);
...
<td align="center"><input type="hidden" name="hr<%=h%>" size=2 value=<%=cc%>  maxlength='1'><SELECT NAME="set<%=h%>">
        <%
    String qry123 = "select  nvl(NOFHRS,'')NOFHRS,nvl(set1,'')set1,nvl(set2,'')set2,nvl(set3,'')set3,FACULTYSET,nvl(CLASSINAWEEK,'')CLASSINAWEEK  from MULTIFACULTYSUBJECTTAGGING  where fstid='" + mImMergeSec + "' and EMPLOYEEID='" + dd + "'";// and COMPANYCODE='"+mcmp+"' ";
    //out.println(qry123);
    rssession = db.getRowset(qry123);
    if (rssession.next()) {
        ...
        <INPUT TYPE="hidden" NAME="setText3" value=<%=rssession.getString("set3")%>><%
                    ...
                    String qry1234 = "";
                    qry1234 = "selectnvl(NOFHRS,'')NOFHRS,nvl(set1,'')set1,nvl(set2,'')set2,nvl(set3,'')set3,nvl(FACULTYSET,'')FACULTYSET,nvl(CLASSINAWEEK,'')CLASSINAWEEK  from Temp#pr#loaddistribution  where  EMPLOYEEID='" + dd + "' and LTP ='" + mLTP + "' and SECTIONBRANCH='" + session.getAttribute("secbranch") + "' and SUBSECTIONCODE='" + session.getAttribute("subsecsess") + "'";
                    //out.println(qry1234);
                    rssession1 = db.getRowset(qry1234);
                    if (rssession1.next()) {
...
//	if(request.getParameter("counter")==null)
                            ...
                            for (int j = 0; j < i; j++) {
                                ...
                                String id = ltp + session.getAttribute("secbranch") + session.getAttribute("subsecsess") + request.getParameter("mAcad") + request.getParameter("mProg");
