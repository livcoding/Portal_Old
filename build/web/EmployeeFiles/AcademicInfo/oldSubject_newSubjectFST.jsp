...
%>
<form name="frm2"  method="Post" action='MergeoldSubject_newSubjectFST.jsp'>
<br><br>
...
qry="select a.employeeid, PROGRAMCODE, SECTIONBRANCH, SUBSECTIONCODE, SEMESTERTYPE,employeename||' ('||Employeecode||')'empname,A.LTP,NVL(A.MERGEWITHFSTID,'N')MERGEWITHFSTID,a.FSTID from FAcultysubjecttagging a , Employeemaster b where subjectid='"+oldSubject+"' and examcode='"+mExamcode+"' and semestertype='RWJ' and a.institutecode='"+mInst+"' and a.companycode='"+mCompcode+"' and     a.employeeid = b.employeeid   and a.companycode=b.companycode   and b.COMPANYCODE='"+mCompcode+"'   and nvl(a.deactive,'N')='N'    and nvl(b.deactive,'N')='N' and nvl(a.LTP,'N')='"+mLT+"'  order by subsectioncode";

				/*qry=" select employeeid, PROGRAMCODE, SECTIONBRANCH, SUBSECTIONCODE, SEMESTERTYPE,employeename||' ('||Employeecode||')'empname,c.DEPARTMENT,c.DEPARTMENTCODE,A.LTP,NVL(A.MERGEWITHFSTID,'N')MERGEWITHFSTID,a.FSTID from   FAcultysubjecttagging a , Employeemaster b,departmentmaster c where subjectid='"+oldSubject+"' and examcode='"+mExamcode+"'   and a.DEPARTMENTRUNNIG=c.DEPARTMENTCODE and semestertype='RWJ' and a.institutecode='"+mInst+"' and a.companycode='"+mCompcode+"' and     a.employeeid = b.employeeid   and a.companycode=b.companycode   and b.COMPANYCODE='"+mCompcode+"'   and nvl(a.deactive,'N')='N'    and nvl(b.deactive,'N')='N' and nvl(a.LTP,'N')='"+mLT+"'  order by subsectioncode  ";*/
				//out.print(qry);
				rs= db.getRowset(qry);	
				...
				if(rs1.next())
					...
					<TD><A HREF="removemergsubjectsFST.jsp?fstid=<%=rs.getString("FSTID")%>&examcode=<%=mExamcode%>&oldSubject=<%=oldSubject%>&mLTP=<%=mLT%>"> Remove Subject</A></TD>
					<%
					}
					else
					{
