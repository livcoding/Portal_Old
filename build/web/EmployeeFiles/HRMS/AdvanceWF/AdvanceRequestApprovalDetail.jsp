...
int CTR=0, mMaxLevel=0, mAdvanceApprovalLevel=0,mTotOnWFLevel=0,QryNoOfEMI=0;
...
String mRightsID="171", mShowRemTo="", mShowRemarksTo="";
String mMemberID="";
String mDMemberID="";
String mMemberType="";
String mDMemberType="";
...
String mMemberName="";
String mInst="", mComp="";
String QryEName="", QryECode="", QryAdvDesc="", QryDeptCode="", QryDateOfAdv="", QryReqPurp="";
double QryAdvAmount=0, QryFinAdvAmount=0;
...
function FunSubmit1()
{
	document.frm.action="AdvanceRequestApprovalToNextLevel.jsp" ;
}
function FunSubmit2()
{
	document.frm.action="AdvanceRequestApprovalToChangedRoute.jsp" ;
}
function validate()
{
	alert('saddsadsa');
...
<!-------------Page Heading and Marquee Message----------------------->
			...
			try
			{
				String mPageHeader="Advance Work Flow For Approval/Cancellation", mMarqMsg="", CurrDate="";
				...
				qry="Select nvl(B.PAGEHEADER,'Advance Work Flow For Approval/Cancellation')PageHeader FROM WEBKIOSKRIGHTSMASTER B WHERE B.RIGHTSID='"+mRightsID+"' and B.RELATEDTO LIKE '%E%'";
				rs=db.getRowset(qry);
				if(rs.next())
				{
...
<!-------------Page Heading and Marquee Message----------------------->
			...
			qry="Select nvl(A.EMPLOYEENAME,' ') STAFF, A.EMPLOYEECODE EMPCODE, nvl(A.DEPARTMENTCODE,' ')DeptCode, B.ADVANCETYPE||' ('||C.ADVANCEDESC||')' ADVDESC, B.ADVANCEAMOUNT ADVAMOUNT, nvl(B.REQUESTREMARKS,' ') POA,";
			...
			qry=qry+" and A.EMPLOYEEID=B.MEMBERID and A.EMPLOYEEID='"+QryFaculty+"' and B.REQUESTID='"+QryRID+"' and B.ADVANCETYPE='"+QryWFType+"'";
			...
			if(rs.next())
			{
				...
				QryAdvDesc=rs.getString("ADVDESC");
				QryAdvAmount=rs.getDouble("ADVAMOUNT");
				QryReqPurp=rs.getString("POA");
				QryReqDate=rs.getString("RDATE");
				QryDateOfAdv=rs.getString("ADATE");
			}
			...
			//out.print(qry);
		    	rs=db.getRowset(qry);
			if(rs.next())
				QryFinAdvAmount=rs.getDouble("APPROVEDREQVALUE");
			else
				QryFinAdvAmount=QryAdvAmount;

			%>
			<input type=hidden Name='DOR' ID='DOR' value='<%=QryReqDate%>'>
			<input type=hidden Name='DOA' ID='DOA' value='<%=QryDateOfAdv%>'>
			<input type=hidden Name='REQVAL' ID='REQVAL' value='<%=QryAdvAmount%>'>
			...
			<tr><td nowrap><font color=black face=arial size=2><STRONG>&nbsp;Advance(<%=QryWFType%>) Amount</STRONG></font></td>
			<td nowrap><font color=black face=arial size=2><b>: &nbsp; &nbsp;</b></font></td>
			<td nowrap><font color=black face=arial size=2><STRONG>INR </STRONG><%=QryAdvAmount%></Font></td>
			<td nowrap><font color=black face=arial size=2><STRONG>&nbsp; &nbsp; Date of Advance : &nbsp;</STRONG></font></td>
			<td nowrap><font color=black face=arial size=2><%=QryDateOfAdv%></Font></td></tr>
			<tr><td nowrap><font color=black face=arial size=2><STRONG>&nbsp;Last Recommendation</STRONG></font></td>
			<td nowrap><font color=black face=arial size=2><b>: &nbsp; &nbsp;</b></font></td>
			<td nowrap><font color=black face=arial size=2><STRONG>INR </STRONG><%=QryFinAdvAmount%></Font></td>
			...
			<tr><td colspan=5 nowrap><font color=black face=arial size=2><STRONG>&nbsp;Purpose of Advance : &nbsp;</STRONG><%=QryReqPurp%></Font></td>
...
//---------------Start Of if(rsFrIndv.next())----
				...
				try
				{
					while(rs.next())
					{
						...
						if(rs1.next())
						{
							mAdvanceApprovalLevel=rs1.getInt("WFL");
						}
					...
					//------------------------------
						if(CTR>=mAdvanceApprovalLevel)
						{
							//------------------------------
							if(mAdvanceApprovalLevel<=0)
							{
								//out.print(qry);
								%>
							...
							else if(CTR==mAdvanceApprovalLevel)
							{
								if(mAprMemID.equals(""))
				...
				try
				{
					while(rs.next())
					{
						...
						if(mTotOnWFLevel>1)
						{
							...
							qry=qry+" AND EMPLOYEEID='"+QryFaculty+"' AND FACULTYTYPE='"+mFacType+"' AND WFLEVEL='"+CTR+"' AND "+QryAdvAmount+" between FROMVALUE and TOVALUE";
				...
				try
				{
					while(rs.next())
					{
						...
						if(mTotOnWFLevel>1)
						{
							...
							qry=qry+" CompanyCode='"+mComp+"' and InstituteCode='"+mInst+"' and WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"'";
							qry=qry+" AND DEPARTMENTCODE='"+QryDeptCode+"' AND WFLEVEL='"+CTR+"' AND "+QryAdvAmount+" between FROMVALUE and TOVALUE";
							qry=qry+" GROUP BY APPROVALBY, APPROVALAUTHORITY, DEPARTMENTCODE, WFSEQUENCE";
							rs1=db.getRowset(qry);
							//out.print(qry);
							if(rs1.next())
...
//---------------End Of if(rsFrDept.next())----
			...
			if(mAprMemID.equals(mChkMemID))
			{
				if(mTextToAprFrom.equals("I"))
				{
					...
					qry=qry+" REQUESTID='"+QryRID+"' AND CompanyCode='"+mComp+"' and InstituteCode='"+mInst+"'";
					qry=qry+" AND DEPARTMENTCODE='"+QryDeptCode+"' AND WFLEVEL='"+mAdvanceApprovalLevel+"'";
				}
				else if(mTextToAprFrom.equals("E"))
				{
					...
					qry=qry+" CompanyCode='"+mComp+"' and InstituteCode='"+mInst+"' and WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"'";
					qry=qry+" AND EMPLOYEEID='"+QryFaculty+"' AND FACULTYTYPE='"+mFacType+"' and nvl(DEACTIVE,'N')='N' AND WFLEVEL='"+mAdvanceApprovalLevel+"'";
				}
				...
				%>
				<tr><td colspan="5">
				&nbsp;<font color=black face=arial size=2><STRONG>Advance Recommendation As : &nbsp;</STRONG></font>
				&nbsp; &nbsp; &nbsp; <input type="radio" value="A" name="AprStat" checked><b><font face="arial" size="2" color=green> <%=mTextToAprDisp%> : <Input style="text-align=right" type="text" maxlength=8 style="position:relative;width:75px;font-size:10px" id="APRADVAMOUNT" name="APRADVAMOUNT" maxlength=12 value=<%=QryFinAdvAmount%>><FONT color=red>*</FONT></STRONG></font>
				<%
			...
			if(request.getParameter("x")!=null)
			{
				...
				try
				{
					if(request.getParameter("APRADVAMOUNT")==null)
						QryFinAdvAmount=0;
					else
						QryFinAdvAmount=Double.parseDouble(request.getParameter("APRADVAMOUNT").toString().trim());
				}
				catch(Exception e)
				{
					errFlag++;;
				...
				//out.print(mShowRemTo+"-"+mRemarks1+"-"+mFullCons+"-"+mApproveBy+"-"+mDepartment+"-"+mWFSequence+"-"+mWFLevel+" - "+QryFinAdvAmount);
				...
				if(!rs.next())
				{
				   if(errFlag==0 && QryFinAdvAmount>0 && QryFinAdvAmount<=QryAdvAmount)
				   {
					if(!mRemarks1.equals("") && mRemarks1.length()<=200)
					{
						...
						qry=qry+" VALUES('"+mComp+"', '"+mInst+"', '"+QryWFCode+"', '"+QryWFType+"', '"+mDepartment+"', "+mWFSequence+", "+mWFLevel+", '"+QryRID+"', '"+mFullCons+"', "+QryFinAdvAmount+", '"+mRemarks1+"', '"+mApproveBy+"', sysdate, '"+mTextToAprDisp+"', '"+mCanAllow+"', '"+mShowRemTo+"')";
						n=db.insertRow(qry);
						//out.print(qry);
						if(n>0 && mFullCons.equals("A"))
						{
						  //-----------Log Entry------------
							db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"ADVANCE WORK FLOW APPROVAL/CANCELLATION", "Staff: "+QryFaculty+" Advance Date: "+QryDateOfAdv+" Request ID: "+QryRID+" - Approved", "No MAC Address" , mIPAddress);
						  //----------------------------------
							if(mDiffPath.equals("Y"))
							{
								...
								{
									%>
									<LEFT><Br><b><font size=2 face='Arial' color='Green'>Advance Recommendation on Current Level is Successful...</font></b></br></LEFT>
									<%
									mLastLvlInWFDetail++;
								}
								else
								{
									...
									Advance Recommendation on Current Level is Successful... <a href='AdvanceRequestApprovalToChangedRoute.jsp?EID=<%=QryFaculty%>&amp;RID=<%=QryRID%>&amp;WFCode=<%=QryWFCode%>&amp;WFType=<%=QryWFType%>&amp;DOA=<%=QryDateOfAdv%>&amp;DOR=<%=QryReqDate%>&amp;DeptCode=<%=QryDeptCode%>&amp;REQVAL=<%=QryAdvAmount%>&amp;WFMaxLvlApr=N' style='cursor:hand'><font color=blue>Click </font></a>To Change Subsequent Route
									</font></LEFT>
									<%
								}
							}
							...
							{
								%><CENTER><%
								out.print("<br>&nbsp;&nbsp;&nbsp <b><font size=2 face='Arial' color='Green'>Advance Recommendation on Current Level is Successful...</font> <br>");
								%></CENTER><%
							}
							mSendInfoFlag++;
							...
							{
								qry="select nvl(A.NOOFINSTALLMENT,0)TOTEMI, nvl(B.INTERESTRATE,0)INTRATE, nvl(B.TYPEOFINTEREST,'S')TOI from PAY#EMPOTHERADVANCEREQUEST A, PAY#ADVANCETYPEMASTER B";
								qry=qry+" where A.REQUESTID='"+QryRID+"' AND A.COMPANYCODE='"+mComp+"' AND A.ADVANCETYPE='"+QryWFType+"' ";
								qry=qry+" AND A.MEMBERTYPE='E' and A.MEMBERID='"+QryFaculty+"' AND to_char(A.ADVANCEDATE,'DD-MM-YYYY')='"+QryDateOfAdv+"'";
								qry=qry+" AND A.COMPANYCODE=B.COMPANYCODE AND A.ADVANCETYPE=B.ADVANCETYPE AND A.MEMBERTYPE=B.APPLICABLETO";
								rs=db.getRowset(qry);
								if(rs.next())
								{
								...
								if(QryIntType.equals("C"))
								{
									...
									QryNOOFEMI=QryNOOFEMI*12;
									QryTotalVal=QryFinAdvAmount*QryIntAmt;
									QryEMIVal=(QryFinAdvAmount*(QryIntAmt/QryNOOFEMI));
								}
								else if(QryIntType.equals("S"))
								{
									QryIntAmt=(QryFinAdvAmount*QryIntRate*QryNOOFEMI)/100;
									QryNOOFEMI=QryNOOFEMI*12;
									QryTotalVal=QryFinAdvAmount+QryIntAmt;
									QryEMIVal=((QryFinAdvAmount+QryIntAmt)/QryNOOFEMI);
								}
								QryTotalIntAmt=gb.getRound((QryTotalVal-QryFinAdvAmount),2);
					...
					//-------------------------------
								qry="UPDATE PAY#EMPOTHERADVANCEREQUEST SET";
								...
								qry=qry+" APPROVEDADVANCEAMOUNT='"+QryFinAdvAmount+"'";
								qry=qry+" where REQUESTID='"+QryRID+"' AND MEMBERID='"+QryFaculty+"' AND ADVANCETYPE='"+QryWFType+"' AND ADVANCEDATE=to_date('"+QryDateOfAdv+"','dd-mm-yyyy')";
								...
								{
									%><LEFT><%
									out.print("&nbsp;&nbsp;&nbsp <br><b><font size=3 face='Arial' color='Green'>Advance Request has been approved successfully... </font> <br>");
									%></LEFT><%
									if(mLastLvlInWFDetail>0)
									{
										%>
										<LEFT><Br><b><font size=2 face='Arial' color='Green'>
										<a href='AdvanceRequestApprovalToChangedRoute.jsp?EID=<%=QryFaculty%>&amp;RID=<%=QryRID%>&amp;WFCode=<%=QryWFCode%>&amp;WFType=<%=QryWFType%>&amp;DOA=<%=QryDateOfAdv%>&amp;DOR=<%=QryReqDate%>&amp;DeptCode=<%=QryDeptCode%>&amp;REQVAL=<%=QryAdvAmount%>&amp;WFMaxLvlApr=Y' style='cursor:hand'><font color=blue>Click </font></a>To Change Other Request Route
										</font></LEFT>
										<%
									}
									%>
									...
									<tr><td colspan=7 align=center><font color="#a52a2a" style="FONT-SIZE: small; FONT-FAMILY: fantasy"><b><u>Final Approved Advance Detail</u></b></font></td></tr>
									<tr>
									<td align=center nowrap><B>Adv. Amount</B></td>
									...
									<tr>
									<td align=center nowrap><%=QryAdvAmount%></td>
									<td align=center nowrap><%=QryFinAdvAmount%></td>
					...
					//------------End----------------
						...
						{
							%><CENTER><%
							out.print("&nbsp;&nbsp;&nbsp <br><br><b><font size=2 face='Arial' color='Green'>Advance </Font><Font color='RED'>Cancellation</Font> <Font color='Green'>Successful...</font>");
							%></CENTER><br><%
						  //-----------Log Entry------------
							db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"ADVANCE WORK FLOW APPROVAL/CANCELLATION", "Staff: "+QryFaculty+" Advance Date: "+QryDateOfAdv+" Request ID: "+QryRID+" - Cancelled", "No MAC Address" , mIPAddress);
						  //----------------------------------
						...
						else
						{
							...
							out.print("&nbsp;&nbsp;&nbsp <b><font size=2 face='Arial' color='Red'>Error while Advance Approval/Cancellation!</font> <br>");
							%></CENTER><%
						}
						if(mSendInfoFlag>0)
						{
								...
								if(!mRemarks2.equals(""))
								{
								   if(mRemarks2.length()<=200)
								   {
									...
									if(!rs.next())
									{
										...
										qry=qry+" VALUES('"+mEmpID+"', '"+mEmpType+"', '"+QryRID+"', '"+mComp+"', '"+mInst+"', '"+QryWFCode+"', '"+QryWFType+"', '"+mDepartment+"', "+mWFSequence+", '"+mFullCons+"', "+QryAdvAmount+", '"+mRemarks2+"', '"+mApproveBy+"', sysdate, '', 'N')";
										n1=db.insertRow(qry);
										//out.print(qry);
										if(n1>0 && mFullCons.equals("A"))
										{
										  //-----------Log Entry------------
											db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"INFO FOR ADVANCE WORK FLOW APPROVAL/CANCELLATION", "Informed To: "+mEmpID+" Request ID: "+QryRID+" - Approved", "No MAC Address" , mIPAddress);
										  //----------------------------------
										}
										else if(n1>0 && mFullCons.equals("C"))
										{
										  //-----------Log Entry------------
											db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"INFO FOR ADVANCE WORK FLOW APPROVAL/CANCELLATION", "Informed To: "+mEmpID+" Request ID: "+QryRID+" - Cancelled", "No MAC Address" , mIPAddress);
										  //----------------------------------
										}
									}
								   }
					...
					else
					{
						%><CENTER><%
						out.print("<img src='../../../Images/Error1.gif'>");
						out.print("&nbsp;&nbsp;&nbsp <b><font size=2 face='Arial' color='Red'>Remarks for Advance Recommendation can't be left blank or more than 200 characters!</font> <br>");
						%></CENTER><%
					}
				   }
				   else
				   {
					...
					<b><font size=2 face='Arial' color='RED'>'Advance Amount' must be Numeric and Between 1.0 to <%=QryAdvAmount%>!</font></b>
					</Center><%
				   }
		   ...
		   else
		   {
			%><CENTER><%
			out.print("<img src='../../../Images/Error1.gif'>");
			out.print("&nbsp;&nbsp;&nbsp <br><b><font size=2 face='Arial' color='Red'>Advance Workflow of this request has been cancelled!</font></b><br>");
			%></CENTER><%
		   }
//-----------------------------
//---Enable Security Page Level  
