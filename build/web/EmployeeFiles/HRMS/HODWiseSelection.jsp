            ...
            $("select").change(function(){
                $("#value").html(this.options[this.selectedIndex].text + " (VALUE: " + this.value + ")");
				       ...
				       if(this.id=="Departmentcode"){
						  // alert($("#VaccCode").val());
                 $.get("getDesignation.jsp",{vacancy:$("select#Vacancy").val(),departmentcode:$("select#Departmentcode").val(),dated:getCurrentDateTime()}, successfunction1);
              }
            });
        });
...
%>
       ...
       <table borderColor="#D98242" align=center rules=none topmargin=0 cellspacing=0 cellpadding=4  border=1 >
	       ...
	       qry = "SELECT distinct COMPANYCODE, INSTITUTECODE, VACANCYCODE, VACANCYDESCRIPTION," +
            ...
            " VACANCYCODE in (       SELECT DISTINCT VACANCYCODE                                 FROM hr#vacancydepartmentselection                                WHERE  GUESTID='"+mChkMemID+"')";
 //  	out.print("ssdsd"+qry);
	rs = db.getRowset(qry);
...
qry = "select distinct departmentcode,department from DEPARTMENTMASTER where" +
" nvl(DEACTIVE,'N')='N' and   ";
if(mChkMemID.equals("JIIT-S00008"))
	{
qry=qry+"departmentcode in (select distinct DEPARTMENTCODE from " +
"HR#VACANCYDEPARTMENTSELECTION   where  GUESTID='"+mChkMemID+"' and VACANCYCODE IN (SELECT DISTINCT VACANCYCODE FROM   HR#VACANCYMASTER WHERE   NVL(BROADCAST,'N')='Y')) ";
	}
	else
	{
...
if(mChkMemID.equals("JIIT-S00008"))
	{
qry=qry+" and  departmentcode in (select distinct DEPARTMENTCODE from " +
"HR#VACANCYDEPARTMENTSELECTION   where  GUESTID='"+mChkMemID+"' and VACANCYCODE IN (SELECT DISTINCT VACANCYCODE FROM   HR#VACANCYMASTER WHERE   NVL(BROADCAST,'N')='Y')) ";
	}
	else
	{
...
int mTSelected=0 ;
		qry1="select count(c.applicantid)selected from   hr#applicantshortlistmaster c,  hr#applicationmaster a,hr#applicantaddress D " +
                "where a.vacancycode in (select vacancycode from HR#VACANCYMASTER WHERE a.vacancycode='"+mVacCode+"' " +
                "and institutecode='"+mInst+"' and NVL(BROADCAST,'N')='Y'  ) and a.DESIGNATIONCODE='"+mDesignation+"' and a.departmentcode ='"+mDeptCode+"'   " +
                "      and c.STATUS='S'   and  c.SHORTLISTSEQNO =3  and a.vacancycode ='"+mVacCode+"'    AND a.applicantid = c.applicantid			and a.institutecode='"+mInst+"' AND a.vacancycode = c.vacancycode AND a.institutecode = c.institutecode  AND a.applicantid = D.applicantid AND C.applicantid = D.applicantid ";
		//out.print(qry1);
...
//192.168.4.221   juet4guna
		...
		int mTReject=0 ;
			qry2="select count(c.applicantid)reject from   hr#applicantshortlistmaster c, " +
                    ...
                    " and a.DESIGNATIONCODE='"+mDesignation+"' and a.departmentcode ='"+mDeptCode+"'       " +
		...
		int mALLSum=0;
		qry2="select count(c.applicantid)AllResume from   hr#applicantshortlistmaster c,  hr#applicationmaster a ,hr#applicantaddress D" +
                " where a.vacancycode in (select vacancycode from HR#VACANCYMASTER WHERE institutecode='"+mInst+"'" +
                " and NVL(BROADCAST,'N')='Y'  ) and a.DESIGNATIONCODE='"+mDesignation+"' and a.departmentcode ='"+mDeptCode+"'     " +
		...
		if(rs1.next())
			...
			int mTNotProcess=0;
			qry2="select count(c.applicantid)mTNotProcess from   hr#applicantshortlistmaster c,  hr#applicationmaster " +
                    "a,hr#applicantaddress D where a.vacancycode in (select vacancycode from HR#VACANCYMASTER WHERE" +
                    " a.vacancycode='"+mVacCode+"' and institutecode='"+mInst+"' and NVL(BROADCAST,'N')='Y'  ) " +
                    "and a.departmentcode ='"+mDeptCode+"'  and a.DESIGNATIONCODE='"+mDesignation+"'       and nvl(c.STATUS,'N')='N'   and  c.SHORTLISTSEQNO =3  and a.vacancycode ='"+mVacCode+"'              AND a.applicantid = c.applicantid			and a.institutecode='"+mInst+"' AND a.vacancycode = c.vacancycode AND a.institutecode = c.institutecode  AND a.applicantid = D.applicantid AND C.applicantid = D.applicantid";
			//out.print(qry2);
		rs1=db.getRowset(qry2);
		if(rs1.next())
...
//		mTNotProcess=mALLSum-(mTReject+mTSelected);
		...
		<tr>
		<td class="labelcell" >
		<a href="HodLinks.jsp?ShowStatus=ALL&amp;Vacancy=<%=mVacCode%>&amp;DeptCode=<%=mDeptCode%>&amp;INSTITUTECODE=<%=mInst%>&amp;COMPANYCODE=<%=mComp%>&amp;DESCODE=<%=mDesignation%>"> <font  size=2><b>*	Resumes to be reviewed </td><td> <%=mALLSum%> </td>
		</tr>
		
		<tr bgColor="lightgreen">
		<td class="labelcell">
		<a href="HodLinks.jsp?ShowStatus=S&amp;Vacancy=<%=mVacCode%>&amp;DeptCode=<%=mDeptCode%>&amp;INSTITUTECODE=<%=mInst%>&amp;COMPANYCODE=<%=mComp%>&amp;DESCODE=<%=mDesignation%>"> 	<font color=green size=2><b>* Resumes Shortlisted  </a></td><td> <%=mTSelected%> </td>
		</tr>

		<tr bgColor="pink">
		<td class="labelcell">
		<a href="HodLinks.jsp?ShowStatus=R&amp;Vacancy=<%=mVacCode%>&amp;DeptCode=<%=mDeptCode%>&amp;INSTITUTECODE=<%=mInst%>&amp;COMPANYCODE=<%=mComp%>&amp;DESCODE=<%=mDesignation%>"><font color=red size=2><b>*	Resumes Not Shortlisted </a> </td><td> <%=mTReject%> 		</td>
		</tr>
	
		
		<tr bgColor="white">
		<td class="labelcell">
		<a href="HodLinks.jsp?ShowStatus=N&amp;Vacancy=<%=mVacCode%>&amp;DeptCode=<%=mDeptCode%>&amp;INSTITUTECODE=<%=mInst%>&amp;COMPANYCODE=<%=mComp%>&amp;DESCODE=<%=mDesignation%>">	<font color=blue size=2><b>* Resume Yet to be Processed  </a></td><td> <%=mTNotProcess%> 		</td>
		</tr>
		...
		<P ALIGN=CENTER class="labelcell"><BR><bR>
		<a href="HodLinks.jsp?ShowStatus=ALLSR&amp;Vacancy=<%=mVacCode%>&amp;DeptCode=<%=mDeptCode%>&amp;INSTITUTECODE=<%=mInst%>&amp;COMPANYCODE=<%=mComp%>&amp;DESCODE=<%=mDesignation%>">	<font color=blue size=2><b>* All Rejected Resume Processed in Inital/Final Level  ---  <%=mALLSRSum%> </a>
		</P>
		</form>
