...
<html>
    <head>
        <TITLE>#### <%=mHead%> [ HODSelection ]</TITLE>
...
<table  border=1  borderColor=#D98242 rules=group align=left  topmargin=0 cellspacing=0 cellpadding=0>
	...
	<table  align=center >
			...
			<tr><td align=center class="labelcell">Name : <%=mMemberName%>-(<%=mDMemberCode%>)</td></tr>
        </table>
	<%
...
if(request.getParameter("DeptCode")==null)
	...
	if(!mStatus.equals("N"))
		...
		//		out.print("------------------------------------"+qryupdate);
			...
			//if(mStatus.equals("R"))
			//{
				 qry="UPDATE HR#APPLICATIONMASTER SET SHORTLISTED='"+mStatus+"' ,SHORTLISTEDBY  = '"+ mChkMemID+"', SHORTLISTEDDATE = sysdate    ,FINAL='"+mFreeze+"', FINALIZEDBY='"+ mChkMemID+"', FINALIZEDDATE=sysdate  WHERE 		APPLICANTID = '"+ApplicationID+"' and VACANCYCODE    = '"+mVacCode+"' ";
			//	out.print("------------------------------------"+qry);
...
if(request.getParameter("Submit1")==null &&  request.getParameter("Freeze")==null )
		{
		response.sendRedirect("HodLinks.jsp?ShowStatus="+mShow+"&Vacancy="+mVacCode+"&DeptCode="+mDeptCode+" ");
		}
		else 
		{	
...
if(mShow.equals("ALLSR"))
	qry=qry+" and nvl(c.status,'R') ='R' ";
