		...
		if (RsChk.next() && RsChk.getString("SL").equals("Y"))
	   	{
			...
			qry=" SELECT MSGFROMUSERD, MSGFROMMEMBERTYPE, MSGSUBJECT, MSGTEXT, to_char(MSGDATETIME,'dd-Mon-yyyy') DT, nvl(SHOWNEXTTIME,'Y') , nvl(READLASTTIME,'N') FROM MESSAGESLIST Where  INSTITUTECODE='"+mInst+"' And MSGTOUSERD='"+ mDMemberID+"' And MSGTOMEMBERTYPE='E' And nvl(DEACTIVE,'N')='N'";				
			...
			if(RsChk.next())
		   	{
				mEmpName=RsChk.getString("MSGFROMUSERD");
				mEmpCode=RsChk.getString("MSGFROMMEMBERTYPE");
				%>
				<font size=3 color='Navy' Face='ARIAL'>&nbsp; &nbsp; Member Name :  <%=mMemberName%> &nbsp; (<%=mDMemberCode%>)</font><br><BR>
				<hr>
				&nbsp; &nbsp; Staff Name :  <%=mEmpName%> &nbsp; (<%=mEmpCode%>)<br>	
				&nbsp; &nbsp; Dated : <%=RsChk.getString("dt")%><br>
				&nbsp; &nbsp; <font size=3 color='Blue'>Request Status : <%=RsChk.getString("MSGSUBJECT")%></font>
				&nbsp; &nbsp; <font size=3 color='Blue'>Request Status : <%=RsChk.getString("MSGTEXT")%></font>
				<hr>
				<b></font>
				<%
			}
