...
String mComp="", mInst="", QryLTCType="",WFCode="LTC";
...
String mDateOfReq="", mDateOfAdv="", CurrDate="",mLTCtype="",mMemType="",mEname="";
		...
		if (RsChk.next() && RsChk.getString("SL").equals("Y"))
	   	{
			...
			<tr>
			<TD colspan=0 align=middle width=34%><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><b><u><FONT SIZE=4>Edit LTC Request Detail</FONT></u></b></td>
			</tr>
			</table>
			<br>
	<%
			qry="SELECT nvl(A.MEMBERID,' ')MEMBERID,nvl(A.MEMBERTYPE,' ')MEMBERTYPE,nvl(to_char(A.LTCDATE,'dd-mm-yyyy'),' ')LTCDATE,nvl(C.EMPLOYEENAME,' ')EMPLOYEENAME, nvl(B.LTCTYPE,' ')LTCTYPE,NVL(A.REQUESTID,' ')REQUESTID,decode(A.LTCAMOUNT,'',' ',LTCAMOUNT)LTCAMOUNT,decode(A.NOOFINSTALLMENT,'',' ',NOOFINSTALLMENT)NOOFINSTALLMENT, nvl(A.REQUESTREMARKS,' ')REQUESTREMARKS FROM PAY#EMPOTHERLTCREQUEST A,PAY#LTCTYPEMASTER B,V#STAFF C where A.MEMBERID='"+mEID+"' and  A.REQUESTID='"+mRID+"' AND A.COMPANYCODE='"+mComp+"' and A.INSTITUTECODE='"+mInst+"' AND A.LTCTYPE=B.LTCTYPE AND A.MEMBERID=C.EMPLOYEEID ";
			//out.print(qry);
			rs=db.getRowset(qry);
			if(rs.next())
			{
			mAdvDate=rs.getString("LTCDATE");
			mLTCtype=rs.getString("LTCTYPE");
		...
		%>
			...
			qry=qry+" from PAY#LTCTYPEMASTER where COMPANYCODE='"+mComp+"' AND APPLICABLETO='"+mMemType+"' AND LTCTYPE='"+mLTCtype+"'  and nvl(DEACTIVE,'N')='N' ORDER BY ADVDESC";
			//out.print(qry);
			rs=db.getRowset(qry);
			%>
			...
			<td nowrap colspan=3>
			<%
			qry="Select MIN(MINNOOFEMI)MINNOOFEMI, MAX(MAXNOOFEMI)MAXNOOFEMI from PAY#LTCTYPEMASTER WHERE COMPANYCODE='"+mComp+"' AND APPLICABLETO='"+mMemType+"' AND nvl(DEACTIVE,'N')='N' GROUP BY COMPANYCODE, APPLICABLETO";
			rs=db.getRowset(qry);
			//out.print(qry);
			if(rs.next())
			{
				...
				QRYNOOFEMI=(int)QryNOOFEMI;
					...
					if(QryAdvAmt>0)
					{
						...
						if(rs.next())
						{
							...
							<tr><td colspan=2 align=center><INPUT id=submit2 style="BACKGROUND-COLOR:TRANSPARENT; FONT-WEIGHT: bold; FONT-SIZE: smaller; FLOAT: none; WIDTH: 70px; HEIGHT: 23px; FONT-VARIANT: normal" type=submit value="Edit" name=submit2 title="Edit Request for LTC" onclick="return ActiveSubmit();"></td></tr>
							</table>
							</form>
							<%
						}
			...
			if(request.getParameter("y")!=null)
			{
				...
				if(!QryPOA.equals(""))
				{
					...
					if(rs.next())
					{
						...
						qry="UPDATE PAY#EMPOTHERLTCREQUEST SET  INSTITUTECODE='"+mInst+"',  LTCAMOUNT='"+QryAdvAmt+"', INSTALLMENT='Y', NOOFINSTALLMENT='"+QryNOOFEMI+"', INTERESTAMOUNT='"+QryTotalIntAmt+"', DEDUCTEDAMOUNT='"+QryEMIVal+"', REQUESTREMARKS='"+QryPOA+"', REQUESTBY='"+mEID+"', REQUESTDATE=sysdate, WORKFLOWCODE='LTC', WORKFLOWTYPE= '"+QryAdvType+"', STATUS='D' WHERE REQUESTID='"+mRID+"' and  MEMBERID='"+mEID+"' and COMPANYCODE='"+mComp+"' and LTCTYPE='"+QryAdvType+"' and MEMBERTYPE='"+mMemType+"' ";
						//out.print(qry);
						n=db.update(qry);
						if(n>0)
						{
							...
							// Log Entry
							//-----------------
							db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType," EDIT LTC LEAVE REQUEST ", "Staff:"+mEname+" EmployeeID:"+mEID,"NO MAC ADDRESS ", mIPAddress);
							//-----------------
								%>
								<table align=center><tr><td align=center>
								<b><font size=2 face='Arial' color='Green'>Your LTC Leave "<%=mAdvDesc%>" is Edited  Successfully...</font></b>
								</td></tr>
								<tr><td align=center>
								<a Title='Do You Want to Edit More Leaves' href='LTCRequestEdit.jsp'>
						...
						{
							%><table align=center><tr><td>
							<b><font size=2 face='Arial' color='RED'>Error while LTC request Edit!</font></b>
							</td></tr></table><%
						}
