...
String mMemberType="";
String mDMemberType="";
String mMemberCode="", mFaculty="", mFacCode="", mEvent="", mSubevent="";
String mDMemberCode="";
...
long mAvailable=0, mWeight=0, mTotPerm=100, mAbcd=0;
	...
	if (request.getParameter("ECODE").toString().trim()==null)
	{
		mECode="";
	}
	else
	...
	if (request.getParameter("SUBJ").toString().trim()==null)
	{
		mSubj="";
	}
	else
	...
	if (request.getParameter("EVENT").toString().trim()==null)
	{
		mSubEvent="";
	}
	else
	...
	if (request.getParameter("SUBEVENT").toString().trim()==null)
	{
		mSubevent="";
	}
	else
...
<INPUT ID="NewWValue" Name="NewWValue" style="WIDTH: 30px;" maxLength=3 value="<%=mWValue%>"></td>
...
<table bgcolor=#fce9c5 class="sort-table" id="table-1" bottommargin=0 rules=columns topmargin=0 cellspacing=0 cellpadding=0 border=1 align=center width="90%">
	...
	<tbody>
	<%
		qry=" Select distinct nvl(A.EXAMCODE,' ')ECode, (A.SUBJECT||'('||A.SUBJECTCODE||')')Subj, nvl(A.EVENTSUBEVENT,' ')SubEvent, ";
		qry=qry+" to_char(B.ENTRYDATE,'dd-mm-yyyy')AssignedDate, nvl(A.WEIGHTAGE,0) WEIGHTAGE, nvl(A.MAXMARKS,0) MAXMARKS";
		qry=qry+" from V#EXAMEVENTSUBJECTTAGGING A, EXAMEVENTSUBJECTTAGGING B WHERE A.EVENTSUBEVENT=B.EVENTSUBEVENT and A.MAXMARKS=b.MAXMARKS";
		qry=qry+" and A.fstid=B.fstid and a.WEIGHTAGE=b.WEIGHTAGE and A.INSTITUTECODE='"+mInst+"' AND A.EXAMCODE='"+mECode+"' ";
		qry=qry+" AND A.SUBJECTCODE='"+mSubjCode+"' and A.EMPLOYEEID='"+mChkMemID+"'";
		qry=qry+" Group By A.EXAMCODE, A.SUBJECT, A.SUBJECTCODE, A.EVENTSUBEVENT, B.ENTRYDATE, A.WEIGHTAGE, A.MAXMARKS";
		qry=qry+" Order By ECode, SUBJ, SubEvent";
		//out.print(qry);
		...
		while(rs1.next())
		{
			...
			qry1=qry1+" AND A.SUBJECTCODE='"+mSubjCode+"' and A.EMPLOYEEID='"+mChkMemID+"'";
			//qry1=qry1+" Group By A.EXAMCODE, A.SUBJECTCODE";
...
<%
		if(request.getParameter("x")!=null)
		{
			...
			//---------------
				...
				qry2=qry2+" Where EVENTSUBEVENT='"+mEvent+"' and WEIGHTAGE='"+mCurrWValue+"' and ";
				qry2=qry2+" MAXMARKS='"+mCurrMarks+"' ";
				...
				qry2=qry2+" and EXAMCODE='"+mECode+"' and SUBJECTCODE='"+mSubjCode+"' and EMPLOYEEID='"+mChkMemID+"')";
				//out.print(qry2);

				int n=db.update(qry2);
				if(n>0)
				{
				 //----------- Log Entry
		  		 //-----------------
				  db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"EDIT WEIGHTAGE / MARKS", "Exam Code: "+mECode+" Event/SubEvent : "+mEvent+" Subject: "+mSubjCode+" New Weightage: "+mNewWValue+" New FullMarks : "+mNewMarks, "No MAC Address" , mIPAddress);
				 //-----------------
