...
//out.print("ID - "+enc.encode("admin"));
//out.print("<BR>PWD - "+enc.encode("qaz@iitr123"));
...
String mSysdate="",mDateFrom="",mDateTo="",mFYear="",Panno="",mFREEZE="",mTimeOver="",mDaysLeft="";	
		...
		qry="SELECT to_char(FROMPERIOD,'dd-mm-yyyy')FROMPERIOD, to_char(TOPERIOD,'dd-mm-yyyy')TOPERIOD, NVL(STATUS,'N')STATUS ,NVL(ASSESSMENTYEAR,' ')ASSESSMENTYEAR , NVL(FINYEAR,' ')FINYEAR	,nvl(TDSCODE,' ')TDSCODE  ,(TOPERIOD-trunc(sysdate))DAYCOUNT	FROM TDS#PARAMETER where COMPANYCODE='"+mComp+"' ";
rs=db.getRowset(qry);
if(rs.next())
		{
...
qry1="select 'Y' from tds#parameter where sysdate between fromperiod and toperiod";
...
%><br>
	...
	<h1>
		...
		<font face=arial size=4 color=blue >Tax Declaration Instructions
		</font>
	</h1>
	</center>
<hr>
	<p align=center><font face=arial size=2 >
		<UL>
			<LI>Declaration of Financial Year <b><u><%=mFYear%></u></b> Assessment Year <b><u><%=mAssYear%></u></b>
	...
	<hr>
<p align=center><font face=arial color=red size=2 >
		<UL>
		<%
		if(mFREEZE.equals("X"))
		{
