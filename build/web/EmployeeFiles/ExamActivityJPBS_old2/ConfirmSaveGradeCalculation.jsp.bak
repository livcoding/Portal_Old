...
if(hht==1) {
		...
		qry2+="  NVL (SUM (DECODE (xyz.eventsubevent, '"+rstu.getString("SUBEVENTCODE")+"', total, 0)),  '0'  )  "+rstu.getString("AA")+"  ";
	 //out.print(qry2);
}else{
...
Qrycut="select ATTENDANCEPERCENTAGE,MASSCUTS,TATOTAL, NVL ( ( (CASE WHEN (TATOTAL - Masscuts) < 0 THEN TATOTAL  ELSE  Masscuts  END)),0       )NETMARKSCUT from STUDENTGRADEMASSCUTS where fstid  IN  ("+mCheckFstid+" ) and studentid='"+mStudidInit+"'";
...
%>
		<td align=center >- <%=mMassCutMarks%><FONT SIZE="1" COLOR="red">*</FONT></td>				
