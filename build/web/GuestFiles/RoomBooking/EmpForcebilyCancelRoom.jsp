...
<HTML>
<head>
<TITLE>#### <%=mHead%> [ Room Cancellation By Admin] </TITLE>
...
<table width="100%" ALIGN=CENTER bottommargin=0 topmargin=0>
 <tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: small; FONT-FAMILY: fantasy"><u><b><FONT SIZE=4>Forcibly Room Cancellation</FONT></b></u></font></td></tr>
</table>
<!*********--Institute--************>
...
<%
		...
		qry=qry+" nvl(C.Employeename,' ')EmpName, nvl(A.CANCELLATIONSTATUS,'N')CanStat, nvl(B.ROOMDESC,' ')ROOMDESC";
		qry=qry+" From ROOMBOOKINGINFO A, ROOMMASTER B, EMPLOYEEMASTER C Where trunc(A.BOOKINGUPTODATE)>=trunc(sysdate) ";
		qry=qry+" AND NVL(A.DEACTIVE,'N')='N' AND A.ROOMCODE=B.ROOMCODE AND NVL(B.DEACTIVE,'N')='N' AND C.EMPLOYEEID=A.BOOKEDBY ORDER BY BDF, BTF, BDT, BTT";
		rs=db.getRowset(qry);
//		out.print(qry);
		SNo=0;
		while(rs.next())
		...
		%>
			...
			<input type=hidden Name=<%=mName4%> ID=<%=mName4%> value='<%=rs.getString("RD")%>'>
			...
			<td nowrap><FONT COLOR=GREEN><%=rs.getString("EmpName")%></FONT></td>
			</tr>
		<%
			}
			else
...
var st1 = new SortableTable(document.getElementById("table-1"),["Number","CaseInsensitiveString","CaseInsensitiveString","CaseInsensitiveString","CaseInsensitiveString","CaseInsensitiveString","CaseInsensitiveString"]);
</script>
<table width=96%>
<tr><td align=right>
