...
%>
<!--*********Room Type**********-->
<tr><td><FONT color=black face=Arial size=2><b>Room Type</b></FONT>
<%
try
{
	...
	{
		%>
		<Select Name=RoomType tabindex="0" id="RoomType" style="WIDTH: 150px">	
		<%   
		while(rs.next())
		{
			mRoom=rs.getString("ROOMTYPE");
			%>
				<OPTION Value =<%=mRoom%>><%=rs.getString("ROOMTYPEDESC")%></option>
			<%
		}
		%>
		</select>
	...
	else
	{
		...
		{
			mRoom=rs.getString("ROOMTYPE");
			if(mRoom.equals(request.getParameter("RoomType").toString().trim()))
 			{
			%>
				<OPTION selected Value =<%=mRoom%>><%=rs.getString("ROOMTYPEDESC")%></option>
			<%			
		     	}
		     	else
		      {
	...
	if(request.getParameter("x")!=null)
	{
		...
		{	
			...
			qry1=qry1+" A.ROOMTYPE=B.ROOMTYPE AND A.institutecode='"+mInst+"' and nvl(Webkiosk.IsRoomAvailable('"+mInst+"',A.RoomCode,To_date('"+mDate1+"','dd-MM-yyyy'),to_date('"+mDate2+"','dd-MM-yyyy'), to_date('"+mFromTime+"','dd-mm-yyyy HH:MI PM'),to_Date('"+mUptoTime+"','dd-mm-yyyy hh:MI: PM')),'N')='Y'";
 			qry1=qry1+" And A.ROOMFOR='"+mRoomFor+"' and A.ROOMTYPE='"+mRoomType+"' and nvl(A.DEACTIVE,'N')='N' ";
