...
String mFacultyName="",mFacultyCode="", mFaculty="",QryFaculty="";
...
<table align=center width="100%" bottommargin=0 topmargin=0>
 <tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><b><u><FONT SIZE=4>Leave/Time Prefernce on Invigilation Duty [By Admin]</FONT></u></b></font></td></tr>
</table>
...
qry=qry+" order by EXAMPERIODFROM DESC ";
	...
	<%
	qry="select distinct nvl(EMPLOYEEID,' ')||'***'||'E' Faculty, nvl(EMPLOYEENAME,' ')FacultyName, nvl(EMPLOYEECODE,' ')FacultyCode from EMPLOYEEMASTER";
	qry=qry+" where EMPLOYEETYPE in '"+mEmpType+"' and EMPLOYEEID in (select Employeeid from EMPLOYEEMASTER where nvl(deactive,'N')='N' and CompanyCode='"+mComp+"')";
	qry=qry+" union select distinct nvl(A.STAFFID,' ')||'***'||nvl(A.STAFFTYPE,' ')Faculty, nvl(B.STUDENTNAME,' ')FacultyName, nvl(B.ENROLLMENTNO,' ')FacultyCode";
	qry=qry+" from OTHERSTAFFLOADTAGGING A, STUDENTMASTER B where A.STAFFID=B.STUDENTID";
	qry=qry+" union select distinct nvl(A.STAFFID,' ')||'***'||nvl(A.STAFFTYPE,' ')Faculty, nvl(B.EMPLOYEENAME,' ')FacultyName, nvl(B.EmployeeCode,' ')FacultyCode";
	...
	%>
<!******************Date**************-->
	<b>Leave on Duty From &nbsp;<font size=1 face=arial color=teal>(DD-MM-YYYY)</font>&nbsp; 
	...
	<tr><td><input id="DutyHoliday" name="DutyHoliday" Type=Radio value="Y"><STRONG><FONT color=black face=Arial size=2>Duty Holiday (Full day duty deny)</strong></font>
	</td></tr></table>
	</td>
	</tr>
	<%
	...
	if(!mFaculty.equals("") && !mFaculty.equals(" "))
	{
		int len=0, pos1=0;
		len=mFaculty.length();
		pos1=mFaculty.indexOf("***");
		QryFaculty=mFaculty.substring(0,pos1);
		QryFacultyType=mFaculty.substring(pos1+3,len);
		EQryFaculty=enc.encode(QryFaculty);
	}
	%>
	<tr><td colspan=2><b>Reason/Remarks (Maximum 250 Characters) </b><br>	
	<TextArea Name='TEXT' Id='TEXT' maxlength=250 cols=68 rows=3><%=mtext%></TextArea><FONT color=red>*</FONT>
		...
		if(mDiffInDays>=0)
		  {
			...
			{						  
				...
				if(!mtext.equals(""))
				{
					...
					qry=qry+" and EXAMCODE='"+qryexam+"'";
   					qry=qry+" And INVIGILATORID=Decode('"+QryFaculty+ "','ALL',INVIGILATORID,'"+QryFaculty+"')";
					qry=qry+" and (trunc(to_date('"+mDate1+"','dd-mm-yyyy')) between trunc(PREFROMDATE) and trunc(PRETODATE) or trunc(to_date('"+mDate2+"','dd-mm-yyyy')) between trunc(PREFROMDATE) and trunc(PRETODATE))";
					//qry=qry+" and (to_date('"+mFromTime+"','dd-mm-yyyy hh:mi PM') between PREFFROMTIME and PRETOTIME or to_date('"+mUptoTime+"','dd-mm-yyyy hh:mi PM') between PREFFROMTIME and PRETOTIME)";
					//out.print(qry);
					rs= db.getRowset(qry);		  
					if (!rs.next())
					{
		   ...
		   // Log Entry
  		   //-----------------
			db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"INVIGILATION DUTY HOLIDAY BY GUEST LOGIN", "Date from : "+mDate1+" to "+ mDate2+" INVGTYP : "+ mStaffType+" INVGID : "+ QryFaculty, "No MAC Address" , mIPAddress);
		   //-----------------
       					}
		      	 		catch(Exception e){}
					}
	...
	qry=" SELECT nvl(A.EMPLOYEENAME,' ')INVIGILATORID, to_Char(A.EntryDate,'dd-mm-yyyy') RFDT, to_char(A.PREFROMDATE,'dd-mm-yyyy') ||' to '||to_char(A.PRETODATE,'dd-mm-yyyy') || '( '||decode(A.DUTYHOLIDAY,'Y','Full Day Leave/Holiday',to_char(A.PREFFROMTIME,'hh:mi PM')||' to '|| to_char(A.PRETOTIME,'hh:mi PM'))||' )' Tim,";
	qry=qry+" nvl(A.REMARKS,' ') REMARKS, nvl(A.APPROVED,'N')APPROVED FROM V#INVIGILATIONTIMEPREF A"; 
	qry=qry+" Where A.INSTITUTECODE='"+mInst+"' and A.COMPANYCODE='"+mComp+"' and A.EXAMCODE='"+qryexam+"'"; 
	...
	<table class="sort-table" id="table-1"  width='100%' border=1 cellpadding=0 cellspacing=0 align=center>
		<thead>
			<tr bgcolor="#ff8c00">
				<td><font color=white>SNo</font></td>
				...
				<td><font color=white>Approved?</font></td>					
				<td><font color=white>Invigilator</font></td>
			</tr>
		</thead>
		<tbody>
		<%
