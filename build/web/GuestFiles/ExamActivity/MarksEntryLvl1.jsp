...
<html>
<head>
<TITLE>#### <%=mHead%> [ First Level Marks Entry] </TITLE>
...
function showAlert()
{
 ...
 {
	alert('Once You will check and save, You cannot enter marks of the rest students further');
 }
 else
 {
	alert('You cannot start IInd Level Marks entry until you check it and save.');
 }
}
...
else
{
	...
	<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy">Event/Sub Event wise Students Marks Entry (Level-I)</TD>
	</font></td></tr>
	</TABLE>
...
<%  
	try
	{
		...
		{
		%>
			<select name=Exam tabindex="2" id="Exam" style="WIDTH: 150px">	
		<%   
			while(rs.next())
			{
...
<%    try
	...
	qry=qry+" and nvl(PROCEEDSECOND,'N')='N' GROUP BY EVENTSUBEVENT "; 
	//out.print(qry);
	...
	{
		%>
		<select name=Event tabindex="3" id="Event" style="WIDTH: 156px">	
		<%   
		while(rse.next())
		{
...
<%
	...
	qry=qry+" where facultytype=decode('"+mDMemberType+"','E','I','E') and employeeid='"+mDMemberID+"' and nvl(deactive,'N')='N' AND (LTP='L' OR PROJECTSUBJECT='Y') ) ";  
	qry=qry+" and nvl(locked,'N')='N' and nvl(PUBLISHED,'N')='N' ";
	qry=qry+" and nvl(PROCEEDSECOND,'N')='N' GROUP BY subject||' ( '||subjectcode||' )',subjectID"; 
	//out.print(qry);
...
//******************************************************************
	...
	%>
		...
		<INPUT Type="submit"  tabindex="<%=tabctrtxt%>" Value="Save First Level Marks Entry"></td></tr>
