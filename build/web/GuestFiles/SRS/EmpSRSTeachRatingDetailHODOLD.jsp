...
<HTML>
<head>
<TITLE>#### <%=mHead%> [ Student Reaction Survey detailed report (HOD wise) ] </TITLE>
...
String mChkMemID=enc.decode(session.getAttribute("MemberID").toString().trim());
  ...
  //-----------------------------
	qry="Select WEBKIOSK.ShowLink('54','"+ mChkMemID+"','"+mChkMType+"','"+mRole+"','"+ mIPAddress +"') SL from dual";
...
<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy">SRS Teacher Rating Detail Report-HOD/Department wise
</font></td></tr>
</TABLE>
...
//out.print(qry1);
	...
	qry1="Select Distinct B.SubjectCode, B.Subject||' ('||B.SubjectCode||') ' Subj ";
	...
	qry1=qry1 +" and nvl(B.FINALIZED,'N')='Y' and NVL(B.APPROVED,'N')='Y' and nvl(B.deactive,'N')='N'  order by Subj"; 
	//qry1=qry1 +" and B.subjectcode in (select distinct subjectcode from v#srseventsuggestion where srseventcode=B.srseventcode  and nvl(deactive,'N')='N') order by Subj";
//	out.print(qry1);
...
try
{   
qry="select Distinct nvl(EMPLOYEEID,' ') EMPLOYEEID,nvl(EMPLOYEENAME,' ') EMPLOYEENAME from v#SRSEVENTS A";
qry=qry +" Where Employeeid in (select Employeeid ";
qry=qry +" from EMPLOYEEMASTER where DEPARTMENTCODE in (select DEPARTMENTCODE from ";
qry=qry +" hodlist where employeeid='"+mDMemberID+"' and nvl(deactive,'N')='N')) and nvl(FINALIZED,'N')='Y' and nvl(deactive,'N')='N'";
