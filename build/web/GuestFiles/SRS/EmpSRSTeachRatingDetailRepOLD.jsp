...
String qry="";
String qry1="";
String mInst="",mSubj="",msubj="",mFacultyID="",mFaculty="",mfaculty="",mSubhead="";
...
<%
 try
 {
	...
	qry="Select WEBKIOSK.ShowLink('11','"+ mChkMemID+"','"+mChkMType+"','"+mRole+"','"+ mIPAddress +"') SL from dual";
...
<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy">SRS Teacher Rating Detail Report
</font></td></tr>
</TABLE>
...
<%
qry="select distinct nvl(SRSEVENTCODE,' ')SRSEVENTCODE from v#SRSEVENTS WHERE  NVL(FINALIZED,'N')='Y' and nvl(deactive,'N')='N' "; 
...
{ 	qry1="Select Distinct B.SubjectCode, B.Subject||' ('||B.SubjectCode||') ' Subj from v#SRSEVENTS B where nvl(B.FINALIZED,'N')='Y' and NVL(B.APPROVED,'N')='Y' AND nvl(B.deactive,'N')='N' ";
	qry1=qry1+" and b.subjectcode in (select distinct subjectcode from v#srseventsuggestion where srseventcode=B.srseventcode and nvl(deactive,'N')='N') order by Subj";
//out.print(qry1);
...
try
{ 
	//qry="select Distinct nvl(A.EMPLOYEEID,' ') EMPLOYEEID,nvl(A.EMPLOYEENAME,' ') EMPLOYEENAME from V#SRSEVENTS A Where nvl(A.FINALIZED,'N')='Y' and nvl(APPROVED,'N')='Y' and nvl(Deactive,'N')='N'";
	
	qry="select Distinct nvl(A.EMPLOYEEID,' ') EMPLOYEEID,nvl(EMPLOYEENAME,' ') EMPLOYEENAME from v#SRSEvents A";
	qry=qry +" Where nvl(A.Approved,'N')='Y'  and nvl(A.Finalized,'N')='Y' and nvl(A.Deactive,'N')='N' and A.employeeid in (select distinct employeeid from v#srseventsuggestion where srseventcode=A.srseventcode  and nvl(deactive,'N')='N') order by EMPLOYEENAME ";
	//out.print(qry);	
	...
	if (request.getParameter("Faculty")==null)
	{
		...
		while(rsf.next())
		{
			mFacultyID=rsf.getString("EMPLOYEEID");
			mFaculty=rsf.getString("EMPLOYEENAME");
			if(mfaculty.equals(""))
 			mfaculty=mFacultyID;
			%>
...
//		mT=request.getParameter("mCount").toString().trim();
	...
	qry=qry+ " where A.SRSEVENTCODE='"+mSrsevent+"' AND A.SRSEVENTCODE=B.SRSEVENTCODE AND A.SRSEVENTCODE=C.SRSEVENTCODE  and A.SRSCODE=B.SRSCODE AND A.SRSSUBCODE=C.SRSSUBCODE AND A.SRSCODE=C.SRSCODE AND A.institutecode='"+mInst+"' and A.Employeeid='"+mFac+"'  ";
...
//	out.print(qry);
	...
	String mSubheading="";
//----------------------------------------
//loop at question level and subtype,type
//---------------------------------------
	while(rs.next())
	{
		...
		if(x.equals(""))
			...
			{
//--------------------------
//for printing of srstype
//---------------------------

				avg=(rating1/ev1)*100;					
				%>  
				<tr><td colspan=0><b>Overall Average</b></td><td><%=gb.getRound(avg,1)%>%</td></tr>
				<%
				if(mExTot>0)
				{
				...
				%>
				<tr><td colspan=0><b>Overall Average Excluding(<%=mSubheading%>)</b></td><td><%=gb.getRound(exavg,1)%>%</td></tr>
				<%
				}
				%>
			   <tr><td colspan=0><b><%=rs.getString("SRSSUBDESCRIPTION")%></b></td></tr>
			...
			{
//--------------------------
//for printing of sub type and excluding
//---------------------------
			...
			msno++;
			qry="select  A.srsquestion,B.ratingcode,B.evaluationupto from srsquestionmaster A, srsratingmaster B where ";
			...
			{	
				...
				qry="select Max(RatingValue) MaxValue,MIN(RatingValue) MinValue, count(*) cnt from V#SRSEVENTDETAIL where ";
				...
				qry=qry+ " SRSQUESTIONCODE='"+rs.getString("SRSQUESTIONCODE")+"' ";
				...
				<%
//-----------------------------------------------
// for calculating rating at individual question 
//------------------------------------------------
				rating=a/cnt1;	
...
//---------------------------
	...
	qry=qry+ " where SRSEVENTCODE='"+mSrsevent+"'AND institutecode='"+mInst+"' and Employeeid='"+mFac+"'";
