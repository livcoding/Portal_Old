...
try
{
	...
	qry=qry+"  InstituteCode='"+mInst+"' And  nvl(Deactive,'N')='N' and Nvl(LOCKEXAM,'N')='N' and Nvl(FINALIZED,'N')='N' order by EXAMPERIODFROM DESC";
	//out.print(qry);
	rs=db.getRowset(qry);
...
{     
	...
	qry=qry+" Group by nvl(A.subjectid,' ') ,nvl(B.subjectcode,' ') ,A.examcode,nvl(B.subject,' ')||' ('||B.subjectcode||') ' order by subject";
	rs=db.getRowset(qry);
	//out.print(qry);
	if (request.getParameter("x")==null) 
	{
...
<%	
	qry=" Select nvl(A.subjectid,' ') subjectid,nvl(B.subjectcode,' ') subjectcode,B.subject sbj, nvl(B.subject,' ')||' ('||B.subjectcode||') ' subject ";
	qry=qry+" from  facultysubjecttagging A,SUBJECTMASTER B where  ";
	qry=qry+"  A.facultytype='I' and ";
	qry=qry+" A.examcode not in (select examcode from exammaster where InstituteCode='"+mInst+"' And (nvl(LOCKEXAM,'N')='Y' ";
	qry=qry+" or nvl(FINALIZED,'N')='Y' or NVL(DEACTIVE,'N')='Y' )) AND A.SUBJECTID=B.SUBJECTID"; 
	qry=qry+" and A.EXAMCODE='"+qryexam+"' Group By nvl(A.subjectid,' ') ,nvl(B.subjectcode,' '),B.subject, nvl(B.subject,' ')||' ('||B.subjectcode||') ' order by 2";
	//out.print(qry);
	rs=db.getRowset(qry);
	%>
...
{ 
	...
	qry1=qry1+"  facultytype='I' and ";
	qry1=qry1+" examcode not in (select examcode from exammaster where InstituteCode='"+mInst+"' And (nvl(LOCKEXAM,'N')='Y' ";
	qry1=qry1+" or nvl(FINALIZED,'N')='Y' or InstituteCode='"+mInst+"' or NVL(DEACTIVE,'N')='Y') ) ";
	qry1=qry1+" and examcode='"+qryexam+"' and subjectid=decode('"+qrysubj+"','ALL',subjectid,'"+qrysubj+"') Group By nvl(SECTIONBRANCH,' ')  order by Section";
...
{ 
	...
	qry1=qry1+" Group By nvl(SECTIONBRANCH,' ') ,nvl(subjectid,' ') ,nvl(EXAMCODE,' ') order by Section";
	rs1=db.getRowset(qry1);
...
%>
	...
	&nbsp;
	<marquee  scrolldelay=225 width=55% behavior=alternate><A href="../../Images/PageSetupXL.bmp" Title="Instruction before print a Students List" Target=_New><font size=3 color=Blue><b>Recommended Page Setup: Paper Size - A4 and Top/Bottom Margin - .25</b></font></a></marquee>
	<td></tr>
	</table>
	</form>
	<%
...
<%	
	...
	qry=qry+" NVL(A.SECTIONBRANCH,' ')SECTIONBRANCH, nvl(A.BASKET,'A')BASKET, NVL(A.SUBJECTCODE,' ') SUBJECTCODE, NVL(A.SUBJECTID,' ') SUBJECTID, count(A.studentid) STUDENTID, nvl(REGCONFIRMATION,' ')REGCONFIRMATION";
	...
	qry=qry+" Group by A.ACADEMICYEAR, A.PROGRAMCODE, A.TAGGINGFOR, A.SECTIONBRANCH, A.BASKET, A.SUBJECTCODE,A.SUBJECTID, B.REGCONFIRMATION";
	qry=qry+" order by A.ACADEMICYEAR, A.PROGRAMCODE, A.TAGGINGFOR, A.SECTIONBRANCH, A.BASKET, A.SUBJECTCODE,A.SUBJECTID, B.REGCONFIRMATION";
