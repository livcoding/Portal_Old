...
<HTML>
<head>
<TITLE>#### <%=mHead%> [ Elective/Free Elective Subject Running List] </TITLE>
	...
	if(!mMemberID.equals("") && !mMemberCode.equals("") && !mMemberName.equals(""))
	{
		...
		if (RsChk.next() && RsChk.getString("SL").equals("Y"))
		  ...
		  //----------------------
			...
			qry=" SELECT distinct a.EXAMCODE from PREVENTMASTER a WHERE a.INSTITUTECODE='"+ mInst+"' and NVL(a.FREEELECTIVERUNFINALIZED,'N')='Y' and nvl(a.PRCOMPLETED,'N')='N' and nvl(a.PRBROADCAST,'N')='Y'";
			...
			{		
				...
				<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><b>Elective/Free Elective Subject(s) Running List</B></TD>
				</font></td></tr>
				</TABLE>
				...
				try
				{
					qry=" SELECT distinct a.EXAMCODE exam from PREVENTMASTER a WHERE a.INSTITUTECODE='"+ mInstitute +"' and NVL(a.FREEELECTIVERUNFINALIZED,'N')='Y' and  nvl(a.PRCOMPLETED,'N')='N' and nvl(a.PRBROADCAST,'N')='Y'";
					qry=qry+" AND NVL(PRREQUIREDFOR,'S')<>'S' AND NVL(a.DEACTIVE,'N')='N' And (a.INSTITUTECODE, a.PREVENTCODE) In ";
					qry=qry+" (select b.INSTITUTECODE, b.PREVENTCODE from PREVENTS b where nvl(B.DEACTIVE,'N')='N'";
					qry=qry+" And NVL(B.MEMBERTYPE,'N')<>'S' GROUP BY b.INSTITUTECODE, b.PREVENTCODE)";
					rs=db.getRowset(qry);
				...
				else
				{
					...
					if(request.getParameter("SubjType").toString().trim().equals("F"))
					{
						%>
						<option value="F" Selected>Free Elective</option>
						<%
					}
					else
					{
...
//--------------------------------------------HOD-------------------------------------
					if(mType.equals("H"))
					{
					   if (mST.equals("E"))
					   {
						qry="select nvl(A.Subject,A.SubjectCode ) SUBJECT , A.SubjectCode SC, B.SubjectID SID, nvl(B.ElectiveCode, ' ') ElectiveCode, max(B.CHOICE) MaxChoice, nvl(C.SubjectRunning,'N') LastStatus,nvl(C.L,'0') L,nvl(C.T,'0') T,nvl(C.P,'0') P,nvl(COURSECREDITPOINT,'0')COURSECREDITPOINT from SUBJECTMASTER A, ";
						qry=qry+" PR#STUDENTSUBJECTCHOICE B, PR#ELECTIVESUBJECTS C where A.INSTITUTECODE=B.INSTITUTECODE And A.INSTITUTECODE=C.INSTITUTECODE And B.SUBJECTTYPE='"+mST+"'and B.ELECTIVECODE=C.ELECTIVECODE";
						...
						qry=qry+" from PREVENTMASTER D Where D.EXAMCODE='"+mE+"' And nvl(D.FREEELECTIVERUNFINALIZED,'N')='Y' and nvl(D.DEACTIVE,'N')='N') ";
			...
			mSubcode=rs.getString("SC");
                        if(mLastStatus.equals("Y"))
				{
					...
					%>
							...
							{
							%>
							<td align=center title="Free Elective Subject Running Deparment List"><a target=_New href="ElectiveSubjRunningDeptList.jsp?SUBJID=<%=mSubid%>&amp;SUBJ=<%=mSubcode%>&amp;INST=<%=mE%>&amp;EXAM=<%=mInstitute%>&amp;STYPE=<%=mST%>&amp;SEMTYPE=<%=mSemType%>"><%=rs.getString("SC")%></a></td>
							<%
							}
