...
if(!mMemberID.equals("") && !mMemberCode.equals("") && !mMemberName.equals(""))
  ...
  qry="Select WEBKIOSK.ShowLink('56','"+ mChkMemID+"','"+mChkMType+"','"+mRole+"','"+ mIPAddress +"') SL from dual";
		...
		qry=qry+" AND NVL(DEACTIVE,'N')='N') and  nvl(LOADDISTRIBUTIONSTATUS,'N') not in ('F','A') and nvl(ELRNNINGFINALIZEDBYHOD,'N')='Y' and MEMBERTYPE='E' and MEMBERID ='"+mChkMemID+"' ";
...
<!--*********Exam**********-->
				...
				try
				{
					...
					*/
					...
					qry=qry+" AND NVL(DEACTIVE,'N')='N' And (INSTITUTECODE, PREVENTCODE) In ";
					qry=qry+" (select INSTITUTECODE, PREVENTCODE from PREVENTS where nvl(DEACTIVE,'N')='N' ";
					qry=qry+" and MEMBERID='"+mChkMemID+"'  and nvl(ELRNNINGFINALIZEDBYHOD,'N')='Y' and nvl(APPROVED,'N')='Y' and nvl(LOADDISTRIBUTIONSTATUS,'N') not in ('F','A') and MEMBERTYPE<>'S' )";									
...
<%         
	...
	if (mExamcode!=null && !mExamcode.equals(""))
	{
		...
		*/
		qry="select distinct 'C' CEF, nvl(L,0)L,nvl(T,0)T,nvl(P,0)P ,A.subjectid subjectid,B.subjectcode subjectcode,NULL ELECTIVECODE,A.Basket Basket,B.Subject||' ('||B.SubjectCode||') ' subj from programscheme A ,Subjectmaster B ";
		qry=qry+" where A.institutecode='"+mInst+"' ";
		...
		qry=qry+" AND A.SECTIONBRANCH = C.SECTIONBRANCH AND C.departmentcode in (select departmentcode ";
		qry=qry+" from employeemaster where employeeid in (select employeeid from hodlist ";
		qry=qry+" where employeeid='"+mDMemberID+"')))  AND A.SUBJECTID=B.SUBJECTID ";
		qry=qry+" and (nvl(L,0)>0 or nvl(T,0)>0 or nvl(P,0)>0 )  ";
		qry=qry+" union ";
		qry=qry+" select distinct 'E' CEF,nvl(L,0)L,nvl(T,0)T,nvl(P,0)P , A.subjectid subjectid, B.subjectcode subjectcode,A.ELECTIVECODE ELECTIVECODE,A.Basket Basket,B.Subject||' ('||B.SubjectCode||') ' subj from PR#ELECTIVESUBJECTS A ,Subjectmaster B  ";
		qry=qry+" where A.institutecode='"+mInst+"' and A.examcode='"+mExamcode+"' ";
		qry=qry+" and A.subjectid in (select C.subjectid from PR#DEPARTMENTSUBJECTTAGGING C ";
		qry=qry+" where  C.examcode='"+mExamcode+"' ";
		qry=qry+" AND A.INSTITUTECODE = C.INSTITUTECODE ";
		...
		qry=qry+" AND A.SECTIONBRANCH = C.SECTIONBRANCH AND  C.departmentcode in (select departmentcode ";
		qry=qry+" from employeemaster where employeeid in (select employeeid from hodlist ";
		qry=qry+" where employeeid='"+mDMemberID+"'))) and A.SUBJECTRUNNING='Y' AND A.SUBJECTID=B.SUBJECTID   ";
		qry=qry+" and (nvl(L,0)>0 or nvl(T,0)>0 or nvl(P,0)>0 )  ";
		qry=qry+" union ";
		qry=qry+" select distinct 'F' CEF, nvl(L,0)L,nvl(T,0)T,nvl(P,0)P ,A.subjectid subjectid,B.subjectcode subjectcode,NULL ELECTIVECODE,A.Basket Basket,B.Subject||' ('||A.SubjectCode||') ' subj from FREEELECTIVE A ,Subjectmaster B  ";
		qry=qry+" where A.institutecode='"+mInst+"' and A.examcode='"+mExamcode+"' ";
		qry=qry+" and A.subjectid in (select C.subjectid from PR#DEPARTMENTSUBJECTTAGGING C ";
		qry=qry+" where  C.examcode='"+mExamcode+"' ";
		qry=qry+" AND A.INSTITUTECODE = C.INSTITUTECODE ";
...
<table cellspacing=0 align=center border=2 rules=groups width=100%>
<tr bgcolor="#ff8c00">
<TR><TD  ALIGN=CENTER><b><font color=green><sup>#</sup></font>Finalized and Send For Approval:-</b>
<input checked type=radio id=radio1 name=radio1 value='N'><b>No</b>
<input type=radio id=radio1 name=radio1 value='Y'><b>Yes</b>
<INPUT Type="submit" Value="Send For Approval"></TD></TR>
</table>
<input type=hidden name=PRCODE ID=PRCODE value='<%=mPrcode%>'>
<input type=hidden name=EXAMCODE ID=EXAMCODE value='<%=mExamcode%>'>
</form>
<font color=green ># Once you will finalize and send for approval,Load distribution can not be modifed.</font> 
	<%
