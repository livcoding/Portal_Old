...
<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><b>Submitted/Requested Subject Choices and Room/Time Preferences</b></TD>
</font></td></tr>
</TABLE>
...
<%
%>
<TABLE rules=ALL align=center cellSpacing=0 cellPadding=0 width="97%" border=1 >
<tr bgcolor='#ff8c00'>
 <td><b><font color="white">SNo.</font></td>
 <td><b><font color="white">Subject</font></td>
 <td><b><font color="white">Elective Code</font></td>
 <td><b><font color="white">Preferences</font></td>
 ...
 <%
	...
	{
/*
	qry="select A.SUBJECTCODE, B.subject,decode(A.SUBJECTTYPE,'E',A.ELECTIVECODE,'C','Core','Free Elective')  ELECTIVECODE, nvl(A.REQROOMTYPE,'None') REQROOMTYPE from PR#FACULTYSUBJECTCHOICES A,subjectmaster B where A.institutecode='"+mI+"' and A.examcode='"+mE+"' ";
	qry=qry+" and A.facultytype='"+mfactype+"' and A.facultyid='"+mDMemberID+"' ";
	qry=qry+" and A.SUBJECTCODE=B.SUBJECTCODE group by A.SUBJECTCODE,B.subject,decode(A.SUBJECTTYPE,'E',A.ELECTIVECODE,'C','Core','Free Elective'), nvl(A.REQROOMTYPE,'None') ";
	qry=qry+"  ORDER BY B.SUBJECT ";
*/
	qry="select A.SUBJECTID,B.SUBJECTCODE, B.subject,decode(A.SUBJECTTYPE,'E',A.ELECTIVECODE,'C','Core','Free Elective')  ELECTIVECODE, nvl(A.REQROOMTYPE,'None') REQROOMTYPE from PR#FACULTYSUBJECTCHOICES A,subjectmaster B where A.institutecode='"+mI+"' and A.examcode='"+mE+"' ";
	qry=qry+" and A.facultytype='"+mfactype+"' and A.facultyid='"+mDMemberID+"' ";
	qry=qry+" and A.SUBJECTID=B.SUBJECTID group by A.SUBJECTID,B.SUBJECTCODE,B.subject,decode(A.SUBJECTTYPE,'E',A.ELECTIVECODE,'C','Core','Free Elective'), nvl(A.REQROOMTYPE,'None') ";
	qry=qry+"  ORDER BY B.SUBJECT ";
	...
	while(rs1.next()) 
	  {  	 kk1++;
			...
			<td><%=rs1.getString("ELECTIVECODE")%></td>
			<td><a Target=_new href="ViewRoomTimePreference.jsp?EC=<%=mE%>&amp;EID=<%=mDMemberID%>&amp;SBJ=<%=rs1.getString("SUBJECTID")%>" title='Click to See/View Class Time and Room Preferences (if any)'>Preference</a></td>
	        </tr>
	        <%
	  } //closing of while
%>
