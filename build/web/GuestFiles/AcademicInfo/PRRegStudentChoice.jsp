...
qry=" SELECT distinct a.EXAMCODE exam from PREVENTMASTER a WHERE a.INSTITUTECODE='"+ mInstitute +"' and nvl(a.PRCOMPLETED,'N')='N' and nvl(a.PRBROADCAST,'N')='Y'";
					qry=qry+" AND NVL(PRREQUIREDFOR,'S')<>'S' AND NVL(a.DEACTIVE,'N')='N' order by exam ";
					rs=db.getRowset(qry);
...
try
	  ...
	  {
		%>
			<select name=DataComboProgram tabindex="0" id="DataComboProgram" style="WIDTH: 0px">	
		<%   
				while(rs.next())
				{
...
try
	{
			...
			rs=db.getRowset(qry);
%>
		<select name=DataComboSection tabindex="0" id="DataComboSection" style="WIDTH:0px" >	 
<%   
	if (request.getParameter("x")==null) 
	{
		while(rs.next())
		...
		try
		{
				...
				rs=db.getRowset(qry);
					if (request.getParameter("x")==null) 
				 	  {
						%>
						<select name=DataComboElective tabindex="0" id="DataComboElective" style="WIDTH: 0px">	
						<%   
						while(rs.next())
					{
...
//***********Elective code*******
	...
	qry=qry+" sectionbranch='"+mSB+"' and electivecode='"+mEL+"' and nvl(DEACTIVE,'N')='N' ";
	//out.print(qry);
	rs=db.getRowset(qry);
	if (rs.next()) 
		maxCol=rs.getInt(1); 
...
qry=qry+" and examcode='"+mEC+"' and programcode='"+mPC+"' and electivecode='"+mEL+"'  ";
...
{
qry1="select subjectid from PR#STUDENTSUBJECTCHOICE where institutecode='"+mInstitute+"' and electivecode='"+mEL+"' ";
