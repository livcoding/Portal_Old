...
try
{
	...
	qry=qry+" InstituteCode='"+mInstitute+"' And nvl(Deactive,'N')='N' and Nvl(LOCKEXAM,'N')='N' order by EXAMPERIODFROM DESC";
	rs=db.getRowset(qry);
...
{     
	...
	qry=qry+" and A.facultytype='I' and ";
	qry=qry+" A.examcode not in (select examcode from exammaster where InstituteCode='"+mInstitute+"' And ( nvl(LOCKEXAM,'N')='Y' ";
	qry=qry+" or nvl(FINALIZED,'N')='Y' or NVL(DEACTIVE,'N')='Y' )) AND A.SUBJECTID=B.SUBJECTID "; 
	qry=qry+" Group By nvl(A.subjectID,' ') , nvl(B.subjectcode,' ') ,A.examcode,nvl(B.subject,' ')||' ('||B.subjectcode||') ' "; 
...
try
{
	...
	qry1=qry1+" select nvl(SECTIONBRANCH,' ') Section from  facultysubjecttagging where employeeid='"+mDMemberID+"' ";
	qry1=qry1+" and facultytype='I' and ";
	qry1=qry1+" examcode not in (select examcode from exammaster where InstituteCode='"+mInstitute+"' And ( nvl(LOCKEXAM,'N')='Y' ";
...
try
{ 
	qry1=" select nvl(SECTIONBRANCH,' ') Section,nvl(subjectid,' ')subjectid,nvl(EXAMCODE,' ')examcode from  facultysubjecttagging where employeeid='"+mDMemberID+"' ";
	qry1=qry1+" and facultytype='I' and ";
	qry1=qry1+" examcode not in (select examcode from exammaster where InstituteCode='"+mInstitute+"' And ( nvl(LOCKEXAM,'N')='Y' ";
	qry1=qry1+" or nvl(FINALIZED,'N')='Y' or NVL(DEACTIVE,'N')='Y' )) ";
...
try
{ 
	qry1="Select SUBSECTIONCODE SubSection from  facultysubjecttagging where employeeid='"+mDMemberID+"' ";
	qry1=qry1+" and facultytype='I' and ";
	qry1=qry1+" examcode not in (select examcode from exammaster where InstituteCode='"+mInstitute+"' And ( nvl(LOCKEXAM,'N')='Y' ";
	qry1=qry1+" or nvl(FINALIZED,'N')='Y' or NVL(DEACTIVE,'N')='Y' ) ) ";
...
<tr><td nowrap colspan=6>
	...
	<td Title="Sort on Roll No"><font color="White"><b>Roll No.</b></font></td>
	<td Title="Sort on Temporary Roll No"><font color="White"><b>Temp Roll No.</b></font></td>
...
<%	
	...
	%>
		...
		<script type="text/javascript">
			var st1 = new SortableTable(document.getElementById("table-1"),["Number", "Number", "Number", "CaseInsensitiveString", "Date", "CaseInsensitiveString"]);
		</script>
	<%
	     }
