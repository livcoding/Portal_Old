...
*/
	...
	<%
		...
		%>
		<select name="<%=mName1%>" id=<%=mName1%> tabindex="0" style="WIDTH: 200px"  onChange="ChangeOptions1('<%=mName1%>','<%=mName2%>',<%=mlt1%>,<%=mlt%>);">
		<%	
		if (request.getParameter("x")==null) 
		{
		if(moldemp1.equals(""))
...
//	{	
	...
	<tr>
	<td>&nbsp;</td>
	<td><%=mSubsection%>&nbsp;(<%=mProgramcode%>)</td> 
	<td>
	<%
	}	
	...
	%>
	     <select <%=mDis%> name="<%=mName1%>" id=<%=mName1%> tabindex="0" style="WIDTH: 200px" onChange="ChangeOptions3('<%=mName1%>','<%=mName2%>',<%=mlt1%>,<%=mlt%>);">
	<%
		if (request.getParameter("x")==null) 
		{	
			if(moldemp.equals(""))
...
%>
	...
	<td>
	<select name='<%=mNameLML%>' id='<%=mNameLML%>' tabindex="0" style="WIDTH: 110px" onChange="ChangeOptions5('<%=mName1%>','<%=mNameLML%>');"> 
	<%
		if(moldMerge.equals(""))
		{
	%>
...
// } closing of else  111
	...
	%>
	<TR>
	<TD ALIGN=CENTER COLSPAN=8><table width=100%><tr><hr><td align=center><input type=submit name=btn id=btn value='Save'></td></tr></table></TD>
	</TR>
	<%
	}
%>
