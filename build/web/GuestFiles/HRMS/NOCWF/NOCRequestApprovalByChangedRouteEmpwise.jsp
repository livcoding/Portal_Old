...
//-------------------------------------------------------
			if(QryChDvFor.equals("CurrEmp"))
			{
				...
				{
					%>
					<form name="frm" method="POST" ACTION="NOCRequestApprovalByChangedRouteActionForIndvEmp.jsp">
...
//---------------Start Of Approval Authority for if(rsFrDept.next())----
				...
				<!--		<font color=Navy face=arial size=2><STRONG>&nbsp; Criteria		<%=FrCbtBsdVal%> to <%=ToCbtBsdVal%> : <img src="../../../Images/arrow.gif">&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; </STRONG>-->
							<input type="hidden" value="E" Name="<%=mName11%>" ID="<%=mName11%>" checked onclick="FunActCurr()"><STRONG><font face=arial size=2 color=green>
							<!--Keep Enabled -->
							</font></STRONG>
							...
							{
								%>
							<!--	<a target=_new href="AddCrBasedWorkFlowLevelForIndvEmp.jsp?DeptCode=<%=mDeptCode%>&amp;WFC=<%=QryWFCode%>&amp;WFT=<%=QryWFType%>&amp;WFL=<%=QryLevelEdit%>&amp;EID=<%=QryFaculty%>" title="Click to add more Approval Level">
								<marquee behavior=alternate scrolldelay=100 width=100% style="cursor:hand"><font color="blue" face=arial size=2>
								<STRONG>Next Criteria-Based Approval Authority Required ?</STRONG>
								</font></marquee></a>-->
								<%
							}
							else
