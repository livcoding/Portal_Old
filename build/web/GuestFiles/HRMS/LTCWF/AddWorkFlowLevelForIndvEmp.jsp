...
try
{
	...
	if(request.getParameter("x")!=null)
	{
		...
		if(mChkLevel.equals("Required"))
		{
		   if(!mApprAuth.equals(""))
		   {
			...
			if(!rs.next())
			{
				qry="INSERT INTO WF#EMPWISEWORKFLOWAUTHORITY (COMPANYCODE, INSTITUTECODE, WORKFLOWCODE, WORKFLOWTYPE, EMPLOYEEID, FACULTYTYPE, WFSEQUENCE, APPROVALBY, APPROVALAUTHORITY, WFLEVEL, ENTRYBY, ENTRYDATE, APPROVALTEXTTOBEDISPLAYED)";
				qry=qry+"VALUES('"+mComp+"','"+mInst+"','"+QryWFCode+"','"+QryWFType+"','"+QryFaculty+"','"+mFacType+"','"+mWFSeqc+"','"+mApprBy+"','"+mApprAuth+"','"+QryWFLevel+"','"+mChkMemID+"',sysdate, '"+mTextToDisp+"')";
				n=db.insertRow(qry);
				//out.print(qry);
				if(n>0)
				{
					...
					rs.next();
				  //-----------Log Entry------------
					db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"WORK FLOW LEVEL ADDITION - FOR <%=%QryWFCode>", "WFCode: "+QryWFCode+" WFType: "+QryWFType+" WFLevel: "+QryWFLevel+" WFSeq: "+mWFSeqc+" Dept: "+mDeptCode, "No MAC Address" , mIPAddress);
				  //----------------------------------
				}
				else
				{
			...
			else
			{
				%><CENTER><br><%
				out.print("<img src='../../../Images/Error1.gif'>");
				out.print("&nbsp;&nbsp;&nbsp <b><font size=3 face='Arial' color='Red'>Level already exists!</font> <br>");
				%></CENTER><%
			}
		   }
		}
	...
	qry=qry+" AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND EMPLOYEEID='"+QryFaculty+"' AND FACULTYTYPE='"+mFacType+"' AND WFLEVEL='"+QryWFLevel+"' Order By WFLEVEL";
	rs=db.getRowset(qry);
	//out.print(qry);
	%>
