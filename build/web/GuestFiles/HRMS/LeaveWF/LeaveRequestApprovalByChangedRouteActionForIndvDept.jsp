...
String mApprAuth="", mApprBy="", mApprovalById="", mWFCode="LEAVE";
String mDepartmentCode="", mSelfDeptCode="", QryDept="", mChangeCase="";
String mChk="",mReqNxt="", mActCurr="", mAddDropLvl="";
		...
		if (RsChk.next() && RsChk.getString("SL").equals("Y"))
		{
			...
			if(QryChDvFor.equals("CurrDept"))
			{
			   if(mAddDropLvl.equals("KP"))
			   {
				...
				//out.print("CHECK - "+mChk+" ReqNext - "+mReqNxt+" ActiveCurr - "+mActCurr+" WFLev - "+mWFLevel+" WFSeq - "+mWFSeq+" SelfDept - "+mDepartmentCode);
			//--------------------------------------------------
						...
						//out.print("ApprBy- "+mApprBy+" ApprAuth - "+mApprAuth);
							...
							//-------------
							   ...
							   {
								qry="UPDATE WF#WORKFLOWAUTHORITY SET APPROVALBY='"+mApprBy+"', APPROVALAUTHORITY='"+mApprAuth+"', APPROVALTEXTTOBEDISPLAYED='"+mTextToFwd+"' WHERE";
								qry=qry+" COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"' AND WORKFLOWCODE='"+mWFCode+"'";
								qry=qry+" AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDepartmentCode+"' AND WFSEQUENCE='"+mWFSeq+"'";
							   }
							   else
							   {
								qry="UPDATE WF#WORKFLOWAUTHORITY SET APPROVALBY='"+mApprBy+"', APPROVALAUTHORITY='"+mApprAuth+"' WHERE";
								qry=qry+" COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"' AND WORKFLOWCODE='"+mWFCode+"'";
								qry=qry+" AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDepartmentCode+"' AND WFSEQUENCE='"+mWFSeq+"'";
							   }
							   //out.print(qry);
			...
			//--------------------------------------------------
						...
						{
							qry="DELETE FROM WF#WORKFLOWAUTHORITY WHERE ";
							qry=qry+" COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"' AND WORKFLOWCODE='"+mWFCode+"'";
							qry=qry+" AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDepartmentCode+"' AND WFSEQUENCE='"+mWFSeq+"' AND WFLEVEL='"+mWFLevel+"'";
							//out.print(qry);
							n1=db.update(qry);
							if(n1>0)
							{
							// Log Entry
							//-----------------
							      db.saveTransLog(mInstitute,mLogEntryMemberID,mLogEntryMemberType ,"DISCARD LEAVE WORK FLOW CRITERIA ON THIS LEVEL", "WFCode:"+mWFCode+" WFType:"+QryWFType+" Req Dept:"+mDepartmentCode+" WFSeq:"+mWFSeq, "NO MAC Address" , mIPAddress);
							//-----------------
							}
						}
			//--------------------------------------------------
					...
					{
						%><BR><CENTER><%
						out.print(" &nbsp;&nbsp;&nbsp <b><font size=3 face='Arial' color='green'>Leave Work Flow Route has been changed successfully...</font></b><br>");
						%></CENTER><%
					}
					if(mTotal==ChkFlag)
					{
					...
					{
						%><BR><CENTER><%
						out.print(" &nbsp;&nbsp;&nbsp <b><font size=3 face='Arial' color='green'>Leave Work Flow Criteria on this level has been discarded successfully...</font></b><br>");
						%></CENTER><%
					}
				}
			//-----Use Iterator Here------
			/*
				if(AL1!=null)
				{
					...
					while(itr.hasNext())
					{
						...
						qry="Select 'Y' from WF#WORKFLOWAUTHORITY WHERE COMPANYCODE='"+COMP+"' AND INSTITUTECODE='"+INST+"' AND WORKFLOWCODE='"+mWFCode+"'";
						qry=qry+" AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+DEPT+"' AND WFSEQUENCE='"+SEQ+"' AND WFLEVEL='"+LVL+"'";
						rs=db.getRowset(qry);
						//out.print(qry);
						if(rs.next())
						{
							qry="UPDATE WF#WORKFLOWAUTHORITY SET APPROVALBY='"+APBY+"', APPROVALAUTHORITY='"+APAU+"', APPROVALTEXTTOBEDISPLAYED='"+mTextToFwd+"' WHERE COMPANYCODE='"+COMP+"' AND INSTITUTECODE='"+INST+"'";
							qry=qry+" AND WORKFLOWCODE='"+mWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDepartmentCode+"' AND WFSEQUENCE='"+SEQ+"'";
							nnn=db.update(qry);
						}
						else
						{
							qry="INSERT INTO WF#WORKFLOWAUTHORITY(COMPANYCODE, INSTITUTECODE, AND WORKFLOWCODE, WORKFLOWTYPE, DEPARTMENTCODE, WFSEQUENCE, WFLEVEL, APPROVALBY, APPROVALAUTHORITY, APPROVALTEXTTOBEDISPLAYED)";
							qry=qry+"VALUES('"+RID+"','"+COMP+"','"+INST+"','"+QryWFCode+"',='"+QryWFType+"','"+DEPT+"','"+SEQ+"','"+LVL+"','"+APBY+"','"+APAU+"','"+mTextToFwd+"')";
						...
						if(nnn>0)
						{
							...
							rs.next();
						  //-----------Log Entry------------
							db.saveTransLog(mInstitute,mLogEntryMemberID,mLogEntryMemberType,"WORK FLOW LEVEL ADDITION - FOR LEAVE", "WFCode: "+QryWFCode+" WFType: "+QryWFType+" WFLevel: "+LVL+" WFSeq: "+SEQ+" Dept: "+mDepartmentCode, "No MAC Address" , mIPAddress);
						  //----------------------------------
						}
					}
					if(nnnFlag>0)
					{
						%><CENTER><br><%
						out.print("&nbsp;&nbsp;&nbsp <b><font size=3 face='Arial' color='Green'>Leave WorkFlow Level Addition Successful...</font> <br>");
						%></CENTER><%
			...
			*/
			   ...
			   else if(mAddDropLvl.equals("DC"))
			   {
				qry="DELETE FROM WF#WORKFLOWAUTHORITY WHERE ";
				qry=qry+" COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"' AND WORKFLOWCODE='"+mWFCode+"'";
				qry=qry+" AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDepartmentCode+"' AND WFLEVEL='"+mWFLevel+"'";
				//out.print(qry);
				n1=db.update(qry);
				if(n1>0)
				{
				// Log Entry
				//-----------------
				      db.saveTransLog(mInstitute,mLogEntryMemberID,mLogEntryMemberType ,"DISCARD LEAVE WORK FLOW LEVEL", "WFCode:"+mWFCode+" WFType:"+QryWFType+" Req Dept:"+mDepartmentCode+" WFLvl:"+mWFLevel, "NO MAC Address" , mIPAddress);
				//-----------------
					mDelFlag++;
					mDelWFLvl=mWFLevel;

					qry="SELECT nvl(WFLEVEL,0) WFLEVEL FROM WF#WORKFLOWAUTHORITY WHERE COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"'";
					qry=qry+" AND WORKFLOWCODE='"+mWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDepartmentCode+"' AND WFLEVEL > "+mDelWFLvl+" order by WFLEVEL ASC";
					rs=db.getRowset(qry);
					//out.print(qry);
					while(rs.next())
					{
						...
						qry="UPDATE WF#WORKFLOWAUTHORITY SET WFLEVEL='"+mApplWFLvl+"' WHERE";
						qry=qry+" COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"' AND WORKFLOWCODE='"+mWFCode+"'";
						qry=qry+" AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDepartmentCode+"' AND WFLEVEL='"+mWFLvl+"'";
						//out.print(qry);
						int pn=db.update(qry);
					}
					%><CENTER><br><%
					out.print(" &nbsp;&nbsp;&nbsp <b><font size=3 face='Arial' color='green'>Leave Work Flow Level has been discarded successfully...</font></b><br>");
					%></CENTER><%
				}
			   }
			//----------------------------
