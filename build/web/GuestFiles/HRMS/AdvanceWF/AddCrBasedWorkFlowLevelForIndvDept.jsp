...
int n=0;
double mCBFR=0, mCBTO=0;
int mWFSeqc=0, QryWFLevel=0;
double QryTotReqVal=0;
double mSetCBFrom=0,mSetCBTo=0;
...
{
	QryDOA=request.getParameter("DATEFROM").toString().trim();
}
...
{
	QryDOR=request.getParameter("DATETO").toString().trim();
}
...
//out.print("RID-"+QryRID+" WFC-"+QryWFCode+" WFT-"+QryWFType+" WFL-"+QryWFLevel+" LvDateFr-"+QryDOA+" LvDateTo-"+QryDOR+" TotalLvDays-"+QryTotReqVal);
String mHead="";
...
try
{
	...
	<input type=hidden Name='DATEFROM' ID='DATEFROM' value='<%=QryDOA%>'>
	<input type=hidden Name='DATETO' ID='DATETO' value='<%=QryDOR%>'>
	...
	mWFSeqc++;
	qry="Select Max(TOVALUE) MaxToVal from WF#WORKFLOWAUTHORITY where CompanyCode='"+mComp+"' and InstituteCode='"+mInst+"' AND ";
	qry=qry+" WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"' AND WFLEVEL='"+QryWFLevel+"'";
	qry=qry+" GROUP BY COMPANYCODE, INSTITUTECODE, WORKFLOWCODE, WORKFLOWTYPE, DEPARTMENTCODE, WFLEVEL";
	rs2=db.getRowset(qry);
	//out.print(qry);
	if(rs2.next())
	{
		mSetCBFrom=rs2.getDouble("MaxToVal")+1;
		mSetCBTo=rs2.getDouble("MaxToVal")+1;
	}
	%>
	<tr><td nowrap>
	...
	<STRONG> Set Criteria From <input type=text style="text-align:right" size=8 maxlength=8 name='CBFR' id='CBFR' value=<%=mSetCBFrom%>></STRONG>
	<STRONG> To <input type=text style="text-align:right" size=8 maxlength=8 name='CBTO' id='CBTO' value=<%=mSetCBTo%>></STRONG>
	</td>
	</tr>
	...
	if(request.getParameter("x")!=null)
	{
		if(request.getParameter("CBFR")==null)
			...
			mCBFR=Double.parseDouble(request.getParameter("CBFR").toString().trim());

		if(request.getParameter("CBTO")==null)
			mCBTO=0;
		else
			mCBTO=Double.parseDouble(request.getParameter("CBTO").toString().trim());
		...
		if(mChkLevel.equals("Required"))
		{
		   if(!mApprAuth.equals(""))
		   {
			...
			if(!rs.next())
			{
				qry="INSERT INTO WF#WORKFLOWAUTHORITY (COMPANYCODE, INSTITUTECODE, WORKFLOWCODE, WORKFLOWTYPE, DEPARTMENTCODE, WFSEQUENCE, APPROVALBY, APPROVALAUTHORITY, WFLEVEL, CRITERIABASEDON, FROMVALUE, TOVALUE, ENTRYBY, ENTRYDATE, APPROVALTEXTTOBEDISPLAYED)";
				qry=qry+"VALUES('"+mComp+"','"+mInst+"','"+QryWFCode+"','"+QryWFType+"','"+mDeptCode+"','"+mWFSeqc+"','"+mApprBy+"','"+mApprAuth+"','"+QryWFLevel+"','D','"+mCBFR+"','"+mCBTO+"','"+mChkMemID+"',sysdate, '"+mTextToDisp+"')";
				n=db.insertRow(qry);
				//out.print(qry);
				if(n>0)
				{
					%><CENTER><br><%
					out.print("&nbsp;&nbsp;&nbsp <b><font size=3 face='Arial' color='Green'>"+gb.toTtitleCase(QryWFCode)+" WorkFlow Criteria Addition Successful...</font> <br>");
					%></CENTER><%
					qry="Select Employeecode from Employeemaster where employeeid='"+mChkMemID+"'";
					rs=db.getRowset(qry);
					rs.next();
				  //-----------Log Entry------------
					db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"ADD WORK FLOW CRITERIA - FOR "+QryWFCode+"", "WFCode: "+QryWFCode+" WFType: "+QryWFType+" WFLevel: "+QryWFLevel+" WFSeq: "+mWFSeqc+" Dept: "+mDeptCode, "No MAC Address" , mIPAddress);
				  //----------------------------------
				}
				else
				{
	...
	qry=qry+" AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"' AND WFLEVEL='"+QryWFLevel+"' Order By CBFR, CBTO";
	rs=db.getRowset(qry);
	//out.print(qry);
	%>
	...
	while(rs.next())
	{
		...
		<td align=center><font color=<%=mColor%>><%=rs.getDouble("CBFR")%></font></td>
		<td align=center><font color=<%=mColor%>><%=rs.getDouble("CBTO")%></font></td>
