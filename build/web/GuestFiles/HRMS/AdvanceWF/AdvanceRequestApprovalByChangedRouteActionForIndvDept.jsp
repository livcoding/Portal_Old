...
int mFlag=0, mDelFlag=0, mRecFlag=0, ChkFlag=0;
int mWFSeq=0, mWFLevel=0;
double QryReqVal=0;
String mApprAuth="", mApprBy="";
String QryDept="", mChangeCase="";
String mChk="",mReqNxt="", mActCurr="", mAddDropLvl="";
String qry="",AL1Val="", mSetTextToFwd="", mTextToFwd="";
...
{
	QryReqVal=Double.parseDouble(request.getParameter("REQVAL").toString().trim());
}
...
{
	QryDept=request.getParameter("DeptCode").toString().trim();
}
	...
	if(!mMemberID.equals("") && !mMemberCode.equals(""))
	{
	  ...
	  //-----------------------------
		...
		if (RsChk.next() && RsChk.getString("SL").equals("Y"))
		{
			...
			<input type=hidden Name='REQVAL' ID='REQVAL' value='<%=QryReqVal%>'>
			<input type=hidden Name='QryWFL' ID='QryLVL' value='<%=mWFLevel%>'>
			<input type=hidden Name='DeptCode' ID='DeptCode' value='<%=QryDept%>'>
			<%
			//----------------------
			if(QryChDvFor.equals("CurrDept"))
			{
			   if(mAddDropLvl.equals("KP"))
			   {
				...
				//out.print("CHECK - "+mChk+" ReqNext - "+mReqNxt+" ActiveCurr - "+mActCurr+" WFLev - "+mWFLevel+" WFSeq - "+mWFSeq+" SelfDept - "+QryDept);
			//--------------------------------------------------
						...
						//out.print("ApprBy- "+mApprBy+" ApprAuth - "+mApprAuth);
							...
							//-------------
							   if(mSetTextToFwd.equals("Y"))
							   {
								...
								qry=qry+" COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInst+"' AND WORKFLOWCODE='"+QryWFCode+"'";
								qry=qry+" AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+QryDept+"' AND WFSEQUENCE='"+mWFSeq+"'";
							   }
							   else
							   {
							   ...
							   if(n>0)
							   {
								...
								// Log Entry
								//-----------------
							      db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"APPROVE "+QryWFCode+" WORK FLOW CRITERIA ON THIS LEVEL", "WFCode:"+QryWFCode+" WFType:"+QryWFType+" Req Dept:"+QryDept+" WFSeq:"+mWFSeq, "NO MAC Address" , mIPAddress);
								//-----------------
							   }
							   mFlag=0;
							}
			...
			//--------------------------------------------------
						else if(mActCurr.equals("D") && mChk.equals("Checked"))
						{
							...
							qry=qry+" COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInst+"' AND WORKFLOWCODE='"+QryWFCode+"'";
							qry=qry+" AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+QryDept+"' AND WFSEQUENCE='"+mWFSeq+"' AND WFLEVEL='"+mWFLevel+"'";
							//out.print(qry);
							n1=db.update(qry);
							if(n1>0)
							{
							// Log Entry
							//-----------------
							      db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"DISCARD "+QryWFCode+" WORK FLOW CRITERIA ON THIS LEVEL", "WFCode:"+QryWFCode+" WFType:"+QryWFType+" Req Dept:"+QryDept+" WFSeq:"+mWFSeq, "NO MAC Address" , mIPAddress);
							//-----------------
							mDelFlag++;
							}
						}
			//--------------------------------------------------
					...
					{
						%><BR><CENTER><%
						out.print(" &nbsp;&nbsp;&nbsp <b><font size=3 face='Arial' color='green'>"+gb.toTtitleCase(QryWFCode)+" Work Flow Criteria on this level has been discarded successfully...</font></b><br>");
						%></CENTER><%
					}
				}
			//-----Use Iterator Here------
			/*
				if(AL1!=null)
				{
					...
					while(itr.hasNext())
					{
						...
						qry="Select 'Y' from WF#WORKFLOWAUTHORITY WHERE COMPANYCODE='"+COMP+"' AND INSTITUTECODE='"+INST+"' AND WORKFLOWCODE='"+QryWFCode+"'";
						...
						if(rs.next())
						{
							...
							qry=qry+" AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+QryDept+"' AND WFSEQUENCE='"+SEQ+"'";
							nnn=db.update(qry);
						}
						else
						{
			...
			*/
			   ...
			   else if(mAddDropLvl.equals("DC"))
			   {
				qry="DELETE FROM WF#WORKFLOWAUTHORITY WHERE ";
				qry=qry+" COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInst+"' AND WORKFLOWCODE='"+QryWFCode+"'";
				qry=qry+" AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+QryDept+"' AND WFLEVEL='"+mWFLevel+"'";
				//out.print(qry);
				n1=db.update(qry);
				if(n1>0)
				{
				// Log Entry
				//-----------------
				      db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"DISCARD "+QryWFCode+" WORK FLOW LEVEL", "WFCode:"+QryWFCode+" WFType:"+QryWFType+" Req Dept:"+QryDept+" WFLvl:"+mWFLevel, "NO MAC Address" , mIPAddress);
				//-----------------
					mDelFlag++;
					mDelWFLvl=mWFLevel;

					qry="SELECT nvl(WFLEVEL,0) WFLEVEL FROM WF#WORKFLOWAUTHORITY WHERE COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInst+"'";
					qry=qry+" AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+QryDept+"' AND WFLEVEL > "+mDelWFLvl+" order by WFLEVEL ASC";
					rs=db.getRowset(qry);
					//out.print(qry);
					while(rs.next())
					{
						...
						qry=qry+" COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInst+"' AND WORKFLOWCODE='"+QryWFCode+"'";
						qry=qry+" AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+QryDept+"' AND WFLEVEL='"+mWFLvl+"'";
						//out.print(qry);
