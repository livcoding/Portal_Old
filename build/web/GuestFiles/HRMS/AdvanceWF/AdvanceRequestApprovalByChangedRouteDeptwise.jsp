...
int ctr=0, mWFSeqc=0, mLevelEdit=0, QryLevelEdit=0, mMaxLevelEdit=0, QryWFLevel=0;
double QryTotReqVal=0, QryTotAdvAmount=0;
String mDeptCode="", mDptCode="", QryDept="", mChangeCase="";
String mEmpName="", mEmpCode="", QryEmp="", mEmpID="";
String mDegsName="", mDegsCode="", QryDegs="";
...
//-------------------------------------------------------
			if(QryChDvFor.equals("CurrDept"))
			{
				...
				<td nowrap>
				<%
				qry="Select Distinct nvl(WFLevel,0)WFL From WF#WORKFLOWAUTHORITY where COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInst+"' AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"' AND nvl(DEACTIVE,'N')='N' ORDER BY WFL";
				...
				qry="Select nvl(WEBKIOSK.HasPendingWF('"+mComp+"','"+mInst+"','"+QryWFCode+"','"+QryWFType+"','"+mDeptCode+"','','"+QryLevelEdit+"'),'Zero')HasPendWF from dual";
				rs=db.getRowset(qry);
				//out.print(qry);
				if((rs.next() && rs.getString("HasPendWF").equals("N")) && (QryLevelEdit!=1000))
				{
					%>
					<form name="frm" method="POST" ACTION="AdvanceRequestApprovalByChangedRouteActionForIndvDept.jsp">
...
//---------------Start Of Approval Authority for if(rsFrDept.next())----
						...
						qry=qry+" from WF#WORKFLOWAUTHORITY WHERE CompanyCode='"+mComp+"' and InstituteCode='"+mInst+"' AND WORKFLOWCODE='"+QryWFCode+"'";
						...
						while(rs.next())
						{
							...
							<tr>
							<td nowrap><input type="radio" onclick="FunLReqBy()" value="D" Name="<%=mName1%>" ID="<%=mName1%>"><font color=black face=arial size=2><STRONG>Degs Based</STRONG></font></td>
							<td nowrap>
							<%
							...
							{
								%>
								<a target=_new href="AddCrBasedWorkFlowLevelForIndvDept.jsp?DeptCode=<%=mDeptCode%>&amp;WFC=<%=QryWFCode%>&amp;WFT=<%=QryWFType%>&amp;WFL=<%=QryLevelEdit%>&amp;RID=<%=QryRID%>&amp;TLD=<%=QryTotReqVal%>&amp;DOA=<%=QryDOA%>&amp;DOR=<%=QryDOR%>" title="Click to add more Approval Level"><marquee behavior=alternate scrolldelay=100 width=100% style="cursor:hand"><font color="blue" face=arial size=2><STRONG>Next Criteria-Based Approval Authority Required ?</STRONG></font></marquee></a>
								<%
							}
							else
							{
