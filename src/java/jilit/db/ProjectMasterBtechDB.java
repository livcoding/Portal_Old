 ...
 * @author nipun.gupta
 */
public class ProjectMasterBtechDB {
    private Connection dbConnection;
    private PreparedStatement pStmt;
    private CallableStatement callableStatement = null;
    private ResultSet rs;
    SimpleDateFormat formatter = new SimpleDateFormat("dd-MM-yyyy");
    public ProjectMasterBtechDB() {
        dbConnection = DBUtility.getConnection();
    }

     private enum scase {
        selectFacultyInfo,selectStudentInfo,saveupdate, select,SelectforUpdate,setDepartment
    }
     ...
     public String selectSaveUpdateData(String jdata) {
        ...
        try {
            ...
            switch (ProjectMasterBtechDB.scase.valueOf((String) hm.get("handller").toString())) {
        ...
        try {
            ...
            if(!hm.get("searchNames").equals("")){
              searchBoxValue="and (sm.studentname like '%"+hm.get("searchNames")+"%')";
                ...
                sqry.append("SELECT a.*, B.*" + "  FROM (SELECT COUNT (studentid) Totalrecord FROM studentmaster where nvl(deactive,'N')='N' and programcode in('B.T')) a,\n"
                        ...
                        + "from studentmaster sm where nvl(sm.deactive,'N')='N' and programcode in('B.T') "
                        + ""+searchBoxValue+" order by R) WHERE r > ").append(hm.get("spg")).append(" AND r <= ").append(hm.get("epg")).append(") b");
     ...
     private String SaveUpdateData(Map hm) 
   {
       ...
       try {
           if (hm.get("projectID").equals("0")) {
               ...
               eqry.append("insert into  AP#PROJECTMASTERBTECH ( COMPANYID,DEPARTMENTCODE,PROJECTID,PROJECTCODE,");
               eqry.append("PROJECTTITLE,APPUBLICATIONTYPEID,ACADEMICYEAR,STUDENTID,STAFFID,PROJECTPERSTATUS,PROJECTSTATUS,");
              ...
              eqry.append(hm.get("projectTitle")).append("',");
               eqry.append("'").append(hm.get("publicationsID")).append("','")
               ...
               sqry.append("Update AP#PROJECTMASTERBTECH set COMPANYID='").append(hm.get("companyid"));
               ...
               sqry.append("',PROJECTCODE='").append(hm.get("projectCode"));
                ...
                sqry.append("',PROJECTTITLE='").append(hm.get("projectTitle"))
                       .append("',APPUBLICATIONTYPEID='").append(hm.get("publicationsID")).append("',");
     ...
     private Map getSelectData(Map hm) {
          ...
          try {
             if (!hm.get("searchbox").equals("")) {
                 ...
                 searchBoxValue = searchBoxValue + " or apm.appublicationtypedescription like '%" + hm.get("searchbox") + "%'";
                 searchBoxValue = searchBoxValue + " or am.apiscore like '%" + hm.get("searchbox") + "%')";
             }

             sqry.append("SELECT a.*, B.* FROM (SELECT COUNT (projectid) Totalrecord FROM  ap#projectmasterbtech where companyid='"+hm.get("companyID")+"') a,\n"
                     + "  (SELECT *\n"
                     ...
                     + "nvl(apm.appublicationtypedescription,'')appublicationtypedescription,"
                     ...
                     + " row_number() over (order by am.projectid desc)  R from ap#projectmasterbtech am,departmentmaster dm,v#staff vs,studentmaster sm,ap#publicationtypemaster apm"
                     + " where dm.departmentcode=am.departmentcode and vs.employeeid=am.staffid and vs.companycode=am.companyid and sm.studentid=am.studentid and apm.appublicationtypeid=am.appublicationtypeid and am.companyid='"+hm.get("companyID")+"' " + searchBoxValue + ")\n"
             ...
             while (rs.next()) {
                 ...
                 data.put("publication", rs.getString(6));
         ...
         Map SelectData = new HashMap();
             try {
                 sqry.append(" select nvl(am.DEPARTMENTCODE,'')DEPARTMENTCODE,nvl(dm.DEPARTMENT,'')DEPARTMENT,"
                         ...
                         + "nvl(am.APPUBLICATIONTYPEID,'')APPUBLICATIONTYPEID,nvl(am.ACADEMICYEAR,'')ACADEMICYEAR,"
                         ...
                         + "nvl(am.DEACTIVE,'')DEACTIVE,nvl(sm.ENROLLMENTNO,'')ENROLLMENTNO from AP#PROJECTMASTERBTECH am,v#staff vs,STUDENTMASTER sm,departmentmaster dm");
