...
<HTML>
<head>
<TITLE>#### <%=mHead%> [ Deactive WebKiosk Member ] </TITLE>
...
<INPUT Type="Hidden" Name=Inst id=Inst Value=<%=mInst%>>
		...
		else
		{
			...
			{
			%>
				<option value="P" Selected>Parent</option>
			<%
			}
			else
			{
...
%>
	<INPUT Readonly name=txtCode value='<%=mMcode%>' style =" FONT-WEIGHT: bold; BACKGROUND-COLOR: #fce9c5; BORDER-BOTTOM: medium none; BORDER-LEFT-STYLE: none; BORDER-RIGHT-STYLE: none; BORDER-TOP-STYLE: none; FONT-FAMILY: sans-serif; FONT-SIZE: x-small; FONT-STYLE: Bold; HEIGHT: 22px; TEXT-ALIGN: right; 
	...
	<%	
		...
		if(!mMemberCode.equals(""))
		{
			...
			{
				qry="select nvl(GUESTNAME,' ')GUESTNAME from GUEST where GUESTCODE='"+mMemberCode+"' ";
				rss=db.getRowset(qry);
				if(rss.next())
				{
					memployee=rss.getString("GUESTNAME");
				}
			}  // closing of flag4
			if(mradio.equals("Y"))
			{
				...
				else
				{
  					...
  					if(n>0)		
					{
            				//---- Log Entry
						//-----------------																
						db.saveTransLog(mInst,"ADMIN","A","DEACTIVE/ACTIVE WEBKIOSK MEMBER", "DEACTIVE KIOSK MEMBER ,Member Code : "+mMemberCode+" Member Type :"+mMType, "No MAC Address" , mIPAddress);
						//-----------------
						%>
			...
			{
				qry="select 'Y' from employeemaster where employeecode='"+mMemberCode+"' UNION select 'Y' from studentmaster where enrollmentno='"+mMemberCode+"'  UNION select 'Y' from guest where guestcode='"+mMemberCode+"'";
				rse=db.getRowset(qry);
				//out.print(qry);
				if(rse.next())
				{
					...
					{*/
						qry="update MEMBERMASTER set DEACTIVE='N',FLAC=0  Where trim(ORATYP)='"+mEMType+"' and trim(ORACD)='"+mEMemberCode+"'";
						...
						if(n>0)			  
						{
							//---- Log Entry
							//-----------------
							db.saveTransLog(mInst,"ADMIN","A","DEACTIVE/ACTIVE WEBKIOSK MEMBER", "ACTIVE KIOSK MEMBER ,Member Code : "+mMemberCode+"Member Type :"+mMType, "No MAC Address" , mIPAddress);
							//-----------------
							%>
							<table rules=none border=1 Bgcolor=white align=center>
