...
int mSNO=0;
String pPassd="",mCounsPass="";
String minst="", mAcademicyear="";
String mName1=""	,mName2="",mName3="",mName4="",mName5="",mInst="";
String pMemberID="" ,pMemberCode="" , pMemberType=""  ,pMemberRole="" , pPass="", mName="",pCounsPass="";
int mMaxPWD=20;
int mMinPWD=4;
int mFlag=0;
String qryt="",mTs="";
...
//{
	...
	{
		 mSem=request.getParameter("Semester");
	}
	...
	if (mStr.equals(""))
	{
		 qry="select STUDENTID, ENROLLMENTNO, STUDENTNAME, PROGRAMCODE,COUNSELLINGPASSWORD from STUDENTMASTER Where academicyear='"+mAcademicyear+"' and  semester='"+mSem+"' and QUOTA= decode('"+mQUOTA+"','ALL',quota,'"+mQUOTA+"')   ";
		 qry=qry+" and INSTITUTECODE='"+mInst+"' and ENROLLMENTNO is not null and nvl(deactive,'N')='N' AND NVL(PASSWORDRESETFLAG,'N')='N'   AND COUNSELLINGPASSWORD IS NOT NULL";
	}
	else
	{
		qry="select STUDENTID, ENROLLMENTNO, STUDENTNAME, PROGRAMCODE,COUNSELLINGPASSWORD from STUDENTMASTER Where ProgramCode IN ("+mStr+") and ";
		qry=qry+" academicyear='"+mAcademicyear+"' and  semester='"+mSem+"' and QUOTA= decode('"+mQUOTA+"','ALL',quota,'"+mQUOTA+"')  and ENROLLMENTNO is not null and INSTITUTECODE='"+mInst+"' and nvl(deactive,'N')='N' AND NVL(PASSWORDRESETFLAG,'N')='N'   AND COUNSELLINGPASSWORD IS NOT NULL  ";
	}
	rs=db.getRowset(qry);
	//out.print(qry);
//	int mFlag=0;mInst mAcademicyear
	...
	<TABLE cellSpacing=1 cellPadding=1 width="100%" border=1 rules="groups" align=center bordercolor=black>
				...
				<TD><FONT face=arial color=black><B>Password</B></FONT></TD>
                                <TD><FONT face=arial color=black><B>Couns Password</B></FONT></TD>
	...
	while(rs.next())
	{
		...
		mName=rs.getString("STUDENTNAME");
                mCounsPass=rs.getString("COUNSELLINGPASSWORD");

		pMemberID =enc.encode(pdMemberID);
		pMemberCode=enc.encode(pdMemberCode);
                pCounsPass=enc.encode(mCounsPass);
		...
		//out.print(mI+":"+mE+":"+mN);
		//pPassd=mI+mE+mN;
		//if(mCounsPass.equals(""))
		//	mCounsPass=" ";
		//else
		//	mCounsPass=mCounsPass;

		//pPass=enc.encode(mCounsPass);
		...
		if(!rs1.next())
		{
			...
			qryi=qryi+"VALUES ('"+pMemberID+"','"+pMemberType+"','"+pMemberRole+"','"+pCounsPass+"','"+pMemberCode+"','"+mCounsPass+"','"+mSTemail+"','"+pdMemberCode+"','"+mName+"')";
			//out.print(qryi);
			int n=0;
			n=db.insertRow(qryi);
			if(n>0)
			...
			//	db.memberSignp(pMemberID ,pMemberCode , pMemberType  ,pMemberRole , pPass);
				mFlag=1;
                              qryus="UPDATE STUDENTMASTER SET PASSWORDRESETFLAG='Y' WHERE STUDENTID='"+pdMemberID+"' AND INSTITUTECODE='"+mInst+"' AND academicyear='"+mAcademicyear+"'";

                               int  q=db.update(qryus);
                               if(q>0)
                                 {
                                 }
                               else
                                 {
                                    out.print("<font color=red>Error while Updation in Student record during insert logic..</font>");
                                 }
				%>
			...
			{
				out.print("<font color=red>Error while LoginId Insert creation...</font>");
			}
                ...
                {
                 qryu="UPDATE MEMBERMASTER SET ORAPW='"+pCounsPass+"' , PWD='"+mCounsPass+"' where ORATYP='"+pMemberType+"' AND ORAADM='"+pMemberRole+"' and ORAID='"+pMemberID+"'";

                 int m=db.update(qryu);
                  if(m>0)
                  {
                   qryus="UPDATE STUDENTMASTER SET PASSWORDRESETFLAG='Y' WHERE STUDENTID='"+pdMemberID+"' AND INSTITUTECODE='"+mInst+"' AND academicyear='"+mAcademicyear+"'";
                   
                   int p=db.update(qryus);
                   if(p>0)
                   {
                   }
                   else
                   ...
                   out.print("<font color=red>Error while Updation in Student record..</font>");
                   }

                 }
                 else
                 {
                   out.print("<font color=red>Error while LoginId Update operation..</font>");
                 }
...
//---- Log Entry
	...
	//{
		...
		if (mStr.equals(""))
		{
		 qry="select A.STUDENTID, A.ENROLLMENTNO, A.STUDENTNAME,a.BRANCHCODE, A.PROGRAMCODE, A.academicyear, A.COUNSELLINGPASSWORD" +
                 ", B.PWD from STUDENTMASTER A, MEMBERMASTER b Where A.academicyear='"+mAcademicyear+"' " +
                 "and A.semester='"+mSem+"' AND B.ORATYP='"+pMemberType+"' AND B.ORAADM='"+pMemberRole+"' ";
		 qry=qry+"and A.INSTITUTECODE='"+mInst+"' and QUOTA=decode('"+mQUOTA+"','ALL',quota,'"+mQUOTA+"')  AND a.ENROLLMENTNO=b.MEMCODE " +
                 "and A.ENROLLMENTNO is not null and nvl(A.deactive,'N')='N' AND NVL(A.PASSWORDRESETFLAG,'N')='Y'   AND COUNSELLINGPASSWORD IS NOT NULL order by ENROLLMENTNO";
		}
		else
		{
		qry="select A.STUDENTID, A.ENROLLMENTNO, A.STUDENTNAME, A.PROGRAMCODE,a.BRANCHCODE," +
                " A.academicyear, A.COUNSELLINGPASSWORD,B.PWD from STUDENTMASTER A, MEMBERMASTER b Where" +
		...
		qry=qry+" A.academicyear='"+mAcademicyear+"'" +
                " and A.semester='"+mSem+"' and QUOTA=decode('"+mQUOTA+"','ALL',quota,'"+mQUOTA+"')  " +
                "and A.ENROLLMENTNO is not null and A.INSTITUTECODE='"+mInst+"'" +
                " and nvl(A.deactive,'N')='N' AND NVL(A.PASSWORDRESETFLAG,'N')='Y'   AND COUNSELLINGPASSWORD IS NOT NULL order by ENROLLMENTNO";
		}
		rs=db.getRowset(qry);
 //out.print(qry);
		while (rs.next())
			...
			%>
				...
				<tr>
					...
					<td ><%=rs.getString("PWD")%></td>
                                        <td ><%=rs.getString("COUNSELLINGPASSWORD")%></td>
