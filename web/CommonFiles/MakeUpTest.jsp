...
String mSubjectID="",mSubjectcode="",mEmployeecode="",mFacultytype="";
...
%>
	...
	<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy">Allow Students for MakeUp Test</TD>
	</font></td></tr>
	</TABLE>
	...
	<%
		qry="select distinct nvl(institutecode,' ')IC from INSTITUTEMASTER where nvl(DEACTIVE,'N')='N' ";
	      ResultSet rsi=db.getRowset(qry);
		while(rsi.next())
		{
...
<%  
	...
	{
		/*
		qry="select distinct examcode GRADEENTRYEXAMID  from V#studenteventsubjectmarks where examcode not in ";
		qry=qry+" (select distinct examcode from studentwisegrade) ";
		*/
		
		qry="select distinct EXAMID GRADEENTRYEXAMID  from DEFAULTVALUES";
		rs=db.getRowset(qry);
	...
	// *************DataComboEvent********* 
	/*
	qry="select  EVENTSUBEVENT,examcode ";
	qry=qry+"   from V#STUDENTEVENTSUBJECTMARKS  ";
	qry=qry+" WHERE Institutecode='"+mInst+"' ";
	qry=qry+" And examcode not in (select examcode from studentwisegrade group by examcode) ";
...
<%
	...
	qry="select A.subject||'('||A.subjectcode||')' subject,A.examcode,A.eventsubevent, ";
	qry=qry+" A.subjectcode, A.SubjectID from V#STUDENTEVENTSUBJECTMARKS  A";
	qry=qry+" WHERE A.Institutecode='"+mInst+"' ";
	qry=qry+" And A.examcode in (select examid from DEFAULTVALUES) ";
	qry=qry+" And A.subjectid not in (select b.subjectid from studentwisegrade b where b.examcode in (select c.examid from DEFAULTVALUES c)) ";
	qry=qry+" and nvl(PUBLISHED,'N')='Y' AND NVL(LOCKED,'N')='Y'  ";
	qry=qry+" and (nvl(Detained,'N')='D' or nvl(Detained,'N')='A' or nvl(Detained,'N')='M') "; 		
	qry=qry+" group by subject||'('||subjectcode||')',examcode,eventsubevent,subjectcode, SubjectID ";
...
<%
	try
	{
	qry="select  subject||'('||subjectcode||')' subject, ";
	qry=qry+" subjectcode, SubjectID from V#STUDENTEVENTSUBJECTMARKS  ";
	qry=qry+" WHERE Institutecode='"+mInst+"' ";
	qry=qry+" And examcode in (select examid from DEFAULTVALUES) ";
	qry=qry+" And subjectid not in (select subjectid from studentwisegrade where examcode in (select examid from DEFAULTVALUES)) ";
	qry=qry+" and nvl(PUBLISHED,'N')='Y' AND NVL(LOCKED,'N')='Y'  ";
	qry=qry+" and (nvl(Detained,'N')='D' or nvl(Detained,'N')='A' or nvl(Detained,'N')='M') and eventsubevent='"+qrymevent+"' and examcode='"+qryexam+"' "; 		
	qry=qry+" group by subject||'('||subjectcode||')', subjectcode, SubjectID";
...
try
{  
	qry="select distinct fstid,subjectcode,subjectid,detained2,maxmarks,sectionbranch,subsectioncode,employeename,nvl(studentid,' ')studentid,nvl(studentname,' ')studentname, ";
	qry=qry+" nvl(enrollmentno,' ')enrollmentno";
	qry=qry+ " from V#STUDENTEVENTSUBJECTMARKS ";
	qry=qry+" where institutecode='"+mIC+"' and nvl(locked,'N')='Y' and MARKSAWARDED1 is  null and MARKSAWARDED2 is null  ";
	qry=qry+"and  examcode='"+mEC+"' and subjectid=decode('"+mSC+"','ALL',subjectid,'"+mSC+"') and EVENTSUBEVENT='"+mEvent+"' and ltp='L' and nvl(PUBLISHED,'N')='Y' ";
	qry=qry+" and (nvl(Detained,'N')='D' or nvl(Detained,'N')='A' or nvl(Detained,'N')='M') "; 		
  ...
  %>	
	...
	<td nowrap><%=rs1.getString("subjectcode")%></td>
	<td nowrap><%=GlobalFunctions.toTtitleCase(rs1.getString("employeename"))%></td>
  ...
  %>	
	...
	<input type=hidden name='<%=mName5%>' id='<%=mName5%>' value='<%=rs1.getString("subjectcode")%>'>	
