...
{
	mlisp=session.getAttribute("IspMemberType").toString();
}
...
if(!mOldPwd.equals("") && !mNewPwd.equals("") && !mRetPwd.equals("") && (mNewPwd.length()>=mMinPWD && mNewPwd.length()<=mMaxPWD) )
{
	try
	{
		...
		mCode=enc.decode(mMemberCode);
	        mType=enc.decode(mMemberType);
              if(mlisp!=null &&mlisp!="" ){
              lisp=enc.decode(mlisp);
              }else{
              session.removeAttribute("IspMemberType");
              }
	...
	if (mNewPwd.equals(mRetPwd))
	{
		try
		{
                        ...
                        qry="Select nvl(ORAPW,' ')PWD From MEMBERMASTER Where trim(ORATYP)='"+mlisp+"' and ORACD='"+mMemberCode+"'";
                        }else{
			...
			if(rs.next())
			{
				...
				{
					//qry="update MEMBERMASTER set ORAPW='"+mENewPwd+"' ,PWD='"+mNewPwd+"' Where trim(ORACD)='"+mMemberCode+"' and trim(ORATYP)='"+mMemberType+"'";

                                        if(lisp.equalsIgnoreCase("P")){
                                        qry="update MEMBERMASTER set ORAPW='"+mENewPwd+"' ,PWD=NULL,EMAIL=NULL,MEMCODE=NULL,MEMNAME=NULL  ,FIRSTLOGIN='Y' , FLAC=0  Where trim(ORACD)='"+mMemberCode+"' and trim(ORATYP)='"+mlisp+"'";
                                        }else{
					...
					<table align=center>
					<tr><td>
					<a href ="ChangePassword.jsp"><img border=0 src='../Images/Back.jpg'></a>
					</td></tr>
					 </table>
					<%
					}
