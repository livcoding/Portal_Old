...
/*
***************************************************************************************
	' * File Name:		StudEmpDrCrAdvice.JSP			[For Employee]				
...
String mSem="",mEnrollment="", mType="", mSName="";
int mSemPlusOne=0;
String mProg="",mBranch="",mName="",mDesignation="",mDepartment="", mEmpName="",mSID="";
String mINSTITUTECODE="",mComp="";
...
{//1
	...
	//----------------------------------------------------------------------- 
	int mRID=0;
	mType=request.getParameter("mType").toString().trim();
	...
	<table width=100% rules=groups cellspacing=1 cellpadding=1 align=center border=1>
	  <tr>
		...
		%>
		<td><font color=black face=arial size=2><STRONG>Name:&nbsp;</STRONG></font><%=mEmpName%></td>
		<td><font color=black face=arial size=2><STRONG>Department:&nbsp;</STRONG></font><%=mDepartment%></td>
		<td><font color=black face=arial size=2><STRONG>Designation:&nbsp;</STRONG></font><%=mDesignation%></td>
	  </tr>
	  <%
  			}
		}
...
%>
	  <tr>
	   <td colspan="8" class="tablecell" align="left">
	   <b>Report Between Date:&nbsp;</b>
<%
   // ***************************** FROM DATE TNTRY ********************************
...
<%
		...
		mToDate=""+CurrentDay+"/"+CurrentMonth+"/"+CurrentYear;
		 ...
		 qry2=qry2+" Where COMPANYCODE='"+mComp+"' And INSTITUTECODE='"+mINSTITUTECODE+"' And SLTYPE='"+mType+"' and SLCODE ='"+mSID+"'";
...
%>
	...
	<thead>
	  <tr bgcolor="#ff8c00">
	   <td align=center Title="Click on Advice Date to sort"><b><font  color="White">Date</font></b>
	   </td>
	   <td align=center Title="Click on Location to Sort"><b><font color="White">Location</font></b>
	   </td>
	   <td align=center Title="Click on Reason to Sort"><b><font color="White">Reason </font></b>
	   </td> 
	   <td align=center Title="Click on Advice Type to Sort"><b><font color="White">Advice Type </font></b>
	   </td> 
	   <td align=center Title="Click on Amount to Sort"><b><font color="White">Amount</font></b>
	   </td> 
	   <td align=center Title="Click on Status to Sort"><b><font color="White">Status</font></b>
	   </td> 
	  </tr>
	</thead>
	<tbody>
...
<%
					...
					{
						qry6=" select nvl(C.DEPARTMENT,'') mNAME from DEPARTMENTMASTER C where  c.DEPARTMENTCODE='"+rs.getString("FROMDEPARTMENT")+"' ";
					}
					else if(rs.getString("FROMWHERE").equals("C"))
					{
						qry6=" select nvl(D.COSTCENTRENAME,'') mNAME from COSTCENTREMASTER D where  d.COSTCENTRECODE='"+rs.getString("FROMCOSTCENTRECODE")+"' ";
					}
					if (!qry6.equals(""))
						{
...
<%					 
					 ...
					 {
%>
					 <td class="tablecell"> &nbsp;<A title='Click here to Dr/Cr Amount detail.' href='DetailStudEmpDrCrAdvice.jsp?mMType=<%=mType%>&amp;SID=<%=mSID%>' target='_new'>&nbsp;<%=rs.getString("STATUS")%></td>
<%
					 }
					 else
					 {
...
<%	
			    ...
			    {
%>
	<FORM METHOD=POST ACTION="ADMNEmpStudDrCrAdvice.jsp">
	  ...
	  <input type="hidden" Name="MemberName" value="<%=mEmpName%>">
...
<%
		...
		qry4=qry4+" And nvl(DEACTIVE,'N')<>'Y' And COMPANYCODE='"+mComp+"' And INSTITUTECODE='"+mINSTITUTECODE+"' And SLTYPE='"+mType+"' and SLCODE ='"+mSID+"'";
...
<%
		...
		qry5=qry5+" And nvl(DEACTIVE,'N')<>'Y' And COMPANYCODE='"+mComp+"' And INSTITUTECODE='"+mINSTITUTECODE+"' And SLTYPE='"+mType+"' and SLCODE ='"+mSID+"'";
