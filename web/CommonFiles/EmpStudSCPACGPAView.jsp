...
String mMemberType="";
String mMemberName="";
String mMemberCode="",mInst="",mDMemberCode="",mWebEmail="",mSID="", mProg="";
int mSems=0;
int mFlag=0,	flag=0;
...
<thead>
 <tr bgcolor="#ff8c00">
 <td nowrap Title="Click on Course to sort"><b><font color="White">Program</font></b></td>
...
<tbody>
<%
	qry="select programcode from studentmaster where studentid='"+mSID+"'";
	rs1=db.getRowset(qry);
	if(rs1.next())
	{
		mProg=rs1.getString(1);
	}
	qry="select SEMESTER,TOTALGRADEPOINTS,TOTALCOURSECREDIT, ";
	qry=qry+" TOTALEARNEDCREDIT,TOTALPOINTSECUREDSGPA,TOTALPOINTSECUREDCGPA,SGPA,CGPA FROM ";
	qry=qry+" STUDENTSGPACGPA#pub WHERE institutecode='"+mInst+"' and  STUDENTID='"+mSID+"' ";
	//out.print(qry);
	rs1=db.getRowset(qry);
...
>> Result withheld due to fee deficiency/document deficiency/other reason please contact to registry.
</font>
...
<%
	...
	<tr>
		<td><%=mProg%></td>
		<td align=center><A Href='AcademicDetSemADMN.jsp?INSCODE=<%=mInst%>&amp;SID=<%=mSID%>&Semester=<%=rs1.getInt("SEMESTER")%>&SGP=<%=rs1.getDouble("SGPA")%>&CGP=<%=rs1.getDouble("CGPA")%>'><%=rs1.getInt("SEMESTER")%></a></td>
   ...
   <%
		...
		if(flag==0)
		{
qry1="select 'Y' from HOLDRESULTLIST#PUB A where NVL(A.hold,'N')='Y' and A.INSTITUTECODE='"+mInst+"' and A.studentid='"+mChkMemID+"'     AND EXISTS ( SELECT 'Y' FROM PUBLISHRESULT#PUB B WHERE NVL(B.PUBLISH,'N')='Y' AND B.INSTITUTECODE='"+mInst+"'  AND  A.INSTITUTECODE=B.INSTITUTECODE  AND A.ACADEMICYEAR =B.ACADEMICYEAR  AND A.PROGRAMCODE=B.PROGRAMCODE  AND B.SECTIONBRANCH=A.SECTIONBRANCH  AND A.EXAMCODE=B.EXAMCODE  AND A.SEMESTER=B.SEMESTER) ";
//out.print(qry);
//out.print(qry1);
