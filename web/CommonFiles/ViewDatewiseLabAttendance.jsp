...
String mSubj="", mStatus="", mCtype="", mSID="", mSUBJID="";
String mExam="";
String mLTP="";
String mTotal="";
String mStudID="";
...
<HTML>
<head>
<TITLE>#### <%=mHead%> [Student Attendance Report (Practical Attendance Detail)]</TITLE>
...
<tr>
<TD colspan=0 align=middle ><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><b>Student Practical Attendance Detail</b></font></td>
</tr>
</TABLE>
<%
String mSnm="", mENo="";
...
<table width="100%">
<tr><td align=center>
 <font color="#00008b">Subject Code: <b><%=mSubj%></B> &nbsp; &nbsp;Student Name: <B><%=GlobalFunctions.toTtitleCase(mSnm)%> [<%=mENo%>]</B> &nbsp; &nbsp; LTP: <B><%=GlobalFunctions.getLTPDescWSQ(mLTP)%></B></font>
</td></tr></table>
...
<tbody>
<%
	qry="select to_char(ATTENDANCEDATE,'DD-MM-YYYY')ADate, nvl(ATTENDANCETYPE,'R')CType, nvl(ENTRYBYFACULTYNAME,' ')EName, nvl(PRESENT,'N')Status ,TO_CHAR (attendancedate, 'YYYYMMDDmihh') aadate from V#STUDENTATTENDANCE ";
	qry=qry+" where EXAMCODE='"+mExam+"'";
	qry=qry+" and ATTENDANCETYPE=Decode('"+mCType + "','ALL',ATTENDANCETYPE,'"+ mCType +"')";
	qry=qry+" and SUBJECTID='"+mSUBJID+"'";
	qry=qry+" and LTP='"+mLTP+"'";
	qry=qry+" and STUDENTID='"+mSID+"' and nvl(DEACTIVE,'N')='N'";
	qry=qry+" order by aadate ";
	rs=db.getRowset(qry);
//out.print(qry);
	mSNO=0;
	while(rs.next())
...
var st1 = new SortableTable(document.getElementById("table-1"),["Number","CaseInsensitiveString","CaseInsensitiveString","CaseInsensitiveString","CaseInsensitiveString","Number","Number"]);
</script>
