...
<!-- Modified Date 06-07-2020 16:35-->
...
if(session.getAttribute("PageHeading")!=null && !session.getAttribute("PageHeading").equals(""))
   ...
   mHead="JIIT ";
enc = new OLTEncryption();
            String decap=request.getSession().getAttribute("cptid").toString();
            String orgcap=enc.decrypt(decap);
            if (orgcap.equals(request.getParameter("txtcap").toString()))  {

            }else{
                errorsms="Captch Error.";
            response.sendRedirect("./captchaerror.jsp?sms="+errorsms);
            }
...
<%
String FreezeStatus="",feedBackForwardStatus="N";
ResultSet Bnkdtl=null;
ResultSet rsFeedBack=null;
String UniqueID="";
String qrybankdetails="",qryFeedBack="";
...
//System.out.println("Coming in UserAction -----------");
...
{
	mDOB=request.getParameter("DATE1").toString().trim();
}
...
//INSTITUTECODE MEMBERCODE    MEMBERTYPE     MEMBERPWD     LOGINDATE    IPADDRESS
...

	{
		...
		try
			...
			qry="INSERT INTO LOGINATTEMPT(INSTITUTECODE, MEMBERCODE, MEMBERTYPE,IPADDRESS) VALUES('"+mInst+"','"+mCode+"','"+mMemberType+"','"+ipAddress+"')";
		...
		if(rs.next())
		{
			...
			{
				qry=" Select GUESTID,companycode from GUEST where GUESTCODE='"+mCode+"'  and nvl(DEACTIVE,'N')='N'";//And CompanyCode='"+mComp2+"'
				//out.print(qry);
				rs11=db.getRowset(qry);
				if(rs11.next())
				{
			...
			else if(mMemberType.equals("S"))
			{
				...
				qry="Select a.STUDENTID,b.COMPANYTAGGING companycode from STUDENTMASTER a,institutemaster b where ENROLLMENTNO='"+mCode+"' And a.INSTITUTECODE='"+mInst+"' And a.DATEOFBIRTH= TO_DATE( '"+mDOB+"' ,'DD-MM-YYYY') and a.INSTITUTECODE=b.INSTITUTECODE and (nvl(a.PROGRAMCOMPLETED,'N')='Y' or nvl(a.DEACTIVE,'N')='N')";
				...
				if(rs11.next())
				{
					...
					session.setAttribute("securityval","10#11#2017#");
                                    ...
                                    errorsms="invalid Password or Date or Birth Please check ";
                                response.sendRedirect("./captchaerror.jsp?sms="+errorsms);
                                }
			}
			else
          ...
          //out.println("BLOCK STUDENTDDDDD ELSEE");
			...
			mpass=enc.encode(mpass);
                     ...
                     //  System.out.println("Password is -----------------" +pass + "FACULTYID CODE---" + mCode +"membertype is---" + mMemberType+ "company code is--" + mComp2);
			...
			qry="select to_char(sysdate,'DYDDMMYYYY')ss from dual";
			rs=db.getRowset(qry);
			if(rs.next())
				{

				mBASEADMINPWD=enc.encode( mInst+"_"+rs.getString("ss"));
				...
				if(mMemberType.equals("S"))
					{
						...
						//	session.setAttribute("InstituteCode",mInst);
							...
							session.setAttribute("NickName",mNickname);
							 ...
							 // System.out.println("qrybankdetails--"+qrybankdetails);
							  //out.print("qrybankdetails--"+qrybankdetails);
                                                Bnkdtl=db.getRowset(qrybankdetails);


                                               // if(mAcadmeicYear.equals("1617")&&mProgram.equals("B.T"))
                                               // {
                                                qryFeedBack="select 'Y' ABC from V#SRSEVENTS a,  srseventmaster b, studentltpdetail t where sysdate between a.eventfrom and a.eventTo and b.institutecode = '"+mInst+"' and a.srseventcode = b.srseventcode and nvl(b.srscompleted,'N') <>'Y' and t.fstid = a.fstid and T.STUDENTID = '"+mDID+"' and not exists(select 'Y' from SRSEVENTdetail c   where  a.srseventcode = c.srseventcode and a.fstid = c.fstid   and     c.studentid = '"+mDID+"' and     c.employeeid = a.employeeid ) and rownum = 1";
												//System.out.println("qryFeedBack--"+qryFeedBack);
												//out.println("qryFeedBack--"+qryFeedBack);
                                                rsFeedBack=db.getRowset(qryFeedBack);
                                                if(rsFeedBack.next())
                                                {
                                                    feedBackForwardStatus=rsFeedBack.getString("ABC").toString();
                                                }
                                                else
                                                {
                                                    feedBackForwardStatus="N";
                                                }
                                               // }
                                                ...
                                                // System.out.println("Freeze status"+FreezeStatus);
...
// null
                                                    ...
                                                    else if(feedBackForwardStatus.equals("Y"))
                                                    {
														//System.out.println("33333333");

                                                           response.sendRedirect("../StudentFiles/SRS/StudNewSrsEntry.jsp");
