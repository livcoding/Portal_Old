...
<MARQUEE id=MARQUEE1  behavior=scroll height='500px' scrolldelay=250 LANGUAGE=javascript onfinish="return MARQUEE1_onfinish()" direction=up loop=1> 
	...
	<%
	/*
	qry="Select RANKNO, ROLLNO, NAME, CATEGORYCODE, PROGRAMCODE, BRANCHALLOTED, COUNSELLINGDONE, NRICATEGORY ";
	qry=qry+" from C#StudentMaster where INSTITUTECODE='"+mInst+"' and COUNSELLINGID ='"+mCYear+"' And CategoryCode in ("+mCat+") ";
	...
	*/
qry="Select RANKNO, ROLLNO, NAME,CATEGORYCODE,PROGRAMCODE, BRANCHALLOTED, COUNSELLINGDONE, NRICATEGORY ";
	qry=qry+" from C#StudentMaster where INSTITUTECODE='"+mInst+"' and COUNSELLINGID ='"+mCYear+"' And NRICategory in ("+mCat+") ";
	qry=qry+" And nvl(DEACTIVE,'N')='N' And PROGRAMTYPECODE='"+mPType+"' And trunc(COUNSELLINGDATE) Between To_Date('"+mDt1+"','DD-MM-YYYY') And To_Date('"+mDt2+"','DD-MM-YYYY')";
	qry=qry+" And nvl(COUNSELLINGDONE,'N')='Y' and nvl(defaulter,'N')='N' ";

	qry=qry+" Order by PROGRAMCODE,BRANCHALLOTED,CATEGORYCODE,to_number(RANKNO),COUNSELLINGDATE";
	...
	while (rs.next())
	{
		...
		if(mPbranch.equals(""))
		{
			...
			*/
qry="SELECT nvl(B.CATEGORYDESC,b.CATEGORYCODE)||' ('||b.CATEGORYCODE||' )' cat from C#CATEGORYMASTER B Where B.CATEGORYCODE='"+rs.getString("NRICATEGORY")+"'";
		...
		mPCat=rs.getString("NRICATEGORY");

		}

	//	else if(!mPbranch.equals(rs.getString("PROGRAMCODE")+rs.getString("BRANCHALLOTED"))||!mPCat.equals(rs.getString("CATEGORYCODE")))
	else if(!mPbranch.equals(rs.getString("PROGRAMCODE")+rs.getString("BRANCHALLOTED"))||!mPCat.equals(rs.getString("NRICATEGORY")))
		...
		sno++;
			%> 
				<tr>
					...
					<td width='21%'><b><%=rs.getString("PROGRAMCODE")%>/<%=rs.getString("BRANCHALLOTED")%> - <%=rs.getString("NRICATEGORY")%></b></td>
