...
<HTML>
<head>
<TITLE>#### <%=mHead%> [ View Faculty wise Class Attendance ] </TITLE>
...
<tr><TD align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><b>Faculty wise All Class Attendance</b><font size=3></font</font></td></tr>
</table>
...
<%
  qry="select distinct nvl(entrybyfacultyid,' ')Faculty, nvl(ENTRYBYFACULTYNAME,' ')FacultyName from v#studentattendance where nvl(deactive,'N')='N'";
  qry=qry+" and ExamCode='"+QryExam+"' order by FacultyName asc";
//out.print(qry);
  rs=db.getRowset(qry);
	%>
...
<tbody>
<%
	qry="select distinct nvl(FSTID,' ')FSTID, nvl(ENTRYBYFACULTYID,' ')EmpID, nvl(ENTRYBYFACULTYNAME,' ')AttendanceBy, nvl(SUBJECT,' ')Subject, ";
	...
	%>
	<td align=center><a Title="View Date wise Attendance History" target=_New href='ViewDatewiseStudAttendanceALL.jsp?EXAM=<%=QryExam%>&amp;FAC=<%=mEmpID%>&amp;TYPE=<%=QryType%>&amp;SID=<%=mSUBJECTID%>&amp;SC=<%=mSUBJECTCODE%>&amp;LTP=<%=mLTP%>&amp;PROG=<%=mProg%>&amp;SEC=<%=mSec%>&amp;SUB=<%=mSub%>&amp;AttDate=<%=mAttDate%>&amp;Date1=<%=mDate1%>&amp;Date2=<%=mDate2%>&amp;TotClass=<%=mTotClass%>'><%=mTotClass%></a></td>
	<%
