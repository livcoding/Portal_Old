...
<td>
 ...
 <b>No. of Class in a Week : </b><input readonly type=text name=Class1 id=Class1 value='<%=mlt%>' maxlength=3 size=4>
	...
	<td align=middle><b><font color=white>Faculty Having Choice</font><b></td>
	<td align=middle><b><font color=white>Merge with</font><b></td>
	<td align=middle><b><font color=white>Faculty-No Choice</font><b></td>
	<td align=middle><b><font color=white>Merge with</font><b></td>
...
qry1=qry1+" From Pr#StudentSubjectChoice a Where SUBJECTTYPE='"+mType+"' AND  ELECTIVECODE='"+mEle+"'  and EXAMCODE='"+mExamcode+"' ";
...
qry1="Select Subjectid, Count(Subjectid) cnt From (Select StudentID, Subjectid ";
qry1=qry1+" From Pr#StudentSubjectChoice a Where SUBJECTTYPE='"+mType+"' AND   EXAMCODE='"+mExamcode+"' ";
qry1=qry1+" and SECTIONBRANCH='"+mSection+"' and PROGRAMCODE='"+rs1.getString("PROGRAMCODE")+"' and Choice = (Select Min(Choice) From PR#StudentSubjectChoice ";
qry1=qry1+"	Where Subjectid IN("+SC+")	And StudentID = a.StudentID    ) ) ";
qry1=qry1+" Group By Subjectid Having Subjectid in('"+mSubj+"') Order By Subjectid ";
rsc=db.getRowset(qry1);
...
*/
	...
	<%
		...
		rs=db.getRowset(qry);
	 	...
	 	%>
		   <select name="<%=mName1%>" id=<%=mName1%> tabindex="0" style="WIDTH: 200px"  onChange="ChangeOptions1('<%=mName1%>','<%=mNameLML%>',<%=mlt1%>,<%=mlt%>);">
		<%	
		if (request.getParameter("x")==null) 
		{
		if(moldemp1.equals(""))
...
//	mNameLML=mSection+"***L///"+String.valueOf(ctr).trim()+"###"+mSeccount+"merge";  
	%>
	<select name='<%=mNameLML%>' id='<%=mNameLML%>' tabindex="0" style="WIDTH: 135px" onChange="ChangeOptions3('<%=mName1%>','<%=mNameLML%>');">
	<%
		if(moldMerge.equals(""))
		{
	%>
...
// mvalue=rs1.getString("SECTIONBRANCH")+"***"+rs1.getString("subsectioncode")+"///"+mChkFac[jp]+"###"+rs1.getString("PROGRAMCODE");
	...
	*/
	...
	%>
	<select name="<%=mName1%>" id=<%=mName1%> tabindex="0" style="WIDTH: 200px"  onChange="ChangeOptions2('<%=mName1%>','<%=mNameLMR%>',<%=mlt1%>,<%=mlt%>);">
	<%
	if (request.getParameter("x")==null) 
	{
		if(moldemp1.equals(""))
...
%>
	...
	//   mNameLMR=mSection+"***R///"+String.valueOf(ctr).trim()+"###"+mSeccount+"merge";    
	%>
	<select name='<%=mNameLMR%>' id='<%=mNameLMR%>' tabindex="0" style="WIDTH: 135px" onChange="ChangeOptions4('<%=mName1%>','<%=mNameLMR%>');">
	<%
		if(moldMerge.equals(""))
		{
	%>
...
// } closing of else  111
	...
	%>
	<TR>
	<TD ALIGN=CENTER COLSPAN=6><table width=100%><tr><hr><td align=center><input type=submit name=btn id=btn value='Save'></td></tr></table></TD>
	</TR>
	<%
	}
%>
