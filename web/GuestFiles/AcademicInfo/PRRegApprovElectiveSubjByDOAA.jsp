		  ...
		  //----------------------
			...
			{		
				...
				if (request.getParameter("InstCode")==null)
				  ...
				  if (mST.equals("E"))
					{
qry = "select DEPARTMENTCODE,decode(nvl(T.APPROVED,'N'),'N','No','Y','Yes','Unk') LastStatus , V.EmployeeID,V.EMPLOYEENAME From V#STAFFSTUDENT V, PREVENTS T";
qry = qry+" Where decode(T.MEMBERTYPE,'E','I','S','S','E')=V.EMPLOYEETYPE and T.MEMBERID=V.EMPLOYEEID ";
...
qry = qry+" Where H.INSTITUTECODE='"+mInstitute+"' and nvl(H.DEACTIVE,'N')='N') ";
						...
						qry = qry+" And nvl(M.PRREQUIREDFOR,'N') <>'S' and nvl(M.DEACTIVE,'N')='N')  order by DEPARTMENTCODE ";
						...
						%>
							<td><%=jk%></td>
							<td><%=rs.getString("DEPARTMENTCODE")%></td>
