			 ...
			 mPrcode=rs.getString("PREVENTCODE");
				...
				<tr bgcolor='#e68a06'>
				 ...
				 <%
					...
					qry="select SUBJECTID SUBJECTID,FACULTYID FACULTYID from PR#FACULTYSUBJECTCHOICES where ";
					qry=qry+" INSTITUTECODE='"+mInstitute+"' and EXAMCODE='"+mE+"' and ";
					qry=qry+" FACULTYTYPE='"+mfactype+"' and choice='"+i+"' and FACULTYID='"+mDMemberID+"' and LTP='L' ";
					...
					else
					{
					  moldsubjectid=""; 			
						...
						qry="select Distinct 'C' C,'Core' code, A.Subject SUBJECT, B.SubjectID SC,A.Subjectcode SC1, nvl(B.L,0) L, nvl(B.T,0) T, nvl(B.p,0) P from PROGRAMSCHEME B, SUBJECTMASTER A";
						qry=qry+ " Where A.SUBJECTID=B.SUBJECTID and B.INSTITUTECODE='"+mInstitute+"' and ";
						...
						qry=qry+" and (nvl(B.L,0)>0 or nvl(B.T,0)>0 or nvl(B.p,0)>0 )  ";
						qry=qry+" union ";
						qry=qry+" select Distinct 'E' C,nvl(B.electivecode,' ')code, A.Subject SUBJECT , B.SubjectID SC,A.Subjectcode SC1, nvl(B.L,0) L, nvl(B.T,0) T, nvl(B.p,0) P from SUBJECTMASTER A, ";
						...
						qry=qry+" and (nvl(B.L,0)>0 or nvl(B.T,0)>0 or nvl(B.p,0)>0 )  ";
						qry=qry+" union ";	
						qry=qry+" select Distinct 'F' C,'Free' code, A.Subject SUBJECT , B.SubjectID SC,A.Subjectcode SC1, nvl(B.L,0) L, nvl(B.T,0) T, nvl(B.p,0) P from SUBJECTMASTER A, ";
						qry=qry+" FREEELECTIVE B where A.INSTITUTECODE=B.INSTITUTECODE ";
						qry=qry+" And B.INSTITUTECODE='"+mInstitute+"' AND B.SUBJECTRUNNING='Y' AND A.SUBJECTID=B.SUBJECTID and EXAMCODE='"+mE+"'";
						qry=qry+ " And (B.ACADEMICYEAR,B.PROGRAMCODE,B.TAGGINGFOR, B.SECTIONBRANCH,B.SUBJECTID)in (Select PR.ACADEMICYEAR,PR.PROGRAMCODE,PR.TAGGINGFOR, PR.SECTIONBRANCH, PR.SUBJECTID from PR#DEPARTMENTSUBJECTTAGGING PR  where Pr.INSTITUTECODE='"+mInstitute+"' And Pr.EXAMCODE='"+mE+"' And Pr.DEPARTMENTCODE =(select departmentcode from v#Staff where EmployeeID='"+mDMemberID+"'))";
						qry=qry+" and nvl(A.DEACTIVE,'N')='N' and nvl(B.DEACTIVE,'N')='N' ";  
						qry=qry+" and (nvl(B.L,0)>0 or nvl(B.T,0)>0 or nvl(B.p,0)>0 ) Order by Subject  ";
						rs=db.getRowset(qry);	
