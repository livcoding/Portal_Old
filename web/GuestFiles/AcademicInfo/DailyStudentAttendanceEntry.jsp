...
String mDFrom="", mTFrom="";
String mDUpto="", mTUpto="";
...
<table align=center><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><B>Daily Class Attendance Entry [Today's Attendance or Previous Day's Excuse]</b></font></TABLE>
<table id=id2 cellpadding=1 cellspacing=0 width="90%" align=center rules=groups border=2>
<!--Institute****-->
...
<TR>
<TD width=50% nowrap>
<marquee behavior=alternate scrolldelay=200 direction=right><a href='DailyStudentAttendanceEdit.jsp'><font color=blue><b>Edit/Change Previous Day Attendance</b></font></a></marquee>
</TD>
<TD width=50% nowrap>
...
<table bgcolor=#fce9c5 class="sort-table" id="table-1" width='90%' bottommargin=0 rules=groups topmargin=0 cellspacing=0 cellpadding=0 border=1 align=center>
<thead>
<tr bgcolor="#ff8c00">
	...
	if(mDiffInDate>=0)
	{
		...
		{	
			...
			if (RsChk1.next() && RsChk1.getString("SL").equals("Y") && RsChk1.getString("SL1").equals("Y"))
			{
				...
				mTFrom= request.getParameter("timepicker1").toString().trim().toUpperCase();
				mTUpto= request.getParameter("timepicker2").toString().trim().toUpperCase();
				//out.print(mTFrom+" "+mTUpto);
				mDFrom= mDate+" "+request.getParameter("timepicker1").toString().trim().toUpperCase();
				mDUpto= mDate+" "+request.getParameter("timepicker2").toString().trim().toUpperCase();

				qry=" SELECT TO_CHAR(TO_DATE('"+mDUpto+"','DD-MM-YYYY HH:MI PM'),'YYYYMMDDHH24MI') ";
				qry=qry+" - TO_CHAR(TO_DATE('"+mDFrom+"','DD-MM-YYYY HH:MI PM'),'YYYYMMDDHH24MI') ";
				qry=qry+" from dual ";	
...
*/
					qry=qry+" And ( to_date('"+mDFrom+"','dd-mm-yyyy hh:mi PM') between CLASSTIMEFROM and CLASSTIMEUPTO ";
					qry=qry+" AND  to_date('"+mDUpto+"','dd-mm-yyyy hh:mi PM') between CLASSTIMEFROM and CLASSTIMEUPTO))";
	...
	<input type=hidden name=Timefrom ID=Timefrom value='<%=mDFrom%>'>
	<input type=hidden name=Timeupto ID=Timeupto value='<%=mDUpto%>'>
	<input type=hidden name=TimeFr ID=TimeFr value='<%=mTFrom%>'>
	<input type=hidden name=TimeTo ID=TimeTo value='<%=mTUpto%>'>
	</tbody>
	</table>
