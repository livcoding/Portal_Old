...
try
	...
	*/
	...
	qry1="SELECT nvl(B.EMPLOYEENAME,' ')EMPLOYEENAME,nvl(B.EMPLOYEECODE,' ')EMPLOYEECODE,B.SUBJECT SUBJECT,B.SECTIONBRANCH SECTIONBRANCH,B.SUBSECTIONCODE SUBSECTIONCODE FROM  V#EXAMEVENTSUBJECTTAGGING B WHERE B.institutecode='"+mIC+"' AND B.examcode='"+mEC+"' and B.EVENTSUBEVENT='"+mEvent+"' AND nvl(B.PROCEEDSECOND,'N')='N' and nvl(B.DEACTIVE,'N')='N' and nvl(B.locked,'N')='N' AND B.SUBJECTID='"+mSC+"' AND B.CORDFACULTYID='"+mDMemberID+"' AND (B.ltp='L' OR B.PROJECTSUBJECT='N') AND  EXISTS (SELECT FSTID FROM V#STUDENTEVENTSUBJECTMARKS A WHERE A.EVENTSUBEVENT=B.EVENTSUBEVENT AND a.fstid=b.fstid AND A.STUDENTID=B.STUDENTID AND A.EXAMCODE=B.EXAMCODE and nvl(A.LOCKED,'N')='N' AND (A.LTP='L' OR A.PROJECTSUBJECT='N') AND A.PUBLISHED='N') GROUP BY B.EMPLOYEENAME,B.EMPLOYEECODE,B.SECTIONBRANCH,B.SUBSECTIONCODE,B.SUBJECT ";
   // out.println(qry1);
