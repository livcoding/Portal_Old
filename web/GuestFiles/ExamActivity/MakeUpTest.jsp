...
<%  
	try
	{
		...
		qry="select distinct nvl(EXAMID,' ') GRADEENTRYEXAMID  from DEFAULTVALUES";
		rs=db.getRowset(qry);
...
try
{  
	qry="select distinct fstid,SUBJECTCODE SUBCODE,subjectID,detained2,maxmarks,sectionbranch,subsectioncode,employeename,nvl(studentid,' ')studentid,nvl(studentname,' ')studentname, ";
	...
	qry=qry+"and  examcode='"+mEC+"' and subjectID=decode('"+mSC+"','ALL',subjectID,'"+mSC+"') and EVENTSUBEVENT='"+mEvent+"' and (ltp='L' OR PROJECTSUBJECT='Y' ) and nvl(PUBLISHED,'N')='Y' ";
	qry=qry+" and (nvl(Detained,'N')='D' or nvl(Detained,'N')='A' or nvl(Detained,'N')='M') "; 		
