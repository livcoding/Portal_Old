...
%>
	...
	<tr><TD align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><b>Ongoing performance record of Students</b><font size=3></font</font></td></tr>
	</table>
...
<%  
	...
	if(mType.equals("H"))
	{
	 qry="select distinct examcode GRADEENTRYEXAMID  from V#EXAMEVENTSUBJECTTAGGING where ";
	 qry=qry+" InstituteCode='"+mInst+"' And (fstid) in (select "; 
	 qry=qry+" fstid from  facultysubjecttagging  ";
	 qry=qry+" where facultytype=decode('"+mDMemberType+"','E','I','E') and employeeid in (select employeeid from employeemaster where departmentcode='"+mDept+"' and nvl(deactive,'N')='N') and nvl(deactive,'N')='N' AND (LTP='L' or PROJECTSUBJECT='Y')) ";  
...
if(mType.equals("H"))
	...
	{
		%>
		<select name=DataComboSubj tabindex="0" id="DataComboSubj" style="WIDTH: 0px">	
		<%   
		while(rssd.next())
		{
...
&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <b>Course No.</b>&nbsp;&nbsp;<%=mname%>
&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <br><b>L :</b><%=mL%>&nbsp;&nbsp;<b>T :</b><%=mT%>&nbsp;&nbsp;<b>P :</b><%=mP%>&nbsp;&nbsp;<b>Cr :</b><%=mCredit%>
...
<!-- <b>Year :</b>&nbsp;&nbsp;<%=macad%> -->
 <b>Year :</b>&nbsp;&nbsp;<%=mEC%>
	...
	<input id="x" name="x" type=hidden>
		...
		<%	
		qry="select EVENTSUBEVENT ,WEIGHTAGE from ( ";
		qry=qry+" select distinct EVENTSUBEVENT ,WEIGHTAGE,FROMDATE from V#EXAMEVENTSUBJECTTAGGING ";
		...
		qry=qry+" where institutecode='"+mInst+"' and SEMESTERTYPE=decode('"+mSems+"','ALL',SEMESTERTYPE,'"+mSems+"') and examcode='"+mEC+"' and employeeid='"+mFEmp+"'  ";
		qry=qry+" and facultytype=decode('"+mDMemberType+"','E','I','E') and subjectID='"+mSC+"')";
		qry=qry+" ORDER BY FROMDATE) GROUP BY EVENTSUBEVENT ,WEIGHTAGE ";
...
<br>
Mark the Makeup Test with red ink.
<%
