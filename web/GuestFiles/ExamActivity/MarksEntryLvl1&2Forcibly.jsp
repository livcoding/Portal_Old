...
<html>
<head>
<TITLE>#### <%=mHead%> [ Forcibly Marks Entry I & II ] </TITLE>
...
//		if(oldm<=mMax1)
		{
		var mChoice=confirm('In Level-II entry Student has been '+print+'! Do You want to remove Old '+print+' Flag? with New Marks : '+ newm +' ?');
		if(mChoice)
			{
...
%>
  ...
  <font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy">Forcibly Marks Entry I & II<br>(After completing Ist and 2nd level marks entry by the concerened faculty/co-ordinator.)</TD></font></td></tr></TABLE>
...
<%    try{
	...
	qry=qry+" and fstid in (select fstid from studenteventsubjectmarks b where ";
...
<%
	...
	qry=qry+"  nvl(locked,'N')='Y' and a.eventsubevent=b.eventsubevent ) and nvl(deactive,'N')='N' "; 
	qry=qry+" and nvl(locked,'N')='N' AND ExamCODE='"+mExamid+"'";
	qry=qry+" and (ltp='L' OR PROJECTSUBJECT='Y') and nvl(PROCEEDSECOND,'N')='Y' GROUP BY subject||' ( '||subjectcode||' )',subjectID"; 
...
<INPUT Type="submit" tabindex="7" Value="Submit">&nbsp;&nbsp;
	...
	<%	
		...
		qry=qry+" And EVENTSUBEVENT='"+mEvent+"' and CORDFACULTYTYPE=decode('"+mDMemberType+"','E','I','E') and (ltp='L' OR PROJECTSUBJECT='Y') and subjectID='"+mSC+"' ";
...
try
	...
	*/
			...
			qry=qry+" where institutecode='"+mIC+"' and nvl(PROCEEDSECOND,'N')='Y' and nvl(locked,'N')='N' and ";
			qry=qry+" EVENTSUBEVENT='"+mEvent+"' and nvl(a.DEACTIVE,'N')='N' and  ";
			qry=qry+" Examcode='"+mEC+"' and CORDFACULTYID='"+mDMemberID+"' and CORDFACULTYTYPE=decode('"+mDMemberType+"','E','I','E') and (ltp='L' OR PROJECTSUBJECT='Y') and subjectID='"+mSC+"' and ";
			qry=qry+" Exists (select studentid from V#STUDENTEVENTSUBJECTMARKS b where b.fstid=a.fstid and b.EVENTSUBEVENT=a.EVENTSUBEVENT and a.studentid=b.studentid and nvl(b.LOCKED,'N')='Y' and (b.LTP='L' OR PROJECTSUBJECT='Y')) ";
			qry=qry+" GROUP BY fstid,nvl(studentid,' '),nvl(studentname,' '), ";
			qry=qry+" nvl(enrollmentno,' '),nvl(semester,0),subsectioncode, ";
			qry=qry+" nvl(programcode,' '),nvl(SECTIONBRANCH,' ') ";
			qry=qry+" order by "+mList+ " "+mOrder+ " ,enrollmentno " ;
			...
			while(rs1.next())
			{
				...
				{	
					...
					if ((rs3.getString("OLDDETAINED2")).equals("D")||(rs3.getString("OLDDETAINED2")).equals("A"))
					{
						//**************************************************************			
				    	if(mStatus.equals("B"))
						{
						    if((rs3.getString("OLDDETAINED2")).equals("D"))
						    {
								%>
								<td align=center>
							...
							else if((rs3.getString("OLDDETAINED2")).equals("A"))
							{
								%>
								<td align=center>
								<select name='<%=mName4%>' id='<%=mName4%>' onchange="return detained_check(<%=mName4%>,<%=mName3%>,<%=mName7%>,<%=mName8%>,'<%=mStatus%>');" onclick="return detained_check(<%=mName4%>,<%=mName3%>,<%=mName7%>,<%=mName8%>,'<%=mStatus%>');">
						...
						{
							if((rs3.getString("OLDDETAINED2")).equals("A"))
							{
								%><td align=center>
								<select name='<%=mName4%>' id='<%=mName4%>' onchange="return detained_check(<%=mName4%>,<%=mName3%>,<%=mName7%>,<%=mName8%>,'<%=mStatus%>');" onclick="return detained_check(<%=mName4%>,<%=mName3%>,<%=mName7%>,<%=mName8%>,'<%=mStatus%>');">
								<option  value="N">NA</option>
						...
						<%
				}//end of rs3.getString("OLDDETAINED2");
				else
				{		
					if(mMOP.equals("P"))
						mvalue=gb.getRound((rs3.getDouble("OLDMARKSAWARDED2")*100)/mMaxmarks,2);
					else
						mvalue=rs3.getDouble("OLDMARKSAWARDED2");
					if(mvalue<0)
						mmvalue="";
					else
				...
				mchkmarks=gb.getRound((rs3.getDouble("OLDMARKSAWARDED2")*100)/mMaxmarks,2);
				else
					mchkmarks=rs3.getDouble("OLDMARKSAWARDED2");
				%>	
				...
				<input type=hidden name='<%=mName7%>' id='<%=mName7%>' value=<%=rs3.getString("OLDDETAINED2")%>>
		...
		<input type="submit" tabindex="<%=tabctrtxt%>" Value="Save Forcibly Marks Entry I & II"></td></tr>
		<%
		}//end of rs1.next()
		else
		{
