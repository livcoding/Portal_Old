...
<%  
	try
	{
		qry="select distinct nvl(ExamCode,' ') GRADEENTRYEXAMID,examperiodfrom FROM ExamMaster A where InstituteCode='"+mInst+"' And nvl(Deactive,'N')='N' and Exists (Select ExamCode From STUDENTRESULTDETAIL#Pub B Where B.ExamCode=A.ExamCode and RowNum<=1 And InstituteCode='"+mInst+"') order by examperiodfrom  desc "; 
		//out.print(qry);
		rs=db.getRowset(qry);
...
<%
	...
	qry=qry+" where SubjectID in (select SUBJECTID From STUDENTRESULT#Pub where InstituteCode='"+mInst+"' GROUP BY SUBJECTID)";  
