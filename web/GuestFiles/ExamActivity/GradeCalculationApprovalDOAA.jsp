...
*/
		...
		if (RsChk.next() && RsChk.getString("SL").equals("Y"))
		  ...
		  //----------------------
				...
				qry=qry+" WHERE A.INSTITUTECODE='"+mInst+"' and A.INSTITUTECODE=B.INSTITUTECODE and A.EXAMCODE=B.EXAMCODE and nvl(A.STATUS,'N')='D' order by EXAMPERIODFROM DESC";
				//out.print(qry);
				rs=db.getRowset(qry);
				%>
...
<!--
 ...
 <input type=RADIO name=ETOD id=ETOD VALUE="E" ><b>EtoD</b> -->
				...
				*/
				...
				qry=qry+" Where A.INSTITUTECODE=B.INSTITUTECODE AND  A.SUBJECTID=B.SUBJECTID AND A.EXAMCODE='"+QryExam+"' AND A.STATUS=DECODE('"+QryStatus+"','ALL',A.STATUS,'"+QryStatus+"')";
				...
				while(rss.next())
				{
					...
					<td nowrap><%=GlobalFunctions.toTtitleCase(mSubjName)%> [<%=rss.getString("SUBJCODE")%>]</td>
