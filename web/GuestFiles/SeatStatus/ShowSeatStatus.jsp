...
qry="SELECT nvl(COUNSELLINGNO,1)CNO FROM C#CURRENT WHERE COUNSELLINGID='"+mCYear+"'";
//out.print(qry);
ResultSet rsCNo=db.getRowset(qry);
if(rsCNo.next())
{
	mCNO=rsCNo.getString("CNO");
}
...
if (session.getAttribute("mCatArray")!=null)
	...
	Nqry=Nqry + " And A.PROGRAMTYPE='"+mProgType+"' And A.LOCATIONCODE='"+mLocation+"' And Nvl(A.FIRSTATTENDANCE,'N')='Y'";
	Nqry=Nqry + " And exists (select 1 from C#CATEGORYAPPLIEDFOR B where B.INSTITUTECODE in ("+mInst+") And B.COUNSELLINGID='"+mCYear +"' And B.PROGRAMTYPE='"+ mProgType +"' And B.RANK=A.RANK And B.CHOICENO=1 And B.CATEGORY in ("+mcattt+") And nvl(B.DEACTIVE,'N')='N') ";
//----Added By Vijay----
	Nqry=Nqry + " and exists (select null from c#Studentmaster C where C.CounsellingID=A.CounsellingID and C.Rank=A.Rank and nvl(C.Deactive,'N')='N')";
	Nqry=Nqry + " and exists (select null from c#AllocationDetail D where D.CounsellingID=A.CounsellingID and D.Rank=A.Rank and D.CounsellingNo=(Select CounsellingNo from c#Current where rownum=1) and nvl(Defaulter,'N')='N')";
//----------------------
	//out.print(Nqry);	
	if(mAcumulated.equals("N"))
	{
