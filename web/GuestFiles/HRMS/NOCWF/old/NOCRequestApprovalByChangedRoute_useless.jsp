...
//---------------Start Of Approval Authority for if(rsFrIndv.next())----
				...
				while(rs.next())
				{
					...
					{
						%>
						<a target=_new href="AddWorkFlowLevel.jsp?EID=<%=QryFaculty%>&amp;DeptCode=<%=mSelfDeptCode%>&amp;WFC=<%=QryWFCode%>&amp;WFT=<%=QryWFType%>&amp;WFL=<%=CTR%>&amp;RID=<%=QryRID%>&amp;TLD=<%=mTotalLvDays%>&amp;DATEFROM=<%=QryLvDateFr%>&amp;DATETO=<%=QryLvDateTo%>" title="Click to add more Approval Level"><marquee behavior=alternate scrolldelay=100 width=300px% style="cursor:hand"><font color="blue" face=arial size=2><STRONG>Next Approval Authority Required ?</STRONG></font></marquee></a>
						<%
					}
					else
					{
...
//---------------Start Of Approval Authority for if(rsFrEmp.next())----
				...
				while(rs.next())
				{
					...
					{
						%>
						<a target=_new href="AddWorkFlowLevel.jsp?EID=<%=QryFaculty%>&amp;DeptCode=<%=mDeptCode%>&amp;WFC=<%=QryWFCode%>&amp;WFT=<%=QryWFType%>&amp;WFL=<%=CTR%>&amp;RID=<%=QryRID%>&amp;TLD=<%=mTotalLvDays%>&amp;DATEFROM=<%=QryLvDateFr%>&amp;DATETO=<%=QryLvDateTo%>" title="Click to add more Approval Level"><marquee behavior=alternate scrolldelay=100 width=300px% style="cursor:hand"><font color="blue" face=arial size=2><STRONG>Next Approval Authority Required ?</STRONG></font></marquee></a>
						<%
					}
					else
					{
...
//---------------Start Of Approval Authority for if(rsFrDept.next())----
				...
				while(rs.next())
				{
					...
					if(mTotOnWFLevel>1)
					{
						...
						qry=qry+" AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"' and WFLEVEL='"+CTR+"' AND "+mTotalLvDays+" between FROMVALUE and TOVALUE";
					    	rs2=db.getRowset(qry);
						//out.print(qry);
						if(rs2.next())
						{
					...
					{
						%>
						<a target=_new href="AddWorkFlowLevel.jsp?DeptCode=<%=mSelfDeptCode%>&amp;WFC=<%=QryWFCode%>&amp;WFT=<%=QryWFType%>&amp;WFL=<%=CTR%>&amp;RID=<%=QryRID%>&amp;TLD=<%=mTotalLvDays%>&amp;DATEFROM=<%=QryLvDateFr%>&amp;DATETO=<%=QryLvDateTo%>" title="Click to add more Approval Level"><marquee behavior=alternate scrolldelay=100 width=300px% style="cursor:hand"><font color="blue" face=arial size=2><STRONG>Next Approval Authority Required ?</STRONG></font></marquee></a>
						<%
					}
					else
					{
...
//-------------------------------------------------------
			...
			{
				RequestDispatcher rd=request.getRequestDispatcher("LeaveRequestApprovalByChangedRouteEmpwise.jsp");
...
//-------------------------------------------------------
			...
			{
				RequestDispatcher rd=request.getRequestDispatcher("LeaveRequestApprovalByChangedRouteDeptwise.jsp");
				request.setAttribute("message","Msg1");
				rd.forward(request,response);
			}
	//-----------------------------
