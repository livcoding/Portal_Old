...
<%
	...
	mWFSeqc++;
	qry="Select 'Y' from WF#EMPWISEWORKFLOWAUTHORITY where COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"' AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND EMPLOYEEID='"+QryFaculty+"' AND FACULTYTYPE='"+mFacType+"' AND WFLEVEL='"+QryWFLevel+"' AND nvl(DEACTIVE,'N')='N'";
...
*/
		...
		if(mChkLevel.equals("Required"))
		{
		   if(!mApprAuth.equals(""))
		   {
			qry="Select 'Y' from WF#EMPWISEWORKFLOWAUTHORITY WHERE COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"' AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND EMPLOYEEID='"+QryFaculty+"' AND FACULTYTYPE='"+mFacType+"' ";
		//	qry=qry+" and ((FROMVALUE between '"+mCBFR+"' AND '"+mCBFR+"') OR (TOVALUE between '"+mCBFR+"' AND '"+mCBFR+"'))";
			...
			if(!rs.next())
			{
				...
				qry=qry+"VALUES('"+mComp+"','"+mInstitute+"','"+QryWFCode+"','"+QryWFType+"','"+QryFaculty+"','"+mFacType+"','"+mWFSeqc+"','"+mApprBy+"','"+mApprAuth+"','"+QryWFLevel+"','"+mChkMemID+"',sysdate, '"+mTextToDisp+"')";
				n=db.insertRow(qry);
				//out.print(qry);
				if(n>0)
				{
			...
			else
			{
				%><CENTER><br><%
				out.print("<img src='../../../Images/Error1.gif'>");
				out.print("&nbsp;&nbsp;&nbsp <b><font size=3 face='Arial' color='Red'>NOC Level Value are already exists!</font> <br>");
				%></CENTER><%
			}
		   }
		}
