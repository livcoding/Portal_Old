...
int mLevelEdit=0, QryLevelEdit=0;
...
String QryChDvFor="";
String mName1="",mName2="",mName3="",mName4="",mName5="",mName6="",mName7="",mName8="",mName9="", mName10="", mName11="",  mName12="",mName13="", mName14="";
String mTextToFwd="";
...
function FunSubmit1()
{
	document.frm.action="ApprovalforNOC.jsp" ;
}
</SCRIPT>
...
//-------------------------------------------------------
			...
			{
		%>
			<!--<form name="frm" method="POST" ACTION="NOCRequestApprovalToChangedRouteActionForIndvReq.jsp"> -->
			<form name="frm" method="POST" ACTION="LeaveRequestApprovalByChangedRouteActionForIndvReq.jsp"> 
...
//---------------Start Of if(rsFrIndv.next())----
			...
			qry=qry+" MINUS ";
			qry=qry+" Select nvl(WFLEVEL,0)WFL from WF#WORKFLOWDETAIL Where REQUESTID='"+QryRID+"' AND CompanyCode='"+mComp+"' and InstituteCode='"+mInstitute+"' and WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"' AND REQUESTID='"+QryRID+"' GROUP BY WFLEVEL)";
...
//---------------Start Of Approval Authority for if(rsFrIndv.next())----
				...
				qry=qry+" CompanyCode='"+mComp+"' and InstituteCode='"+mInstitute+"' AND DEPARTMENTCODE='"+mDeptCode+"'";
				...
				while(rs.next())
				{
					...
					qry=qry+" AND CompanyCode='"+mComp+"' and InstituteCode='"+mInstitute+"' AND DEPARTMENTCODE='"+mDeptCode+"' and WFLEVEL='"+CTR+"'";
				    rs2=db.getRowset(qry);
					//out.print(qry);
					if(rs2.next())
					{
...
<tr>
					<td nowrap width=300px><input type='checkbox' name='<%=mName12%>' id='<%=mName12%>' value="Y" onclick="FunTxtToFwd()"><font color=black face=arial size=2><STRONG>Text of NOC Recommendation</STRONG></font></td>
					<td nowrap>
					...
					{
						%>
						<a target=_new href="AddWorkFlowLevel.jsp?EID=<%=QryFaculty%>&amp;DeptCode=<%=mSelfDeptCode%>&amp;WFC=<%=QryWFCode%>&amp;WFT=<%=QryWFType%>&amp;WFL=<%=CTR%>&amp;RID=<%=QryRID%>" title="Click to add more Approval Level"><marquee behavior=alternate scrolldelay=100 width=300px% style="cursor:hand"><font color="blue" face=arial size=2><STRONG>Next Approval Authority Required ?</STRONG></font></marquee></a>
						<%
					}
					else
					{
						%>
						<input type='checkbox' name='<%=mName10%>' id='<%=mName10%>' value='Required' checked onclick="FunReqNxt('<%=ctr%>')"><font color="#000099" face=arial size=2><STRONG>Next Approval Authority Required ?</font>
						<%
					}
					%>
					</td></tr>
...
//---------------Start Of if(rsFrEmp.next())----
						qry="Select nvl(MIN(WFL),0) WFL FROM";
						qry=qry+" (Select nvl(WFLEVEL,0)WFL from WF#EMPWISEWORKFLOWAUTHORITY Where CompanyCode='"+mComp+"' and InstituteCode='"+mInstitute+"' and WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND EMPLOYEEID='"+QryFaculty+"' AND FACULTYTYPE='"+mFacType+"' GROUP BY WFLEVEL";
						qry=qry+" MINUS ";
						qry=qry+" Select nvl(WFLEVEL,0)WFL from WF#WORKFLOWDETAIL Where REQUESTID='"+QryRID+"' AND CompanyCode='"+mComp+"' and InstituteCode='"+mInstitute+"' and WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"' AND REQUESTID='"+QryRID+"' GROUP BY WFLEVEL)";
						rs1=db.getRowset(qry);
						//out.print(qry);
				...
				try
				{
					while(rs.next())
					{
						...
						if(mTotOnWFL>1)
						{
							...
							qry=qry+" CompanyCode='"+mComp+"' and InstituteCode='"+mInstitute+"' and WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"'";
...
//---------------Start Of Approval Authority for if(rsFrEmp.next())----
				...
				while(rs.next())
				{
					...
					if(mTotOnWFLevel>1)
					{
						qry="select nvl(WFSEQUENCE,0)WFSEQC from WF#EMPWISEWORKFLOWAUTHORITY where CompanyCode='"+mComp+"' and InstituteCode='"+mInstitute+"'";
						qry=qry+" AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND EMPLOYEEID='"+QryFaculty+"' AND FACULTYTYPE='"+mFacType+"' and WFLEVEL='"+CTR+"' AND "+mTotalLvDays+" between FROMVALUE and TOVALUE";
					    	rs2=db.getRowset(qry);
						//out.print(qry);
						if(rs2.next())
						{
...
<tr>
					...
					{
						%>
						<a target=_new href="AddWorkFlowLevel.jsp?EID=<%=QryFaculty%>&amp;DeptCode=<%=mDeptCode%>&amp;WFC=<%=QryWFCode%>&amp;WFT=<%=QryWFType%>&amp;WFL=<%=CTR%>&amp;RID=<%=QryRID%>" title="Click to add more Approval Level"><marquee behavior=alternate scrolldelay=100 width=300px% style="cursor:hand"><font color="blue" face=arial size=2><STRONG>Next Approval Authority Required ?</STRONG></font></marquee></a>
						<%
					}
					else
					{
...
//---------------Start Of if(rsFrDept.next())----
					...
					qry="Select nvl(MIN(WFL),0) WFL FROM";
					qry=qry+" (Select nvl(WFLEVEL,0)WFL from WF#WORKFLOWAUTHORITY Where CompanyCode='"+mComp+"' and InstituteCode='"+mInstitute+"' and WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' GROUP BY WFLEVEL";
					qry=qry+" MINUS ";
					qry=qry+" Select nvl(WFLEVEL,0)WFL from WF#WORKFLOWDETAIL Where REQUESTID='"+QryRID+"' AND CompanyCode='"+mComp+"' and InstituteCode='"+mInstitute+"' and WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"' AND REQUESTID='"+QryRID+"' GROUP BY WFLEVEL)";
					rs1=db.getRowset(qry);
					//out.print(qry);
				...
				try
				{
					while(rs.next())
					{
						...
						if(mTotOnWFL>1)
						{
							...
							qry=qry+" CompanyCode='"+mComp+"' and InstituteCode='"+mInstitute+"' and WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"' AND WFLEVEL='"+CTR+"' AND "+mTotalLvDays+" between FROMVALUE and TOVALUE";
							qry=qry+" GROUP BY APPROVALBY, APPROVALAUTHORITY, DEPARTMENTCODE, WFSEQUENCE";
						}
						else
						{
							qry="Select nvl(APPROVALBY,' ')APPRBY, nvl(APPROVALAUTHORITY,' ')APPRAUTH, nvl(DEPARTMENTCODE,' ')DEPTCODE, nvl(WFSEQUENCE,0)WFSEQ FROM WF#WORKFLOWAUTHORITY WHERE ";
							qry=qry+" CompanyCode='"+mComp+"' and InstituteCode='"+mInstitute+"' and WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"' AND WFLEVEL='"+CTR+"'";
							qry=qry+" GROUP BY APPROVALBY, APPROVALAUTHORITY, DEPARTMENTCODE, WFSEQUENCE";
						}
						rs1=db.getRowset(qry);
						//out.print(qry);
...
//---------------Start Of Approval Authority for if(rsFrDept.next())----
				...
				while(rs.next())
				{
					...
					CTR=rs.getInt("MaxLevel");
						...
						qry="select nvl(WFSEQUENCE,0)WFSEQC, nvl(DEPARTMENTCODE,' ')SelfDeptCode from WF#WORKFLOWAUTHORITY where CompanyCode='"+mComp+"' and InstituteCode='"+mInstitute+"'";
						qry=qry+" AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"' and WFLEVEL='"+CTR+"'";
					    	rs2=db.getRowset(qry);
						//out.print(qry);
						if(rs2.next())
...
<tr>
					...
					{
						%>
						<a target=_new href="AddWorkFlowLevel.jsp?DeptCode=<%=mSelfDeptCode%>&amp;WFC=<%=QryWFCode%>&amp;WFT=<%=QryWFType%>&amp;WFL=<%=CTR%>&amp;RID=<%=QryRID%>" title="Click to add more Approval Level"><marquee behavior=alternate scrolldelay=100 width=300px% style="cursor:hand"><font color="blue" face=arial size=2><STRONG>Next Approval Authority Required ?</STRONG></font></marquee></a>
						<%
					}
					else
					{
...
//-------------------------------------------------------
			...
			{
				//NOCRequestApprovalByChangedRoute
				RequestDispatcher rd=request.getRequestDispatcher("NOCRequestApprovalByChangedRouteEmpwise.jsp");

				//request.setAttribute("WFC",QryWFCode);
				//request.setAttribute("WFT",QryWFType);
...
//-------------------------------------------------------
			...
			{
				RequestDispatcher rd=request.getRequestDispatcher("NOCRequestApprovalByChangedRouteDeptwise.jsp");
				request.setAttribute("message","Msg1");
				rd.forward(request,response);
			}
	//-----------------------------
