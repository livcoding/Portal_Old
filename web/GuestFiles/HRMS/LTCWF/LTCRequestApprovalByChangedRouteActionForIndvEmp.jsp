...
String QryChDvFor="", QryRID="", QryFaculty="", QryWFCode="", QryWFType="", QryDOA="", QryDOR="";
	...
	if(!mMemberID.equals("") && !mMemberCode.equals(""))
	{
	  ...
	  //-----------------------------
		...
		if (RsChk.next() && RsChk.getString("SL").equals("Y"))
		{
			...
			<input type=hidden Name='QryWFType' ID='QryWFType' value='<%=QryWFType%>'>
						...
						//out.print("ApprBy- "+mApprBy+" ApprAuth - "+mApprAuth);
							...
							//-------------
							   ...
							   else
							   {
								...
								// Log Entry
								//-----------------
								      db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,QryWFCode+" APPROVAL", "EMPLOYEEID:"+QryFaculty +" WFSEQUENCE:"+ mWFSeq, "NO MAC Address" , mIPAddress);
								//-----------------
								}
								mFlag=0;
							}
			...
			//--------------------------------------------------
						...
						{
							qry="DELETE FROM WF#EMPWISEWORKFLOWAUTHORITY WHERE COMPANYCODE='"+mComp+"' AND ";
							qry=qry+" INSTITUTECODE='"+mInst+"' AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"'";
							qry=qry+" AND EMPLOYEEID='"+QryFaculty+"' AND FACULTYTYPE='"+mFacType+"' AND WFSEQUENCE='"+mWFSeq+"'";
							n1=db.update(qry);
							if(n1>0)
							{
							// Log Entry
							//-----------------
							      db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"DISCARD "+QryWFCode+" WORK FLOW LEVEL", "WFCode:"+QryWFCode+" WFType:"+QryWFType+" Req Dept:"+mDepartmentCode+" WFSeq:"+mWFSeq, "NO MAC Address" , mIPAddress);
							//-----------------
							mDelFlag++;
							}
						}
			...
			/*	if(AL1!=null)
				{
					...
					while(itr.hasNext())
					{
						...
						if(nnn>0)
						{
							...
							rs.next();
						  //-----------Log Entry------------
							db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType,"WORK FLOW LEVEL ADDITION - FOR "+QryWFCode, "WFCode: "+QryWFCode+" WFType: "+QryWFType+" WFLevel: "+LVL+" WFSeq: "+SEQ+" Dept: "+mDepartmentCode, "No MAC Address" , mIPAddress);
						  //----------------------------------
						}
					}
					if(nnnFlag>0)
