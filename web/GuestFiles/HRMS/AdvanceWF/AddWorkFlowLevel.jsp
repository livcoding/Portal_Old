...
int n=0, CTR=0, TotalInAppAuthTbl=0, TotalInRequestWiseTbl=0;
int mCBOnLevel=0, NewLvlVal=0;
int mWFSeqc=0, QryWFLevel=0;
double QryTotReqVal=0;
String mApprBy="", mApprAuth="", mApprovalBy="", mApprovalByName="";
...
String qry="", mTextToFwd="", mTextToDisp="";
...
String mInst="";
String QryRID="", QryWFCode="", QryWFType="", QryDOA="", QryDOR="";
...
if(request.getParameter("TRV")==null)
{
	QryTotReqVal=0;
}
else
{
	QryTotReqVal=Double.parseDouble(request.getParameter("TRV").toString().trim());
}
//out.print("RID-"+QryRID+" WFC-"+QryWFCode+" WFT-"+QryWFType+" WFL-"+QryWFLevel+" DOA-"+QryDOA+" DOR-"+QryDOR+" TotalReqVal-"+QryTotReqVal);
String mHead="";
...
function FunSubmit1()
{
	//document.frm.action="AdvanceRequestInbox.jsp" ;
	window.close();
}
function FunLReqBy()
{
...
try
{
	...
	<input type=hidden Name='TRV' ID='TRV' value='<%=QryTotReqVal%>'>
	<%
	...
	if(rsFrIndv.next())
	{
		...
		qry=qry+" AND CompanyCode='"+mComp+"' and InstituteCode='"+mInst+"' AND DEPARTMENTCODE='"+mDeptCode+"'";
	...
	{
		qry="select nvl(Max(WFSEQUENCE),0)WFSEQC from WF#EMPWISEWORKFLOWAUTHORITY WHERE CompanyCode='"+mComp+"' and InstituteCode='"+mInst+"' AND";
		...
		qry="select nvl(Max(WFLEVEL),0)WFLVL from WF#EMPWISEWORKFLOWAUTHORITY WHERE CompanyCode='"+mComp+"' and InstituteCode='"+mInst+"' AND";
		qry=qry+" WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND EMPLOYEEID='"+QryFaculty+"' AND FACULTYTYPE='"+mFacType+"'";
		qry=qry+" GROUP BY COMPANYCODE, INSTITUTECODE, WORKFLOWCODE, WORKFLOWTYPE, EMPLOYEEID, FACULTYTYPE";
		rs2=db.getRowset(qry);
		//out.print(qry);
	...
	else if(rsFrDept.next())
	{
		...
		qry="select nvl(Max(WFLEVEL),0)WFLVL from WF#WORKFLOWAUTHORITY where CompanyCode='"+mComp+"' and InstituteCode='"+mInst+"' AND";
		qry=qry+" WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"'";
		qry=qry+" GROUP BY COMPANYCODE, INSTITUTECODE, WORKFLOWCODE, WORKFLOWTYPE, DEPARTMENTCODE";
		rs2=db.getRowset(qry);
		//out.print(qry);
	...
	if(request.getParameter("x")!=null)
	{
		...
		if (request.getParameter("TextToBeDisp")!=null)
			...
			mTextToDisp=mTextToDisp;

		//out.print("ApprBy "+mApprBy+" ApprAuth "+mApprAuth+" TextToDisp "+mTextToDisp);
...
//------------------------------------
			qry="Select nvl(count(*),0) TotalInRequestWiseTbl from WF#REQUESTWORKFLOWPATH where REQUESTID='"+QryRID+"' AND COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInst+"' AND DEPARTMENTCODE='"+mDeptCode+"'";
			qry=qry+"GROUP BY REQUESTID, COMPANYCODE, INSTITUTECODE, DEPARTMENTCODE";
			rs=db.getRowset(qry);
			if(rs.next())
			{
			...
			if(TotalInRequestWiseTbl<1)
			{
				...
				{
					qry="SELECT nvl(Count(CRITERIABASEDON),0) CBW FROM WF#WORKFLOWAUTHORITY WHERE COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInst+"' AND ";
					...
					if(mCBOnLevel>1)
					{
						...
						qry=qry+" from WF#WORKFLOWAUTHORITY where COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInst+"' AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"'";
						qry=qry+" AND DEPARTMENTCODE='"+mDeptCode+"' AND WFLEVEL='"+i+"' AND '"+QryTotReqVal+"' between FROMVALUE and TOVALUE";
					...
					while(rs.next())
					{
						...
						qry=qry+"VALUES('"+QryRID+"','"+mCC+"','"+mIC+"','"+mDC+"','"+mWS+"','"+mWL+"','"+mAB+"','"+mAA+"','"+mTextToDisp+"')";
						//out.print(qry);
...
//------------------------------------
			...
			qry="Select 'Y' from WF#REQUESTWORKFLOWPATH WHERE REQUESTID='"+QryRID+"' AND COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInst+"' AND DEPARTMENTCODE='"+mDeptCode+"' AND WFSEQUENCE='"+mWFSeqc+"' AND WFLEVEL='"+CTR+"'";
			rs=db.getRowset(qry);
			//out.print(qry);
			if(!rs.next())
			{
				//out.print(mApprBy+" "+mApprAuth);
				ValForAL=QryRID+"///"+mComp+"***"+mInst+"@@@"+mDeptCode+"###"+mWFSeqc+"&&&"+CTR+"((("+mApprBy+")))"+mApprAuth+"[[["+mTextToDisp;
				//out.print(ValForAL);
				AL.add(ValForAL);
...
//-----------------------------
			...
			if(AL1!=null)
			{
				...
				<tr bgcolor="#ff8c00">
					...
					<td align=left nowrap><font color=white><B>Approval Text</B></font></td>
				</tr>
				</thead>
				<tbody>
				<%
				...
				while(itr.hasNext())
				{
					...
					%>
					<tr bgcolor="white">
					<td nowrap align=center><font color=black face=arial><%=LVL%></font></td>
					<%
					if(APBY.equals("E"))
					{
						mApprovalBy="Employee";
						qry="Select EmployeeID EmpID, EmployeeName EmpName from EmployeeMaster where CompanyCode='"+COMP+"' and EmployeeID='"+APAU+"'";
					}
					else if(APBY.equals("H"))
					{
						mApprovalBy="HOD [Self]";
						qry="Select EmployeeID EmpID, EmployeeName EmpName from EmployeeMaster where CompanyCode='"+COMP+"' and EmployeeID IN (SELECT EmployeeID from HODLIST where DepartmentCode='"+APAU+"' AND nvl(DEACTIVE,'N')='N')";
					}
					else if(APBY.equals("O"))
					{
						mApprovalBy="HOD [Other]";
						qry="Select EmployeeID EmpID, EmployeeName EmpName from EmployeeMaster where CompanyCode='"+COMP+"' and EmployeeID IN (SELECT EmployeeID from HODLIST where DepartmentCode='"+APAU+"' AND nvl(DEACTIVE,'N')='N')";
					}
					else if(APBY.equals("D"))
					{
						mApprovalBy="Desg. Based";
						qry="Select EmployeeID EmpID, EmployeeName EmpName from EmployeeMaster where CompanyCode='"+COMP+"' and DESIGNATIONCODE='"+APAU+"' AND NVL(DEACTIVE,'N')='N'";
					}
					rs2=db.getRowset(qry);
					//out.print(qry);
					if(rs2.next())
					...
					%>
					<td nowrap align=left><font color=black face=arial><%=mApprovalBy%></font></td>
					<td nowrap align=left><font color=black face=arial><%=mApprovalByName%></font></td>
					<%
					if(TXTDSP.equals("F"))
						TXTDSP="Forward";
					else if(TXTDSP.equals("G"))
						TXTDSP="Grant";
					else if(TXTDSP.equals("R"))
						TXTDSP="Recommend";
					else
						TXTDSP="Approve";
					%>
					<td nowrap align=left><font color=black face=arial><%=TXTDSP%></font></td>
					<%
