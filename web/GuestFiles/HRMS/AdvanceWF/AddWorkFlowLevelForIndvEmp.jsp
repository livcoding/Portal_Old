...
try
{
	...
	mWFSeqc++;
	qry="Select 'Y' from WF#EMPWISEWORKFLOWAUTHORITY where COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInst+"' AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND EMPLOYEEID='"+QryFaculty+"' AND FACULTYTYPE='"+mFacType+"' AND WFLEVEL='"+QryWFLevel+"' AND nvl(DEACTIVE,'N')='N'";
	rsFrDept=db.getRowset(qry);
	//out.print(qry);

	if(rsFrDept.next())
	{
		qry="Select Max(TOVALUE) MaxToVal from WF#EMPWISEWORKFLOWAUTHORITY where CompanyCode='"+mComp+"' and InstituteCode='"+mInst+"' AND ";
	...
	if(request.getParameter("x")!=null)
	{
		...
		if(mChkLevel.equals("Required"))
		{
		   if(!mApprAuth.equals(""))
		   {
			qry="Select 'Y' from WF#EMPWISEWORKFLOWAUTHORITY WHERE COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInst+"' AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND EMPLOYEEID='"+QryFaculty+"' AND FACULTYTYPE='"+mFacType+"' AND WFLEVEL='"+QryWFLevel+"'";
			...
			if(!rs.next())
			{
				...
				qry=qry+"VALUES('"+mComp+"','"+mInst+"','"+QryWFCode+"','"+QryWFType+"','"+QryFaculty+"','"+mFacType+"','"+mWFSeqc+"','"+mApprBy+"','"+mApprAuth+"','"+QryWFLevel+"','D','"+mCBFR+"','"+mCBTO+"','"+mChkMemID+"',sysdate, '"+mTextToDisp+"')";
				n=db.insertRow(qry);
				//out.print(qry);
				if(n>0)
				{
				...
				else
				{
					...
					out.print("&nbsp;&nbsp;&nbsp <b><font size=3 face='Arial' color='Red'>Error while addition!</font> <br>");
					%></CENTER><%
				}
			}
			else
		...
		else
		{
			%><CENTER><br><%
			out.print("<img src='../../../Images/Error1.gif'>");
			out.print("&nbsp;&nbsp;&nbsp <b><font size=3 face='Arial' color='Red'>Please Check Level!</font> <br>");
			%></CENTER><%
		}
	} //closing of outer if
	...
	qry=qry+" FROM WF#EMPWISEWORKFLOWAUTHORITY WHERE COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInst+"' ";
