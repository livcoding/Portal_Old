...
String mComp="", mInst="", QryAdvanceType="", mAdvReqID="", WFCode="ADVANCE";
...
<HTML>
<HEAD>
<TITLE>#### <%=mHead%> [ Advance Request ] </TITLE>
...
<!-------------Page Heading and Marquee Message----------------------->
		...
		<!------------------->
			...
			qry=qry+" from PAY#ADVANCETYPEMASTER where COMPANYCODE='"+mComp+"' AND APPLICABLETO='"+mDMemberType+"' and nvl(DEACTIVE,'N')='N' ORDER BY ADVDESC";
			//out.print(qry);
			rs=db.getRowset(qry);
			%>
			...
			<td nowrap colspan=3>
			<%
			qry="Select MIN(MINNOOFEMI)MINNOOFEMI, MAX(MAXNOOFEMI)MAXNOOFEMI from PAY#ADVANCETYPEMASTER WHERE COMPANYCODE='"+mComp+"' AND APPLICABLETO='"+mDMemberType+"' AND nvl(DEACTIVE,'N')='N' GROUP BY COMPANYCODE, APPLICABLETO";
			rs=db.getRowset(qry);
			//out.print(qry);
			if(rs.next())
			{
		...
		<!-------------------->
			...
			if(request.getParameter("x")!=null)
			{
				...
				if(rs.next())
				{
					if(QryAdvAmt>0)
					{
						...
						if(rs.next())
						{
							...
							<tr><td colspan=2 align=center><INPUT id=submit2 style="BACKGROUND-COLOR:TRANSPARENT; FONT-WEIGHT: bold; FONT-SIZE: smaller; FLOAT: none; WIDTH: 70px; HEIGHT: 23px; FONT-VARIANT: normal" type=submit value="Request" name=submit2 title="Request for Advance" onclick="return ActiveSubmit();"></td></tr>
							</table>
							</form>
							<%
						}
				...
				{
					%><table align=center><tr><td>
					<b><font size=2 face='Arial' color='RED'>'Date OF Advance' must be of current date or later!</font></b>
					</td></tr></table><%
				}
			}
			if(errFlag>0)
			...
			if(request.getParameter("y")!=null)
			{
				...
				if(!QryPOA.equals(""))
				{
					qry="SELECT 'Y' FROM PAY#EMPOTHERADVANCEREQUEST WHERE COMPANYCODE='"+mComp+"' AND ADVANCETYPE='"+QryAdvType+"' AND MEMBERTYPE='"+mDMemberType+"' AND MEMBERID='"+mDMemberID+"' AND ADVANCEDATE=to_date('"+mDateOfAdv+"','DD-MM-YYYY')";
					rs=db.getRowset(qry);
					//out.print(qry);
					if(!rs.next())
					{
						...
						qry="INSERT INTO PAY#EMPOTHERADVANCEREQUEST(COMPANYCODE, INSTITUTECODE, ADVANCETYPE, MEMBERTYPE, MEMBERID, ADVANCEDATE, REQUESTID, ADVANCEAMOUNT, INSTALLMENT, NOOFINSTALLMENT, INTERESTAMOUNT, DEDUCTEDAMOUNT, REQUESTREMARKS, REQUESTBY, REQUESTDATE, WORKFLOWCODE, WORKFLOWTYPE, STATUS)";
						qry=qry+" VALUES('"+mComp+"', '"+mInst+"', '"+QryAdvType+"', '"+mDMemberType+"', '"+mDMemberID+"', to_date('"+mDateOfAdv+"','DD-MM-YYYY'), '"+mAdvReqID+"', '"+QryAdvAmt+"', 'Y', '"+QryNOOFEMI+"', '"+QryTotalIntAmt+"', '"+QryEMIVal+"', '"+QryPOA+"', '"+mDMemberID+"', sysdate ,'ADVANCE', '"+QryAdvType+"','D')";
						n=db.insertRow(qry);
						//out.print(qry);
						if(n>0)
						{
							qry="Select nvl(ADVANCEDESC,' ')ADVANCEDESC from PAY#ADVANCETYPEMASTER where COMPANYCODE='"+mComp+"' AND ADVANCETYPE='"+QryAdvType+"' AND APPLICABLETO='"+mDMemberType+"'";
							rs=db.getRowset(qry);
							//out.print(qry);
							if(rs.next())
							{
								%><table align=center><tr><td>
								<b><font size=2 face='Arial' color='Green'>Your Advance Request for <%=rs.getString("ADVANCEDESC")%> has been sent Successfully...</font></b>
								</td></tr></table><%
							}
						}
						else
