		...
		if (RsChk.next() && RsChk.getString("SL").equals("Y"))
		{
						...
						//out.print("ApprBy- "+mApprBy+" ApprAuth - "+mApprAuth);
							...
							//-------------
							   ...
							   else
							   {
								...
								// Log Entry
								//-----------------
								      //db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,QryWFCode+" APPROVAL", "ExamCode:"+mECode +" SubjectCode:"+ mSCode, "NO MAC Address" , mIPAddress);
								//-----------------
								}
								mFlag=0;
							}
			...
			//--------------------------------------------------
						else if(mActCurr.equals("D") && mChk.equals("Checked"))
						{
							...
							// Log Entry
							//-----------------
							      db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"DISCARD "+QryWFCode+" WORK FLOW CRITERIA ON THIS LEVEL", "WFCode:"+QryWFCode+" WFType:"+QryWFType+" Req Dept:"+mDepartmentCode+" WFSeq:"+mWFSeq, "NO MAC Address" , mIPAddress);
							//-----------------
							}
							mDelFlag++;
						}
			...
			//-----Use Iterator Here------
			   ...
			   else if(mAddDropLvl.equals("DC"))
			   {
				...
				// Log Entry
				//-----------------
				      db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"DISCARD "+gb.toTtitleCase(QryWFCode)+" WORK FLOW LEVEL", "WFCode:"+QryWFCode+" WFType:"+QryWFType+" Req Dept:"+mDepartmentCode+" WFLvl:"+mWFLevel, "NO MAC Address" , mIPAddress);
				//-----------------
					mDelFlag++;
					mDelWFLvl=mWFLevel;

					qry="SELECT nvl(WFLEVEL,0) WFLEVEL FROM WF#EMPWISEWORKFLOWAUTHORITY WHERE COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInst+"'";
