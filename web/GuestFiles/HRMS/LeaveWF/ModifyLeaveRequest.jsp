...
<HTML>
<head>
<TITLE>#### <%=mHead%> [ Leave Request Modify] </TITLE>
...
try
{
	String mPageHeader="Modify Leave Request", mMarqMsg="", CurrDate="";
	...
	qry="Select nvl(B.PAGEHEADER,'Modify Leave Request')PageHeader FROM WEBKIOSKRIGHTSMASTER B WHERE B.RIGHTSID='"+mRightsID+"' and B.RELATEDTO LIKE '%E%'";
	rs=db.getRowset(qry);
	//out.print(qry);
	if(rs.next())
	{
...
<!--**Leave Code Combo**-->
	...
	qry=qry+" FROM LEAVEMASTER A,LEAVEREQUEST B WHERE A.LEAVECODE=B.LEAVECODE  AND B.EMPLOYEEID='"+mChkMemID+"' AND nvl(B.STATUS,'D')='D' order by LeaveDesc";
	//out.print(qry);
	rs=db.getRowset(qry);
	if(request.getParameter("x")==null)
	{	
	...
	//out.print(qry);
		...
		if(count==0)
			...
			%>
				...
				<td align=center rowspan=2 nowrap ><b><font color="white">Edit! </font>
				</b></td>
				</tr>
				<tr bgcolor="#ff8c00">
		...
		<%
				...
				%>
				<td nowrap align=center><a Title="Click To Modify <%=rs.getString("STAFF")%>'s Modify Leave Request" href='ModifyLeaveRequestDetail.jsp?RID=<%=rs.getString("RID")%>'><font color=blue>Edit</Font></a>
				</td>
			<%
