...
//out.print("RID "+QryRID+" DEPT "+mDeptCode+" TOTLVDAYS "+mTotalLvDays+" EID "+QryFaculty+" WFCode "+QryWFCode+" WFType "+QryWFType+" From "+QryLvDateFr+" To "+QryLvDateTo);
...
try
{
	if(!mMemberID.equals("") && !mMemberCode.equals("") && !mMemberName.equals(""))
	{
	  ...
	  //----------------------
	%>
	<form name="frm"  method="post" Action="LeaveRequestApprovalByChangedRoute.jsp">
	...
	<%
	qry="select EmployeeName||' ['||EmployeeCode||']' ENM from employeemaster where employeeid='"+QryFaculty+"'";
	rs=db.getRowset(qry);
	if(rs.next())
...
//---------------Start Of if(rsFrIndv.next())----
		...
		try
		{
			while(rs.next())
			{
				...
				qry=qry+" MINUS ";
				qry=qry+" Select nvl(WFLEVEL,0)WFL from WF#WORKFLOWDETAIL Where REQUESTID='"+QryRID+"' AND CompanyCode='"+mComp+"' and InstituteCode='"+mInstitute+"' and WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' GROUP BY WFLEVEL)";
...
//---------------Start Of if(rsFrDept.next())----
		...
		try
		{
			while(rs.next())
			{
				...
				qry=qry+" MINUS ";
				qry=qry+" Select nvl(WFLEVEL,0)WFL from WF#WORKFLOWDETAIL Where REQUESTID='"+QryRID+"' AND REQUESTID='"+QryRID+"' AND CompanyCode='"+mComp+"' and InstituteCode='"+mInstitute+"' and WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' GROUP BY WFLEVEL)";
				rs1=db.getRowset(qry);
				//out.print(qry);
				if(rs1.next())
				{
