...
<HTML>
<head>
<TITLE>#### <%=mHead%> [ Leave Work Flow ] </TITLE>
...
try
{
	...
	mWFSeqc++;
	qry="Select 'Y' from WF#WORKFLOWAUTHORITY where COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"' AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"' AND WFLEVEL='"+QryWFLevel+"' AND nvl(DEACTIVE,'N')='N'";
	rsFrDept=db.getRowset(qry);
	//out.print(qry);

	if(rsFrDept.next())
	{
		qry="Select Max(TOVALUE) MaxToVal from WF#WORKFLOWAUTHORITY where CompanyCode='"+mComp+"' and InstituteCode='"+mInstitute+"' AND ";
	...
	//out.print(qry);
		...
		<tr>
		<td nowrap width=300px><font color=navy face=arial size=2><STRONG>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Text of Leave Recommendation</STRONG></font></td>
		<td nowrap>
	...
	if(request.getParameter("x")!=null)
	{
		...
		if(mChkLevel.equals("Required"))
		{
		   if(!mApprAuth.equals(""))
		   {
			qry="Select 'Y' from WF#WORKFLOWAUTHORITY WHERE COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"' AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"' AND WFLEVEL='"+QryWFLevel+"'";
			...
			if(!rs.next())
			{
				...
				qry=qry+"VALUES('"+mComp+"','"+mInstitute+"','"+QryWFCode+"','"+QryWFType+"','"+mDeptCode+"','"+mWFSeqc+"','"+mApprBy+"','"+mApprAuth+"','"+QryWFLevel+"','D','"+mCBFR+"','"+mCBTO+"','"+mChkMemID+"',sysdate, '"+mTextToDisp+"')";
				n=db.insertRow(qry);
				//out.print(qry);
				if(n>0)
				{
					...
					rs.next();
				  //-----------Log Entry------------
					db.saveTransLog(mInstitute,mLogEntryMemberID,mLogEntryMemberType ,"WORK FLOW LEVEL CRITERIA ADDITION - FOR LEAVE", "WFCode: "+QryWFCode+" WFType: "+QryWFType+" WFLevel: "+QryWFLevel+" WFSeq: "+mWFSeqc+" Dept: "+mDeptCode, "No MAC Address" , mIPAddress);
				  //----------------------------------
				}
				else
				{
	...
	qry=qry+" FROM WF#WORKFLOWAUTHORITY WHERE COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"' ";
