...
String  qry1="",qry2="",mPrevExmCode1="",mPrevExmCode2="";
String mMemberID="";
String mMemberType="";
String mMemberName="";
...
int mSupFees=0;
String mSupRegcode="";
qry="SELECT NVL(PREVEXAMCODE,' ')PREVEXAMCODE,NVL(PREVEXAMCODE1,' ')PREVEXAMCODE1,NVL(PREVEXAMCODE2,' ')PREVEXAMCODE2, nvl(SUPFEES,0)SUPFEES,NVL(SUPREGCODE,' ')SUPREGCODE  FROM EXAMMASTER where InstiTuteCode='"+mInst+"' and   examcode='"+SupExamCode+"' and NVL(LOCKEXAM,'N')='N'";
//out.println(qry);
rs=db.getRowset(qry);
if(rs.next())
{
...
/*
qrys="SELECT distinct nvl(FEEAMOUNT,0)FEEAMOUNT  FROM FEESTRUCTUREINDIVIDUAL WHERE INSTITUTECODE='"+mInst+"' AND FEEHEAD" +
        " LIKE '%SEF%' AND  STUDENTID='"+mChkMemID+"' AND" +
        " REGCODE= '"+mSupRegcode+"' AND SEMESTER='"+mSems+"' ";
			//	out.print(qrys);
				
					rs=db.getRowset(qrys);
				if(rs.next())
				{
					mTot=rs.getInt("FEEAMOUNT");
				}
*/
...
for(int yy =1; yy<=sno;yy++)
{
	if(request.getParameter("Checked"+yy)==null)
		...
		{
			mChk=request.getParameter("Checked"+yy);
			
		}


		if(request.getParameter("SubjectID"+yy)==null)
		{
			msubject="N";
		}
		else
...
//out.print(msubject+"aaaa");
		...
		qry="SELECT DISTINCT v.fstid fstid , a.enrollmentno en2, v.subjectid subjectid, v.subject, v.subjectcode, " +
                "  nvl(v.semester,0) semester,v.semestertype semestertype, b.grade, nvl(B.coursecreditpoint,0)coursecreditpoint,     " +
                "           TO_CHAR (examperiodto, 'Month YYYY') exammonth           FROM studentmaster a,            " +
                "    studentresultdetail b,                exammaster d,                v#studentltpdetail v    " +
                "                   WHERE a.institutecode ='"+ mInst +"'            AND a.studentid = b.studentid   " +
                "         AND a.studentid = v.studentid            AND b.studentid = v.studentid   and v.subjectid='"+msubject+"'   " +
                "      AND d.examcode = b.examcode            AND d.institutecode = a.institutecode    AND b.fstid = v.fstid    " +
                "        AND b.examcode = v.examcode            AND  B.examcode in( '"+mPrevExmCode+"','"+mPrevExmCode1+"','"+mPrevExmCode2+"','"+SummerExamCode+"')  " +
	...
	//	out.print(qry);
        StudentRecordSet = db.getRowset(qry); 	
			...
			if(!rs1.next())
				...
				mTot+=mSupFees;
				//mTot+=mSupFees;
				qry1="INSERT INTO SUPPLEMENTARYSUBJECTTAGGING (   INSTITUTECODE, COMPANYCODE, EXAMCODE," +
                        "    REGCODE, STUDENTID, SUBJECTID,    SEMESTERTYPE, OLDFSTID,   " +
                        " ENTRYBY, ENTRYDATE) VALUES ('"+mInst+"' , '"+mComp+"','"+SupExamCode+"','"+mSupRegcode+"' ,'"+mSID+"', '"+msubject+"' ,'SUP','"+StudentRecordSet.getString("fstid")+"', '"+mSID+"' ,SYSDATE )";
			//out.print(qry1);
				...
				//	mTot=mTot+mSupFees;
						...
						qrys="SELECT 'Y' FROM FEESTRUCTUREINDIVIDUAL WHERE INSTITUTECODE='"+mInst+"' AND FEEHEAD LIKE '%SEF%'" +
                                " AND  STUDENTID='"+mChkMemID+"' AND REGCODE= '"+mSupRegcode+"' AND SEMESTER='"+mSems+"' ";
				rss=db.getRowset(qrys);
				if(!rss.next())
					{
						

						qry="INSERT INTO FEESTRUCTUREINDIVIDUAL (   COMPANYCODE, INSTITUTECODE, ACADEMICYEAR, " +
                                "   PROGRAMCODE, BRANCHCODE, SEMESTER,    SEMESTERTYPE, FEEHEAD, CURRENCYCODE,   " +
                                "STUDENTID, FEEAMOUNT,     ENTRYBY, ENTRYDATE,    REGCODE, POSTINGCOMPANY,GLID,DEACTIVE) " +
                                "VALUES ( '"+mComp+"','"+mInst+"' ,'"+mAcadYear+"' ,'"+mProg+"','"+mBranch+"' ,'"+mSems+"'" +
                                " ,'SUP','"+mFEEHEAD+"','INR'	,'"+mSID+"' ,'"+mTot+"' ,'"+mSID+"',SYSDATE,'"+mSupRegcode+"','"+mPostComp+"' ,'"+mGLID+"','N' )";
						// out.print(qry);
                         //mGLID="",mPostComp=""
					...
					{
						qry=" UPDATE FEESTRUCTUREINDIVIDUAL SET   FEEAMOUNT='"+mTot+"' WHERE 		" +
                                "		INSTITUTECODE  = '"+mInst+"' AND    FEEHEAD  ='"+mFEEHEAD+"' AND STUDENTID ='"+mChkMemID+"'" +
                                " and COMPANYCODE='"+mComp+"'  AND SEMESTER='"+mSems+"' and REGCODE='"+mSupRegcode+"'  ";
						//out.print(qry);
	...
	//out.print(qry2);
		...
		qry="alter trigger TRG_PoplateCoreSubjects disable";
		int x=db.update(qry);
		...
		qry="alter trigger TRG_PoplateCoreSubjects enable";
		int s=db.update(qry);
...
qry="DELETE FROM SUPPLEMENTARYSUBJECTTAGGING WHERE studentid='"+mSID+"' and examcode='"+SupExamCode+"' " +
        "and SUBJECTID='"+msubject+"'  AND INSTITUTECODE='"+mInst+"'  ";
out.print(qry);
int b=db.update(qry);
