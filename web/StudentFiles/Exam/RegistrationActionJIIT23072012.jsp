...
<form name="frm" method=post>
			...
			<%
			qry="select distinct nvl(examcode,' ')examcode, EXAMPERIODFROM from exammaster where institutecode='"+mInst+"' AND eXAMCODE LIKE '%SUP%' and nvl(deactive,'N')='N' order by examcode Desc";
			//out.println(qry);
			rs=db.getRowset(qry);
			%>
...
qry="SELECT PREVEXAMCODE, SUPFEES FROM EXAMMASTER where InstiTuteCode='"+mInst+"' and examcode='"+QryExam+"' and NVL(LOCKEXAM,'N')='N'";
//out.println(qry);
rs=db.getRowset(qry);
if(rs.next())
{
...
int SrNo=0;
	...
	{
		SrNo++;
		qrydebar="SELECT 'Y' FROM DEBARSTUDENTDETAIL WHERE INSTITUTECODE ='"+ mInst +"'   and STUDENTID='"+mChkMemID+"'  AND EXAMCODE='"+mPrevExmCode+"'   AND  SUBJECTID='"+StudentRecordSet.getString("subjectid")+"'     AND NVL(REGISTEREDSTATUS,'N')='N'";
...
<% int ss=0;
	qry="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,                v.subjectcode, v.semester, b.grade, b.coursecreditpoint,                TO_CHAR (examperiodto, 'Month YYYY') exammonth           FROM studentmaster a,                studentresultdetail b,                exammaster d,                v#studentltpdetail v          WHERE a.institutecode = '"+ mInst +"'            AND a.studentid = b.studentid            AND a.studentid = v.studentid            AND b.studentid = v.studentid            AND d.examcode = b.examcode            AND d.institutecode = a.institutecode            AND b.fstid = v.fstid            AND b.examcode = v.examcode            AND b.examcode = '"+mPrevExmCode+"'                       AND a.studentid = '"+mChkMemID+"' and (a.studentid,v.subjectid)  in             (select studentid,subjectid from  DEBARSTUDENTDETAIL where             EXAMCODE='"+mPrevExmCode+"' and studentid='"+mChkMemID+"' AND INSTITUTECODE='"+ mInst +"'          )";
	rs=db.getRowset(qry);
	rs1=db.getRowset(qry);
	if(rs1.next())
		{
