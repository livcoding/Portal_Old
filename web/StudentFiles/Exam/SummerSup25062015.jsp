...
*/
qry=qry+" union SELECT DISTINCT v.enrollmentno en2, c.subjectid subjectid, c.subject,                 c.subjectcode, B.semester, 'Not Registered' grade, a.coursecreditpoint,                a.supfees,decode(a.BASKET,'A','CORE','B','ELECTIVE','D','ELECTIVE','',a.BASKET)BASKET FROM offersubjecttagging a, nrstudentfailsubjects b,studentmaster v,subjectmaster c WHERE a.institutecode = '"+ mInst +"' AND v.studentid = b.studentid AND v.studentid = v.studentid and nvl(v.DEACTIVE,'N')='N' AND NVL(V.DEACTIVE,'N')='N' AND a.subjectid = b.subjectid AND a.subjectid = b.subjectid  and a.examcode ='"+QryExam+"' and a.subjectid=c.subjectid and a.subjectid=b.subjectid and a.institutecode=c.institutecode and a.institutecode=v.institutecode AND NVL(REGISTERED,'N')='N'  AND V.studentid = '"+mChkMemID+"'  ) ";
