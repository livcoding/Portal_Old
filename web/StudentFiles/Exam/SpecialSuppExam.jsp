...
<HTML>
<head>
<TITLE> <%=mHead%> [Special Supplimentary Registration Form]</TITLE>
...
if(Psum!=0 && Lsum!=0)
	{
		if( (Lsum>4 && Psum>1) || (Lsum>3 && Psum>2) )
			alert("You can select only L-4 ,P-1 or L-3,P-2")
				return false;
	...
	else
	{
		...
		if(Psum>2)
		{
		alert("You can select only 2 Practical Subjects !");
		...
		if(Lsum>4)
		{
		alert("You can select only 4 Lectuer Subjects !");
...
<form name="frm" method=post>
			...
			<tr>
			<TD align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: arial"><u><b>SPECIAL SUPPLEMENTARY REGISTRATION FORM 
			...
			<%
			qry="select distinct nvl(examcode,' ')examcode, EXAMPERIODFROM from exammaster where institutecode='"+mInst+"' AND (eXAMCODE LIKE '%SPL%' ) and nvl(deactive,'N')='N' and nvl(LOCKEXAM,'N')='N'  and  trunc(sysdate) >= trunc(examperiodfrom) and trunc(sysdate) <=trunc(examperiodto)    order by examcode Desc";
			//out.println(qry);
			rs=db.getRowset(qry);
			%>
...
String mSupFees="",mSumExam="",mCurrExam="";
...
<form name="frm1" method=post action="SpecialSuppExamAction.jsp">
...
/*	qry="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject, v.subjectcode,                v.semester, b.grade, B.coursecreditpoint,                TO_CHAR (examperiodto, 'Month YYYY') exammonth ,v.LTP          FROM studentmaster a,                studentresultdetail b,                exammaster d,                v#studentltpdetail v                       WHERE    v.SEMESTER = 4          and v.LTP in  ('L','P') and  a.institutecode ='"+ mInst +"'            AND a.studentid = b.studentid            AND a.studentid = v.studentid            AND b.studentid = v.studentid            AND d.examcode = b.examcode            AND d.institutecode = a.institutecode    AND b.fstid = v.fstid            AND b.examcode = v.examcode            AND  b.examcode in( '"+mPrevExmCode+"','"+mSumExam+"')   AND NVL(b.fail,'N')='Y'  AND a.studentid =  '"+mChkMemID+"'  ";

if(	mInst.equals("JIIT"))
qry=qry+ "	AND  (v.subject NOT LIKE '%PROJECT%'            OR v.subject LIKE '%PROJECT MANAGEMENT%') " ;

	qry=qry+ " AND    SUBJECTID NOT IN (             SELECT SUBJECTID FROM debarstudentdetail WHERE NVL (debar, 'N') = 'Y'  AND institutecode ='"+ mInst +"'             AND examcode IN ('"+mPrevExmCode+"') AND studentid ='"+mChkMemID+"'              )";
	qry=qry+ "  AND subjectid NOT IN (                   SELECT subjectid                     FROM NRSTUDENTFAILSUBJECTS                    WHERE                       institutecode = '"+ mInst +"'                               AND studentid = '"+mChkMemID+"') ";

	qry=qry+ " 	AND subjectid NOT IN (   SELECT subjectid                      FROM  v#studentltpdetail                     WHERE institutecode =  '"+ mInst +"'    AND studentid = '"+mChkMemID+"' and  examcode IN ('"+mCurrExam+"')   )                                                         ";
	qry=qry+" UNION  SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject, v.subjectcode,                v.semester, b.grade, B.coursecreditpoint,                TO_CHAR (examperiodto, 'Month YYYY') exammonth    , v.ltp        FROM studentmaster a,                studentresultdetail b,                exammaster d,                v#studentltpdetail v                       WHERE    v.ltp IN ('L', 'P')      and v.SEMESTER=4 AND a.institutecode ='"+ mInst +"'            AND a.studentid = b.studentid            AND a.studentid = v.studentid            AND b.studentid = v.studentid            AND d.examcode = b.examcode            AND d.institutecode = a.institutecode    AND b.fstid = v.fstid            AND b.examcode = v.examcode            AND  b.examcode in('"+mSumExam+"') AND NVL(b.fail,'N')='Y'  AND a.studentid =  '"+mChkMemID+"'  ";
...
*/
/*
qry="SELECT  DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject, v.subjectcode,         v.semester, b.grade, B.coursecreditpoint, v.LTP FROM    studentmaster a, studentresultDetail b, exammaster d, v#studentltpdetail v WHERE   a.SEMESTER      >6 and     v.LTP in ('L','P') and     a.institutecode ='"+ mInst +"'  AND     a.studentid     = b.studentid AND     a.studentid     = v.studentid AND     b.studentid     = v.studentid AND     b.fstid         = v.fstid AND     b.examcode      = v.examcode AND     b.examcode not in( '"+mPrevExmCode+"') And     d.ExamCode      = v.ExamCode And     d.InstituteCode = v.InstituteCode AND     NVL(b.fail,'N')='Y'  AND     a.studentid = '"+mChkMemID+"'  And     nvl(a.deactive,'N') = 'N' AND     (v.subject NOT LIKE '%PROJECT%' OR v.subject LIKE '%PROJECT MANAGEMENT%') AND     v.subjectid NOT IN (                   SELECT subjectid                     FROM   v#studentltpdetail                    WHERE  institutecode ='"+ mInst +"'                       AND examcode IN ( '"+mPrevExmCode+"')) AND     v.SUBJECTID NOT IN (  SELECT  SUBJECTID                             FROM    debarstudentdetail                             WHERE   NVL (debar, 'N')    = 'Y'                             AND     institutecode       ='"+ mInst +"'                            AND     examcode IN ( '"+mPrevExmCode+"')                             ) AND v.subjectid NOT IN ( SELECT   x.subjectid                              FROM    NRSTUDENTFAILSUBJECTS x                             WHERE   x.institutecode   = '"+ mInst +"'                            And     not exists(select 'y'                                                from  v#studentltpdetail y                                                where y.institutecode = a.institutecode                                                and   y.subjectid = v.subjectid                                                and   y.ltp =  v.LTP                                                and   y.studentid = a.studentid)                            AND     studentid       = a.StudentID) AND v.subjectid NOT IN ( SELECT    t.subjectid                                  FROM    studentresult t                                 WHERE   t.institutecode   ='"+ mInst +"'                                AND     t.SubjectID       = subjectid                                    AND     t.studentid       = a.StudentID                                and     Nvl(t.fail,'N')   <> 'Y')                             ";*/

/*qry="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,                v.subjectcode, v.semester, b.grade, b.coursecreditpoint,                v.ltp           FROM studentmaster a,                studentresultdetail b,                exammaster d,                v#studentltpdetail v          WHERE a.semester > 6            AND v.ltp IN ('L', 'P')            AND a.institutecode ='"+ mInst +"'            AND a.studentid = b.studentid            AND a.studentid = v.studentid            AND b.studentid = v.studentid            AND b.fstid = v.fstid            AND d.examcode = v.examcode            AND d.institutecode = v.institutecode            AND NVL (b.fail, 'N') = 'Y'            AND a.studentid ='"+mChkMemID+"'            AND NVL (a.deactive, 'N') = 'N'            AND (   v.subject NOT LIKE '%PROJECT%'                 OR v.subject LIKE '%PROJECT MANAGEMENT%'                )                  AND v.subjectid NOT IN (                   SELECT subjectid                     FROM v#studentltpdetail                    WHERE institutecode = '"+ mInst +"' and studentid ='"+mChkMemID+"'                          AND examcode IN ( '"+mPrevExmCode+"'))                     ";*/


qry="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,                v.subjectcode, v.semester, b.grade, b.coursecreditpoint,                v.ltp           FROM studentmaster a,                studentresult b,                exammaster d,                v#studentltpdetail v          WHERE a.semester > 6            AND v.ltp IN ('L', 'P')    and a.STUDENTID IN ( SELECT c.STUDENTID FROM STUDENTSGPACGPA c  WHERE c.semester="+mMaxsem+" AND   c.institutecode = '"+ mInst +"'                  and a.INSTITUTECODE=c.institutecode and A.STUDENTID=c.STUDENTID AND c.CGPA >= "+mMinsem+"  AND c.studentid = '"+mChkMemID+"'        )          AND a.institutecode = '"+ mInst +"'             AND a.studentid = b.studentid            AND a.studentid = v.studentid            and b.SUBJECTID=v.subjectid            AND b.studentid = v.studentid                               AND d.examcode = v.examcode            AND d.institutecode = v.institutecode            AND NVL (b.fail, 'N') = 'Y'            AND a.studentid ='"+mChkMemID+"'             AND NVL (a.deactive, 'N') = 'N'            AND (   v.subject NOT LIKE '%PROJECT%'                 OR v.subject LIKE '%PROJECT MANAGEMENT%'                )                  AND v.subjectid NOT IN (                   SELECT subjectid                     FROM v#studentltpdetail                    WHERE institutecode =  '"+ mInst +"'  and studentid = '"+mChkMemID+"'                           AND examcode IN ('"+mPrevExmCode+"'))         ";
//out.print(qry);
        StudentRecordSet = db.getRowset(qry); 	
int SrNo=0;
...
qrydebar="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,                v.subjectcode, nvl(v.semester,0)semester, b.grade, nvl(b.coursecreditpoint,0)coursecreditpoint,   TO_CHAR (examperiodto, 'Month YYYY') exammonth    FROM studentmaster a,   studentresultdetail b,                exammaster d,                v#studentltpdetail v     WHERE a.institutecode = '"+ mInst +"'    and a.STUDENTID IN ( SELECT c.STUDENTID FROM STUDENTSGPACGPA c  WHERE c.semester="+mMaxsem+" AND   c.institutecode = '"+ mInst +"'                  and a.INSTITUTECODE=c.institutecode and A.STUDENTID=c.STUDENTID AND c.CGPA >= "+mMinsem+"  AND c.studentid = '"+mChkMemID+"'        )          AND a.studentid = b.studentid     AND a.studentid = v.studentid    AND b.studentid = v.studentid            AND d.examcode = b.examcode            AND d.institutecode = a.institutecode    AND b.fstid = v.fstid            AND b.examcode = v.examcode            AND B.examcode in( '"+mPrevExmCode+"','"+mSumExam+"')                    AND a.studentid = '"+mChkMemID+"' and (a.studentid,v.subjectid)  in             (select studentid,subjectid from  DEBARSTUDENTDETAIL where             EXAMCODE IN ( '"+mPrevExmCode+"')       and studentid='"+mChkMemID+"' AND INSTITUTECODE='"+ mInst +"'  AND (NVL(DEBAR,'N')='Y' or NVL (MEDICALCASE, 'N') = 'Y')   )  AND  (a.studentid, v.subjectid)  IN (                   SELECT studentid, subjectid                     FROM debarstudentdetail                    WHERE examcode IN ( '"+mSumExam+"')                            AND studentid = '"+mChkMemID+"'                      AND institutecode = '"+ mInst +"'                       AND (NVL (debar, 'N') = 'Y'                           OR NVL (medicalcase, 'N') = 'Y'                          ))      and  (v.subject NOT LIKE '%PROJECT%'            OR v.subject LIKE '%PROJECT MANAGEMENT%') ";
	...
	<%
		...
		%>
	<TD align=center>   <INPUT TYPE="checkbox" NAME="Checked<%=sno%>" <%=Checked%> id="Checked" onClick="FunCheck('<%=StudentRecordSet.getDouble("CourseCreditPoint")%>','<%=sno%>')"></TD>

<input type="hidden" name="LTP<%=sno%>"
id="LTP<%=sno%>" value="<%=StudentRecordSet.getString("LTP")%>" >
...
<% int ss=0;
	qry="SELECT DISTINCT a.enrollmentno en2, v.subjectid subjectid, v.subject,                v.subjectcode, nvl(v.semester,0)semester, b.grade, b.coursecreditpoint,   TO_CHAR (examperiodto, 'Month YYYY') exammonth    FROM studentmaster a,   studentresultdetail b,                exammaster d,                v#studentltpdetail v     WHERE a.institutecode = '"+ mInst +"'     and a.STUDENTID IN ( SELECT c.STUDENTID FROM STUDENTSGPACGPA c  WHERE c.semester="+mMaxsem+" AND   c.institutecode = '"+ mInst +"'                  and a.INSTITUTECODE=c.institutecode and  A.STUDENTID=c.STUDENTID AND c.CGPA >= "+mMinsem+"  AND c.studentid = '"+mChkMemID+"'        )       AND a.studentid = b.studentid     AND a.studentid = v.studentid    AND b.studentid = v.studentid            AND d.examcode = b.examcode            AND d.institutecode = a.institutecode    AND b.fstid = v.fstid            AND b.examcode = v.examcode            AND B.examcode in( '"+mPrevExmCode+"','"+mSumExam+"')                    AND a.studentid = '"+mChkMemID+"' and (a.studentid,v.subjectid)  in             (select studentid,subjectid from  DEBARSTUDENTDETAIL where             EXAMCODE IN ( '"+mPrevExmCode+"')       and studentid='"+mChkMemID+"' AND INSTITUTECODE='"+ mInst +"'  AND (NVL(DEBAR,'N')='Y' or NVL (MEDICALCASE, 'N') = 'Y')   )  AND  (a.studentid, v.subjectid)  IN (                   SELECT studentid, subjectid                     FROM debarstudentdetail                    WHERE examcode IN ( '"+mSumExam+"')                            AND studentid = '"+mChkMemID+"'                      AND institutecode = '"+ mInst +"'                       AND (NVL (debar, 'N') = 'Y'                           OR NVL (medicalcase, 'N') = 'Y'                          ))      and  (v.subject NOT LIKE '%PROJECT%'            OR v.subject LIKE '%PROJECT MANAGEMENT%') ";
