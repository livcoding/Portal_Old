...
<%
	...
	String mInst="",mCurExamCode="",mWebEmail="";
	int  mSemesterNo=0;
	double mSGP=0, mCGP=0;      
	String mSUBJ="", mSUBJCODE="", mGRD="", mGRP="", mCCP="", mECP="", mSGP1="", mCGP1="", mFail="";
	String mStudentID="",mEnrollmentNo ="",mName="",mDMemberCode="",mSection="";
	ResultSet rsSub=null, rs=null;
...
if (session.getAttribute("InstituteCode")==null)
	...
	{
		mStudentID=session.getAttribute("MemberID").toString().trim();
	}
	...
	{
		mEnrollmentNo =session.getAttribute("MemberCode").toString().trim();
	}
...
try{
	...
	if(!mStudentID.equals("") && !mEnrollmentNo.equals("")) 
	{

	mStudentID=enc.decode(mStudentID);

	mDMemberCode=enc.decode(mEnrollmentNo);
  ...
  //----------------------
     ...
     qry="Select StudentName, Enrollmentno ,semester,academicyear,BRANCHCODE from StudentMaster where institutecode='"+mInst+"'  and Studentid='"+mChkMemID+"'";	
      rs= db.getRowset(qry);
	if (rs.next())
	{
		
		mAcad=rs.getString("academicyear");	
		mSection=rs.getString("BRANCHCODE");	
	}
%>

	<CENTER><FONT face=Arial><FONT size=2><P align=center><FONT color=black><B>Student Name : <%=mName %> </B></FONT>
<hr><b><FONT color=black><b>Student Examination Result Details of Semester <%=mSemesterNo%></b></FONT>&nbsp; 
	 ...
	 <br>
			...
			<th><FONT color=white face=Arial size=2>Subject Code</FONT></th>
			<th><FONT color=white face=Arial size=2>Subject</FONT></th>
...
/*     qry = "Select  INDUSTRIALPROJECTGRADE from studentindustrialgrade Where InstituteCode='" + mInst + "' and studentid='" + mStudentID.toString().trim() + "' and  SEMESTER=" + mSemesterNo;
	...
	//	out.print(qry);
     //   qry ="Select  NVL(SUBJECTID,'') subjectID, GRADE, nvl(GRADEPOINT,0) GradePoint, NVL(COURSECREDITPOINT,0) COURSECREDITPOINT, nvl(EARNEDCREDIT,0) EARNEDCREDIT, nvl(SGPAPOINTS,0) SGPAPOINTS, nvl(CGPAPOINTS,0) CGPAPOINTS, Decode(FAIL,'N','No','Y','Yes','No') FAIL From studentresult#pub  Where InstituteCode='" + mInst + "' and studentid='" + mStudentID.toString().trim()  + "' and  SEMESTER=" + mSemesterNo + " Order By  Grade Desc";
	  StudentRecordSet = db.getRowset(qry);
		...
		{
		  //2
		 mSUBJCODE=StudentRecordSet.getString("SubjectCode");
		 mSUBJ=StudentRecordSet.getString("Subject");
		 ...
		 mSGP1= StudentRecordSet.getString(8);
			...
			{
			%>
			<td><FONT color=red><%=mSUBJCODE%></FONT></td>
		       <%
			 }
			else 
			{
			...
			{
			%>
			<td><FONT color=black><%=mSUBJCODE%></FONT></td>
		       <% }
			else 
			{	
			%>
...
<%
            qry = "select nvl(PERCENTAGEEQUIVALENT,0)PERCENTAGEEQUIVALENT from CGPAConversionChart where INSTITUTECODE='" + mInst + "' and CGPA='" + mCGP + "' and nvl(deactive,'N')='N' and CHARTYEARCODE='" + mAcad + "'  ";
//out.print(qry);
            rs = db.getRowset(qry);
            rs.next();

%>
<center><FONT color=black face=Arial size=2> * The CGPA of <%=mCGP%> on the scale of 10 equates to <%=rs.getString(1)%> percent as per Conversion Table -2013,approved by the Academic Council of the University.</FONT></center>
