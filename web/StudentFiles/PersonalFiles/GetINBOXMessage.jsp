...
<table id=id1 width="100%" ALIGN=CENTER bottommargin=0  topmargin=0>
	<tr><TD align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: verdana"><b><u>Inbox </u></B></font></TD>
	</font></td></tr>
	</TABLE>
...
try
{
	...
	{	
		...
		//	qry="SELECT trim(MSGFROMUSERD)||trim(MSGFROMMEMBERTYPE)||trim(to_char(MSGDATETIME,'yyyymmddhh24missss')) rid, MSGFROMUSERD, MSGFROMMEMBERTYPE, MSGSUBJECT, MSGTEXT, to_char(MSGDATETIME,'dd-Mon-yyyy HH:MI PM') DT , nvl(READLASTTIME,'N') FROM MESSAGESLIST Where  INSTITUTECODE='"+mInst+"' And MSGTOUSERD='"+ mDMemberID+"' And MSGTOMEMBERTYPE='"+mDMemberType+"' And nvl(DEACTIVE,'N')='N' Order By  DT Desc";
			...
			qry="  SELECT EMAILMSGID  rid,    EMAILMSGID, SLNO, INSTITUTECODE,    FROMUSERID MSGFROMUSERD,to_char(SENDDATETIME,'dd-Mon-yyyy HH:MI PM') DT  , FROMUSERTYPE MSGFROMMEMBERTYPE,    MSGSUBJECT MSGSUBJECT,  FILEATACHED FROM EMAILSENDMASTER WHERE INSTITUTECODE='"+mInst+"'  AND  EMAILMSGID IN (SELECT EMAILMSGID FROM  EMAILSENDDETAIL WHERE INSTITUTECODE='"+mInst+"' AND  TOUSERID = '"+ mDMemberID+"')";
			...
			//out.print(qry);	
				...
				if(Rs.next())
				{
					...
					<td><A target=new href='ReadINBOXMsgAction.jsp?mrid=<%=mRID%>' Title='Click to view Detailed'><font color=black face='Verdana' size=2><%=msbj%></font></a></td>
