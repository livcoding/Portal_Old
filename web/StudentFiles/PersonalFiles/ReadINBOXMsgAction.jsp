...
<table id=id1 width="100%" ALIGN=CENTER bottommargin=0  topmargin=0>
	<tr><TD align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: verdana"><b><u>Mail Detail</u></B></font></TD>
	</font></td></tr>
	</TABLE>
...
<tr>
<td><font color=black face=Verdana size=2>Member Name: <%=mMemberName%></b></td>
<td><font color=black face=Verdana size=2>Member Code: <%=enc.decode(mMemberCode)%></b></td>
</tr>
</table>
<hr>
<%
...
if(request.getParameter("mrid")==null)
	...
	mEMsgID=request.getParameter("mrid").toString().trim();
			...
			//qry=" SELECT MSGFROMUSERD, nvl(MSGFROMMEMBERTYPE,'E') MSGFROMMEMBERTYPE, MSGSUBJECT, MSGTEXT, to_char(MSGDATETIME,'dd-Mon-yyyy HH:MI PM') DT , nvl(READLASTTIME,'N') FROM MESSAGESLIST Where trim(MSGFROMUSERD)||trim(MSGFROMMEMBERTYPE)||trim(to_char(MSGDATETIME,'yyyymmddhh24missss'))='"+mrid+"'";
				...
				qry="  SELECT trim(EMAILMSGID)||trim(FROMUSERTYPE)||trim(to_char(SENDDATETIME,'yyyymmddhh24missss')) rid,    EMAILMSGID, SLNO, INSTITUTECODE,    FROMUSERID MSGFROMUSERD,to_char(SENDDATETIME,'dd-Mon-yyyy HH:MI PM') DT  , FROMUSERTYPE MSGFROMMEMBERTYPE,    MSGSUBJECT MSGSUBJECT, MESSAGESSENT MSGTEXT, NVL(FILEATACHED,'N')FILEATACHED  FROM EMAILSENDMASTER WHERE INSTITUTECODE='"+mInst+"' and EMAILMSGID='"+mEMsgID+"' AND  EMAILMSGID IN (SELECT EMAILMSGID FROM  EMAILSENDDETAIL WHERE INSTITUTECODE='"+mInst+"' AND  TOUSERID = '"+ mDMemberID+"' and  EMAILMSGID='"+mEMsgID+"')";
			//out.print(qry);				
			RsChk=db.getRowset(qry);
			...
			%>		
			<br>
			<table border=1 cellpadding=0 cellspacing=0 align=center width="100%"  borderColor=#ff8c00 borderColorDark=white >
			<%
			if (RsChk.next())
			{
				...
				if (rs.next())
				{
					...
					%>
					<tr>
					<td><font color=black face='Verdana' size=2> Message From :</td><td><%=mEmpName%> (<%=mEmpCode%>)</td>
					</tr>
					<tr>
					<td><font color=black face='Verdana' size=2>Subject :</td><td> <%=msbj%></td>
					</tr>
					<td><font color=black face='Verdana' size=2>Date Time :</td><td> <%=DT%></td>
					</tr>					
					<tr>
					<td valign=top><font color=black face='Verdana' size=2>Mesage:</td><td bgColor=white> 
					
					<%
					/*if(RsChk.getString("FILEATACHED").equals("Y"))
					{
						link=RsChk.getString("FILEPATH").toString().trim();
					*/
						%>
						<!-- <a href="<%=link%>" title="AttachmentFile<%=link%>" target="_NEW" >View Attachment</a>  -->
						<%
				//	}
						%>
