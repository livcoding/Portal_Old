...
<HTML>
<head>
<TITLE>#### <%=mHead%> [  Leave Information] </TITLE>
...
try
{
	...
	{	
		...
		%>
			...
			<tr>
			<TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><b><u><FONT SIZE=4> Leave Information</FONT></u></b></td>
			</tr>
			</table>
			</center>
			<%
			try
			{
				qry="SELECT DISTINCT NVL (a.studentname, ' ') employeename,   a.programcode department, a.branchcode designation  FROM studentmaster a   WHERE a.studentid = '"+mChkMemID+"' AND NVL (a.deactive, 'N') = 'N' and a.institutecode='"+mInst+"' ";
				//out.println(qry);
				rs=db.getRowset(qry);
				if(rs.next())
				{
			...
			<tr>
				...
				<b><FONT face=Arial size=2>&nbsp;&nbsp;Program : </font> </B>
				<font color="#00008b" face=Arial><b> &nbsp;<%=mDesignation1%> </b></font>
				<b><FONT face=Arial size=2>&nbsp; Branch : </font></B><font color="#00008b" face=Arial><b>&nbsp;&nbsp;<%=mDepartment1%></b></font><hr>
				</td>
			</tr>							
			</table>
			</center>
			...
			try
			{
				qry="Select Least(sysdate,Todate) valDate,Trunc(Least(sysdate,Todate) -trunc(Fromdate))+1 day,to_char(FromDate,'dd-mm-rrrr')FromDate,to_char((LEAST(trunc(ToDate),trunc(SYSDATE))),'dd-mm-rrrr')ToDate from LeaveYear where COMPANYCODE='"+mComp+"' and LEAVEYEARCODE='"+mLeaveYearCode+"'";
			//	out.println(qry);
				rs=db.getRowset(qry);
				while(rs.next())  
				{						
				...
				qry="Select Leave.StudentPresent('"+mComp+"','"+mInst+"','"+mCategoryCode+"','"+mChkMemID+"',To_date('"+mFromDate+"','dd-mm-yyyy'),To_date('"+mToDate+"','dd-mm-yyyy')) Present from dual";
				//out.println(qry);
				rs=db.getRowset(qry);
				while(rs.next())  
				{
