 ...
 <style type="text/css" media="print">
    ...
    <body aLink=#ff00ff bgcolor=#fce9c5 rightmargin=0 leftmargin=0 topmargin=0 bottommargin=0 >
        ...
        <td colspan=0 align=middle>
            <font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: arial"><FONT SIZE="4" COLOR=""><B><u>Selection Of Students</u></B></FONT></font></td></tr>
        </table>
...
<%
           ...
           // out.print(qry1);
              ...
              while(rs2.next())
              {
                  ...
                  map.put("written",rs2.getString(1));
                  map.put("groupdiss",rs2.getString(2));
                  map.put("interview",rs2.getString(3)); 
              written= map.get("written")==null?"":map.get("written").trim();
              gd=map.get("groupdiss")==null?"":map.get("groupdiss").trim();
              interview=map.get("interview")==null?"":map.get("interview").trim();
             ...
             roundname="I";
              ...
              if(written.equals("Y")){%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
              Written: <input type="radio" name="round" id="round" value="W"/>
              <%}
              if(gd.equals("Y")){%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
               G.D.: <input type="radio" name="round" id="round" value="G"/>
              <%}%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                Interview:<input type="radio" name="round" id="round" value="I"/>
             <%
                   //  {
                }
round=request.getParameter("round")==null?"":request.getParameter("round").trim();
                ...
                {
        out.print("error in rounds"+e);
        }%>
	</td>
    
   <td align="center">
       <input type="Submit" name="btn1" value="Submit" onclick="return vari();">
   </td>
</tr>
  </table>
  ...
  <table width="30%" border="2" cellpadding=3  id="tblSearch" class="mytable filterable" cellspacing=2 align=center bordercolor="black" bottommargin=0  topmargin=0>
       ...
       <td align="center"><Font size="3" COLOR="black" align="center">Enrollment No. &nbsp; &nbsp;  &nbsp; &nbsp; &nbsp;&nbsp;Student's Name &nbsp; &nbsp; &nbsp;&nbsp;&nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp;&nbsp;</FONT></td>
        ...
        <% 
           ...
           qry="SELECT DISTINCT studentname,studentid,ENROLLMENTNO FROM studentmaster WHERE studentid IN (SELECT DISTINCT studentid " +
             "FROM tp#eventparticipents WHERE registered = 'Y' and eventcode='"+eventcode+"' and " +
             "companycode='"+company+"') order by studentname";
               //out.print(qry);
               rs=db.getRowset(qry);
               while(rs.next()){
               i++;
       ...
       <td align="left" nowrap>
        <%   qry1="SELECT  status FROM tp#afterinterview WHERE" +
               " eventcode = '"+eventcode+"' AND companycode = '"+company+"' " +
...
<%
  ...
  %>
  <tr><td align="center" colspan="3"><input type="Submit" value="Save"></td></tr></table>
<%
      ...
      <td align="center"><Font size="3" COLOR="black" align="center">Enrollment No. &nbsp; &nbsp;  &nbsp; &nbsp; &nbsp;&nbsp;Student's Name &nbsp; &nbsp; &nbsp;&nbsp;&nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp;&nbsp; &nbsp; &nbsp;&nbsp;</FONT></td>
        ...
        <% 
           qry="SELECT DISTINCT studentname, studentid,ENROLLMENTNO FROM studentmaster WHERE studentid IN (SELECT DISTINCT studentid " +
             "FROM tp#AfterInterview WHERE status = 'W' and eventcode='"+eventcode+"' and " +
             "companycode='"+company+"') union SELECT DISTINCT studentname, studentid,ENROLLMENTNO FROM studentmaster WHERE studentid IN (SELECT DISTINCT studentid " +
             "FROM tp#AfterInterview WHERE status = 'G' and eventcode='"+eventcode+"' and " +
             "companycode='"+company+"') union SELECT DISTINCT studentname, studentid,ENROLLMENTNO FROM studentmaster WHERE studentid IN (SELECT DISTINCT studentid " +
             "FROM tp#AfterInterview WHERE status = 'I' and eventcode='"+eventcode+"' and " +
             "companycode='"+company+"') order by studentname";
         //  out.print(qry);
         rs=db.getRowset(qry);
           while(rs.next()){
...
<input type="text" id="sname<%=i%>" name="sname<%=i%>"  readonly value="<%=rs.getString("studentname")%>" style="width:250px;"/><%}%>
  ...
  <% }%>
  <tr><td align="center"><input type="Submit" value="Save"></td></tr></table><%
...
<%}%>
           <input type="hidden" name="event" id="event" value="<%=eventcode%>">
           <input type="hidden" name="companycode" id="companycode" value="<%=company%>">
...
if(round1.equals("G"))
 {
    qry2=qry2+"ABSENTIN ='G'";
 }else if(round1.equals("I"))
 {
    qry2=qry2+"ABSENTIN ='I'";
 }
...
qry2=qry2+" ,absentin='',selected='Y' WHERE  eventcode='"+Event+"' and companycode='"+Companycode+"' and " +
...
qry2="INSERT INTO TP#AFTERINTERVIEW (EVENTCODE, COMPANYCODE, INSTITUTECODE, STUDENTID, SELECTED, " +
        "ENTRYDATE, STATUS) VALUES ('"+Event+"','"+Companycode+"','"+mInst+"','"+studentid+"','Y',sysdate,'W')";
//out.print(qry2);
d=db.insertRow(qry2);
}
...
if (round1.equals("G"))
  {
qry2="INSERT INTO TP#AFTERINTERVIEW (EVENTCODE, COMPANYCODE, INSTITUTECODE, STUDENTID, SELECTED, " +
        "ENTRYDATE, STATUS) VALUES ('"+Event+"','"+Companycode+"','"+mInst+"','"+studentid+"','Y',sysdate,'G')";
//out.print(qry2);
d=db.insertRow(qry2);
}
}else if(roundname.equals("WI"))
...
if(p>0||x==0&&d==0 ||p>0&&x==0&&d==0)

    {
out.print("<center><font color=Red size=4>Students are not Selected </font></center>");
}
if(d>0||x>0)
