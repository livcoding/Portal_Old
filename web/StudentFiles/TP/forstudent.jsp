...
table="TP#REGISTRATIONDETAIL@tpjiit";
...
out.print("Error in showing Registration History"+e);
    ...
    qry="SELECT COMPANYCODE,SELECTED, PACKAGEOFFEREDINLACKS FROM TP#AFTERINTERVIEW@tpjiit " +
        "where " +
...
<FONT face=Arial size=2><STRONG>Choose Event:</STRONG></FONT><font color=red face=arial size=2><STRONG>*</STRONG></font>
			...
			try
            {
                 qry="SELECT DISTINCT NVL (a.eventcode, '') e ,TO_CHAR (b.eventstartdate,'dd-mm-yyyy')StartDate,TO_CHAR (b.eventenddate,'dd-mm-yyyy')Enddate  FROM tp#eligibilitycriteria@tpjiit a,TP#EVENTMASTER@tpjiit b" +
...
<%
             try{
qry1="SELECT DISTINCT NVL (a.companycode, ' ') c FROM tp#eligibilitycriteria@tpjiit a, tp#eventmaster@tpjiit b " +
        ...
        "AND NVL (b.lockevent, 'N') = 'N' AND a.eventcode = b.eventcode  And not exists(Select   'Y' From    tp#eventcompanyparticipents@tpjiit x where   x.EventCode   = a.EventCode  And x.CompanyCode = a.CompanyCode And ( Nvl(x.Deactive,'N') = 'Y' Or   Nvl(x.LockCompany,'N') = 'Y'))";
...
qry="SELECT   distinct a.CSET FROM " +
        "TP#ELIGIBILITYCRITERIAvalue@tpjiit a, tp#criteriamaster@tpjiit b where a.eventcode='"+Event+"' " +
...
qry2=" SELECT a.criteriaid, a.criteriaoperatorbefore, a.slno FROM tp#eligibilitycriteria@tpjiit a,TP#CRITERIAMASTER@tpjiit b where a.companycode='"+Companycode+"' and a.eventcode='"+Event+"' and a.cset='"+rs.getInt("CSET")+"' and a.criteriaid=b.criteriaid AND NVL (b.checkbacklog, 'N') <> 'Y' AND NVL (b.checkgap, 'N') <> 'Y' AND NVL (a.requiredinregistration, 'N') = 'Y' ";
...
qry4=" select CRITERIAID ,CRITERIATABLE ,CRITERIAFIELD  from TP#CRITERIAMASTER@tpjiit where criteriaid='"+rs2.getString("CRITERIAID")+"' AND nvl(checkbacklog,'N') <> 'Y' AND nvl(checkgap,'N') <> 'Y' ";

rs4=db.getRowset(qry4);
if(rs4.next())
{
 qry3=" select CRITERIAVALUE,SUBSLNO  from  TP#ELIGIBILITYCRITERIAVALUE@tpjiit  where companycode='"+Companycode+"' and eventcode='"+Event+"' and cset='"+rs.getInt("cset")+"'  and  criteriaid='"+rs2.getString("CRITERIAID")+"' and    slno='"+rs2.getString("slno")+"'  ";
...
qry6=" select distinct 'Y' from studentmaster a , "+rs4.getString("CRITERIATABLE")+ADD+" b,TP#ELIGIBILITYCRITERIAVALUE@tpjiit c where c.criteriaid='"+rs2.getString("CRITERIAID")+"' and b."+rs4.getString("CRITERIAFIELD")+"  "+rs2.getString("criteriaoperatorbefore")+" ("+value+") and b.studentid='"+mChkMemID+"'" +
...
s++;
 qry8="SELECT DISTINCT b.criteriaid,b.CRITERIAFIELD FROM tp#eligibilitycriteria@tpjiit a, tp#criteriamaster@tpjiit b WHERE a.companycode = '"+Companycode+"' AND a.eventcode = '"+Event+"' AND a.cset = '"+cSet+"' AND a.criteriaid = b.criteriaid AND NVL (b.checkbacklog, 'N') = 'Y' OR NVL (b.checkgap, 'N') = 'Y' AND NVL (a.requiredinregistration, 'N') = 'Y'";
	//out.print(qry8+"<br>");
...
if(reason.equals("YEAROFPASSING"))
		{
		qry11="SELECT DISTINCT a.criteriaoperatorbefore,b.CRITERIATABLE,b.CRITERIAFIELD,c.CRITERIAVALUE,to_char(d.ENTRYDATE,'yyyy') ENTRYDATE FROM tp#eligibilitycriteria@tpjiit a, tp#criteriamaster@tpjiit b,tp#eligibilitycriteriavalue@tpjiit c,studentmaster d WHERE a.companycode = '"+Companycode+"' AND a.eventcode = '"+Event+"' AND a.cset = '"+cSet+"' AND a.criteriaid = b.criteriaid AND b.criteriafield = '"+reason+"' AND a.criteriaid = c.criteriaid and a.CSET=c.CSET  and d.studentid='"+mChkMemID+"' ";
  //out.print(qry11);
		rs11=db.getRowset(qry11);
        if(rs11.next()){
        qry9=" select distinct 'Y' from "+rs11.getString("CRITERIATABLE")+"@tpjiit"+" where  to_number('"+rs11.getString("ENTRYDATE")+"') -"+rs11.getString("CRITERIAFIELD")+"" +rs11.getString("criteriaoperatorbefore")+"'"+rs11.getString("CRITERIAVALUE")+"' and Upper(qualificationcode) = '12TH' and studentid='"+mChkMemID+"' ";
    //  out.print(qry9);
...
mprint="Y";
		...
		{qry11="SELECT DISTINCT a.criteriaoperatorbefore,b.CRITERIATABLE,b.CRITERIAFIELD,c.CRITERIAVALUE FROM tp#eligibilitycriteria@tpjiit a, tp#criteriamaster@tpjiit b,tp#eligibilitycriteriavalue@tpjiit c WHERE a.companycode = '"+Companycode+"' AND a.eventcode = '"+Event+"' AND a.cset = '"+cSet+"' AND a.criteriaid = b.criteriaid AND   b.criteriafield  = '"+reason+"' AND a.criteriaid = c.criteriaid and a.CSET=c.CSET";
 //out.print(qry11);
		rs11=db.getRowset(qry11);
        if(rs11.next()){
...
<%qry="SELECT COMPANYNAME name, EMAILID email,ADDRESS1, ADDRESS2, ADDRESS3 FROM TP#COMPANYMASTER@tpjiit " +
...
%>
 ...
 <tr bgcolor="silver">
    ...
    <%
        ...
        "FROM tp#participentseventdates@tpjiit WHERE eventcode = '"+event+"' AND companycode = '"+companycode+"'";
//out.print(qury);
...
<%qry="SELECT status FROM TP#REGISTRATIONDETAIL@tpjiit where " +
...
<%}%>
 ...
 <b>Remarks:&nbsp;&nbsp;&nbsp;
 <%qry="SELECT REMARKS FROM TP#REGISTRATIONDETAIL@tpjiit where " +
        "Eventcode='"+event+"' and companycode='"+companycode+"' and INSTITUTECODE='"+mInst+"' " +
        "and STUDENTID='"+mMemberID+"'";
 //out.print(qry);
rs=db.getRowset(qry);
