...
qry1="SELECT distinct NVL(COMPANYCODE,' ') c,status s,remarks r FROM "+table1+"  Where " +
...
out.print("Error in showing Registration History"+e);
    ...
    qry="SELECT COMPANYCODE,SELECTED, PACKAGEOFFEREDINLACKS FROM "+table2+" " +
        "where " +
...
{}
 ...
 <!--table width="100%" ALIGN=CENTER bottommargin=0  topmargin=0>   @tpjuit 
        <tr>
        <td colspan=0 align=middle>
...
<FONT face=Arial size=2><STRONG>Choose Event:</STRONG></FONT><font color=red face=arial size=2><STRONG>*</STRONG></font>
			...
			try
            {
                 qry="SELECT DISTINCT NVL (a.eventcode, '') e ,TO_CHAR (b.eventstartdate,'dd-mm-yyyy')StartDate,TO_CHAR (b.eventenddate,'dd-mm-yyyy')Enddate  FROM "+table3+" a, "+table4+" b" +
...
<%
             try{
qry1="SELECT DISTINCT NVL (a.companycode, ' ') c FROM "+table3+" a, "+table4+" b " +
       ...
       //out.print("qryyyyyyyyyyyyyy:"+qry1);
...
qry="SELECT   distinct a.CSET FROM " +
        " "+table6+" a, "+table7+" b where a.eventcode='"+Event+"' " +
...
qry2=" SELECT a.criteriaid, a.criteriaoperatorbefore, a.slno FROM "+table3+"  a,"+table7+" b where a.companycode='"+Companycode+"' and a.eventcode='"+Event+"' and a.cset='"+rs.getInt("CSET")+"' and a.criteriaid=b.criteriaid AND NVL (b.checkbacklog, 'N') <> 'Y' AND NVL (b.checkgap, 'N') <> 'Y' AND NVL (a.requiredinregistration, 'N') = 'Y' ";
...
qry4=" select CRITERIAID ,CRITERIATABLE ,CRITERIAFIELD  from "+table7+" where criteriaid='"+rs2.getString("CRITERIAID")+"' AND nvl(checkbacklog,'N') <> 'Y' AND nvl(checkgap,'N') <> 'Y' ";
//out.print(qry4);
rs4=db.getRowset(qry4);
if(rs4.next())
{
 qry3=" select CRITERIAVALUE,SUBSLNO  from  "+table6+"  where companycode='"+Companycode+"' and eventcode='"+Event+"' and cset='"+rs.getInt("cset")+"'  and  criteriaid='"+rs2.getString("CRITERIAID")+"' and    slno='"+rs2.getString("slno")+"'  ";
...
if (rs3.getInt("SUBSLNO")>1 && !mCritvales.equals("")) {
     mCritvales1 = mCritvales + ",'" +rs3.getString("CRITERIAVALUE")+ "'";
...
if(rs4.getString("CRITERIATABLE").equals("TP#STUDENTDATA") || rs4.getString("CRITERIATABLE").equals("TP#STUDENTQUALIFICATION") ){
Extention="@tpjiit";
}else{
Extention="";
}
...
qry6=" select distinct 'Y' from "+table21+" a , "+rs4.getString("CRITERIATABLE")+Extention+"  b,"+table6+" c where c.criteriaid='"+rs2.getString("CRITERIAID")+"' and b."+rs4.getString("CRITERIAFIELD")+"  "+rs2.getString("criteriaoperatorbefore")+" ("+value+") and b.studentid='"+mChkMemID+"'" +
...
s++;
 qry8="SELECT DISTINCT b.criteriaid,b.CRITERIAFIELD FROM "+table3+" a, "+table7+" b WHERE a.companycode = '"+Companycode+"' AND a.eventcode = '"+Event+"' AND a.cset = '"+cSet+"' AND a.criteriaid = b.criteriaid AND NVL (b.checkbacklog, 'N') = 'Y' OR NVL (b.checkgap, 'N') = 'Y' AND NVL (a.requiredinregistration, 'N') = 'Y'";
	//out.print(qry8+"<br>");
...
if(reason.equals("YEAROFPASSING"))
		{
		qry11="SELECT DISTINCT a.criteriaoperatorbefore,b.CRITERIATABLE,b.CRITERIAFIELD,c.CRITERIAVALUE,to_char(d.ENTRYDATE,'yyyy') ENTRYDATE FROM "+table3+" a, "+table7+" b, "+table6+" c,"+table21+" d WHERE a.companycode = '"+Companycode+"' AND a.eventcode = '"+Event+"' AND a.cset = '"+cSet+"' AND a.criteriaid = b.criteriaid AND b.criteriafield = '"+reason+"' AND a.criteriaid = c.criteriaid and a.CSET=c.CSET  and d.studentid='"+mChkMemID+"' ";
 // out.print(qry11);
		rs11=db.getRowset(qry11);
        if(rs11.next()){

if(rs11.getString("CRITERIATABLE").equals("TP#STUDENTDATA") || rs11.getString("CRITERIATABLE").equals("TP#STUDENTQUALIFICATION") ){
Extention="@tpjiit";
}else{
Extention="";
}
        ...
        qry9=" select distinct 'Y' from "+rs11.getString("CRITERIATABLE")+Extention+" where  to_number('"+rs11.getString("ENTRYDATE")+"') -"+rs11.getString("CRITERIAFIELD")+"" +rs11.getString("criteriaoperatorbefore")+"'"+rs11.getString("CRITERIAVALUE")+"' and Upper(qualificationcode) = '12TH' and studentid='"+mChkMemID+"' ";
    //  out.print(qry9);
...
mprint="Y";
		...
		{qry11="SELECT DISTINCT a.criteriaoperatorbefore,b.CRITERIATABLE,b.CRITERIAFIELD,c.CRITERIAVALUE FROM "+table3+" a, "+table7+" b,"+table6+" c WHERE a.companycode = '"+Companycode+"' AND a.eventcode = '"+Event+"' AND a.cset = '"+cSet+"' AND a.criteriaid = b.criteriaid AND   b.criteriafield  = '"+reason+"' AND a.criteriaid = c.criteriaid and a.CSET=c.CSET";
 //out.print(qry11);
		rs11=db.getRowset(qry11);
        if(rs11.next()){
...
qry9=" select 'Y' from "+rs11.getString("CRITERIATABLE")+Extention+" where institutecode = '"+mInst+"' and nvl("+rs11.getString("CRITERIAFIELD")+",'N') = 'Y' and studentid = '"+mChkMemID+"' having count(*)"+rs11.getString("criteriaoperatorbefore")+""+rs11.getString("CRITERIAVALUE")+"  ";
 //out.print(qry9);
...
<%qry="SELECT COMPANYNAME name, EMAILID email,ADDRESS1, ADDRESS2, ADDRESS3 FROM "+table8+" " +
...
%>
 ...
 <tr bgcolor="silver">
    ...
    <%
        ...
        "FROM "+table9+" WHERE eventcode = '"+event+"' AND companycode = '"+companycode+"'";
//out.print(qury);
...
<%qry="SELECT status FROM "+table1+" where " +
...
<td align="left" nowrap>
 ...
 &nbsp;&nbsp;&nbsp;&nbsp;
 <b>Project:
 <%qry="SELECT PROJECT FROM "+table1+" where " +
        "Eventcode='"+event+"' and companycode='"+companycode+"' and INSTITUTECODE='"+mInst+"' " +
        "and STUDENTID='"+mMemberID+"'";
 //out.print(qry);
rs=db.getRowset(qry);
...
<td align="left" nowrap>
 ...
 <b>Intern Ship:
 <%qry="SELECT INTERNSHIP FROM "+table1+" where " +
        "Eventcode='"+event+"' and companycode='"+companycode+"' and INSTITUTECODE='"+mInst+"' " +
        "and STUDENTID='"+mMemberID+"'";
 //out.print(qry);
rs=db.getRowset(qry);
if(!rs.next())
{
 ...
 <b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Work Experience:
 <%qry="SELECT WORKEXPERIANCE FROM "+table1+" where " +
        "Eventcode='"+event+"' and companycode='"+companycode+"' and INSTITUTECODE='"+mInst+"' " +
        "and STUDENTID='"+mMemberID+"'";
 //out.print(qry);
rs=db.getRowset(qry);
if(!rs.next())
{
 ...
 <b>Remarks:&nbsp;&nbsp;&nbsp;
 <%qry="SELECT REMARKS FROM "+table1+" where " +
        "Eventcode='"+event+"' and companycode='"+companycode+"' and INSTITUTECODE='"+mInst+"' " +
        "and STUDENTID='"+mMemberID+"'";
 //out.print(qry);
rs=db.getRowset(qry);
...
if(rs1.next()){
qry="UPDATE "+table1+" SET STATUS='"+status+"',PROJECT ='"+PROJECT+"',INTERNSHIP ='"+INTERNSHIP+"',WORKEXPERIANCE ='"+WORKEXPERIANCE+"'  ,REMARKS ='"+remarks+"',DATEOFREGISTRATION = sysdate " +
...
{
qry1="INSERT INTO "+table1+" (EVENTCODE, COMPANYCODE, INSTITUTECODE, ACADEMICYEAR, PROGRAMCODE, " +
