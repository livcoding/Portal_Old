...
//    {
  ...
  PdfPCell cell99 = new PdfPCell(new Paragraph("USE 'PAY FEE' MENU OPTION ONLY"));
            cell99.setColspan(4);
            ...
            Phrase ph = new Phrase(10, "PUNJAB NATIONAL BANK  ");
            ph.add("\n");
            ph.add(chunkyc);
            ...
            chunkyc = new Chunk("INSTITUTE ID:JPUIT ", font12c);
            ...
            qry1 = "SELECT   TO_CHAR(s.collseqid)collseqid, g.currencycode, g.feehead,Initcap(s.FEEHEADDESC)FEEHEADDESC," +
                    "         g.postingcompany postingcompany, g.glid, TO_CHAR(g.feeamount)feeamount," +
                    "         'Academic Year Wise' feesource, feetype" +
                    ...
                    "               AND fsc.feehead = g.feehead               AND fsc.QUOTA = g.QUOTA" +
                    "               AND (    fsc.OPERATOR IN ('IN', '=')                    AND fsc.criteriavalue = 'N'" +
                    "                   )) UNION ALL SELECT   TO_CHAR(s.collseqid)collseqid, g.currencycode, g.feehead ,Initcap(s.FEEHEADDESC)FEEHEADDESC ,  " +
                    "       g.postingcompany postingcompany, g.glid, TO_CHAR(g.feeamount)feeamount,         'Criteria Wise' feesource, feetype" +
                    "    FROM feestructurecriteria g, feeheads s   WHERE g.institutecode = '" + mInst + "'" +
                    ...
                    "               AND fsi1.semestertype = g.semestertype" +
                    "               AND fsi1.feehead = g.feehead) UNION ALL " +
                    "SELECT TO_CHAR(s.collseqid)collseqid, fi.currencycode, fi.feehead, Initcap(s.FEEHEADDESC)FEEHEADDESC   ,      fi.postingcompany postingcompany, fi.glid, TO_CHAR(fi.feeamount)feeamount," +
                    "         'Individual' feesource, feetype   FROM feestructureindividual fi, feeheads s" +
                    "   WHERE fi.institutecode = '" + mInst + "'     AND fi.companycode = '" + mCompanyCode + "'" +
                    "     AND fi.studentid = '" + mStudID + "'     AND fi.academicyear = '" + mAcademicYearCode + "'" +
                    "     AND fi.programcode = '" + mProgram + "'     AND fi.branchcode = '" + mBranch + "'" +
                    ...
                    "               UNION ALL SELECT   TO_CHAR(collseqid)collseqid , '" + Currencycode + "'   currencycode, feehead, Initcap(FEEHEADDESC)FEEHEADDESC ," +
                    "         postingcompany postingcompany, glid, TO_CHAR(0)feeamount,         'FeeHeads' feesource, feetype" +
                    "    FROM feeheads   WHERE companycode = '" + mCompanyCode + "'" +
                    "     AND institutecode = '" + mInst + "'     AND NVL (deactive, 'N') = 'N'" +
...
//bold font 
  ...
  PdfPCell cell88 = new PdfPCell(new Paragraph(" PAY FEE "));
            cell88.setColspan(4);
            cell88.setHorizontalAlignment(Element.ALIGN_CENTER);
            tableS.addCell(cell88);
            ...
            Phrase phS = new Phrase(10, "PUNJAB NATIONAL BANK  ");
            phS.add("\n");
            phS.add(chunky);
            ...
            Chunk chunky1 = new Chunk("INSTITUTE ID:JPUIT", font12c1);
