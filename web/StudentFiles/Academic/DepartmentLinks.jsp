...
String mTSelected="" ;
		qry1="select count(c.applicantid)selected from   hr#applicantshortlistmaster c,  hr#applicationmaster a where a.vacancycode in (select vacancycode from HR#VACANCYMASTER WHERE a.vacancycode='"+mVacCode+"' and institutecode='"+mInst+"' and NVL(BROADCAST,'N')='Y' AND SYSDATE BETWEEN FROMPERIOD AND TOPERIOD ) and a.departmentcode ='"+mDeptCode+"'         and c.STATUS='S'   and c.SHORTLISTSEQNO IN (  SELECT (MAX (d.shortlistseqno))                        FROM hr#applicantshortlistmaster d                       WHERE d.institutecode =  '"+mInst+"' and d.vacancycode='"+mVacCode+"'      )     AND a.applicantid = c.applicantid			and a.institutecode='"+mInst+"' AND a.vacancycode = c.vacancycode AND a.institutecode = c.institutecode";
		//out.print(qry1);
		...
		String mTReject="" ;
		qry2="select count(c.applicantid)reject from   hr#applicantshortlistmaster c,  hr#applicationmaster a where a.vacancycode in (select vacancycode from HR#VACANCYMASTER WHERE a.vacancycode='"+mVacCode+"' and institutecode='"+mInst+"' and NVL(BROADCAST,'N')='Y' AND SYSDATE BETWEEN FROMPERIOD AND TOPERIOD ) and a.departmentcode ='"+mDeptCode+"'         and nvl(c.STATUS,'N')='R'   and c.SHORTLISTSEQNO IN (  SELECT (MAX (d.shortlistseqno))                        FROM hr#applicantshortlistmaster d                       WHERE d.institutecode =  '"+mInst+"' and d.vacancycode='"+mVacCode+"'      )             AND a.applicantid = c.applicantid			and a.institutecode='"+mInst+"' AND a.vacancycode = c.vacancycode AND a.institutecode = c.institutecode";
		...
		String mALLSum="";
		qry2="select count(c.applicantid)AllResume from   hr#applicantshortlistmaster c,  hr#applicationmaster a where a.vacancycode in (select vacancycode from HR#VACANCYMASTER WHERE institutecode='"+mInst+"' and NVL(BROADCAST,'N')='Y' AND SYSDATE BETWEEN FROMPERIOD AND TOPERIOD ) and a.departmentcode ='"+mDeptCode+"'         and  c.SHORTLISTSEQNO IN (  SELECT (MAX (d.shortlistseqno))                        FROM hr#applicantshortlistmaster d                       WHERE d.institutecode =  '"+mInst+"' and d.vacancycode='"+mVacCode+"'      )    AND a.applicantid = c.applicantid			and a.institutecode='"+mInst+"' AND a.vacancycode = c.vacancycode AND a.institutecode = c.institutecode";
		//out.print(qry2);
		rs1=db.getRowset(qry2);
		if(rs1.next())
		...
		<td class="labelcell">
		<!-- <a href="DepartmentLinks.jsp?ShowStatus=N&amp;Vacancy=<%=mVacCode%>&amp;DeptCode=<%=mDeptCode%>"> -->	<font color=blue size=2><b> Balance resumes to be considered  </a></td><td> <%=mTNotProcess%> 		</td>
		</tr>
		</table>
   ...
   <table  align=center >
            <tr><td align=center ><h2>&nbsp;&nbsp; Initial ShortListing by Nominated Members &nbsp;&nbsp;</h2></td></tr>
			   ...
			   <tr><td align=center class="labelcell"><b>Name : <%=mMemberName%>-(<%=mDMemberCode%>)</td></tr>
        </table>
 ...
 <table width="100%" border=1  borderColor=#D98242 rules=group align=center bottommargin=1 topmargin=0 cellspacing=0 cellpadding=2>
     ...
     <tr bgcolor="#FFCF83">
	  <td  class="labelcell"><b><CENTER>Sr.No.</CENTER></b></td>
       <td  class="labelcell"><b><CENTER>Candidate<br>Selection Status</CENTER></b></td>
         ...
         <TD  align=left class="labelcell" nowrap valign=top> <table cellspacing=0 cellpadding=0 width="100%" align=left> <td class="labelcell" nowrap colspan=4><b> Applicant Qualification
				 <hr> </td>
				  <tr >
   ...
   <TD  align=center class="labelcell"><b>Click to View CV </b></TD>
         ...
         <td class="labelcell"><b><CENTER> Remarks of Shortlisting<br>(max. 100 words)</CENTER></b> </td>
     </tr>
...
"  nvl(a.AREAOFQUALIFICATION,' ')AREAOFQUALIFICATION ,decode(a.yearofpassing,'0',' ',a.yearofpassing )yearofpassing,nvl(b.shortname ,' ')shortname ,b.SEQNO,nvl(a.QUALIFICATIONINSTITUTE,' ')QUALIFICATIONINSTITUTE FROM HR#APPLICANTQUALIFICATION a,HR#QUALIFICATIONTAGGING b where a.APPLICANTID='" + mApplicantID + "' and b.INSTITUTECODE='"+mInst+"' AND  b.VACANCYCODE='" + mVacCode + "' and a.COMPANYCODE=b.COMPANYCODE and  a.INSTITUTECODE=b.INSTITUTECODE  and  a.QUALIFICATIONCODE=b.QUALIFICATIONCODE  order by a.yearofpassing";
//out.print(qry1);
rs1 = db.getRowset(qry1);
while (rs1.next()) {
