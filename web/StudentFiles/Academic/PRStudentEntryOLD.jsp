...
//	alert(total+"svfsdfgsd"+(parseInt(corecr)+3));
...
//--------------------Changed by Vijay---------------
		...
		if(rse1.next())
		{
			...
			qry="SELECT MINCREDITPOINT MINCRPT, MAXCREDITPOINT MAXCRPT FROM PR#PROGRAMMINMAXCP WHERE INSTITUTECODE='"+mInst+"' and ExamCode='"+mExam+"' AND (EXAMCODE,REGCODE) in ";
...
//out.print(qry+"ddd");
		   ...
		   if(rsso.next())
		   {
			...
			//out.println(mMaxCrLmt+"asdasdasdadas");
		   }
	/*********************************************/
 	...
 	if(!rss1.next())
		      ...
		      else if(rss1.getString("SENDTOHOD").equals("Y"))
			{
			...
			else
	      	{
				...
				<%
String mVl=rss1.getString("NOOFELESUBJECTSCHOICES");				
if(mSem==8){	
				
				if (mVl.equals("0")) mVl="";
				%>				
				<FONT face=Arial size=2 color=black><STRONG>How many elective(s) you want to opt:</STRONG></FONT>&nbsp;&nbsp;
				<input type="text" name="studelecchoise" value="<%=mVl%>" maxlength='2' size='2'  onKeyup="return limit(event);" onkeydown="if(event.keyCode==13){event.keyCode=9;return event.keyCode}" <%=readonly%>
				<%
				}else{%>
				<FONT face=Arial size=2 color=black><STRONG>No. of elective(s) allowed as per curriculum structure/Maximum credit Limit:
</STRONG></FONT>&nbsp;&nbsp;
				<input type="text" name="studelecchoise" value="<%=mVl%>" maxlength='2' size='2'  readonly>	
