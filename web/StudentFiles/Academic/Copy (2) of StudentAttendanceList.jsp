...
qry="select distinct nvl(SUBJECT,' ') ||' - '|| NVL(SUBJECTCODE,' ') Subject , SUBJECTID,ACADEMICYEAR, SEMESTER, STUDENTID,NVL(SEMESTERTYPE,' ')SEMESTERTYPE, SECTIONBRANCH SECBR, SUBSECTIONCODE SUBSEC,employeeid,employeename,employeecode ";
				qry=qry+" from V#STUDENTLTPDETAIL A where  NVL(A.DEACTIVE,'N')='N' and nvl(STUDENTDEACTIVE,'N')='N' and A.examcode='"+QryExam+"' and a.STUDENTID='"+mMemberID+"'  and a.INSTITUTECODE='"+mINSTITUTECODE+"' and  (A.fstid) in (Select C.fstid from StudentAttendance C Where A.FSTID=C.FSTID)  ";
...
//	out.print(qry);
	...
	QryEmpID=rs.getString("employeeid").toString().trim();
		 			...
		 			qry2=" Select nvl(to_char(REGCONFIRMATIONDATE,'dd-mm-yyyy'),' ') REGCONFIRMATIONDATE  From StudentRegistration Where INSTITUTECODE='"+mINSTITUTECODE+"'";
...
//out.print(qry2);
	...
	<tr>
		...
		<td><%=rs.getString("SUBJECT")%></td>
				<td><%=rs.getString("Employeename")%>-<%=rs.getString("EmployeeCode")%></td>
...
qry=" SELECT distinct nvl(count(pcount ),0)  pcount FROM (select distinct CLASSTIMEFROM Pcount from V#STUDENTATTENDANCE a where SubjectID= '"+rs.getString("SubjectID")+"'  and LTP='L' and EXAMCODE= '"+QryExam+"'  AND  ( A.FSTID='"+mLFSTID+"'   OR (a.fstid IN ( SELECT b.mergewithfstid FROM FacultySubjecttagging b where  b.examcode='"+QryExam+"' and b.institutecode='"+mINSTITUTECODE+"' and b.SUBJECTID='"+rs.getString("SubjectID")+"' and  b.LTP='L')))  ";                            qry=qry+" and trunc(A.attendancedate) < trunc(TO_Date(nvl('"+mREGCONFIRMATIONDATE+"',ATTENDANCEDATE),'dd-mm-yyyy'))  and a.studentid<>'"+mMemberID+"' ";
qry=qry+" and INSTITUTECODE='"+mINSTITUTECODE+"'   and nvl(DEACTIVE,'N')='N' and not exists (select 1 from STUDENTPREVATTENDENCE c where  a.studentid=c.studentid and  c.SubjectID= '"+rs.getString("SubjectID")+"'  and c.LTP='L' and c.EXAMCODE= '"+QryExam+"' and c.institutecode='"+mINSTITUTECODE+"' ) ";
qry=qry+" and (('"+rs.getString("SEMESTER")+"'='1' and trunc(ATTENDANCEDATE)>=TO_Date(nvl('"+mREGCONFIRMATIONDATE+"',ATTENDANCEDATE),'dd-mm-yyyy')) ";
qry=qry+"   or ("+rs.getLong("SEMESTER")+">1)        )    )";
...
qry1="SELECT distinct count(pcount ) pcount FROM (select distinct CLASSTIMEFROM pcount from V#STUDENTATTENDANCE a   where SubjectID= '"+rs.getString("SubjectID")+"' and EXAMCODE= '"+QryExam+"' AND  A.FSTID='"+mLFSTID+"'  and a.ltp='L' and a.studentid='"+mMemberID+"'   ";
qry1=qry1+" and INSTITUTECODE='"+mINSTITUTECODE+"'  and nvl(DEACTIVE,'N')='N' and ( ("+rs.getLong("Semester")+"=1 and trunc(ATTENDANCEDATE)>=TO_Date(nvl('"+mREGCONFIRMATIONDATE+"',ATTENDANCEDATE),'dd-mm-yyyy'))  or  ("+rs.getLong("Semester")+">1) ) ";
qry1=qry1+" UNION   ";
...
qry1="SELECT distinct count(pcount ) pcount FROM (select distinct CLASSTIMEFROM pcount from V#STUDENTATTENDANCE a   where SubjectID= '"+rs.getString("SubjectID")+"' and EXAMCODE= '"+QryExam+"' AND  A.FSTID='"+mLFSTID+"'  and a.ltp='L' and a.studentid='"+mMemberID+"' and nvl(a.present,'N')='Y' ";
qry1=qry1+" and INSTITUTECODE='"+mINSTITUTECODE+"'  and nvl(DEACTIVE,'N')='N' and ( ("+rs.getLong("Semester")+"=1 and trunc(ATTENDANCEDATE)>=TO_Date(nvl('"+mREGCONFIRMATIONDATE+"',ATTENDANCEDATE),'dd-mm-yyyy'))  or  ("+rs.getLong("Semester")+">1) ) ";
qry1=qry1+" UNION   ";
qry1=qry1+" select  distinct CLASSTIMEFROM  pcount from STUDENTPREVATTENDENCE where  subjectid ='"+rs.getString("SubjectID")+"'     AND ltp ='L'    ";
...
mNotAttendedAttendance=0;
qry=" SELECT distinct nvl(count(pcount ),0)  pcount FROM (select distinct CLASSTIMEFROM Pcount from V#STUDENTATTENDANCE a where SubjectID= '"+rs.getString("SubjectID")+"'  and LTP='T' and EXAMCODE= '"+QryExam+"'  AND  ( A.FSTID='"+mTFSTID+"'   OR (a.fstid IN ( SELECT b.mergewithfstid FROM FacultySubjecttagging b where  b.examcode='"+QryExam+"' and b.institutecode='"+mINSTITUTECODE+"' and b.SUBJECTID='"+rs.getString("SubjectID")+"' and  b.LTP='T')))  ";                            qry=qry+" and trunc(A.attendancedate) < trunc(TO_Date(nvl('"+mREGCONFIRMATIONDATE+"',ATTENDANCEDATE),'dd-mm-yyyy'))  and a.studentid<>'"+mMemberID+"' ";
qry=qry+" and INSTITUTECODE='"+mINSTITUTECODE+"'   and nvl(DEACTIVE,'N')='N' and not exists (select 1 from STUDENTPREVATTENDENCE c where  a.studentid=c.studentid and  c.SubjectID= '"+rs.getString("SubjectID")+"'  and c.LTP='T' and c.EXAMCODE= '"+QryExam+"' and c.institutecode='"+mINSTITUTECODE+"' ) ";
qry=qry+" and (('"+rs.getString("SEMESTER")+"'='1' and trunc(ATTENDANCEDATE)>=TO_Date(nvl('"+mREGCONFIRMATIONDATE+"',ATTENDANCEDATE),'dd-mm-yyyy')) ";
qry=qry+"   or ("+rs.getLong("SEMESTER")+">1)        )    )";
rs1=db.getRowset(qry);
//out.print(qry);
...
qry1="SELECT distinct count(pcount ) pcount FROM (select distinct CLASSTIMEFROM pcount from V#STUDENTATTENDANCE a   where SubjectID= '"+rs.getString("SubjectID")+"' and EXAMCODE= '"+QryExam+"' AND  A.FSTID='"+mTFSTID+"'  and a.ltp='T' and a.studentid='"+mMemberID+"'   ";
qry1=qry1+" and INSTITUTECODE='"+mINSTITUTECODE+"'  and nvl(DEACTIVE,'N')='N' and ( ("+rs.getLong("Semester")+"=1 and trunc(ATTENDANCEDATE)>=TO_Date(nvl('"+mREGCONFIRMATIONDATE+"',ATTENDANCEDATE),'dd-mm-yyyy'))  or  ("+rs.getLong("Semester")+">1) ) ";
qry1=qry1+" UNION   ";
...
qry1="SELECT distinct count(pcount ) pcount FROM (select distinct CLASSTIMEFROM pcount from V#STUDENTATTENDANCE a   where SubjectID= '"+rs.getString("SubjectID")+"' and EXAMCODE= '"+QryExam+"' AND  A.FSTID='"+mTFSTID+"'  and a.ltp='T' and a.studentid='"+mMemberID+"' and nvl(a.present,'N')='Y' ";
qry1=qry1+" and INSTITUTECODE='"+mINSTITUTECODE+"'  and nvl(DEACTIVE,'N')='N' and ( ("+rs.getLong("Semester")+"=1 and trunc(ATTENDANCEDATE)>=TO_Date(nvl('"+mREGCONFIRMATIONDATE+"',ATTENDANCEDATE),'dd-mm-yyyy'))  or  ("+rs.getLong("Semester")+">1) ) ";
qry1=qry1+" UNION   ";
qry1=qry1+" select  distinct CLASSTIMEFROM  pcount from STUDENTPREVATTENDENCE where  subjectid ='"+rs.getString("SubjectID")+"'     AND ltp ='T'    ";
qry1=qry1+" AND examcode =  '"+QryExam+"'   AND studentid = '"+mMemberID+"' and nvl(present,'N')='Y' ";       
...
mNotAttendedAttendance=0;
qry=" SELECT distinct nvl(count(pcount ),0)  pcount FROM (select distinct CLASSTIMEFROM Pcount from V#STUDENTATTENDANCE a where SubjectID= '"+rs.getString("SubjectID")+"'  and LTP='P' and EXAMCODE= '"+QryExam+"'  AND  ( A.FSTID='"+mPFSTID+"'   OR (a.fstid IN ( SELECT b.mergewithfstid FROM FacultySubjecttagging b where  b.examcode='"+QryExam+"' and b.institutecode='"+mINSTITUTECODE+"' and b.SUBJECTID='"+rs.getString("SubjectID")+"' and  b.LTP='P')))  ";                            qry=qry+" and trunc(A.attendancedate) < trunc(TO_Date(nvl('"+mREGCONFIRMATIONDATE+"',ATTENDANCEDATE),'dd-mm-yyyy'))  and a.studentid<>'"+mMemberID+"' ";
qry=qry+" and INSTITUTECODE='"+mINSTITUTECODE+"'   and nvl(DEACTIVE,'N')='N' and not exists (select 1 from STUDENTPREVATTENDENCE c where  a.studentid=c.studentid and  c.SubjectID= '"+rs.getString("SubjectID")+"'  and c.LTP='P' and c.EXAMCODE= '"+QryExam+"' and c.institutecode='"+mINSTITUTECODE+"' ) ";
qry=qry+" and (('"+rs.getString("SEMESTER")+"'='1' and trunc(ATTENDANCEDATE)>=TO_Date(nvl('"+mREGCONFIRMATIONDATE+"',ATTENDANCEDATE),'dd-mm-yyyy')) ";
qry=qry+"   or ("+rs.getLong("SEMESTER")+">1)        )    )";
rs1=db.getRowset(qry);
//out.print(qry);
...
qry1="SELECT distinct count(pcount ) pcount FROM (select distinct CLASSTIMEFROM pcount from V#STUDENTATTENDANCE a   where SubjectID= '"+rs.getString("SubjectID")+"' and EXAMCODE= '"+QryExam+"' AND  A.FSTID='"+mPFSTID+"'  and a.ltp='P' and a.studentid='"+mMemberID+"'   ";
qry1=qry1+" and INSTITUTECODE='"+mINSTITUTECODE+"'  and nvl(DEACTIVE,'N')='N' and ( ("+rs.getLong("Semester")+"=1 and trunc(ATTENDANCEDATE)>=TO_Date(nvl('"+mREGCONFIRMATIONDATE+"',ATTENDANCEDATE),'dd-mm-yyyy'))  or  ("+rs.getLong("Semester")+">1) ) ";
qry1=qry1+" UNION   ";
...
qry1="SELECT distinct count(pcount ) pcount FROM (select distinct CLASSTIMEFROM pcount from V#STUDENTATTENDANCE a   where SubjectID= '"+rs.getString("SubjectID")+"' and EXAMCODE= '"+QryExam+"' AND  A.FSTID='"+mPFSTID+"'  and a.ltp='P' and a.studentid='"+mMemberID+"' and nvl(a.present,'N')='Y' ";
qry1=qry1+" and INSTITUTECODE='"+mINSTITUTECODE+"'  and nvl(DEACTIVE,'N')='N' and ( ("+rs.getLong("Semester")+"=1 and trunc(ATTENDANCEDATE)>=TO_Date(nvl('"+mREGCONFIRMATIONDATE+"',ATTENDANCEDATE),'dd-mm-yyyy'))  or  ("+rs.getLong("Semester")+">1) ) ";
qry1=qry1+" UNION   ";
qry1=qry1+" select  distinct CLASSTIMEFROM  pcount from STUDENTPREVATTENDENCE where  subjectid ='"+rs.getString("SubjectID")+"'     AND ltp ='P'    ";
qry1=qry1+" AND examcode =  '"+QryExam+"'   AND studentid = '"+mMemberID+"' and nvl(present,'N')='Y' ";       
...
if(mL>0)
{
		...
		%>
	<td align=center><a Title="View Date wise Lecture + Tutorial Attendance" target=_New href='ViewDatewiseLecAttendance.jsp?EXAM=<%=QryExam%>&amp;CTYPE=<%=QryType%>&amp;SC=<%=rs.getString("SUBJECTID")%>&amp;LTP=LT&amp;SEC=<%=QrySecBr%>&amp;SUBSEC=<%=QrySubSec%>&amp;SEMESTERTYPE=<%=QrySemType%>&amp;EMPLOYEEID=<%=QryEmpID%>&amp;prevTFSTID=<%=prevTFSTID%>&amp;prevLFSTID=<%=prevLFSTID%>&amp;mLFSTID=<%=mLFSTID%>&amp;mTFSTID=<%=mTFSTID%>'><%=mPercLT%></a></td>
	
	<td align=center><a Title="View Date wise Lecture Attendance" target=_New href='ViewDatewiseLecAttendance.jsp?EXAM=<%=QryExam%>&amp;CTYPE=<%=QryType%>&amp;SC=<%=rs.getString("SUBJECTID")%>&amp;LTP=L&amp;SEC=<%=QrySecBr%>&amp;SUBSEC=<%=QrySubSec%>&amp;SEMESTERTYPE=<%=QrySemType%>&amp;EMPLOYEEID=<%=QryEmpID%>&amp;prevLFSTID=<%=prevLFSTID%>&amp;mLFSTID=<%=mLFSTID%>'><font color=blue><%=mPercL%></font></a></td>
<%
...
if(mT>0)
{
		...
		%>
		<td align=center><a Title="View Date wise Tutorial Attendance" target=_New href='ViewDatewiseLecAttendance.jsp?EXAM=<%=QryExam%>&amp;CTYPE=<%=QryType%>&amp;SC=<%=rs.getString("SUBJECTID")%>&amp;LTP=T&amp;SEC=<%=QrySecBr%>&amp;SUBSEC=<%=QrySubSec%>&amp;SEMESTERTYPE=<%=QrySemType%>&amp;EMPLOYEEID=<%=QryEmpID%>&amp;prevTFSTID=<%=prevTFSTID%>&amp;mTFSTID=<%=mTFSTID%>'><%=mPercT%></a></td>
		<%
...
if(mP>0)
{
		...
		%>
		<td align=center><a Title="View Date wise Practical Attendance" target=_New href='ViewDatewiseLecAttendance.jsp?EXAM=<%=QryExam%>&amp;CTYPE=<%=QryType%>&amp;SC=<%=rs.getString("SUBJECTID")%>&amp;LTP=P&amp;SEC=<%=QrySecBr%>&amp;SUBSEC=<%=QrySubSec%>&amp;SEMESTERTYPE=<%=QrySemType%>&amp;EMPLOYEEID=<%=QryEmpID%>&amp;prevPFSTID=<%=prevPFSTID%>&amp;mPFSTID=<%=mPFSTID%>'><%=mPercP%></a></td>
		<%
