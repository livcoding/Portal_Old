...
function Clickedsel11(k,j,c,b,t,g)
{
	var gg,flag;
	var el="Selectf"+g+k;
	var temp="temp1"+g+k;
	...
	{
		var pdt=0;
		if(document.frm1.PDElective==null)
			pdt=0;
		else
			pdt =document.frm1.PDElective.value;
		for(i=1;i<=pdt;i++)
		{
			if(document.getElementById("SelectfPD"+pdt).value=="")
			{
				tt=0;			
			}
		}
		if(tt==0)
		{
			...
			for(i=1;i<=t;i++)
			{
				...
				document.getElementById("SelectfPD1").focus();
			}
			return false;			
		}
		
		if(parseInt(document.frm1.TotalCCP.value)>=parseInt(document.frm1.TotalCCPAld.value)+6){
		var eltt=1;
		var elt=0;
		if(document.frm1.electivecount==null)
			elt=0;
		else
			elt =document.frm1.electivecount.value;
		for(i=1;i<=elt;i++)
		{
			if(document.getElementById("Select"+elt).value=="")
			{
				eltt=0;			
			}
		}
		if(eltt==0){
			alert("Please Select Department Elective , Then balance Elective if you are within Course Credit Limit");	
			for(i=1;i<=t;i++){
			document.getElementById(el).value="";
	...
	for(i=1;i<=t;i++)
	{
		gg=c+i;
		if(gg!=(el))
		if(document.getElementById(el).value==document.getElementById(gg).value){
					flag=1;
		}	
	}
	if(flag==1 && document.getElementById(el).value!=""){
		...
		if(document.getElementById(temp).value=='Y'){	
			...
			if(document.frm1.TotalCCP.value>=z){
				...
				for(i=1;i<=t;i++){		
					...
					document.getElementById(tem+i).value='N';
				}
			}else{
				alert('You have exceed (Course Credit Point) : '+z+' and Total allowed : '+document.frm1.TotalCCP.value);
				document.getElementById(el).value="";
	...
	if(document.getElementById(el).value=="" && flag2==0){
		//alert(j+""+document.frm1.TotalCCPAld.value);
		var z=parseInt(document.frm1.TotalCCPAld.value)-parseInt(j);
		document.frm1.TotalCCPAld.setAttribute("value",z);
		for(i=1;i<=t;i++){
			gg=c+i;
...
function finalsave()
{
	...
	var t=0;
		if(document.frm1.PDElective==null)
			...
			t =document.frm1.PDElective.value;	
	if(parseInt(cr)<=total)
	{		
		for(i=1;i<=t;i++)
		{
			if(document.getElementById("SelectfPD"+i).value=="")
			{
				tt=0;	
				document.getElementById("SelectfPD"+i).focus();
			}
		}
		if(tt==0){
			alert("First chose one compulsory PD Elective. Then balance Elective if you are within Course Credit Limit");		
	...
	if ((corecr+3+(2*3))<=parseInt(document.frm1.TotalCCPAld.value)){
	for(i=1;i<=ec;i++)
	{
	...
	if(ttt==0)
	{
		alert("Please Select Department Elective , Then balance Elective if you are within Course Credit Limit");	return false;
	}
	}
	
	
	
	if(document.frm1.PHElective==null)
		t=0;
	else
		t =document.frm1.PHElective.value;				
	var check=0;
	for(i=1;i<=t;i++)
	{
		if(document.getElementById("SelectfPH"+i).value=="")
		{
			check++;
		}
	}
	if(check != t)
	{
		for(i=1;i<=t;i++)
		{
			...
			{
				tt=0;	
				document.getElementById("SelectfPH"+i).focus();		
			}
		}
		if(tt==0)
		{
	...
	if(document.frm1.MAElective==null)
		t=0;
	else
		t =document.frm1.MAElective.value;
	var check=1;
	for(i=1;i<=t;i++)
	{
		if(document.getElementById("SelectfPH"+i).value=="")
	...
	if(check != t)
	{
		for(i=1;i<=t;i++)
		{
			if(document.getElementById("SelectfMA"+i).value=="")
			{
				tt=0;	
				document.getElementById("SelectfMA"+i).focus();		
			}					
		}
		if(tt==0)
		{
...
function Clickedsel(k,j,c,b,t){
	...
	if(document.frm1.PDElective==null)
			...
			ttt =document.frm1.PDElective.value;
	var tt=1;
	for(i=1;i<=ttt;i++){
		if(document.getElementById("SelectfPD"+i).value==""){
			tt=0;			
	...
	if(tt==0){
		...
		for(i=1;i<=t;i++){
			gg="Select"+i;
			if(gg!=(c))
				if(document.getElementById(c).value==document.getElementById(gg).value){
					flag=1;
				}	
		}
...
temp="temp1";	
	if(document.getElementById(temp).value=='Y'){	
			var z=parseInt(document.frm1.TotalCCPAld.value)+parseInt(2*j);
			if(document.frm1.TotalCCP.value>=z){
				var y=parseInt(document.frm1.TotalCCPAld.value)+parseInt(3*j);	
				if(document.frm1.TotalCCP.value>=y){
					document.frm1.TotalCCPAld.setAttribute("value",y);								
				}else{
					document.frm1.TotalCCPAld.setAttribute("value",z);
				}
...
function finalsave(){
	...
	if(t==0 && ec==0){
		...
		if(parseInt(cr)+3<=total){		
			...
			if(c!=0 && ttt==0){
				alert("Please select all choice in Elective(s)");		
				return false;
			}
		...
		if((tt-1)!=(t) ){
			alert("Please select all choice in Elective(PD)");		
			return false;
		}
		for(i=1;i<=ec;i++){
			if(document.getElementById("Select"+i).value==""){
		...
		if(c!=0 && ttt==0 ){
			alert("Please select all choice in Elective");		
			return false;
		}
	}
	var op=0;
			...
			qry=qry+" from ("; 
			 ...
			 qry=qry+" (select distinct C.semester Semester, 'SAP' SemesterType,nvl(C.SUBJECTTYPE,'C') SUBJECTTYPE, C.SUBJECTID SUBJECTID, B.Subject||' ('||B.SubjectCode||')' SUBJECT, A.COURSECREDITPOINT COURSECREDITPOINT, nvl(C.BASKET,'A')BASKET from NRSTUDENTFAILSUBJECTS C,SUBJECTMASTER B,OfferSubjectTagging A"; 
                 ...
                 qry=qry+" union "; 
								...
								qry=qry+"  and C.semester<"+(mSem-1)+"  AND A.institutecode=B.institutecode and A.subjectID=B.subjectID ) ";        qry=qry+"UNION (select distinct A.semester Semester, 'REG' SEMESTERTYPE, 'E' SUBJECTTYPE, A.SUBJECTID SUBJECTID, B.Subject||' ('||B.SubjectCode||')'||'***'||nvl (A.ELECTIVECODE,' ') SUBJECT, A.COURSECREDITPOINT COURSECREDITPOINT, nvl(A.BASKET,'D')BASKET from PR#ELECTIVESUBJECTS A,SUBJECTMASTER B"; 
 	...
 	if(!rss1.next())
		      ...
		      else if(rss1.getString("SENDTOHOD").equals("Y"))
			{
			...
			else
	      	{
				...
				<input type="test" name="studelecchoise" value="<%//=rss1.getString("NOOFELESUBJECTSCHOICES")%>" maxlength='2' size='2'  onKeyup="return limit(event);" onkeydown="if(event.keyCode==13){event.keyCode=9;return event.keyCode}" <%//=readonly%>> -->
				<%
				//}else{%>
				<!-- <FONT face=Arial size=2 color=black><STRONG>No. of elective(s) allowed as per curriculum structure/Maximum credit Limit:
</STRONG></FONT>&nbsp;&nbsp;
				<input type="test" name="studelecchoise" value="<%//=rss1.getString("NOOFELESUBJECTSCHOICES")%>" maxlength='2' size='2'  readonly>	 -->
...
/*getting draft values*/
					...
					else
					{
						...
						<select name='<%=mName10%>' id='Selecth<%=i%>' 							onChange="Clickedselll('<%=mSno%>','<%=mCourseCrPt%>','Selecth<%=i%>','<%=mBasket%>','<%=electivecount%>')"	style="Width:40">
								<%
...
i=0;
				...
				qry=qry+" group by A.ELECTIVECODE, A.BASKET order by cnt desc";
				...
				while(rsc.next())
				{
					...
					<%
					if(mElecCode.equals("PD"))
					{
						%>
						<INPUT TYPE="hidden" NAME="PDElective" value=<%=mTot%>>
						<%
					}else if(mElecCode.equals("PH"))
					{
						%>
						<INPUT TYPE="hidden" NAME="PHElective" value=<%=mTot%>>
						<%
					}else if(mElecCode.equals("MD"))
					{
						%>
						<INPUT TYPE="hidden" NAME="MDElective" value=<%=mTot%>>
						<%
					}
...
//out.println(mElecCode);
	      			      if(!mElective.equals(mElecCode))
						   {
							...
							-->
							<input type=hidden name='temp1<%=nn%>' value="<%=value%>" size=1 id="temp1<%=nn%>">
							<input type="hidden" value='Subjectf<%=nn%>'/>
							<select name='<%=mName10%>' id='Selectf<%=nn%>' onchange="Clickedsel11('<%=jpk++%>','<%=mCourseCrPt%>','Selectf<%=mElective%>','<%=mBasket%>','<%=mTot%>','<%=mElecCode%>')" style="Width:40">								
							<%
							//i++;	
							String st1="";
							%><option selected Value=''> </option><%
							for ( m=1;m<=mTot;m++)
							{
								if(mochoice==m)
								{
									...
									//out.println("hello"+mMaxCrLmtAld);
								}
									else{
										st1="";
										//out.println("hello"+mMaxCrLmtAld);
						...
						else
						{
							...
							<%String nn=mElective+""+jpk;%>
							<input Type=hidden id='check<%=++i%>' name='<%=mName1%>' value='Y' size=1 >
																<input type="hidden" value='Subjectf<%=nn%>'/>					
							<!--onclick="Clicked('<%=mSno%>','<%=mochoice%>')"-->
							<input type="hidden" name='temp1<%=nn%>' value="<%=value%>" size=1 id="temp1<%=nn%>">
...
/*getting draft values*/
						...
						if(pflag==0){	
								if ((mMaxCrLmtAld+3*3) <= mMaxCrLmt && mochoice>0){
									mMaxCrLmtAld=mMaxCrLmtAld+3*3;
								}else if ((mMaxCrLmtAld+2*3) <= mMaxCrLmt &&mochoice>0 ){
									mMaxCrLmtAld=mMaxCrLmtAld+2*3;
									//out.println("ddddddd");
								}
					...
					else
					{
						...
						<input type=hidden value="Select<%=i%>:::<%=electivecount%>" >
...
//---------------------------------------
	   ...
	   else
	   {
		...
		rsexam=db.getRowset(qry);
			if(rsexam.next())
		   {
			...
			qry=qry+" STUDENTREGISTRATION where StudentID='" +mDID+ "' and examcode='"+mExam+"' and  InstituteCode='" + mInst + "'  and nvl(REGALLOW,'N')='Y' ";
			rs=db.getRowset(qry);
 			//out.print(qry);
	...
	qry="Select to_char(Sysdate,'dd-mm-yyyy hh:mi PM')date1 from Dual";	
		...
		%>		</tr>
				...
				<font color=Green>
		<br><h3>You have automatically been registered for following</FONT><Br>
		...
		if(rs.next())
		{
			mSubSect=rs.getString(1);
				...
				qry=qry+" where A.institutecode='"+mInst +"' and A.Academicyear='"+mAcad+"' and A.programcode='"+mProg+"' and A.TAGGINGFOR='"+mTag+"' AND A.SECTIONBRANCH='"+mySect+"' ";
				qry=qry+" and A.semester='"+sc+"' AND A.BASKET='A' AND A.institutecode=B.institutecode and A.subjectID=B.subjectID  order by Subject";
				//out.print(qry);
				int mysno=0;	
				rs=db.getRowset(qry);
				//out.print(qry);
