...
<tr bgcolor="#c00000">
 ...
 <td align=center Title="Click on Status to Sort"><b><font color="White">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Status</font></b></td> 
  <td Title="Click on LT to Sort"><b><font color="White">Lecture + Tutorial</font></b></td> 
 <td Title="Click on Class Type to Sort"><b><font color="White">Class Type</font></b></td> 
</tr>
</thead>
<tbody>
<%
	qry="select to_char(ATTENDANCEDATE,'DD-MM-YYYY')ADate, nvl(ATTENDANCETYPE,'R')CType, nvl(ENTRYBYFACULTYNAME,' ')EName,decode(LTP,'L','Lecture','T','Tutorial','P','Practical') LTP,nvl(PRESENT,'N')Status from V#STUDENTATTENDANCE ";
	...
	qry=qry+" and LTP in ('L','T')";
	qry=qry+" and STUDENTID='"+mChkMemID+"' and nvl(DEACTIVE,'N')='N'";
	qry=qry+" union	select to_char(a.ATTENDANCEDATE,'DD-MM-YYYY')ADate,decode(LTP,'L','Lecture','T','Tutorial','P','Practical') LTP, nvl(a.ATTENDANCETYPE,'R')CType, nvl(b.Employeename,' ')EName, nvl(a.PRESENT,'N')Status from STUDENTPREVATTENDENCE a , Employeemaster b	where a.EXAMCODE='"+mExam+"' and a.EMPLOYEEID = B.EMPLOYEEID and a.ATTENDANCETYPE=Decode('"+mCType + "','ALL',a.ATTENDANCETYPE,'"+ mCType +"') and a.SUBJECTID='"+mSubj+"' and a.LTP in ('L','T') and a.STUDENTID='"+mChkMemID+"' and nvl(a.DEACTIVE,'N')='N' "; 
	rs=db.getRowset(qry);
