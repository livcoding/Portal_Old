...
String seqqry="Select WEBKIOSK.ShowLink('275','"+ mChkMemID+"','"+mChkMType+"','"+mRole+"','"+ mIPAddress +"') SL from dual";
  ...
  //----------------------
	...
	session.setAttribute("pacademicyear",request.getParameter("academicyear")==null?"":request.getParameter("academicyear"));
	...
	//session.setAttribute("ppgtransactionID",request.getParameter("")==null?"":request.getParameter(""));
...
<tr>
<td><FONT color=black>&nbsp; <FONT face=Arial size=2><STRONG>Registration No:</STRONG></FONT>&nbsp;&nbsp;&nbsp; </FONT></td>
 <td nowrap colspan="3"><FONT color=black>&nbsp; <FONT face=Arial size=2><%=regcode%></FONT> </FONT></td>
...
<form name="bankoptions" id="bankoptions" methods="post" action="makepayment.jsp" target="_blank" onSubmit="javascript:changeURL();">
...
String cctype="";
String ccvalue="";
String selnetbk="checked";
String cpoid="";

String bankvis="hidden";
String bpoid="";
String bpodesc="";
String bpocode="";
qry="select poid,pocode,podesc from pg#paymentoptions  where institutecode='"+mInstituteCode+"' and creditcardflag='Y' and nvl(deactive,'N')='N' order by seqid";
  rs=db.getRowset(qry);
  if(rs.next()){
  vis="visible";
  cpoid=rs.getString(1);
  cpocode=rs.getString(2);// not in use yet
  cpodesc=rs.getString(3);


selnetbk="";
*/
%>
<!-- for payment options from table
<td width="272" height="50" bgcolor="#F9F4C1">

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<Input Type="radio" name="options" style="visibility:"<%//=vis%>   id="options" value="C"  >
<%//=cpodesc%>
<input type="hidden" name="cpocode" id="cpocode" value="<%//=cpocode%>" />
<input type="hidden" name="cpoid" id="cpoid" value="<%//=cpoid%>" />
<input type="hidden" name="cpodesc" id="cpodesc" value="<%//=cpodesc%>" />
</td>
-->
  ...
  <!--
  <td width="129" bgcolor="#F9F4C1"><div align="right">
...
-->
  <%
  /*
qry="select poid,pocode,podesc from pg#paymentoptions  where institutecode='"+mInstituteCode+"' and NETBANKINGFLAG='Y' and nvl(deactive,'N')='N' order by seqid";
  rs=db.getRowset(qry);
  if(rs.next()){
  bankvis="visible";
  bpoid=rs.getString(1);
 bpocode=rs.getString(2);
 bpodesc=rs.getString(3);
...
<!--
&nbsp;&nbsp;&nbsp;&nbsp;
bank name option from table
<Input Type="radio" name="options" style="visibility:<%//=bankvis%>"   id="options2" value="N"   <%//=selnetbk%> >
<%//=bpodesc%>

<input type="hidden" name="bpoid" id="bpoid" value="<%//=bpoid%>" />
<input type="hidden" name="netbanks" id="netbanks" value="<%//=bpocode%>" />
</td>
  -->
  <%//}%>
...
<br>
<!--<i><font size="-1">Note:Transaction Charges(if any) will be show on next page.</font></i>-->
<!--<A href="FeePaymentTransactionCharges.html" target="_new"  alt="Click here to view on line Transaction Charges details"><font size=4><b>Click to view On Line Transaction Charge detail</a></b></font> -->
</td>
</tr>
...
<!--
<input type="button" id="btn1" name="btn1"  style="height:35px;" onClick="javascript:submitForm()"   value="Pay Now">
<input type="submit" id="btn1" name="btn1" style="height:35px;" onClick="return  validateValues()"  disabled value="Procced For Payment">
-->
</td>
</tr>
</Table>
