<%-- 
    Document   : EmpBookRoom
    Created on : 25 Nov, 2016, 11:03:33 AM
    Author     : VIVEK.SONI
--%>
	...
	if(request.getParameter("x")!=null)
	{
		...
		if (mDate1.length()==10 && mDate2.length()==10  &&  GlobalFunctions.iSValidDate(mDate1)==true && GlobalFunctions.iSValidDate(mDate2))
		{
			...
			mRoomCode=request.getParameter("RoomCode").toString().trim();
                        ...
                        qry="select Roomcode from UserWiseRoomTagging where ROOMCODE='"+mRoomCode+"' and EMPLOYEEID='"+mDMemberID+"'";
                        RsChk1= db.getRowset(qry);

			while(RsChk1.next()){
                        isBothfind=RsChk1.getString("Roomcode");
                        }

                        qry="select Roomcode from UserWiseRoomTagging where ROOMCODE='"+mRoomCode+"'";
                        RsChk1= db.getRowset(qry);

			while(RsChk1.next()){
                        onlyCode=RsChk1.getString("Roomcode");
                        }

                        if(isBothfind!=null || onlyCode==null){
...
while(rs.next())
	...
	<%
				...
				}// closing Of Checking of Room Tagging with MemberCode...
                                else
				{
				out.print("<br><img src='../../Images/Error1.jpg'>");
				out.print("&nbsp;&nbsp;&nbsp <b><font size=3 face='Arial' color='Red'> Room Booking Not Allowed.. </font> <br>");
