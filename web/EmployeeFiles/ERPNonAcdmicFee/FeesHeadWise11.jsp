...
<TITLE>#### <%=mHead%> [ Student Fee Head Wise-1] </TITLE>
		...
		if (RsChk1.next() && RsChk1.getString("SL").equals("Y"))
		   {
                    ...
                    <form name="frm" method="get" onsubmit="return validateform()">
                        <br><br>
                        <div align="center">
                            <b>  <font color="red">Fees  Collection Fee  </font></b> 
                        </div>
                        <br>
...
<%  qry="select FEEHEADS,HEADDESC,RATE from NA#FeeHeads  Where InstituteCode='"+mInstCode+"' and nvl(Deactive,'N')='N' ";
                    ...
                    {
                        %>
                        <select name=feehead tabindex="1" id="feehead" style="WIDTH: 230px" onmousedown="this.value='';" onchange="jsFunction(this.value);"  >
                            <OPTION selected value=""><b><------------Select FeeHead------------></b></option>
                        <%
                        int cnter=0;
                        while(rs.next())
                        {
                            ...
                            // String FeeDesc1=rs.getString("RATE");
...
<font size=1 color=teal>(DD-MM-YYYY)</font></td> 
                                <td  colspan=1><input type="submit" value="Submit" onClick="return validateform();"></td>
                                
                         
<%
    if(request.getParameter("feehead")!=null&&request.getParameter("txtamount")!=null&&request.getParameter("date")!=null)
    {
        String string = request.getParameter("feehead").trim();
        String[] parts = string.split("==");
        String part1 = parts[0]; 
        String part2 = parts[1];
        try{
        qry="SELECT * FROM NA#STUDENTFeedetail WHERE FEEHEADS='"+part1+"' AND to_char(FEEHEADDATE,'dd-mm-yyyy')='"+request.getParameter("date").trim()+"' AND INSTITUTECODE='"+mInstCode+"' AND VOUCHERCODE  IS  NULL";
        rs=db.getRowset(qry);
        if(rs.next())
            {
            %>
                  <td  colspan=1> <p align="center">  <a href="../ERPNonAcdmicFee/FeesHeadLink.jsp">Student FeeHead Link</a></p> </td></tr>
...
<%  qry="select distinct PROGRAMCODE from STUDENTREGISTRATION     where INSTITUTECODE='"+mInstCode+"' order by PROGRAMCODE ";
 ...
 <%  qry="select distinct  SECTIONBRANCH from STUDENTREGISTRATION   where INSTITUTECODE='"+mInstCode+"' order by SECTIONBRANCH ";
                        ...
                        %>
                            ...
                            <tr>
                                <td colspan="7" align="center">
                                  ...
                                  // {
                                  %>
                                  <input type="submit" value="Submit11111" name="ListForSave">
                                  <%
                                  //}
                                  %>
                                </td>
   ...
   <form action="FeeHeadwiseAction.jsp">
                        <%
                         if (request.getParameter("x")!=null&&request.getParameter("ListForSave")!=null){%>
                         <table width="60%" align="center" border="1" cellpadding="2" cellspacing="2">
                                 ...
                                 // session.setAttribute("feehead", mfeehead);
                                  if(rs.next()){
                                   ...
                                   qry="INSERT INTO NA#STUDENTFeedetail(INSTITUTECODE, FEEHEADS, FEEHEADDATE,REGCODE,STUDENTID,FEEHEADRATE,FEEHEADQTY,AMOUNT,ENTRYBY,ENTRYDATE,DEACTIVE,LASTUPDATE) " +
                                           "VALUES('"+mInstCode+"','"+mfeehead+"','"+adate+"','REGCODE','"+mstdid+"','"+mamt+"','1','"+mamt+"','"+mMemberName+"',sysdate,'N',sysdate)";
                           ...
                           <%  }else  if(selection!=null && selection.equalsIgnoreCase("grp1")){
                            ...
                            <% }
                             ...
                             <p align="center">  <a href="http://localhost:8084/ROOT/EmployeeFiles/ERPNonAcdmicFee/FeesHeadWise.jsp">Reset</a></p>
