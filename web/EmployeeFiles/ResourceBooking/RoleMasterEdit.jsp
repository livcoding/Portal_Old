...
{
listQry="SELECT ROLEID,ROLECODE, ROLEDESC, SEQID,NVL(DEACTIVE,'N') DEACTIVE, decode(DEACTIVE,'Y','Yes','No')ACT_DCT FROM RS#ROLEMASTER where RoleId='"+RoleId+"' Order by 4";
rs=db.getRowset(listQry);
while(rs.next()){
RoleCode=rs.getString("ROLECODE")==null?"":rs.getString("ROLECODE")+"";
RoleDes=rs.getString("ROLEDESC")==null?"":rs.getString("ROLEDESC")+"";
active=rs.getString("DEACTIVE");//=="N"?"N":"Y";
}

%>
		<form name="frm1"  method="get" action="RoleMaster.jsp" >
		<input id="x" name="x" type=hidden> <input id="z" name="z" type=hidden>
		 ...
		 <tr><td align="center">
		<font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: arial"><U>Role Master</U>
		</td></tr></table>
		...
		<td align="center" width="80%" colspan="4"><FONT color=black face=Arial size=2>&nbsp;Role Code/Description<font	color=red size=5px>*</font>
		<input type="text" value="<%=RoleCode%>" name="rolecode" id="rolecode"  style="width:75px;" >
		<input type="text" value="<%=RoleDes%>" name="roledes" id="roledes"  style="width:160px;">
    	</td>
		...
		<input type="Reset" name="reset" value="Reset"  onClick="clearForm()"  accesskey="r" title="Alt+r"></td>	
		</td></tr>
		</table>
		</form>
