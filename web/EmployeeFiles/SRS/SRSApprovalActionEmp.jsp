...
int More=1, mLevel=0, mNoStudent=0;
String mInst="", mT="5";
...
<center><font size=4>Approval Status [ <b>Not Approved</b> status - if suggestion contains abusing word]</font></center>
<hr>
<table border=1 cellpadding=3 cellspacing=1 rules="All" align=center>
<tr bgcolor=#ff8c00>
<td><b>Program</b></td>
<td><b>Section-<br>Subsection</b></td>
<td><b>Subject</b></td>
<td><b>LTP</b></td>
<td><b>Faculty</b></td>
<td><b>Status</b></td>
...
{  //1
  ...
  //-----------------------------
		...
		//----------------------
			...
			int kk=0, kk1=0, mTotalRec = 0;
			String mApproved="", mExamCode="", mINSTITUTECODE="", mEmployeeID="", mPROGRAMCODE="";
			String mSUBJECTCODE="", mSUBJECTID="", mSRSEVENTCODE="", mSec="", mSubSec="", mEName=" ";
			...
			{  //3
				...
				for (kk=1;kk<=mTotalRec ;kk++)
				{
					...
					mSUBJECTID= request.getParameter(mName3);
					mSec= request.getParameter(mName8);
					mSubSec= request.getParameter(mName9);
					mSUBJECTCODE= request.getParameter(mName10);
					...
					if(mApproved!=null)
					{
					//   Find abusing words in the respective feedback
		
						qry="select distinct nvl(upper(ABUSINGWORD),' ')ABUSINGWORD,nvl(ABUSINGLEVEL,0)ABUSINGLEVEL FROM ABUSINGWORDS ";
						qry=qry +" where nvl(DEACTIVE,'N')='N' ";
						rs=db.getRowset(qry);
						More=1;
						mLevel=0;
						while(rs.next()&& More==1)
						{
							qry1="select 'y' from V#SRSEVENTSUGGESTION where INSTITUTECODE='"+mINSTITUTECODE+"' AND ";
							qry1=qry1 +" EXAMCODE='"+mExamCode+"' AND SRSEVENTCODE='"+mSRSEVENTCODE+"' and";
							qry1=qry1 +" PROGRAMCODE='"+mPROGRAMCODE+"' AND SUBJECTID='"+mSUBJECTID+"' AND ";
							qry1=qry1 +" EmployeeID='"+mEmployeeID+"' And SectionBranch='"+mSec+"' and SubSectionCode='"+mSubSec+"' And LTP IN("+mLTP+")";
							qry1=qry1 +" and upper(SUGGESTION) like '"+'%'+rs.getString("ABUSINGWORD")+'%'+"' AND NVL(ISABUSING,'N')='N'";
 							rs1=db.getRowset(qry1);
							while(rs1.next())
							{
								mLevel=rs.getInt("ABUSINGLEVEL");
								More=0;
							}
						}
						if(More>0)
						{
							...
							while(RsFSTID.next())
				  			{
								qry="UPDATE SRSEVENTS SET APPROVED='Y' ,APPROVEBY='"+mMemberID+"' , ";
								qry=qry +" APPROVEDDATE=SYSDATE WHERE FSTID='"+RsFSTID.getString(1)+"' and SRSEVENTCODE='"+mSRSEVENTCODE+"'";
								//out.print(qry);
								n=db.update(qry);

								//--------- Log Entry
								//-----------------
								db.saveTransLog(mINSTITUTECODE,mLogEntryMemberID,mLogEntryMemberType ,"SRS PRINTING APPROVAL", "SRSEventCode:"+mSRSEVENTCODE+"SubjectCode:"+mSUBJECTCODE+"EmployeeID:"+mEmployeeID+"Sec-SubSec.:"+mSec+"-"+mSubSec+"Fstid"+RsFSTID.getString("ltp"), "No MAC Address" , mIPAddress);
								//-----------------
							}
						}
						else
						{
							%>
							<tr><td><%=mPROGRAMCODE%></td><td><%=mSec%>-<%=mSubSec%></td><td><%=mSUBJECTCODE%>&nbsp; &nbsp; &nbsp;&nbsp;</td><td><%=mLTP%></td><td><font size=2><%=mEName%></font></td><td><a href="SrsEventSuggestionAbusing.jsp?ICD=<%=mINSTITUTECODE%>&amp;SEC=<%=mSRSEVENTCODE%>&amp;EC=<%=mExamCode%>&amp;SC=<%=mSUBJECTCODE%>&amp;SID%=mSUBJECTID>&amp;LTP=<%=GlobalFunctions.getSortedLTPSQ(mLTP)%>&amp;EMPID=<%=mEmployeeID%>&amp;SB=<%=mSec%>&amp;SSC=<%=mSubSec%>&amp;PC=<%=mPROGRAMCODE%>"target=_New><font color=red>Abusing Word Level:<%=mLevel%></font></a></td></tr>
							<%
						}
						if (n>0)
						{
							%>
							<tr><td><%=mPROGRAMCODE%></td><td><%=mSec%>-<%=mSubSec%></td><td><%=mSUBJECTCODE%>&nbsp; &nbsp; &nbsp;&nbsp;</td><td><%=GlobalFunctions.getSortedLTPSQ(mLTP)%></td><td><font size=2><%=mEName%></font></td><td><font color=Green>Approved</font></td></tr>
							<%
						}
					//} //  abusing block
					}
			...
			else
			{
				...
				out.print(" &nbsp;&nbsp;&nbsp <b><font size=3 face='Arial' color='Red'>Please select check box to approve the respective SRS!</font> <br>");
			}
			//out.print("Hello - "+mNoStudent);

			out.print("<center><a href='SrsApprovalEmployee.jsp'><B><Font color=blue face=verdana size=3>Back & Next</Font></B></a></center>");

			response.sendRedirect("SrsApprovalEmployee.jsp?NoStudent="+mNoStudent+"&mCount="+mT+"");
			//-----------------------------
			//-- Enable Security Page Level  
			//-----------------------------
		}
