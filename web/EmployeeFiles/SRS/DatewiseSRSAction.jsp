...
String mWidth="100";
ResultSet rsi=null,rs2=null,rs3=null;
String qry="";
String mInst="",mSubj="",mSubjID="",mSubhead="";
String mFac="",mdate="";
String mMemberID="";
String mMemberType="";
String mMemberCode="";
...
<HTML>
<head>
<TITLE>#### <%=mHead%> [ Student Reaction Survey detailed report(SRS Event Date wise) ] </TITLE>
...
<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy">SRS Teacher Rating Detail Report (SRS Event Date wise)</font></td></tr>
</TABLE>
...
<%
  ...
  mdate=request.getParameter("date").toString().trim();
...
if (rs.next())
{
	...
	%>
<b>
&nbsp;&nbsp; SRS Event date <%=mdate%><br>
&nbsp;&nbsp; Faculty Name:&nbsp;<%=mFacltyNM%><br>
&nbsp;&nbsp; Subject :&nbsp;<%=mSubj%> ( <%=GlobalFunctions.getLTPDescWSQ(myLTP)%>)<br>
...
<%	
	...
	qry=qry+ " and A.Employeeid='"+mFac+"' and trunc(A.ENTRYDATE)=to_date('"+mdate+"','dd-mm-yyyy') ";
	qry=qry+ " and A.subjectid='"+mSubjID+"' and '"+myLTP+"' like '%'||A.LTP||'%' and nvl(A.naselected,'N')='N' ";
...
//-------------------------------
			...
			{	
				...
				qry=qry+ " EMPLOYEEID='"+mFac+"' and trunc(ENTRYDATE)=to_date('"+mdate+"','dd-mm-yyyy')";
				qry=qry+ " and SRSEVENTCODE='"+mSrsevent+"' and '"+myLTP+"' like '%'||LTP||'%' and ";
...
//	} // closing of request.getParameter("x")!=null
	...
	qry=qry+ " and Employeeid='"+mFac+"' and trunc(ENTRYDATE)=to_date('"+mdate+"','dd-mm-yyyy')";
