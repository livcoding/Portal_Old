...
long mL=0,mPercL=0,mLP=0;
					...
					String qry2="Select ltp, academicyear, semester,enrollmentno, studentname, studentid,fstid,"+qryadd+","+sumadd+"  Weightage ";
					if(!sumadd1.equals(""))
						{
					qry2+="	, "+sumadd1+" TAWeightage ";
						}					
					qry2+=" from ( select  a.LTP, a.academicyear, a.semester,";
					...
					qry2+=" order by enrollmentno,studentname)xyz group by  LTP,academicyear,semester,enrollmentno,studentname,studentid,fstid  order by enrollmentno, studentname   ";
					//out.print(qry2);
					String qryrow="";
...
qry1="select  LTP,fstid from V#StudentLTPDetail a where SubjectID= '"+mSubjectID+"' and EXAMCODE= '"+mExamCode1+"' AND INSTITUTECODE='"+mInst+"' and a.studentid='"+mSID+"' and a.ltp='"+mLTP+"' group by LTP,fstid";
...
qry1="select distinct fstid from StudentPrevAttendence a where SubjectID= '"+mSubjectID+"' and EXAMCODE= '"+mExamCode1+"' AND  a.LTP='"+mLTP+"' ";
qry1=qry1+" and INSTITUTECODE='"+mInst+"' and a.studentid='"+mSID+"' and  trunc(attendancedate)=(select max(trunc(attendancedate)) from StudentPrevAttendence a where SubjectID= '"+mSubjectID+"' and EXAMCODE= '"+mExamCode1+"'  AND  a.LTP='"+mLTP+"' and INSTITUTECODE='"+mInst+"' and a.studentid='"+mSID+"' ) ";
rs1=db.getRowset(qry1);
	while(rs1.next())
		{
...
qry=" SELECT   nvl(count(pcount ),0)  pcount FROM (select distinct  CLASSTIMEFROM Pcount from V#STUDENTATTENDANCE a where SubjectID= '"+mSubjectID+"'  and LTP='"+mLTP+"' and EXAMCODE= '"+mExamCode1+"'  AND  ( A.FSTID='"+mLFSTID+"'   OR (A.FSTID  IN ( SELECT b.mergewithfstid FROM FacultySubjecttagging b where  b.examcode='"+mExamCode1+"' and b.institutecode='"+mInst+"' and b.SUBJECTID='"+mSubjectID+"' and  b.LTP='"+mLTP+"' and b.FSTID='"+mLFSTID+"')))  "; 
qry=qry+" and (("+QrySem+">1) or ("+QrySem+"=1 and trunc(A.attendancedate) >= trunc(TO_Date(nvl('"+mREGCONFIRMATIONDATE+"',ATTENDANCEDATE),'dd-mm-yyyy'))))  and a.studentid<>'"+mSID+"' ";
 qry=qry+" and trunc(a.classtimefrom)<  NVL((SELECT min(trunc(c.classtimefrom)) from v#STUDENTATTENDANCE c where c.studentid='"+mSID+"' and c.SubjectID= '"+mSubjectID+"'  and c.LTP='"+mLTP+"' and c.EXAMCODE= '"+mExamCode1+"' and c.institutecode='"+mInst+"' ),a.classtimefrom)";
qry=qry+" and INSTITUTECODE='"+mInst+"'   and nvl(DEACTIVE,'N')='N' and trunc(a.classtimefrom)<  NVL((                              SELECT min(trunc(c.classtimefrom)) from STUDENTPREVATTENDENCE c where  c.studentid='"+mSID+"' and  c.SubjectID= '"+mSubjectID+"'  and c.LTP='"+mLTP+"' and c.EXAMCODE= '"+mExamCode1+"' and c.institutecode='"+mInst+"' ),a.classtimefrom) ";
...
qry=" select count( distinct CLASSTIMEFROM) tot  from V#StudentAttendance  a where SubjectID= '"+mSubjectID+"'  and LTP='"+mLTP+"' and EXAMCODE=  '"+mExamCode1+"' ";
qry=qry+"  AND  A.FSTID='"+prevLFSTID+"'   and INSTITUTECODE='"+mInst+"'   and a.studentid<>'"+mSID+"' ";
qry=qry+" and not exists (select 1 from STUDENTPREVATTENDENCE c where c.studentid='"+mSID+"' and c.SubjectID= '"+mSubjectID+"'  and c.LTP='"+mLTP+"' and c.EXAMCODE= '"+mExamCode1+"' and c.institutecode='"+mInst+"' and  trunc(c.CLASSTIMEFROM)= trunc(a.CLASSTIMEFROM))";
...
qry=qry+"  and c.LTP='"+mLTP+"' and c.EXAMCODE=  '"+mExamCode1+"' and c.institutecode='"+mInst+"'   and c.fstid='"+prevLFSTID+"' ) ";
//if(mSID.equals("J1281100708"))
//	out.print(qry);
rs1=db.getRowset(qry);
...
qry1="SELECT   count(pcount ) pcount FROM (select distinct  CLASSTIMEFROM pcount from V#STUDENTATTENDANCE a   where SubjectID= '"+mSubjectID+"' and EXAMCODE= '"+mExamCode1+"' AND  a.ltp='"+mLTP+"' and a.studentid='"+mSID+"'   ";
...
qry1=qry1+" UNION   ";
qry1=qry1+" select   distinct CLASSTIMEFROM  pcount from STUDENTPREVATTENDENCE where  subjectid ='"+mSubjectID+"'     AND ltp ='"+mLTP+"'    ";
...
qry1="SELECT   count(pcount ) pcount FROM (select distinct   CLASSTIMEFROM pcount from V#STUDENTATTENDANCE a   where SubjectID= '"+mSubjectID+"' and EXAMCODE= '"+mExamCode1+"' AND   a.ltp='L' and a.studentid='"+mSID+"' and nvl(a.present,'N')='Y' ";
qry1=qry1+" and INSTITUTECODE='"+mInst+"'  and nvl(DEACTIVE,'N')='N' and ( ("+QrySem+"=1 and trunc(ATTENDANCEDATE)>=TO_Date(nvl('"+mREGCONFIRMATIONDATE+"',ATTENDANCEDATE),'dd-mm-yyyy'))  or  ("+QrySem+">1) ) ";
qry1=qry1+" UNION   ";
qry1=qry1+" select  distinct  CLASSTIMEFROM  pcount from STUDENTPREVATTENDENCE where  subjectid ='"+mSubjectID+"'     AND ltp ='L'    ";
...
//out.print(mL+" ::mL "+mLP);
		...
		mPercL=Math.round(((mLP*100)/mL));
			...
			//to add marks in TA Marks		
							...
							qry10="SELECT  distinct nvl(b.academicyear,'N')academicyear ,A.EXAMEVENTCODE,nvl(A.MASSCUTSAPPLICABLE,'N')MASSCUTSAPPLICABLE,NVL(A.NEGATIVEMASSCUTSALLOWED,'N')NEGATIVEMASSCUTSALLOWED ,b.MASSCUTS MASSCUTS1,nvl(b.typeofaction,' ')typeofaction  FROM EXAMEVENTMASTER A,MASSCUTSCRITERIA B,V#STUDENTEVENTSUBJECTMARKS c WHERE							A.EXAMCODE='"+mExamCode1+"'  and b.EXAMCODE=c.EXAMCODE and a.EXAMCODE=c.EXAMCODE and c.STUDENTID='"+rs2.getString("studentid")+"'  and a.INSTITUTECODE=c.INSTITUTECODE and nvl(c.DEACTIVE,'N')='N' AND A.INSTITUTECODE='"+mInst+"' AND A.EXAMCODE=B.EXAMCODE AND A.EXAMEVENTCODE=B.EXAMEVENTCODE and a.INSTITUTECODE=b.INSTITUTECODE  	 and TO_NUMBER ("+mPercL+") between TO_NUMBER (b.attendancepercentagefrom) and  TO_NUMBER (b.attendancepercentageto)  ";
							...
							rs10=db.getRowset(qry10);
								...
								mNetMarks=mTATotalMarks ;
											...
											/*	mTotalMarksAdd=mTATotalMarks+mMarksAdd;	
												out.print(mTotalMarksAdd + " : TAWeight : "+TAWeight);
											if(TAWeight < mTotalMarksAdd )
											{
													mNetMarks=TAWeight;
											}
		...
		// TO SUBTRACT THE MARKS CUT
							...
							qrys="SELECT  distinct nvl(b.academicyear,'N')academicyear ,A.EXAMEVENTCODE,nvl(A.MASSCUTSAPPLICABLE,'N')MASSCUTSAPPLICABLE,NVL(A.NEGATIVEMASSCUTSALLOWED,'N')NEGATIVEMASSCUTSALLOWED ,b.MASSCUTS,nvl(b.typeofaction,' ')typeofaction  FROM EXAMEVENTMASTER A,MASSCUTSCRITERIA B,V#STUDENTEVENTSUBJECTMARKS c WHERE							A.EXAMCODE='"+mExamCode1+"'  and b.EXAMCODE=c.EXAMCODE and a.EXAMCODE=c.EXAMCODE and c.STUDENTID='"+rs2.getString("studentid")+"'  and a.INSTITUTECODE=c.INSTITUTECODE and nvl(c.DEACTIVE,'N')='N' AND A.INSTITUTECODE='"+mInst+"' AND A.EXAMCODE=B.EXAMCODE AND A.EXAMEVENTCODE=B.EXAMEVENTCODE and a.INSTITUTECODE=b.INSTITUTECODE  	 and TO_NUMBER ("+mPercL+") between TO_NUMBER (b.attendancepercentagefrom) and  TO_NUMBER (b.attendancepercentageto)  and nvl(b.TYPEOFACTION,'N')='-' AND C.PROGRAMCODE NOT IN('PHD','PHDP','PHDS') ";
	...
	//out.print(mNetMarks+" ------mNetMarks" + mMassCutMarks +" :: mTATotalMarks :: "+mTATotalMarks);

//	out.print(mFinalMarks11+"sdfsf"+mFinalMarks1);

										if(mMassCutMarks > mTATotalMarks )
										{
											//mMassCutMarks=mNetMarks;
											mNetMarks=0;
								...
								//CHECK OF ALREADY SAVED DATA
								if(mFinalMarks11==1 ||  mGrade1.equals(" ") )
										{
										mFinalMarks1=mFinalMarks1-mTATotalMarks+mNetMarks;
							//		out.print(mNetMarks+" ------mNetMarks -- mFinalMarks1 :" + mFinalMarks1 +" :: mTATotalMarks :: "+mTATotalMarks);	
										}
		...
		//out.print(mTotalMarksAdd+" :: mTotalMarksAdd :: "+mNetMarks + ": TAWeight :"+TAWeight  );
									...
									//	out.print(mMassCutMarks);
										...
										<td align='center'> <font size=2> <%=mPercL%> (%)</font></td>	
										...
										<td align='center'><INPUT TYPE="text" readonly NAME="FinalMarks<%=rs2.getString("rn")%>" value="<%=Math.round(mFinalMarks1)%>"  readonly size=2 ></td>		
