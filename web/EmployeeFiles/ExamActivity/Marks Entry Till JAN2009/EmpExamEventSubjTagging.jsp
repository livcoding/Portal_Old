...
try
{
  ...
  //----------------------
		...
		<!--*********Exam Event Code**********-->
			...
			onclick="ChangeCombo(Event.value,DataCombo,SubEvent);" 
		...
		<!-- *****************DataCombo******************* -->
			...
			try
			{
			 qry="Select Distinct NVL(SUBEVENTCODE,' ')EXAMEVENTCODE,nvl(EXAMEVENTCODE,' ')EXAMEVENT1, SUBEVENTDESC ExamEvent from EXAMSUBEVENTMASTER WHERE ExamCode='"+mExamid+"' and nvl(Deactive,'N')='N' order by EXAMEVENT1 ";
			// out.println(qry);
			 rs=db.getRowset(qry);
			 %>
...
//*******************************************************************
					...
					//out.print(qry);
					%>
					<table align=center border=1 cellsapcing=0 cellpadding=1>
					<%
					String QryFSTID="";
					while(rss.next())
					{
					...
					{
						%>
						<tr bgcolor=white><td><font size=2 face=arial><%=rs1.getString("EMPNM")%> : <%=rs1.getString("PROGRAMCODE")%> (<%=rs1.getString("SECBR")%>-<%=rs1.getString("SUBSEC")%>)</b></font></td></tr>
						<%
					}
					//out.print(QryFSTID);
					%>
...
//*******************************************************************
					...
					else
					{
						...
						out.print(" &nbsp;&nbsp;&nbsp <b><font size=3 face='Arial' color='Red'>Please Select An Event!</font> <br>");
					}
