...
String TAN="";
String TAN1="";
String mPrevExamCode="";
String mHead="",Rrevied1="",mprog="" ,StudidOL="''",StudidOLAP="''" ,StudidOLFP="''" ;
String mSubJ="",mEmplo="",mDEL="",qry_gradeValue="";
...
int pregraded=0;
String del2="",del1="",del3="",del4="",del5="",xBRK="",oldexamcode="";
...
int mFcount=0;
Map oldbound=new LinkedHashMap();
...
String mDebarGrade="",mDebarStudID="";
double mDebarWeight=0;
...
<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><b>Grade Calculation Supplementry  .</b></TD>
</font></td></tr>
</TABLE>
...
qry="select nvl(GRADE,'') GRADE,nvl(RECOMMENDEDTO,0) RECOMMENDEDTO  from gradecalculationgrades where SUBJECTID='"+mSubjectCode+"' and examcode='"+mExamCode+"'";
rs=db.getRowset(qry);
//out.print(qry+"<br><br>");
while(rs.next())
{
oldbound.put(rs.getString("GRADE")==null?"":rs.getString("GRADE").toString().trim(),rs.getString("RECOMMENDEDTO")==null?0:rs.getString("RECOMMENDEDTO").toString().trim());
}
...
while(rs2.next())
{
	...
	//debar students
	//int DRejected=0;
qry1=" select distinct NVL(A.GRADE,'N')GRADE from DEBARSTUDENTDETAIL a  WHERE  a.INSTITUTECODE='"+mInst+"' and a.EXAMCODE='"+mExamCode+"'  and a.STUDENTID='"+Studid+"' AND A.SUBJECTID='"+mSubjectCode+"' and  NVL(A.PRORATA,'N')='N' AND nvl(a.DEACTIVE,'N')='N' AND (NVL(A.MEDICALCASE,'Y')='Y'   OR  NVL(A.ABSENTCASE,'N')='N' OR NVL(A.UFM,'N')='N' OR NVL (a.DEBAR, 'N') = 'Y' ) AND A.GRADE IS NOT NULL ";
 //out.print(qry1+"@@@@@####<br>");
...
//out.print(mDeviationwithoutround+"mDeviationwithoutround"+mDeviation+"mDeviation"+"<BR>");
     ...
     qry="select  examcode from gradecalculation where break#slno like '%"+mSubjectCode+"%'   and" +
        " (examcode not like '%SUP%' and examcode not like '%SUM%') and institutecode='"+mInst+"' " +
        "   order by approveddate desc";
     rs=db.getRowset(qry);
     if(rs.next())
     {
     oldexamcode=rs.getString("examcode");
     }

    Map<String,String> map_oldboundy=new LinkedHashMap<String,String>();

     qry_gradeValue="select nvl(GRADE,'N') GRADE,nvl(RECOMMENDEDTO,0) RECOMMENDEDTO ,nvl(finalcount,'') finalcount from gradecalculationgrades where examcode='"+oldexamcode+"' " +
             "and subjectid='"+mSubjectCode+"' and institutecode='"+mInst+"' order by RECOMMENDEDTO desc";
//  out.print(qry_gradeValue);
      rs=db.getRowset(qry_gradeValue);
        while(rs.next())
        {
        map_oldboundy.put(rs.getString("GRADE"),rs.getInt("RECOMMENDEDTO")+"@@"+rs.getInt("finalcount"));
        }



     %>
	<div id="oldgradeboundry">
    <table ALIGN=CENTER rules=COLUMNS WIDTH=76% CELLSPACING=0 BORDER=1>
    <tr><td nowrap colspan='10'>Old Exam Code:<%=oldexamcode%></td></tr>
    <tr><td nowrap >Old Grades</td>
    <%
  for (Map.Entry<String,String> entry : map_oldboundy.entrySet()) {
   %>
    <td><%=entry.getKey()==null?"":entry.getKey()%></td>
   <%
}
%></tr>
    ...
    <tr><td>Old Range</td>
    <%
  for (Map.Entry entry : map_oldboundy.entrySet()) {
   %>
    <td><%=entry.getValue()==null?"":entry.getValue().toString().split("@@")[0]%></td>
   <%
}
%></tr>
  ...
  for (Map.Entry entry : map_oldboundy.entrySet()) {
   %>
    <td><%=entry.getValue()==null?"":entry.getValue().toString().split("@@")[1]%></td>
   <%
}
%></tr>
...
%>
<TD ALIGN=CENTER><%=Marks80%></td><INPUT TYPE="hidden"  id="chk1" NAME="chk1" value="<%=Marks80%>"></td>
...
<TD ALIGN=CENTER> <%=Marks10%></td><INPUT TYPE="hidden"  id="chk2" NAME="chk2" value="<%=Marks10%>"></td>
...
<TD ALIGN=CENTER>  <%=MarksB20%></td><INPUT TYPE="hidden"  id="chk3" NAME="chk3" value="<%=MarksB20%>"></td>
...
%>
 ...
 <TD ALIGN=CENTER> <%=MarksB25%></td><INPUT TYPE="hidden"  id="chk4" NAME="chk4" value="<%=MarksB25%>"></td>
...
%>
 ...
 <TD ALIGN=CENTER>   <%=MarksC25%></td><INPUT TYPE="hidden"  id="chk5" NAME="chk5" value="<%=MarksC25%>"></td>
...
%>
 ...
 <TD ALIGN=CENTER>   <%=MarksC15%></td><INPUT TYPE="hidden"  id="chk6" NAME="chk6" value="<%=MarksC15%>"></td>
...
%>
 ...
 <TD ALIGN=CENTER>  <%=CALVALUEF%></td><INPUT TYPE="hidden"  id="chk7" NAME="chk7" value="<%=CALVALUEF%>"></td>
...
<TD ALIGN=CENTER><%=Marks30%></td><INPUT TYPE="hidden"  id="chk8" NAME="chk8" value="<%=Marks30%>"></td>
...
<!-- <TD ALIGN=CENTER >   <input type="text" style="text-align:right" name="<%=mNameLower%>" id="<%=mNameLower%>"  size=3 maxlength=5  value="<%=MarksMedical%>" disabled >  </TD> -->
	...
	%>
		...
		<TD ALIGN=CENTER ><input type="text" style="text-align:right"  id="<%=mNameLower%>"  name="<%=mNameLower%>" id="<%=mNameLower%>"  size=3 maxlength=5 onKeyup="isInteger(this.value)" onkeypress="return isNumber(event);" onblur="return isIntegerAP(this.value)" value="<%=oldbound.get("A+")==null?"":oldbound.get("A+")%>" ></TD>
<%
		...
		<TD ALIGN=CENTER ><input type="text" style="text-align:right"  id="<%=mNameLower%>"  name="<%=mNameLower%>" id="<%=mNameLower%>"  size=3 maxlength=5 onKeyup="isInteger(this.value)"  onblur="return isIntegerD(this.value)"  onkeypress="return isNumber(event)" value="<%=oldbound.get("D")==null?"":oldbound.get("D")%>"  >   </TD>
		...
		%>
		<TD ALIGN=CENTER ><input type="text" style="text-align:right"   name="<%=mNameLower%>" id="<%=mNameLower%>"  size=3 maxlength=5   onKeyup="isInteger(this.value)" onkeypress="return isNumber(event)" value="<%=oldbound.get("A")==null?"":oldbound.get("A")%>"></TD>

		<%
		}else if(rs1.getString("GRADE").equals("B+")){

		%>
		<TD ALIGN=CENTER ><input type="text" style="text-align:right"   name="<%=mNameLower%>" id="<%=mNameLower%>"  size=3 maxlength=5   onKeyup="isInteger(this.value)" onkeypress="return isNumber(event)" value="<%=oldbound.get("B+")==null?"":oldbound.get("B+")%>"></TD>
		...
		%>
		<TD ALIGN=CENTER ><input type="text" style="text-align:right"   name="<%=mNameLower%>" id="<%=mNameLower%>"  size=3 maxlength=5   onKeyup="isInteger(this.value)" onkeypress="return isNumber(event)" value="<%=oldbound.get("C+")==null?"":oldbound.get("C+")%>"></TD>
