...
<html>
<head>
<TITLE>#### <%=mHead%> [ Event-Wise Project Marks Finalization ] </TITLE>
...
String mMOP="",mlistorder="",mctr="",qrys="",mSelf="";		
String mName1="",mName2="",mName3="",mName4="",mName5="",mLoginComp="";
...
else
{
	...
	<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: arial"><B>Event/Sub Event wise Project Marks Finalization</B></font></TD></tr>
	</TABLE>
...
*/
                        ...
                        qry=qry+" AND (fstid) in (select fstid from EX#SUBJECTGRADECOORDINATOR where companycode='"+mComp+"' and institutecode='"+mInst+"' and facultytype=decode('"+mDMemberType+"','E','I','E') and FACULTYID='"+mDMemberID+"' AND FSTID IN (SELECT FSTID FROM FACULTYSUBJECTTAGGING WHERE PROJECTSUBJECT='Y'))";
                        qry=qry+"GROUP BY EVENTSUBEVENT ORDER BY EVENTSUBEVENT";
                        //out.print(qry);
	...
	if(request.getParameter("Subject")!=null && request.getParameter("Event")!=null && request.getParameter("Exam")!=null)
      {
		...
		qry="Select 'Y' from Ex#SubjectGradeCoordinator where CompanyCode='"+mComp+"' and InstituteCode='"+mInst+"' and FACULTYID='"+mChkMemID+"' ";
		qry=qry+" AND FSTID IN (SELECT FSTID FROM FACULTYSUBJECTTAGGING WHERE EXAMCODE='"+mEC+"' AND SUBJECTID='"+mSC+"')";
		//out.print(qry);
		...
		if(mSelf.equals("Self"))
            {
            	qry="select WEIGHTAGE, MARKSORPERCENTAGE, MAXMARKS from V#EXAMEVENTSUBJECTTAGGING  ";
                  ...
                  qry=qry+" AND ((EMPLOYEEID=(Select '"+mDMemberID+"' EmployeeID from dual)) OR (fstid in (select fstid from MULTIFACULTYSUBJECTTAGGING where companycode='"+mComp+"' and institutecode='"+mInst+"' and facultytype=decode('"+mDMemberType+"','E','I','E') and employeeid='"+mDMemberID+"')) and facultytype=decode('"+mDMemberType+"','E','I','E'))";
                  qry=qry+" And EVENTSUBEVENT='"+mEvent+"' AND LTP='P' AND PROJECTSUBJECT='Y' and subjectID='"+mSC+"' AND  NVL (deactive, 'N') = 'N' ";
		}
            else
            {
            	qry="select  WEIGHTAGE, MARKSORPERCENTAGE, MAXMARKS from V#EXAMEVENTSUBJECTTAGGING  ";
		...
		%>	
		<form name="frm1"  method="post" action="ProjMarksEntryFinalAction.jsp">
		<%
		//len=mEvent.length();
		//pos=mEvent.indexOf("#");
	      ...
	      if(mSelf.equals("Self"))
		{
			...
			//qry=qry+" AND employeeid='"+mDMemberID+"' and facultytype=decode('"+mDMemberType+"','E','I','E')";
      	      qry=qry+" AND ((EMPLOYEEID=(Select '"+mDMemberID+"' EmployeeID from dual)) OR (fstid in (select fstid from MULTIFACULTYSUBJECTTAGGING where companycode='"+mComp+"' and institutecode='"+mIC+"' and facultytype=decode('"+mDMemberType+"','E','I','E') and employeeid='"+mDMemberID+"')) and facultytype=decode('"+mDMemberType+"','E','I','E'))";
		...
		while(rs1.next())
		{
			...
			if(ctr==1)
			{
				...
				<td align=center><b><font color=white face=arial size=2 nowrap>Examiner-I<BR>Allotted out of <%=mMaxmarks%></font></b></td>
				<td align=center><b><font color=white face=arial size=2 nowrap>Examiner-II<BR>Allotted out of <%=mMaxmarks%></font></b></td>
				<td align=center><b><font color=white face=arial size=2 nowrap>Marks Final Allotted<BR> out of <%=mMaxmarks%><BR>AVG. of I & II</font></b></td>
...
//---------------------------------------------End of Proj Marks Final---------------------------------------
		    ...
		    <td align=center><input tabindex="<%=tabctrtxt%>" type=text name='<%=mName3%>' id='<%=mName3%>' value="<%=mSMarks%>" style="WIDTH: 40px; height: 20px; text-align:right; width:35px; font-size:10px; color:navy;" maxlength=5 onBlur="Marks_Check(<%=mName4%>,<%=mName3%>,<%=MyMax%>);"  onchange="Marks_Check(<%=mName4%>,<%=mName3%>,<%=MyMax%>);" READONLY><%=x%></td>
