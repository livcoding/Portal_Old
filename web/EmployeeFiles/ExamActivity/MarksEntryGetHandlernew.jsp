...
*/
        ...
        if(mSelf.equals("Self") && qryCtr==0)
	{
		...
		qry2="SELECT  'Y' from V#StudentEventSubjectMarks ";
                qry2=qry2+" where institutecode='"+mIC+"' and nvl(DEACTIVE,'N')='N'  and ";
...
//System.out.print("JILIT - "+qry2);
		...
		qry=qry+" where institutecode='"+mIC+"' and nvl(DEACTIVE,'N')='N'  and  nvl(STUDENTLTPDEACTIVE,'N')='N' and ";
		...
		//qry=qry+" AND employeeid in (Select '"+mDMemberID+"' EmployeeID from dual UNION Select EmployeeID from FacultySubjectTagging where FSTID in (SELECT FSTID FROM MULTIFACULTYSUBJECTTAGGING WHERE COMPANYCODE='"+mComp+"' and INSTITUTECODE='"+mIC+"' and FACULTYTYPE=decode('"+mDMemberType+"','E','I','E') and EMPLOYEEID='"+mDMemberID+"')) and facultytype=decode('"+mDMemberType+"','E','I','E')";
                qry=qry+" AND EVENTSUBEVENT='"+mEvent+"'  and STUDENTID   IN (Select distinct studentid from debarstudentdetail where nvl(REEXAMINATION,'N' )='Y' and nvl(MEDICALWITHDRAWAL,'N')<>'Y' and subjectID='"+mSC+"' and examcode='"+mEC+"' and REEXAMEVENTSUBEVENT='"+mEvent+"')   and ( (studentid,fstid) in (select  studentid,fstid from StudentEventSubjectMarks  where EVENTSUBEVENT='"+mEvent+"'  and nvl(DEACTIVE,'N')='N'  	) OR  (studentid,fstid) NOT in (select  studentid,fstid from StudentEventSubjectMarks  where EVENTSUBEVENT='"+mEvent+"'  and nvl(DEACTIVE,'N')='N' and nvl(locked,'N')='Y'	)  )" ;
		...
		//qry=qry+" AND employeeid='"+mDMemberID+"' and facultytype=decode('"+mDMemberType+"','E','I','E')";
                qry=qry+" AND (EMPLOYEEID=(Select '"+mDMemberID+"' EmployeeID from dual)) ";
		//qry=qry+" AND employeeid in (Select '"+mDMemberID+"' EmployeeID from dual UNION Select EmployeeID from FacultySubjectTagging where FSTID in (SELECT FSTID FROM MULTIFACULTYSUBJECTTAGGING WHERE COMPANYCODE='"+mComp+"' and INSTITUTECODE='"+mIC+"' and FACULTYTYPE=decode('"+mDMemberType+"','E','I','E') and EMPLOYEEID='"+mDMemberID+"')) and facultytype=decode('"+mDMemberType+"','E','I','E')";
                qry=qry+" AND EVENTSUBEVENT='"+mEvent+"'   and STUDENTID   IN (Select distinct studentid from debarstudentdetail where nvl(REEXAMINATION,'N' )='Y' and nvl(MEDICALWITHDRAWAL,'N')<>'Y' and subjectID='"+mSC+"' and REEXAMEVENTSUBEVENT='"+mEvent+"' and examcode='"+mEC+"')  " ;
		qry=qry+" GROUP BY fstid,studentid,StudentName,enrollmentno,Semester, programcode,SECTIONBRANCH, subsectioncode";
		qry=qry+" order by "+mList+ " "+mOrder+ " ";
...
//System.out.print("JILIT - "+qry2);
		...
		qry=qry+"  and STUDENTID   IN (Select distinct studentid from debarstudentdetail where nvl(REEXAMINATION,'N' )='Y' and nvl(MEDICALWITHDRAWAL,'N')<>'Y'  AND FSTID IN (SELECT FSTID FROM EX#SUBJECTGRADECOORDINATOR WHERE COMPANYCODE='"+mComp+"' and INSTITUTECODE='"+mIC+"' and FACULTYTYPE=decode('"+mDMemberType+"','E','I','E') and FACULTYID='"+mDMemberID+"') and REEXAMEVENTSUBEVENT='"+mEvent+"' and examcode='"+mEC+"') GROUP BY fstid,studentid,StudentName,enrollmentno,Semester, programcode,SECTIONBRANCH, subsectioncode";
		qry=qry+" order by "+mList+ " "+mOrder+ " ";
		...
		qry=qry+" AND FSTID IN (SELECT FSTID FROM EX#SUBJECTGRADECOORDINATOR WHERE COMPANYCODE='"+mComp+"' and INSTITUTECODE='"+mIC+"' and FACULTYTYPE=decode('"+mDMemberType+"','E','I','E') and FACULTYID='"+mDMemberID+"')";
		qry=qry+" and STUDENTID   IN (Select distinct studentid from debarstudentdetail where nvl(REEXAMINATION,'N' )='Y' and nvl(MEDICALWITHDRAWAL,'N')<>'Y'  and  examcode='"+mEC+"' and REEXAMEVENTSUBEVENT='"+mEvent+"' and subjectID='"+mSC+"' )  GROUP BY fstid,studentid,StudentName,enrollmentno,Semester, programcode,SECTIONBRANCH, subsectioncode";
		qry=qry+" order by "+mList+ " "+mOrder+ " ";
		 ...
		 //  	System.out.print("JILIT - "+qry);
			rs1=db.getRowset(qry);
                if(rs1.next())
                {
					...
					if(rs1.getString("MARKSAWARDED1").equals("-1")){
					mSMarks ="";
					//System.out.print("**************"+mSMarks);
					}else{
...
//System.out.println("*******"+mSMarks+"********"+rs1.getString("DETAINED")+"*********");
                rs2 = db.getRowset(qry1);
...
qry2=" select NVL(A.DEBAR,'N')DEBAR,NVL(A.MEDICALCASE,'N')MEDICALCASE,NVL(A.ABSENTCASE,'N')ABSENTCASE,NVL(A.UFM ,'N')UFM,NVL(A.UFMMARKS ,'N')UFMMARKS,NVL(A.PRORATA,'N')PRORATA from DEBARSTUDENTDETAIL a  WHERE a.EXAMCODE='"+mEC+"' and a.fstid = '"+rs.getString("fstid")+"'  and a.INSTITUTECODE='"+mIC+"' and a.STUDENTID='"+mSID+"' AND A.SUBJECTID='"+mSC+"' and nvl(a.DEACTIVE,'N')='N' AND   A.REEXAMEVENTSUBEVENT='"+mEvent+"' ";
...
//---------------------------------------------------------------
	...
	//System.out.print("<bR>JILIT - "+mSMarks+" -- "+rs.getString("studentid"));
...
//---------------------- debar studentdetail
			...
			qryd="Select DISTINCT NVL(EVENTSUBEVENT,' ')EVENTSUBEVENT, STUDENTID , NVL(MEDICALCASE,'N') MEDICALCASE, NVL( ABSENTCASE,'N')  ABSENTCASE, DECODE(PRORATA,'Y','P','N','N',' ','',PRORATA)PRORATA, nvl(UFMMARKS,'N')UFMMARKS from DEBARSTUDENTDETAIL      ";
			qryd=qryd+" where INSTITUTECODE='"+mIC+"' and EXAMCODE='"+mEC+"' and ";
		    qryd=qryd+" REEXAMEVENTSUBEVENT='"+mEvent1+"' and   ";
