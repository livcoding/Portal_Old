...
<%
String mInst="", mHead="";
String qryy123="",msubject="",mAPPLICABLEFOR="";
...
function validate()
{
	var event = document.frm2.EventSubEventCode.value;
var inst=document.frm2.InstCode.value;
...
var TAMaxValue=trim(document.frm2.TAMaxValue.value);
//	alert(expw+" :: "+wtg+" :: "+mmax+" event "+event);
var len=event.lenght;
var pos=event.indexOf('#');
var events=event.substring(0,pos);
//alert(events);
...
if(events=='TA' && inst=='JPBS')
		{
	//alert(wtg+" LL "+TAMaxValue);
		if (parseFloat(wtg)==parseFloat(TAMaxValue))
		{
			alert('Weightage must be divided into TA parts' );
			document.frm2.WEIGHTAGE.focus();
			return false;
		}
		}
...
String mCurDate="", mDate1="", mDate2="", msubeven="",mQryBaseLTP="";
...
//*******************************************************************
			...
			qry=qry+" subjectmaster B where (A.LTP='L' OR (A.LTP='P' AND A.PROJECTSUBJECT='Y')) and A.employeeid='"+mDMemberID+"' and A.examcode='"+mExamid+"' and A.facultytype=decode('"+mDMemberType+"','E','I','E') ";
			...
			qry=qry+" SUBJECTMASTER B where A.LTP IN ('L','P','E') and A.COORDINATORID='"+mDMemberID+"' and A.EXAMCODE='"+mExamid+"' and A.COORDINATORTYPE=decode('"+mDMemberType+"','E','I','E') ";
			...
			qry=qry+" ProgramSubjectTagging BB where AA.LTP='P' AND nvl(AA.PROJECTSUBJECT,'N')<>'Y' And BB.L=0 and BB.T=0 and BB.P>0 And BB.ExamCode='"+mExamid+"' And AA.employeeid='"+mDMemberID+"' and AA.examcode='"+mExamid+"' ";
		...
		//out.print(qry);
			...
			while(rss.next())
			{
				if(QryBaseFSTID.equals(""))
					QryBaseFSTID="'"+rss.getString(1)+"'";
				else
					QryBaseFSTID=QryBaseFSTID+",'"+rss.getString(1)+"'";
			}
//*******************************************************************
//out.print("QryBaseFSTID - "+QryBaseFSTID);
			if(!QryBaseFSTID.equals(""))
			{
			%>
		...
		<!--*********Exam Event Code**********-->
			...
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<%
			qry="Select Distinct LTP, Decode(LTP,'L',1,'T',2,'P',3,'E',4)LTPSEQ from facultysubjecttagging" +
                    " where ExamCode='"+mExamid+"' and institutecode='"+mInst+"'" +
                    " and fstid in ("+QryBaseFSTID+") and nvl(Deactive,'N')='N' ORDER BY LTPSEQ";
			rss=db.getRowset(qry);
			//out.print(qry+"<br>");
			String QryBaseLTP="";
			int Counter=0;
			while(rss.next())
			{
				...
				qry=qry+"AND APPLICABLEFOR LIKE '%"+QryBaseLTP+"%' and trunc(sysdate) >= trunc(EVENTFROM) and trunc(sysdate) <=trunc(EVENTUPTO) Order By EVENTFROM, EXAMEVENTCODE";
		...
		<!-- *****************DataCombo******************* -->
			...
			//	out.print(QryBaseLTP+"QryBaseLTP");
			try
			{
		...
		<!--********Subject*****-->
			...
			qry="Select distinct subject, subjectID From (";
		 	qry=qry+"(select A.FSTID, nvl(B.subject,' ')||'('|| nvl(B.subjectcode,' ')||')' subject , A.subjectID subjectID from facultysubjecttagging A, ";
			qry=qry+" subjectmaster B where (A.LTP='L' OR (A.LTP='P' )) and A.employeeid='"+mDMemberID+"' and A.examcode='"+mExamid+"' and A.facultytype=decode('"+mDMemberType+"','E','I','E') ";
			qry=qry+"   and a.INSTITUTECODE='"+mInst+"' and A.INSTITUTECODE=B.INSTITUTECODE and A.subjectID=B.subjectID and nvl(A.deactive,'N')='N' and nvl(B.Deactive,'N')='N'";
			qry=qry+" and a.SUBJECTID not IN (SELECT DISTINCT SUBJECTID FROM GRADECALCULATION WHERE examcode='"+mExamid+"'";
			qry=qry+" and NVL(STATUS,'D')='F') GROUP BY A.FSTID, nvl(B.subject,' ')||'('|| nvl(B.subjectcode,' ')||')' , A.subjectID)";
			qry=qry+" UNION";
			...
			qry=qry+" UNION ";
		 	qry=qry+" (Select AA.FSTID, nvl(CC.subject,' ')||'('|| nvl(CC.subjectcode,' ')||')' subject , AA.subjectID subjectID from facultysubjecttagging AA, SubjectMaster CC , ";
			qry=qry+" ProgramSubjectTagging BB where AA.LTP='P' And BB.L=0 and BB.T=0 and nvl(AA.PROJECTSUBJECT,'N')<>'Y' and BB.P>0 And BB.ExamCode='"+mExamid+"' And AA.employeeid='"+mDMemberID+"' and AA.examcode='"+mExamid+"' ";
			qry=qry+" and aa.INSTITUTECODE='"+mInst+"' AND AA.INSTITUTECODE=BB.INSTITUTECODE And AA.EXAMCODE=BB.EXAMCODE And AA.ACADEMICYEAR=BB.ACADEMICYEAR And AA.PROGRAMCODE=BB.PROGRAMCODE And AA.TAGGINGFOR=BB.TAGGINGFOR And AA.SECTIONBRANCH=BB.SECTIONBRANCH And AA.SEMESTER=BB.SEMESTER And AA.BASKET=BB.BASKET And AA.SUBJECTID=BB.SUBJECTID ";
			qry=qry+" And AA.facultytype=decode('"+mDMemberType+"','E','I','E') ";
			...
			try
			{
				...
				else
				{
					...
					{
						mSubj=rss.getString("SubjectID");
						//msubject=rss.getString("Subject");
			...
			if(request.getParameter("x")!=null)
			{
				...
				if(request.getParameter("Exam")!=null && request.getParameter("Event")!=null && request.getParameter("Subject")!=null )
				{
					...
					/*String qryy="SELECT 'Y' FROM    EX#SUBJECTGRADECOORDINATOR a where a.institutecode = '"+mInst+"'  and a.fstid in (select b.fstid  from v#StudentLtpDetail b where b.institutecode = a.InstituteCode and b.examcode = '"+mExamID+"'  and b.subjectid = '"+mSubj+"' and Rownum = 1)";
                    */
                      //added by satendra
 if(mInst.equals("JPBS")){
	 ...
	 qryy123="select * from EX#SUBJECTGRADECOORDINATOR a where  a.fstid in(select b.fstid from v#studentLtpDetail b ";
					 qryy123=qryy123+" where b.Institutecode = '"+mInst+"' and b.ExamCode = '"+mExamID+"' and b.subjectid = '"+mSubj+"'   and b.LTP in('L','P')  and rownum =1)  ";


 //out.print(qryy123);
					   rs=db.getRowset(qryy123);
		    if(rs.next())
		{
                  Showdetails="Y";
                }
                 ...
                 if(Showdetails.equals("Y")){
					...
					//out.println(mEventCode);

					qry="SELECT  nvl(APPLICABLEFOR,' ')APPLICABLEFOR  FROM EXAMEVENTMASTER where INSTITUTECODE='"+mInst+"' and EXAMCODE='"+mExamID+"' and  EXAMEVENTCODE='"+mEventCode+"' ";
					rs=db.getRowset(qry);
					//out.print(qry);
					if(rs.next())
					{
						mQryBaseLTP	=rs.getString("APPLICABLEFOR");
						mQryBaseLTP= mQryBaseLTP.substring(0,1);
					}

					qry="select 'Y' from facultysubjecttagging where ltp  LIKE '%"+mQryBaseLTP+"%' and examcode='"+mExamID+"' AND INSTITUTECODE='"+mInst+"' AND SUBJECTID = '"+mSubj+"' and nvl(deactive,'N')='N' ";
					//out.print(qry);
					rs=db.getRowset(qry);
					if(rs.next())
...
//*******************************************************************
					...
					qry=qry+" UNION ";
				 	qry=qry+" (Select AA.FSTID, nvl(CC.subject,' ')||'('|| nvl(CC.subjectcode,' ')||')' subject , AA.subjectID subjectID from facultysubjecttagging AA, SubjectMaster CC , ";
					qry=qry+" ProgramSubjectTagging BB where AA.SUBJECTID='"+mSubj+"' AND AA.LTP='P' AND nvl(AA.PROJECTSUBJECT,'N')<>'Y' And BB.L=0 and BB.T=0 and BB.P>0 And BB.ExamCode='"+mExamid+"' And AA.employeeid='"+mDMemberID+"' and AA.examcode='"+mExamid+"' ";
					qry=qry+" and aa.INSTITUTECODE='"+mInst+"' AND AA.INSTITUTECODE=BB.INSTITUTECODE And AA.EXAMCODE=BB.EXAMCODE And AA.ACADEMICYEAR=BB.ACADEMICYEAR And AA.PROGRAMCODE=BB.PROGRAMCODE And AA.TAGGINGFOR=BB.TAGGINGFOR And AA.SECTIONBRANCH=BB.SECTIONBRANCH And AA.SEMESTER=BB.SEMESTER And AA.BASKET=BB.BASKET And AA.SUBJECTID=BB.SUBJECTID ";
					qry=qry+" And AA.facultytype=decode('"+mDMemberType+"','E','I','E') ";
					qry=qry+" and nvl(AA.deactive,'N')='N' and nvl(BB.Deactive,'N')='N' and aa.INSTITUTECODE='"+mInst+"' AND ";
					qry=qry+" AA.INSTITUTECODE=CC.INSTITUTECODE AND BB.INSTITUTECODE=CC.INSTITUTECODE AND cc.SUBJECTID=BB.SUBJECTID AND AA.SUBJECTID=CC.SUBJECTID ";
...
//*******************************************************************
				...
				//	out.print(qry);
					...
					qry="Select TO_CHAR("+MaxAW+") Weightage from EXAMEVENTMASTER where INSTITUTECODE='"+mInst+"' and EXAMCODE='"+mExamID+"'";
					qry=qry+" and EXAMEVENTCODE='"+mEventCode+"' and nvl(Deactive,'N')='N'";
				 	 //out.print(qry);
					try
					{
				...
				// out.println(qry);
					...
					catch(Exception e)
					{
                                        ...
                                        qry="select  decode(APPLICABLEFOR,'L','L','LT','L','P') APPLICABLEFOR  from exameventmaster where institutecode='"+mInst+"' and examcode='"+mExamID+"' and EXAMEVENTCODE='"+mEventCode+"'";
					// out.println(qry);
					try
					{
						rs=db.getRowset(qry);
						...
						if (rs.next() )
						{
							//mAPPLICABLEFOR=rs.geString("APPLICABLEFOR");
                                                        mAPPLICABLEFOR=rs.getString("APPLICABLEFOR");
						}
                                         session.setAttribute("APPLICABLEFOR",mAPPLICABLEFOR);
					}
					catch(Exception e)
					{
					}
					...
					<%
					qry="Select distinct 'Y' from FacultySubjectTagging Where INSTITUTECODE='"+mInst+"' and COMPANYCODE='"+mComp+"' and ExamCode='"+mExamID+"' and SubjectId='"+mSubj+"' and LTP='P' ";
				...
				//	out.print(qry);
					...
					{
                           // new 02-03-2017
                            qry="Select nvl(WEIGHTAGE,0)WEIGHTAGE from SubjectWiseWeightage WHERE INSTITUTECODE='"+mInst+"' AND EXAMCODE='"+mExamID+"' AND EXAMEVENTCODE='"+mEventCode+"' AND SUBEVENTCODE='"+mSubEventCode+"' AND SUBJECTID='"+mSubj+"' and nvl(Deactive,'N')='N'";
                           //new
						...
						<td><b>Assign '<%=mEv%>' <br>Full Marks for Evaluation :<b></td><td><input size=3 maxlength=6 Type=text name='MAXMARKS' id='MAXMARKS' value=<%=EveSubEveVal%>></td>
						<%
					}
					else
					{
                           //new 02-03-2017
                            qry="Select nvl(WEIGHTAGE,0)WEIGHTAGE from SubjectWiseWeightage WHERE INSTITUTECODE='"+mInst+"' AND EXAMCODE='"+mExamID+"' AND EXAMEVENTCODE='"+mEventCode+"' AND SUBJECTID='"+mSubj+"' and nvl(Deactive,'N')='N'";
                           //new
                          //  out.print("sUBJECTwISE"+qry);
                            rrss=db.getRowset(qry);
                            if(rrss.next())
                                {
                               EveSubEveVal=rrss.getDouble(1);
						...
						<td><b>Assign '<%=mEventCode%>' Full Marks for Evaluation :<b></td><td><input size=5 maxlength=6 Type=text name='MAXMARKS' id='MAXMARKS' value=<%=EveSubEveVal%>></td>
						<%
					}
					%>
					<input Type=hidden value=<%=mInst%> id="InstCode" name="InstCode">
	...
	<input Type=hidden value="<%=MaxAWSubEv%>" Name="TAMaxValue" ID="TAMaxValue">
				...
				{
					out.print(" &nbsp;&nbsp;&nbsp <img src='../../Images/Error1.jpg'> <b><font size=3 face='Arial' color='Red'>Please choose right subject with event-subevent</font> <br>");
				}


                                }else{

                                out.print(" &nbsp;&nbsp;&nbsp <img src='../../Images/Error1.jpg'> <b><font size=3 face='Arial' color='Red'>Coordinator not defined for selected subject ...Please contact to registry department !</font> <br>");
                                }
//*******************************************************************
		      ...
		      {
				%><CENTER><img src='../../Images/Error1.jpg'> <b><font size=3 face='Arial' color='Red'>No such data found !</font></CENTER><%
			}
			   } // EventSubevent is already tagged
		   else
		   {
