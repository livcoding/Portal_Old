<%@ page  autoFlush="true" language="java" import="java.sql.*,tietwebkiosk.*,java.util.*" %>
...
<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: large; FONT-FAMILY: fantasy"><b>Project Freezed Grade Sheet</b></TD>
</font></td></tr>
</TABLE>
<br>
<%
...
if (RsChk.next() && RsChk.getString("SL").equals("Y"))
 {
	...
	String mSc="";
	String mSem1="";
String mCheckFstid="",Stdidclub="",mBreakSlno="",mGrade11="",prevFstid="",prevStudid="";
	ResultSet rs=null,rssub=null,rsi=null;
...
String PREVIOUSSUBJECTID="",PREVIOUSEXAMCODE="";
	...
	%>
	<form name="frm" method="post" action="ProjectStudentGradesXLS.jsp">
...
String 	qryi=" SELECT distinct a.fstid prevfstid,B.enrollmentno,B.studentname,A.STUDENTID STUDENTID,A.INITIALMARKS,A.INITIALGRADE,A.MASSCUTS,A.FINALMARKS,";
	...
	qryi=qryi+" and A.INSTITUTECODE='"+mInst+"' and a.institutecode=b.institutecode AND A.FSTID=B.FSTID and A.EXAMCODE='"+PREVIOUSEXAMCODE+"' AND A.STUDENTID=B.STUDENTID AND NVL(A.DEACTIVE,'N')='N'  order by B.studentname";
	//out.print(qryi);
	rsi=db.getRowset(qryi);
	while(rsi.next())
	{
		mGrade11=rsi.getString("FINALGRADE");
		prevFstid=rsi.getString("prevfstid");
		prevStudid=rsi.getString("STUDENTID");

		qry="UPDATE STUDENTWISEGRADE SET INITIALGRADE='"+mGrade11+"', ENTRYBY ='"+mChkMemID+"',ENTRYDATE=SYSDATE WHERE INSTITUTECODE='"+mInst+"' AND EXAMCODE='"+PREVIOUSEXAMCODE+"' AND FSTID='"+prevFstid+"' AND  STUDENTID='"+prevStudid+"'  ";
				  up=db.update(qry);
