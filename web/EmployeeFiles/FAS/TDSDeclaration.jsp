...
String mHead="";
	...
	String  Mlandlordname="",mLanDpANNO="";
...
if(flag==0 )
 ...
 if(finalflag==0 )
		{
			 if(finalflag1!=0 || flag1!=0)
			{
				//alert("done done ");
				return true;
			}
			else 	 if(finalflag1==0 || flag1==0)
			{
					return true;
			}
			else
			{
			alert("Please Enter Actual Amount cc!");
			return false;
			}
		}


//alert(document.getElementById("DeclarationAmt"+14).value +" ------");

if(  (document.getElementById("ReceiptAmt"+14).value!=null))
	{
			
//alert(document.getElementById("DeclarationAmt"+14).value +" - -- --");

if( (   ((document.getElementById("ReceiptAmt"+14).value) >= 100000 ) ) && (document.getElementById("LandlordName").value=='' || document.getElementById("LandlordName").value==null ) 	)	
	{
	alert("Please Enter LandlordName !");
TDS.LandlordName.focus();
	return false;
     }


if( (   ((document.getElementById("ReceiptAmt"+14).value) >= 100000 ) ) && 	 ( document.getElementById("LandlordPAN").value=='' || document.getElementById("LandlordPAN").value==null ) 	)	
	{
	alert("Please Enter  Landlord PAN. !");
TDS.LandlordPAN.focus();
	return false;
		}
...
if((document.getElementById("ReceiptAmt"+14).value=='') || (document.getElementById("ReceiptAmt"+14).value==null))
	{
	//alert(document.TDS.PARAMETERVALUE.value+"sdsfd");
if((document.getElementById("DeclarationAmt"+14).value!='') &&  (document.TDS.PARAMETERVALUE.value==' ' ||  document.TDS.PARAMETERVALUE.value==null || document.TDS.PARAMETERVALUE.value==''))
		{
...
if ((document.getElementById("DeclarationAmt"+14).value=='' ) &&  (document.TDS.PARAMETERVALUE.value!='' ))
	{
		...
		if((document.getElementById("ReceiptAmt"+14).value!='') &&  (document.TDS.PARAMETERVALUE.value==' ' ||  document.TDS.PARAMETERVALUE.value==null || document.TDS.PARAMETERVALUE.value==''))
		{
			alert("Please Enter the Date of Possession !");
			document.TDS.PARAMETERVALUE.disabled=false;
			TDS.PARAMETERVALUE.focus();
...
if ((document.getElementById("ReceiptAmt"+14).value=='' ) &&  (document.TDS.PARAMETERVALUE.value!='' ))
	{
			alert("Please Enter Actual Amount against Date of Possession !");
			//document.TDS.PARAMETERVALUE.disabled=false;
			//TDS.PARAMETERVALUE.focus();
			return false;
...
//alert( parseInt(document.getElementById("DeclarationAmt"+14).value) +" ------");


if(  (document.getElementById("ReceiptAmt"+14).value!=null))
	{
			
//alert(document.getElementById("DeclarationAmt"+15).value +" - -- --");

if( (   ((document.getElementById("ReceiptAmt"+15).value) >= 100000 ) ) && (document.getElementById("LandlordName").value=='' || document.getElementById("LandlordName").value==null ) 	)	
	{
	alert("Please Enter LandlordName !");
TDS.LandlordName.focus();
	return false;
...
if( (   ((document.getElementById("ReceiptAmt"+15).value) >= 100000 ) ) && 	 ( document.getElementById("LandlordPAN").value=='' || document.getElementById("LandlordPAN").value==null ) 	)	
	{
	alert("Please Enter  Landlord PAN. !");
TDS.LandlordPAN.focus();
	return false;
...
//alert(document.getElementById("DeclarationAmt"+14).value +" ------");
/*
if((document.getElementById("DeclarationAmt"+14).value=='') || (document.getElementById("ReceiptAmt"+14).value==null))
	{
			
if((document.getElementById("DeclarationAmt"+14).value) >= 100000 )
		{
	alert("Please Enter LandlordName and Landlord PAN No. !");
//TDS.PARAMETERVALUE.focus();
	return false;
		}
...
//alert(document.getElementById("ACTUALAMOUNT"+i)+"88888");

//alert(flag1+"sdfsdfsfsfsffsdfsdfsf"+finalflag1);
if(flag==0 )
		{
			 if(flag1!=0)
			{
...
String mType="";
int mRights=0,j=0,p=0,k=0,mflag1=0;
String SubQry="",mySub="";
...
String mAssYear="",qrymAssYear="";
  ...
  String mStatus="",mFreeze="N",mTextReadonly="";
...
qry="Select TDSCODE ,TDSDESC From TDS#TDSMASTER Where COMPANYCODE='"+mComp+"' And SLTYPE='"+mDDMemberType+"'";
...
String mYearFrom="",mSrCitizen="",Gender1="";
		...
		qry="SELECT to_char(FROMPERIOD,'dd-mm-yyyy')FROMPERIOD, to_char(TOPERIOD,'dd-mm-yyyy')TOPERIOD, NVL(STATUS,'N')STATUS ,NVL(ASSESSMENTYEAR,' ')ASSESSMENTYEAR , NVL(FINYEAR,' ')FINYEAR		FROM TDS#PARAMETER where trunc(SYSDATE) >= trunc(fromperiod)   and  trunc(SYSDATE) <= trunc(toperiod) and COMPANYCODE='"+mComp+"' ";
rs=db.getRowset(qry);
if(rs.next())
		{
	...
	<table bordercolor=maroon cellpadding=1 cellspacing=1  align=center rules=groups  border=1  >
		...
		%>
			...
			<TD>
			<input type=text name="Designation" size=60 value="<%=GlobalFunctions.toTtitleCase(mDesg)%>(<%=mDept%>)" readonly >
			</td>
		<%
		}
		else
		...
		%>
			...
			<TD>
			<input type=text name="Designation" size=60 value="<%=GlobalFunctions.toTtitleCase(mDesg)%>(<%=mDept%>)" readonly ></td>
		<%
		}
		%>
		<!-- Institute **************** ********************** *******************-->
...
<INPUT TYPE="submit" name="submit" value="Click here to Open Form " onclick="return ValidatePan();">
</td></tr>
</table>
</Form>
...
//out.print(mSeniorCitizen+"sadfsdmSeniorCitizen"+Gender1);	
	%>
<form Name="TDS" Action='TDSDeclarationAction.jsp' method=post >
...
//LandlordName=request.getParameter("LandlordName").toString().trim().toUpperCase();
//LandlordPAN=request.getParameter("LandlordPAN").toString().trim().toUpperCase();
...
try
  {
String mFinalFreeze="N",mFinalTextReadonly="",mFinalDisablecombo="";
	
		

 qry1="select  nvl(FREEZE,'N')FREEZE , NVL(FINALFREEZE,'N')FINALFREEZE from TDS#EDIDECLARATIONHEADER where  COMPANYCODE='"+mComp+"' and  TDSCODE='"+mTDSCode+"' and ASSESSMENTYEAR='"+mAssessYear+"' and EMPLOYEEID='"+mDMemberID+"' ";

rs=db.getRowset(qry1);
if(rs.next())
	  {
	mFreeze=rs.getString("FREEZE");
	mFinalFreeze=rs.getString("FINALFREEZE");
	  }
...
// finally freezing the TDS Form 

	if(mFinalFreeze.equals("Y"))
	  {
		mFinalTextReadonly="readonly";
		mFinalDisablecombo="disabled";
	  }
	else
	  {
		mFinalTextReadonly=" ";
...
<%
	if(mStatus.equals("F"))
	  {
	%>
	<td align=center><Font face='arial' size=3 COLOR=WHITE ><b>Actual Amount </b></font></td>
...
//out.print(mSeniorCitizen+"sadfsdmSeniorCitizen"+Gender1);	
qry="select edicode,  eddescription,                editype,                DECODE (editype,'E', 'Earnings', 'I', 'Investment', 'D', 'Deduction',                        editype                       ) edtypedescription,                edicode categorycode, edicode categorydescription,                sectiondesc, NVL (declarationseqid, 0) declarationseqid,                sectioncode, NVL (otherparameter, 'N') otherparameter,                'N' otherinvestparameter,                NVL (sequenceid, 0) sequenceid               from  (";
qry=qry+"SELECT distinct  a.edicode, nvl(a.EDIDESCRIPTION,' ') eddescription, a.editype,DECODE (a.editype,'E', 'Earnings','I', 'Investment','D', 'Deduction',    a.editype  ) edtypedescription,         a.edicode categorycode, a.edicode categorydescription,c.SECTIONDESC,nvl(c.DECLARATIONSEQID,0)DECLARATIONSEQID  ,c.SECTIONCODE,nvl(a.OTHERPARAMETER,'N')OTHERPARAMETER ,'N' OTHERInvestPARAMETER ,nvl(D.SEQUENCEID,0)SEQUENCEID   FROM tds#edideclaration a, edmaster b,TDS#SECTIONMASTER c,  TDS#TDSSECTIONTAGGING d   WHERE a.companycode ='" + mComp + "'      AND a.tdscode = '"+mTDSCode+"'      AND a.ASSESSMENTYEAR = '"+mAssessYear+"'   and a.FINYEAR='"+mFinancialYear+"'             and d.SENIORCITIZEN='"+mSeniorCitizen+"'            and d.GENDER='"+Gender1+"'  AND b.companycode = a.companycode     AND b.edid = a.edicode     AND b.TYPE = a.editype     and c.COMPANYCODE=d.COMPANYCODE     and c.TDSCODE=d.TDSCODE     and c.SECTIONCODE=d.SECTIONCODE     and a.COMPANYCODE=d.COMPANYCODE     and a.TDSCODE=d.TDSCODE     and a.FINYEAR=d.FINYEAR     and a.EDICODE=d.EDICODE     and a.EDITYPE=d.EDITYPE   UNION ALL SELECT  distinct nvl(a.edicode,' ')edicode , nvl(a.EDIDESCRIPTION,' ')eddescription, nvl(a.editype,' ')editype ,      nvl(   DECODE (a.editype, 'E', 'Earnings','I', 'Investment', 'D', 'Deduction',  a.editype          ),' ') edtypedescription,         nvl(a.edicode,' ') categorycode, nvl(a.edicode,' ') categorydescription,nvl(c.SECTIONDESC,' ')SECTIONDESC,nvl(c.DECLARATIONSEQID,0)DECLARATIONSEQID  ,nvl(c.SECTIONCODE,' ')SECTIONCODE  ,nvl(a.OTHERPARAMETER,'N')OTHERPARAMETER ,'N' OTHERInvestPARAMETER ,nvl(D.SEQUENCEID,0)SEQUENCEID    FROM tds#edideclaration a, tds#investrebatemaster b,TDS#SECTIONMASTER c,  TDS#TDSSECTIONTAGGING d   WHERE a.companycode ='" + mComp + "'      AND a.tdscode ='"+mTDSCode+"'      AND a.ASSESSMENTYEAR = '"+mAssessYear+"'     AND b.companycode = a.companycode     AND b.tdscode = a.tdscode     AND b.investrebatecode = a.edicode      and c.COMPANYCODE=d.COMPANYCODE     and c.TDSCODE=d.TDSCODE     and c.SECTIONCODE=d.SECTIONCODE     and a.COMPANYCODE=d.COMPANYCODE     and a.TDSCODE=d.TDSCODE     and a.FINYEAR=d.FINYEAR     and a.EDICODE=d.EDICODE     and a.EDITYPE=d.EDITYPE   and a.FINYEAR='"+mFinancialYear+"'             and d.SENIORCITIZEN='"+mSeniorCitizen+"'            and d.GENDER='"+Gender1+"'     )  where edicode <> 'PP'     ORDER BY DECLARATIONSEQID,SEQUENCEID";
// out.print(qry);
...
//out.print(" 999 9");
	...
	sno++;
		...
		qry="  SELECT DISTINCT nvl(LANDLORDPANNO,' ')LANDLORDPANNO,nvl(LANDLORDNAME,' ')LANDLORDNAME, CATEGORYCODE, EDICODE,  nvl(decode(DECLARATIONAMOUNT,'0','',DECLARATIONAMOUNT),0)DECLARATIONAMOUNT, FINYEAR, ACTUALRECEIVED, nvl(FINAL,' ')FINAL, nvl(decode(actualamount,'0','',actualamount),0)actualamount, nvl(EXEMPTEDAMOUNT,0)EXEMPTEDAMOUNT FROM TDS#EDIDECLARATIONDETAIL WHERE COMPANYCODE= '" + mComp + "' 	AND  TDSCODE='"+mTDSCode+"' AND  ASSESSMENTYEAR='"+mAssessYear+"' AND EMPLOYEEID='"+mChkMemID+"' AND EDITYPE='"+rstable.getString("EDITYPE")+"' AND EDICODE='"+rstable.getString("EDICODE")+"'";
	//out.print(qry);
	rs=db.getRowset(qry);
	if(rs.next())
		{

			mDeclAmt=rs.getString("DECLARATIONAMOUNT").toString();
			mActAmt=rs.getString("ACTUALAMOUNT").toString();
			Mlandlordname=rs.getString("LANDLORDNAME").toString().trim();
				mLanDpANNO=rs.getString("LANDLORDPANNO").toString().trim();
...
//Others Investment ,Please Specify 
		...
		//	out.print(qry2);
			...
			if(rs1.next())
			{
				...
				{ i++;
						%>
								<tr>
					<td><%=i%> </td>
					<td> <INPUT TYPE="text" NAME="IDESCRIPTION<%=i%>" ID="IDESCRIPTION" value="<%=rs2.getString("INVESTMENTDESC")%>" maxlength="75" > </td>

					<td> <INPUT TYPE="text" NAME="IAMOUNT<%=i%>" ID="IAMOUNT" value="<%=rs2.getString("INVESTMENTAMOUNT")%>" onKeyPress="return numbersonly(this, event);"  size=7 maxlength="10"  style="TEXT-ALIGN:right;" > </td>
							<%
					if(mStatus.equals("F"))
					  {
					%>
						<td> <INPUT TYPE="text" NAME="ACTUALAMOUNT<%=i%>" ID="ACTUALAMOUNT" value="<%=rs2.getString("ACTUALAMOUNT")%>" <%=mFinalTextReadonly%> onKeyPress="return numbersonly(this, event);" size=7 maxlength="10"  style="TEXT-ALIGN:right;" > </td>
					<%
					  }
						%>
					</tr><%
			...
			<tr>
			<td><%=i%> </td>
			<td> <INPUT TYPE="text" NAME="IDESCRIPTION<%=i%>" ID="IDESCRIPTION"  maxlength="75"> </td>
			<td> <INPUT TYPE="text" NAME="IAMOUNT<%=i%>"  ID="IAMOUNT" onKeyPress="return numbersonly(this, event);" size=7 maxlength="10"  style="TEXT-ALIGN:right;" > </td>
					<%
			if(mStatus.equals("F"))
			  {
			%>
				<td> <INPUT TYPE="text" NAME="ACTUALAMOUNT<%=i%>" <%=mFinalTextReadonly%> ID="ACTUALAMOUNT" onKeyPress="return numbersonly(this, event);" maxlength="10" size=7  style="TEXT-ALIGN:right;" > </td>
			<%
			  }
				%>
			</tr>
			...
			if(rstable.getString("EDITYPE").equals("E"))
		{
			...
			rs1=db.getRowset(qry1);
						%>
			<SELECT NAME="CategoryDesc<%=sno%>" ID="CategoryDesc<%=sno%>"  >
			<%
			while(rs1.next())
			{
...
<FONT face=Arial size=2><STRONG>Landlord Name
&nbsp; &nbsp;
<input type=text  Name="LandlordName"   value="<%=Mlandlordname%>" size=30  tabindex="0" id="LandlordName" maxlength=60>	 8300 permonth	

	<br>

<FONT face=Arial size=2><STRONG>Landlord PAN.  
&nbsp;
<INPUT TYPE="text" NAME="LandlordPAN"  value="<%=mLanDpANNO%>" id="LandlordPAN" size=15   maxlength=15>		
&nbsp;
...
//	TDS#EDIDECLARATIONPARA  Date of Possession		
	if(mOtherPara.equals("Y"))
		{
			...
			qry="SELECT  COMPANYCODE, TDSCODE, ASSESSMENTYEAR,   EDITYPE, EDICODE, FINYEAR,   nvl(PARAMETERID,0)PARAMETERID, PARAMETERDESC,DATATYPE FROM TDS#EDIDECLARATIONPARA where   COMPANYCODE= '" + mComp + "' 	AND  TDSCODE='"+mTDSCode+"' AND  ASSESSMENTYEAR='"+mAssessYear+"' AND EDITYPE='"+rstable.getString("EDITYPE")+"' AND EDICODE='"+rstable.getString("EDICODE")+"'";
			rs=db.getRowset(qry);
			if(rs.next())
			{

				if(mParaValue.equals(""))
					//mdisable="disabled";
		...
		<br>
			...
			{	
				//out.print(mTextReadonly+"saas"+mdisable);
				%>	
				<INPUT TYPE=TEXT Name="PARAMETERVALUE" <%=mdisable%> id="PARAMETERVALUE" value="<%=mParaValue%>" MAXLENGTH=10 SIZE=10  onchange="return iSValidDate(PARAMETERVALUE.value);">		
			...
			{
				%>
				<INPUT TYPE=TEXT Name="PARAMETERVALUE" <%=mdisable%> id="PARAMETERVALUE" value="<%=mParaValue%>" MAXLENGTH=75 SIZE=10 >		
				<%
	...
	%>
	 <img SRC="images/Rs.jpg.png" > 
	 <Input maxlength=15 size=8 Type=Text ID='ReceiptAmt<%=sno%>' Name='ReceiptAmt<%=sno%>' <%=mFinalTextReadonly%> value="<%=mActAmt%>" onChange="<%=monClick%>" onKeyPress="return numbersonly(this, event);"  style="TEXT-ALIGN:right;"  >
...
//if(mSave.equals("1"))
//{
///out.print(rstable.getString("OTHERInvestPARAMETER")+" 99999");
		if(mflag1==1)
	  {
%>
<tr>
...
<tr bgcolor="#ff8c00"><td colspan=6 align=center ><Font face='arial' size=3 color='white'><b>Additional Information if any </b></font>
 ...
 <tr bgcolor="#ff8c00" >
	...
	if(mStatus.equals("F"))
	  {
	%>
			<td align=center><Font face='ARIAL' color=white size=2><b>Actual Amount</b></font></td>
	<%}
		%>
</tr>
</thead>
...
<tbody>
	...
	if(rs1.next())
	{
		...
		{ k++;
				%>
						<tr>
			<td><%=k%> </td>
			<td> <INPUT TYPE="text" NAME="ADDIDESCRIPTION<%=k%>" ID="ADDIDESCRIPTION" value="<%=rs2.getString("INVESTMENTDESC")%>"  maxlength="75"  size=40> </td>

			<td> <INPUT TYPE="text" NAME="ADDIAMOUNT<%=k%>" ID="ADDIAMOUNT" value="<%=rs2.getString("INVESTMENTAMOUNT")%>" onKeyPress="return numbersonly(this, event);"  size=7 maxlength="10"  style="TEXT-ALIGN:right;" > </td>
					<%
			if(mStatus.equals("F"))
			  {
			%>
				<td> <INPUT TYPE="text" NAME="ADDACTUALAMOUNT<%=k%>" <%=mFinalTextReadonly%> ID="ADDACTUALAMOUNT" value="<%=rs2.getString("ACTUALAMOUNT")%>" onKeyPress="return numbersonly(this, event);" size=7 maxlength="10"  style="TEXT-ALIGN:right;" > </td>
			<%
			  }
				%>
			</tr><%
	...
	<tr>
	<td><%=k%> </td>
	<td> <INPUT TYPE="text" NAME="ADDIDESCRIPTION<%=k%>"  ID="ADDIDESCRIPTION" size=40 maxlength="75"> </td>
	<td> <INPUT TYPE="text" NAME="ADDIAMOUNT<%=k%>" onKeyPress="return numbersonly(this, event);" ID="ADDIAMOUNT" size=7 maxlength="10"  style="TEXT-ALIGN:right;" > </td>
			<%
	if(mStatus.equals("F"))
	  {
	%>
		<td> <INPUT TYPE="text" NAME="ADDACTUALAMOUNT<%=k%>" <%=mFinalTextReadonly%> onKeyPress="return numbersonly(this, event);"  ID="ADDACTUALAMOUNT" maxlength="10" size=7  style="TEXT-ALIGN:right;" > </td>
	<%
	  }
		%>
	</tr>
...
<INPUT TYPE="HIDDEN" NAME="mFreeze" value="<%=mFreeze%>" id="mFreeze">

<INPUT TYPE="HIDDEN" NAME="mFinalFreeze" value="<%=mFinalFreeze%>" id="mFinalFreeze">
<%

if(mStatus.equals("F"))
 {
	 if(mFinalFreeze.equals("N")  || mFreeze.equals("N") )
	{
		%>
...
<tr><td colspan=10 align=center>
	...
	<Input Type='Submit' Value='FinalSave' name="FinalSave"  onclick="return Validatedate();" >
	
<%
	 qry1="select 'y' from TDS#EDIDECLARATIONHEADER where  COMPANYCODE='"+mComp+"' and  TDSCODE='"+mTDSCode+"' and ASSESSMENTYEAR='"+mAssessYear+"' and EMPLOYEEID='"+mChkMemID+"' and NVL(FINALFREEZE,'N')='N' ";
		rs=db.getRowset(qry1);
		if(rs.next())
		{
		%>
		<Input Type='Submit' Value='Freeze & Submit' name="PrintFinalFreeze"  onClick="return Validate();">
		<%
		}
		%>
...
<!-- 	<Input Type='Submit' Value='Click To Print' name="Print" > -->
	</td>
</tr>
	<%
	}
 ...
 else
{
	...
	//out.print(mFreeze+"sadad"+mFinalFreeze);
	if(mFreeze.equals("N") || mFreeze.equals("N") )
	{
		%>
...
<%
	 qry1="select 'y' from TDS#EDIDECLARATIONHEADER where  COMPANYCODE='"+mComp+"' and  TDSCODE='"+mTDSCode+"' and ASSESSMENTYEAR='"+mAssessYear+"' and EMPLOYEEID='"+mChkMemID+"' ";
		rs=db.getRowset(qry1);
		if(rs.next())
		{
		%>
		<Input Type='Submit' Value='Freeze & Submit' name="PrintFreeze"  onClick="return Validate();">
		<%
		}
		%>
