...
%>
   ...
   <td><font face=arial size=2><STRONG>&nbsp;Financial Year </STRONG></FONT>
        <%
        qry="select nvl(FINANCIALYEARCODE,' ')FINANCIALYEARCODE  from  Financialyearmaster a where a.COMPANYCODE='"+mComp+"' ORDER BY A.FINANCIALYEARCODE DESC";
        rs=db.getRowset(qry);
       %>
   ...
   <td ><font face=arial size=2><STRONG>&nbsp;From Period </STRONG></FONT>
        ...
        qry="Select distinct YearMonth ,to_char(to_date('01'||yearmonth,'ddyyyymm'),'MON-YYYY')yearmm " +
                "from payablesalary where CompanyCode='" + mComp + "' order by YearMonth DESC";
		//out.print(qry);
			%>
 ...
 <font face=arial size=2><STRONG>&nbsp;To Period </STRONG></FONT> 
    &nbsp;
		...
		//out.print(qry);
			...
			while(rst.next())
			{
				msmvt=rst.getString(1);
				msmt=rst.getString(2);
				if(request.getParameter("x")==null)
				{
                if(mSmvt.equals(""))
				{
					mSmvt=msmvt;
					%>
					<OPTION selected Value ="<%=msmvt%>"><font name="Arial" ><%=msmt%></font></option>
					<%
				}
				else
				{
					%>
					<OPTION Value ="<%=msmvt%>"><font name="Arial" ><%=msmt%></font></option>
					<%
				}
                }
                else
...
if(mTypeDed.equals("2008D000001"))
{
//out.print("INNNN");
            qry="select  NVL(a.COMPANYCODE,' ')COMPANYCODE,NVL(A.EMPLOYEEID,' ')EMPLOYEEID,to_char(A.OPENINGDATE,'dd-mm-yyyy')OPENINGDATE," +
            " to_char(a.OPENINGDATE,'yyyymm')opendt,A.EDID," +
            " nvl(A.OPENNINGBALANCE,0)OPENNINGBALANCE from EDOpeningBalance a , EDMASTER b " +
            ...
            if(rs.next())
                {
                 ...
                 mopbalance=rs.getDouble("OPENNINGBALANCE");
                  //mdate=rs1.getString("datee");
                 mdate=rs.getString("OPENINGDATE");;
                 }
     // else
      //    {

 //out.print(mopbalance+"LLLLL");
 if(mopbalance==0.0)
{
 qryd="select to_char(to_date('"+mFromPeriod+"','rrrrmm')-1,'dd-mm-yyyy')datee," +
        " to_char(to_date('"+mFromPeriod+"','yyyymm')-1,'yyyymm')opendt from dual";
//out.print(qryd);
rs1=db.getRowset(qryd);
if(rs1.next())
    {
    mdate=rs1.getString("datee");
    mOpenDT=rs1.getString("opendt");
    }
//mopbalance=0;
}
//}
...
// out.print(mopbalance+"ss");
  
  // out.print(mopbalance+"ss");
if(mopbalance==0.0)
{
    mopbalance=mSumfirst;
}
else
{
//if(mopbalance)
...
<table width="100%" align="center"><hr>
    ...
    <tr><td align="center"><font face="verdana" color="#a52a2a" size="2"> <B><U>Available PF Details</U> </b> </font> </td></tr>
</table>
...
qry1="select nvl(a.yearmonth,' ')yearmonth, to_char(to_date('01'||a.yearmonth,'ddyyyymm'),'MON-YYYY')yearmm , " +
