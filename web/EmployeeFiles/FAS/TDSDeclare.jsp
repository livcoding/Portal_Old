...
String mParaDesc="",mParaValue="",mFYear="";
...
qry=" select nvl(a.PANNO,' ')PANNO  , decode(c.sex,'M','Male','F','Female',c.sex)sex,nvl(to_char(c.DateofBirth, 'dd-mm-yyyy'),'N')DOB   from employeedetail a ,employeemaster c where a.EMPLOYEEID='"+mDMemberID+"' and  a.EMPLOYEEID=c.EMPLOYEEID  ";
//out.print(qry);
rs=db.getRowset(qry);
 if(rs.next())
			{
...
<form name="frm"  method="post">
			...
			<tr>
			<TD colspan=0 align=middle > 	<font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: verdana"><b><U>TDS Declaration Report </U></b></td>
			</tr>
			</table>
<table cellpadding="3" ALIGN=CENTER cellspacing="0" border=1 rules="groups">
			<tr>
			<td><font face="arial" size=2><b> Financial Year </b></font>


<%qry="Select distinct  FINYEAR From TDS#PARAMETER Where COMPANYCODE='"+mComp+"'   ORDER BY FINYEAR DESC ";
			//out.print(qry);
			rs=db.getRowset(qry);
			%>
...
qry="SELECT to_char(FROMPERIOD,'dd-mm-yyyy')FROMPERIOD, to_char(TOPERIOD,'dd-mm-yyyy')TOPERIOD, NVL(STATUS,'N')STATUS ,NVL(ASSESSMENTYEAR,' ')ASSESSMENTYEAR , NVL(FINYEAR,' ')FINYEAR,NVL(TDSCODE,' ')TDSCODE		FROM TDS#PARAMETER where companycode='"+mComp+"' And FINYEAR='"+mFinYear+"' ";
//and trunc(SYSDATE) >= trunc(fromperiod)  and  trunc(SYSDATE) <= trunc(toperiod) ";  //sysdate between fromperiod and toperiod and
//out.print(qry);
rs=db.getRowset(qry);
if(rs.next())
		{
		...
		mTDSCode=rs.getString("TDSCODE");
			...
			mTDSStatus=rs.getString("STATUS");
 ...
 qry1="select COMPANYCODE, TDSCODE, ASSESSMENTYEAR, EMPLOYEEID, to_char(DECLARATIONDATE,'dd-mm-yyyy')DECLARATIONDATE, FINYEAR,nvl(FINALFREEZE,'N')FINALFREEZE  from TDS#EDIDECLARATIONHEADER where  COMPANYCODE='"+mComp+"' and  TDSCODE='"+mTDSCode+"' and ASSESSMENTYEAR='"+mAssessYear+"' and EMPLOYEEID='"+mDMemberID+"' ";
if(mTDSStatus.equals("F"))
			{
			qry1=qry1+" AND  nvl(FINALFREEZE,'N')='Y' ";
			}
			else
			{
				qry1=qry1+" AND  nvl(FREEZE,'N')='Y' ";
			}
...
if(rs1.next())
		{
			...
			mStatus=rs1.getString("FINALFREEZE");
%>
...
//913201021449  
	...
	qry2="SELECT  nvl(a.PARAMETERVALUE,' ')PARAMETERVALUE,nvl(b.PARAMETERDESC,' ')PARAMETERDESC FROM TDS#DECLARATIONPARADETAIL a,TDS#EDIDECLARATIONPARA b WHERE a.COMPANYCODE='"+mComp+"' AND a.TDSCODE='"+mTDSCode+"' AND a.ASSESSMENTYEAR='"+mAssessYear+"' AND  a.employeeid='"+mDMemberID+"' and a.COMPANYCODE=b.COMPANYCODE and a.TDSCODE=b.TDSCODE and a.ASSESSMENTYEAR=b.ASSESSMENTYEAR and a.EDITYPE=b.EDITYPE and a.EDICODE=b.EDICODE and a.PARAMETERID=b.PARAMETERID    ";
	//out.print(qry2);
		rs2=db.getRowset(qry2);
		if(rs2.next())
			{
...
qry="SELECT distinct a.COMPANYCODE, a.TDSCODE, a.ASSESSMENTYEAR,    a.EMPLOYEEID, a.EDITYPE, a.CATEGORYCODE,    a.EDICODE,  nvl(decode(a.DECLARATIONAMOUNT,'0',' ',a.DECLARATIONAMOUNT),' ')DECLARATIONAMOUNT, a.FINYEAR,    a.ACTUALRECEIVED, nvl(a.FINAL,' ')FINAL,    nvl(decode(a.actualamount,'0',' ',a.actualamount),' ')actualamount, nvl(a.LANDLORDPANNO,' ')LANDLORDPANNO, nvl(a.LANDLORDNAME,' ')LANDLORDNAME, nvl(a.EXEMPTEDAMOUNT,0)EXEMPTEDAMOUNT,b.SECTIONCODE,b.SECTIONDESC,nvl(b.DECLARATIONSEQID,0)DECLARATIONSEQID,nvl(c.SEQUENCEID,0)SEQUENCEID FROM TDS#EDIDECLARATIONDETAIL a,TDS#SECTIONMASTER b, TDS#TDSSECTIONTAGGING c where a.companycode =  '"+mComp+"'     AND a.tdscode =  '"+mTDSCode+"'        AND a.finyear = '"+mFinancialYear+"'     and a.EMPLOYEEID='"+mDMemberID+"'     and a.ASSESSMENTYEAR='"+mAssessYear+"'      and a.EDITYPE=c.EDITYPE     and  a.EDICODE=c.EDICODE     and a.COMPANYCODE=c.COMPANYCODE     and a.FINYEAR=c.FINYEAR     and a.TDSCODE=c.TDSCODE     and a.COMPANYCODE=b.COMPANYCODE     and a.TDSCODE=b.TDSCODE     and b.COMPANYCODE=c.COMPANYCODE     and b.TDSCODE=c.TDSCODE     and b.SECTIONCODE=c.SECTIONCODE order by nvl(b.DECLARATIONSEQID,0),nvl(c.SEQUENCEID,0)";
...
//out.print(rs.getString("DECLARATIONAMOUNT")+" 999 ");
...
/*
	qry2="SELECT nvl(INVESTMENTAMOUNT,0)INVESTMENTAMOUNT,nvl(INVESTMENTDESC,' ')INVESTMENTDESC,nvl(ACTUALAMOUNT,0)ACTUALAMOUNT FROM TDS#OTHERDECLARATION WHERE 	 COMPANYCODE='"+mComp+"' AND TDSCODE='"+mTDSCode+"' AND ASSESSMENTYEAR='"+mAssessYear+"' and  EMPLOYEEID='"+mChkMemID+"' and nvl(ADDITIONALINFO,'N')<>'Y' ";
	*/
//	out.print(qry2);
	rs2=db.getRowset(qry2);
					...
					%>
						...
						else
						{
							...
							<UL>
								...
								<%
									if(!rs.getString("LANDLORDPANNO").equals(""))
							{
									%><br>
								LandLord Name	 : <%=rs.getString("LANDLORDNAME")%>&nbsp;&nbsp;
								LandLord PANNO.	 : <%=rs.getString("LANDLORDPANNO")%>
									<%
							}
