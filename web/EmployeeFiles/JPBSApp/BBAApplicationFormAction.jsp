...
String mQYear="",mCategory="",mFatherName="",mMotherName="",mOTHYEAR="",mGMAT="",mAdhar="";
...
String dddate="",  ddamount="",   ddbankname="";
String acname="",  acbankname="",   acno="";
String acbranch="",  acaddress="",  acifsc="";
...
if(request.getParameter("dddate")==null)
	...
	dddate=request.getParameter("dddate").toString().trim();

if(request.getParameter("ddamount")==null)
	ddamount="";
else
	ddamount=request.getParameter("ddamount").toString().trim();

if(request.getParameter("ddbankname")==null)
	ddbankname="";
else
	ddbankname=request.getParameter("ddbankname").toString().trim();

if(request.getParameter("acname")==null)
	acname="";
else
	acname=request.getParameter("acname").toString().trim();

if(request.getParameter("acno")==null)
	acno="";
else
	acno=request.getParameter("acno").toString().trim();

if(request.getParameter("acbankname")==null)
	acbankname="";
else
	acbankname=request.getParameter("acbankname").toString().trim();


if(request.getParameter("acbranch")==null)
	acbranch="";
else
	acbranch=request.getParameter("acbranch").toString().trim();
if(request.getParameter("acaddress")==null)
	acaddress="";
else
	acaddress=request.getParameter("acaddress").toString().trim();
if(request.getParameter("acifsc")==null)
	acifsc="";
else
	acifsc=request.getParameter("acifsc").toString().trim();
...
if (request.getParameter("adhar")==null)
	mAdhar="";
else
 	mAdhar=request.getParameter("adhar").toString().trim();
...
if (request.getParameter("DDDATE")==null)
	...
	mdd="";
 	...
 	mdd=request.getParameter("dd").toString().trim();
...
{
	qrya="SELECT NVL (MAX (TO_char (NVL (SUBSTR (APPLICATIONSLNO, -4), 0))), 0) cnt FROM C#BBAAPPLICATIONMASTER where " +
         " applicationslno like '2020BBAO%'";

     //     "Substr(APPLICATIONSLNO,1,7)='2019BBA' and APPLICATIONSLNO not like '%WE%' and SUBSTR (applicationslno, 1, 8)" +
      //    " <> '2019BBAO'  ";
...
qry="select 'Y' from C#BBAAPPLICATIONMASTER where APPLICATIONNO='"+mAppNo+"' and APPLICATIONSLNO='"+mApplicantSlno +"' and nvl(deactive,'N')='N'  ";
//out.print(qry);
rs=db.getRowset(qry);
...
if(!mAppNo.equals(""))
{
qry="INSERT INTO C#BBAAPPLICATIONMASTER (APPLICATIONSLNO, APPLICATIONNO, STUDENTNAME, FATHERNAME,MOTHERNAME,AADHAAR_NO, ";
qry=qry+" DATEOFBIRTH, CATEGORY,SEX, ENTRYBY, ENTRYDATE,   DEACTIVE,INSTITUTECODE, NATIONALITY) VALUES ( ";
qry=qry+" '"+mApplicantSlno+"','"+mAppNo+"' ,'"+mFName+"'  ,'"+mFatherName+"' ,'"+mMotherName+"' ,'"+mAdhar+"' , ";
qry=qry+" to_date('"+mDOB+"','dd-mm-yyyy')  ,'"+mCategory+"' , '"+mSex+"','"+mChkMemID+"',Sysdate ,'N','JIIT', '"+mNationality+"')";
//out.print(qry);
//int n=1;
...
if(acname!=null && !acname.equalsIgnoreCase("")&& acbankname!=null && !acbankname.equalsIgnoreCase("")&&acno!=null && !acno.equalsIgnoreCase("")){
    
    String pqery="INSERT INTO C#BBAPERSONALBANKDETAIL (APPLICATIONSLNO, NAMEOFACHOLDER,  BANKACCOUNTNO, NAMEOFBANK, BRANCHNAME,    BANKADDRESS,IFSCCODE)" +
            " VALUES ('"+mApplicantSlno+"' ,'"+acname+"' ,'"+acno+"' ,'"+acbankname+"' , '"+acbranch+"','"+acaddress+"','"+acifsc+"')";

}



if(!dddate.equals("") && !ddamount.equals("") && !ddbankname.equals(""))
	{
	qryfee="INSERT INTO C#BBAFEE (   APPLICATIONSLNO, CHEQUEDDNO,  CHEQUEDDDATE, CHQDDTYPE, AMOUNT,    BANKNAME) VALUES ('"+mApplicantSlno+"' ,'"+mdd+"' , to_date('"+dddate+"','dd-mm-yyyy')   ,'D' ,'"+ddamount+"' , '"+ddbankname+"')";
	}
	else
	{
		mDDAMT="500";
	qryfee="INSERT INTO C#BBAFEE (   APPLICATIONSLNO,  CHEQUEDDNO,  CHEQUEDDDATE, CHQDDTYPE, AMOUNT,    BANKNAME) VALUES ('"+mApplicantSlno+"',' ' , to_date('"+dddate+"','dd-mm-yyyy'),'P','"+ddamount+"' , '')";
//out.print(qryfee);
	}
...
if(!TenYear.equals(""))
	{
	qryten="INSERT INTO C#BBAQUALIFICATION (  APPLICATIONSLNO, QUALIFICATIONCODE, QUALIFICATIONDESC, PERCENTAGE,DOCUMENT,BOARD, PASSING_YEAR, SUBJECTS) VALUES ('"+mApplicantSlno+"' ,'10TH' ,'TENTH' , '"+TenPercent+"','"+Doc10+"','"+TenBoard+"','"+TenYear+"','"+TenStream+"')";
		int ten=db.insertRow(qryten);
	}
if(!TewYear.equals("") )
	{
		qrytw="INSERT INTO C#BBAQUALIFICATION (   APPLICATIONSLNO, QUALIFICATIONCODE, QUALIFICATIONDESC, PERCENTAGE,DOCUMENT ,BOARD, PASSING_YEAR, SUBJECTS) VALUES ( '"+mApplicantSlno+"','12TH' ,'TWELFTH' , '"+TewPercent+"','"+Doc12+"','"+TewBoard+"','"+TewYear+"','"+TewStream+"')";
...
<tr><td ALIGN=middle>
<STRONG><FONT face=Verdana size=4>BBA APPLICATION FORM - 2020 </FONT><br><FONT size=2></FONT></STRONG>
<BR>
</td></tr>
</table>
<%
...
qry="SELECT a.applicationslno, a.applicationno, a.studentname, a.fathername,to_char(nvl(A.DATEOFBIRTH,SYSDATE),'dd-MM-yyyy') DOB,      decode(a.CATEGORY,'GN','General','SC','SC','ST','ST','OBC','OBC')CATEGORY,       DECODE(a.sex,'F','Female','M','Male')SEX, nvl(a.nationality, '')nationality, nvl(b.ADDRESS1,' ')ADDRESS1, nvl(b.ADDRESS2,' ')ADDRESS2, nvl(b.CITY,' ')CITY, nvl(b.STATE,' ')STATE, nvl(b.PIN, ' ')PIN, nvl(b.COUNTRY,' ')COUNTRY, nvl(b.PHONE,' ')PHONE, nvl(b.EMAILID,' ')EMAILID FROM C#bbaapplicationmaster a,C#BBAAPPLICANTADDRESS b WHERE a.applicationslno = '"+mApplicantSlno+"'  and a.APPLICATIONSLNO=b.APPLICATIONSLNO";


// decode(a.APPEARINGININST,'Y','Yes','N','No','No')APPEARINGININST,, decode(a.checkscorecard,'Y','Yes','N','No','No')checkscorecard
...
<%
	qryqua="SELECT distinct QUALIFICATIONCODE, QUALIFICATIONDESC,PERCENTAGE, decode(DOCUMENT,'Y','Yes','N','No','No')DOCUMENT FROM C#BBAQUALIFICATION WHERE APPLICATIONSLNO='"+mApplicantSlno+"' " ;
...
<%
qry="SELECT  nvl(QUALIFICATIONCODE,' ')QUALIFICATIONCODE, nvl(QUALIFICATIONDESC,' ')QUALIFICATIONDESC, PERCENTAGE,  nvl(BOARD,' ')BOARD,PASSING_YEAR, nvl(SUBJECTS,' ')SUBJECTS from C#BBAQUALIFICATION where APPLICATIONSLNO='"+mApplicantSlno+"'";
rs=db.getRowset(qry);
while(rs.next())
{
%>
...
{
qryfee=" SELECT APPLICATIONSLNO, nvl(CHEQUEDDNO,' ')CHEQUEDDNO,to_char(nvl(CHEQUEDDDATE,SYSDATE),'dd-MM-yyyy')CHQDATE,CHQDDTYPE, nvl(AMOUNT,'')AMOUNT, nvl(BANKNAME,' ')BANKNAME  FROM C#BBAFEE where APPLICATIONSLNO='"+mApplicantSlno+"'  ";
...
<tr>
<%--<td colspan=3>
<FONT face=Verdana>15.  <b>Rs. 1000/-</b>
</FONT>
</td>--%>
</tr>
<%
