...
int CTR=0, mMaxLevel=0, mLeaveApprovalLevel=0, mTotOnWFLevel=0;
...
function FunSubmit2()
{
	document.frm.action="LeaveRequestApprovalToChangedRoute.jsp" ;
}
function validate()
{
	alert('saddsadsa');
...
<!-------------Page Heading and Marquee Message----------------------->
			...
			try
			{
				String mPageHeader="Leave Work Flow For Approval/Cancellation", mMarqMsg="", CurrDate="";
				...
				qry="Select nvl(B.PAGEHEADER,'Leave Work Flow For Approval/Cancellation')PageHeader FROM WEBKIOSKRIGHTSMASTER B WHERE B.RIGHTSID='"+mRightsID+"' and B.RELATEDTO LIKE '%E%'";
				rs=db.getRowset(qry);
				if(rs.next())
				{
...
<!-------------Page Heading and Marquee Message----------------------->
			...
			qry="select nvl(PURPOSEOFLEAVE,'No such reason')POL, nvl(PAID,0)PAID, nvl(WITHOUTPAY,0)LWP, to_char(ENTRYDATE,'DD-MM-YYYY')ReqDate from LeaveRequest where CompanyCode='"+mComp+"' and RequestID='"+QryRID+"'";
			rs=db.getRowset(qry);
			//out.print(qry);
			if(rs.next())
			{
			...
			<td nowrap><font color=black face=arial size=2><STRONG>&nbsp; &nbsp; By : &nbsp;</STRONG></font></td>
			<td nowrap><font color=black face=arial size=2><%=mENM%></Font></td></tr>
			<tr><td nowrap><font color=black face=arial size=2><STRONG>&nbsp;No. of day(s) leave (<%=QryWFType%>) applied</STRONG></font></td>
			<td nowrap><font color=black face=arial size=2><b>: &nbsp; &nbsp;</b></font></td>
			<td nowrap><font color=black face=arial size=2><%=mTotalLvDays%><STRONG> day(s)</STRONG></Font></td>
			<td nowrap><font color=black face=arial size=2><STRONG>&nbsp; &nbsp; on : &nbsp;</STRONG></font></td>
			<td nowrap><font color=black face=arial size=2><%=QryReqDate%></Font></td></tr>
			<tr><td nowrap><font color=black face=arial size=2><STRONG>&nbsp;Period of leave applied from</STRONG></font></td>
			<td nowrap><font color=black face=arial size=2><b>: &nbsp; &nbsp;</b></font></td>
			<td nowrap><font color=black face=arial size=2><%=QryLvDateFr%></Font></td>
			<td nowrap><font color=black face=arial size=2><STRONG>&nbsp; &nbsp; to &nbsp;: &nbsp;</STRONG></font></td>
			<td nowrap><font color=black face=arial size=2><%=QryLvDateTo%></Font></td></tr>
			<tr><td colspan=5 nowrap><font color=black face=arial size=2><STRONG>&nbsp;Purpose of leave : &nbsp;</STRONG><%=QryLvPurp%></Font></td>
...
//---------------Start Of if(rsFrDept.next())----
				...
				try
				{
					while(rs.next())
					{
						//out.print(QryRID);
						qry="Select nvl(MIN(WFL),0) WFL FROM";
						qry=qry+" (Select nvl(WFLEVEL,0)WFL from WF#WORKFLOWAUTHORITY Where CompanyCode='"+mComp+"' and InstituteCode='"+mInst+"' and WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"' GROUP BY WFLEVEL";
						qry=qry+" MINUS ";
						qry=qry+" Select nvl(WFLEVEL,0)WFL from WF#WORKFLOWDETAIL Where REQUESTID='"+QryRID+"' AND CompanyCode='"+mComp+"' and InstituteCode='"+mInst+"' and WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"' AND REQUESTID='"+QryRID+"' GROUP BY WFLEVEL)";
						rs1=db.getRowset(qry);
						//out.print(qry);
						...
						if(mTotOnWFLevel>1)
						{
							...
							qry=qry+" CompanyCode='"+mComp+"' and InstituteCode='"+mInst+"' and WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"'";
							qry=qry+" AND DEPARTMENTCODE='"+mDeptCode+"' AND WFLEVEL='"+CTR+"' AND "+mTotalLvDays+" between FROMVALUE and TOVALUE";
							qry=qry+" GROUP BY APPROVALBY, APPROVALAUTHORITY, DEPARTMENTCODE, WFSEQUENCE";
							rs1=db.getRowset(qry);
							//out.print(qry);
							if(rs1.next())
...
//---------------End Of if(rsFrDept.next())----
			...
			if(mAprMemID.equals(mChkMemID))
			{
				...
				%>
				<tr><td colspan="5">
				&nbsp;<font color=black face=arial size=2><STRONG>Leave Recommendation As : &nbsp;</STRONG></font>
			...
			if(request.getParameter("x")!=null)
			{
				...
				if(request.getParameter("AprStat")==null)
					...
					QryStatus=request.getParameter("AprStat").toString().trim();
				...
				//out.print(mShowRemTo+"-"+mRemarks1+"-"+QryStatus+"-"+mApproveBy+"-"+mDepartment+"-"+mWFSequence+"-"+mWFLevel);
				...
				if(!rs.next())
				{
					if(!mRemarks1.equals("") && mRemarks1.length()<=200)
					{
						...
						qry=qry+" VALUES('"+mComp+"', '"+mInst+"', '"+QryWFCode+"', '"+QryWFType+"', '"+mDepartment+"', "+mWFSequence+", "+mWFLevel+", '"+QryRID+"', '"+QryStatus+"', "+mTotalLvDays+", '"+mRemarks1+"', '"+mApproveBy+"', sysdate, '"+mTextToAprDisp+"', '"+mCanAllow+"', '"+mShowRemTo+"')";
						n=db.insertRow(qry);
						//out.print(qry);
						if(n>0 && QryStatus.equals("A"))
						{
						  //-----------Log Entry------------
							db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"LEAVE WORK FLOW APPROVAL/CANCELLATION", "Staff: "+QryFaculty+" Leave From: "+QryLvDateFr+" To: "+QryLvDateTo+" Request: "+QryRID+" - Approved", "No MAC Address" , mIPAddress);
						  //----------------------------------
							if(mDiffPath.equals("Y"))
							{
								%>
								<LEFT><Br><b><font size=2 face='Arial' color='Green'>
								Leave Recommendation on Current Level is Successful... <a href='LeaveRequestApprovalToChangedRoute.jsp?EID=<%=QryFaculty%>&amp;RID=<%=QryRID%>&amp;LEAVECODE=<%=QryWFType%>&amp;DATEFROM=<%=QryLvDateFr%>&amp;DATETO=<%=QryLvDateTo%>&amp;DeptCode=<%=mDeptCode%>&amp;LeaveDays=<%=mTotalLvDays%>' style='cursor:hand'><font color=blue>Click </font></a>To Change Subsequent Route
								</font></LEFT>
								<%
							...
							else
							{
								...
								//rd.forward(request,response);
								%><CENTER><%
								out.print("<br>&nbsp;&nbsp;&nbsp <b><font size=2 face='Arial' color='Green'>Leave Recommendation on Current Level is Successful...</font> <br>");
								%></CENTER><%
							}
							mSendInfoFlag++;
						}
						else if(n>0 && QryStatus.equals("C"))
						{
						  //-----------Log Entry------------
							db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"LEAVE WORK FLOW APPROVAL/CANCELLATION", "Staff: "+QryFaculty+" Leave From: "+QryLvDateFr+" To: "+QryLvDateTo+" Request: "+QryRID+" - Canceled", "No MAC Address" , mIPAddress);
						  //----------------------------------
							mSendInfoFlag++;

							qry="UPDATE LEAVEREQUEST SET";
							qry=qry+" STATUS='"+QryStatus+"',";
							qry=qry+" APPROVEDSTATUS='"+QryStatus+"',";
							qry=qry+" APPROVEDBY='"+mApproveBy+"',";
							qry=qry+" APPROVEDDATE=sysdate,";
							qry=qry+" WORKFLOWCODE='"+QryWFCode+"',";
							qry=qry+" WORKFLOWTYPE='"+QryWFType+"'";
							qry=qry+" where REQUESTID='"+QryRID+"' AND EMPLOYEEID='"+QryFaculty+"' AND LEAVECODE='"+QryWFType+"' AND STARTDATE=to_date('"+QryLvDateFr+"','dd-mm-yyyy')";
							int nn=db.update(qry);
							//out.print(qry);
							if(nn>0)
							{
								%><CENTER><%
								out.print("&nbsp;&nbsp;&nbsp <b><font size=2 face='Arial' color='Green'>Leave </Font><Font color='RED'>Cancellation</Font> <Font color='Green'>Successful...</font> <br>");
								%></CENTER><br><%
							}
						}
						else
						{
							...
							out.print("&nbsp;&nbsp;&nbsp <b><font size=2 face='Arial' color='Red'>Error while Leave Approval/Cancellation!</font> <br>");
							%></CENTER><%
						}
						if(mSendInfoFlag>0)
						{
								...
								if(!mRemarks2.equals(""))
								{
									if(mRemarks2.length()<=200)
									{
										...
										if(!rs.next())
										{
											...
											qry=qry+" VALUES('"+mEmpID+"', '"+mEmpType+"', '"+QryRID+"', '"+mComp+"', '"+mInst+"', '"+QryWFCode+"', '"+QryWFType+"', '"+mDepartment+"', "+mWFSequence+", '"+QryStatus+"', "+mTotalLvDays+", '"+mRemarks2+"', '"+mApproveBy+"', sysdate, '', 'N')";
											n1=db.insertRow(qry);
											//out.print(qry);
											if(n1>0 && QryStatus.equals("A"))
											{
											  //-----------Log Entry------------
												db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"INFO FOR LEAVE WORK FLOW APPROVAL/CANCELLATION", "Informed To: "+mEmpID+" Request ID: "+QryRID+" - Approved", "No MAC Address" , mIPAddress);
											  //----------------------------------
											}
											else if(n1>0 && QryStatus.equals("C"))
											{
											  //-----------Log Entry------------
												db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"INFO FOR LEAVE WORK FLOW APPROVAL/CANCELLATION", "Informed To: "+mEmpID+" Request ID: "+QryRID+" - Cancelled", "No MAC Address" , mIPAddress);
											  //----------------------------------
											}
										}
									}
					...
					else
					{
						%><CENTER><%
						out.print("<img src='../../../Images/Error1.gif'>");
						out.print("&nbsp;&nbsp;&nbsp <b><font size=2 face='Arial' color='Red'>Remarks for Leave Recommendation can't be left blank or more than 200 characters!</font> <br>");
						%></CENTER><%
					}
				}
				...
				if(mWFLevel==mMaxLevel)
				{
					...
					qry=qry+" WORKFLOWTYPE='"+QryWFType+"'";
					qry=qry+" where REQUESTID='"+QryRID+"' AND EMPLOYEEID='"+QryFaculty+"' AND LEAVECODE='"+QryWFType+"' AND STARTDATE=to_date('"+QryLvDateFr+"','dd-mm-yyyy')";
					int nnn=db.update(qry);
					//out.print(qry);
					if (nnn>0)
					{
						%><LEFT><%
						out.print("&nbsp;&nbsp;&nbsp <b><font size=2 face='Arial' color='Green'>Leave Request Approval/Cancellation has been approved successfully... </font> <br>");
						%></LEFT><br><%
					}
				}
			} //closing of outer if
		   ...
		   else
		   {
			%><CENTER><%
			out.print("<img src='../../../Images/Error1.gif'>");
			out.print("&nbsp;&nbsp;&nbsp <b><font size=2 face='Arial' color='Red'>Leave Workflow of this request has been canceled!</font> <br>");
			%></CENTER><%
		   }
//-----------------------------
//---Enable Security Page Level  
