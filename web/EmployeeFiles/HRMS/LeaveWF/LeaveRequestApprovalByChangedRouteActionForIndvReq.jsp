...
int ctr=0, CTR=0, mTotal=0, n=0, n1=0, nnn=0;
				...
				if (request.getParameter("TotalCount")!=null && Integer.parseInt(request.getParameter("TotalCount").toString().trim())>0)
				{
					...
					for (int i=1;i<=mTotal;i++)
					{
						...
						if (request.getParameter(mName9)!=null)
							mDepartmentCode=request.getParameter(mName9).toString().trim();
						else
							mDepartmentCode="";
			//--------------------------------------------------
						if(mChk.equals("UnChecked"))
						{
							...
							qry="Select nvl(APPROVALBY,' ')APPROVALBY from WF#WORKFLOWDETAIL where COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"' AND DEPARTMENTCODE='"+mDepartmentCode+"' AND WFSEQUENCE='"+mWFSeq+"' AND WFLEVEL='"+mWFLevel+"' AND REQUESTID='"+QryRID+"'";
							rs=db.getRowset(qry);
							if(rs.next())
							{
								mApprAuth=rs.getString("APPROVALBY");
							}
							qry="Select 'Y' from WF#REQUESTWORKFLOWPATH where REQUESTID='"+QryRID+"' AND COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"' AND DEPARTMENTCODE='"+mDepartmentCode+"' AND WFSEQUENCE='"+mWFSeq+"'";
							rs=db.getRowset(qry);
							if(rs.next())
							{
								...
								qry=qry+" COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"' AND DEPARTMENTCODE='"+mDepartmentCode+"' AND WFSEQUENCE='"+mWFSeq+"'";
								n=db.update(qry);
							}
							else
							{
								...
								qry=qry+"VALUES ('"+QryRID+"', '"+mComp+"', '"+mInstitute+"', '"+mDepartmentCode+"', '"+mWFSeq+"', '"+mWFLevel+"', 'E', '"+mApprAuth+"', '"+mTextToFwd+"')";
								n=db.insertRow(qry);
							}
						}
			//--------------------------------------------------
						else if(mActCurr.equals("E") && mChk.equals("Checked"))
						{
							...
							if(mChk.equals("Checked"))
							{
							   if(mSetTextToFwd.equals("Y"))
							   {
								...
								{
									qry="INSERT INTO WF#REQUESTWORKFLOWPATH (REQUESTID, COMPANYCODE, INSTITUTECODE, DEPARTMENTCODE, WFSEQUENCE, WFLEVEL, APPROVALBY, APPROVALAUTHORITY, APPROVALTEXTTOBEDISPLAYED)";
									qry=qry+"VALUES ('"+QryRID+"', '"+mComp+"', '"+mInstitute+"', '"+mDepartmentCode+"', '"+mWFSeq+"', '"+mWFLevel+"', '"+mApprBy+"', '"+mApprAuth+"', '"+mTextToFwd+"')";
									n=db.insertRow(qry);
								}
								//out.print(qry);
							   }
							   else if(!mSetTextToFwd.equals("Y"))
							   {
								...
								else
								{
									...
									qry=qry+"VALUES ('"+QryRID+"', '"+mComp+"', '"+mInstitute+"', '"+mDepartmentCode+"', '"+mWFSeq+"', '"+mWFLevel+"', '"+mApprBy+"', '"+mApprAuth+"')";
									n=db.insertRow(qry);
								}
							   }
			...
			//--------------------------------------------------
						else if(mActCurr.equals("D") && mChk.equals("Checked"))
						{
							...
							if(rs.next())
							{
								...
								qry=qry+" AND DEPARTMENTCODE='"+mDepartmentCode+"' AND WFLEVEL='"+mWFLevel+"'";
								//out.print(qry);
								n1=db.update(qry);
								if(n1>0)
								{
									// Log Entry
									//-----------------
								      db.saveTransLog(mInstitute,mLogEntryMemberID,mLogEntryMemberType ,"DISCARD LEAVE WORK FLOW LEVEL", "WFCode:"+mWFCode+" WFType:"+QryWFType+" Req Dept:"+mDepartmentCode+" WFSeq:"+mWFSeq, "NO MAC Address" , mIPAddress);
									//-----------------
									mDelFlag++;
									mDelWFLvl=mWFLevel;

									qry="SELECT nvl(WFLEVEL,0) WFLEVEL FROM WF#REQUESTWORKFLOWPATH WHERE REQUESTID='"+QryRID+"' AND COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"'";
									qry=qry+" AND DEPARTMENTCODE='"+mDepartmentCode+"' AND WFLEVEL > "+mDelWFLvl+" order by WFLEVEL ASC";
									rs=db.getRowset(qry);
									//out.print(qry);
									while(rs.next())
									{
										...
										qry=qry+" REQUESTID='"+QryRID+"' AND COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"'";
										qry=qry+" AND DEPARTMENTCODE='"+mDepartmentCode+"' AND WFLEVEL='"+mWFLvl+"'";
										//out.print(qry);
				...
				if(AL1!=null)
				{
					...
					while(itr.hasNext())
					{
						...
						if(rs.next())
						{
							...
							qry=qry+" COMPANYCODE='"+COMP+"' AND INSTITUTECODE='"+INST+"' AND DEPARTMENTCODE='"+mDepartmentCode+"' AND WFSEQUENCE='"+SEQ+"'";
							nnn=db.update(qry);
						}
						else
						{
