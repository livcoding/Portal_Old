...
int mTSelected=0 ;
		qry1="select count(c.applicantid)selected from   hr#applicantshortlistmaster c,  " +
                ...
                "and NVL(BROADCAST,'N')='Y'  ) and a.departmentcode ='"+mDeptCode+"'   and a.DESIGNATIONCODE='"+mDesignation+"'       and c.STATUS='S'   " +
                "and  c.SHORTLISTSEQNO =3  and a.vacancycode ='"+mVacCode+"'    AND a.applicantid = c.applicantid	" +
...
//192.168.4.221   juet4guna
		...
		int mTReject=0 ;
			qry2="select count(c.applicantid)reject from   hr#applicantshortlistmaster c,  hr#applicationmaster a " +
                    ",hr#applicantaddress D where a.vacancycode in (select vacancycode from HR#VACANCYMASTER WHERE" +
                    " a.vacancycode='"+mVacCode+"' and institutecode='"+mInst+"' and NVL(BROADCAST,'N')='Y'  ) and a.departmentcode ='"+mDeptCode+"' and a.DESIGNATIONCODE='"+mDesignation+"'   " +
                    "       and nvl(c.STATUS,'N')='R'   and  c.SHORTLISTSEQNO =3  and a.vacancycode ='"+mVacCode+"'              AND a.applicantid = c.applicantid			and a.institutecode='"+mInst+"' AND a.vacancycode = c.vacancycode AND a.institutecode = c.institutecode  AND a.applicantid = D.applicantid AND C.applicantid = D.applicantid ";
		...
		int mALLSum=0;
		qry2="select count(c.applicantid)AllResume from   hr#applicantshortlistmaster c,  hr#applicationmaster a ,hr#applicantaddress D where a.vacancycode in (select vacancycode from HR#VACANCYMASTER WHERE institutecode='"+mInst+"' and NVL(BROADCAST,'N')='Y'  ) and a.departmentcode ='"+mDeptCode+"' and a.DESIGNATIONCODE='"+mDesignation+"' " +
                "        and  c.SHORTLISTSEQNO =2 and nvl(c.STATUS,'N')='S' and a.vacancycode ='"+mVacCode+"'   AND a.applicantid = D.applicantid AND C.applicantid = D.applicantid AND a.applicantid = c.applicantid			and a.institutecode='"+mInst+"' AND a.vacancycode = c.vacancycode AND a.institutecode = c.institutecode and nvl(c.final,'N')='Y'";
		//out.print(qry2);
		rs1=db.getRowset(qry2);
		if(rs1.next())
			...
			int mTNotProcess=0;
			qry2="select count(c.applicantid)mTNotProcess from   hr#applicantshortlistmaster c,  hr#applicationmaster a,hr#applicantaddress D where a.vacancycode in (select vacancycode from HR#VACANCYMASTER WHERE a.vacancycode='"+mVacCode+"' and institutecode='"+mInst+"' and NVL(BROADCAST,'N')='Y'  ) and a.departmentcode ='"+mDeptCode+"'   " +
                    "   and a.DESIGNATIONCODE='"+mDesignation+"'    and nvl(c.STATUS,'N')='N'   and  c.SHORTLISTSEQNO =3  and a.vacancycode ='"+mVacCode+"'              AND a.applicantid = c.applicantid			and a.institutecode='"+mInst+"' AND a.vacancycode = c.vacancycode AND a.institutecode = c.institutecode  AND a.applicantid = D.applicantid AND C.applicantid = D.applicantid";
			//out.print(qry2);
		rs1=db.getRowset(qry2);
		if(rs1.next())
...
if(request.getParameter("DESCODE")==null)
 ...
 qry = "SELECT distinct COMPANYCODE, INSTITUTECODE, VACANCYCODE, VACANCYDESCRIPTION," +
            ...
            "  VACANCYCODE in (       SELECT DISTINCT VACANCYCODE                                 FROM hr#vacancydepartmentselection                                WHERE  GUESTID='"+mChkMemID+"' and DEPARTMENTCODE='"+mDeptCode+"' and VACANCYCODE='"+mVacCode+"' )";
rs=db.getRowset(qry);
if(rs.next())
					{
...
qqry="select * from (select  emailid,      " +
      ...
      " , to_char(ROWNUM) rn " +
        ...
        " where rn>="+start+" and rn<="+last+"  order by  firstname,shortlistseqno desc ,forsorting,rn ";
...
memailid=rs.getString("EMAILID").toString().trim();
	...
	//out.print(mStatus+"sdsss"+rs.getString("STATUS").toString().trim());		
				...
				qry1="SELECT  nvl(SHORTLISTSEQNO,0)SHORTLISTSEQNO ,  nvl(STATUS,' ')STATUS,nvl(REMARKS,' ')REMARKS " +
                        "   FROM HR#APPLICANTSHORTLISTMASTER a WHERE INSTITUTECODE='"+mInst+"' AND  VACANCYCODE='" + mVacCode + "'" +
                        " AND APPLICANTID ='"+mApplicantID+"'  and ";
				if(mShow.equals("ALL"))
		{
		...
		{
						qry1=qry1+"   NVL(A.status,'R') ='R' and A.shortlistseqno  in (           " +
                                "           SELECT (MAX (d.shortlistseqno))                " +
                                "        FROM hr#applicantshortlistmaster d                 " +
                                "      WHERE d.institutecode =  '"+mInst+"'        and d.vacancycode = '" + mVacCode + "' " +
                                "and a.APPLICANTID=d.APPLICANTID)  ";
		}
...
/*
                qry2="select notesofinterviewer,emailid,vacancycode1 from ( SELECT DISTINCT NVL (a.notesofinterviewer, ' ') notesofinterviewer, x.emailid emailid ,         " +
 
              "        NVL (c.vacancycode, ' ') vacancycode1            FROM hr#applicantaddress x,      " +
              "           hr#interviewresult a,                 hr#applicationmaster c WHERE x.emailid = LTRIM ('"+memailid+"')             AND x.applicantid  = c.applicantid             And c.VACANCYCODE  <> '" + mVacCode + "'            And c.COMPANYCODE = a.companycode             and c.institutecode = a.institutecode             and c.applicantid = a.applicantid             AND nvl(a.RESULT,'N') <> 'S'        ";
qry2=qry2+"  union SELECT DISTINCT NVL (a.INTERVIEWNOTE, ' ') notesofinterviewer, x.emailid emailid,to_char(' ') vacancycode1     " +
...
qry5="                  select  a.VACANCYCODE,a.APPLICANTID              from HR#SHORTLISTAPPLICANT a,hr#applicantaddress c    " +
        ...
        "            and NVL(a.ATTENDANCESTATUS,'N')='N'            AND NVL(A.SHORTLISTEDSTATUS,'N')='Y'           " +
        "      AND c.emailid = LTRIM ('"+memailid+"')    and a.vacancycode <>  '" + mVacCode + "'             " +
        "   and a.applicantid not in(select d.applicantid                                            from HR#interviewResult d       " +
        "                                     where d.vacancycode = a.vacancycode             " +
        "                               and d.applicantid = a.applicantid                   " +
        "                         and d.RESULT in ('R','S')                               " +
        "             AND d.INSTITUTECODE=a.INSTITUTECODE                                     " +
        "       and d.COMPANYCODE=a.COMPANYCODE                                         " +
        "  and d.INTERVIEWCODE=a.INTERVIEWCODE                                         )";
//out.print(qry5);
...
int mMonth=0;
            qry2 = "SELECT nvl(a.prevorganisation,' ')prevorganisation, " +
                    ...
                    " ,   nvl(SUBSTR (MONTHS_BETWEEN ( decode(a.TILLDATE,'Y',sysdate, a.todate),a.FROMDATE) / 12,1," +
                    "INSTR (MONTHS_BETWEEN (decode(a.TILLDATE,'Y',sysdate, a.todate),a.FROMDATE) / 12, '.') - 1  ),'N') year1, " +
                    "nvl(ROUND (  SUBSTR (MONTHS_BETWEEN (decode(a.TILLDATE,'Y',sysdate, a.todate),a.FROMDATE) / 12," +
                    "INSTR (MONTHS_BETWEEN (decode(a.TILLDATE,'Y',sysdate, a.todate),FROMDATE) / 12,'.')) * 12 ),'0') month1, " +
                    "decode(a.tillDate,'Y','TillDate','',' ',a.tillDate)tillDate,to_char(a.todate,'yyymmdd')todatee FROM " +
                    "hr#applicantexperience a WHERE applicantid = '" + mApplicantID + "' order by  todatee desc";
       // out.print(qry2);
            rs2 = db.getRowset(qry2);
            while (rs2.next()) {
