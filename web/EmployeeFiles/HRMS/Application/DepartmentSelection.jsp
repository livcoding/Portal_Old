...
%>
        <table  align=center >
            <tr><td ><h2>Department Wise Selection</h2></td></tr>
        </table>
        <br>
...
<form method=post action="DepartmentSelectAction.jsp">
 ...
 <table width="100%" border=1  borderColor=#D98242 rules=group align=center bottommargin=1 topmargin=0 cellspacing=0 cellpadding=2>
   ...
   <TD  align=center class="labelcell"><b>Click to View CV </b></TD>
         ...
         <td class="labelcell"><b><CENTER>H.O.D. Selection</CENTER></b></td>
         <td class="labelcell"><b><CENTER> Remark</CENTER></b> </td>
     </tr>

     <%
        qry = " SELECT nvl(a.SHORTLISTED,' ')SHORTLISTED, decode(a.SHORTLISTED,'S','Selected','R','Rejected','',' ',a.SHORTLISTED)FINALSHORTLISTED,c.SHORTLISTSEQNO,a.APPLICANTID,nvl(a.applicantname,' ') applicantname,to_char(a.dateofbirth,'DD-MM-YYYY')dateofbirth " +
                ",nvl(b.CADDRESS1,' ')CADDRESS1, nvl(b.CADDRESS2,' ')CADDRESS2, nvl(b.CADDRESS3,' ')CADDRESS3, " +
                "nvl(b.CCITY,' ')ccity, nvl(b.CDISTRICT,' ')CDISTRICT, nvl(b.CSTATE,' ')CSTATE, b.CPIN,nvl(c.STATUS ,' ')STATUS ,nvl(c.REMARKS,' ')REMARKS    " +
                ...
                "   AND a.departmentcode = '" + mDeptCode + "'    and a.APPLICANTID=c.APPLICANTID   and b.APPLICANTID=c.APPLICANTID   and a.VACANCYCODE=c.VACANCYCODE   and a.INSTITUTECODE=c.INSTITUTECODE AND A.COMPANYCODE=C.COMPANYCODE     AND A.INSTITUTECODE=C.INSTITUTECODE     AND a.institutecode = c.institutecode  and nvl(c.STATUS,'N')='S' and c.SHORTLISTSEQNO in (select LEVELNO FROM HR#SELECTIONLEVELS  where  INSTITUTECODE='"+mInst+"' AND  VACANCYCODE='" + mVacCode + "' AND SELECTIONBY LIKE '%DEP%' and nvl(FINAL,'N')='N'  ) ORDER BY a.applicantname";
        rs = db.getRowset(qry);
      // out.print(qry);
        while (rs.next())
        {
	 ...
	 <td valign=top>
             <select name="Status<%=slno%>" id="Status<%=slno%>" <%=mDisable%> onchange="return EnterRemarks('Status<%=slno%>','<%=slno%>')">
...
%>
<tr >
<td nowrap valign="top" class="labelcell"><%=mQCode%>-<%=rs1.getString("QUALIFICATIONCATEGORYCODE").toString()%></td>
<td class="labelcell" valign="top"><%=mINSTITUTION%></td>
<td class="labelcell" valign="top"><%=mYEAROFPASSING%></td>
        ...
        <table borderColor=#D98242 rules=group cellspacing=2 cellpadding=0>
          ...
          // out.print(qry);
            ...
            while (rs2.next()) {
             ...
             {
                         %>
             <td class="labelcell" valign=top nowrap><%=rs3.getString("department").toString().toUpperCase()%></td>
             <%
             }
             %>
             <td valign=top class="labelcell"> <font Color=red size=2><b><%=rs.getString("FINALSHORTLISTED").toString()%>                   </font>
             </td>
