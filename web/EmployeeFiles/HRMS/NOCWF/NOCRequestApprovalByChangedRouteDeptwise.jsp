...
String QryChDvFor="";
String mName1="",mName2="",mName3="",mName4="",mName5="",mName6="",mName8="",mName10="", mName11="", mName12="", mName13="", mName14="";
...
//-------------------------------------------------------
			if(QryChDvFor.equals("CurrDept"))
			{
				...
				<td nowrap>
				<%
				qry="Select Distinct nvl(WFLevel,0)WFL From WF#WORKFLOWAUTHORITY where COMPANYCODE='"+mComp+"' AND INSTITUTECODE='"+mInstitute+"' AND WORKFLOWCODE='"+QryWFCode+"' AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"' AND nvl(DEACTIVE,'N')='N' ORDER BY WFL";
				...
				qry="Select nvl(WEBKIOSK.HasPendingWF('"+mComp+"','"+mInstitute+"','"+QryWFCode+"','"+QryWFType+"','"+mDeptCode+"','','"+QryLevelEdit+"'),'Zero')HasPendWF from dual";
				rs=db.getRowset(qry);
				//out.print(qry);
				if((rs.next() && rs.getString("HasPendWF").equals("N")) && (QryLevelEdit!=1000))
				{
					%>
					<form name="frm" method="POST" ACTION="NOCRequestApprovalByChangedRouteActionForIndvDept.jsp">
...
//---------------Start Of Approval Authority for if(rsFrDept.next())----
						...
						qry=qry+" from WF#WORKFLOWAUTHORITY WHERE CompanyCode='"+mComp+"' and InstituteCode='"+mInstitute+"' AND WORKFLOWCODE='"+QryWFCode+"'";
						qry=qry+" AND WORKFLOWTYPE='"+QryWFType+"' AND DEPARTMENTCODE='"+mDeptCode+"' and WFLEVEL='"+QryLevelEdit+"' ORDER BY FRVAL, TOVAL";
				    		rs=db.getRowset(qry);
						//out.print(qry);
						...
						*/
						...
						-->
							<input type="hidden" value="E" Name="<%=mName11%>" ID="<%=mName11%>" checked onclick="FunActCurr()"><STRONG><font face=arial size=2 color=green><!--Keep Enabled--></font></STRONG>
							<input type="hidden" value="D" Name="<%=mName11%>" ID="<%=mName11%>" onclick="FunActCurr()"><STRONG><font face=arial size=2 color=red><!--Discard--></font></STRONG>
							</td>
							</tr>
							<tr>
							...
							{
								%>
							<!--	<a target=_new href="AddCrBasedWorkFlowLevelForIndvDept.jsp?DeptCode=<%=mDeptCode%>&amp;WFC=<%=QryWFCode%>&amp;WFT=<%=QryWFType%>&amp;WFL=<%=QryLevelEdit%>&amp;RID=<%=QryRID%>&amp;TLD=<%=mTotalLvDays%>&amp;DATEFROM=<%=QryLvDateFr%>&amp;DATETO=<%=QryLvDateTo%>" title="Click to add more Approval Level"><marquee behavior=alternate scrolldelay=100 width=100% style="cursor:hand"><font color="blue" face=arial size=2><STRONG>Next Criteria-Based Approval Authority Required ?</STRONG></font></marquee></a> -->
								<%
							}
							else
							{
