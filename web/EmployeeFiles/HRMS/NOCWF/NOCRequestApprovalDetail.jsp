...
String mDMemberType="";
String mInst="";
String mMemberID="",mComp="",mDepartmentCode="";
String mMemberType="",mMemberName="";
String QryFaculty="",QryRID="",mDiffPath="";
String mENM="",mDeptCode="";
String 	mRepDate="",mRelDate="",mRemark="",mWFC="",mWFT="";
int mNoticeDay=0;
int CTR=0,mLeaveApprovalLevel=0,mWFSeq=0,mMaxLevel=0,mTotOnWFLevel=0;
String mApprBy="",mApprAuth="",mApprovalById="",mApprovalByName="";
String mAprMemID="";
String mFacType="I";
ResultSet rs1=null;
ResultSet rs2=null;
...
String mRemarks="",mFullCons="",mApproveBy="",mDepartment="";
int mWFLevel=0;
String CurrStat="",CanStat="";
String mTextToAprFrom="";
String mRightsID="162";
String mTextToAprDisp="",mAprTxt="",mCanAllow="";
String mShowRemTo="",mShowRemarksTo="";
int mFlush=0;
...
// For Log Entry Purpose
			...
			//--------------------------------------
%>
			<form name="frm"  method="post"  action="NOCRequestApprovalDetail.jsp">
			<input id="x" name="x" type=hidden>
			<table id=id1 ALIGN=CENTER bottommargin=0  topmargin=0 width=100%>
...
//---------------Start Of if(rsFrEmp.next())----
				...
				qry=qry+" AND WORKFLOWCODE='"+mWFC+"' AND WORKFLOWTYPE='"+mWFT+"' AND EMPLOYEEID='"+QryFaculty+"' AND FACULTYTYPE='"+mFacType+"' GROUP BY WFLEVEL order by MaxLevel Asc";
			    	rs=db.getRowset(qry);
				//out.print(qry);
				try
				{
...
//---------------Start Of if(rsFrDept.next())----
			...
			if(rs1.next())
			{
				...
				qry=qry+" AND WORKFLOWCODE='"+mWFC+"' AND WORKFLOWTYPE='"+mWFT+"' AND DEPARTMENTCODE='"+mDeptCode+"' GROUP BY WFLEVEL order by MaxLevel Asc ";
				rs=db.getRowset(qry);
				// out.print(qry);
				try
				{
					while(rs.next())
					{
						...
						%>
							...
							//------------------------------
						}  //----closing of if(CTR>=mLeaveApprovalLevel)
...
<!--			<tr><td colspan="5">
				...
				&nbsp; &nbsp; &nbsp; <input type="radio" value="A" name="AprStat" checked><b><font face="arial" size="2" color=green> Approve </STRONG></font>
				...
				<tr><td valign=middle colspan=5 nowrap><font color=black face=arial size=2><STRONG>&nbsp;Remarks for Approval/Cancellation ( If Any? ) : </STRONG></font>
				...
				<tr><td colspan=5 nowrap>&nbsp;<TextArea Name='Remarks1' Id='Remarks1' maxlength=200 cols=68 rows=2></TextArea><FONT color=red>*</FONT>
				</td></tr>
...
<%
			if(request.getParameter("x")!=null)
			{
				...
				if(request.getParameter("WFSeq")==null)
					...
					mWFSeq=Integer.parseInt(request.getParameter("WFSeq").toString().trim());
...
//***************************************************************
				//	qry="INSERT INTO WF#WORKFLOWDETAIL(COMPANYCODE, INSTITUTECODE, WORKFLOWCODE, WORKFLOWTYPE, DEPARTMENTCODE, WFSEQUENCE, WFLEVEL, REQUESTID, STATUS,  REMARKS, APPROVALBY, APPROVALDATETIME)";
				//	qry=qry+" VALUES('"+mComp+"', '"+mInst+"', '"+mWFC+"', '"+mWFT+"', '"+mDeptCode+"', "+mWFSeq+", "+mWFLevel+", '"+QryRID+"', '"+mFullCons+"', '"+mRemarks+"', '"+mApproveBy+"', sysdate)";
					...
					db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType ,"NOC WORK FLOW APPROVAL/CANCELLATION", "Staff: "+QryFaculty+"  Request: "+QryRID+" - Approved", "No MAC Address" , mIPAddress);
						  //----------------------------------
							if(mDiffPath.equals("Y"))
							{
						//		RequestDispatcher rd=request.getRequestDispatcher("NOCRequestApprovalToChangedRoute.jsp");
						//		request.setAttribute("message","Msg1");
						//		rd.forward(request,response);
						%>
							<LEFT><b><font size=2 face='Arial' color='Green'>
							NOC Recommendation on Current Level is Successful... <a href='NOCRequestApprovalToChangedRoute.jsp?EID=<%=QryFaculty%>&amp;RID=<%=QryRID%>&amp;DeptCode=<%=mDeptCode%>' style='cursor:hand'><font color=blue>Click </font></a>To Change Subsequent Route
							</font></LEFT>
						<%
						}
							else
				...
				else if(n>0 && mFullCons.equals("C"))
				{
					...
					{
						out.print("&nbsp;&nbsp;&nbsp <b><font size=2 face='Arial' color='red'>  Error while NOC cancellation...");
					}
						  //-----------Log Entry------------
					...
					if(mSendInfoFlag>0)
						{
							...
							{
								%><LEFT><%
								out.print("&nbsp;&nbsp;&nbsp <b><font size=2 face='Arial' color='Green'>An intimation is being sent to concerned people regarding this Recommendation...</font><br>");
								%></LEFT><br><%
							}
						} // closing of if(mSendInfoFlag>0)
	
					if(mFlush>0)
					{
//RequestDispatcher rd=request.getRequestDispatcher("NOCRequestApprovalDetail.jsp");
								//request.setAttribute("message","Msg1");
							//	rd.forward(request,response);
								%><CENTER><%
								out.print("&nbsp;&nbsp;&nbsp <b><font size=2 face='Arial' color='Green'>NOC Recommendation on Current Level is Successful...</font> <br>");
					}
