		...
		if (RsChk.next() && RsChk.getString("SL").equals("Y"))
	   	{
			...
			<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: fantasy"><u><b>Cancel Advance Leave Detail </b></u></font></td></tr>
			</TABLE>
			<%
			qry="SELECT  B.EmployeeName||' ['||B.EmployeeCode||']' EMPLOYEENAME ,nvl(B.EMPLOYEEID,' ')EMPLOYEEID,nvl(to_char(A.ADVANCEDATE,'dd-mm-yyyy'),' ')ADVANCEDATE, nvl(C.ADVANCEDESC,' ')ADVANCEDESC,decode(A.ADVANCEAMOUNT,'',' ',ADVANCEAMOUNT)ADVANCEAMOUNT,decode(A.NOOFINSTALLMENT,'',' ',NOOFINSTALLMENT)NOOFINSTALLMENT, nvl(A.REQUESTREMARKS,' ')REQUESTREMARKS FROM PAY#EMPOTHERADVANCEREQUEST A,V#STAFF B,PAY#ADVANCETYPEMASTER C WHERE A.MEMBERID=B.EMPLOYEEID AND A.MEMBERID='"+mEID+"' and A.REQUESTID='"+mRID+"' and  A.COMPANYCODE='"+mComp+"' and A.INSTITUTECODE='"+mInst+"' AND A.ADVANCETYPE=C.ADVANCETYPE AND A.MEMBERID=B.EMPLOYEEID	";
			...
			if(rs.next())
			{
				...
				mAdvDesc=rs.getString("ADVANCEDESC");
			...
			<tr><td nowrap><FONT Color=Black face=arial size=2><B>&nbsp;Advance Leave Description </B></td><td><b>&nbsp; : </B></td><td> </Font><FONT Color=Black face=arial size=2><%=mAdvDesc%>&nbsp;&nbsp;&nbsp;&nbsp;<b>No. of Installment :</b>&nbsp;<%=mNoInstal%></Font></td></tr>

			<tr><td nowrap><FONT Color=Black face=arial size=2><B>&nbsp;Advance Date</B></td><td><b>&nbsp; : </b></td><td> </Font><FONT Color=Black face=arial size=2><%=mAdvDate%>&nbsp;</Font></td></tr>
			...
			if(request.getParameter("x")!=null)
			{
				...
				if(!mRemark.equals(""))
				{
					...
					qry="UPDATE PAY#EMPOTHERADVANCEREQUEST SET STATUS='C' ,APPROVEDBY='"+mChkMemID+"', APPROVEDDATE=SYSDATE ,APPROVALCANCELREMARKS='"+mRemark+"' where MEMBERID='"+mEID+"' AND REQUESTID='"+mRID+"' ";
					n=db.update(qry);
					if(n>0)
					{
					// Log Entry
					//-----------------
 					db.saveTransLog(mInst,mLogEntryMemberID,mLogEntryMemberType,"ADVANCE LEAVE CANCEL", "Staff :"+mEmpName+" RequestID :"+mRID, "NO MAC ADDRESS",  mIPAddress);
					//-----------------
					response.sendRedirect("AdvanceRequestCancel.jsp");
					}
					else
					{
						%><CENTER><%
