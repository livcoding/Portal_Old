...
String mDMemberType="";
String mMemberCode="";
String mDMemberCode="",mInst="",mRID="", mRightsID="176";
...
<HTML>
<head>
<TITLE>#### <%=mHead%> [ Cancel Advance Leave Request ] </TITLE>
...
try
{
	String mPageHeader="Cancel Advance Leave Request", mMarqMsg="", CurrDate="";
...
<!-------------Page Heading and Marquee Message----------------------->
	...
	<tr><td >
	 <font face='arial' size=2 color=black ><strong> Employee </strong></font>
	<% 
		qry="select distinct nvl(A.EMPLOYEEID,' ')EMPLOYEEID,nvl(A.EMPLOYEENAME,' ')EMPLOYEENAME, ";
		qry=qry+" nvl(A.EMPLOYEECODE,' ')EMPLOYEECODE from V#STAFF A,PAY#EMPOTHERADVANCEREQUEST B WHERE		A.EMPLOYEEID='"+mChkMemID+"' and A.EMPLOYEEID=B.MEMBERID  AND A.COMPANYCODE=B.COMPANYCODE AND NVL(A.DEACTIVE,'N')='N' "; 
		//out.print(qry);
		rs=db.getRowset(qry);
		%>
...
<%
	...
	try
	{
		...
		<td align=center ><font color=white><b>Advance Desc</b></font></td>
		<td align=center ><font color=white><b>Advance Date</b></font></td>
		...
		<td nowrap align=center ><%=rs.getString("ADVANCEDESC")%></td>
		<td nowrap align=center ><%=rs.getString("ADVANCEDATE")%></td>
		<td nowrap align=center ><%=rs.getString("NOOFINSTALLMENT")%>&nbsp;</td>
		<td nowrap align=center ><%=rs.getString("REQUESTREMARKS")%>&nbsp;</td>
		
		<td nowrap align=center><a Title="Click to Cancel <%=rs.getString("EMPLOYEENAME")%>'s Advance Leave Request" href='AdvanceRequestCancelDetail.jsp?EID=<%=rs.getString("MEMBERID")%>&amp;RID=<%=rs.getString("REQUESTID")%>'><FONT COLOR=Red >Click To Cancel</FONT></a>
		</td>
		</tr>
		</tbody>
		<%
		...
		if(Count==0)
		{
			...
			{
				%><CENTER><%
				out.print(" &nbsp;&nbsp;&nbsp <br><br><font size=3 face='Arial' color='Red'><b>No Records Exist...</b></font><br>");
				%></CENTER><%
			}
		}
		}//END OF TRY
