...
//System.out.print("*****2***********");
	 int hht=0;
String Qrtyu=""; ResultSet rstu=null;
String Qrycut=""; ResultSet rscut=null;
long mL=0,mLP=0;
double mPercL=0,mMassCutMarks=0.0,mNetMarks=0.0,mTATotalMarks=0.0,mMarksAdd=0.0;
String mLFSTID="",prevLFSTID="",mREGCONFIRMATIONDATE="",qryspe="",mSpecial="",mAcademicYeay="",qrys="",mTypeofaction="";
String mHead="",weightageround="",mSubSec="";
String mFFCOUNT="",mIICOUNT="";
String qryttt=""; 
String mprog="";
String  qrydd="";  
String xAbsent ="",mFFFCount="",mIIICount="",mACAD="";
String  Qryapimr=""; 
ResultSet rsiimr=null,rsQREY=null,rse=null,rsBatchDate=null,rss=null; int APIMCA=0 ,hhhh=0;
...
if(document.frm.Save.checked==false)
{
	
document.frm.SaveButton.enabled=false;
...
<%
		...
		ResultSet rsCal=null;
			qryCAl="select  distinct nvl( (select  Distinct CALCULATIONVALUE from grademaster where examcode='"+mExamCode+"' and  grade ='A+' AND PROGRAMCODE='"+mprog+"'   ),0)  CALAP,nvl( (select  Distinct CALCULATIONVALUE from grademaster where examcode='"+mExamCode+"' and  grade ='F' AND PROGRAMCODE='"+mprog+"'  ),0)  CALF from grademaster where examcode='"+mExamCode+"' and institutecode='"+mInst+"' AND PROGRAMCODE='"+mprog+"'  " ;
//out.print(qryCAl);
...
while (x < mTotalStudents+1) 
{
 ...
 %>
	...
	<td><b><font color=white >Student<BR>Name<BR></font></b></td>
		<!-- <td align=left ><b><font color=white >Att.<BR>(%)</font></b></td> -->
	<%
	String qryadd="";
	String sumadd="",aa="";
	int i=0;
...
qry=qry+"decode(a.eventsubevent,'TEST-1','TEST -1',1,'TEST-2', 2, 'TEST-3', 3,'TA#TA1',4,'TA#TA2',5,'TA#TA3',6,'TA#TA4',7,'TA#TA5',8,'TA',9,'TA#MINPROJ',10,'TA#CASE',11,'TA#ASSGN',12,'TA#QUIZZES',13,'TA#TERMPAPER',14) ordno";
...
if(mLTP.equals("P")){
	  ...
	  tttt=aaa.substring(0,aaa.indexOf("$$$"));
							...
							if(qryadd.equals(""))
							{
								qryadd=qryadd+" nvl(TRUNC(DECODE(Sum(Decode(xyz.EventSubEvent, '"+cccc+"', total,0)),' ','Absent',Sum(Decode(xyz.EventSubEvent, '"+cccc+"', total,0))),1),0)  AB"+i+" ";	
						...
						sumadd=sumadd+" ) ";
						%>
<td  align=center><b><font color=white >MassCut</font></b></td>
	<td  align=center><b><font color=white >TA<BR>TOTAL</font></b></td> 
...
if(hht==1) {
		...
		qry2+=" CEIL ( NVL (SUM (DECODE (xyz.eventsubevent, '"+rstu.getString("SUBEVENTCODE")+"', total, 0)),  '0'  )  "+rstu.getString("AA")+"  ";
	 //out.print(qry2);
}else{
	qry2+=" "+rstu.getString("AA")+"  NVL (SUM (DECODE (xyz.eventsubevent, '"+rstu.getString("SUBEVENTCODE")+"', total, 0)),  '0'  )    ";
}

	}
if(hht==0)
    {
qry2+="( NVL (SUM (DECODE (xyz.eventsubevent, 'TA', total, 0)),  '0'  )";
}

 
		 
	 
	 qry2+="  ) T1T2T3Total from ( select ";
					qry2+=" a.EventSubEvent,nvl(a.MARKSAWARDED2,0) MARKSAWARDED2,  nvl((((a.marksawarded2/a.maxmarks)*b.weightage)),0) total,";
...
///Querry for Marks Cut     mCheckFstid  mStudidInit

Qrycut="select ATTENDANCEPERCENTAGE,NVL(MASSCUTS,0)MASSCUTS,NVL(TATOTAL,0)TATOTAL, NVL ( ( (CASE WHEN (TATOTAL - Masscuts) < 0 THEN TATOTAL  ELSE  Masscuts  END)),0       )NETMARKSCUT from STUDENTGRADEMASSCUTS where fstid  IN  ("+mCheckFstid+" ) and studentid='"+mStudidInit+"'";
rscut=db.getRowset(Qrycut);
//out.print(Qrycut);
	if (rscut.next())
	{
	mPercL=rscut.getDouble("ATTENDANCEPERCENTAGE");
		mMassCutMarks=rscut.getDouble("NETMARKSCUT");
	}	
...
%>
		<td align=center >- <%=mMassCutMarks%><FONT SIZE="1" COLOR="red"><!-- * /<%=mPercL%> --></FONT></td>				
	
	<%
}else{
	
	%>
	<td align=center ><%=mMassCutMarks%></td>	
	<%
	}

double value = Double.parseDouble(rs.getString("T1T2T3Total"));
	//int TAint = Integer.parseInt(rs.getString("T1T2T3Total"));
value=value-mMassCutMarks;
...
double valueWeight = Double.parseDouble(weightageround);
valueWeight=valueWeight-mMassCutMarks;

%>
	<td align=center ><%=valueWeight%></td>				
 		...
 		while(it.hasNext())	
		   {
				...
				//out.println(mStudidInit+"===="+mStudid+"         ");
...
<b>*Save Grade now<input type=checkbox name='Save' id='Save'  value='Y'  onClick="apply()" onclick="return showAlert1();">
&nbsp; &nbsp; &nbsp;
<input type=submit  name="SaveButton" id="SaveButton" value="Save" disabled  >&nbsp; &nbsp;    



 <a href="GradeCalculationAction.jsp?Check=<%=mCheck%>&amp;ACAD=<%=mACAD%>&amp;prog=<%=mprog%>&amp;EXAM=<%=mExamCode%>&amp;SUB=<%=mSubjectCode%>&amp;FSTIDL=<%=mCheckFstid%>&amp;ETOD=<%=mETOD%>"><font color=blue>Re-Evaluate the students Grade</font></a>  
</TD>
</font></td></tr>
<tr><td>
