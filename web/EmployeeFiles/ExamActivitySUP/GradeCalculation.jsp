	   ...
	   if(request.getParameter("ExamCode")!=null && !request.getParameter("ExamCode").equals("NONE"))
	   {
		...
		if(request.getParameter("x")!=null)
		{
			...
			*/
			...
			qry="select 'y' from v#exameventsubjecttagging a where  a.examcode='"+mEC+"' and ";
			qry=qry+"  a.CORDFACULTYID='"+mChkMemID+"' and exists ";
			qry=qry+" (select 'y' from v#studenteventsubjectmarks b where b.examcode='"+mEC+"' and ";
			qry=qry+" b.subjectid='"+mSC+"' and nvl(b.locked,'N')='N' and  a.examcode=b.examcode and ";
			qry=qry+" a.subjectid=b.subjectid and a.fstid=b.fstid and a.studentid=b.studentid )";
			qry=qry+"   and a.subjectid ='"+mSC+"'        and ((a.subjectid = '090131' AND a.eventsubevent  not LIKE 'BA%')         or (a.subjectid <>  '090131')) ";
		//	out.print(qry);
			...
			if(!rs.next())
			{
				...
				qry=qry+" WHERE A.INSTITUTECODE='"+mInst+"' AND A.EXAMCODE='"+mEC+"' AND   NVL(A.PUBLISHED,'N')='Y'";
				qry=qry+" and A.CORDFACULTYID='"+mChkMemID+"' ";
				qry=qry+"    and a.subjectid ='"+mSC+"'        and ((a.subjectid = '090131' AND a.eventsubevent  not LIKE 'BA%')         or (a.subjectid <>  '090131'))";
...
//out.print(qry);
				...
				if(mAllowedWeightage==0.0)
				{
					...
					<input type=hidden value=<%=mExamid%> id="ExamCode" name="ExamCode">
					<table>
					<TABLE align=center class="sort-table"  rules=group border=1 cellspacing=0 width=98% >
					<%
					...
					while(rs.next())
					{
						...
						<TR>
						<TD>
						<INPUT TYPE="hidden" NAME="<%=mName1%>" ID="<%=mName1%>" VALUE="<%=rs.getString("fstid")%>"><Font  size=2><%=ctr%>.</TD> 
