...
String mCheckFstid="",Stdidclub="",mBreakSlno="";
	...
	%>
	<form name="frm" method="post" action="StudentGradesXLS.jsp">
...
<%
		int count=0;
String mDet="";
String query="select distinct a.fstid,a.studentid, b.ENROLLMENTNO,b.STUDENTNAME,a.FINALMARKS,nvl(a.FINALGRADE,' ')FINALGRADE"
        + " from STUDENTWISEGRADE a , Studentmaster b WHERE a.EXAMCODE='"+mExamCode+"' "
        + "and a.fstid in ("+mCheckFstid+")  and a.INSTITUTECODE='"+mInst+"' and a.BREAK#SLNO='"+mBreakSlno+"' "
        + "and a.STUDENTID=b.STUDENTID and nvl(a.DEACTIVE,'N')<>'Y' and nvl(b.DEACTIVE,'N')<>'Y' order by studentname";	
...
qry=" select 'Y' from DEBARSTUDENTDETAIL a  WHERE a.EXAMCODE='"+mExamCode+"' and a.fstid = '"+rslist.getString("fstid")+"'  and a.INSTITUTECODE='"+mInst+"' and a.STUDENTID='"+rslist.getString("studentid")+"' and nvl(a.DEACTIVE,'N')='N' and (NVL(A.DEBAR,'N')='Y' or  NVL(A.ABSENTCASE,'N')='Y')";
							//out.print(qry);
							rs=db.getRowset(qry);
							if(rs.next())
							{
...
%>
	...
	<td>&nbsp;<%=rslist.getString("STUDENTNAME")%>  <font color=red size=2><b><%=mDet%></b></font> </b></td>
