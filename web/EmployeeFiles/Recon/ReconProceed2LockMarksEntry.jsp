<%@ page language="java" import="java.sql.*,java.math.*,tietwebkiosk.*,java.util.*,oracle.sql.*,org.apache.poi.hssf.*,java.io.*,org.apache.poi.ss.usermodel.*,org.apache.poi.hssf.usermodel.*;" %>
<%@ page errorPage="../../CommonFiles/ExceptionHandler.jsp" %>
<%

//Last Modified on 10.04.2019 (sysdate-LASTACCESS)>0.05) Anoop Tiwari
...
ResultSet rs = null, rs1=null, rs2=null,rs3=null,rs6=null,rs4=null,rseve=null;
String mIC="";
String qry = "",qry2="",qry1="",qry3="",qryevent="",qryevent2="",inuse="";
String mself="";
String path="";
...
if (session.getAttribute("svrpath")==null)
{
	path="";
}
else
{
	path=session.getAttribute("svrpath").toString().trim();
}
...
// and A.employeeid='"+mDMemberID+"' and A.facultytype=decode('"+mDMemberType+"','E','I','E') ";
  ...
  {
	qry=qry+" UNION select A.fstid from EX#SUBJECTGRADECOORDINATOR where  institutecode='"+mInst+"' and facultytype=decode('"+mDMemberType+"','E','I','E') and FACULTYID='"+mDMemberID+"' and FSTID in (select fstid from facultysubjecttagging where examcode='"+mEC+"' AND subjectID='"+mSC+"')";
  }
...
else if(!mself.equals("Self"))
{
	...
	qry1=qry1+" a.examcode='"+mEC+"'  and (a.ltp='L' OR (a.LTP='E' AND a.PROJECTSUBJECT='Y') OR a.LTP='P' ) and a.subjectID='"+mSC+"' ";
	qry1=qry1+" And a.EVENTSUBEVENT='"+mEventsubevent+"' " ;
	qry1=qry1+" AND a.FSTID IN (SELECT FSTID FROM EX#SUBJECTGRADECOORDINATOR WHERE  INSTITUTECODE='"+mIC+"' and FACULTYTYPE=decode('"+mDMemberType+"','E','I','E') and FACULTYID='"+mDMemberID+"')";
	qry1=qry1+" and not exists (select 'y' from V#STUDENTEVENTSUBJECTMARKS b where a.fstid=b.fstid and a.studentid=b.studentid   AND EVENTSUBEVENT='"+mEventsubevent+"' and (nvl(detained,'N')='D'                             OR  NVL (detained, 'N') = 'U'                            OR NVL (detained, 'N') = 'M'   or nvl(detained,'N')='A' or nvl(marksawarded1,-1)>=0 ))";
	qry1=qry1+" GROUP BY a.fstid, a.studentid, a.StudentName, a.enrollmentno, a.Semester, a.programcode, a.SECTIONBRANCH, a.subsectioncode";
	qry1=qry1+" order by enrollmentno";
}
//System.out.println("JILIT - "+qry1);
 ///out.println("JILIT - "+qry1);
  ...
  {
	%>
	<tr><td colspan=3 align=center><a href='Recon/ReconMarksEntryGridVersion.jsp'><font color=blue><b>Continue with Marks Entry</b></font></a></td></tr>
	</table>
	<%
  }
  else
  {
	...
	{
            //************************************** Auto Reconcilation Start Dt 14.03.2017********************************************
...
String merQry="Select distinct  B.EMPLOYEEID EMPLOYEEID from EX#SUBJECTGRADECOORDINATOR A,FacultySubjectTagging B   WHERE " +
        " A.INSTITUTECODE=B.INSTITUTECODE   AND    A.FSTID= B.FSTID   AND    A.FACULTYID='"+mDMemberID+"'   AND " +
        "   B.INSTITUTECODE='"+mInst+"' And B.ExamCode='"+mEC+"' and B.SubjectID='"+mSC+"' and (B.LTP='L' OR B.PROJECTSUBJECT='Y' OR B.LTP='P')";
ResultSet mrres;
mrres=db.getRowset(merQry);
            while(mrres.next())
	{
	if(mergvar.equals(""))
	mergvar="'"+mrres.getString("EMPLOYEEID")+"'";
	else
	mergvar=mergvar+",'"+mrres.getString("EMPLOYEEID")+"'";
	}
  ...
  /* qryevent="select distinct eventsubevent from v#STUDENTEVENTSUBJECTMARKS  where INSTITUTECODE='"+mInst+"'  and SubjectID='"+mSC+"' " +
        " And ExamCode='"+mEC+"' and EventSubEvent<>'"+mEventsubevent+"'   and (LTP='L' OR PROJECTSUBJECT='Y' OR LTP='P') and " +
        "employeeid='"+mDMemberID+"'  and  not exists(select EventSubEvent from MR#ReconciledDetail where INSTITUTECODE='"+mInst+"' " +
        " and SubjectID='"+mSC+"'  And ExamCode='"+mEC+"' and ForEventSubEvent='"+mEventsubevent+"'   and employeeid='"+mDMemberID+"' " +
        " and nvl(reconciled,'R')  in ('M','V','R'))";*/

  qryevent="select DISTINCT EVENTSUBEVENT ,EMPLOYEEID from STUDENTEVENTSUBJECTMARKS c ,FacultySubjectTagging b  WHERE " +
          " c.FSTID= B.FSTID  AND    b.EMPLOYEEID in ("+mergvar+")  AND    B.INSTITUTECODE='"+mInst+"' And B.ExamCode in (select examcode from exammaster e where e.examcode='"+mEC+"' and e.SUMMEREXAMCODE is null AND e.INSTITUTECODE='"+mInst+"'  )  and " +
          "B.SubjectID='"+mSC+"' and (B.LTP='L' OR B.PROJECTSUBJECT='Y' OR B.LTP='P') and    c.EVENTSUBEVENT<>'"+mEventsubevent+"' and c.locked='Y' and " +
          " (EventSubEvent,EMPLOYEEID) not in(select EventSubEvent,EMPLOYEEID from MR#ReconciledDetail where INSTITUTECODE='"+mInst+"' " +
        " and SubjectID='"+mSC+"'  And ExamCode in (select examcode from exammaster e where e.examcode='"+mEC+"' and e.SUMMEREXAMCODE is null AND e.INSTITUTECODE='"+mInst+"'  )  and ForEventSubEvent='"+mEventsubevent+"'   and employeeid in ("+mergvar+") " +
        " and nvl(reconciled,'R')  in ('M','V','R'))";
  //System.out.println("qryevent--"+qryevent);
  rseve=db.getRowset(qryevent);
  while(rseve.next())
{
      String fname="";
      String mevesubev=rseve.getString("EVENTSUBEVENT");
      String mmemberID=rseve.getString("EMPLOYEEID");
      
String seleqry="select filename from  MR#ExcelFileDetail where INSTITUTECODE='"+mInst+"'  and SubjectID='"+mSC+"' " +
        "And ExamCode='"+mEC+"'  and EventSubEvent='"+mevesubev+"' And Employeeid='"+mmemberID+"'";
System.out.println("seleqry---"+seleqry);
ResultSet sel=db.getRowset(seleqry);
int rowcnt=0;
while(sel.next()){
fname=sel.getString("FILENAME");
++rowcnt;
}
if(rowcnt==1){  // check of rowcount =1

qryevent2="update MR#ExcelFileDetail set status='L' , LASTACCESS =sysdate where INSTITUTECODE='"+mInst+"'  and SubjectID='"+mSC+"' " +
        "And ExamCode='"+mEC+"'  and EventSubEvent='"+mevesubev+"' And Employeeid='"+mmemberID+"' " +
        "and (nvl(status,'N')<>'L' or (sysdate-LASTACCESS)>0.005)";



int isup=db.update(qryevent2);
...
try{
if(f.exists()){  //**********************If File  Found in Related Path*****************************************************
 ...
 // ***********************************************Fetch Data From Excel File and Load in arraylist**************************
              ...
              HSSFRow row=sheet.getRow(i);
               StringBuffer sb=new StringBuffer();
                for (int cn = 0; cn < row.getLastCellNum(); cn++) {
                 ...
                 if(cn==1){
                      // double sid=0.0;
                      String studID=cell.getStringCellValue();
                     //  String studID=Double.toString(sid);
                       sb.append(studID);
                       sb.append("*");
 ...
 // End of For loop for Insertion of Excel Data .............

             //************************Insert Excel Data into DataBase*******************************************
            String delqry1=" delete from  Temp#ExcelStudentmarks where   FileName='"+fname+"'";
            int delre = db.update(delqry1);

             int isinsert=0;
             int isexception=0;
             for(int ki=0;ki< StudentList.size();ki++){
             String []data=StudentList.get(ki).toString().split("\\*");
             String sValue="";
            if(data.length>0){
                sValue=data[1].toString();
                if(sValue.toString().equalsIgnoreCase("M") || sValue.equalsIgnoreCase("A") || sValue.equalsIgnoreCase("D")|| sValue.equalsIgnoreCase("U")){
                sValue="-1";
                }
            String insqry=" insert into  Temp#ExcelStudentmarks(  FileName ,  Enrollmentno ,Marks )values ('"+fname+"', '"+data[0].toString()+"' , '"+sValue.toString()+"'  )";
            try{
               
            isinsert = db.insertRow(insqry);
            if(isinsert<=0){
            ki=StudentList.size();
            String delqry=" delete from  Temp#ExcelStudentmarks where   FileName='"+fname+"'";
            int del = db.update(delqry);
            inuse="e";
            String updevent="update MR#ExcelFileDetail set status='N' , LASTACCESS =sysdate where INSTITUTECODE='"+mInst+"'  and SubjectID='"+mSC+"' " +
            "And ExamCode='"+mEC+"'  and EventSubEvent='"+mevesubev+"' And Employeeid='"+mmemberID+"' ";
   ...
   //***************************Autoreconciliation Function Call*******************************************
      String result=" ";
      result= db.AutoReconciliation(fname, "Y", mInst, mEC, mSC, mevesubev, mmemberID, mEventsubevent, mDMemberCode);
       if(!result.equals(" ")){
    ...
    //  System.out.println("ooooooo  "+result);
    ...
    //**********************In case File Not Found in Related Path*****************************************************
        String FnF="insert into MR#ReconciledDetail(InstituteCode ,ExamCode , SubjectID ,EventSubEvent ,EmployeeId ,ForEventSubEvent ," +
                "FileName ,Reconciled ,Reconciledby  ,RECONCILEDDATE )Values('"+mInst+"' ,'"+mEC+"', '"+mSC+"'  ,'"+mevesubev+"' ," +
                "'"+mmemberID+"' ,'"+mEventsubevent+"' ,'"+fname+"', 'F' ,'"+mDMemberCode+"' ,sysdate)";
         int fnf= db.insertRow(FnF);
         inuse="F";
}      
...
}// End of isup If for update
else{
inuse="Y";
}
}else{
inuse="c";
} // End of check of rowcount =1

}//Out of While Loop

//*********************************Print Error Messages******************************************************

 if(inuse.equalsIgnoreCase("f")){%>
 <table border="1">
<input type=hidden name='Exam' id='Exam' value="<%=mEC%>">
                    <input type=hidden name='EventSubevent' id='EventSubevent' value="<%=mEventsubevent%>">
                      <input type=hidden name='Subject' id='Subject' value="<%=mSC%>">
                                            <tbody>
                                                <tr>
                                                    <td> <a href="Reconmismatch.jsp?Exam=<%=mEC%>&EventSubevent=<%=mEventsubevent%>&Subject=<%=mSC%>&empid=<%=mergvar%>">Data Mismatch Found Click to See Details..</a></td>
                                                </tr>
                                            </tbody>
                                        </table>


<% }else  if(inuse.equalsIgnoreCase("e")|| inuse.equalsIgnoreCase("Y")||inuse.equalsIgnoreCase("c")){
   // RequestDispatcher fwd= request.getRequestDispatcher("/EmployeeFiles/ExamActivity/ReconError.jsp?status='"+inuse+"'");
                     response.sendRedirect(request.getContextPath()+"/EmployeeFiles/Recon/ReconError.jsp?status="+inuse+"");
    }else  if(inuse.equalsIgnoreCase("")){
     String data="";
     String StaQry="select EMPLOYEEID from MR#ReconciledDetail where INSTITUTECODE='"+mInst+"' " +
        " and SubjectID='"+mSC+"'  And ExamCode='"+mEC+"' and ForEventSubEvent='"+mEventsubevent+"'   and employeeid in ("+mergvar+") " +
        " and nvl(reconciled,'R')  in ('M','F')";
     ResultSet seldata=db.getRowset(StaQry);
     if(seldata.next()){%>
    <table border="1">
<input type=hidden name='Exam' id='Exam' value="<%=mEC%>">
                    <input type=hidden name='EventSubevent' id='EventSubevent' value="<%=mEventsubevent%>">
...
catch(Exception e)
{
	out.print(e);
  		...
  		if (a>0)
		{
                     ...
                     RequestDispatcher rd= request.getRequestDispatcher("../../AutoReconMarksEntryExcelGenerateServlet");
