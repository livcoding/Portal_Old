...
{
    if(id=="pdfGenerator"){
    document.frm2.action="../../ReconFacultyMarksEntryPDFServlet";
    document.frm2.submit();
    }
    if(id=="Excel")
        {
    document.frm2.institute.value=document.frm2.institute.value;
    document.frm2.Exam.value=document.frm2.Exam.value;
    document.frm2.EventSubevent.value=document.frm2.EventSubevent.value;
    document.frm2.subjectcode.value=document.frm2.subjectcode.value;
    document.getElementById("LockEntry").value="";
    document.getElementById("ExcelGen").value="Excel";
    document.frm2.action="../../ReconMarksEntryExcelGenerateServlet";
    document.frm2.submit();
        }
        if(id=="Lock")
            {

    document.frm2.action="ReconProceed2LockMarksEntry.jsp";
    document.frm2.institute.value=document.frm2.institute.value;
    document.frm2.Exam.value=document.frm2.Exam.value;
    document.frm2.EventSubevent.value=document.frm2.EventSubevent.value;
    document.frm2.subjectcode.value=document.frm2.subjectcode.value;
    document.getElementById("LockEntry").value="Lock";
    document.getElementById("ExcelGen").value="";
   // gridvaria=1;
    //var myMasterGrid = document.frm2.nitobi.getComponent('SimpleGrid1');

  //  myMasterGrid.getCellObject(1,3).readOnly(true);
  //   alert(myMasterGrid);
    document.frm2.submit();

  //  alert( myMasterGrid.getCellObject(1,3).value);
...
{
    document.excelUploadForm.action="../../ReconMarksEntryExcelFileUploadServlet";
	...
	if(!mMemberID.equals("") && !mMemberCode.equals("") && !mMemberName.equals(""))
	{
	  ...
	  //----------------------
        	...
        	<form name="frm" method="get" >
                ...
                <input type="hidden" name="marksOfStudent" value="<%=testMarks%>">


                <table width="100%" ALIGN=CENTER bottommargin=0  topmargin=0>
                <tr><TD colspan="0" align="middle"><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: arial"><B><u>Event/Sub Event wise Students Marks Entry</u></B></TD>
                </font></td></tr>
                </TABLE>
				...
				qry+=") where rownum<11 ORDER BY 1 DESC";
//out.print(qry);
...
qry="select EVENTSUBEVENT,ExamCode from V#EXAMEVENTSUBJECTTAGGING  Where INSTITUTECODE='"+mInst+"' AND ";
                        qry=qry+"  trunc(sysdate) >= trunc(FROMDATE) and trunc(sysdate) <= trunc(TODATE) ";// and facultytype=decode('"+mDMemberType+"','E','I','E')";
                        qry=qry+"  and  nvl(STUDENTLTPDEACTIVE,'N')='N'  AND (fstid) in ((select fstid from facultysubjecttagging where ";
                        //facultytype=decode('"+mDMemberType+"','E','I','E')
                        qry=qry+"  employeeid='"+mDMemberID+"' and nvl(deactive,'N')='N' AND (LTP='L' OR LTP='P') AND INSTITUTECODE='"+mInst+"' AND  nvl(PROJECTSUBJECT,'N')='N' ";
					   ...
					   */
					   qry=qry+")";
                        qry=qry+" UNION select fstid from EX#SUBJECTGRADECOORDINATOR where  institutecode='"+mInst+"' ";
                        //and facultytype=decode('"+mDMemberType+"','E','I','E')
                        qry=qry+" and FACULTYID='"+mDMemberID+"' AND FSTID NOT IN (SELECT FSTID FROM FACULTYSUBJECTTAGGING WHERE PROJECTSUBJECT='Y'))";
	...
	%>
                    ...
                    try
                    {
                        ...
                        qry=qry+"  trunc(sysdate) >= trunc(FROMDATE) and trunc(sysdate) <= trunc(TODATE)";// and facultytype=decode('"+mDMemberType+"','E','I','E')";
                        qry=qry+"  and  nvl(STUDENTLTPDEACTIVE,'N')='N'  AND (fstid) in ((select fstid from facultysubjecttagging where";
                        //facultytype=decode('"+mDMemberType+"','E','I','E') and
                           qry=qry+"  employeeid='"+mDMemberID+"' and nvl(deactive,'N')='N' AND (LTP='L' OR LTP='P') AND INSTITUTECODE='"+mInst+"' AND  nvl(PROJECTSUBJECT,'N')='N' ";
                       /* qry=qry+" UNION select fstid from MULTIFACULTYSUBJECTTAGGING where companycode='"+mComp+"' and institutecode='"+mInst+"' and facultytype=decode('"+mDMemberType+"','E','I','E') and employeeid='"+mDMemberID+"' AND FSTID NOT IN (SELECT FSTID FROM FACULTYSUBJECTTAGGING WHERE PROJECTSUBJECT='Y')
					   */
					   qry=qry+")";
...
{/*
	...
	qry="select subject||' ( '||subjectcode||' )' subject, subjectID,examcode,eventsubevent from V#EXAMEVENTSUBJECTTAGGING where INSTITUTECODE='"+mInst+"' AND  ";
                        qry=qry+" trunc(sysdate) >= trunc(FROMDATE) and trunc(sysdate) <= trunc(TODATE)";//  and facultytype=decode('"+mDMemberType+"','E','I','E')";
                        qry=qry+"  and  nvl(STUDENTLTPDEACTIVE,'N')='N'  AND (fstid) in ((select fstid from  facultysubjecttagging where ";// facultytype=decode('"+mDMemberType+"','E','I','E') and
                        qry=qry+" employeeid='"+mDMemberID+"' and nvl(deactive,'N')='N' AND (LTP='L' OR LTP='P') AND INSTITUTECODE='"+mInst+"' AND  PROJECTSUBJECT='N' ";
                       /* qry=qry+" UNION select fstid from MULTIFACULTYSUBJECTTAGGING where companycode='"+mComp+"' and institutecode='"+mInst+"' and facultytype=decode('"+mDMemberType+"','E','I','E') and employeeid='"+mDMemberID+"' AND FSTID NOT IN (SELECT FSTID FROM FACULTYSUBJECTTAGGING WHERE PROJECTSUBJECT='Y')*/
						qry=qry+" )";
                        qry=qry+" UNION select fstid from EX#SUBJECTGRADECOORDINATOR where  institutecode='"+mInst+"' ";// and facultytype=decode('"+mDMemberType+"','E','I','E')
                        qry=qry+" and FACULTYID='"+mDMemberID+"' AND FSTID NOT IN (SELECT FSTID FROM FACULTYSUBJECTTAGGING WHERE PROJECTSUBJECT='Y'))";
	...
	{
		%>
		<select name=DataCombo tabindex="0" id="DataCombo" style="WIDTH: 0px;display:none;">
		<%
		while(rss.next())
		{
...
%>
                ...
                if(request.getParameter("x")!=null)
                {
                    ...
                    if(request.getParameter("Click")!=null && request.getParameter("Subject")!=null && request.getParameter("Event")!=null && request.getParameter("Exam")!=null)
                    {
                        ...
                        {
                          qry="select  distinct 'Y', proceedsecond from V#EXAMEVENTSUBJECTTAGGING";
                            ...
                            qry=qry+"  and  nvl(STUDENTLTPDEACTIVE,'N')='N'  AND ((EMPLOYEEID=(Select '"+mDMemberID+"' EmployeeID from dual)) OR (fstid in (select fstid from FACULTYSUBJECTTAGGING where  institutecode='"+mIC+"' and facultytype=decode('"+mDMemberType+"','E','I','E') and employeeid='"+mDMemberID+"')) and facultytype=decode('"+mDMemberType+"','E','I','E'))";
                            qry=qry+" AND EVENTSUBEVENT='"+mEvent+"' " ;
                            qry=qry+" GROUP BY fstid,studentid,StudentName,enrollmentno,Semester, programcode,SECTIONBRANCH, subsectioncode,proceedsecond";
                		}
                        else
                        {
                            qry="select  distinct 'Y', proceedsecond from V#EXAMEVENTSUBJECTTAGGING";
                            qry=qry+" where institutecode='"+mIC+"' and nvl(DEACTIVE,'N')='N'  and nvl(locked,'N')='N'   ";
                            qry=qry+"  and  nvl(STUDENTLTPDEACTIVE,'N')='N'  and examcode='"+mEC+"'  and (LTP='L' OR LTP='P') AND PROJECTSUBJECT='N' and subjectID='"+mSC+"' ";
                            qry=qry+" And EVENTSUBEVENT='"+mEvent+"' " ;
                            qry=qry+" AND FSTID IN (SELECT FSTID FROM EX#SUBJECTGRADECOORDINATOR WHERE  INSTITUTECODE='"+mIC+"' and FACULTYTYPE=decode('"+mDMemberType+"','E','I','E') and FACULTYID='"+mChkMemID+"')";
                            qry=qry+" GROUP BY fstid,studentid,StudentName,enrollmentno,Semester, programcode,SECTIONBRANCH, subsectioncode,proceedsecond";
                        }
			...
			if(rsmm.next())
                        {
                            markslocked=rsmm.getString("proceedsecond");
...
function afterPaste(eventArgs){
    ...
    {
myMasterGrid.getCellObject(myRow,3).readOnly(true);
    }*/
...
//alert("isdeb---aaaaaaaaaaaaaaaaaaaaaaa");
    var maxmarks = ${sessionScope['MAXMARKS']};
...
var isdeb=myMasterGrid.getCellObject(myRow,2).getValue().toUpperCase();
var nstudent =isdeb.search("DEACTIVE");
//alert(isdeb);
//alert(nstudent);
if(nstudent>0)
    {
...
//alert(value);
   if( value=='D' || value=='M' || value=='U' || value=='P' )
    {
myMasterGrid.getCellObject(myRow,3).readOnly(true);
    }
    if(/^[a-zA-Z0-9- ]*$/.test(isdeb) == false ){ // if special charector in Student name (will be for debar case) than Marks can't be Editable.
     myMasterGrid.getCellObject(myRow,3).readOnly(true);
    }
...
<font  face="Arial" size="2"><STRONG>No of records changed since last saved:</STRONG></font> <input type="text" readonly="true" size="3" value="0" style="border:none;background-color:#fce9c5;font-weight:bold;font-size:11px;"  id="updated"/>
                      ...
                      onbeforepasteevent="beforepaste(eventArgs)"
                        toolbarenabled="true"
						 ...
						 onbeforecopyevent=""
						  ...
						  gethandler="ReconMarksEntryGetHandler.jsp"
						  savehandler="ReconMarksEntrySaveHandler.jsp"
						  entertab="down"
						  >
					    ...
					    <ntb:columns  >
                               ...
                               <%}%>
                                ...
                                <% // ankur
                                if(mSelf.equals("Self"))//session.getAttribute("PageHeading").equals(""))
                                {
 ...
 <ntb:textcolumn sortenabled="false" width="100" align="center" label="TotalSum" xdatafld="Sum1" editable="false"></ntb:textcolumn>
				 ...
				 <ntb:textcolumn sortenabled="false" width="75" align="center" label="aa" xdatafld="aa" editable="false"></ntb:textcolumn>
                            ...
                            <tr>
                            <TD><form name="frm2" id="frm2" method="post" ></TD>
                            </tr>
                            <tr>
                            <TD>&nbsp;</TD>
                            </tr>
                            <tr>
                            <td align=Left valign=top nowrap><font size=2 color="#993300" face=arial>
				    <form name="frm2" id="frm2" method="post" >
                         <% if(markslocked.equals("Y")){   /*Added By Vivek Soni*/ %>

                           <b>** Lock Marks Entry <input type=checkbox name='Proceed' id='Proceed' value='Y' onclick="return showAlert();" disabled>
                            <INPUT NAME="Lock" id="Lock" Type="button" Value="Lock Marks Entry Now" onclick="submitForm(this.id)" disabled>
                             &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
                              &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
				   <!-- <INPUT NAME="Excel" id="Excel" Type="button" Value="Click to Export in Excel" onclick="submitForm(this.id)" disabled>-->
                         <%}else{%>
                         <b>** Lock Marks Entry <input type=checkbox name='Proceed' id='Proceed' value='Y' onclick="return showAlert();">
                            <INPUT NAME="Lock" id="Lock" Type="button" Value="Lock Marks Entry Now" onclick="submitForm(this.id)" onSubmit="window.location.reload();">

				   <!-- <INPUT NAME="Excel" id="Excel" Type="button" Value="Click to Export in Excel" onclick="submitForm(this.id)">-->
                          <%}%>



                                    &nbsp; &nbsp; &nbsp;
                                <!--    <INPUT NAME="pdfGenerator" id="pdfGenerator" Type="button" Value="Click to Export in PDF" onclick="submitForm(this.id)">-->
                            ...
                            <input type=hidden name='ExcelGen' id='ExcelGen' value="ExcelGen">
                            <input type=hidden name='LockEntry' id='LockEntry' value="LockEntry">
                            </form>
                            </td>
                            </tr>
                            ...
                            <td>
                                 <% if(markslocked.equals("Y")){   /*Added By Satendra*/ %>
                         ...
                         <%}else{%>
                                ...
                                <%}%>
                                 ...
                                 <tr><td nowrap>&nbsp;</td></tr>
                                  <td align=left valign=top nowrap><font size=2 color="#993300" face=arial>
				    <form name="myform" id="myform" method="post" action="ReconMarksEntryTempPage.jsp">

				    <INPUT NAME="Excelnew" Type="submit" Value="Click to Export in Excel">
                            <input type=hidden name='institute' id='institute' value="<%=mIC%>">
                            <input type=hidden name='Exam' id='Exam' value="<%=mEC%>">
                            <input type=hidden name='EventSubevent' id='EventSubevent' value="<%=mEvent%>">
                            <input type=hidden name='subjectcode' id='subjectcode' value="<%=mSC%>">
