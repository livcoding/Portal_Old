...
String mEC="",mSC="";
String mName1="";
String academicyear="",programcode="";
...
<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: large; FONT-FAMILY: verdana"><b>Grade Edit</b></TD>
</font></td></tr>
</table><%
...
<form name="frm1" action="GradeEditAction2.jsp" method="post">
    <input type="hidden" name="Examcode" value="<%=mEC%>">
    <input type="hidden" name="Subject" value="<%=mNam%><%=mSc%>">
    <input type="hidden" name="blno" value="<%=bno%>">
...
<tr bgcolor="#ff8c00">
	...
	<td><b><font color=white >Update Marks</font></b></td>
    <td><b><font color=white >Grade</font></b></td>
    <td><b><font color=white >Update Grade</font></b></td>
</tr>
</thead>
<tbody>
<%

qryi=" SELECT distinct B.enrollmentno,B.studentname,A.STUDENTID,A.INITIALMARKS,A.INITIALGRADE,A.MASSCUTS,A.FINALMARKS,a.fstid,";
qryi=qryi+" nvl(A.FINALGRADE,' ')FINALGRADE FROM STUDENTWISEGRADE A,V#STUDENTEVENTSUBJECTMARKS  B where A.BREAK#SLNO= '"+bno+"'  ";
qryi=qryi+" and A.INSTITUTECODE='"+mInst+"' and a.institutecode=b.institutecode AND A.FSTID=B.FSTID and A.EXAMCODE='"+mEC+"'  AND A.STUDENTID=B.STUDENTID AND NVL(A.DEACTIVE,'N')='N'  order by B.enrollmentno";
...
%>
		<input type="hidden" name="studentid<%=ctr%>" value="<%=rsi.getString("STUDENTID")%>">
        <input type="hidden" name="fstid<%=ctr%>" value="<%=rsi.getString("FSTID")%>">
...
<%
	...
	%>
        ...
        <td><input type="text" name="update<%=ctr%>" size="5" value="<%=String.valueOf(rsi.getDouble("FINALMARKS"))%>"></td>
		<td>&nbsp;<%=rsi.getString("FINALGRADE")%></td>
        <%
         String x=rsi.getString("FINALGRADE");
   
        %>
        <td>
            <select name="gradeupdate<%=ctr%>" id="gradeupdate<%=ctr%>">
                <%
               
                qryx="select distinct grade from grademaster where INSTITUTECODE='"+mInst+"' AND EXAMCODE='"+mEC+"' order by grade";
                rsx=db.getRowset(qryx);
                //out.print(qryx);
                while(rsx.next())
                    {
                    String y=rsx.getString("grade");
                if(y.equals(x))
                {   
                 %>              
                <option selected value="<%=rsx.getString("grade")%>"><%=rsx.getString("grade")%></option>
               <% }
                else
                {
                %>    
              <option value="<%=rsx.getString("grade")%>"><%=rsx.getString("grade")%></option>
                <%
                }
                }
                %>
