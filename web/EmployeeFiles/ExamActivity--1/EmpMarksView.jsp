...
<HTML>
<head>
<TITLE>#### <%=mHead%> [ View Students Marks ] </TITLE>
...
%>
	...
	<tr><TD align=middle><font color="#a52a2a" size=4 face=verdana >View Student Marks</font></td></tr>
	</table>
...
//********************DataEvent Combo*************/
	...
	qry+=" UNION select AA.fstid from StudentEventSubjectMarks aa where  aa.ENTRYBY='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
      qry=qry+" ) and nvl(A.PUBLISHED,'N')='Y' order by eventsubevent";
		...
		{
		qry="select distinct A.EVENTSUBEVENT from V#STUDENTEVENTSUBJECTMARKS A where ";
	qry+=" a.INSTITUTECODE='"+mInst+"' and a.examcode='"+QryExam+"' and  (A.fstid) in ("; 
	qry+=" select AA.fstid from facultysubjecttagging AA where aa.INSTITUTECODE='"+mInst+"'  and facultytype=decode('"+mDMemberType+"','E','I','E') and nvl(deactive,'N')='N' AND employeeid='"+mDMemberID+"'  AND (LTP='L' or LTP='P' OR NVL(PROJECTSUBJECT,'N')='Y')";
	qry+=" UNION select AA.fstid from MultiFacultySubjectTagging aa where aa.INSTITUTECODE='"+mInst+"' and aa.COMPANYCODE='"+mComp+"' and aa.EMPLOYEEID='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
	qry+=" UNION select AA.fstid from Ex#SubjectGradeCoordinator aa where aa.INSTITUTECODE='"+mInst+"' and aa.COMPANYCODE='"+mComp+"' and aa.FACULTYID='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
	//qry+=" UNION select AA.fstid from StudentEventProjectMarks1 aa where aa.MARKS1ENTRYBY='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
...
try
{
	qry="select distinct A.subject ||' ( '||A.subjectcode||' )' subject,A.subjectID,A.examcode,A.eventsubevent from V#STUDENTEVENTSUBJECTMARKS A where ";
	qry+=" a.INSTITUTECODE='"+mInst+"'  and (A.fstid) in ("; 
	qry+=" select AA.fstid from facultysubjecttagging AA where aa.INSTITUTECODE='"+mInst+"'  and facultytype=decode('"+mDMemberType+"','E','I','E') and nvl(deactive,'N')='N' AND employeeid='"+mDMemberID+"'  AND (LTP='L' or LTP='P' OR NVL(PROJECTSUBJECT,'N')='Y')";
	qry+=" UNION select AA.fstid from MultiFacultySubjectTagging aa where aa.INSTITUTECODE='"+mInst+"' and aa.COMPANYCODE='"+mComp+"' and aa.EMPLOYEEID='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
	qry+=" UNION select AA.fstid from Ex#SubjectGradeCoordinator aa where aa.INSTITUTECODE='"+mInst+"' and aa.COMPANYCODE='"+mComp+"' and aa.FACULTYID='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
...
<%
	try
	{
	qry="select distinct A.subject ||' ( '||A.subjectcode||' )' subject, A.subjectID from V#STUDENTEVENTSUBJECTMARKS A where ";
	qry+=" a.INSTITUTECODE='"+mInst+"'  and  (A.fstid) in ("; 
	qry+=" select AA.fstid from facultysubjecttagging AA where aa.INSTITUTECODE='"+mInst+"'  and facultytype=decode('"+mDMemberType+"','E','I','E') and nvl(deactive,'N')='N' AND employeeid='"+mDMemberID+"'  AND (LTP='L' or LTP='P' OR NVL(PROJECTSUBJECT,'N')='Y')";
	qry+=" UNION select AA.fstid from MultiFacultySubjectTagging aa where aa.INSTITUTECODE='"+mInst+"' and aa.COMPANYCODE='"+mComp+"' and aa.EMPLOYEEID='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
	qry+=" UNION select AA.fstid from Ex#SubjectGradeCoordinator aa where aa.INSTITUTECODE='"+mInst+"' and aa.COMPANYCODE='"+mComp+"' and aa.FACULTYID='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
...
try{
	...
	//qry+=" OR select AA.fstid from Ex#SubjectGradeCoordinator aa where aa.INSTITUTECODE='"+mInst+"' and aa.COMPANYCODE='"+mComp+"' and aa.FACULTYID='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
	...
	qry=qry+" A.examcode='"+mEC+"'  and A.facultytype=decode('"+mDMemberType+"','E','I','E') and (A.ltp='L' OR A.PROJECTSUBJECT='Y' OR A.LTP='P') and A.subjectID='"+mSC+"' and (a.employeeid='"+mDMemberID+"')   ";
	qry+=" OR a.fstid in (select AA.fstid from Ex#SubjectGradeCoordinator aa where aa.INSTITUTECODE='"+mInst+"' and aa.COMPANYCODE='"+mComp+"' and aa.FACULTYID='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N' ) ";
...
qry="select  A.fstid,nvl(A.studentid,' ')studentid,nvl(A.studentname,' ')studentname, ";
	qry=qry+" nvl(A.enrollmentno,' ')enrollmentno,nvl(A.semester,0)semester,A.subsectioncode, ";
	qry=qry+ " nvl(A.programcode,' ')programcode,nvl(A.SECTIONBRANCH,' ')SECTIONBRANCH  from V#STUDENTEVENTSUBJECTMARKS A   where ";
	qry+=" a.INSTITUTECODE='"+mInst+"'  and  (A.fstid) in ("; 
	qry+=" select AA.fstid from facultysubjecttagging AA where aa.INSTITUTECODE='"+mInst+"'  and facultytype=decode('"+mDMemberType+"','E','I','E') AND  AA.examcode='"+mEC+"' AND  AA.subjectID='"+mSC+"' and nvl(deactive,'N')='N' AND employeeid='"+mDMemberID+"'  AND (LTP='L' or LTP='P' OR NVL(PROJECTSUBJECT,'N')='Y')";
	qry+=" UNION select AA.fstid from MultiFacultySubjectTagging aa where aa.INSTITUTECODE='"+mInst+"' and aa.COMPANYCODE='"+mComp+"' and aa.EMPLOYEEID='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
	qry+=" UNION select AA.fstid from Ex#SubjectGradeCoordinator aa where aa.INSTITUTECODE='"+mInst+"' and aa.COMPANYCODE='"+mComp+"' and aa.FACULTYID='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
	//qry+=" UNION select AA.fstid from StudentEventProjectMarks1 aa where aa.MARKS1ENTRYBY='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
	//qry+=" UNION select AA.fstid from StudentEventProjectMarks2 aa where aa.MARKS2ENTRYBY='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
	//qry+=" UNION select AA.fstid from StudentEventSubjectMarks aa where aa.INSTITUTECODE='"+mInst+"'  and  aa.ENTRYBY='"+mDMemberID+"' and aa.FSTID=a.FSTID AND NVL(AA.DEACTIVE,'N')='N'";
	qry=qry+" )and nvl(A.PUBLISHED,'N')='Y' and A.eventsubevent='"+mEvent+"' and A.examcode='"+mEC+"' AND A.subjectID='"+mSC+"' ";
	qry=qry+" and nvl(a.deactive,'N')='N' and not EXISTS ( ";
    qry=qry+" select p.studentid from debarstudentdetail p where a.FSTID=p.FSTID ";
    qry=qry+" and a.INSTITUTECODE=p.INSTITUTECODE and a.STUDENTID=p.STUDENTID ";
    qry=qry+" and nvl(MEDICALWITHDRAWAL,'N')='Y' ) ";
    qry=qry+" GROUP BY A.fstid,nvl(A.studentid,' '),nvl(A.studentname,' '), ";
	qry=qry+" nvl(A.enrollmentno,' '),nvl(A.semester,0),A.subsectioncode, ";
	qry=qry+ " nvl(A.programcode,' '),nvl(A.SECTIONBRANCH,' ') Order by enrollmentno" ;
...
if(ctr==1){
%>
	<table bgcolor=#fce9c5 class="sort-table" id="table-1" width='80%' bottommargin=0 rules=ROWS topmargin=0 cellspacing=0 cellpadding=0 border=1 align=center>	
		<thead>
		<tr bgcolor="#ff8c00">	
...
qrymass="   select nvl(MASSCUTS,0)MASSCUTS,nvl(TATOTAL,0)TATOTAL,nvl(MARKSAWARDED,0)MARKSAWARDED  from  STUDENTGRADEMASSCUTS where fstid='"+rs1.getString("fstid")+"' and studentid='"+rs1.getString("studentid")+"'  and examcode='"+mEC+"'  " ;
rsmass=db.getRowset(qrymass);
//out.println("HEllo*************************"+qrymass);
		
		 if(rsmass.next())
		{
//out.println("HEllo");
%>
	<td align=center><b><font color=white>Attendance(%)<font></b></td>

		<td align=center><b><font color=white>Total Marks Awarded<BR> In (<%=mEC%>)<font></b></td>
		<td align=center><b><font color=white>Total TA Marks<BR> In (<%=mEC%>)<font></b></td>
		<td align=center><b><font color=white>Total Marks Cut<font></b></td>
		<td align=center><b><font color=white>NET TA Marks <font></b></td>
...
qrymass="   select nvl(ATTENDANCEPERCENTAGE,0)ATTENDANCEPERCENTAGE,nvl(MASSCUTS,0)MASSCUTS,nvl(TATOTAL,0)TATOTAL,nvl(MARKSAWARDED,0)MARKSAWARDED from  STUDENTGRADEMASSCUTS where fstid='"+rs1.getString("fstid")+"' and studentid='"+rs1.getString("studentid")+"'  and examcode='"+mEC+"'  " ;
rsmass=db.getRowset(qrymass);
		
		 if(rsmass.next())
		{

mNETTAMARKS=rsmass.getDouble("TATOTAL")-rsmass.getDouble("MASSCUTS");

if (mNETTAMARKS<0){
mNETTAMARKS=0;
}else{
mNETTAMARKS=mNETTAMARKS;
}

%>
<td align=center>&nbsp; &nbsp; &nbsp; &nbsp;<%=rsmass.getString("ATTENDANCEPERCENTAGE")%></td>
	<td align=center>&nbsp; &nbsp; &nbsp; &nbsp;<%=rsmass.getString("MARKSAWARDED")%></td>
		<td align=center>&nbsp; &nbsp; &nbsp; &nbsp;<%=rsmass.getDouble("TATOTAL")%></td>
			<td align=center>&nbsp; &nbsp; &nbsp; &nbsp;<%=rsmass.getDouble("MASSCUTS")%></td>
<td align=center>&nbsp; &nbsp; &nbsp; &nbsp;<%=mNETTAMARKS%></td>
<%
		}else{
...
<script type="text/javascript">
	var st1 = new SortableTable(document.getElementById("table-1"),[ "Number", "CaseInsensitiveString", "CaseInsensitiveString", "Number", "CaseInsensitiveString","Number", "Number", "Number","Number", "Number","Number"]);
		</script>
