...
else
{
	...
	if(!mMemberID.equals("") && !mMemberCode.equals("") && !mMemberName.equals("")) 
	{
	  ...
	  //-----------------------------
		...
		qry="Select WEBKIOSK.ShowLink('63','"+ mChkMemID+"','"+mChkMType+"','"+mRole+"','"+ mIPAddress +"') SL from dual";
...
<%
	...
	{	
		...
		qry=qry+" trunc(sysdate) between trunc(FROMDATE) and trunc(TODATE) and facultytype=decode('"+mDMemberType+"','E','I','E')"; 
        	qry=qry+" AND (fstid) in ((select fstid from  facultysubjecttagging where facultytype=decode('"+mDMemberType+"','E','I','E') and employeeid='"+mDMemberID+"' and nvl(deactive,'N')='N' AND (LTP='L' OR (LTP='E' AND PROJECTSUBJECT='Y') OR LTP='P') ";  
                qry=qry+" UNION select fstid from MULTIFACULTYSUBJECTTAGGING where companycode='"+mComp+"' and institutecode='"+mInst+"' and facultytype=decode('"+mDMemberType+"','E','I','E') and employeeid='"+mDMemberID+"'))";
...
if(request.getParameter("x")!=null)
{
	...
	if( request.getParameter("Subject")!=null && request.getParameter("Event")!=null && request.getParameter("Exam")!=null)
	{
		...
		if(mSelf.equals("Self"))
		{
			...
			qry=qry+" And EVENTSUBEVENT='"+mEvent+"' and (ltp='L' OR (LTP='E' AND PROJECTSUBJECT='Y') OR LTP='P') and subjectID='"+mSC+"' AND  NVL (deactive, 'N') = 'N' ";
		}
		else 
		{
			qry="select  WEIGHTAGE, MARKSORPERCENTAGE, MAXMARKS from V#EXAMEVENTSUBJECTTAGGING  ";
		...
		if (mMOP.equals("M"))
			...
			MyMax=100;
		%>
		<Marquee><Font Color=Arial size=3 color=mahroon><B>Due to some technical reason, Marks Entry (Grid version) is not available to you. You will shortly get the same. Sorry for your inconvenience.</B></Font></Marquee>
		<%
		if(mMOP.equals("M"))
		{
			%>
		...
		//qry=qry+" AND employeeid='"+mDMemberID+"' and facultytype=decode('"+mDMemberType+"','E','I','E')";
                qry=qry+" AND ((EMPLOYEEID=(Select '"+mDMemberID+"' EmployeeID from dual)) OR (fstid in (select fstid from MULTIFACULTYSUBJECTTAGGING where companycode='"+mComp+"' and institutecode='"+mInst+"' and facultytype=decode('"+mDMemberType+"','E','I','E') and employeeid='"+mDMemberID+"')) and facultytype=decode('"+mDMemberType+"','E','I','E'))";
		//qry=qry+" AND employeeid in (Select '"+mDMemberID+"' EmployeeID from dual UNION Select EmployeeID from FacultySubjectTagging where FSTID in (SELECT FSTID FROM MULTIFACULTYSUBJECTTAGGING WHERE COMPANYCODE='"+mComp+"' and INSTITUTECODE='"+mInst+"' and FACULTYTYPE=decode('"+mDMemberType+"','E','I','E') and EMPLOYEEID='"+mChkMemID+"')) and facultytype=decode('"+mDMemberType+"','E','I','E')";
                qry=qry+" AND EVENTSUBEVENT='"+mEvent+"' " ;
...
//******************************************************************
	...
	<%
		...
		{
			out.print(" &nbsp;&nbsp;&nbsp <b><font size=3 face='Arial' color='Red'>Either you have locked Marked Entry or not authorized to proceed. Please Contact Subject/Grade Coordinator ! </font> <br>");
			%>
			</td></tr>
			<%
		}
