...
try{
                      ...
                      qry = "SELECT distinct SEMESTERTYPE, SECTIONBRANCH,subjectid, enrollmentno,subsectioncode,  studentid,ENTRYBYFACULTYID,ENTRYBYFACULTYNAME,studentname ,COUNT (*) noofclass    FROM v#studentattendance a  Where  a.LTP='" + mLTP + "' and a.subsectioncode='"+SUBCODE+"' and a.subjectcode='"+mSubjectcode+"' AND NVL (a.present, 'N') = 'Y' and nvl(Studentdeactive,'N')='N'";
...
//DateWiseAttendanceStudentDetail
					...
					<a target="NEW" href="ViewDatewiseLecAttendance.jsp?CTYPE=''&EXAM=<%=mEXAMCODE%>&SC=<%=rs1.getString("subjectid")%>&LTP=<%=mLTP%>&SEC=<%=rs1.getString("SECTIONBRANCH")%>&SUBSEC=<%=rs1.getString("subsectioncode")%>&SEMESTERTYPE=<%=rs1.getString("SEMESTERTYPE")%>&mMemberID=<%=rs1.getString("studentid")%>"
					>
