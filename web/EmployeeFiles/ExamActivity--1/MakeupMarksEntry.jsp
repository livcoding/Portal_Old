...
{    
	...
	for(i=0;i<DataCombo.options.length;i++)
       {       	
		...
		//	alert("Events "+QryEvent+" events "+evs1)	
...
try
	...
	if (request.getParameter("x")==null) 
	{
		while(rsdc.next())
		{
			...
			mEventsubevent2=rsdc.getString("EVENTSUBEVENT").toString().trim();
...
%>
	...
	{
		%>
		<select name="Event" tabindex="3" id="Event" style="WIDTH: 156px" onclick="ChangeOptions1(Exam.value,Event.value,DataComboSubject,Subject);" onChange="ChangeOptions1(Exam.value,Event.value,DataComboSubject,Subject);">	
		<%   
		while(rse.next())
		{
...
try
{
	...
	if (request.getParameter("x")==null) 
	{
		...
		while(rsds.next())
		{
		mSubj1=rsds.getString("examcode")+"***"+rsds.getString("eventsubevent").toString().trim()+"///"+rsds.getString("SubjectID") ;
			if(msubj.equals(""))
 			msubj=mSubj1;
			%>
...
<%
	...
	{
		%>
		<select name="Subject" tabindex="4" id="Subject" style="WIDTH: 350px">	
		<%   
		while(rss.next())
		{
...
<INPUT Type="submit" tabindex="7" Value="Show/Refresh">&nbsp;&nbsp;&nbsp;
	...
	<%	
		...
		qry=qry+" And EVENTSUBEVENT='"+mEvent+"' and facultytype=decode('"+mDMemberType+"','E','I','E') and (ltp='L' OR PROJECTSUBJECT='Y' or ltp='P') and subjectID='"+mSC+"' ";
...
qry=qry+" AND examcode='"+mEC+"' and employeeid='"+mDMemberID+"' and facultytype=decode('"+mDMemberType+"','E','I','E') and (ltp='L' OR PROJECTSUBJECT='Y' or ltp='P') and subjectID='"+mSC+"' ";
