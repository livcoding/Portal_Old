...
int nointime=0, noouttime=0,mSpecialCase=0;
...
qry2="SELECT DISTINCT a.membercode, a.employeename, a.designation,                                TO_CHAR (b.intime, 'hh:mi PM') intime,                NVL (TO_CHAR (b.outtime, 'HH:MI PM'), ' ') outtime,                NVL (TO_CHAR (b.intime, 'hh24mi'), '0') ingtime,                NVL (TO_CHAR (b.outtime, 'HH24MI'), '0') outgtime ,to_char(to_date('"+mWorkDate+"','dd-mm-yyyy'),'Day')ssDay ,					 TO_CHAR (b.satintime, 'hh24mi') satintime,                NVL (TO_CHAR (b.satouttime, 'hh24mi'), ' ') satouttime				           FROM ATT#OTHERDUTYTIMING b, v#allmembers a          WHERE b.institutecode = a.institutecode            AND a.institutecode = '"+mInst+"'            AND a.memberid = b.userid            AND a.membertype = b.usertype            AND a.membertype = 'E'            AND a.memberid ='"+mFaculty+"'                     and  trunc(to_date('"+mWorkDate+"','dd-mm-yyyy')) >= trunc(b.FROMDATE)          and trunc(to_date('"+mWorkDate+"','dd-mm-yyyy')) <=trunc(b.TODATE)";	
	...
	if(rs2.next())
		...
		//mHalfDay=rs.getString("HALFDAY");
			...
			{
					mPInTime= rs2.getInt("satintime") ;
			mPOutTime=rs2.getInt("satouttime") ;
	...
	if(rs4.next())
			...
			//	out.print(rs.getString("LEAVESTATUS")+":::LL");
			...
			if(rs1.next())
			{
				...
				if(nointime!=-1 && noouttime!=-1 && mVacation!=1 && mSpecialCase!=1 && !mWorkDate1.equals(mCDate) && !mDayss.equals("sunday"))
				{
				...
				if(mSatDay.equals(mHalfDay))
				{
					...
					{ //out.print((mOutgtime >  mAHalfDayTime)+"OUT"+(mOutgtime < mHalfDayTime ) );
			...
			<td nowrap align=center>&nbsp;--- </td>	
				<%
				 ...
				 if(rs.getString("LEAVESTATUS").equals("N") && mSpecialCase!=1  && mVacation!=1 && !mDayss.equals("sunday"))
				{
						++nointime;
						++noouttime;
				}
	...
	//out.print(mIngtime+"ss"+mOutgtime+":nointime:"+nointime+":noouttime:"+noouttime+":::"+mSatDay);
		
			if ( (mInMaxTime == nointime) ||  (mOutMaxTime == noouttime) &&  mSpecialCase!=1  && !mWorkDate1.equals(mCDate) && mVacation!=1 && !mDayss.equals("sunday") )
			{
...
else if(nointime==-1 && noouttime==-1 &&  (rs.getString("LEAVESTATUS").equals("P")) &&  mSpecialCase!=1 && !mWorkDate1.equals(mCDate) && mVacation!=1 && !mDayss.equals("sunday"))
			{
				 ...
				 if(mPInTime < mIngtime)
					...
					//out.print(mIngtime+"ss"+mOutgtime+":nointime:"+nointime+":noouttime:"+noouttime+":::"+mSatDay+"KKK"+mHalfDay);
			
//out.print(mSatDay==mHalfDay);

						if(mSatDay.equals(mHalfDay))
						{
							//out.print("LLLLLL");	
								
								if(mHalfDayTime > mOutgtime)
							{
						%>
						  <td align=center><font color=red size=2><b>-PENALITY-</b></font></td>
						<%
		...
		else if(rs.getString("LEAVESTATUS").equals("P"))
			{
			%>
			...
			else if(rs.getString("LEAVESTATUS").equals("F") || rs.getString("LEAVESTATUS").equals("F/W"))
			{
			%>
			...
			else if(rs.getString("LEAVESTATUS").equals("V") || mVacation==1)
			{
			%>
