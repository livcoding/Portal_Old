...
try
{
  ...
  //----------------------
			...
			<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: verdana"><B>Level Wise Marks Entry List </B></TD>
			</font></td></tr>
			</TABLE>
...
/*********************************/
		...
		/*String 	qrys="SELECT distinct FSTID FROM V#STUDENTLTPDETAIL WHERE INSTITUTECODE='"+mInst+"' AND COMPANYCODE='"+mComp+"' AND EXAMCODE='"+mExamID+"' AND nvl(DEACTIVE,'N')='N' and nvl(STUDENTDEACTIVE,'N')='N'  ";
			qrys=qrys+" AND FSTID  IN (SELECT FSTID FROM v#studenteventsubjectmarks  WHERE INSTITUTECODE='"+mInst+"' and LTP='"+QryLTP+"' AND EXAMCODE='"+mExamID+"' AND EVENTSUBEVENT like  '%"+mEventCode+"%'  and NVL(DEACTIVE,'N')='N')";
			out.print(qrys);
		...
		{*/
			...
			while(rs.next() )
			{
				...
				{
					%>
					<td nowrap><Font size=2 face=arial><%=rs.getString("Subject")%></font></td>
					<%
				...
				//}
					%>
				<td nowrap><Font size=2 face=arial><%=rs.getString("PC")%> (<%=rs.getString("SEC")%>-<%=rs.getString("SUBSEC")%>)</font></td>
				<%
				...
				{
					String qry1="select COORDINATORNAME from V#EX#SUBJECTGRADECOORDINATOR where FSTID='"+rs.getString("FSTID")+"' AND examcode='"+mExamID+"' AND INSTITUTECODE='"+mInst+"'";
					//out.print(qry1);
					ResultSet rsg=db.getRowset(qry1);
					if(rsg.next())
					{
					%>
					<td nowrap><Font size=2 COLOR=PURPLE face=arial><b>Pre-Marks Entry <BR>Not Done by Faculty</b></font></td>
					<%
					}
					else
					{
