...
<HTML>
<head>
<TITLE>#### <%=mHead%> [ Pre Registration Load Distribution ] </TITLE>
...
String qry="",mWebEmail="",EmpIDType="",qry1="",mSname="",Type="",mltp="",moldPSS="",mPSS="";
...
ResultSet rs=null,rs1=null,rse=null,rsi=null,rsee=null,rso=null,rso1=null,rsl=null,rsse=null;
String mInst="",mType="",mSendhod="",qryl="";
String mName1="",mName2="",mName3="",mName4="",mName5="",mSubjectType="",qryo="";
String mName6="",mName7="",mName8="",mName9="",mName10="",moldnew="",mEid1="";
int  mTotalRec = 0;
int t=0;
String mChoice="",mMerge="",mImMergeSec="",moSectionBranch="",moSectionBranch1="";
String mExamCode="",mNameL="",mNameR="";
String mINSTITUTECODE="",mSemt="",mBasket="",mDuration="",mClass="",mNameLML="",mNameLMR="";
String mEmployeeID="";
String mSUBJECTCODE="",mSubjectname="",mEmployeename="",mElective="",mEmp="",mLTP="",mDept="";
int kk=0;
int z=0;
int ctr=0;
int msno=0;
int ctr1=0;
String mSectBranch="",mFac="",mSection1="";
String mSeccount="",mSubsection="",mSection="",mEname="";
int len=0,pos1=0,pos2=0,pos3=0,pos4=0,pos5=0,pos6=0;
int mlen=0,mpos1=0;
String mMainSec="",mMergeSec="";
String mAssign1="",mMin1="",mMax1="";
float mAssign=0,mAssign11=0,mMin=0,mMax=0,mweightage=0;
String mEid="",mETyp="",mEcmp="";
String mfrmtime="",mtotime="",mroomtype="",mPrCode="",mOldsectionbranch="",mSUBJECTID="";
int mFlag=0;
String Fstid="";
int mTotal=0;
int mDurationi=0;
...
%>
	...
	<td><b>Duration of Class</b></td>
	<td><b>Status</b></td>
	</tr>
<%
	if(mType.equals("C"))
	{
	qry="select distinct SUBSECTIONCODE subsectioncode,to_char(seqid)seqid,";
...
//				out.println(mEmp);
				...
				mMergeSec=mMerge.substring(mpos1+3,mlen);
				}		
	...
	if(mType.equals("C"))
	{
	qry1="select distinct SUBSECTIONCODE subsectioncode,to_char(seqid)seqid,";
...
String mSection2="";
	...
	{  
		msno++;
			...
			if(!mSection2.equals(rsi.getString("SECTIONBRANCH")))
			{
			ctr1=0;
			mSection2=rsi.getString("SECTIONBRANCH");	
			mNameL=mSection+"***L///"+String.valueOf(ctr1).trim()+"###"+mSeccount;  
			mNameR=mSection+"***R///"+String.valueOf(ctr1).trim()+"###"+mSeccount;
			mNameLML=mSection+"***L///"+String.valueOf(ctr1).trim()+"###"+mSeccount+"merge";    
			mNameLMR=mSection+"***R///"+String.valueOf(ctr1).trim()+"###"+mSeccount+"merge";   
			}
		 	mMerge=request.getParameter(mNameLML).toString().trim();
		...
		qry="select 'y' from PR#HODLOADDISTRIBUTION A  where A.institutecode='"+mINSTITUTECODE+"' ";
		...
		//out.println(qry);
			...
			mAssign11=mAssign;
				...
				if(!mOldsectionbranch.equals(rsi.getString("SECTIONBRANCH")) && mMerge.equals("NONE"))	
				{
					mweightage = mweightage+mTotal;
					mOldsectionbranch = rsi.getString("SECTIONBRANCH");
	...
	<%
	qry="delete from TEMP#PR#LOADDISTRIBUTION where LTP='"+mLTP+"' and  COMPANYCODE='"+mEcmp+"' and SECTIONBRANCH ='"+mSectBranch+"' and SUBJECTID='"+mSUBJECTID+"' ";
			//out.println(qry);
			int n1233=db.insertRow(qry);
	...
	{  
		...
		if(!mSection1.equals(rs1.getString("SECTIONBRANCH")))
		{
		   mSection1=rs1.getString("SECTIONBRANCH");	
		...
		//	mSeccount=rs1.getString("cnt");
			...
			if(!mMerge.equals("NONE"))
			{	
				if(mMainSec.equals(mSectBranch))
				{
					qryo="select fstid from PR#HODLOADDISTRIBUTION where institutecode='"+mINSTITUTECODE+"' and examcode='"+mExamCode+"' and ";
					qryo=qryo+" SUBJECTID='"+mSUBJECTID+"' and sectionbranch='"+mMergeSec+"' and ";
					qryo=qryo+" LTP='"+mLTP+"' and semestertype='REG'  order by fstid ";
			...
			{	
				...
				if(moSectionBranch.equals(mSectBranch))
				{
				qryl="select fstid from PR#HODLOADDISTRIBUTION where institutecode='"+mINSTITUTECODE+"' and examcode='"+mExamCode+"' and ";
				qryl=qryl+" SUBJECTID='"+mSUBJECTID+"' and sectionbranch='"+moSectionBranch+"' and ";
				qryl=qryl+" LTP='"+mLTP+"' and semestertype='REG' order by fstid ";
			...
			try{
			qry="";
			qry="select a.EMPLOYEEID,a.FACULTYTYPE,a.NOFHRS from Temp#pr#loaddistribution a where COMPANYCODE='"+mEcmp+"' and SUBJECTID='"+mSUBJECTID+"' and LTP='"+mLTP+"' and SECTIONBRANCH='"+mSectBranch+"'";
			//out.print(qry);
			rs=db.getRowset(qry);
			while(rs.next())
			{
				if(!mEid.equals(rs.getString("EMPLOYEEID")))
				{
					...
					qry=qry+" INSTITUTECODE, COMPANYCODE, FSTID, FACULTYTYPE, EMPLOYEEID, NOFHRS,ENTRYBY,  ENTRYDATE, DEACTIVE)";
					qry=qry+" VALUES ('"+mINSTITUTECODE+"','"+mEcmp+"','"+Fstid+"','"+rs.getString("FACULTYTYPE")+"','"+rs.getString("EMPLOYEEID")+"' , '"+rs.getString("NOFHRS")+"','"+mChkMemID+"', sysdate,'N')";		
					//out.println(qry);
			...
			String qryl123456="select DURATIONOFCLASS from PR#HODLOADDISTRIBUTION where institutecode='"+mINSTITUTECODE+"' and examcode='"+mEcmp+"' and ";
			qryl123456=qryl123456+" SUBJECTID='"+mSUBJECTID+"' and ";
			qryl123456=qryl123456+" LTP='"+mLTP+"' and semestertype='"+mSemt+"' and fstid='"+Fstid+"' order by DURATIONOFCLASS ";
			//out.println(qryl123456);
			rse=db.getRowset(qryl123456);	
			if(rse.next())
			{
				duration=rse.getString("DURATIONOFCLASS");
			}
		   ...
		   <% 
			qry="select b.EMPLOYEENAME,a.NOFHRS from MULTIFACULTYSUBJECTTAGGING a,V#STAFF b  where a.EMPLOYEEID=b.EMPLOYEEID	and a.COMPANYCODE =b.COMPANYCODE and a.FSTID='"+Fstid+"'";
			//out.println(qry);
...
//		out.println(mMerge);
			...
			{
				//out.println(moSectionBranch1+"12 12 21 2 "+mSectBranch);
				
				if(moSectionBranch1.equals(mSectBranch))
				{
					
					qryl="select fstid from PR#HODLOADDISTRIBUTION where institutecode='"+mINSTITUTECODE+"' and examcode='"+mExamCode+"' and ";
					qryl=qryl+" SUBJECTID='"+mSUBJECTID+"' and sectionbranch='"+mSectBranch+"' and ";
					qryl=qryl+" LTP='"+mLTP+"' and semestertype='REG'  order by fstid ";
					//out.println(qry1);
					rsl=db.getRowset(qryl);	
					if(rsl.next())
	...
	//out.println("hello123 "+mImMergeSec);
	int n=0;
	if(!mImMergeSec.equals(""))
				{
		...
		qry=qry+"  and SUBSECTIONCODE='"+mSubSect+"' ";
		qry=qry+"  and SEMESTER='"+mSem+"' and SEMESTERTYPE='"+mSemt+"' ";
		qry=qry+" and BASKET='"+mBasket+"' and SUBJECTID='"+mSUBJECTID+"' and LTP='"+mLTP+"' ";
		n=db.update(qry);
		//out.println(n+" Update");
		//out.println(qry);
		
		//out.println("adfad"+session.getAttribute("MultiFaculti")+"||||||||||");
		try
		{
			...
			if(rs.next())
			{
				qry="delete MULTIFACULTYSUBJECTTAGGING where FSTID='"+mImMergeSec+"'";
			...
			while(rs.next())
			{
				if(!mEid.equals(rs.getString("EMPLOYEEID")))
				{
					qry="INSERT INTO MULTIFACULTYSUBJECTTAGGING (";
					qry=qry+" INSTITUTECODE, COMPANYCODE, FSTID, FACULTYTYPE, EMPLOYEEID, NOFHRS,ENTRYBY,  ENTRYDATE, DEACTIVE)";
					qry=qry+" VALUES ('"+mINSTITUTECODE+"','"+mEcmp+"','"+mImMergeSec+"','"+rs.getString("FACULTYTYPE")+"','"+rs.getString("EMPLOYEEID")+"' , '"+rs.getString("NOFHRS")+"','"+mChkMemID+"', sysdate,'N')";		
					//out.println(qry);
					int n1=db.insertRow(qry);
				}
			...
			/*qry="delete from TEMP#PR#LOADDISTRIBUTION where LTP='"+mLTP+"' and  COMPANYCODE='"+mEcmp+"' and SECTIONBRANCH ='"+mSectBranch+"' and SUBJECTID='"+mSUBJECTID+"' and USERID='"+mChkMemID+"'";
			//out.println(qry);
		...
		if (n==0)
		{
			FstidNew=db.GenerateFSTID(mINSTITUTECODE);
			qry="insert into PR#HODLOADDISTRIBUTION (INSTITUTECODE, COMPANYCODE, FACULTYTYPE, ";
			qry=qry+" FACULTYID, EXAMCODE, ACADEMICYEAR, PROGRAMCODE, TAGGINGFOR, SECTIONBRANCH, ";
			qry=qry+" SUBSECTIONCODE, SEMESTER, SEMESTERTYPE, BASKET, SUBJECTID, LTP,REQROOMTYPE, ";
			qry=qry+"  DEPARTMENTRUNNIG, DURATIONOFCLASS, NOOFCLASSINAWEEK, ";
			...
			qry=qry+" '"+mProg+"','"+mTag+"','"+mSectBranch+"','"+mSubSect+"','"+mSem+"','"+mSemt+"', ";
			qry=qry+" '"+mBasket+"','"+mSUBJECTID+"','"+mLTP+"','"+mroomtype+"', ";	
			qry=qry+" '"+mDept+"','"+mDuration+"','"+mClass+"','D','"+mChkMemID+"',sysdate,'"+FstidNew+"','"+mImMergeSec+"','"+mType+"','"+mElective+"' ) ";
			n=db.insertRow(qry);
			//out.print(qry);
			try{
		...
		qry1="select employeename from V#STAFF where employeeid='"+mEid+"' ";
		rse=db.getRowset(qry1);
		//out.println("heplppadfadf");
		if (rse.next())
		{
			mEid1=rse.getString("employeename");
		}
		//qry1="";
		String qryl11="select DURATIONOFCLASS from PR#HODLOADDISTRIBUTION where institutecode='"+mINSTITUTECODE+"' and COMPANYCODE='"+mEcmp+"' and ";
			qryl11=qryl11+" SUBJECTID='"+mSUBJECTID+"'  and ";
			qryl11=qryl11+" LTP='"+mLTP+"' and semestertype='"+mSemt+"' and fstid='"+mImMergeSec+"' order by DURATIONOFCLASS ";
			//out.println("4444"+qryl11);
			rse=db.getRowset(qryl11);	
			if(rse.next())
			{
				duration=rse.getString("DURATIONOFCLASS");
			}
	  %>
		...
		<%
  	if(FstidNew.equals(""))
	  FstidNew=mImMergeSec;
	  qry="select b.EMPLOYEENAME ,a.NOFHRS from MULTIFACULTYSUBJECTTAGGING a,V#STAFF b  where a.EMPLOYEEID=b.EMPLOYEEID	and a.COMPANYCODE =b.COMPANYCODE and a.FSTID='"+FstidNew+"'";
			//out.println(qry);
			rsse=db.getRowset(qry);
			while (rsse.next())
			{
