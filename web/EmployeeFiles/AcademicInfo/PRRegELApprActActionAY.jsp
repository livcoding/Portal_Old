...
String mName1="",mName2="",mName3="",mName4="",mName5="",mName6="",mName7="",mName8="",mName9="",Approvetype="";
 Approvetype=request.getParameter("Approvaltype")==null?"":request.getParameter("Approvaltype");
  Academicyear=request.getParameter("acdyear")==null?"":request.getParameter("acdyear");
 //Approvetype = request.getParameterValues("Approvaltype").toString().trim();
 /*String[] items = request.getParameterValues("acadyear");
        for(int loopIndex = 0; loopIndex < items.length; loopIndex++){
            out.println(items[loopIndex] + "','");
        }
  out.print("Approved for "+Approvetype);
*/
 // out.print(Academicyear+"hello");
...
//out.print(mSubjCode+"sdfsdadfsa");
						...
						<%		
							...
							if (mLastStatus.equals(mRunning))
								 ...
								 if(mRunning.equals("Y"))
								  ...
								  else
								  {
									...
									<%
                                                                    ...
                                                                    //out.print(mRunning+"mLastStatus"+mLastStatus);
                                                                   // out.print(mST);
							  }
							 else if (mRunning.equals("Y") && !mLastStatus.equals(mRunning))
							  {
								...
								if(mST.equals("E"))
								  {
									qry="Update PR#ELECTIVESUBJECTS B set SUBJECTRUNNING='Y' Where B.INSTITUTECODE='"+mInst+"' AND B.EXAMCODE='"+mExam+"' and b.ACADEMICYEAR in ("+Academicyear+") ";
							...
							else 
							 {
								if(mST.equals("E"))
								   {	
									qry="Update PR#ELECTIVESUBJECTS B set SUBJECTRUNNING='N' Where B.INSTITUTECODE='"+mInst+"' AND B.EXAMCODE='"+mExam+"' and b.ACADEMICYEAR in ("+Academicyear+")";
									qry=qry+" And B.SubjectID='"+mSubjID+"' And (b.INSTITUTECODE,b.EXAMCODE) in (select PE.INSTITUTECODE,PE.ExamCode from PREVENTMASTER PE ";
									qry=qry+" Where PE.INSTITUTECODE='"+mInst+"' and PE.EXAMCODE='"+mExam+"' and nvl(PE.PRCOMPLETED,'N')='N' and nvl(PE.PRBROADCAST,'N')='Y' ";
									qry=qry+" and nvl(PE.PRREQUIREDFOR,'N')<>'S' and nvl(PE.DEACTIVE,'N')='N'";
									qry=qry+" And (PE.INSTITUTECODE, PE.PREVENTCODE) in (Select D.INSTITUTECODE, D.PREVENTCODE ";
          ...
          //out.print("Before procedure Calling+mInst+"+mInst+"<BR>"+mPreventcode+"<BR>"+mExam+"<BR>"+mHODMemberID);
			//out.print(mfinalized);
				if (mfinalized.equals("YES"))

					{
                        // out.print("Before procedure Calling"+mInst+""+mPreventcode+""+mExam+""+mHODMemberID);
				/*		qry="Update PREVENTS B set APPROVED='Y', APPROVEDBY='"+mDMemberID+"', APPROVALDATE=sysdate Where B.INSTITUTECODE='"+mInst+"'";
						...
						qry=qry+" and nvl(PE.PRREQUIREDFOR,'N')<>'S' and nvl(PE.DEACTIVE,'N')='N') And nvl(B.DEACTIVE,'N')='N'";
                                    ...
                                    //new added
                                          ...
                                          qry="Update PREVENTSaytagging B set APPROVED='Y', APPROVEDBY='"+mDMemberID+"', APPROVEDDATE=sysdate Where B.INSTITUTECODE='"+mInst+"'";
                                          qry=qry+ " And B.MEMBERTYPE<>'S' and B.MEMBERID='"+mHODMemberID+"' ";
                                          qry=qry+" and b.academicyear in ("+Academicyear+") ";
                                           qry=qry+ " And (b.INSTITUTECODE,b.PREVENTCODE) in (select PE.INSTITUTECODE,PE.PREVENTCODE from PREVENTMASTER PE ";
                                          qry=qry+" Where PE.INSTITUTECODE='"+mInst+"' and PE.EXAMCODE='"+mExam+"' and nvl(PE.PRCOMPLETED,'N')='N' and nvl(PE.PRBROADCAST,'N')='Y' ";
                                          qry=qry+" and nvl(PE.PRREQUIREDFOR,'N')<>'S' and nvl(PE.DEACTIVE,'N')='N') And nvl(B.DEACTIVE,'N')='N'";
              ...
              //------------------------------------DEFAULT PROCEDURE (START)-------------------------------------//
                 if(Approvetype.equals("D")){
	          //out.print(mInst+" ---- "+mPreventcode+" -- "+mExam+"  --  "+mHODMemberID);
							try{
                              rrr=db.PopulateElectiveRunning(mInst,mPreventcode ,mExam, mHODMemberID)==null?"data updated":db.PopulateElectiveRunning(mInst,mPreventcode ,mExam, mHODMemberID);
                            // out.print("Default Running..........");
                             }catch(Exception e){
                              //out.print("Default Running   :  "+e+"<br>");
                              }
                               }    // out.print("Satendra : "+rrr );
              //------------------------------------DEFAULT PROCEDURE (END)-------------------------------------//



       //----------------------------------------FIFO PROCEDURE(START)-------------------------------------------------------------------//
                            else if(Approvetype.equals("F")){ 
                                 try{
                              rrr=db.PopulateElectiveRunning_FIFS(mInst,mPreventcode ,mExam, mHODMemberID)==null?"data updated":db.PopulateElectiveRunning_FIFS(mInst,mPreventcode ,mExam, mHODMemberID);
                             // out.print("FIFO Running..........");
                                 }catch(Exception e){
       ...
       //----------------------------------------CGPA PROCEDURE(START)----------------------------------------------//

                                    else if(Approvetype.equals("C")){ 
                             try{

                               rrr=db.PopEleRunning_CGPAAY(mInst,mPreventcode ,mExam, mHODMemberID)==null?"data updated":db.PopEleRunning_CGPAAY(mInst,mPreventcode ,mExam, mHODMemberID);
                              out.print("CGPA Running..........");
                             }catch(Exception e){
                               //out.print("cgpa Running  :  "+e+"<br>");
                              }
...
//out.print("Procedure running successfully");
