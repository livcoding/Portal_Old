...
<option  selected value="N"><--Select Institiute--></option>
<%
while(rs.next())
{
...
%>
&nbsp;<select name="InstCode" tabindex="1" id="InstCode" style="WIDTH: 150px">
<option   value="N"><--Select Institiute--></option>
<%
while(rs.next())
{
mInst=rs.getString("InstCode");
...
qry = " Select REGCODE RegCode, ExamCode EXAMCODE, EXAMPERIODFROM,INSTITUTECODE FROM (";
				...
				qry += " Where A.INSTITUTECODE=B.INSTITUTECODE   and A.EXAMCODE=B.EXAMCODE  AND nvl(B.Deactive,'N')='N' AND  A.ExamCode  NOT LIKE '%SUP%' ";
...
{
%>
<select name="DataRegCode" tabindex="0" id="DataRegCode"  style="WIDTH:0px;background-color:transparent;border-bottom-style:hidden; border-left-style:hidden; border-right-style:hidden;border-top-style:hidden; ">	
<%
while(rsfd.next())
{
...
%>
<FONT face=Arial size=2><STRONG>T1 Date:
...
<td>
 <FONT face=Arial size=2><STRONG>T2 Date:
<FONT face=Arial COLOR=GREEN size=1><STRONG>
(DD-MM-YYY)</STRONG>
 </FONT></STRONG></FONT>
...
<td>
 <FONT face=Arial size=2><STRONG>T3 Date:
<FONT face=Arial COLOR=GREEN size=1><STRONG>
(DD-MM-YYY)</STRONG>
 </FONT></STRONG></FONT>
...
qry1="select distinct  programcode from V#StudentAttendance a Where  a.LTP in ("+mLTP+") AND   nvl(Studentdeactive,'N')='N' ";
		...
		qry1=qry1+" group by  programcode order by programcode" ;
//	out.print(qry1);
...
//out.print(mpp+"PLPLP");
		...
		qry=qry+"  group by  institutecode , examcode,SUBJECTCODE,Subject,SUBJECTID order by Subject" ;
