...
//out.print(rs1.getString("subsection1")+"-----");
/*
...
qry2="            Select Substr(sys_connect_by_path(subsection,','),2) subsection, SUBSTR (SYS_CONNECT_BY_PATH (subsection1, ','), 2) subsection1 from (SELECT DISTINCT   chr(39)||x.subsectioncode||chr(39) subsection, x.subsectioncode subsection1, row_number() over (order by x.subsectioncode) xx, count(*) over()cnt            FROM facultysubjecttagging x          WHERE x.subjectid ='"+mSubject+"'              AND x.mergewithfstid IN ('"+rs1.getString("fstid")+"' )            AND x.examcode ='"+mExam+"'              AND x.institutecode ='"+mInst+"'                  AND x.facultytype =DECODE ('"+mDMemberType+"','E','I','E')                AND x.employeeid =  '"+mDMemberID+"'  ) where xx=cnt start with xx=1 connect by xx=prior xx+1            ";
	...
	while(rs3.next())
		{
			...
			if(mSubs1.equals("ALL"))
			{
				...
				<b><%=rs3.getString("subsection1").toString().trim()%></b> 
				<%
			}		
			else
			{
					%>
				<input type="checkbox" name="SubSectionM<%=cnt1%>" id="SubSectionM<%=cnt1%>" <%=mcheck1%> value="Y" >
				<%=rs3.getString("subsection1").toString().trim()%>
...
//out.print(request.getParameter("SubSectionM"+j)+"-------------"+"SubSectionM"+j);
...
<td nowrap align=center><b><A HREF="StudAttregister.jsp?ViewType=CNF&amp;STUDID=<%=rs1.getString("studentid")%>&amp;FSTID=<%=rs1.getString("Fstid")%>&amp;SUBID=<%=mSubject%>&amp;ATTTYPE=<%=mType%>&amp;EXAMCODE=<%=mExam%>&amp;LTP=<%=mLTP%>&amp;ACAD=<%=mAcademicYear%>&amp;BRANCH=<%=mSection%>&amp;SUBSEC=<%=mSubsection%>" target=_new>Show Attendance</A></b></td>
	...
	<script type="text/javascript">
	var st1 = new SortableTable(document.getElementById("table-1"),["undefined", "Number", "CaseInsensitiveString"]);
	</script>
