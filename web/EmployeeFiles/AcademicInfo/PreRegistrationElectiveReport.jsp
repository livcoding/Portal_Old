...
try
{
   ...
   */
     String qryyr="";
    qryyr="Select  distinct ACADEMICYEAR from pr#studentsubjectchoice where examcode='"+mExam+"'  and INSTITUTECODE='"+mInstitute+"' order by 1 desc ";
                                //out.print(qryyr);
				...
				// If EL has not been approved then process INSTITUTECODE='"+mInstitute+"'
				rs=db.getRowset(qry);
				//out.print(qry+"qry1<br><br><br>");
				if (!rs.next())
				{
				...
				<form name="frm1" ID="frm1"  method=post action="PreRegistrationElectiveReportXLS.jsp">
				<tr bgcolor='#ff8c00'>
				...
				qry=qry+" And B.SUBJECTTYPE='"+mST+"' AND B.ACADEMICYEAR in("+sb1+") And (b.INSTITUTECODE,b.EXAMCODE) in (Select D.INSTITUTECODE, D.EXAMCODE ";
				...
				rs=db.getRowset(qry);
					...
					if (mST.equals("E"))
						...
						qry=qry+" And B.INSTITUTECODE=BDT.INSTITUTECODE and B.ACADEMICYEAR in ("+sb1+") and B.PROGRAMCODE=BDT.PROGRAMCODE and B.TAGGINGFOR=BDT.TAGGINGFOR and B.SECTIONBRANCH=BDT.SECTIONBRANCH";
						...
						qry=qry+" Order by count(*) Desc, Subject,ElectiveCode";

						}

					//out.print(qry+"qry3<br><br><br>");
					rs=db.getRowset(qry);
					%>
					 ...
					 if(!mOldSubject.equals(rs.getString("SC")))
						{
							 ...
							 for (int mCh=1;mCh<=maxCol;mCh++)
							{
								...
								{*/
								qry="select nvl(count(B.CHOICE),-1) Tot from  PR#STUDENTSUBJECTCHOICE B Where B.INSTITUTECODE='"+mInstitute+"' AND B.EXAMCODE='"+mE+"' And B.SUBJECTTYPE='"+mST+"' AND B.ACADEMICYEAR in("+sb1+")" ;
...
<tr bgcolor=black>
			...
			%>
                         <input type=hidden Name='acdyear1' ID='acdyear1' value="<%=sb1%>">
