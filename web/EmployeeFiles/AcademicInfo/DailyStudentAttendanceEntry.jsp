...
//	mDataComboSubSec=DataComboSub.options.length;
//	if(mDataComboSubSec==0)
//	{
//		mDataComboSubSec=document.frm.DataComboSubSec.value;
//	}
//alert(mDataComboSubSec);
...
<tr><TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: arial"><B>Daily Class Attendance Entry [ Today's Attendance or Previous Day's Excuse]</B></TD>
</font></td></tr>
</TABLE>
...
{ 
	...
	{
		%>
		<select name=Section tabindex="0" id="Section" style="WIDTH: 80px" onclick="ChangeSection(Exam.value,Subject.value,Section.value,DataComboSub,SubSection);" onChange="ChangeSection(Exam.value,Subject.value,Section.value,DataComboSub,SubSection);">
		<%   
		while(rs1.next())
		{
...
qry1+=" facultytype=decode('"+mDMemberType+"','E','I','E') and employeeid='"+mDMemberID+"' and ";
qry1+=" examcode not in (select examcode from exammaster where nvl(LOCKEXAM,'N')='Y' ";
qry1+=" and nvl(FINALIZED,'N')='Y' and NVL(DEACTIVE,'N')='Y' ) ";
qry1+=" Group By SUBSECTIONCODE ,nvl(SECTIONBRANCH,' ') ,nvl(Examcode,' '),nvl(subjectid,' ')";
qry1+=" UNION";
qry1+=" Select SUBSECTIONCODE SubSection,nvl(SECTIONBRANCH,' ') Section,nvl(Examcode,' ')examcode,nvl(subjectid,' ')subjectid from  facultysubjecttagging where ";
qry1+=" fstid in (select fstid from MULTIFACULTYSUBJECTTAGGING where companycode='"+mComp+"' and institutecode='"+mInst+"' and facultytype=decode('"+mDMemberType+"','E','I','E') and employeeid='"+mDMemberID+"') and ";
qry1+=" examcode not in (select examcode from exammaster where nvl(LOCKEXAM,'N')='Y' ";
qry1+=" and nvl(FINALIZED,'N')='Y' and NVL(DEACTIVE,'N')='Y' ) ";
qry1+=" Group By SUBSECTIONCODE ,nvl(SECTIONBRANCH,' ') ,nvl(Examcode,' '),nvl(subjectid,' ') order by SubSection ";
//out.print(qry1);
ResultSet rrss=db.getRowset(qry1);
while(rrss.next())
{
	mDataComboSubSec++;
}
%><INPUT TYPE="HIDDEN" NAME="DataComboSubSec" VALUE=<%=mDataComboSubSec%>><%
//=====================================
//-------------------------------------------------------------------------------------------
...
<%
	...
	{
		%>
		<select name="SubSection" tabindex="0" id="SubSection" multiple style="WIDTH: 90px; height:75px; vertical-align:middle">	
		<option Selected value='ALL'>ALL</option>
		<%   
		while(rs1.next())
		{
	...
	else
	{
		...
		<%
//System.out.println(request.getParameter("SubSection"));
		...
		while(rs1.next())
		{
			...
			DataSublist= request.getParameterValues("SubSection");
		

					for(int h=0;h<DataSublist.length;h++)
						{			
							
							 if(DataSublist[h]==null)
							{

							}
							else if(DataSublist[h].equals(mSubj))
							{
...
/*
	...
	qry1=qry1+" and nvl(FINALIZED,'N')='Y' and NVL(DEACTIVE,'N')='Y' )  ";
	qry1=qry1+" and examcode='"+qryexam+"' and subjectid='"+qrysubj+"'";
//	qry1=qry1+" and sectionbranch=decode('"+qrysec+"','ALL',sectionbranch,'"+qrysec+"') and
	qry1=qry1+" and institutecode='"+mInst+"' Group By SUBSECTIONCODE";
	qry1=qry1+" UNION";
	qry1=qry1+" Select SUBSECTIONCODE SubSection from  facultysubjecttagging where  ";
	qry1=qry1+" fstid in (select fstid from MULTIFACULTYSUBJECTTAGGING where companycode='"+mComp+"' and institutecode='"+mInst+"' and facultytype=decode('"+mDMemberType+"','E','I','E') and employeeid='"+mDMemberID+"') and ";
...
//	out.print(qry1);
	...
	{		
		%>
		<select name="DataComboSub" tabindex="0" id="DataComboSub" multiple style="WIDTH: 0px">	
		<%   
		while(rs1.next())
		{	
			mSubj=rs1.getString("subjectid");
	...
	else
	{
		...
		while(rs1.next())
		{
			...
			Sublist= request.getParameterValues("DataComboSub");
...
%>
 <font face="arial" size="2"><b>Students  of batches shown in window are tagged with you *</b></font>
</td></tr>
<tr><td colspan="3">
<FONT color=black face=Arial size=2><b>&nbsp; Class Period </b></FONT><input id='timepicker1' name='timepicker1' type='text' value="<%=mtimepicker1%>" size=8 maxlength=8 ONBLUR="validateDatePicker(this)">&nbsp;<IMG SRC="../../Images/timepicker.gif" BORDER="0" ALT="Pick a Time!" ONCLICK="selectTime(this,timepicker1)" STYLE="cursor:hand">
&nbsp;&nbsp;<STRONG><FONT color=black face=Arial size=2>&nbsp; To &nbsp; </FONT></STRONG>
<input id='timepicker2' name='timepicker2' type='text' value='<%=mtimepicker2%>' size=8 maxlength=8 ONBLUR="validateDatePicker(this)">&nbsp;
...
<tr><td nowrap colspan=2><font face=verdana size=2>
    <B><u>Hint:-</u>
 If Sub Sec. does not show any value (other than ALL) click again to 'Attendance Entry' from the left pane
<br>
    * In case of doubt contact department tagging co-ordinator or JIITERP<br>
   &nbsp; In case of regular Zero attendance of student check registration status with registry.<br>
   &nbsp; In case of additional name shown than what you are teaching contact department tagging co-ordinator or JIITERP.
...
if (request.getParameter("AcademicYear")!=null)
	...
	String mSubSec[]={"ALL"};
	mSubSec=request.getParameterValues("SubSection");
	try
	{
		if(mSubSec.equals("") || mSubSec==null)
		{
			mSubSec[0]="ALL";
			mSubsection="ALL";
		}
		for (int i=0;i<mSubSec.length;i++)
		{
			if(mSubsection.equals(""))
				mSubsection="'"+mSubSec[i]+"'";
			else
				mSubsection=mSubsection+",'"+mSubSec[i]+"'";
		}
	}
	catch(Exception e)
	{
...
<input style="width:220px;font-size:16px;position:absolute;text-align:middle;
	...
	<form name="frm1"  method="post" action="DailyStudentAttendanceEntryAction.jsp"> 
	...
	if(mDiffInDate>=0)
	{
		...
		{	
			...
			if (RsChk1.next() && RsChk1.getString("SL").equals("Y") && RsChk1.getString("SL1").equals("Y"))
			{
				...
				qry=qry+" - TO_CHAR(TO_DATE('"+mDTfrom+"','DD-MM-YYYY HH:MI PM'),'YYYYMMDDHH24MI')dt ";
				qry=qry+" from dual ";	
				//out.print(qry);
				...
				if(dt>0)
				{
					...
					qry=qry+" NVL(A.SECTIONBRANCH,' ')|| '('||A.SUBSECTIONCODE||')' sectionbranch, nvl(B.SEMESTER,1) SEMESTER, nvl(to_char(B.REGCONFIRMATIONDATE,'dd-mm-yyyy'),'N') REGCONFIRMATIONDATE ,nvl(B.REGCONFIRMATION,'N')REGCONFIRMATION, ";
...
//------------------Start of 'L' Percentage--------------------
						...
						if(rs1.getLong("Semester")==1 && !mREGCONFIRMATIONDATE.equals("N"))
						{
