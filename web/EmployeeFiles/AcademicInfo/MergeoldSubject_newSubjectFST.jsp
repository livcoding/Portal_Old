		...
		//if(1==1)
	  	{
			...
			/*qry="select DEPARTMENTCODE,DEPARTMENT from departmentmaster where DEPARTMENTCODE='"+DeptCode+"'";
			//out.println(qry);
			rs=db.getRowset(qry);
			if(rs.next())
			{
...
<td valign="top">			
			...
			for( ii=1; ii<=Integer.parseInt(counter); ii++)
			{
				...
				if(request.getParameter("fstid"+ii)!=null)
				{
					...
					qry=" select distinct a.employeeid, PROGRAMCODE, SECTIONBRANCH, SUBSECTIONCODE, SEMESTERTYPE,employeename||' ('||Employeecode||')'empname,A.LTP,A.MERGEWITHFSTID,a.FSTID from  facultysubjecttagging a , Employeemaster b where subjectid='"+subjectid+"' and examcode='"+mExamcode+"'   and semestertype='RWJ' and a.institutecode='"+mInst+"' and a.companycode='"+mCompcode+"' and    a.employeeid=b.employeeid   and a.companycode=b.companycode   and b.COMPANYCODE='"+mCompcode+"'   and nvl(a.deactive,'N')='N'    and nvl(b.deactive,'N')='N' and A.FSTID='"+request.getParameter("fstid"+ii)+"'  order by subsectioncode  ";
					//out.print(qry);
					rs= db.getRowset(qry);								
					while (rs.next())
					{				
		...
		<!-- 	<table id=id1 width="100%" ALIGN=CENTER bottommargin=0  topmargin=0>
			...
			qry="select distinct subjectid, subject|| ' ('|| subjectcode ||') ' sub from subjectmaster where subjectid in (select distinct subjectid from facultysubjecttagging where examcode='"+mExamcode+"' and semestertype='REG' and nvl(deactive,'N')='N' and institutecode='"+mInst+"' and COMPANYCODE='"+mCompcode+"' and ltp='"+mLT+"' ) AND NVL (DEACTIVE, 'N') = 'N' order by sub";	
			//out.println(qry);
			rs= db.getRowset(qry);
						String selected="";
...
%>
<form name="frm2"  method="Post" action='MergeoldSubject_newSubjectActionFST.jsp'>
...
<TD><B>Click</B></TD>
	...
	<TD><B>Sub Section</B></TD>
				...
				qry="   select NVL(MERGEWITHFSTID,'N') MERG,a.employeeid, PROGRAMCODE, SECTIONBRANCH, SUBSECTIONCODE, SEMESTERTYPE,employeename||' ('||Employeecode||')'empname,A.LTP,a.fstid,a.subjectid from   facultysubjecttagging a , Employeemaster b where subjectid='"+oldSubject+"' and examcode='"+mExamcode+"'  and a.institutecode='"+mInst+"' and a.companycode='"+mCompcode+"' and    a.employeeid=b.employeeid   and a.companycode=b.companycode   and b.COMPANYCODE='"+mCompcode+"'   and nvl(a.deactive,'N')='N'  and a.LTP='"+mLT+"'  and nvl(b.deactive,'N')='N' order by subsectioncode  ";
				//out.print(qry);
				rs= db.getRowset(qry);	
