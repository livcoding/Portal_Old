...
String QryFSTID="",QrySemType="",QryEMPID="";
...
if (request.getParameter("EMPID")==null)
	...
	{	
		QryEMPID=request.getParameter("EMPID").toString().trim();
	}
...
qry=qry+" and (   a.employeeid = '"+QryEMPID+"'          OR (a.fstid IN (                      SELECT b.fstid                        FROM multifacultysubjecttagging b                       WHERE a.fstid = b.fstid                             AND b.employeeid = '"+QryEMPID+"'    UNION (SELECT C.FSTID  FROM    EX#SUBJECTGRADECOORDINATOR c WHERE   c.INSTITUTECODE = '"+mInst+"' AND C.FACULTYID = '"+QryEMPID+"' AND C.FSTID=A.FSTID  )   )             )) ";
...
qry=qry+" and (   a.employeeid = '"+QryEMPID+"'          OR (a.fstid IN (                      SELECT b.fstid                        FROM multifacultysubjecttagging b                       WHERE a.fstid = b.fstid                             AND b.employeeid = '"+QryEMPID+"'   UNION (SELECT C.FSTID  FROM    EX#SUBJECTGRADECOORDINATOR c WHERE   c.INSTITUTECODE = '"+mInst+"' AND C.FACULTYID = '"+QryEMPID+"' AND C.FSTID=A.FSTID  ) )             )) ";
		...
		//		qry=qry+"  and s.employeeid = '"+QryEMPID+"'     		";  
		qry=qry+" and (   S.employeeid = '"+QryEMPID+"'          OR (S.fstid IN (                      SELECT b.fstid                        FROM multifacultysubjecttagging b                       WHERE S.fstid = b.fstid                             AND b.employeeid = '"+QryEMPID+"')             )) ";	
		if(mLTP.equals("LT"))
	        qry=qry+" and S.LTP in ('L','T')   AND  s.semestertype='"+QrySemType+"' " ;
      	else
	        qry=qry+" and S.LTP='"+mLTP+"'  AND  s.semestertype='"+QrySemType+"' ";
