...
double mMinCrLmt=0, mMaxCrLmt=0, mCourseCrPtTkn=0, mCourseCrPt=0,mTotalCrLmtTkn=0,mImpCCPCount=0;
String mBasket="", mChoice="0",mBasketB="",qry2="";
String mExamCode="",subjectrunning="";
String mInst="";
String mEmployeeID="";
String mSUBJECTID="",mEmployeename="",mElective="",msendtohod="",mCredit="";
String mEName=" ";
int n=0, BackCoreFlag=0, kk=0,mNo=0,mBas=0,mMsg=0,mBasketD=0,mDF=0,mError=0;
double sum=0,sum1=0,mMinCCPCore=0,backcore=0;
double mMinBasketD=0,mMaxBasketD=0,sumcore=0,mCCP=0;
String Flag="N",Improvement="";

ResultSet rsCCP1=null;

String qry1="",qryhead="",qrys="", mGLID="",mPostComp="",mFEEHEAD="";

     ResultSet rs12=null,rss=null,rs2=null;
...
qry=qry+" AND ACADEMICYEAR='"+mAcad+"' AND PROGRAMCODE='"+mProg+"' AND TAGGINGFOR='"+mTag+"' AND SECTIONBRANCH='"+mSect+"' AND SEMESTER="+mSem+" and basket='D' ";
//out.print(qry);
rs=db.getRowset(qry);
if(rs.next())
{
...
else
{
	mLoginComp=session.getAttribute("LoginComp").toString().trim();
			...
			%>
			<form name="frm1" method="post" action="EMP_PRStudSendToHODReAction.jsp">
...
String mStartDate="",mEndDate="",mImpRegCode="";
...
qry="SELECT nvl(PREVEXAMCODE,' ')PREVEXAMCODE, nvl(SUPREGCODE,' ')SUPREGCODE  FROM EXAMMASTER where InstiTuteCode='"+mInst+"'" +
        " and examcode='"+mExamCode+"' and NVL(LOCKEXAM,'N')='N'";
//out.println(qry);
rs=db.getRowset(qry);
if(rs.next())
{
...
mImpRegCode=mExamCode;
				...
				for (int TT1=1;TT1<=mTotalRec;TT1++)
				{
					...
					mName10="CHOICE"+String.valueOf(TT1).trim();
                                        ...
                                        //System.out.println("---111--"+request.getParameter(mName1)+"---22--"+mName1);
					if (request.getParameter(mName1)==null)
						mChk="N";
					else
						mChk=request.getParameter(mName1).toString().trim();
                                       // System.out.println("mChk-"+mChk+"Name--");
					...
					//out.println("::<br>choice---"+mChoice);
					if(mChk.equals("Y") )
					{
						//out.println(mBasket);
//out.print("ddd666666dd--:"+sum1+"---");
					/*
...
//out.print(mBasket+"ddddd:"+studelecchoise);
							if(mBasket.equals("B") )
							{
								//out.print("before sum111--"+sum1);
                                                               // mBasketB="B";
								if(!mBasketB.equals("B")&& !mChoice.equals("0") && !mChoice.equals(""))
									{
                                                                    mBasketB="B";
                                                                   // out.print("before sum2222--"+sum1);
								sum1=sum1+(Double.parseDouble(mCredit) * mMaxBasketB);
                                                                //out.print("wwwwww"+sum1);
								//mBasketB="B";
									}

									//out.print("sum==="+sum1+"--basket--"+mBasket+"--credit--"+mCredit);
								aa++;
							}
							else if (!mBasket.equals("B") && !mBasket.equals("D") )
							{
								sum1=sum1+Double.parseDouble(mCredit);
								//out.print("elseif"+sum1);
                                                                //System.out.println("ggggggg"+sum1);
							}
							else if(mBasket.equals("D") && !mChoice.equals("0") && !mChoice.equals(""))
							{
...
//									out.print(mChoice+"<br>ddddd:"+sum1);

							}
						//	out.print(sum1+"mMaxBasketB"+mMaxBasketB+"mMaxBasketD"+mMaxBasketD);
...
mTot=mTot+FeeAmt;
				...
				qrys="SELECT 'Y' FROM FEESTRUCTUREINDIVIDUAL WHERE INSTITUTECODE='"+mInst+"' AND FEEHEAD LIKE 'GIEF%' AND  " +
                        "STUDENTID='"+mMemberID+"' AND REGCODE= '"+mImpRegCode+"' AND SEMESTER='"+mSemester+"' ";
			//	out.print(qrys);
...
qry2="Select 'Y' from Studentregistration where studentid='"+mMemberID+"' and Examcode='"+mExamCode+"'" +
        " AND  SEMESTER='"+mSemester+"' AND  SEMESTERTYPE='GIP' ";
//out.print(qry2);
		...
		qry="alter trigger jiit0410.TRG_PoplateCoreSubjects disable";
		int x=db.update(qry);
		...
		qry="alter trigger jiit0410.TRG_PoplateCoreSubjects enable";
		int s=db.update(qry);
...
int srno=0,slno=0;
				...
				<td><font color=white FACE=ARIAL><b>Pre-Reg. Status</b></font></td></tr><%
							String qry12="Select SUBJECTID,subject,nvl(choice,0) abc,subjectrunning,decode(electivecode,'B', 1,null,2,'PDE',3,'DE',4)aa,nvl(electivecode,' ')electivecode,decode(electivecode,null,'Core','PDE','Elective(PDE)','DE','Elective(DE)','B','BackCore',  'DE4', 'Elective(DE4)',                 'DE5', 'Elective(DE5)',                 'DE6', 'Elective(DE6)',                 'HSS', 'Elective(HSS)',subjecttype)subjecttype,semestertype,bb from (SELECT DISTINCT   A.SUBJECTID , NVL (a.subject, ' ')                 || '('                || NVL (a.subjectcode, ' ')                || ')' subject,                NVL (b.choice, 0) choice,                NVL (b.subjectrunning, ' ') subjectrunning,                NVL (b.electivecode, '') electivecode,                NVL (b.subjecttype, '') subjecttype,                NVL (b.semestertype, '') semestertype , '1' bb          FROM subjectmaster a, pr#studentsubjectchoice b          WHERE b.examcode = '"+mExamCode+"'            AND b.institutecode = '"+mInst+"'            AND b.semestertype = 'REG'            AND b.subjecttype = 'C'            AND b.studentid = '"+mMemberID+"'   and a.INSTITUTECODE=b.INSTITUTECODE          AND a.subjectid = b.subjectid  ";
						//	qry12=qry12+" and (a.subjectid) not IN (                                    SELECT DISTINCT subjectid                                               FROM studentresult                                              WHERE institutecode ='"+mInst +"'                                                AND studentid ='"+mMemberID+"' )";
							...
							qry12=qry12+" union all      SELECT DISTINCT  A.SUBJECTID ,  NVL (a.subject, ' ')                || '('                || NVL (a.subjectcode, ' ')                || ')' subject,                NVL (b.choice, 0) choice,                NVL (b.subjectrunning, ' ') subjectrunning,                NVL (b.electivecode, ' ') electivecode,                NVL (b.subjecttype, '') subjecttype,                NVL (b.semestertype, '') semestertype  ,'3' bb         FROM subjectmaster a, pr#studentsubjectchoice b   ,PR#ELECTIVESUBJECTS c        WHERE b.examcode = '"+mExamCode+"'             AND b.institutecode = '"+mInst+"'             AND b.semestertype = 'REG'             AND b.subjecttype = 'E'             AND b.studentid = '"+mMemberID+"'      and a.INSTITUTECODE=b.INSTITUTECODE        AND a.subjectid = b.subjectid  							and b.PROGRAMCODE='"+mProg+"'                      and b.ACADEMICYEAR='"+mAcad+"'                      AND B.SECTIONBRANCH='"+mSect+"' AND                       B.semester="+mSem+" AND                       b.EXAMCODE=c.EXAMCODE AND                       b.ACADEMICYEAR=c.ACADEMICYEAR AND                       b.PROGRAMCODE=c.PROGRAMCODE AND                        b.SECTIONBRANCH=c.SECTIONBRANCH and                          b.SEMESTER=c.SEMESTER and                           C.BASKET IN ('B') and                            b.ELECTIVECODE=c.ELECTIVECODE and                             b.SUBJECTID=c.SUBJECTID         ";

								qry12=qry12+" union all      SELECT DISTINCT  A.SUBJECTID ,  NVL (a.subject, ' ')                || '('                || NVL (a.subjectcode, ' ')                || ')' subject,                NVL (b.choice, 0) choice,                NVL (b.subjectrunning, ' ') subjectrunning,                NVL (b.electivecode, ' ') electivecode,                NVL (b.subjecttype, '') subjecttype,                NVL (b.semestertype, '') semestertype  ,'4' bb         FROM subjectmaster a, pr#studentsubjectchoice b   ,PR#ELECTIVESUBJECTS c        WHERE b.examcode = '"+mExamCode+"'             AND b.institutecode = '"+mInst+"'             AND b.semestertype = 'REG'             AND b.subjecttype = 'E'             AND b.studentid = '"+mMemberID+"'      and a.INSTITUTECODE=b.INSTITUTECODE        AND a.subjectid = b.subjectid  							and b.PROGRAMCODE='"+mProg+"'                      and b.ACADEMICYEAR='"+mAcad+"'                      AND B.SECTIONBRANCH='"+mSect+"' AND                       B.semester="+mSem+" AND                       b.EXAMCODE=c.EXAMCODE AND                       b.ACADEMICYEAR=c.ACADEMICYEAR AND                       b.PROGRAMCODE=c.PROGRAMCODE AND                        b.SECTIONBRANCH=c.SECTIONBRANCH and                          b.SEMESTER=c.SEMESTER and                           C.BASKET IN ('D','E') and                            b.ELECTIVECODE=c.ELECTIVECODE and                             b.SUBJECTID=c.SUBJECTID  ";

							qry12=qry12+" union all SELECT DISTINCT  A.SUBJECTID ,  NVL (a.subject, ' ')                || '('                || NVL (a.subjectcode, ' ')                || ')' subject,                NVL (b.choice, 0) choice,                NVL (b.subjectrunning, ' ') subjectrunning,                'B' electivecode, NVL (b.subjecttype, '')                subjecttype,                NVL (b.semestertype, '') semestertype  ,'2' bb         FROM subjectmaster a, pr#studentsubjectchoice b          WHERE b.examcode = '"+mExamCode+"'            AND b.institutecode = '"+mInst+"'            AND b.semestertype in('RWJ','SAP')            AND b.studentid = '"+mMemberID+"'    and a.INSTITUTECODE=b.INSTITUTECODE         AND a.subjectid = b.subjectid    ";
					//		qry12=qry12+" and (a.subjectid) not IN (                                    SELECT DISTINCT subjectid                                               FROM studentresult                                              WHERE institutecode ='"+mInst +"'                                                AND studentid ='"+mMemberID+"' )";
							qry12=qry12+" )ORDER BY  bb, electivecode,abc ";
						...
						//	out.println(qry12);
							...
							while(rs1.next())
							{
								...
								if(rs1.getString("subjecttype").equals("Elective(HSS)"))
								{
									mSubjTypeDesc="PD-ELECTIVE";

									%><tr bgcolor='lightyellow'><%
								}
								else if(rs1.getString("subjecttype").equals("Elective(DE4)")){

									mSubjTypeDesc="DEPARTMENTAL-ELECTIVE(DE4)  ";

									%><tr bgcolor='LIGHTPINK'><%
								}
								else if(rs1.getString("subjecttype").equals("Elective(DE5)")){
									mSubjTypeDesc="DEPARTMENTAL-ELECTIVE(DE5)  ";

									%><tr bgcolor='LIGHTBLUE'><%
								}
									else if(rs1.getString("subjecttype").equals("Elective(DE6)")){
									mSubjTypeDesc="DEPARTMENTAL-ELECTIVE(DE6)  ";
									%><tr bgcolor='LIGHTGREEN'><%
								}
								else if(rs1.getString("subjecttype").equals("Core")){
						...
						<td align=center><%=srno%></td>
												...
												<td><%=rs1.getString("subject")%>   (<%=rs1.getString("electivecode")%>)</td>
												<td align=center><%=mSubjTypeDesc%></td>
...
String qry123="Select SUBJECTID,subject,nvl(choice,0) abc,subjectrunning,decode(electivecode,'B', 1,null,2,'PDE',3,'DE',4)aa,nvl(electivecode,'N')electivecode," +
        "decode(electivecode,null,'Core','PDE','Elective(PDE)','DE','Elective(DE)','B','BackCore',subjecttype)subjecttype,semestertype from " +
        "(SELECT DISTINCT   A.SUBJECTID , NVL (a.subject, ' ')                 || '('                || NVL (a.subjectcode, ' ')     " +
        "           || ')' subject,                NVL (b.choice, 0) choice,                NVL (b.subjectrunning, ' ') subjectrunning,                NVL (b.electivecode, '') electivecode,                NVL (b.subjecttype, '') subjecttype,                NVL (b.semestertype, '') semestertype           FROM subjectmaster a, pr#studentsubjectchoice b          " +
        "WHERE  nvl(b.IMPROVEMENTSUBJECT,'N')='Y' and  b.examcode = '"+mExamCode+"'            AND b.institutecode = '"+mInst+"'           " +
        " AND b.semestertype = 'GIP'            AND b.subjecttype = 'C'            AND b.studentid = '"+mMemberID+"'" +
        "   and a.INSTITUTECODE=b.INSTITUTECODE          AND a.subjectid = b.subjectid            union   " +
        "    SELECT DISTINCT  A.SUBJECTID ," +
        "  NVL (a.subject, ' ')                || '('                || NVL (a.subjectcode, ' ')                || ')' subject,                " +
        "NVL (b.choice, 0) choice,                NVL (b.subjectrunning, ' ') subjectrunning,             " +
        "   NVL (b.electivecode, '') electivecode,                NVL (b.subjecttype, '') subjecttype,           " +
        "     NVL (b.semestertype, '') semestertype           FROM subjectmaster a, pr#studentsubjectchoice b      " +
        "    WHERE  NVL (b.improvementsubject, 'N') = 'Y' and  b.examcode = '"+mExamCode+"'            " +
        " AND b.institutecode = '"+mInst+"'             AND b.semestertype = 'GIP'             AND b.subjecttype = 'E'        " +
        "     AND b.studentid = '"+mMemberID+"'      and a.INSTITUTECODE=b.INSTITUTECODE        AND a.subjectid = b.subjectid                  ORDER BY subjecttype,electivecode)       order by aa,abc";
							//out.println(qry123);
							...
							while(rs12.next())
							{
								...
								//out.print(qry);
								rsCCP1=db.getRowset(qry);
								if (rsCCP1.next())
								{
									mCCP=rsCCP1.getDouble("COURSECREDITPOINT");
								}
								else
								{
									mCCP=0;
			...
			if(mNo==1)
			{
				...
				<tr><td colspan=6 align=right><%if(sum1!=0.0){%><Font color=green face=verdana size=4><B>Total Course Credit Points Pre-Registered/Allowed Now : <%=sum1%><b></font><%}%></td></tr>
			...
			//	out.print(qry);
					...
					if(rs.next())
						{
						%>
						<tr><td colspan=6 align=right><Font color=green face=verdana size=4><B>Total Earned Credit Till Date : <%=rs.getDouble("totalearnedcredit")%><b></font></td></tr>
						<%
...
//Examcode check is from companyinstitutetagging current semester
	...
	qry=qry+" nvl(A.deactive,'N')='N' and B.BASKET=A.BASKET AND A.EXAMCODE=B.EXAMCODE AND  A.SEMESTER<='"+mSem+"'  and c.COMPANYCODE='"+mLoginComp+"' ";
...
%>
<tr><td colspan=6 align=right><Font color=green face=verdana size=4><B>Credits Registered in <%=mSem-1%> Semester :</b> <b><%=mTotalCrLmtTkn%></b></font></td></tr>



				<td colspan=6 align=left><%if(sum!=0.0){%><Font color=green face=verdana size=3><B>Once You  Freeze, you cannot change your preference / choice !
				<br>
