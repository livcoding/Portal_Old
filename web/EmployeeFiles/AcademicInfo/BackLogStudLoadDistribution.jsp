...
{  //1
	if(!mMemberID.equals("") && !mMemberType.equals("") && !mMemberCode.equals("") && !mMemberName.equals("")){  //2
		...
		if(1==1){
			...
			<tr>
			<TD colspan=0 align=middle><font color="#a52a2a" style="FONT-SIZE: medium; FONT-FAMILY: VERDANA">Back Log Student Load Distribution [Load Distribution]</TD></font>
			</tr>
			</TABLE><br>
			...
			<td><FONT color=black><FONT face=Arial size=2><STRONG>&nbsp;&nbsp;Exam Code&nbsp;&nbsp;</STRONG></FONT></FONT></td>
						...
						%>
							...
							<%
							qry="SELECT DISTINCT a.subjectid subjectid,b.subject || ' (' || b.subjectcode || ')' subject FROM pr#studentsubjectchoice a, subjectmaster b WHERE a.examcode = '"+mExamCode+"' AND a.semestertype IN ('GIP','RWJ', 'SAP') AND NVL (a.deactive, 'N') = 'N' AND NVL (b.deactive, 'N') = 'N' AND a.subjectid = b.subjectid and a.InstituteCode='"+mInst+"' and a.InstituteCode=b.InstituteCode order by subject";
							rs=db.getRowset(qry);
							//out.println(qry);
							while(rs.next()){
					...
					<tr>	
						<TD width="50%" valign="top">
							...
							<tr bgcolor="#ff8c00">
								...
								qry="SELECT a.studentid studentid,ENROLLMENTNO, STUDENTNAME, a.PROGRAMCODE PROGRAMCODE,B.BRANCHCODE BRANCHCODE,a.SEMESTER SEMESTER, b.SUBSECTIONCODE SUBSECTIONCODE,SEMESTERTYPE,nvl(B.SECTIONCODE,'&nbsp;') SECTIONCODE,a.ACADEMICYEAR ACADEMICYEAR,a.TAGGINGFOR TAGGINGFOR,SECTIONBRANCH,a.subsectioncode subsectioncode,a.SUBJECTTYPE SUBJECTTYPE FROM PR#STUDENTSUBJECTCHOICE A, STUDENTMASTER B WHERE A.SUBJECTID ='"+mSubjectid+"' AND A.STUDENTID = B.STUDENTID AND A.EXAMCODE = '"+mExamCode+"' AND A.SEMESTERTYPE IN ('GIP','RWJ', 'SAP') AND A.SUBJECTRUNNING = 'Y' AND NVL (A.DEACTIVE, 'N') = 'N' AND NVL (B.DEACTIVE, 'N') = 'N' and a.InstituteCode='"+mInst+"' and a.InstituteCode=b.InstituteCode order by PROGRAMCODE,BRANCHCODE,semester,a.SUBSECTIONCODE";
								rs=db.getRowset(qry);
								//out.println(qry);
								while(rs.next())	
								{
									...
									if(mLTP.equals("L"))
									{
										checkqry="Select 'Y' from PR#STUDENTSUBJECTCHOICE A WHERE A.SUBJECTID ='"+mSubjectid+"'  AND A.EXAMCODE = '"+mExamCode+"' AND A.SEMESTERTYPE IN ('GIP','RWJ', 'SAP') AND A.SUBJECTRUNNING = 'Y' AND NVL (A.DEACTIVE, 'N') = 'N' and a.InstituteCode='"+mInst+"' AND NVL (lfstid, 'N') = 'N' and  studentid in (select studentid from studentmaster where ENROLLMENTNO='"+rs.getString("ENROLLMENTNO")+"' and nvl(deactive,'N')='N' and institutecode='"+mInst+"') ";
									}else if(mLTP.equals("T"))
									{
										checkqry="Select 'Y' from PR#STUDENTSUBJECTCHOICE A WHERE A.SUBJECTID ='"+mSubjectid+"'  AND A.EXAMCODE = '"+mExamCode+"' AND A.SEMESTERTYPE IN ('GIP','RWJ', 'SAP') AND A.SUBJECTRUNNING = 'Y' AND NVL (A.DEACTIVE, 'N') = 'N' and a.InstituteCode='"+mInst+"' and NVL (tfstid, 'N') = 'N' and  studentid in (select studentid from studentmaster where ENROLLMENTNO='"+rs.getString("ENROLLMENTNO")+"' and nvl(deactive,'N')='N' and institutecode='"+mInst+"') ";
							   


									}else if(mLTP.equals("P"))
									{
										checkqry="Select 'Y' from PR#STUDENTSUBJECTCHOICE A WHERE A.SUBJECTID ='"+mSubjectid+"'  AND A.EXAMCODE = '"+mExamCode+"' AND A.SEMESTERTYPE IN ('GIP','RWJ', 'SAP') AND A.SUBJECTRUNNING = 'Y' AND NVL (A.DEACTIVE, 'N') = 'N' and a.InstituteCode='"+mInst+"' and NVL (pfstid, 'N') = 'N' and  studentid in (select studentid from studentmaster where ENROLLMENTNO='"+rs.getString("ENROLLMENTNO")+"' and nvl(deactive,'N')='N' and institutecode='"+mInst+"') ";
									...
									%>	<%++i;%>
										...
										<TD><B>--</B></TD>
										<%}else{%>
										<TD><B><a href='editBackLogStudLoadDistribution.jsp?mLTP=<%=mLTP%>&examcode=<%=mExamCode%>&subjectid=<%=mSubjectid%>&studentid=<%=rs.getString("studentid")%>' target="_new">Edit</a></B></TD>
										<%}%>
										</TR>
									<%
								}
