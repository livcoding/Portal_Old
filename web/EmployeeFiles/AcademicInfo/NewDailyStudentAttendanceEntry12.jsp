...
<tr><td colspan="2">
                    ...
                    <input id="timepicker1" name="timepicker1" type="text" value="<%=mtimepicker1%>" ALT="Pick a Time!" size=8 maxlength=8>

					<IMG SRC="../../Images/timepicker.gif" BORDER="0" ALT="Pick a Time!" onclick="selectTime1(timepicker1)"  STYLE="cursor:hand" >

					   <label >To</label> 
                    <input id="timepicker2" name="timepicker2" type="text" value="<%=mtimepicker2%>" ALT="Pick a Time!" size=8 maxlength=8>

					<IMG SRC="../../Images/timepicker.gif" BORDER="0" ALT="Pick a Time!" onclick="selectTime2(timepicker2)"  STYLE="cursor:hand" >
...
<table bgcolor=#fce9c5 class="sort-table" width="50%" bottommargin=0 rules=rows topmargin=0 cellspacing=3 cellpadding=0 border=1 align=center>
	<tr>
	
	<td valign=top nowrap>

	<table bgcolor=#fce9c5 class="sort-table"   bottommargin=0 rules=rows topmargin=0 cellspacing=0 cellpadding=0 border=1 align=center>
	<thead>
	<tr bgcolor="#ff8c00">

	<td rowspan=2 Title="Sort on SlNo">
	<font color="White" size=2><b>Sr.<br>No.</b></font>
	</td>
	</tr>
	</thead>
		<tbody>
	...
	<%
		...
		<tr  bgcolor="<%=TRCOLOR1%>">
	<td nowrap  >
		<font size=2 face=arial> <b><%=j%> </font>
		</td>
	</tr>
...
<td nowrap valign=top> 
	...
	%>
		<td rowspan=2 nowrap color="White"  ><font color="White"><b>Attendance Register</b></td>
	</tr>
	<!-- <tr bgcolor="#ff8c00">
...
qry=qry+" and not exists (select 'y' from STUDATTENDANCEBYSPECIALFACULTY ssf Where ";
					...
					{
						qry=qry+" And FSTID not in (Select FSTID from STUDENTATTENDANCE where to_char(ATTENDANCEDATE,'dd-mm-yyyy')='"+mDate+"' and attendancetype IN ( 'E','R') ";
...
//------------merge qry--------------//
				...
				qry=qry+"  union                           SELECT   NVL (a.employeeid, ' ') employeeid, NVL (a.fstid, ' ') fstid, NVL (a.enrollmentno, ' ') enrollmentno, NVL (a.studentname, ' ') studentname,         NVL (a.studentid, ' ') studentid, NVL (a.sectionbranch, ' ') || '('|| a.subsectioncode|| ')' sectionbranch, NVL (b.semester, 1) semester,         NVL (TO_CHAR (b.regconfirmationdate, 'dd-mm-yyyy'), ' ' ) regconfirmationdate, NVL (b.regconfirmation, 'N') regconfirmation, a.sectionbranch secbr,         a.subsectioncode, c.semestertype, c.ltp ltp, NVL (c.slno, -1) slno    FROM v#studentltpdetail a, studentregistration b, subjectwiseslno c   WHERE c.institutecode = a.institutecode      AND NVL (a.studentdeactive, 'N') = 'N'     AND NVL (a.deactive, 'N') = 'N'     AND c.examcode = a.examcode     AND c.studentid = a.studentid     AND c.institutecode = a.institutecode     AND c.examcode = a.examcode     AND c.academicyear = a.academicyear     AND c.programcode = a.programcode     AND c.taggingfor = a.taggingfor     AND c.sectionbranch = a.sectionbranch     AND c.subsectioncode = a.subsectioncode     AND c.subjectid = a.subjectid     AND c.ltp = a.ltp     AND c.institutecode = b.institutecode     AND c.examcode = b.examcode     AND c.regcode = b.regcode     AND c.studentid = b.studentid     AND c.studentid = a.studentid     AND b.institutecode = a.institutecode     AND b.examcode = a.examcode     AND b.academicyear = a.academicyear     AND b.studentid = a.studentid     AND a.academicyear =  decode('"+mAcademicYear+"' ,'ALL',a.academicyear,'"+mAcademicYear+"' )        AND a.institutecode ='"+mInst+"'     And  a.MERGEWITHFSTID in (select x.FSTID from  facultysubjecttagging x where       x.subjectid = '"+mSubject+"'      AND x.ltp IN  ('"+mLTP+"')  AND x.examcode ='"+mExam+"' ";
				if(mSubsection.equals("ALL"))
					{
						qry=qry+" and x.subsectioncode=decode('"+mSubsection+"','ALL',x.subsectioncode,'"+mSubsection+"') ";
					}
					else
					{
						qry=qry+" and x.subsectioncode in ("+mSubsection+")";
					}
...
//	rs2=db.getRowset(qry);
			...
			%>
						...
						%>   
<td nowrap align=center><b><A HREF="StudAttregister.jsp?ViewType=CNF&amp;STUDID=<%=rs1.getString("studentid")%>&amp;FSTID=<%=rs1.getString("Fstid")%>&amp;SUBID=<%=mSubject%>&amp;ATTTYPE=<%=mType%>&amp;EXAMCODE=<%=mExam%>&amp;LTP=<%=mLTP%>&amp;ACAD=<%=mAcademicYear%>&amp;BRANCH=<%=mSection%>&amp;SUBSEC=<%=mSubsection%>" target=_new>Show Attendance</A></b></td>
	...
	//,["false", "Number", "CaseInsensitiveString"]
	%>	
...
//	st.sort(1,true,"CaseInsensitiveString");
//	alert(st,"LLL");
	//st1.sort(2, true, "CaseInsensitiveString");
