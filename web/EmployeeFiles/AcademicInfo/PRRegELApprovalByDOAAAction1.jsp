		  ...
		  //----------------------
			...
			if (mRunStatus.equals("No") && mok.equals("Y"))
				...
				qry=qry+" from PREVENTS E Where E.MEMBERTYPE<>'S' and E.MEMBERID='"+mEID+"' And nvl(E.ELRNNINGFINALIZEDBYHOD,'N')='Y' and nvl(E.LOADDISTRIBUTIONSTATUS,'N')='N' and nvl(E.DEACTIVE,'N')='N') )";
					...
					<%
					qry="select BDT.PROGRAMCODE PROGRAMCODE ,BDT.ACADEMICYEAR ACADEMICYEAR,BDT.TAGGINGFOR TAGGINGFOR, BDT.SECTIONBRANCH SECTIONBRANCH, nvl(A.Subject,A.SubjectID ) SUBJECT ,A.SubjectCode SC, B.SubjectID SID, nvl(B.ElectiveCode, ' ') ElectiveCode, max(B.CHOICE) MaxChoice, nvl(C.SubjectRunning,'N') LastStatus from SUBJECTMASTER A, ";
					...
					qry=qry+" Group By BDT.PROGRAMCODE ,BDT.ACADEMICYEAR,BDT.TAGGINGFOR, BDT.SECTIONBRANCH,B.ElectiveCode,B.SubjectID,A.SubjectCode,nvl(A.Subject,A.SubjectID), nvl(C.SubjectRunning,'N')";					
