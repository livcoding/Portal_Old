			...
			mSubcode=rs.getString("SC");
                if(mLastStatus.equals("Y"))
				{
					...
					<%
							...
							{
					%>
							<td align=center title="Elective Subject Running Deparment List"><a target=_New href="ElectiveSubjRunningDeptListAY.jsp?SUBJID=<%=mSubid%>&amp;SUBJ=<%=mSubcode%>&amp;EXAM=<%=mE%>&amp;INST=<%=mInstitute%>&amp;STYPE=<%=mST%>&amp;SEMTYPE=<%=mSemType%>&amp;ELECODE=<%=mELECTIVECODE%>"><%=rs.getString("SC")%></a></td>
							<%
							}
