...
String mELECTIVECODE="",mSubject="",mCustomRun="",mCustom="";
String mSubjType="";
String mST="";
		...
		if (RsChk.next() && RsChk.getString("SL").equals("Y"))
		  ...
		  //----------------------
			if(request.getParameter("ExamCode")!=null)
				...
				Pre-Elective Subject(s) Running Status by HOD</u></b></font></center>
...
//------------------------
					...
					//out.print(mCustomRun+"dff"+mRunning);
					//out.print("<br>"+mLastStatus);
							if (mLastStatus.equals(mRunning) || !mCustomRun.equals(mRunning))
								
							  {
								   if(mRunning.equals("C"))
								  {
									%>
									<td align=right><font color=Blue>Custom</font></td>
									<%
								  }
								 else if(mRunning.equals("Y"))
								  {
									%>
							...
							
							  {
								...
								if(mST.equals("E"))
								  {
									qry="Update PR#ELECTIVESUBJECTS B set CUSTOMFINALIZED='Y' Where B.INSTITUTECODE='"+mInst+"' AND B.EXAMCODE='"+mExam+"'";
									qry=qry+" And B.SubjectID='"+mSubjID+"' And (b.INSTITUTECODE,b.EXAMCODE) in (select PE.INSTITUTECODE,PE.ExamCode from PREVENTMASTER PE ";
									qry=qry+" Where PE.INSTITUTECODE='"+mInst+"' and PE.EXAMCODE='"+mExam+"' and nvl(PE.PRCOMPLETED,'N')='N' and nvl(PE.PRBROADCAST,'N')='Y' ";
									qry=qry+" and nvl(PE.PRREQUIREDFOR,'N')<>'S' and nvl(PE.DEACTIVE,'N')='N'";
									qry=qry+" And (PE.INSTITUTECODE, PE.PREVENTCODE) in (Select D.INSTITUTECODE, D.PREVENTCODE ";
				...
				if (mfinalized.equals("YES"))
					{
						qry="Update PREVENTS B set ELRNNINGFINALIZEDBYHOD='Y',FINALIZEDBY='"+mDMemberID+"', FINALIZEDDATE=sysdate  Where B.INSTITUTECODE='"+mInst+"'";
						qry=qry+ " And B.MEMBERTYPE<>'S' and B.MEMBERID='"+mDMemberID+"' and nvl(B.LOADDISTRIBUTIONSTATUS,'N')='N'";
