...
ResultSet rs=null,rs1=null,Rsd1=null,rsCCP=null;
double sumcrt=0,mCCP=0;
String mysum="";
...
{  //2
	...
	String mIPAddress =session.getAttribute("IPADD").toString().trim();
	String mRole="STUD";
	qry=" SELECT nvl(SUBSECTIONCODE,' ')SUBSECTIONCODE FROM STUDENTMASTER WHERE STUDENTID='"+mChkMemID+"' AND INSTITUTECODE='"+mINSTITUTECODE+"' ";
...
<BR>
<center><b>PRIORITY OF SUBJECT CHOICES SUBMITTED FOR  EXAMCODE : <%=mExamDesc%></b></center>
...
<tr bgcolor="#ff8c00">
  ...
  <td><b><font color=white>Credit</font></b></td>
    <td><b><font color=white>Freeze Date</font></b></td>
  </tr>
</thead>
	<tbody>
...
qry=qry+"nvl(B.SUBJECTTYPE,'')SUBJECTTYPE,nvl(B.SEMESTERTYPE,'')SEMESTERTYPE,nvl(C.COURSECREDITPOINT,0) COURSECREDITPOINT, TO_CHAR(B.ENTRYDATE,'DD-MM-YYYY hh:mm')ENTRYDATE  from SUBJECTMASTER A,PR#STUDENTSUBJECTCHOICE B,PROGRAMSCHEME C";
...
qry="select DISTINCT nvl(A.SUBJECT,' ')||'('||NVL(A.SUBJECTCODE,' ')||')' SUBJECT,decode(c.ELECTIVECODE,'PDE','1','DE','2',c.ELECTIVECODE) type, ";
...
qry=qry+"nvl(B.SUBJECTTYPE,'')SUBJECTTYPE,nvl(B.SEMESTERTYPE,'')SEMESTERTYPE,nvl(C.COURSECREDITPOINT,0) COURSECREDITPOINT, TO_CHAR(B.ENTRYDATE,'DD-MM-YYYY hh:mm')ENTRYDATE   from SUBJECTMASTER A,PR#STUDENTSUBJECTCHOICE B, PR#ELECTIVESUBJECTS C";
...
qry=qry+"nvl(B.CHOICE,0) CHOICE,nvl(B.SUBJECTRUNNING,' ')SUBJECTRUNNING,decode(B.subjecttype,'C','Core','E','Elective')ELECTIVECODE, ";
qry=qry+"nvl(B.SUBJECTTYPE,'')SUBJECTTYPE,nvl(B.SEMESTERTYPE,'')SEMESTERTYPE  ,TO_CHAR(B.ENTRYDATE,'DD-MM-YYYY hh:mm')ENTRYDATE   from SUBJECTMASTER A,PR#STUDENTSUBJECTCHOICE B";
