...
String qryBatchChng="";
ResultSet rs11=null;
String BatchEMPid="";String FSTid="";
       String mColor1="";
       ResultSet  rrs=null;
String mName11="";
String updtqry="";
...
String mDTfrom="";
String mDTupto="";
int Ctr=0, mDiffInDate=0, mDataComboSubSec=0,mAbsentCount=0;
...
<script>
    ...
    $(function() {
                    $('#timepicker1').timepicker({interval: '60'
});
                });
...
qry1="select 'ALL' SubSection   from dual union all (Select DISTINCT SUBSECTIONCODE SubSection from  V#STUDENTLTPDETAIL where  NVL (deactive, 'N') = 'N' AND NVL (STUDENTDEACTIVE, 'N') = 'N' AND ";
	qry1=qry1+" facultytype=decode('"+mDMemberType+"','E','I','G','I','E') and employeeid='"+mDMemberID+"' and ";
	qry1=qry1+" examcode not in (select examcode from exammaster where nvl(LOCKEXAM,'N')='Y' ";
...
qry1=qry1+"  	Group By SUBSECTIONCODE";
	...
	qry1=qry1+" NVL (deactive, 'N') = 'N' AND NVL (STUDENTDEACTIVE, 'N') = 'N' AND fstid in (select fstid from MULTIFACULTYSUBJECTTAGGING where " +
            " institutecode='"+mInst+"' and facultytype=decode('"+mDMemberType+"','E','I','G','I','E') " +
...
//	mtime2=request.getParameter("timepicker2").toString().trim().toUpperCase()+" "+"pm";
		...
		mtime1=request.getParameter("timepicker1").toString().trim().toUpperCase();
			mtime2=request.getParameter("timepicker2").toString().trim().toUpperCase();
                        String tm = mtime1.contains("AM") ? mtime1.split("AM")[0] : mtime1.split("PM")[0];
                        String amorpm = mtime1.contains("AM") ? "AM" : "PM";
                        ...
                        mtime2 = Integer.parseInt(mtime2.split(":")[0]) >= 12 && amorpm.equalsIgnoreCase("AM") ? ((Integer.parseInt(mtime2.split(":")[0]) % 12 == 0 ? 12 : (Integer.parseInt(mtime2.split(":")[0]) % 12))+":"+mtime2.split(":")[1]+"PM") : (mtime2+":"+amorpm);
                        String tm2 = mtime2.contains(":AM") ? mtime2.split(":AM")[0] : mtime2.split(":PM")[0];
                      mtime1=tm;
...
int j=0;
			...
			qry="SELECT  NVL (employeeid, ' ') employeeid, NVL (fstid, ' ') fstid,                NVL (enrollmentno, ' ') enrollmentno,                NVL (studentname, ' ') studentname,                NVL (studentid, ' ') studentid,                NVL (NVL (sectionbranch, ' ') || '(' || subsectioncode                     || ')',                     ' '                    ) sectionbranch,                NVL (semester, 1) semester,                NVL (regconfirmationdate,                ' '                    ) regconfirmationdate,                NVL (regconfirmation, 'N') regconfirmation,                NVL (sectionbranch, ' ') secbr,                NVL (subsectioncode, ' ') subsectioncode,                NVL (semestertype, ' ') semestertype, NVL (ltp, ' ') ltp FROM                (";
				qry=qry+"  select  nvl(A.EMPLOYEEID,' ')employeeid,nvl(A.FSTID,' ')fstid,nvl(A.enrollmentno,' ')enrollmentno,nvl(A.studentname,' ')studentname, NVL(A.studentid,' ')studentid,";
...
qry=qry+" and (A.EMPLOYEEID in (select employeeid from facultysubjecttagging where employeeid='"+mDMemberID+"'" +
        " OR (a.fstid in (select fstid from MULTIFACULTYSUBJECTTAGGING where institutecode='"+mInst+"' and facultytype=decode('"+mDMemberType+"','E','I','E') and employeeid='"+mDMemberID+"' and fstid in (select fstid from facultysubjecttagging where examcode='"+mExam+"' and subjectid='"+mSubject+"' and LTP in ('"+mLTP+"'))))";
		...
		//		out.print(qry);
			...
			//if(1==1)
			{
				...
				if(dt>=0)
				{
					...
					qry="SELECT  NVL (employeeid, ' ') employeeid, NVL (fstid, ' ') fstid,                NVL (enrollmentno, ' ') enrollmentno,                NVL (studentname, ' ') studentname,                NVL (studentid, ' ') studentid,                NVL (NVL (sectionbranch, ' ') || '(' || subsectioncode                     || ')',                     ' '                    ) sectionbranch,                NVL (semester, 1) semester,                NVL (regconfirmationdate,                ' '                    ) regconfirmationdate,                NVL (regconfirmation, 'N') regconfirmation,                NVL (sectionbranch, ' ') secbr,                NVL (subsectioncode, ' ') subsectioncode,                NVL (semestertype, ' ') semestertype, NVL (ltp, ' ') ltp FROM                (";
				qry=qry+" select  nvl(A.EMPLOYEEID,' ')employeeid,nvl(A.FSTID,' ')fstid, DECODE (a.enrollmentno,NULL,A.RANKNO,A.ENROLLMENTNO )enrollmentno,nvl(A.studentname,' ')studentname, NVL(A.studentid,' ')studentid,";
					qry=qry+" NVL(A.SECTIONBRANCH,' ')|| '('||A.SUBSECTIONCODE||')' sectionbranch, nvl(B.SEMESTER,1) SEMESTER, nvl(to_char(B.REGCONFIRMATIONDATE,'dd-mm-yyyy'),' ') REGCONFIRMATIONDATE ,nvl(B.REGCONFIRMATION,'N')REGCONFIRMATION, ";
					qry=qry+" A.SECTIONBRANCH SECBR, A.SUBSECTIONCODE, A.SemesterType,A.LTP LTP  ";
					qry=qry+" from V#STUDENTLTPDETAIL A,STUDENTREGISTRATION B  " ;
					//qry=qry+" where C.COMPANYCODE=A.COMPANYCODE ";
...
<%
qry="	select count(*)aa from v#STUDENTATTENDANCE a where a.INSTITUTECODE='"+mInst+"' AND A.EXAMCODE='"+mExam+"' " +
        "AND A.studentid='"+rs1.getString("studentid")+"' and nvl(a.PRESENT,'N')='N' and a.SUBJECTID='"+mSubject+"'";
//out.print(qry);
rs=db.getRowset(qry);
if(rs.next())
{
