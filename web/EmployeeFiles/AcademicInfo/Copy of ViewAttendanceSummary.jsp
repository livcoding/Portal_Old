...
<%
                    try {
                        if (mSrcType.equals("I")) {
                            ...
                            qry = qry + " from facultysubjecttagging A,SUBJECTMASTER B";
                            qry = qry + " where a.InstituteCode='"+mInst+"' and a.InstituteCode=b.InstituteCode AND ((a.employeeid = '" + mDMemberID + "')          OR  a.fstid in (select fstid from MULTIFACULTYSUBJECTTAGGING where companycode='" + mComp + "' and institutecode='" + mInst + "' and facultytype=decode('" + mDMemberType + "','E','I','E') and employeeid='" + mDMemberID + "') )";
                            qry = qry + " and A.fstid not in (select fstid from STUDATTENDANCEBYSPECIALFACULTY where trunc(sysdate)=trunc(attendancedate) And InstituteCode='" + mInst + "' and ";
                            qry = qry + " Nvl(deactive,'N')='N' and facultyid<>'" + mDMemberID + "') AND A.SUBJECTID=B.SUBJECTID";
                            qry = qry + " union ";
                            qry = qry + " Select nvl(A.subjectid,' ') subjectid, nvl(B.subjectcode,' ') subjectcode,A.examcode,nvl(B.subject,' ')||' ('||B.subjectcode||') ' subject ";
...
%>
                    ...
                    if (mSrcType.equals("I")) {
                        ...
                        qry = qry + " Select nvl(A.subjectid,' ') subjectid, nvl(B.subjectcode,' ') subjectcode,A.examcode,nvl(B.subject,' ')||' ('||B.subjectcode||') ' subject ";
                        qry = qry + " from facultysubjecttagging A, SUBJECTMASTER B";
                        qry = qry + " where a.InstituteCode='"+mInst+"' and a.InstituteCode=b.InstituteCode AND ((a.employeeid = '" + mDMemberID + "')          OR  a.fstid in (select fstid from MULTIFACULTYSUBJECTTAGGING where companycode='" + mComp + "' and institutecode='" + mInst + "' AND  employeeid = '" + mDMemberID + "' and facultytype=decode('" + mDMemberType + "','E','I','E')) ) ";
                        qry = qry + " and A.fstid not in (select fstid from STUDATTENDANCEBYSPECIALFACULTY where trunc(sysdate)=trunc(attendancedate) and ";
                        qry = qry + " nvl(deactive,'N')='N' and facultyid<>'" + mDMemberID + "' And InstituteCode='" + mInst + "') AND A.SUBJECTID=B.SUBJECTID and A.EXAMCODE='" + qryexam + "' AND nvl(a.PROJECTSUBJECT,'N')='N'";
                        qry = qry + " union ";
                        qry = qry + " Select  nvl(A.subjectid,' ') subjectid, nvl(B.subjectcode,' ') subjectcode,A.examcode,nvl(B.subject,' ')||' ('||B.subjectcode||') ' subject ";
...
//------------------Start of 'L' Percentage--------------------
                               ...
                               qry1 = "SELECT SUM (Tcount)Tcount FROM (select count(CLASSTIMEFROM)Tcount from V#STUDENTATTENDANCE where SubjectID='" + mSubject + "' and LTP='L' ";
			   		 ...
			   		 if(rs1.getLong("Semester")==1 &&   !mREGCONFIRMATIONDATE.equals(" "))
						{
						   qry1 = qry1 +" and trunc(ATTENDANCEDATE)>=TO_Date('"+mREGCONFIRMATIONDATE+"','dd-mm-yy')";
                               ...
                               qry1 = qry1 + "  UNION ALL select  COUNT (classtimefrom) Tcount from STUDENTPREVATTENDENCE where  subjectid = '"+mSubject+"'     AND ltp ='L'     AND examcode = '"+mExam+"'  AND studentid = '"+rs1.getString("studentid")+"' ";
					 if(rs1.getLong("Semester")==1 && !mREGCONFIRMATIONDATE.equals(" "))
					      {
						   qry1 = qry1 + " and trunc(ATTENDANCEDATE)>=TO_Date('"+mREGCONFIRMATIONDATE+"','dd-mm-yy')";
					      }
					...
					//out.print(rs1.getLong("Semester")+" ==="+qry1);
					 ...
					 else
						{
                               ...
                               qry1 = qry1 + " and SECTIONBRANCH=decode('" + rs1.getString("SECBR") + "','ALL',SECTIONBRANCH,'" + rs1.getString("SECBR") + "') and SUBSECTIONCODE=decode('" + rs1.getString("subsectioncode") + "','ALL',SUBSECTIONCODE,'" + rs1.getString("subsectioncode") + "')";
                              qry1=qry1+"  UNION ALL  select  COUNT (classtimefrom) Pcount from STUDENTPREVATTENDENCE where  subjectid = '"+mSubject+"'     AND ltp ='L'  AND NVL (present, 'N') = 'Y'   AND examcode = '"+mExam+"'  AND studentid = '"+ rs1.getString("studentid") +"' ";
							  if(rs1.getLong("Semester")==1 && !mREGCONFIRMATIONDATE.equals(" "))
								{
								qry1=qry1+" and trunc(ATTENDANCEDATE)>=TO_Date('"+mREGCONFIRMATIONDATE+"','dd-mm-yy')";
								}
...
//------------------End of 'L' Percentage--------------------
%>
                        <td ALIGN=left>&nbsp;<a Title="View Date wise Student 'L' Attendance" target=_New href="DailyStudentAttendanceReportDet.jsp?RightsID=<%=mRightsID%>&amp;EXAM=<%=mExam%>&amp;SID=<%=rs1.getString("studentid")%>&amp;SC=<%=mSubject%>&amp;LTP=L&amp;SEC=<%=rs1.getString("SECBR")%>&amp;SUBSEC=<%=rs1.getString("subsectioncode")%>"><font color=""><b><%=QryPercDecimal%></b> %</font></a></td>
                        <%
                           }
                           //else
                           //{
...
//------------------Start of 'T' Percentage--------------------
                               ...
                               mTotTCls = 0;
                               mTotTPrs = 0;
                               qry1 = "SELECT SUM (Tcount)Tcount FROM (select count(CLASSTIMEFROM)Tcount from V#STUDENTATTENDANCE where SubjectID='" + mSubject + "'  and  LTP='T'";
                               ...
                               qry1 = qry1 + " and EXAMCODE='" + mExam + "' and INSTITUTECODE='" + mInst + "' and nvl(DEACTIVE,'N')='N'";
								...
								qry1=qry1+" and trunc(ATTENDANCEDATE)>=TO_Date('"+mREGCONFIRMATIONDATE+"','dd-mm-yy')";
			                               ...
			                               qry1 = qry1 + " and STUDENTID='" + rs1.getString("studentid") + "'";
                  			             qry1 = qry1 + " and SECTIONBRANCH=decode('" + rs1.getString("SECBR") + "','ALL',SECTIONBRANCH,'" + rs1.getString("SECBR") + "') and SUBSECTIONCODE=decode('" + rs1.getString("subsectioncode") + "','ALL',SUBSECTIONCODE,'" + rs1.getString("subsectioncode") + "')";
                                			qry1=qry1+"  UNION ALL select  COUNT (classtimefrom) Tcount from STUDENTPREVATTENDENCE where  subjectid = '"+mSubject+"'   AND ltp ='T'     AND examcode = '"+mExam+"'  AND studentid = '"+rs1.getString("studentid")+"'";
								if(rs1.getLong("Semester")==1 && !mREGCONFIRMATIONDATE.equals(" "))
									{
								qry1=qry1+" and trunc(ATTENDANCEDATE)>=TO_Date('"+mREGCONFIRMATIONDATE+"','dd-mm-yy')";
									}
                               ...
                               qry1 = qry1 + " and SECTIONBRANCH=decode('" + rs1.getString("SECBR") + "','ALL',SECTIONBRANCH,'" + rs1.getString("SECBR") + "') and SUBSECTIONCODE=decode('" + rs1.getString("subsectioncode") + "','ALL',SUBSECTIONCODE,'" + rs1.getString("subsectioncode") + "')";
                              qry1=qry1+"   UNION ALL select  COUNT (classtimefrom) Pcount from STUDENTPREVATTENDENCE where  subjectid = '"+mSubject+"'      AND ltp ='T'  AND NVL (present, 'N') = 'Y'   AND examcode = '"+mExam+"'  AND studentid = '"+ rs1.getString("studentid") +"' ";
							  if(rs1.getLong("Semester")==1 && !mREGCONFIRMATIONDATE.equals(" "))
								{
								qry1=qry1+" and trunc(ATTENDANCEDATE)>=TO_Date('"+mREGCONFIRMATIONDATE+"','dd-mm-yy')";
								}
...
//------------------End of 'T' Percentage--------------------
%>
                        <td ALIGN=left>&nbsp;<a Title="View Date wise Student 'T' Attendance" target=_New href="DailyStudentAttendanceReportDet.jsp?RightsID=<%=mRightsID%>&amp;EXAM=<%=mExam%>&amp;SID=<%=rs1.getString("studentid")%>&amp;SC=<%=mSubject%>&amp;LTP=T&amp;SEC=<%=rs1.getString("SECBR")%>&amp;SUBSEC=<%=rs1.getString("subsectioncode")%>"><font color=""><b><%=QryPercDecimal%></b> %</font></a></td>
                        <%
                           }
                           //else
                           //{
%>
                        ...
                        %>
                        <td ALIGN=CENTER>&nbsp;<a Title="View Date wise Student 'P' Attendance" target=_New href="DailyStudentAttendanceReportDet.jsp?RightsID=<%=mRightsID%>&amp;EXAM=<%=mExam%>&amp;SID=<%=rs1.getString("studentid")%>&amp;SC=<%=mSubject%>&amp;LTP=LT&amp;SEC=<%=rs1.getString("SECBR")%>&amp;SUBSEC=<%=rs1.getString("subsectioncode")%>"><font color=""><b><%=QryPracPercDecimal%></b> %</font></a></td>
                        <%	
                           }
                           //else
                           //{
...
//------------------Start of 'P' Percentage--------------------
                               QryTotCls = 0;
                               QryTotPrs = 0;
                               qry1 = "SELECT SUM (Tcount)Tcount FROM (select count(CLASSTIMEFROM)Tcount from V#STUDENTATTENDANCE where SubjectID='" + mSubject + "' and LTP='P' ";
					 ...
					 qry1 = qry1 + "  UNION ALL select  COUNT (classtimefrom) Tcount from STUDENTPREVATTENDENCE where  subjectid = '"+mSubject+"'      AND ltp ='P'     AND examcode = '"+mExam+"'  AND studentid = '"+rs1.getString("studentid")+"'";
					 if(rs1.getLong("Semester")==1 && !mREGCONFIRMATIONDATE.equals(" "))
						{
							qry1=qry1+" and trunc(ATTENDANCEDATE)>=TO_Date('"+mREGCONFIRMATIONDATE+"','dd-mm-yy')";
						}
					 qry1=qry1+" and   NVL (deactive, 'N') = 'N')";
                               ...
                               qry1 = qry1 + " and SECTIONBRANCH=decode('" + rs1.getString("SECBR") + "','ALL',SECTIONBRANCH,'" + rs1.getString("SECBR") + "') and SUBSECTIONCODE=decode('" + rs1.getString("subsectioncode") + "','ALL',SUBSECTIONCODE,'" + rs1.getString("subsectioncode") + "')";
								 qry1=qry1+"   UNION ALL select  COUNT (classtimefrom) Pcount from STUDENTPREVATTENDENCE where  subjectid = '"+mSubject+"'    AND ltp ='P'  AND NVL (present, 'N') = 'Y'   AND examcode = '"+mExam+"'  AND studentid = '"+ rs1.getString("studentid") +"' ";
								 if(rs1.getLong("Semester")==1 && !mREGCONFIRMATIONDATE.equals(" "))
								{
								qry1=qry1+" and trunc(ATTENDANCEDATE)>=TO_Date('"+mREGCONFIRMATIONDATE+"','dd-mm-yy')";
								}
...
//------------------End of 'P' Percentage--------------------
%>
                        <td ALIGN=left>&nbsp;<a Title="View Date wise Student 'P' Attendance" target=_New href="DailyStudentAttendanceReportDet.jsp?RightsID=<%=mRightsID%>&amp;EXAM=<%=mExam%>&amp;SID=<%=rs1.getString("studentid")%>&amp;SC=<%=mSubject%>&amp;LTP=P&amp;SEC=<%=rs1.getString("SECBR")%>&amp;SUBSEC=<%=rs1.getString("subsectioncode")%>"><font color=""><b><%=QryPercDecimal%></b> %</font></a></td>
                        <%
                           }
                       //else
                       //{
