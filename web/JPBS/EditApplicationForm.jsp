...
<tr><td ALIGN=middle><FONT 
      face=Verdana><STRONG><FONT size=4>MBA APPLICATION FORM - 2010 </font><br><FONT size=2>(Do not submit without copy of CAT/XAT/MAT/GMAT score card)</FONT></STRONG>
      <BR>
</FONT>                         
</td></tr>
</table>
...
<tr><td>
<FONT face=Verdana><b>&nbsp;Applicant Name:</b></FONT>
<%
	qry="select distinct APPLICATIONSLNO,STUDENTNAME FROM C#MBAAPPLICATIONMASTER where nvl(DEACTIVE,'N')='N' order by STUDENTNAME";
	 ...
	 if (request.getParameter("x")==null) 
		 ...
		 while(rs.next())
			{
					...
					%>
					<option value='<%=rs.getString("APPLICATIONSLNO")%>'><%=rs.getString("STUDENTNAME")%>&nbsp;---><%=rs.getString("APPLICATIONSLNO")%></option>
				<%
		...
		else
		{
			while(rs.next())
			{
				...
				{
					%>
					<OPTION selected Value ='<%=mAppSlno%>'><%=rs.getString("STUDENTNAME")%>&nbsp;---><%=mAppSlno%></option>
					<%
				}
				else
				{
					%>
					<OPTION Value ='<%=mAppSlno%>'><%=rs.getString("STUDENTNAME")%>&nbsp;---><%=mAppSlno%></option>
					<%
				}
			}
		}
...
<FONT face=Verdana><b>&nbsp;Applicant No. :</b></FONT>
<%
	qry1="select distinct STUDENTNAME,APPLICATIONSLNO FROM C#MBAAPPLICATIONMASTER where nvl(DEACTIVE,'N')='N' order by APPLICATIONSLNO";
...
else
{
	...
	qry2="SELECT NVL(ADDRESS1,' ')ADDRESS1, NVL(ADDRESS2,' ')ADDRESS2,NVL(CITY,' ')CITY, NVL(STATE,' ')STATE,NVL(PIN,' ')PIN,NVL(COUNTRY,' ')COUNTRY, NVL(PHONE,' ')PHONE,NVL(EMAILID,' ')EMAILID FROM C#MBAAPPLICANTADD where  APPLICATIONSLNO ='"+mAppSlno+"' ";
	//out.print(qry2);
...
qry3="SELECT  NVL(CHEQUEDDNO,' ')CHEQUEDDNO, nvl(to_char(CHEQUEDDDATE,'dd-mm-yyyy'),' ')CHEQUEDDDATE, CHQDDTYPE, NVL(AMOUNT,0)AMOUNT, NVL(BANKNAME,' ')BANKNAME FROM C#MBAFEE where  APPLICATIONSLNO ='"+mAppSlno+"' ";
//out.print(qry3);
...
if(rs3.next())
	...
	qry5="SELECT  TESTCODE, PERCENTAGE,COMPOSITESCORE,CHECKSCORE,CALL1,CALL2,CALL3,to_char(RESULTVALIDUPTO,'dd-mm-yyyy')RESULTVALIDUPTO FROM C#MBATESTSCORE where  APPLICATIONSLNO ='"+mAppSlno+"'  ";
//out.println(qry5);
...
<FONT face=Verdana><b>&nbsp;Application No:</b></FONT><INPUT  Name="AppNo" ID="AppNo" value="<%=mAppNo%>" size=7 maxlength=8 tabIndex=1 LANGUAGE=javascript  onkeypress="return isNumber(event)"	><FONT 
...
<%
qry4="SELECT  nvl(QUALIFICATIONCODE,' ')QUALIFICATIONCODE,nvl(QUALIFICATIONDESC,' ')QUALIFICATIONDESC,NVL(BOARD,' ')BOARD, NVL(PASSINGYEAR,'')PASSINGYEAR, NVL(STREAM,' ')STREAM,nvl(percentage,0) percentage, nvl(DOCUMENT,' ')DOCUMENT FROM C#QUALIFICATION where  APPLICATIONSLNO ='"+mAppSlno+"'  ";
...
<td colspan=3 align=left><FONT face=Verdana>9. Current Qualification
 ...
 <TABLE BORDER=1 ALIGN=center  cellspacing=2 cellpadding=2>
			<tr>
			<td>&nbsp;</td>
			<td><font face='Verdana' size=2 ><b>Year of Completion/Awaited</b></td>
...
<TD><INPUT TYPE="text" NAME="CATDATE" id="CATDATE"   value="<%=mCATDATE%>"  onChange="return ValidCATDATE(CATDATE.value) "  onCLICK="return ValidCATDATE(CATDATE.value) "    tabIndex=42  SIZE=8 MAXLENGTH=10><font color=green face=arialblack font size=2><b> (DD-MM-YYYY)&nbsp;</b></font>
<!-- <a href="javascript:NewCal('CATDATE','ddmmyyyy')"><img src="../images/cal.gif" width="16" height="16" border="0" alt="Pick a Date"></a> -->
			...
			<TR>
				...
				<TD><INPUT TYPE="text" NAME="MATDATE"  id="MATDATE"  value="<%=mMATDATE%>"  onClick="return ValidMATDATE(MATDATE.value) "    onChange="return ValidMATDATE(MATDATE.value) "  tabIndex=44  SIZE=8 MAXLENGTH=10><font color=green face=arialblack font size=2><b> (DD-MM-YYYY)&nbsp;</b></font>
<!-- <a href="javascript:NewCal('MATDATE','ddmmyyyy')"><img src="../images/cal.gif" width="16" height="16" border="0" alt="Pick a Date"></a> -->
			...
			<INPUT Type="hidden" ID="GMAT" Name="GMAT" Value="GMAT" >
				...
				<TD><INPUT TYPE="text" NAME="GMATDATE"  id="GMATDATE"  value="<%=mGMATDATE%>"  onClick="return ValidGMATDATE(GMATDATE.value) " onChange="return ValidGMATDATE(GMATDATE.value) "    tabIndex=46  SIZE=8 MAXLENGTH=10><font color=green face=arialblack font size=2><b> (DD-MM-YYYY)&nbsp;</b></font>
...
<%
	...
	{
	%>
<INPUT Type="checkbox" ID="Doc10" Name="Doc10" Value="Y" checked tabIndex=47 ><font color='red'>*</font>
	<%
	}
	else
	{
	%>
		<INPUT Type="checkbox" ID="Doc10" Name="Doc10" Value="Y" tabIndex=47 ><font color='red'>*</font>
...
&nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp; &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;
	...
	{
	%>
	<INPUT Type="checkbox" ID="Doc12" Name="Doc12" Value="Y" checked tabIndex=48><font color='red'>*</font>
	...
	{
	%>
	<INPUT Type="checkbox" ID="Doc12" Name="Doc12" Value="Y" tabIndex=48><font color='red'>*</font>
...
&nbsp; &nbsp; &nbsp; &nbsp; &nbsp;&nbsp; &nbsp;
	...
	{
	%>
	<INPUT Type="checkbox" ID="DocGrade" Name="DocGrade" checked Value="Y" tabIndex=49><font color='red'>*</font>
	...
	{
	%>
	<INPUT Type="checkbox" ID="DocGrade" Name="DocGrade" Value="Y" tabIndex=49><font color='red'>*</font>
	<%
	}
	%>
...
&nbsp; &nbsp;&nbsp; &nbsp;
	...
	{
	%>
	<INPUT Type="checkbox" ID="DocOther" Name="DocOther" checked Value="Y" tabIndex=50><font color='red'>*</font>
	...
	{
	%>
	<INPUT Type="checkbox" ID="DocOther" Name="DocOther" Value="Y" tabIndex=50><font color='red'>*</font>
	<%
	}
	%>	 
	 <br>
...
{
%>
<INPUT Type="Checkbox" ID="DocScore" Name="DocScore" Value="Y" checked tabIndex=51><font color='red'>*</font>
<%
}
else
	{
	%>
<INPUT Type="Checkbox" ID="DocScore" Name="DocScore" Value="Y"  tabIndex=51><font color='red'>*</font>
...
<%
	...
	%>	
12. Registration fee of Rs. 1,200/- by a Demand Draft favoring Jaypee Business
